 ...
 *     Requires at least: Optional. Specify the minimum required GeChiUI version.
...
function get_plugin_data( $plugin_file, $markup = true, $translate = true ) {
	...
	if ( ! $plugin_data['Network'] && $plugin_data['_sitewide'] ) {
		...
		_deprecated_argument( __FUNCTION__, '3.0.0', sprintf( __( '%1$s插件头已被废弃，请改用%2$s。' ), '<code>Site Wide Only: true</code>', '<code>Network: true</code>' ) );
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a GC_PLUGIN_DIR relative path.
...
function get_plugin_files( $plugin ) {
	$plugin_file = GC_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && GC_PLUGIN_DIR !== $dir ) {
 ...
 * (gc-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_mu_plugins() {
	...
	// Files in gc-content/mu-plugins directory.
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( GCMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( GCMU_PLUGIN_DIR . "/$plugin_file", false, false );

		if ( empty( $plugin_data['Name'] ) ) {
			$plugin_data['Name'] = $plugin_file;
		}

		$gc_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $gc_plugins['index.php'] ) && filesize( GCMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
		unset( $gc_plugins['index.php'] );
	}
...
function get_dropins() {
	...
	// Files in gc-content directory.
	$plugins_dir = @opendir( GC_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( GC_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( GC_CONTENT_DIR . "/$plugin_file", false, false );
...
/**
 * Returns drop-ins that GeChiUI uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( '高级缓存插件。' ), 'GC_CACHE' ),  // GC_CACHE
		'db.php'                  => array( __( '自定义数据库类。' ), true ),          // Auto on load.
		'db-error.php'            => array( __( '自定义数据库错误消息。' ), true ),  // Auto on error.
		'install.php'             => array( __( '自定义安装脚本。' ), true ),     // Auto on installation.
		'maintenance.php'         => array( __( '自定义维护消息。' ), true ),     // Auto on maintenance.
		'object-cache.php'        => array( __( '外部对象缓存。' ), true ),          // Auto on load.
		'php-error.php'           => array( __( '自定义PHP错误信息。' ), true ),       // Auto on error.
		'fatal-error-handler.php' => array( __( '自定义PHP致命错误处理器。' ), true ), // Auto on error.
	);
 ...
 * @return null|GC_Error Null on success, GC_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new GC_Error( 'unexpected_output', __( '此插件产生了一个异常输出。' ), $output );
		}
 ...
 * @return bool|GC_Error True when finished or GC_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new GC_Error( 'plugins_invalid', __( '其中有一个插件无效。' ), $errors );
	}
 ...
 * @return bool|null|GC_Error True on success, false if `$plugins` is empty, `GC_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = gc_nonce_url( 'plugins.php?action=delete-selected&verify-delete=1&' . implode( '&', $checked ), 'bulk-plugins' );
	...
	$plugin_translations = gc_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			...
			$deleted = $gc_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$gc_filesystem->delete( GC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$gc_filesystem->delete( GC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );

				$json_translation_files = glob( GC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '-*.json' );
...
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new GC_Error( 'plugin_invalid', __( '插件路径无效。' ) );
	}
	if ( ! file_exists( GC_PLUGIN_DIR . '/' . $plugin ) ) {
		return new GC_Error( 'plugin_not_found', __( '插件文件不存在。' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new GC_Error( 'no_plugin_header', __( '该插件没有有效的标题。' ) );
	}
	return 0;
}

/**
 * Validates the plugin requirements for GeChiUI version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( GC_PLUGIN_DIR . '/' . $plugin );

	$requirements = array(
		'requires'     => ! empty( $plugin_headers['RequiresGC'] ) ? $plugin_headers['RequiresGC'] : '',
	...
	if ( ! $compatible_gc && ! $compatible_php ) {
		...
		return new GC_Error(
			'plugin_gc_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current GeChiUI version, 2: Plugin name, 3: Required GeChiUI version. */
				_x( '<strong>错误：</strong>当前GeChiUI版本（%1$s）未能达到%2$s的最低版本要求。此插件需要GeChiUI %3$s。', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( GC_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'GC_UNINSTALL_PLUGIN', $file );

		gc_register_plugin_realpath( GC_PLUGIN_DIR . '/' . $file );
		include_once GC_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		gc_register_plugin_realpath( GC_PLUGIN_DIR . '/' . $file );
		include_once GC_PLUGIN_DIR . '/' . $file;
 ...
 * @global bool  $_gc_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
...
function get_admin_page_parent( $parent_page = '' ) {
	...
	if ( 'admin.php' === $pagenow && isset( $plugin_page ) ) {
		foreach ( (array) $menu as $parent_menu ) {
			if ( $parent_menu[2] === $plugin_page ) {
				...
				if ( isset( $_gc_real_parent_file[ $parent_file ] ) ) {
					$parent_file = $_gc_real_parent_file[ $parent_file ];
				}

				return $parent_file;
			}
		}
		if ( isset( $_gc_menu_nopriv[ $plugin_page ] ) ) {
			$parent_file = $plugin_page;
	...
	if ( isset( $plugin_page ) && isset( $_gc_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
		$parent_file = $pagenow;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_gc_menu_nopriv, $_gc_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( ! isset( $plugin_page ) && isset( $_gc_submenu_nopriv[ $parent ][ $pagenow ] ) ) {
		return false;
	}

	if ( isset( $plugin_page ) ) {
		if ( isset( $_gc_submenu_nopriv[ $parent ][ $plugin_page ] ) ) {
			return false;
		}
	...
	if ( empty( $parent ) ) {
		if ( isset( $_gc_menu_nopriv[ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $_gc_submenu_nopriv[ $pagenow ][ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_gc_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_gc_menu_nopriv[ $plugin_page ] ) ) {
			return false;
		}

		foreach ( array_keys( $_gc_submenu_nopriv ) as $key ) {
			if ( isset( $_gc_submenu_nopriv[ $key ][ $pagenow ] ) ) {
				return false;
			}
			if ( isset( $plugin_page ) && isset( $_gc_submenu_nopriv[ $key ][ $plugin_page ] ) ) {
				return false;
			}
		}
	...
	if ( isset( $plugin_page ) && $plugin_page === $parent && isset( $_gc_menu_nopriv[ $plugin_page ] ) ) {
		return false;
	}
...
function settings_fields( $option_group ) {
	...
	gc_nonce_field( "$option_group-options" );
}
...
function gc_clean_plugins_cache( $clear_update_cache = true ) {
	...
	gc_cache_delete( 'plugins', 'plugins' );
}
 ...
 * https://developer.gechiui.com/plugins/privacy/suggesting-text-for-the-site-privacy-policy/.
 *
 ...
 */
function gc_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: admin_init */
				__( '推荐的隐私政策内容只应在gc-admin中通过%s（或之后的）动作加入。' ),
		...
		_doing_it_wrong(
			...
			sprintf(
				/* translators: %s: admin_init */
				__( '推荐的隐私政策内容只应在gc-admin中通过%s（或之后的）动作加入。请查阅内联文档。' ),
				'<code>admin_init</code>'
			),
			'4.9.7'
		);
	...
	GC_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 */
function gc_get_plugin_error( $plugin ) {
	if ( ! isset( $GLOBALS['_paused_plugins'] ) ) {
		return false;
	}
 ...
 * @return bool|GC_Error True on success, false if `$plugin` was not paused,
 *                       `GC_Error` on failure.
 */
function resume_plugin( $plugin, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		gc_redirect(
			add_query_arg(
				'_error_nonce',
				gc_create_nonce( 'plugin-resume-error_' . $plugin ),
				$redirect
			)
		);
	...
	$result = gc_paused_plugins()->delete( $extension );
...
function deactivated_plugins_notice() {
	...
	$blog_deactivated_plugins = get_option( 'gc_force_deactivated_plugins' );
	$site_deactivated_plugins = array();

	if ( false === $blog_deactivated_plugins ) {
		// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
		update_option( 'gc_force_deactivated_plugins', array() );
	}

	if ( is_multisite() ) {
		$site_deactivated_plugins = get_site_option( 'gc_force_deactivated_plugins' );
		if ( false === $site_deactivated_plugins ) {
			// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
			update_site_option( 'gc_force_deactivated_plugins', array() );
		}
	}
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current GC version, 4: Compatible plugin version. */
				__( '%1$s %2$s 由于与 GeChiUI %3$s 不兼容而被禁用，请升级到 %1$s %4$s 或更高版本。' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current GC version. */
				__( '%1$s %2$s 由于与 GeChiUI %3$s 不兼容而被禁用。' ),
				$plugin['plugin_name'],
