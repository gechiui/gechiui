<?php
/**
 * GeChiUI Widgets Administration API
 *
 * @package GeChiUI
...
function gc_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset( $gc_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) ) {
 ...
 */
function gc_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'gc_list_widget_controls_dynamic_sidebar' );

	$description = gc_sidebar_description( $sidebar );
 ...
 * @return array
 */
function gc_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $gc_registered_widgets[ $widget_id ]['callback'] ) ) {
		$gc_registered_widgets[ $widget_id ]['_callback'] = $gc_registered_widgets[ $widget_id ]['callback'];
		$gc_registered_widgets[ $widget_id ]['callback']  = 'gc_widget_control';
	}
 ...
 * @return array
 */
function gc_widget_control( $sidebar_args ) {
	global $gc_registered_widgets, $gc_registered_widget_controls, $sidebars_widgets;

	$widget_id  = $sidebar_args['widget_id'];
	$sidebar_id = isset( $sidebar_args['id'] ) ? $sidebar_args['id'] : false;
	$key        = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[ $sidebar_id ], true ) : '-1'; // Position of widget in sidebar.
	$control    = isset( $gc_registered_widget_controls[ $widget_id ] ) ? $gc_registered_widget_controls[ $widget_id ] : array();
	...
	$gc_registered_widgets[ $widget_id ]['callback'] = $gc_registered_widgets[ $widget_id ]['_callback'];
	unset( $gc_registered_widgets[ $widget_id ]['_callback'] );
	...
	<div class="widget-control-actions">
		<div class="alignleft">
			<button type="button" class="button-link button-link-delete widget-control-remove"><?php _e( '删除' ); ?></button>
			<span class="widget-control-close-wrapper">
				| <button type="button" class="button-link widget-control-close"><?php _e( '完成' ); ?></button>
			</span>
		</div>
		<div class="alignright<?php echo $noform_class; ?>">
			<?php submit_button( __( '保存' ), 'primary widget-control-save right', 'savewidget', false, array( 'id' => 'widget-' . esc_attr( $id_format ) . '-savewidget' ) ); ?>
	...
	<?php
	$widget_description = gc_widget_description( $widget_id );
 ...
 * @return string
 */
function gc_widgets_access_body_class( $classes ) {
