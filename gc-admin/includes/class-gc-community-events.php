...
class GC_Community_Events {
	 ...
	 * than having the browser make the request directly to api.gechiui.com,
	 * because it allows results to be cached server-side and shared with other
	 * users and sites in the network. This makes the process more efficient,
	 * since increasing the number of visits that get cached data means users
	 * don't have to wait as often; if the user's browser made the request
	 * directly, it would also need to make a second request to GC in order to
	 * pass the data for caching. Having GC make the request also introduces
	 ...
	 * @return array|GC_Error A GC_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$api_url                    = 'http://api.gechiui.com/events/1.0/';
		...
		if ( is_gc_error( $response ) ) {
			...
			$response_error = new GC_Error(
				...
				sprintf( __( 'API响应代码无效（%d）。' ), $response_code )
			);
		} elseif ( ! isset( $response_body['location'], $response_body['events'] ) ) {
			$response_error = new GC_Error(
				'api-invalid-response',
				isset( $response_body['error'] ) ? $response_body['error'] : __( '未知的API错误。' )
			);
		}
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `gc_remote_get()`.
		return array(
			'body' => $args,
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = gc_privacy_anonymize_ip( $client_ip, true );
