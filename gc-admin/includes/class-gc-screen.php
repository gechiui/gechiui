...
final class GC_Screen {
	...
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof GC_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'GC_NETWORK_ADMIN' ) && GC_NETWORK_ADMIN ) {
				$in_admin = 'network';
			} elseif ( defined( 'GC_USER_ADMIN' ) && GC_USER_ADMIN ) {
				$in_admin = 'user';
			} else {
				$in_admin = 'site';
			}
	 ...
	 *
	 *
	 * @param GC_Screen $screen A screen object.
	...
	public function set_screen_reader_content( $content = array() ) {
		$defaults = array(
			'heading_views'      => __( '筛选项目列表' ),
			'heading_pagination' => __( '项目列表导航' ),
			'heading_list'       => __( '项目列表' ),
		);
		$content  = gc_parse_args( $content, $defaults );
	...
	public function render_screen_meta() {
		...
		// Time to render!
		?>
		<div id="contextual-help-wrap" class="modal modal-right fade" aria-label="<?php esc_attr_e( '“上下文帮助”选项卡' ); ?>">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"><h3><?php _e( '帮助' ); ?></h3></div>
                    <div class="modal-body scrollable ps-container ps-theme-default">
                            ...
                            foreach ( $this->get_help_tabs() as $tab ) :
                                ...
                                <li id="<?php echo esc_attr( $link_id ); ?>" class="nav-item">
                                    <a href="<?php echo esc_url( "#$panel_id" ); ?>" aria-controls="<?php echo esc_attr( $panel_id ); ?>" class="<?php echo $class; ?>" data-toggle="pill" role="tab" aria-controls="<?php echo esc_url( "$panel_id" ); ?>">
...
-->
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $gc_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );

		$this->_screen_settings = '';

		if ( 'post' === $this->base ) {
			$expand                 = '<fieldset class="editor-expand hidden"><legend>' . __( '其他设置' ) . '</legend><label for="editor-expand-toggle">';
			$expand                .= '<input type="checkbox" id="editor-expand-toggle"' . checked( get_user_setting( 'editor_expand', 'on' ), 'on', false ) . ' />';
			$expand                .= __( '启用全高度编辑器和免打扰功能。' ) . '</label></fieldset>';
	...
	public function render_screen_options( $options = array() ) {
		...
		if ( 'widgets' !== $this->base ) {
			...
			$form_end   = "\n" . gc_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false, false ) . "\n</form>\n";
		}
		...
		if ( $show_button ) {
			submit_button( __( '应用' ), 'primary', 'screen-options-apply', true );
		}
	...
	public function render_meta_boxes_preferences() {
		...
		if ( ! isset( $gc_meta_boxes[ $this->id ] ) ) {
			return;
		}
		?>
		<fieldset class="metabox-prefs">
		<legend><?php _e( '界面元素' ); ?></legend>
		<p>
		...
		if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
			if ( isset( $_GET['welcome'] ) ) {
				...
				if ( 2 === $welcome_checked && gc_get_current_user()->user_email !== get_option( 'admin_email' ) ) {
					$welcome_checked = false;
				}
			}
			echo '<label for="gc_welcome_panel-hide">';
			echo '<input type="checkbox" id="gc_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
	...
	public function render_list_table_columns_preferences() {
		...
		$legend = ! empty( $columns['_title'] ) ? $columns['_title'] : __( '栏目' );
		?>
		<fieldset class="metabox-prefs">
	...
	public function render_screen_layout() {
		...
		<legend class="screen-layout"><?php _e( '布局' ); ?></legend>
		<?php for ( $i = 1; $i <= $num; ++$i ) : ?>
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' === $option ) {
			...
			/** This filter is documented in gc-admin/includes/class-gc-comments-list-table.php */
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );
		} elseif ( 'categories_per_page' === $option ) {
			/** This filter is documented in gc-admin/includes/class-gc-terms-list-table.php */
			$per_page = apply_filters( 'edit_categories_per_page', $per_page );
		} else {
			/** This filter is documented in gc-admin/includes/class-gc-list-table.php */
		...
		<legend><?php _e( '分页' ); ?></legend>
			<?php if ( $per_page_label ) : ?>
				<label for="<?php echo esc_attr( $option ); ?>"><?php echo $per_page_label; ?></label>
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="gc_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>" maxlength="3"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif; ?>
				<input type="hidden" name="gc_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</fieldset>
		<?php
	}
