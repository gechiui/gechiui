 ...
 * @return string|GC_Error New filepath on success, GC_Error on failure.
 */
function gc_crop_image( $src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	$src_file = $src;
	...
	$editor = gc_get_image_editor( $src );
	...
	if ( ! $dst_file ) {
		$dst_file = str_replace( gc_basename( $src_file ), 'cropped-' . gc_basename( $src_file ), $src_file );
	}
	 ...
	 */
	gc_mkdir_p( dirname( $dst_file ) );

	$dst_file = dirname( $dst_file ) . '/' . gc_unique_filename( dirname( $dst_file ), gc_basename( $dst_file ) );
 ...
 */
function gc_get_missing_image_subsizes( $attachment_id ) {
	if ( ! gc_attachment_is_image( $attachment_id ) ) {
		return array();
	}

	$registered_sizes = gc_get_registered_image_subsizes();
	...
	if ( ! empty( $image_meta['original_image'] ) ) {
		...
		$imagesize  = gc_getimagesize( $image_file );
	}
	 ...
	 * @param int     $attachment_id The image attachment post ID.
	 */
	return apply_filters( 'gc_get_missing_image_subsizes', $missing_sizes, $image_meta, $attachment_id );
}
 ...
 */
function gc_update_image_subsizes( $attachment_id ) {
	$image_meta = gc_get_attachment_metadata( $attachment_id );
	$image_file = gc_get_original_image_path( $attachment_id );
	...
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
		...
		if ( ! empty( $image_file ) ) {
			...
			return new GC_Error( 'invalid_attachment', __( '无法找到附件。' ) );
		}
	} else {
		$missing_sizes = gc_get_missing_image_subsizes( $attachment_id );
		...
		$image_meta = _gc_make_subsizes( $missing_sizes, $image_file, $image_meta, $attachment_id );
	}

	/** This filter is documented in gc-admin/includes/image.php */
	$image_meta = apply_filters( 'gc_generate_attachment_metadata', $image_meta, $attachment_id, 'update' );

	// Save the updated metadata.
	gc_update_attachment_metadata( $attachment_id, $image_meta );
 ...
 */
function _gc_image_meta_replace_original( $saved_data, $original_file, $image_meta, $attachment_id ) {
 ...
 */
function gc_create_image_subsizes( $file, $attachment_id ) {
	$imagesize = gc_getimagesize( $file );
	...
	$exif_meta = gc_read_image_metadata( $file );
	...
	if ( 'image/png' !== $imagesize['mime'] ) {
		 ...
		 * the `_gc_attached_file` post meta value.
		 *
		...
		if ( $threshold && ( $image_meta['width'] > $threshold || $image_meta['height'] > $threshold ) ) {
			...
			if ( ! is_gc_error( $resized ) && is_array( $exif_meta ) ) {
			...
			if ( ! is_gc_error( $resized ) ) {
				...
				if ( ! is_gc_error( $saved ) ) {
					$image_meta = _gc_image_meta_replace_original( $saved, $file, $image_meta, $attachment_id );
	...
	$new_sizes = gc_get_registered_image_subsizes();
	...
	return _gc_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id );
}
 ...
 */
function _gc_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id ) {
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
 ...
 */
function gc_generate_attachment_metadata( $attachment_id, $file ) {
	...
	if ( preg_match( '!^image/!', $mime_type ) && file_is_displayable_image( $file ) ) {
		...
		$metadata = gc_create_image_subsizes( $file, $attachment_id );
	} elseif ( gc_attachment_is( 'video', $attachment ) ) {
		$metadata = gc_read_video_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:video' ) || post_type_supports( 'attachment:video', 'thumbnail' );
	} elseif ( gc_attachment_is( 'audio', $attachment ) ) {
		$metadata = gc_read_audio_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:audio' ) || post_type_supports( 'attachment:audio', 'thumbnail' );
	}

	/*
	 * gc_read_video_metadata() and gc_read_audio_metadata() return `false`
	...
	if ( $support && ! empty( $metadata['image']['data'] ) ) {
		...
		if ( ! empty( $exists ) ) {
			...
			$basename = str_replace( '.', '-', gc_basename( $file ) ) . '-image' . $ext;
			$uploaded = gc_upload_bits( $basename, '', $metadata['image']['data'] );
			if ( false === $uploaded['error'] ) {
				...
				$sub_attachment_id = gc_insert_attachment( $image_attachment, $uploaded['file'] );
				add_post_meta( $sub_attachment_id, '_cover_hash', $hash );
				$attach_data = gc_generate_attachment_metadata( $sub_attachment_id, $uploaded['file'] );
				gc_update_attachment_metadata( $sub_attachment_id, $attach_data );
		...
		if ( ! empty( $merged_sizes ) ) {
			...
			if ( ! is_gc_error( $editor ) ) { // No support for this type of file.
				/*
				...
				$preview_file = $dirname . gc_unique_filename( $dirname, gc_basename( $file, $ext ) . '-pdf.jpg' );

				$uploaded = $editor->save( $preview_file, 'image/jpeg' );
				unset( $editor );

				// Resize based on the full size image, rather than the source.
				if ( ! is_gc_error( $uploaded ) ) {
					...
					$metadata = _gc_make_subsizes( $merged_sizes, $image_file, $metadata, $attachment_id );
				}
			}
		}
	}
	 ...
	 */
	return apply_filters( 'gc_generate_attachment_metadata', $metadata, $attachment_id, 'create' );
}
...
function gc_exif_frac2dec( $str ) {
	...
	if ( 0 == $denominator ) { // phpcs:ignore GeChiUI.PHP.StrictComparisons.LooseComparison -- Deliberate loose comparison.
		return 0;
	}
 ...
 */
function gc_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}

	list( , , $image_type ) = gc_getimagesize( $file );
	 ...
	 */
	$exif_image_types = apply_filters( 'gc_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) );
	...
	if ( is_callable( 'exif_read_data' ) && in_array( $image_type, $exif_image_types, true ) ) {
		...
		if ( ! empty( $exif['FNumber'] ) && is_scalar( $exif['FNumber'] ) ) {
			$meta['aperture'] = round( gc_exif_frac2dec( $exif['FNumber'] ), 2 );
		}
		if ( ! empty( $exif['Model'] ) ) {
			$meta['camera'] = trim( $exif['Model'] );
		}
		if ( empty( $meta['created_timestamp'] ) && ! empty( $exif['DateTimeDigitized'] ) ) {
			$meta['created_timestamp'] = gc_exif_date2ts( $exif['DateTimeDigitized'] );
		}
		if ( ! empty( $exif['FocalLength'] ) ) {
			$meta['focal_length'] = (string) $exif['FocalLength'];
			if ( is_scalar( $exif['FocalLength'] ) ) {
				$meta['focal_length'] = (string) gc_exif_frac2dec( $exif['FocalLength'] );
			}
		}
		...
		if ( ! empty( $exif['ExposureTime'] ) ) {
			...
			if ( is_scalar( $exif['ExposureTime'] ) ) {
				$meta['shutter_speed'] = (string) gc_exif_frac2dec( $exif['ExposureTime'] );
			}
		}
	 ...
	 */
	return apply_filters( 'gc_read_image_metadata', $meta, $file, $image_type, $iptc, $exif );
...
function _load_image_to_edit_path( $attachment_id, $size = 'full' ) {
	...
	if ( $filepath && file_exists( $filepath ) ) {
		 ...
		 *                                    an array of width and height values in pixels (in that order).
		 */
		$filepath = apply_filters( 'load_image_to_edit_attachmenturl', gc_get_attachment_url( $attachment_id ), $attachment_id, $size );
	}
...
function _copy_image_file( $attachment_id ) {
	...
	if ( $src_file ) {
		$dst_file = str_replace( gc_basename( $dst_file ), 'copy-' . gc_basename( $dst_file ), $dst_file );
		$dst_file = dirname( $dst_file ) . '/' . gc_unique_filename( dirname( $dst_file ), gc_basename( $dst_file ) );
