 ...
 * @return array|GC_Error Array of post data on success, GC_Error on failure.
 */
function _gc_translate_postdata( $update = false, $post_data = null ) {
	...
	if ( $update && ! current_user_can( 'edit_post', $post_data['ID'] ) ) {
		...
		if ( 'page' === $post_data['post_type'] ) {
			return new GC_Error( 'edit_others_pages', __( '抱歉，您不能作为此用户创建文章。' ) );
		} else {
			return new GC_Error( 'edit_others_posts', __( '抱歉，您不能为此用户创建文章。' ) );
		}
	}
	...
	if ( ! empty( $post_data['edit_date'] ) ) {
		...
		$valid_date = gc_checkdate( $mm, $jj, $aa, $post_data['post_date'] );
		if ( ! $valid_date ) {
 ...
 * @return array|GC_Error Array of post data on success, GC_Error on failure.
 */
function _gc_get_allowed_postdata( $post_data = null ) {
	if ( empty( $post_data ) ) {
		$post_data = $_POST;
	}
...
function edit_post( $post_data = null ) {
	...
	$translated = _gc_get_allowed_postdata( $post_data );
	...
	if ( ! $success && is_callable( array( $gcdb, 'strip_invalid_text_for_column' ) ) ) {
		$fields = array( 'post_title', 'post_content', 'post_excerpt' );

		foreach ( $fields as $field ) {
			if ( isset( $translated[ $field ] ) ) {
				$translated[ $field ] = $gcdb->strip_invalid_text_for_column( $gcdb->posts, $field, $translated[ $field ] );
			}
		}

		gc_update_post( $translated );
	}
...
function bulk_edit_posts( $post_data = null ) {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			gc_die( __( '抱歉，您不能编辑页面。' ) );
		} else {
			gc_die( __( '抱歉，您不能编辑这篇文章。' ) );
		}
	}
	...
	if ( isset( $post_data['post_parent'] ) && (int) $post_data['post_parent'] ) {
		...
		$pages    = $gcdb->get_results( "SELECT ID, post_parent FROM $gcdb->posts WHERE post_type = 'page'" );
		$children = array();
...
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	...
	if ( ! empty( $_REQUEST['post_title'] ) ) {
		$post_title = esc_html( gc_unslash( $_REQUEST['post_title'] ) );
	}

	$post_content = '';
	if ( ! empty( $_REQUEST['content'] ) ) {
		$post_content = esc_html( gc_unslash( $_REQUEST['content'] ) );
	}

	$post_excerpt = '';
	if ( ! empty( $_REQUEST['excerpt'] ) ) {
		$post_excerpt = esc_html( gc_unslash( $_REQUEST['excerpt'] ) );
	}
	...
	if ( $create_in_db ) {
		...
		if ( ! gc_next_scheduled( 'gc_scheduled_auto_draft_delete' ) ) {
...
function post_exists( $title, $content = '', $date = '', $type = '', $status = '' ) {
	...
	$post_title   = gc_unslash( sanitize_post_field( 'post_title', $title, 0, 'db' ) );
	$post_content = gc_unslash( sanitize_post_field( 'post_content', $content, 0, 'db' ) );
	$post_date    = gc_unslash( sanitize_post_field( 'post_date', $date, 0, 'db' ) );
	$post_type    = gc_unslash( sanitize_post_field( 'post_type', $type, 0, 'db' ) );
	$post_status  = gc_unslash( sanitize_post_field( 'post_status', $status, 0, 'db' ) );

	$query = "SELECT ID FROM $gcdb->posts WHERE 1=1";
	$args  = array();
	...
	if ( ! empty( $args ) ) {
		return (int) $gcdb->get_var( $gcdb->prepare( $query, $args ) );
	}
...
function gc_write_post() {
	...
	$translated = _gc_translate_postdata( false );
	if ( is_gc_error( $translated ) ) {
		return $translated;
	}
 ...
 */
function write_post() {
	$result = gc_write_post();
	if ( is_gc_error( $result ) ) {
		gc_die( $result->get_error_message() );
	} else {
		return $result;
	}
}
 ...
 * @return void|int|GC_Error Void if nothing fixed. 0 or GC_Error on update failure. The post ID on update success.
 */
function _fix_attachment_links( $post ) {
	...
	foreach ( $link_matches[1] as $key => $value ) {
		if ( ! strpos( $value, '?attachment_id=' ) || ! strpos( $value, 'gc-att-' )
			|| ! preg_match( '/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match )
			|| ! preg_match( '/rel=["\'][^"\']*gc-att-(\d+)/', $value, $rel_match ) ) {
				continue;
		}
 ...
 * @return array
 */
function gc_edit_posts_query( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
 ...
 */
function gc_edit_attachments_query_vars( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
	$q['m']         = isset( $q['m'] ) ? (int) $q['m'] : 0;
 ...
 * @return array
 */
function gc_edit_attachments_query( $q = false ) {
	gc( gc_edit_attachments_query_vars( $q ) );
...
function get_sample_permalink( $id, $title = null, $name = null ) {
	...
	$post->post_name = gc_unique_post_slug( $post->post_name, $post->ID, $post->post_status, $post->post_type, $post->post_parent );
...
function get_sample_permalink_html( $id, $new_title = null, $new_slug = null ) {
	...
	if ( current_user_can( 'read_post', $post->ID ) ) {
		if ( 'draft' === $post->post_status || empty( $post->post_name ) ) {
			...
			$preview_target = " target='gc-preview-{$post->ID}'";
		} else {
	...
	if ( false === strpos( $permalink, '%postname%' ) && false === strpos( $permalink, '%pagename%' ) ) {
		...
		$return .= '<span id="edit-slug-buttons"><button type="button" class="edit-slug button button-small hide-if-no-js" aria-label="' . __( '编辑固定链接' ) . '">' . __( '编辑' ) . "</button></span>\n";
 ...
 */
function _gc_post_thumbnail_html( $thumbnail_id = null, $post = null ) {
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		$size = isset( $_gc_additional_image_sizes['post-thumbnail'] ) ? 'post-thumbnail' : array( 266, 266 );

		/**
		 * Filters the size used to display the post thumbnail image in the '特色图片' meta box.
		 *
		 ...
		 * @param GC_Post      $post         The post object associated with the thumbnail.
		 */
		$size = apply_filters( 'admin_post_thumbnail_size', $size, $thumbnail_id, $post );

		$thumbnail_html = gc_get_attachment_image( $thumbnail_id, $size );
		...
		if ( ! empty( $thumbnail_html ) ) {
			...
			$content .= '<p class="hide-if-no-js howto" id="set-post-thumbnail-desc">' . __( '点击图片来修改或更新' ) . '</p>';
...
function gc_check_post_lock( $post_id ) {
	...
	$time_window = apply_filters( 'gc_check_post_lock_window', 150 );
	...
	if ( $locked ) {
		...
		if ( get_post_type_object( $post->post_type )->public ) {
			if ( 'publish' === $post->post_status || $user->ID != $post->post_author ) {
				...
				$nonce                       = gc_create_nonce( 'post_preview_' . $post->ID );
		...
		$tab_last = $override ? '' : ' gc-tab-last';

		?>
		<div class="post-locked-message">
		<div class="post-locked-avatar"><?php echo get_avatar( $user->ID, 64 ); ?></div>
		<p class="currently-editing gc-tab-first" tabindex="0">
		<?php
		if ( $override ) {
			/* translators: %s: User's display name. */
			printf( __( '%s正在编辑此文章，您希望接手吗？' ), esc_html( $user->display_name ) );
		} else {
			/* translators: %s: User's display name. */
			printf( __( '%s正在编辑此文章。' ), esc_html( $user->display_name ) );
		}
		?>
		</p>
		<?php
 ...
 * @return int|GC_Error The autosave revision ID. GC_Error or 0 on error.
 */
function gc_create_post_autosave( $post_data ) {
	...
	$old_autosave = gc_get_post_autosave( $post_id, $post_author );
	if ( $old_autosave ) {
		...
		return gc_update_post( $new_autosave );
	}

	// _gc_put_post_revision() expects unescaped.
	$post_data = gc_unslash( $post_data );
...
function post_preview() {
	...
	if ( ! gc_check_post_lock( $post->ID ) && get_current_user_id() == $post->post_author
		...
		$saved_post_id = gc_create_post_autosave( $post->ID );
	}

	if ( is_gc_error( $saved_post_id ) ) {
		gc_die( $saved_post_id->get_error_message() );
	}
	...
	if ( $is_autosave && $saved_post_id ) {
		$query_args['preview_id']    = $post->ID;
		$query_args['preview_nonce'] = gc_create_nonce( 'post_preview_' . $post->ID );
 ...
 */
function gc_autosave( $post_data ) {
	// Back-compat.
	...
	if ( false === gc_verify_nonce( $post_data['_gcnonce'], 'update-post_' . $post_id ) ) {
		return new GC_Error( 'invalid_nonce', __( '保存时发生错误。' ) );
	}

	$post = get_post( $post_id );

	if ( ! current_user_can( 'edit_post', $post->ID ) ) {
		return new GC_Error( 'edit_posts', __( '抱歉，您不能编辑此项目。' ) );
	}
...
function redirect_post( $post_id = '' ) {
	if ( isset( $_POST['save'] ) || isset( $_POST['publish'] ) ) {
		...
		$location = add_query_arg( 'message', 2, gc_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} elseif ( isset( $_POST['deletemeta'] ) && $_POST['deletemeta'] ) {
		$location = add_query_arg( 'message', 3, gc_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} else {
	 ...
	 */
	gc_redirect( apply_filters( 'redirect_post_location', $location, $post_id ) );
	exit;
}
...
function the_block_editor_meta_boxes() {
	...
	$_original_meta_boxes = $gc_meta_boxes;
	 ...
	 */
	$gc_meta_boxes = apply_filters( 'filter_block_editor_meta_boxes', $gc_meta_boxes );
	...
	<form id="toggle-custom-fields-form" method="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>">
		<?php gc_nonce_field( 'toggle-custom-fields', 'toggle-custom-fields-nonce' ); ?>
	...
	foreach ( $locations as $location ) {
		...
		if ( ! isset( $gc_meta_boxes[ $current_screen->id ][ $location ] ) ) {
			continue;
		}

		foreach ( $priorities as $priority ) {
			if ( ! isset( $gc_meta_boxes[ $current_screen->id ][ $location ][ $priority ] ) ) {
				continue;
			}

			$meta_boxes = (array) $gc_meta_boxes[ $current_screen->id ][ $location ][ $priority ];
...
/**
 * This callback disables the content editor for gc_navigation type posts.
 * Content editor cannot handle gc_navigation type posts correctly.
...
/**
 * This callback enables content editor for gc_navigation type posts.
