...
final class GC_Privacy_Policy_Content {
	...
	public static function notice( $post = null ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			?>
			<div class="notice notice-warning inline gc-pp-notice">
				<p>
				<?php
				echo $message;
				printf(
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			...
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当列出您的站点URL、主办站点的公司、组织或个人及准确的联系信息。' ) . '</p>';
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '此章节中，所需的信息的多少将取决于您所在国家或地区的商业法规要求。例如，您有可能需要展示您的实际地址、注册地址或公司注册代码。' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. %s: Site URL. */
			$strings[] = '<p>' . $suggested_text . sprintf( __( '我们的站点地址是：%s。' ), get_bloginfo( 'url', 'display' ) ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当列出您从站点用户和访问者处收集哪些个人数据，这可能包括如姓名、电子邮箱、个人账户设置等个人数据，如购买信息等交易数据、cookies中所包含的信息等技术数据。' ) . '</p>';
			/* translators: Privacy policy tutorial. */
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '个人数据不仅由用户与您的站点的交互而产生，也会由其他技术过程产生，例如联系表单、评论、cookies、统计和第三方嵌入内容等。' ) . '</p>';
			/* translators: Privacy policy tutorial. */
		...
		if ( $description ) {
			...
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . __( '由您的电子邮箱所生成的匿名化字符串（又称为哈希）可能会被提供给Gravatar服务确认您是否有使用该服务。Gravatar服务的隐私政策在此：https://automattic.com/privacy/。在您的评论获批准后，您的资料图片将在您的评论旁公开展示。' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . $suggested_text . __( '如果您向此网站上传图片，您应当避免上传那些有嵌入地理位置信息（EXIF GPS）的图片。此网站的访客将可以下载并提取此网站的图片中的位置信息。' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'GeChiUI默认并不提供联系表单。如果您使用了联系表单插件，请在此章节表明您在访客提交联系表单时所收集个人数据的种类及保留期限。例如，您可以说明，您会将所有提交的联系表单保留一段时间以供客户服务之用，但您不会将这些信息用于市场宣传。' ) . '</p>';
		}
		...
		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当列出您的站点所使用的cookies，这也包括您的插件、社交媒体和统计程序所设置的cookies。我们已经为您列出了GeChiUI默认使用的cookies。' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . $suggested_text . __( '如果您在我们的站点上留下评论，您可以选择用cookies保存您的名字、电子邮箱和网站地址。这是通过让您可以不用在评论时再次填写相关内容而向您提供方便。这些cookies会保留一年。' ) . '</p>';
			/* translators: Default privacy policy text. */
		...
		if ( ! $description ) {
			...
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . $suggested_text . __( '此站点上的文章可能会包含嵌入的内容（如视频、图片、文章等）。来自其他站点的嵌入内容的行为和您直接访问这些其他站点没有区别。' ) . '</p>';
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . __( '这些站点可能会收集关于您的数据、使用cookies、嵌入额外的第三方跟踪程序及监视您与这些嵌入内容的交互，包括在您有这些站点的账户并登录了这些站点时，跟踪您与嵌入内容的交互。' ) . '</p>';
		}
		...
		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当列出您共享站点数据的所有第三方提供商，包括合作伙伴、云服务、支付服务及其他第三方服务提供商。您应当写明您与他们共享数据的种类及共享的原因。如果可能，您也应该提供指向他们隐私政策的链接。' ) . '</p>';
			/* translators: Privacy policy tutorial. */
		...
		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当指出由网站收集或处理的个人数据的保留期限。尽管您有责任决定数据集的保存期限及保存原因，您也需要将这些信息列在此处。例如，您可以说明，您会保留联系表单条目六个月、保留统计记录一年、保留客户购买记录十年等。' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . $suggested_text . __( '如果您留下评论，评论和其元数据将被无限期保存。我们这样做以便能识别并自动批准任何后续评论，而不用将这些后续评论加入待审队列。' ) . '</p>';
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . __( '对于本网站的注册用户，我们也会保存用户在个人资料中提供的个人信息。所有用户可以在任何时候查看、编辑或删除他们的个人信息（除了不能变更用户名外）、站点管理员也可以查看及编辑那些信息。' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Default privacy policy text. */
			$strings[] = '<p>' . $suggested_text . __( '如果您有此站点的账户，或曾经留下评论，您可以请求我们提供我们所拥有的您的个人数据的导出文件，这也包括了所有您提供给我们的数据。您也可以要求我们抹除所有关于您的个人数据。这不包括我们因管理、法规或安全需要而必须保留的数据。' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2>' . __( '我们将您的信息发送到哪' ) . '</h2>';

		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '在此章节中，您应当列出所有将您的站点数据移出欧盟的行为，并描述这些数据受到了何种符合欧洲数据保护标准的保护方式。这些数据可能包括了您网站使用的虚拟主机、云存储及其他第三方服务。' ) . '</p>';
			/* translators: Privacy policy tutorial. */
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '如果您将您的站点用于商业活动，且您进行了更复杂的个人数据收集及处理，您应当在您的隐私政策中写下以下信息。' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '如果您的网站从第三方，如广告商，接收关于用户的数据，这些信息必须在您的隐私政策中涉及到第三方数据的章节予以体现。' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( '如果您的网站提供包含自动决策的服务，如允许客户申请信用卡、或收集客户信息进入广告资料，您必须明示这些事件的发生，并说明信息如何被使用，通过何种汇总数据进行了何种决策，及在决策不受人工干预作出时用户有哪些权利。' ) . '</p>';
		}
