<?php
/**
 * List Table API: GC_Plugins_List_Table class
 *
 * @package GeChiUI
 ...
 * @see GC_List_Table
 */
class GC_Plugins_List_Table extends GC_List_Table {
	/**
	...
	public function __construct( $args = array() ) {
		...
		if ( isset( $_REQUEST['s'] ) ) {
			$_SERVER['REQUEST_URI'] = add_query_arg( 's', gc_unslash( $_REQUEST['s'] ) );
		}
	...
	public function prepare_items() {
		...
		gc_reset_vars( array( 'orderby', 'order' ) );
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			...
			/** This action is documented in gc-admin/includes/class-gc-plugins-list-table.php */
		...
		foreach ( (array) $plugins['all'] as $plugin_file => $plugin_data ) {
			...
			$filter_payload = (object) gc_parse_args( $plugin_data, $filter_payload );

			$auto_update_forced = gc_is_auto_update_forced_for_item( 'plugin', null, $filter_payload );
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( '在GeChiUI插件目录中搜索插件。' ) . '</a>';
			}
		...
		foreach ( $totals as $type => $count ) {
			...
			switch ( $type ) {
				...
				case 'auto-update-enabled':
					/* translators: %s: Number of plugins. */
					$text = _n(
						'自动更新已启用<span class="count">（%s）</span>',
						'自动更新已启用<span class="count">（%s）</span>',
						$count
					);
					break;
				case 'auto-update-disabled':
					/* translators: %s: Number of plugins. */
					$text = _n(
						'自动更新已禁用<span class="count">（%s）</span>',
						'自动更新已禁用<span class="count">（%s）</span>',
						$count
					);
					break;
	...
	protected function get_bulk_actions() {
		...
		if ( ! is_multisite() || $this->screen->in_admin( 'network' ) ) {
			if ( current_user_can( 'update_plugins' ) ) {
				$actions['update-selected'] = __( '更新' );
			}

			if ( current_user_can( 'delete_plugins' ) && ( 'active' !== $status ) ) {
				$actions['delete-selected'] = __( '删除' );
			}

			if ( $this->show_autoupdates ) {
				if ( 'auto-update-enabled' !== $status ) {
					$actions['enable-auto-update-selected'] = __( '启用自动更新' );
				}
				if ( 'auto-update-disabled' !== $status ) {
					$actions['disable-auto-update-selected'] = __( '禁用自动更新' );
				}
			}
		}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			submit_button( __( '清空列表' ), '', 'clear-recent-list', false );
		} elseif ( 'top' === $which && 'mustuse' === $status ) {
			echo '<p>' . sprintf(
				/* translators: %s: mu-plugins directory name. */
				__( '在%s目录中的文件会被自动执行。' ),
				'<code>' . str_replace( ABSPATH, '/', GCMU_PLUGIN_DIR ) . '</code>'
			) . '</p>';
	...
	public function single_row( $item ) {
		...
		$requires_gc  = isset( $plugin_data['RequiresGC'] ) ? $plugin_data['RequiresGC'] : null;
		...
		if ( 'mustuse' === $context ) {
			...
			if ( true === ( $dropins[ $plugin_file ][1] ) ) { // Doesn't require a constant.
				...
				$is_active   = false;
				$description = '<p><strong>' . $dropins[ $plugin_file ][0] . ' <span class="error-message">' . __( '未启用：' ) . '</span></strong> ' .
					sprintf(
						/* translators: 1: Drop-in constant name, 2: gc-config.php */
						__( '需要在%2$s文件中包含%1$s。' ),
				...
				if ( $is_active ) {
					...
					if ( current_user_can( 'delete_plugins' ) && ! is_plugin_active( $plugin_file ) ) {
						$actions['delete'] = sprintf(
							...
							gc_nonce_url( 'plugins.php?action=delete-selected&amp;checked[]=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-plugins' ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
							esc_attr( sprintf( _x( '删除 %s', 'plugin' ), $plugin_data['Name'] ) ),
							__( '删除' )
						);
					}
				}
				...
				if ( $restrict_network_active ) {
					...
					if ( current_user_can( 'deactivate_plugin', $plugin_file ) ) {
						$actions['deactivate'] = sprintf(
							...
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
							esc_attr( sprintf( _x( '禁用%s', 'plugin' ), $plugin_data['Name'] ) ),
							__( '禁用' )
						);
					}
					...
					if ( current_user_can( 'resume_plugin', $plugin_file ) && is_plugin_paused( $plugin_file ) ) {
						$actions['resume'] = sprintf(
							...
							gc_nonce_url( 'plugins.php?action=resume&amp;plugin=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'resume-plugin_' . $plugin_file ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
							esc_attr( sprintf( _x( '恢复%s', 'plugin' ), $plugin_data['Name'] ) ),
							__( '恢复' )
						);
					}
				} else {
			...
			switch ( $column_name ) {
				...
				case 'description':
					...
					if ( ! empty( $plugin_data['Version'] ) ) {
						...
						$plugin_meta[] = sprintf( __( '%s版本' ), $plugin_data['Version'] );
					}
					if ( ! empty( $plugin_data['Author'] ) ) {
						...
						$plugin_meta[] = sprintf( __( '作者：%s' ), $author );
					}
					...
					if ( isset( $plugin_data['slug'] ) && current_user_can( 'install_plugins' ) ) {
						$plugin_meta[] = sprintf(
							...
							/* translators: %s: Plugin name. */
							esc_attr( sprintf( __( '关于%s的更多信息' ), $plugin_name ) ),
					...
					if ( $paused ) {
						$notice_text = __( '此插件未能正确载入并已在恢复模式中暂停。' );

						printf( '<p><span class="dashicons dashicons-warning"></span> <strong>%s</strong></p>', $notice_text );

						$error = gc_get_plugin_error( $plugin_file );

						if ( false !== $error ) {
							printf( '<div class="error-display"><p>%s</p></div>', gc_get_extension_error_description( $error ) );
						}
					}
				...
				case 'auto-updates':
					...
					if ( 'unavailable' === $action ) {
						...
						$html[] = sprintf(
							'<a href="%s" class="toggle-auto-update aria-button-if-js" data-gc-action="%s">',
							gc_nonce_url( $url, 'updates' ),
							$action
						);
