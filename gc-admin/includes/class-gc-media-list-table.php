<?php
/**
 * List Table API: GC_Media_List_Table class
 *
 * @package GeChiUI
 ...
 * @see GC_List_Table
 */
class GC_Media_List_Table extends GC_List_Table {
	/**
	...
	public function prepare_items() {
		global $mode, $gc_query, $post_mime_types, $avail_post_mime_types;
		...
		list( $post_mime_types, $avail_post_mime_types ) = gc_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args(
			array(
				...
				'total_pages' => $gc_query->max_num_pages,
	...
	protected function get_views() {
		...
		$type_links['detached'] = '<option value="detached"' . ( $this->detached ? ' selected="selected"' : '' ) . '>' . _x( '独立', 'media items' ) . '</option>';
	...
	public function get_columns() {
		...
		$posts_columns['title']  = _x( '文件', 'column name' );
		...
		/* translators: Column name. */
		$posts_columns['date'] = _x( '日期', 'column name' );
	...
	public function column_date( $post ) {
		if ( '0000-00-00 00:00:00' === $post->post_date ) {
			...
			if ( $time && $time_diff > 0 && $time_diff < DAY_IN_SECONDS ) {
				...
				$h_time = sprintf( __( '%s前' ), human_time_diff( $time ) );
			} else {
		...
		if ( $parent ) {
			...
			if ( $user_can_edit ) :
				$detach_url = add_query_arg(
					array(
						...
						'media[]'        => $post->ID,
						'_gcnonce'       => gc_create_nonce( 'bulk-' . $this->_args['plural'] ),
					),
					'upload.php'
				);
				printf(
					...
					esc_attr( sprintf( __( '从“%s”分离' ), $title ) ),
					__( '分离' )
				);
			endif;
		} else {
			...
			if ( $user_can_edit ) {
				...
				printf(
					...
					$post->ID,
					/* translators: %s: Attachment title. */
					esc_attr( sprintf( __( '附加“%s”到已有内容' ), $title ) ),
					__( '现在附加到文章或页面' )
				);
			}
		}
	...
	public function display_rows() {
		...
		$post_ids = gc_list_pluck( $gc_query->posts, 'ID' );
		reset( $gc_query->posts );
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		$actions   = $this->_get_row_actions(
			$item, // GC_Post object for an attachment.
			$att_title
		);
