...
class Custom_Image_Header {
	...
	public function show_header_selector( $type = 'default' ) {
		...
		if ( 1 < count( $headers ) ) {
			...
			_e( '<strong>随机：</strong>在每个页面上显示不同的图片。' );
			echo '</label>';
			echo '</div>';
		}
...
(function($){
	...
	$( function() {
		...
		text_color.gcColorPicker({
			change: function( event, ui ) {
				pickColor( text_color.gcColorPicker('color') );
			},
			clear: function() {
				pickColor( '' );
			}
...
<td>
	<p><?php _e( '在这里，您可以上传新图片或从媒体库中已上传的图片中选择，以取代默认的图片。上传完成后，您将可以按喜好自己剪裁图片。' ); ?><br />
			<?php
			if ( ! current_theme_supports( 'custom-header', 'flex-height' )
				&& ! current_theme_supports( 'custom-header', 'flex-width' )
			) {
				printf(
					...
					__( '使用图片的<strong>%1$d &times; %2$d像素</strong>。' ) . '<br />',
	...
	<form enctype="multipart/form-data" id="upload-form" class="gc-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ); ?>">
	<p>
		<label for="upload"><?php _e( '从您的计算机中选择图片：' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php gc_nonce_field( 'custom-header-upload', '_gcnonce-custom-header-upload' ); ?>
...
<tr>
<th scope="row"><?php _e( '已上传的图片' ); ?></th>
<td>
	<p><?php _e( '您可以从上传过的页眉图片中选择，或让GeChiUI在每次显示页面时随机挑选。' ); ?></p>
			<?php
		...
		if ( ! empty( $this->default_headers ) ) :
			?>
<tr>
<th scope="row"><?php _e( '默认图图片' ); ?></th>
<td>
			<?php if ( current_theme_supports( 'custom-header', 'uploads' ) ) : ?>
	<p><?php _e( '若您现在不想上传您自己的图片，可从这些精美的页眉图片中选择，或让GeChiUI在每次显示页面时随机挑选。' ); ?></p>
	<?php else : ?>
	<p><?php _e( '您可以从这些精美的页眉图片中选择，或让GeChiUI在每次显示页面时随机挑选。' ); ?></p>
	<?php endif; ?>
			<?php
...
<td>
	<p><?php _e( '这将移除页眉图片。之后，您将无法还原任何自定义元素。' ); ?></p>
			<?php submit_button( __( '移除页眉图片' ), '', 'removeheader', false ); ?>
</td>
</tr>
			<?php
	endif;
		...
		if ( $default_image && get_header_image() !== $default_image ) :
			?>
<tr>
<th scope="row"><?php _e( '重置图片' ); ?></th>
<td>
	<p><?php _e( '这将重置页眉图片。之后，您将无法还原任何自定义元素。' ); ?></p>
			<?php submit_button( __( '重设页眉图片' ), '', 'resetheader', false ); ?>
</td>
</tr>
	<?php endif; ?>
</tbody>
...
<tbody>
<tr>
<th scope="row"><?php _e( '顶部文本' ); ?></th>
<td>
	<p>
	<label><input type="checkbox" name="display-header-text" id="display-header-text"<?php checked( display_header_text() ); ?> /> <?php _e( '将页眉文本和图片一并显示。' ); ?></label>
	</p>
</td>
</tr>

<tr class="displaying-header-text">
<th scope="row"><?php _e( '文本颜色' ); ?></th>
<td>
	<p>
			<?php
			$default_color = '';
			...
			if ( $default_color ) {
				...
				echo ' <span class="description hide-if-js">' . sprintf( _x( '默认色：%s', 'color' ), esc_html( $default_color ) ) . '</span>';
			}
			?>
	</p>
</td>
		...
		gc_nonce_field( 'custom-header-options', '_gcnonce-custom-header-options' );
		?>
	...
	public function step_2() {
		check_admin_referer( 'custom-header-upload', '_gcnonce-custom-header-upload' );
		...
		if ( empty( $_POST ) && isset( $_GET['file'] ) ) {
			...
			$url           = gc_get_attachment_image_src( $attachment_id, 'full' );
			$url           = $url[0];
		} elseif ( isset( $_POST ) ) {
		...
		if ( file_exists( $file ) ) {
			list( $width, $height, $type, $attr ) = gc_getimagesize( $file );
		} else {
			$data   = gc_get_attachment_metadata( $attachment_id );
		...
		if ( ! current_theme_supports( 'custom-header', 'flex-height' )
			 ...
			 */
			do_action( 'gc_create_file_in_uploads', $file, $attachment_id ); // For replication.
			...
			$image = gc_crop_image(
				...
				$height / $oitar,
				false,
				str_replace( gc_basename( $file ), 'midsize-' . gc_basename( $file ), $file )
			);

			if ( ! $image || is_gc_error( $image ) ) {
				gc_die( __( '无法处理图片。请返回重试。' ), __( '图片处理发生错误' ) );
			}

			/** This filter is documented in gc-admin/includes/class-custom-image-header.php */
			$image = apply_filters( 'gc_create_file_in_uploads', $image, $attachment_id ); // For replication.

			$url    = str_replace( gc_basename( $url ), gc_basename( $image ), $url );
			$width  = $width / $oitar;
			$height = $height / $oitar;
		} else {
			$oitar = 1;
...
<form method="post" action="<?php echo esc_url( add_query_arg( 'step', 3 ) ); ?>">
	<p class="hide-if-no-js"><?php _e( '从图片中选取一部分作为您的页眉图片。' ); ?></p>
	<p class="hide-if-js"><strong><?php _e( '您需要JavaScript支持以选择图片的一部分。' ); ?></strong></p>
	...
	<?php } ?>
		<?php gc_nonce_field( 'custom-header-crop-image' ); ?>

	<p class="submit">
		<?php submit_button( __( '剪裁并发布' ), 'primary', 'submit', false ); ?>
		<?php
		if ( isset( $oitar ) && 1 === $oitar
			&& ( current_theme_supports( 'custom-header', 'flex-height' )
				|| current_theme_supports( 'custom-header', 'flex-width' ) )
		) {
			submit_button( __( '不剪裁以原样发布' ), '', 'skip-cropping', false );
		}
		?>
	</p>
</form>
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_gc_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_crop() {
		...
		if ( ! $cropped || is_gc_error( $cropped ) ) {
			gc_send_json_error( array( 'message' => __( '无法处理图片。请返回重试。' ) ) );
		}
	...
	public function ajax_header_add() {
		...
		update_post_meta( $attachment_id, '_gc_attachment_is_custom_header', get_stylesheet() );
	 ...
	 *
	 *
	 * @param GC_Customize_Manager $gc_customize Customize manager.
	 */
	public function customize_set_last_used( $gc_customize ) {

		$header_image_data_setting = $gc_customize->get_setting( 'header_image_data' );
