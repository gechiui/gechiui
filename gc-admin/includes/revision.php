<?php
/**
 * GeChiUI Administration Revisions API
 *
 * @package GeChiUI
 ...
 *                     Or, false on failure.
 */
function gc_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	$post = get_post( $post );
	if ( ! $post ) {
		return false;
	}
	...
	if ( $compare_from && empty( $compare_from->post_title ) ) {
		$compare_from->post_title = __( '（无标题）' );
	}
	if ( empty( $compare_to->post_title ) ) {
		$compare_to->post_title = __( '（无标题）' );
	}

	$return = array();

	foreach ( _gc_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 *
		 *  - `_gc_post_revision_field_post_title`
		 ...
		 * @param GC_Post $compare_from   The revision post object to compare to or from.
		 * @param string  $context        The context of whether the current revision is the old
		 *                                or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_gc_post_revision_field_{$field}", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in gc-admin/includes/revision.php */
		$content_to = apply_filters( "_gc_post_revision_field_{$field}", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 * @param GC_Post $compare_to   The revision post to compare to.
		 */
	 ...
	 * @param GC_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'gc_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function gc_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post    = get_post( $post );
	$authors = array();
	$now_gmt = time();
	...
	foreach ( $revisions as $revision ) {
		...
		$autosave = (bool) gc_is_post_autosave( $revision );
		...
		$revisions_data = array(
			...
			'date'       => date_i18n( __( 'Y年n月j日 @ H:i' ), $modified ),
			'dateShort'  => date_i18n( _x( 'n月j日 @ H:i', 'revision date short format' ), $modified ),
			/* translators: %s: Human-readable time difference. */
			'timeAgo'    => sprintf( __( '%s前' ), human_time_diff( $modified_gmt, $now_gmt ) ),
			'autosave'   => $autosave,
			'current'    => $current,
		 ...
		 *     @type string     $title      Title for the revision's parent GC_Post object.
		 ...
		 * }
		 * @param GC_Post $revision       The revision's GC_Post object.
		 * @param GC_Post $post           The revision's parent GC_Post object.
		 */
	...
	if ( 1 === count( $revisions ) ) {
		$revisions[ $post->ID ] = array(
			...
			'title'      => get_the_title( $post->ID ),
			'author'     => $authors[ $revision->post_author ],
			'date'       => date_i18n( __( 'Y年n月j日 @ H:i' ), strtotime( $post->post_modified ) ),
			'dateShort'  => date_i18n( _x( 'n月j日 @ H:i', 'revision date short format' ), strtotime( $post->post_modified ) ),
			/* translators: %s: Human-readable time difference. */
			'timeAgo'    => sprintf( __( '%s前' ), human_time_diff( strtotime( $post->post_modified_gmt ), $now_gmt ) ),
			'autosave'   => false,
			'current'    => true,
			'restoreUrl' => false,
		);
	...
	$diffs = array(
		array(
			...
			'fields' => gc_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
		),
	);

	return array(
		'postId'         => $post->ID,
		'nonce'          => gc_create_nonce( 'revisions-ajax-nonce' ),
 ...
 */
function gc_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( gc_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
	...
	<script id="tmpl-revisions-diff" type="text/html">
		...
		<div class="diff-error"><?php _e( '抱歉，发生了错误。无法加载请求的比较。' ); ?></div>
		<div class="diff">
