<?php
/**
 * List Table API: GC_MS_Themes_List_Table class
 *
 * @package GeChiUI
 ...
 * @see GC_List_Table
 */
class GC_MS_Themes_List_Table extends GC_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->show_autoupdates = gc_is_auto_update_enabled_for_type( 'theme' ) &&
	...
	public function prepare_items() {
		...
		gc_reset_vars( array( 'orderby', 'order', 's' ) );
		...
		$themes = array(
			 ...
			 */
			'all'      => apply_filters( 'all_themes', gc_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : gc_get_themes( array( 'errors' => true ) ),
		);
		...
		foreach ( (array) $themes['all'] as $key => $theme ) {
			...
			$auto_update_forced = gc_is_auto_update_forced_for_item( 'theme', null, $filter_payload );
		...
		GC_Theme::sort_by_name( $this->items );
	...
	public function column_name( $theme ) {
		...
		if ( ! $allowed && ! $this->is_site_themes
			...
			/* translators: %s: Theme name. */
			$aria_label = sprintf( _x( '删除 %s', 'theme' ), $theme->display( 'Name' ) );

			$actions['delete'] = sprintf(
				'<a href="%s" class="delete" aria-label="%s">%s</a>',
				esc_url( gc_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( '删除' )
			);
		}
	...
	public function column_description( $theme ) {
		...
		if ( $theme->get( 'Version' ) ) {
			...
			$theme_meta[] = sprintf( __( '%s版本' ), $theme->display( 'Version' ) );
		}

		/* translators: %s: Theme author. */
		$theme_meta[] = sprintf( __( '作者：%s' ), $theme->display( 'Author' ) );
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'gc_theme_update_row' );
		}
