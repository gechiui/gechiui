<?php
/**
 * Upgrade API: GC_Automatic_Updater class
 *
 * @package GeChiUI
 * @subpackage Upgrader
 */
...
class GC_Automatic_Updater {
	 ...
	 * gc-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		...
		if ( 'core' === $type ) {
			...
			if ( ! $update && gc_is_auto_update_enabled_for_type( $type ) ) {
		 ...
		 *
		 * Since GeChiUI 3.7, minor and development versions of core, and translations have
		 * been auto-updated by default. New installs on GeChiUI 5.6 or higher will also
		...
		if ( 'core' === $type ) {
			...
			if ( file_exists( GC_CONTENT_DIR . '/db.php' ) && empty( $gcdb->is_mysql ) ) {
				...
				$mysql_compat = version_compare( $gcdb->db_version(), $item->mysql_version, '>=' );
			}
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from www.GeChiUI.com sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, GeChiUI won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = GC_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for GCMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = GC_CONTENT_DIR; // GC_LANG_DIR;
				break;
		}
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( '更新至GeChiUI %s' ), $item->version );
				/* translators: %s: GeChiUI version. */
				$item_name = sprintf( __( 'GeChiUI %s' ), $item->version );
				break;
			case 'theme':
				$upgrader_item = $item->theme;
				$theme         = gc_get_theme( $upgrader_item );
				...
				$skin->feedback( __( '升级主题：%s' ), $item_name );
				break;
			case 'plugin':
				...
				$skin->feedback( __( '升级插件：%s' ), $item_name );
				break;
			case 'translation':
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new GC_Error( 'fs_unavailable', __( '无法访问文件系统。' ) );
		}

		if ( 'core' === $type ) {
			if ( is_gc_error( $upgrade_result )
			...
			if ( is_gc_error( $upgrade_result ) ) {
				$upgrade_result->add( 'installation_failed', __( '安装失败' ) );
				$skin->error( $upgrade_result );
			} else {
				$skin->feedback( __( 'GeChiUI升级成功。' ) );
			}
		}
	...
	protected function after_core_update( $update_result ) {
		...
		/*
		 * Any other GC_Error code (like download_failed or files_not_writable) occurs before
		 ...
		 * the issue could actually be on www.GeChiUI.com's side.) If that one fails, then email.
		 */
		$send               = true;
		$transient_failures = array( 'incompatible_archive', 'download_failed', 'insane_distro', 'locked' );
		if ( in_array( $error_code, $transient_failures, true ) && ! get_site_option( 'auto_core_update_failed' ) ) {
			gc_schedule_single_event( time() + HOUR_IN_SECONDS, 'gc_maybe_auto_update' );
			$send = false;
		}
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be GC_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be GC_Error.
		 */
		...
		switch ( $type ) {
			...
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: GeChiUI version. */
				$subject = __( '[%1$s] GeChiUI %2$s现已可用，请更新！' );
				break;
		...
		// If the auto-update is not to the latest version, say that the current version of GC is available instead.
		$version = 'success' === $type ? $core_update->current : $next_user_core_update->current;
		$subject = sprintf( $subject, gc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $version );
		...
		switch ( $type ) {
			case 'success':
				...
				if ( $newer_version_available ) {
					/* translators: %s: GeChiUI latest version. */
					$body .= "\n\n" . sprintf( __( 'GeChiUI %s也已可用。' ), $next_user_core_update->current ) . ' ';
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . __( '如果您遇到了任何问题或需要帮助，www.GeChiUI.com支持论坛的志愿者或许能帮到您。' );
		 ...
		 *                            can be returned, as handled by gc_mail().
		...
		gc_mail( $email['to'], gc_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	}
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( '如果您遇到了任何问题或需要帮助，www.GeChiUI.com支持论坛的志愿者或许能帮到您。' );
		...
		$result = gc_mail( $email['to'], gc_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_debug_email() {
		...
		if ( isset( $this->update_results['core'] ) ) {
			...
			if ( $result->result && ! is_gc_error( $result->result ) ) {
				/* translators: %s: GeChiUI version. */
				$body[] = sprintf( __( '成功：GeChiUI已成功升级至%s' ), $result->name );
			} else {
				/* translators: %s: GeChiUI version. */
				$body[] = sprintf( __( '失败：GeChiUI未能升级至%s' ), $result->name );
				$failures++;
			}
		...
		foreach ( array( 'plugin', 'theme', 'translation' ) as $type ) {
			...
			$success_items = gc_list_filter( $this->update_results[ $type ], array( 'result' => true ) );

			if ( $success_items ) {
				$messages = array(
					'plugin'      => __( '以下插件已成功升级：' ),
					'theme'       => __( '以下主题已成功升级：' ),
					'translation' => __( '以下翻译已成功升级：' ),
				);

				$body[] = $messages[ $type ];
				foreach ( gc_list_pluck( $success_items, 'name' ) as $name ) {
					/* translators: %s: Name of plugin / theme / translation. */
					$body[] = ' * ' . sprintf( __( '成功：%s' ), $name );
				}
			}
			...
			if ( $success_items !== $this->update_results[ $type ] ) {
				...
				$messages = array(
					'plugin'      => __( '以下插件未能升级：' ),
					'theme'       => __( '以下主题未能升级：' ),
					'translation' => __( '以下翻译未能升级：' ),
				);

				$body[] = $messages[ $type ];

				foreach ( $this->update_results[ $type ] as $item ) {
					if ( ! $item->result || is_gc_error( $item->result ) ) {
						/* translators: %s: Name of plugin / theme / translation. */
						$body[] = ' * ' . sprintf( __( '失败：%s' ), $item->name );
						$failures++;
					}
				}
			}
				...
				if ( is_gc_error( $update->result ) ) {
					...
					foreach ( $results as $result_type => $result ) {
						...
						if ( 'rollback' === $result_type ) {
							...
							$body[] = '  ' . sprintf( __( '回滚错误：[%1$s] %2$s' ), $result->get_error_code(), $result->get_error_message() );
						} else {
							/* translators: 1: Error code, 2: Error message. */
							$body[] = '  ' . sprintf( __( '错误：[%1$s] %2$s' ), $result->get_error_code(), $result->get_error_message() );
						}
