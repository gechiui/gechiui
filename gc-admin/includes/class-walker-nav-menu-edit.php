...
class Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	 ...
	 * @param GC_Post  $data_object       Menu item data object.
	 * @param int      $depth             Depth of menu item. Used for padding.
	 * @param stdClass $args              Not used.
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		$_gc_nav_menu_max_depth = $depth > $_gc_nav_menu_max_depth ? $depth : $_gc_nav_menu_max_depth;
		...
		if ( ! empty( $menu_item->_invalid ) ) {
			...
			$title = sprintf( __( '%s（无效）' ), $menu_item->title );
		} elseif ( isset( $menu_item->post_status ) && 'draft' === $menu_item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: Title of a menu item in draft status. */
			$title = sprintf( __( '%s（待定）' ), $menu_item->title );
		}
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode( ' ', $classes ); ?>">
			<div class="menu-item-bar">
				<div class="menu-item-handle">
					<label class="item-title" for="menu-item-checkbox-<?php echo $item_id; ?>">
						...
						<span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( '子项目' ); ?></span>
					</label>
			...
			<div class="menu-item-settings gc-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
				 ...
				 */
				do_action( 'gc_nav_menu_item_custom_fields', $item_id, $menu_item, $depth, $args, $current_object_id );
				?>
				...
				<fieldset class="field-move hide-if-no-js description description-wide">
					...
					<button type="button" class="button-link menus-move menus-move-top" data-dir="top"><?php _e( '最顶' ); ?></button>
				</fieldset>
