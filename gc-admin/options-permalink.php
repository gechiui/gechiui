...
get_current_screen()->add_help_tab(
	array(
		'id'      => 'overview',
		'title'   => __( '概述' ),
		'content' => '<p>' . __( '固定链接是指向您的文章、页面、分类归档和标签归档等网页的固定URL。之所以称之为“固定链接”，是因为链接到每篇文章的URL应该永久固定，不应修改。' ) . '</p>' .
			'<p>' . __( '此界面可以让您选择您的固定链接结构。您可以选择常见的设置或建立自定义URL结构。' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( '固定链接设置' ),
		'content' => '<p>' . __( '固定链接可以包含有用的信息，如发布日期、标题或其他元素。您可以从推荐的固定链接格式种选择，也可以选择“自定义结构”来创建您自己的格式。' ) . '</p>' .
			'<p>' . sprintf(
				/* translators: %s: Percent sign (%). */
				__( '如果您选择了“朴素”以外的选项，您的带有结构标签（被%s包含的词）的通用URL路径也将出现在自定义结构字段，您将可以在那里进一步修改您的路径。' ),
				'<code>%</code>'
			) . '</p>' .
			'<p>' . sprintf(
				/* translators: 1: %category%, 2: %tag% */
				__( '若您某篇文章包含多个分类或标签，且固定链接结构包含%1$s或%2$s，则只有排序最靠前的分类或标签才会显示在固定链接中。' ),
				'<code>%category%</code>',
				'<code>%tag%</code>'
			) . '</p>' .
			'<p>' . __( '调整完成后，记得点击页面下方“保存更改”按钮使设置生效。' ) . '</p>',
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( '自定义结构' ),
		'content' => '<p>' . __( '“可选”字段允许您在文章归档的URL中加入分类名和标签名。比如，“未分类”的文章归档页面的地址可以是<code>/topics/uncategorized</code>（默认情况下是<code>/category/uncategorized</code>）。' ) . '</p>' .
			'<p>' . __( '调整完成后，记得点击页面下方“保存更改”按钮使设置生效。' ) . '</p>',
	)
);

$help_sidebar_content = '<p><strong>' . __( '更多信息：' ) . '</strong></p>' .
						'<p>' . __( '<a href="https://www.gechiui.com/support/settings-permalinks-screen/">固定链接设置文档</a>' ) . '</p>' .
						'<p>' . __( '<a href="https://www.gechiui.com/support/using-permalinks/">固定链接使用文档</a>' ) . '</p>';

if ( $is_nginx ) {
	$help_sidebar_content .= '<p>' . __( '<a href="https://www.gechiui.com/support/nginx/">Nginx配置文档</a>。' ) . '</p>';
}

$help_sidebar_content .= '<p>' . __( '<a href="https://www.gechiui.com/support/">支持</a>' ) . '</p>';
 ...
 * base prefix, GeChiUI core can no longer account for the possible collision.
 */
...
if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['category_base'] ) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$gc_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$gc_rewrite->set_tag_base( $tag_base );
	}
}
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess' ) ) {
		...
		$writable       = false;
		$existing_rules = array_filter( extract_from_markers( $home_path . '.htaccess', 'GeChiUI' ) );
...
$using_index_permalinks = $gc_rewrite->using_index_permalinks();

if ( $structure_updated ) {
	$message = __( '固定链接结构已更新。' );
	...
	gc_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}
