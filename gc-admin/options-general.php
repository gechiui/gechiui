...
$options_help = '<p>' . __( '本页面中的选项决定您站点的基本配置。' ) . '</p>' .
	'<p>' . __( '大多数的主题在每个页面的顶端、浏览器标题栏、feed中显示站点标题。许多主题也显示副标题。' ) . '</p>';

if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( '“GeChiUI地址”和“站点地址”可以相同（example.com），也可以不同。例如，GeChiUI的核心文件（example.com/gechiui）可以不置于根目录，而在子目录。' ) . '</p>' .
		'<p>' . __( '如果您希望允许访客自行注册（另外一种方式是由站点管理员代为注册），请勾选“成员资格”复选框。默认用户角色应用于所有新注册的用户，不论其是由管理员代为注册，抑或是自行注册。' ) . '</p>';
}
...
<tr>
<th scope="row"><label for="blogname"><?php _e( '站点标题' ); ?></label></th>
...
<p class="描述" id="home-description">
		...
		printf(
			...
			__( '如果您想<a href="%s">让您的站点主页与GeChiUI安装目录不同</a>，请在此输入地址。' ),
...
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th scope="row"><label for="GCLANG"><?php _e( '站点语言' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
		<td>
			<?php
			$locale = get_locale();
