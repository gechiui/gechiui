...
if ( apply_filters( 'enable_post_by_email_configuration', true ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'title'   => __( '通过电子邮件发布文章' ),
			'content' => '<p>' . __( '通过电子邮件发布文章的设置能让您通过电子邮件将您文章的内容发送到GeChiUI站点。您必须设置一个支持POP3的秘密电子邮箱账户才能使用此功能。该邮箱接收到的任何邮件都将被作为文章发布，所以将此地址保密是一个好主意。' ) . '</p>',
		)
	);
}
...
if ( apply_filters( 'enable_post_by_email_configuration', true ) ) {
	?>
<h2 class="title"><?php _e( '通过电子邮件发布文章' ); ?></h2>
<p>
	<?php
	printf(
		...
		sprintf( '<kbd>%s</kbd>', gc_generate_password( 8, false ) ),
		sprintf( '<kbd>%s</kbd>', gc_generate_password( 8, false ) ),
		sprintf( '<kbd>%s</kbd>', gc_generate_password( 8, false ) )
	);
	?>
...
<h2 class="title"><?php _e( '更新服务' ); ?></h2>
	...
	<p><label for="ping_sites">
		...
		printf(
			...
			__( '在您发表新文章时，GeChiUI会自动通知站点更新服务。要获取更多资讯，请参见Codex上的<a href="%s">更新服务文档</a>。请以换行分隔多个服务URL。' ),
	...
	<p>
		...
		printf(
			...
			__( '基于您站点的<a href="%2$s">可见性设置</a>，GeChiUI不会通知任何<a href="%1$s">更新服务</a>。' ),
			__( 'https://www.gechiui.com/support/update-services/' ),
			'options-reading.php'
		);
		?>
