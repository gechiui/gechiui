...
/**
 * @namespace gc.mediaWidgets
 * @memberOf  gc
 */
gc.mediaWidgets = ( function( $ ) {
	'use strict';
	 ...
	 *
	 * @memberOf gc.mediaWidgets
	 *
	 * @type {Object.<string, gc.mediaWidgets.MediaWidgetModel>}
	 */
	...
	component.PersistentDisplaySettingsLibrary = gc.media.controller.Library.extend(/** @lends gc.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */{

		/**
		 * Library which persists the customized display settings across selections.
		 *
		 * @constructs gc.mediaWidgets.PersistentDisplaySettingsLibrary
		...
		initialize: function initialize( options ) {
			...
			gc.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = gc.media.controller.Library.prototype.display.call( this, model );
	 ...
	 *
	 * @class    gc.mediaWidgets.MediaEmbedView
	 * @augments gc.media.view.Embed
	 */
	component.MediaEmbedView = gc.media.view.Embed.extend(/** @lends gc.mediaWidgets.MediaEmbedView.prototype */{
		...
		initialize: function( options ) {
			...
			gc.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {gc.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
		refresh: function refresh() {
			/**
			 * @class gc.mediaWidgets~Constructor
			 */
			var Constructor;
			...
			if ( 'image' === this.controller.options.mimeType ) {
				...
				Constructor = gc.media.view.EmbedLink.extend(/** @lends gc.mediaWidgets~Constructor.prototype */{
					...
					updateoEmbed: function() {
						...
						gc.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! gc.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== gc.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = gc.apiRequest({
	 ...
	 *
	 * @class    gc.mediaWidgets.MediaFrameSelect
	 * @augments gc.media.view.MediaFrame.Post
	 */
	component.MediaFrameSelect = gc.media.view.MediaFrame.Post.extend(/** @lends gc.mediaWidgets.MediaFrameSelect.prototype */{
		...
		createStates: function createStates() {
			...
			_.each( gc.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new gc.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new gc.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {gc.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {gc.Backbone.View} view - Toolbar view.
		 * @this {gc.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {gc.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {gc.Backbone.View} toolbar - Toolbar view.
		 * @this {gc.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Embed content.
		 *
		 * Forked override of {gc.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends gc.mediaWidgets.MediaWidgetControl.prototype */{

		/**
		 * Translation strings.
		 *
		 * The mapping of translation strings is handled by media widget subclasses,
		 * exported from PHP to JS such as is done in GC_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in GC_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in GC_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		 ...
		 *
		 * @constructs gc.mediaWidgets.MediaWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new gc.media.model.Attachment();
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.gechiui.com/ticket/33507>.
			 */
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( gc.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new gc.media.model.Attachment({
		...
		template: function template() {
			...
			return gc.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new gc.media.model.Selection( selectionModels, { multiple: false } );
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.gechiui.com/ticket/40403>.
			defaultSync = gc.media.model.Attachment.prototype.sync;
			gc.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				gc.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {gc.media.view.MediaFrame.Select} mediaFrame - Select frame.
		 * @return {Object} Props.
		 */
	 ...
	 * Media widget model.
	 *
	 * @class    gc.mediaWidgets.MediaWidgetModel
	 * @augments Backbone.Model
	 */
	component.MediaWidgetModel = Backbone.Model.extend(/** @lends gc.mediaWidgets.MediaWidgetModel.prototype */{
		 ...
		 * @return {gc.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 * @memberOf gc.mediaWidgets
	 *
	 * @type {Object.<string, gc.mediaWidgets.MediaWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of GC_Widget::form() which is
	 ...
	 * When GeChiUI enqueues this script, it should have an inline script
	 * attached which calls gc.mediaWidgets.init().
	 *
	 * @memberOf gc.mediaWidgets
	 *
	 * @return {void}
