...
jQuery( function($) {
	var welcomePanel = $( '#welcome-panel' ),
		welcomePanelHide = $('#gc_welcome_panel-hide'),
		updateWelcomePanel;
	...
	$('.welcome-panel-close, .welcome-panel-dismiss a', welcomePanel).on( 'click', function(e) {
		...
		$('#gc_welcome_panel-hide').prop('checked', false);
	});
	...
	$( '.meta-box-sortables' ).sortable( 'option', 'containment', '#gcwrap' );
...
jQuery( function( $ ) {
	 ...
	 */
	app = window.gc.communityEvents = /** @lends gc.communityEvents */{
		initialized: false,
		model: null,
		...
		getEvents: function( requestParams ) {
			...
			requestParams._gcnonce = communityEventsData.nonce;
			...
			gc.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					...
					template = gc.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = gc.template( 'community-events-attend-event-near' );
				...
				template = gc.template( 'community-events-could-not-locate' );
		 ...
		 * @param {string} timeZone    A time zone string or offset which is parsable by `gc.date.i18n()`.
		 *
		 * @returns {string}
		 */
...
window.communityEventsData.l10n = window.gc.deprecateL10nObject( 'communityEventsData.l10n', window.communityEventsData.l10n, '5.6.0' );
