...
( function( $ ) {
	...
	var tempID = 0;
	var separator = gc.i18n._x( ',', 'tag delimiter' ) || ',';
	 ...
	 *
	 * Example: `$( element ).gcTagsSuggest( options )`.
	 *
	 ...
	 */
	$.fn.gcTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-gc-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'gc-tags-autocomplete-' + ui.item.id );
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="gc-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
