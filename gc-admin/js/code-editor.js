...
if ( 'undefined' === typeof window.gc.codeEditor ) {
	/**
	 * @namespace gc.codeEditor
	 */
	window.gc.codeEditor = {};
}
...
( function( $, gc ) {
	...
	/**
	 * @typedef {object} gc.codeEditor~CodeEditorInstance
	 ...
	 */
	gc.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, gc.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = gc.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = gc.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
