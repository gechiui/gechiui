/**
 * GeChiUI Administration Navigation Menu
...
(function($) {
	...
	/**
	 * Contains all the functions to handle GeChiUI navigation menus administration.
	 *
	 * @namespace gcNavMenu
	 */
	api = window.gcNavMenu = {
		...
		attachMenuItemDeleteButton : function() {
			...
			$( document ).on( 'click', '.menu-items-delete', function( e ) {
				...
				if ( ! $(this).hasClass( 'disabled' ) ) {
					...
					gc.a11y.speak( deletionSpeech, 'polite' );
					that.disableBulkSelection();
				}
			});
		},
		...
		attachUnsavedChangesListener : function() {
			...
			if ( 0 !== $('#menu-to-edit').length || 0 !== $('.menu-location-menus select').length ) {
				window.onbeforeunload = function(){
					if ( api.menusChanged )
						return gc.i18n.__( '离开这个页面，您所做的更改将丢失。' );
				};
			} else {
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			if( ! $items.length ) {
				$('.categorychecklist', panel).html( '<li><p>' + gc.i18n.__( '未找到结果。' ) + '</p></li>' );
		...
		removeMenuItem : function(el) {
			...
			el.addClass('deleting').animate({
					...
					api.refreshAdvancedAccessibility();
					gc.a11y.speak( menus.itemRemoved );
				});
		},
