...
var getCount, updateCount, updateCountText, updatePending, updateApproved,
	...
	updateHtmlTitle = function( diff ) {
		...
		titleRegEx = titleRegEx || new RegExp( __( '评论（%s）' ).replace( '%s', '\\([0-9' + thousandsSeparator + ']+\\)' ) + '?' );
		...
		if ( titleCount >= 1 ) {
			...
			if ( regExMatch ) {
				/* translators: %s: Comments count. */
				newTitle = document.title.replace( regExMatch[0], __( '评论（%s）' ).replace( '%s', titleDiv.text() ) + ' ' );
			}
		} else {
			regExMatch = titleRegEx.exec( newTitle );
			if ( regExMatch ) {
				newTitle = newTitle.replace( regExMatch[0], __( '评论' ) );
			}
		}
...
window.setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			gcListsData = $( settings.target ).attr( 'data-gc-lists' );
		...
		if ( gcListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( gcListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = gcListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_gcnonce=' + settings.data._ajax_nonce);
			a.attr('data-gc-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		var total_items_i18n, total, animated, animatedCallback,
			...
			/*
			 * As `gcList` toggles only the `unapproved` class, the approved comment
		...
		theList.get(0).gcList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function() {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).gcList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See GC_Comments_List_Table::prepare_items() in class-gc-comments-list-table.php.
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).gcList.add( response.rows );
			}
		});
	};
	 ...
	 * @global
	 */
	window.theExtraList = $('#the-extra-comment-list').gcList( { alt: '', delColor: 'none', addColor: 'none' } );
	 ...
	 * @global
	 */
	window.theList = $('#the-comment-list').gcList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('gcListDelEnd', function(e, s){
			var gcListsData = $(s.target).attr('data-gc-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( gcListsData.indexOf(':trash=1') != -1 || gcListsData.indexOf(':spam=1') != -1 )
...
window.commentReply = {
	...
	show : function(xml) {
		...
		r = gcAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': gcAjax.broken});
			return false;
		}
	...
	discardCommentChanges: function() {
		...
		return window.confirm( __( '你确定要这么做吗？\n您所做的评论更改将丢失。' ) );
	}
};
...
$( function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'gc-toggle', 1 ).trigger( 'click' ).removeData( 'gc-toggle' );
		};
