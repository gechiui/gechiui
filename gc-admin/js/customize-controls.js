...
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = gc.customize;
	...
	api.OverlayNotification = api.Notification.extend(/** @lends gc.customize.OverlayNotification.prototype */{
	...
	api.Notifications = api.Values.extend(/** @lends gc.customize.Notifications.prototype */{
		 ...
		 * @return {gc.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( gc.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					gc.a11y.speak( notification.message, 'assertive' );
				}
	...
	api.Setting = api.Value.extend(/** @lends gc.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is GeChiUI data (theme mod, option, menu, etc.) that the user can
		 * draft changes to in the Customizer.
		 *
		 * @see PHP class GC_Customize_Setting.
		 *
		 ...
		 * @param {Object}  [options.previewer]         - The Previewer instance to sync with. Defaults to gc.customize.previewer.
		 */
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// A status would cause a revision to be made, and for this gc.customize.previewer.save() should be used.
		...
		request = gc.ajax.post( 'customize_save', data );
	...
	Container = api.Class.extend(/** @lends gc.customize~Container.prototype */{
		 ...
		 * @param {string}  [options.type=default] - The type of the panel. See gc.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.gc-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.gc-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = gc.template( container.templateSelector );
			} else {
				template = gc.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend(/** @lends gc.customize.Section.prototype */{
		 ...
		 * @param {string}  [options.type=default] - The type of the section. See gc.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.gc-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.gc-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends gc.customize.ThemesSection.prototype */{
		currentTheme: '',
		overlay: '',
		template: '',
		...
		ready: function() {
			...
			section.template = gc.template( 'customize-themes-details-view' );
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.gc-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.gc-filter-search', function() {
			...
			section.contentContainer.on( 'click', '.no-themes-local .search-dotorg-themes', function() {
				api.section( 'gcorg_themes' ).focus();
			});
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.gc-filter-search' ).val();
							section.contentContainer.find( '.gc-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.gc-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-themes' ).hide();
			request = gc.ajax.post( 'customize_load_themes', params );
			request.done(function( data ) {
				...
				if ( 0 !== themes.length ) {
					...
					if ( 1 === page ) {
						...
						if ( 'local' !== section.params.filter_type ) {
							gc.a11y.speak( api.settings.l10n.themeSearchResults.replace( '%d', data.info.results ) );
						}
					}
					...
					if ( 0 === section.loaded ) {
						...
						gc.a11y.speak( section.container.find( '.no-themes' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				...
				section.headContainer.closest( '.gc-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					gc.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		filterSearch: function( term ) {
			var count = 0,
				visible = false,
				section = this,
				noFilter = ( api.section.has( 'gcorg_themes' ) && 'remote' !== section.params.filter_type ) ? '.no-themes-local' : '.no-themes',
				controls = section.controls(),
				terms;
			...
			if ( 0 === count ) {
				...
				gc.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		...
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.gc-filter-search' ).val();
		...
		updateCount: function( count ) {
			...
			if ( 0 === count ) {
				...
				gc.a11y.speak( api.settings.l10n.announceThemeCount.replace( '%d', count ) );
			}
		},
		...
		showDetails: function ( theme, callback ) {
			...
			gc.a11y.speak( api.settings.l10n.announceThemeDetails.replace( '%s', theme.name ) );
			if ( callback ) {
				callback();
			}
		},
	...
	api.OuterSection = api.Section.extend(/** @lends gc.customize.OuterSection.prototype */{

		/**
		 * Class gc.customize.OuterSection.
		 *
	...
	api.Panel = Container.extend(/** @lends gc.customize.Panel.prototype */{
		containerType: 'panel',
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.gc-full-overlay' ),
				container = accordionSection.closest( '.gc-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = gc.template( panel.templateSelector + '-content' );
			} else {
				template = gc.template( 'customize-panel-default-content' );
			}
	...
	api.ThemesPanel = api.Panel.extend(/** @lends gc.customize.ThemsPanel.prototype */{

		/**
		 *  Class gc.customize.ThemesPanel.
		 *
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.gc-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.gc-full-overlay' );
		...
		installTheme: function( event ) {
			...
			request = gc.updates.installTheme( {
				slug: slug
			} );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'gc-theme-update-success', function( e, response ) {
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'gc-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends gc.customize.Control.prototype */{
		defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
		 ...
		 *
		 * @see PHP class GC_Customize_Control.
		 *
		 * @constructs gc.customize.Control
		 * @augments   gc.customize.Class
		 *
		 * @borrows gc.customize~focus as this#focus
		 * @borrows gc.customize~Container#activate as this#activate
		 * @borrows gc.customize~Container#deactivate as this#deactivate
		 * @borrows gc.customize~Container#_toggleActive as this#_toggleActive
		 *
		...
		renderNotifications: function() {
			...
			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] gc.customize.Control.prototype.renderNotifications() is deprecated in favor of instantating a gc.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = gc.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = gc.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = gc.template( 'available-menu-item' );
	 ...
	 *
	 * @class    gc.customize.ColorControl
	 * @augments gc.customize.Control
	 */
	api.ColorControl = api.Control.extend(/** @lends gc.customize.ColorControl.prototype */{
		ready: function() {
			var control = this,
			...
			if ( isHueSlider ) {
				...
				picker.val( control.setting() ).gcColorPicker({
				...
				picker.val( control.setting() ).gcColorPicker({
					change: function() {
						updating = true;
						control.setting.set( picker.gcColorPicker( 'color' ) );
						updating = false;
					},
					clear: function() {
						updating = true;
			...
			control.setting.bind( function ( value ) {
				...
				picker.val( value );
				picker.gcColorPicker( 'color', value );
			} );
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.gc-picker-container' );
				if ( pickerContainer.hasClass( 'gc-picker-active' ) ) {
					picker.gcColorPicker( 'close' );
					control.container.find( '.gc-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	 ...
	 *
	 * @class    gc.customize.MediaControl
	 * @augments gc.customize.Control
	 */
	api.MediaControl = api.Control.extend(/** @lends gc.customize.MediaControl.prototype */{
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					gc.media.attachment( value ).fetch().done( function() {
						...
						gc.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && gc.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = gc.media({
				...
				states: [
					new gc.media.controller.Library({
						...
						library:   gc.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._gcmejsSettings || {};
	 ...
	 *
	 * @class    gc.customize.UploadControl
	 * @augments gc.customize.MediaControl
	 */
	api.UploadControl = api.MediaControl.extend(/** @lends gc.customize.UploadControl.prototype */{
	 ...
	 *
	 * @class    gc.customize.ImageControl
	 * @augments gc.customize.UploadControl
	 */
	api.ImageControl = api.UploadControl.extend(/** @lends gc.customize.ImageControl.prototype */{
		// @deprecated
	 ...
	 *
	 * @class    gc.customize.BackgroundControl
	 * @augments gc.customize.UploadControl
	 */
	api.BackgroundControl = api.UploadControl.extend(/** @lends gc.customize.BackgroundControl.prototype */{
		...
		select: function() {
			...
			gc.ajax.post( 'custom-background-add', {
				nonce: _gcCustomizeBackground.nonces.add,
				gc_customize: 'on',
				customize_theme: api.settings.theme.stylesheet,
	 ...
	 * @augments gc.customize.Control
	 */
	api.BackgroundPositionControl = api.Control.extend(/** @lends gc.customize.BackgroundPositionControl.prototype */{
	 ...
	 *
	 * @class    gc.customize.CroppedImageControl
	 * @augments gc.customize.MediaControl
	 */
	api.CroppedImageControl = api.MediaControl.extend(/** @lends gc.customize.CroppedImageControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = gc.media({
				...
				states: [
					...
					new gc.media.controller.CustomizeImageCropper({
		 ...
		 * @param {gc.media.controller.Cropper} controller
	 ...
	 *
	 * @class    gc.customize.SiteIconControl
	 * @augments gc.customize.CroppedImageControl
	 */
	api.SiteIconControl = api.CroppedImageControl.extend(/** @lends gc.customize.SiteIconControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = gc.media({
				...
				states: [
					...
					new gc.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	/**
	 * @class    gc.customize.HeaderControl
	 * @augments gc.customize.Control
	 */
	api.HeaderControl = api.Control.extend(/** @lends gc.customize.HeaderControl.prototype */{
		ready: function() {
			...
			gc.media.controller.Cropper.prototype.defaults.doCropArgs.gc_customize = 'on';
			gc.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _gcCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_gcCustomizeHeader.data.width, 10),
				yInit = parseInt(_gcCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_gcCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_gcCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = gc.media({
				...
				states: [
					new gc.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _gcCustomizeHeader.data.width,
						suggestedHeight: _gcCustomizeHeader.data.height
					}),
					new gc.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new gc.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * gc.customize.ThemeControl
	 *
	 * @class    gc.customize.ThemeControl
	 * @augments gc.customize.Control
	 */
	api.ThemeControl = api.Control.extend(/** @lends gc.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in GeChiUI 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	 ...
	 * @augments gc.customize.Control
	 */
	api.CodeEditorControl = api.Control.extend(/** @lends gc.customize.CodeEditorControl.prototype */{
		...
		initEditor: function() {
			...
			if ( gc.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = gc.codeEditor.defaultSettings ? _.clone( gc.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = gc.codeEditor.initialize( $textarea, settings );
	 ...
	 * @augments gc.customize.Control
	 */
	api.DateTimeControl = api.Control.extend(/** @lends gc.customize.DateTimeControl.prototype */{
	 ...
	 * @augments gc.customize.Control
	 */
	api.PreviewLinkControl = api.Control.extend(/** @lends gc.customize.PreviewLinkControl.prototype */{
	 ...
	 *
	 * @alias gc.customize.defaultConstructor
	 */
	 ...
	 *
	 * @callback gc.customize.deferredControlsCallback
	 * @param {gc.customize.Control[]} controls Resolved controls.
	 */
	 ...
	 * @return {gc.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * gc.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = gc.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = gc.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * gc.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = gc.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * gc.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = gc.customize.control( 'background_color ').setting.get();
	 * value = gc.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * gc.customize.control.add( new gc.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * gc.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * gc.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * gc.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 *
	 * @callback gc.customize.deferredSectionsCallback
	 * @param {gc.customize.Section[]} sections Resolved sections.
	 */
	 ...
	 * @return {gc.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested sections.
	 *
	 * @example <caption>Loop over all registered sections.</caption>
	 * gc.customize.section.each( function( section ) { ... } )
	 *
	 * @example <caption>Getting `title_tagline` section instance.</caption>
	 * section = gc.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * gc.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link gc.customize.control} for further examples of how to interact with {@link gc.customize.Values} instances.
	 */
	 ...
	 *
	 * @callback gc.customize.deferredPanelsCallback
	 * @param {gc.customize.Panel[]} panels Resolved panels.
	 */
	 ...
	 * @return {gc.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param),
	 *                                                       or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * gc.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = gc.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * gc.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link gc.customize.control} for further examples of how to interact with {@link gc.customize.Values} instances.
	 ...
	 *
	 * @callback gc.customize.deferredNotificationsCallback
	 * @param {gc.customize.Notification[]} notifications Resolved notifications.
	 */
	 ...
	 * @return {gc.customize.Notification|undefined|jQuery.promise} Notification instance or undefined (if function called with one code param),
	 *                                                              or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = gc.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = gc.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * gc.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * gc.customize.notifications.add( new gc.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * gc.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link gc.customize.control} for further examples of how to interact with {@link gc.customize.Values} instances.
	 */
	api.notifications = new api.Notifications();

	api.PreviewFrame = api.Messenger.extend(/** @lends gc.customize.PreviewFrame.prototype */{
		 ...
		 *
		 * @constructs gc.customize.PreviewFrame
		 * @augments   gc.customize.Messenger
		 *
		...
		run: function( deferred ) {
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See GC Trac #38149.
	...
	api.Previewer = api.Messenger.extend(/** @lends gc.customize.Previewer.prototype */{
		refreshBuffer: null, // Will get set to api.settings.timeouts.windowRefresh.

		/**
		 * @constructs gc.customize.Previewer
		 * @augments   gc.customize.Messenger
		 *
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in gc-includes or gc-content.
				if ( /\/gc-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
		...
		refreshNonces: function() {
			...
			request = gc.ajax.post( 'customize_refresh_nonces', {
				gc_customize: 'on',
				customize_theme: api.settings.theme.stylesheet
			});
	...
	$( function() {
		api.settings = window._gcCustomizeSettings;
		api.l10n = window._gcCustomizeControlsL10n;
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.gc-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = gc.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = gc.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /gc_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.gc-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via gc.customizer.previewer.save() will update selection.
			saved( true );
		...
		( function checkAndDisplayLockNotice() {
			...
			var LockedNotification = api.OverlayNotification.extend(/** @lends gc.customize~LockedNotification.prototype */{
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = gc.ajax.post( 'customize_override_changeset_lock', {
							gc_customize: 'on',
							customize_theme: api.settings.theme.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				gc.ajax.post( 'customize_dismiss_autosave_or_lock', {
					gc_customize: 'on',
					customize_theme: api.settings.theme.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.gc-full-overlay-sidebar-content' ),
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.gc-customize-changeset-update' );
					$( window ).off( 'beforeunload.gc-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					gc.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_gcCustomizeBackground.defaults['default-position-x'],
				_gcCustomizeBackground.defaults['default-position-y'],
				_gcCustomizeBackground.defaults['default-size'],
				_gcCustomizeBackground.defaults['default-repeat'],
				_gcCustomizeBackground.defaults['default-attachment']
			];
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.gc-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.gc-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
