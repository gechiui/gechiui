...
gc_reset_vars( array( 'action', 'option_page' ) );
...
if ( ! empty( $_GET['adminhash'] ) ) {
	...
	gc_redirect( admin_url( $redirect ) );
	exit;
} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' === $_GET['dismiss'] ) {
	check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
	delete_option( 'adminhash' );
	delete_option( 'new_admin_email' );
	gc_redirect( admin_url( 'options-general.php?updated=true' ) );
	exit;
}
...
if ( 'update' === $action ) { // We are saving settings sent from a settings page.
	...
	if ( ! isset( $allowed_options[ $option_page ] ) ) {
		gc_die(
			sprintf(
				...
				__( '<strong>错误</strong>：选项页面%s不在允许名单中。' ),
	...
	if ( 'general' === $option_page ) {
		...
		if ( ! empty( $_POST['date_format'] ) && isset( $_POST['date_format_custom'] )
			&& '\c\u\s\t\o\m' === gc_unslash( $_POST['date_format'] )
		) {
			$_POST['date_format'] = $_POST['date_format_custom'];
		}

		if ( ! empty( $_POST['time_format'] ) && isset( $_POST['time_format_custom'] )
			&& '\c\u\s\t\o\m' === gc_unslash( $_POST['time_format'] )
		) {
		...
		if ( ! empty( $_POST['GCLANG'] ) && current_user_can( 'install_languages' ) ) {
			require_once ABSPATH . 'gc-admin/includes/translation-install.php';

			if ( gc_can_install_language_pack() ) {
	...
	if ( $options ) {
		...
		foreach ( $options as $option ) {
			if ( $unregistered ) {
				_deprecated_argument(
					...
					sprintf(
						...
						__( '%s设置已被取消注册。系统会将取消注册的设置废弃不用，请参见https://developer.gechiui.com/plugins/settings/settings-api/' ),
			...
			if ( isset( $_POST[ $option ] ) ) {
				...
				$value = gc_unslash( $value );
			}
			update_option( $option, $value );
		}

		/*
		 * Switch translation in case GCLANG was changed.
	...
	$goback = add_query_arg( 'settings-updated', 'true', gc_get_referer() );
	gc_redirect( $goback );
	exit;
}
...
<?php
$options = $gcdb->get_results( "SELECT * FROM $gcdb->options ORDER BY option_name" );
...
<?php submit_button( __( '保存更改' ), 'primary', 'Update' ); ?>
