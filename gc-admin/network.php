...
if ( is_multisite() ) {
	...
	if ( ! defined( 'MULTISITE' ) ) {
		gc_die( __( '站点网络创建面板不适用于 GeChiUI MU 网络。' ) );
	}
}
...
if ( ! network_domain_check() && ( ! defined( 'GC_ALLOW_MULTISITE' ) || ! GC_ALLOW_MULTISITE ) ) {
	gc_die(
		printf(
			...
			__( '您必须在您的%2$s文件中将%1$s常量设置为“true”（不带中文引号）才能创建站点网络。' ),
...
if ( is_network_admin() ) {
	...
	// Used in the HTML title tag.
	$title       = __( '创建GeChiUI站点网络' );
	$parent_file = 'tools.php';
}

$network_help = '<p>' . __( '您可以在本页面配置使用子域名（<code>site1.example.com</code>）或子目录（<code>example.com/site1</code>）的站点网络。若使用子域名，您需要在Apache和DNS记录中启用泛域名。' ) . '</p>' .
	'<p>' . __( '选择子域名或子目录；此设置只能在事后通过重新配置您的站点网络来更改。填写站点网络详情，然后点击安装。若不起作用，您可能需要添加一个通配 DNS 记录（对于子域名）或修改固定链接的设置（对于子目录）。' ) . '</p>' .
	'<p>' . __( '在“配置网络”的下一个页面，GeChiUI将向您提供专为您生成的几行代码，请将它们按要求加入到gc-config.php和.htaccess文件中。请确保您的FTP客户端不隐藏以点（.）开头的文件，这样您才能看到.htaccess文件；若它确实不存在，您需手工创建这个文件。请在对文件作出更改前，备份这两个文件。' ) . '</p>' .
	'<p>' . __( '加入如下内容到gc-config.php（在<code>/*...stop editing...*/或/*...停止编辑...*/</code>上方）和<code>.htaccess</code>（替换GeChiUI原来生成的内容）。' ) . '</p>' .
	'<p>' . __( '在您添加完代码后，请在浏览器刷新页面，之后多站点功能就应该自动启用了。这个页面将仍然保留这段代码，以备日后使用。您可在“管理网络”界面的导航菜单中再次访问本页面来查看代码。用户可以通过顶部“工具栏”中的“我的站点”下拉菜单在“管理网络”和“管理站点”之间切换。' ) . '</p>' .
	'<p>' . __( '若本站点网络配置完成已经超过一个月了。由于主站点“/blog/”固定链接的问题，您不能选择使用子目录。此问题将很快在未来版本中解决。' ) . '</p>' .
	'<p><strong>' . __( '更多信息：' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://www.gechiui.com/support/create-a-network/">站点网络创建文档</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://www.gechiui.com/support/tools-network-screen/">站点网络界面文档</a>' ) . '</p>';
