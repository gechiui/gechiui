...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #gcadminbar (100000).
if ( ! tinymce.ui.FloatPanel.zIndex || tinymce.ui.FloatPanel.zIndex < 100100 ) {
	tinymce.ui.FloatPanel.zIndex = 100100;
}

tinymce.PluginManager.add( 'gechiui', function( editor ) {
	var gcAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
	...
	function toggleToolbars( state ) {
		...
		if ( ! initial ) {
			if ( state === 'hide' ) {
				...
				gcAdvButton && gcAdvButton.active( false );
			} else {
				setUserSetting( 'hidetb', '1' );
				gcAdvButton && gcAdvButton.active( true );
			}
		}

		editor.fire( 'gc-toolbar-toggle' );
	}
	...
	editor.addButton( 'gc_adv', {
		...
		onPostRender: function() {
			gcAdvButton = this;
			gcAdvButton.active( getUserSetting( 'hidetb' ) === '1' );
		}
	});

	// Hide the toolbars after loading.
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'gechiui_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		} else {
	...
	editor.addCommand( 'GC_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.gcActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-gc-more="more" data-gc-more-text="' + moretext + '" ' +
						'class="gc-more-tag mce-gc-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-gc-more="nextpage" class="gc-more-tag mce-gc-nextpage" ' +
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-gc-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-gc-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-gc-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-gc-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-gc-preserve' ) !== -1 ) {
					if ( match = image.match( / data-gc-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-gc-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands.
	editor.addCommand( 'GC_More', function( tag ) {
		var parent, html, title,
			classname = 'gc-more-tag',
			dom = editor.dom,
	...
	editor.addCommand( 'GC_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'GC_Page', function() {
	...
	editor.addCommand( 'GC_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl+Alt+字母：' ) : __( 'Shift+Alt+字母：' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.gc_shortcut_labels,
		...
		header = [ __( '字母' ), __( '操作' ), __( '字母' ), __( '操作' ) ];
		header = '<tr><th>' + header.join( '</th><th>' ) + '</th></tr>';

		html = '<div class="gc-editor-help">';

		// Main section, default and additional shortcuts.
		html = html +
			'<h2>' + __( '默认快捷方式，' ) + ' ' + meta + '</h2>' +
			'<table class="gc-help-th-center fixed">' +
				header +
				table1.join('') +
			'</table>' +
			'<h2>' + __( '额外的快捷方式，' ) + ' ' + access + '</h2>' +
			'<table class="gc-help-th-center fixed">' +
				header +
				table2.join('') +
			'</table>';

		if ( editor.plugins.gctextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-gc-help' );
	...
	editor.addCommand( 'GC_Medialib', function() {
		if ( gc && gc.media && gc.media.editor ) {
			gc.media.editor.open( editor.id );
		}
	});

	// Register buttons.
	editor.addButton( 'gc_more', {
		tooltip: '插入“More”标签',
		onclick: function() {
			editor.execCommand( 'GC_More', 'more' );
		}
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'gc_lang_attr' ) );
		...
		if ( editor.getParam( 'gc_paste_filters', true ) ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			...
			event.content = event.content.replace( /-->\s*<!-- gc:/g, '-->\n\n<!-- gc:' );
		}
	});
	...
	editor.on( 'preInit', function() {
		...
		if ( ! editor.settings.classic_block_editor ) {
			editor.addShortcut( 'access+z', '', 'GC_Adv' );
		}
		...
		if ( tinymce.Env.mac ) {
			tinymce.$( editor.iframeElement ).attr( 'title', __( '富文本区域。按Control-Option-H获取帮助。' ) );
		}
	} );
	...
	function getTooltip( tooltip ) {
		...
		if ( gcTooltips.hasOwnProperty( translated ) ) {
			label = gcTooltips[ translated ];
		} else if ( gcTooltips.hasOwnProperty( tooltip ) ) {
			label = gcTooltips[ tooltip ];
		}
		...
		each( editor.theme.panel.find( 'listbox' ), function( listbox ) {
			if ( listbox && listbox.settings.text === 'Paragraph' ) {
				each( listbox.settings.values, function( item ) {
					if ( item.text && gcTooltips.hasOwnProperty( item.text ) ) {
						item.shortcut = '(' + gcTooltips[ item.text ] + ')';
					}
				} );
			}
		} );
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			gcAdminbar = document.getElementById( 'gcadminbar' ),
			...
			if ( editor.id === 'content' ) {
				gcStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					gcAdminbarBottom = gcAdminbar ? gcAdminbar.getBoundingClientRect().bottom : 0,
					...
					gcStatusbarTop = gcStatusbar ? windowHeight - gcStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, gcAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, gcStatusbarTop, windowHeight - iframeRect.bottom ),
		...
		editor.gc = editor.gc || {};
		editor.gc._createToolbar = create;
	}, true );
