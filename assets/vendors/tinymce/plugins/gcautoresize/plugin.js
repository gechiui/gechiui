...
// Forked for GeChiUI so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'gcautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'gc-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'gc-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'gc-autoresize' );
	...
	function off() {
		...
		if ( ! settings.gc_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'gc-autoresize' );
	...
	editor.addCommand( 'gcAutoResize', resize );

	// On/off.
	editor.addCommand( 'gcAutoResizeOn', on );
	editor.addCommand( 'gcAutoResizeOff', off );
});
