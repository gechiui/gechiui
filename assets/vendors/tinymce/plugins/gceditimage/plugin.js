/* global tinymce */
tinymce.PluginManager.add( 'gceditimage', function( editor ) {
	...
	editor.addButton( 'gc_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'gc_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'gcAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.gc-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:gc_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:gc_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'gceditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="gc-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="gc-caption-dt">'+ img +'</dt><dd class="gc-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+gc-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /gc-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^gc-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'gc-image-', '' ), 10 );
		...
		// Extract caption.
		captionBlock = dom.getParents( imageNode, '.gc-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.gc-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( $imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'gc-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.gc-caption', captionNode );
				...
				dd = dom.select( '.gc-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="gc-caption-dt"></dt><dd class="gc-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.gc-caption-dt' ).append( node );
		...
		if ( gc.media.events ) {
			gc.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		gc.media.events.trigger( 'editor:frame-create', { frame: frame } );
		...
		frame.on( 'close', function() {
			 ...
			 */
			imageNode = editor.$( 'img[data-gc-editing]' )
			imageNode.removeAttr( 'data-gc-editing' );
		});
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'gceditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Prevent IE11 from making dl.gc-caption resizable.
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+.
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.gc-caption' ) ) {
					// Hide the thick border with resize handles around dl.gc-caption.
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'gc-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.gc-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.gc-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'gcAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.gc-caption' );
	...
	editor.gcSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.gcGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__gc-temp-img-id"]' ).removeAttr( 'id' );
		}
	});

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.gcSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.gcGetImgCaption( event.content );
		}
	});
	...
	// Add to editor.gc.
	editor.gc = editor.gc || {};
	editor.gc.isPlaceholder = isPlaceholder;
