/* global pluploadL10n, plupload, _gcPluploadSettings */
...
( function( exports, $ ) {
	...
	if ( typeof _gcPluploadSettings === 'undefined' ) {
		return;
	}
	 ...
	 * GeChiUI back end and the GeChiUI media experience.
	 *
	 * @class
	 * @memberOf gc
	 * @alias gc.Uploader
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__gc-uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-gc-upload-attachment-id:\s*(\d+)/i );
			...
			if ( times && times > 4 ) {
				...
				$.ajax({
					...
					data: {
						...
						_gcnonce: _gcPluploadSettings.defaults.multipart_params._gcnonce,
						attachment_id: id,
		 ...
		 * and display errors. @see gc.Uploader.errors.
		 *
		 * @param {string}        message Error message.
		 * @param {object}        data    Error data from Plupload.
		 * @param {plupload.File} file    File that was uploaded.
		...
		fileUploaded = function( up, file, response ) {
			...
			gc.media.model.Attachment.get( response.data.id, file.attachment );
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.gc-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.gc-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.gc-uploader, drop.gc-uploader', function() {
				/*
				 ...
				 *
				 * @see https://core.trac.gechiui.com/ticket/21705
				 */
		...
		this.uploader.bind( 'FilesAdded', function( up, files ) {
			_.each( files, function( file ) {
				...
				attributes = _.extend({
					...
					filename:  file.name,
					menuOrder: 0,
					uploadedTo: gc.media.model.settings.post.id
				...
				file.attachment = gc.media.model.Attachment.create( attributes );
	...
	$.extend( Uploader, _gcPluploadSettings );
	...
	$.extend( Uploader.prototype, /** @lends gc.Uploader.prototype */{
		/**
		...
		/**
		 * Make a few internal event callbacks available on the gc.Uploader object
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'gc-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="gc-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'gc-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
	...
	Uploader.queue = new gc.media.model.Attachments( [], { query: false });
