...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.gc.revisions.
	 */
	revisions = gc.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._gcRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			gc.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				gc.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = gc.ajax.send( options ),
	...
	/**
	 * gc.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see gc.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = gc.Backbone.View.extend({
		className: 'revisions',
		template: gc.template('revisions-frame'),
		...
		render: function() {
			gc.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#gcbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = gc.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.gc.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.gc.revisions.pinning click.gc.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.gc.revisions.pinning');
	...
	// The tickmarks view.
	revisions.view.Tickmarks = gc.Backbone.View.extend({
	...
	// The metabox view.
	revisions.view.Metabox = gc.Backbone.View.extend({
	...
	revisions.view.Meta = gc.Backbone.View.extend({
		template: gc.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = gc.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: gc.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = gc.Backbone.View.extend({
	...
	revisions.view.Buttons = gc.Backbone.View.extend({
		className: 'revisions-buttons',
		template: gc.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = gc.Backbone.View.extend({
		className: 'gc-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.gc.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.gc.revisions');
	...
	revisions.view.Diff = gc.Backbone.View.extend({
		className: 'revisions-diff',
		template:  gc.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// gc_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// gc_localize_script does not allow for top-level booleans so do a comparator here.
