...
(function( window, undefined ) {
	...
	function GC_API() {
		...
		this.models = {};
		/** @namespace gc.api.collections */
		this.collections = {};
		/** @namespace gc.api.views */
		this.views = {};
	}
	...
	window.gc            = window.gc || {};
	/** @namespace gc.api */
	gc.api               = gc.api || new GC_API();
	gc.api.versionString = gc.api.versionString || 'gc/v2/';
...
(function( window, undefined ) {
	...
	/** @namespace gc.api */
	gc.api = gc.api || {};
	/** @namespace gc.api.utils */
	gc.api.utils = gc.api.utils || {};
	 ...
	 */
	gc.api.getModelByRoute = function( route ) {
		return _.find( gc.api.models, function( model ) {
	 ...
	 */
	gc.api.getCollectionByRoute = function( route ) {
		return _.find( gc.api.collections, function( collection ) {
	 ...
	 * @param {Date} date.
	 */
	gc.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	gc.api.utils.getRootUrl = function() {
	 ...
	 */
	gc.api.utils.capitalize = function( str ) {
	 ...
	 */
	gc.api.utils.capitalizeAndCamelCaseDashes = function( str ) {
		if ( _.isUndefined( str ) ) {
			return str;
		}
		str = gc.api.utils.capitalize( str );

		return gc.api.utils.camelCaseDashes( str );
	};

	/**
	 * Helper function to camel case the letter after dashes, removing the dashes.
	 */
	gc.api.utils.camelCaseDashes = function( str ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `gc.api.versionString`.
	 * @param {boolean} [reverse]       Whether to reverse the order when extracting the route part. Optional, default false.
	 */
	gc.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		var routeParts;

		part = part || 1;
		versionString = versionString || gc.api.versionString;
	 ...
	 */
	gc.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	gc.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	gc.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. GeChiUI expects dates in UTC.
				 ...
				 *
				 * GeChiUI returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				getDate: function( field ) {
					...
					return new Date( gc.api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new gc.api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a gc.api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new gc.api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				getTags: function() {
					var tagIds = this.get( 'tags' ),
						tags  = new gc.api.collections.Tags();
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new gc.api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new gc.api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new gc.api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new gc.api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new gc.api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new gc.api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new gc.api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', 'gc:featuredmedia', 'source_url' );
				}
			};
...
(function() {
	...
	var gcApiSettings = window.gcApiSettings || {},
	trashableTypes    = [ 'Comment', 'Media', 'Comment', 'Post', 'Page', 'Status', 'Taxonomy', 'Type' ];

	/**
	 * Backbone base model for all models.
	 */
	gc.api.GCApiBaseModel = Backbone.Model.extend(
		/** @lends GCApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-GC-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-GC-Nonce' );
	 ...
	 */
	gc.api.models.Schema = gc.api.GCApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				gc.api.GCApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || gcApiSettings.root;
				model.versionString = options.versionString || gcApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	gc.api.GCApiBaseCollection = Backbone.Collection.extend(
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-gc-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-gc-total' ), 10 );
						}
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		gcApiSettings = window.gcApiSettings || {};
	...
	// If gcApiSettings is unavailable, try the default.
	if ( _.isEmpty( gcApiSettings ) ) {
		gcApiSettings.root = window.location.origin + '/gc-json/';
	}

	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		defaults: {
			apiRoot: gcApiSettings.root,
			versionString: gc.api.versionString,
			nonce: null,
			schema: null,
			models: {},
			collections: {}
		...
		initialize: function() {
			...
			model.schemaModel = new gc.api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( gcApiSettings.cacheSchema ) || gcApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'gc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'gc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( gcApiSettings.cacheSchema ) || gcApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'gc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable gcApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = gcApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( gc.api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
					routeName  = gc.api.utils.extractRoutePart( modelRoute.index, 2, routeModel.get( 'versionString' ), true ),
					parentName = gc.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), false ),
					routeEnd   = gc.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), true );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = gc.api.utils.capitalizeAndCamelCaseDashes( parentName ) + gc.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = gc.api.GCApiBaseModel.extend( {
					...
					modelClassName = gc.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = gc.api.GCApiBaseModel.extend( {
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = gc.api.utils.extractRoutePart( collectionRoute.index, 1, routeModel.get( 'versionString' ), false );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {

					collectionClassName = gc.api.utils.capitalizeAndCamelCaseDashes( parentName ) + gc.api.utils.capitalizeAndCamelCaseDashes( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = gc.api.GCApiBaseCollection.extend( {
					...
					collectionClassName = gc.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = gc.api.GCApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				gc.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = gc.api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	gc.api.endpoints = new Backbone.Collection();

	/**
	 * Initialize the gc-api, optionally passing the API root.
	 *
	 * @param {Object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to gcApiSettings.nonce.
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to gcApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to gcApiSettings.root.
	 * @param {Object} [args.schema] The schema. Optional, will be fetched from API if not provided.
	 */
	gc.api.init = function( args ) {
		var endpoint, attributes = {}, deferred, promise;

		args                      = args || {};
		attributes.nonce          = _.isString( args.nonce ) ? args.nonce : ( gcApiSettings.nonce || '' );
		attributes.apiRoot        = args.apiRoot || gcApiSettings.root || '/gc-json';
		attributes.versionString  = args.versionString || gcApiSettings.versionString || 'gc/v2/';
		attributes.schema         = args.schema || null;
		attributes.modelEndpoints = args.modelEndpoints || [ 'me', 'settings' ];
		if ( ! attributes.schema && attributes.apiRoot === gcApiSettings.root && attributes.versionString === gcApiSettings.versionString ) {
			attributes.schema = gcApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {

			// Look for an existing copy of this endpoint.
			endpoint = gc.api.endpoints.findWhere( { 'apiRoot': attributes.apiRoot, 'versionString': attributes.versionString } );
			if ( ! endpoint ) {
			...
			endpoint.schemaConstructed.done( function( resolvedEndpoint ) {
				gc.api.endpoints.add( resolvedEndpoint );

				// Map the default endpoints, extending any already present items (including Schema model).
				gc.api.models      = _.extend( gc.api.models, resolvedEndpoint.get( 'models' ) );
				gc.api.collections = _.extend( gc.api.collections, resolvedEndpoint.get( 'collections' ) );
	...
	// The gc.api.init function returns a promise that will resolve with the endpoint once it is ready.
	gc.api.loadPromise = gc.api.init();
