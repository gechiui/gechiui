...
/** @namespace gc.customize.selectiveRefresh */
gc.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @memberOf gc.customize.selectiveRefresh
	 *
	 * @see PHP class GC_Customize_Partial.
	 *
	 * @class
	...
	Partial = self.Partial = api.Class.extend(/** @lends gc.customize.SelectiveRefresh.Partial.prototype */{
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( gc.emoji && gc.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = gc.emoji.parse( content );
				}
			...
			/*
			 * Note that the 'gc_audio_shortcode_library' and 'gc_video_shortcode_library' filters
			 * will determine whether or not gc.mediaelement is loaded and whether it will
			 * initialize audio and video respectively. See also https://core.trac.gechiui.com/ticket/40144
			 */
			if ( gc.mediaelement ) {
	...
	self.Placement = Placement = api.Class.extend(/** @lends gc.customize.selectiveRefresh.prototype */{
		 ...
		 *
		 * @param {gc.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	...
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = gc.ajax.send( null, {
					data: data,
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to gc.customize
