...
( function( api, gc, $ ) {
	...
	/**
	 * Set up gcNavMenu for drag and drop.
	 */
	gcNavMenu.originalInit = gcNavMenu.init;
	gcNavMenu.options.menuItemDepthPerLevel = 20;
	gcNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	gcNavMenu.options.targetTolerance       = 10;
	gcNavMenu.init = function() {
		this.jQueryExtensions();
	};

	/**
	 * @namespace gc.customize.Menus
	 */
	api.Menus = api.Menus || {};
	...
	if ( 'undefined' !== typeof _gcCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _gcCustomizeNavMenusSettings );
	}
	 ...
	 *
	 * @alias gc.customize.Menus.generatePlaceholderAutoIncrementId
	 *
	 * @return {number}
	 */
	...
	/**
	 * gc.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's GC_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @class    gc.customize.Menus.AvailableItemModel
	...
	/**
	 * gc.customize.Menus.AvailableItemCollection
	 *
	 * Collection for available menu item models.
	 *
	 * @class    gc.customize.Menus.AvailableItemCollection
	 * @augments Backbone.Collection
	 */
	api.Menus.AvailableItemCollection = Backbone.Collection.extend(/** @lends gc.customize.Menus.AvailableItemCollection.prototype */{
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = gc.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	api.Menus.AvailableMenuItemsPanelView = gc.Backbone.View.extend(/** @lends gc.customize.Menus.AvailableMenuItemsPanelView.prototype */{
		...
		/**
		 * gc.customize.Menus.AvailableMenuItemsPanelView
		 *
		 * View class for the available menu items panel.
		 *
		 * @constructs gc.customize.Menus.AvailableMenuItemsPanelView
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				gc.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = gc.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					gc.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					gc.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = gc.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( gc.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * gc.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the GC_Customize_Menu_Panel::$type.
	 *
	 * @class    gc.customize.Menus.MenusPanel
	 * @augments gc.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend(/** @lends gc.customize.Menus.MenusPanel.prototype */{
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					gc.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = gc.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * gc.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the GC_Customize_Menu_Section::$type.
	 *
	 * @class    gc.customize.Menus.MenuSection
	 * @augments gc.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend(/** @lends gc.customize.Menus.MenuSection.prototype */{
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				gcNavMenu.menuList = section.contentContainer;
				gcNavMenu.targetList = gcNavMenu.menuList;

				// Add attributes needed by gcNavMenu.
				$( '#menu-to-edit' ).removeAttr( 'id' );
				gcNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						gcNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
						section.deferred.initSortables.resolve( gcNavMenu.menuList ); // Now MenuControl can extend the sortable.

						// @todo Note that gc.customize.reflowPaneContents() is debounced,
	...
	/**
	 * gc.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 *
	 * @class    gc.customize.Menus.NewMenuSection
	 * @augments gc.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend(/** @lends gc.customize.Menus.NewMenuSection.prototype */{
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				gc.template( 'nav-menu-create-menu-section-title' )
			);
		...
		submit: function() {
			...
			gc.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * gc.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the GC_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @class    gc.customize.Menus.MenuLocationControl
	 * @augments gc.customize.Control
	 */
	api.Menus.MenuLocationControl = api.Control.extend(/** @lends gc.customize.Menus.MenuLocationControl.prototype */{
		initialize: function( id, options ) {
			var control = this,
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends gc.customize.Menus.MenuItemControl.prototype */{

		/**
		 * gc.customize.Menus.MenuItemControl
		 *
		 * Customizer control for menu items.
		 * Note that 'menu_item' must match the GC_Customize_Menu_Item_Control::$type.
		 *
		 * @constructs gc.customize.Menus.MenuItemControl
		 * @augments   gc.customize.Control
		 *
		 * @inheritDoc
		 */
		...
		_setupReorderUI: function() {
			...
			template = gc.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					gc.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {gc.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			gc.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			gc.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			gc.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			gc.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * gc.customize.Menus.MenuNameControl
	 *
	 * Customizer control for a nav menu's name.
	 *
	 * @class    gc.customize.Menus.MenuNameControl
	 * @augments gc.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend(/** @lends gc.customize.Menus.MenuNameControl.prototype */{
	...
	/**
	 * gc.customize.Menus.MenuLocationsControl
	 *
	 ...
	 * @augments gc.customize.Control
	 */
	api.Menus.MenuLocationsControl = api.Control.extend(/** @lends gc.customize.Menus.MenuLocationsControl.prototype */{
	...
	/**
	 * gc.customize.Menus.MenuAutoAddControl
	 *
	 * Customizer control for a nav menu's auto add.
	 *
	 * @class    gc.customize.Menus.MenuAutoAddControl
	 * @augments gc.customize.Control
	 */
	api.Menus.MenuAutoAddControl = api.Control.extend(/** @lends gc.customize.Menus.MenuAutoAddControl.prototype */{
	...
	/**
	 * gc.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the GC_Menu_Customize_Control::$type
	 *
	 * @class    gc.customize.Menus.MenuControl
	 * @augments gc.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend(/** @lends gc.customize.Menus.MenuControl.prototype */{
		/**
		 * Set up the control.
		 */
		ready: function() {
		 ...
		 *
		 * Notice that the UI aspects here are handled by gcNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that gcNavMenu will be setting this .menu-item-data-parent-id input's value.
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						gc.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				gc.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				gc.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {gc.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @return {gc.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			gc.a11y.speak( api.Menus.data.l10n.itemAdded );
	...
	/**
	 * Extends gc.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends gc.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends gc.customize.sectionConstructor with section constructor for menu.
	 */
	 ...
	 *
	 * @alias gc.customize.Menus.applySavedData
	 *
	 * @param {Object} data
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in GC_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @alias gc.customize.Menus.focusMenuItemControl
	 *
	 ...
	 *
	 * @alias gc.customize.Menus.getMenuControl
	 *
	 * @param menuId
	 * @return {gc.customize.controlConstructor.menus[]}
	 */
	 ...
	 *
	 * @alias gc.customize.Menus.getMenuItemControl
	 *
	 * @param {string} menuItemId
	 * @return {Object|null}
	 */
	...
	/**
	 * @alias gc.customize.Menus~menuItemIdToSettingId
	 *
	 * @param {string} menuItemId
	 */
	 ...
	 *
	 * @alias gc.customize.Menus~displayNavMenuName
	 *
	 * @param {string} name
	 * @return {string}
	 */
	function displayNavMenuName( name ) {
		name = name || '';
		name = gc.sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = name.toString().trim();
		return name || api.Menus.data.l10n.unnamed;
	}

})( gc.customize, gc, jQuery );
