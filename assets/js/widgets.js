...
window.gcWidgets = {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.on( 'click', function() {
				...
				$document.triggerHandler( 'gc-pin-menu' );
			})
			/*
		...
		$( window ).on( 'beforeunload.widgets', function( event ) {
			...
			if ( 0 !== dirtyWidgetIds.length ) {
				...
				event.returnValue = gc.i18n.__( '离开这个页面，您所做的更改将丢失。' );
		...
		$(document.body).on('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				if ( ! widget.data( 'dirty-state-initialized' ) ) {
					...
					saveButton.prop( 'disabled', true ).val( gc.i18n.__( '已保存' ) );
					inside.on( 'input change', function() {
						self.dirtyWidgets[ widgetId ] = true;
						widget.addClass( 'widget-dirty' );
						saveButton.prop( 'disabled', false ).val( gc.i18n.__( '保存' ) );
					});
				...
				gcWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				gcWidgets.save( target.closest('div.widget'), 1, 1, 0 );
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				toggleBtn.attr( 'aria-expanded', 'false' );
				gcWidgets.close( widget );
			} else if ( target.attr( 'id' ) === 'inactive-widgets-control-remove' ) {
				gcWidgets.removeInactiveWidgets();
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#gcbody-content' ).append( chooser.hide() );
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( gcWidgets.hoveredSidebar && ! $wrap.is( gcWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					gcWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					gcWidgets.hoveredSidebar = $wrap;
					$wrap
						.removeClass( 'closed' )
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				gcWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					gcWidgets.save( $widget, 1, 0, 1 ); // Delete widget.
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					gcWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_gcnonce_widgets').val(),
			sidebars: []
		};
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#gc_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'gc_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#gc_inactive_widgets .widget' ).length );
					}
				}
			} else {
				...
				if ( r && r.length > 2 ) {
					...
					gcWidgets.appendTitle( widget );

					// Re-disable the save button.
					widget.find( '.widget-control-save' ).prop( 'disabled', true ).val( gc.i18n.__( '已保存' ) );
	...
	removeInactiveWidgets : function() {
		...
		a = {
			...
			removeinactivewidgets : $( '#_gcnonce_remove_inactive_widgets' ).val()
		};

		data = $.param( a );

		$.post( ajaxurl, data, function() {
			$( '#gc_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
