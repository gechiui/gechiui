...
/**
 * @namespace gc.textWidgets
 */
gc.textWidgets = ( function( $ ) {
	'use strict';
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends gc.textWidgets.TextWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs gc.textWidgets.TextWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( gc.template( 'widget-text-control-fields' ) );

			control.customHtmlWidgetPointer = control.$el.find( '.gc-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.gc-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				gc.ajax.post( 'dismiss-gc-pointer', {
					pointer: pointer
				});
		...
		openAvailableWidgetsPanel: function openAvailableWidgetsPanel() {
			...
			gc.customize.section.each( function( section ) {
				if ( section.extended( gc.customize.Widgets.SidebarSection ) && section.expanded() ) {
					sidebarControl = gc.customize.control( 'sidebars_widgets[' + section.params.sidebarId + ']' );
				}
			});
			if ( ! sidebarControl ) {
				return;
			...
			setTimeout( function() { // Timeout to prevent click event from causing panel to immediately collapse.
				gc.customize.Widgets.availableWidgetsPanel.open( sidebarControl );
				gc.customize.Widgets.availableWidgetsPanel.$search.val( 'HTML' ).trigger( 'keyup' );
			});
		},
		...
		updateFields: function updateFields() {
			...
			if ( control.fields.text.is( ':visible' ) ) {
				...
				control.editor.setContent( gc.oldEditor.autop( syncInput.val() ) );
			}
		},
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				var updateWidgetBuffer = 300; // See gc.customize.Widgets.WidgetControl._setupUpdateUI() which uses 250ms for updateWidgetDebounced.
				if ( control.editor.isDirty() ) {
					 ...
					 * having to make server round-trips to call the respective GC_Widget::update()
					 * callbacks. See <https://core.trac.gechiui.com/ticket/33507>.
					 */
					if ( gc.customize && gc.customize.state ) {
						gc.customize.state( 'processing' ).set( gc.customize.state( 'processing' ).get() + 1 );
						_.delay( function() {
							gc.customize.state( 'processing' ).set( gc.customize.state( 'processing' ).get() - 1 );
						}, updateWidgetBuffer );
					}
			...
			function buildEditor() {
				...
				if ( tinymce.get( id ) ) {
					...
					gc.oldEditor.remove( id );
				}

				// Add or enable the `gcview` plugin.
				$( document ).one( 'gc-before-tinymce-init.text-widget-init', function( event, init ) {
				...
				showPointerElement = function( pointerElement ) {
					...
					gc.a11y.speak( pointerElement.find( 'h3, p' ).map( function() {
						return $( this ).text();
					} ).get().join( '\n\n' ) );
				};
	 ...
	 *
	 * @memberOf gc.textWidgets
	 *
	 * @type {Object.<string, gc.textWidgets.TextWidgetControl>}
	 */
	 ...
	 * attached which calls gc.textWidgets.init().
	 *
	 * @memberOf gc.textWidgets
	 *
	 * @return {void}
