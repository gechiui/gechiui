...
/**
 * @namespace gc.customHtmlWidget
 * @memberOf gc
 */
gc.customHtmlWidgets = ( function( $ ) {
	'use strict';
	...
	component.CustomHtmlWidgetControl = Backbone.View.extend(/** @lends gc.customHtmlWidgets.CustomHtmlWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs gc.customHtmlWidgets.CustomHtmlWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( gc.template( 'widget-custom-html-control-fields' )( { codeEditorDisabled: component.codeEditorSettings.disabled } ) );
		...
		updateErrorNotice: function( errorAnnotations ) {
			...
			if ( gc.customize && gc.customize.has( control.customizeSettingId ) ) {
				customizeSetting = gc.customize( control.customizeSettingId );
				customizeSetting.notifications.remove( 'htmlhint_error' );
				if ( 0 !== errorAnnotations.length ) {
					customizeSetting.notifications.add( 'htmlhint_error', new gc.customize.Notification( 'htmlhint_error', {
						message: message,
						type: 'error'
					} ) );
				}
		...
		initializeEditor: function initializeEditor() {
			...
			control.editor = gc.codeEditor.initialize( control.fields.content, settings );
	 ...
	 *
	 * @alias gc.customHtmlWidgets.widgetControls
	 *
	 * @type {Object.<string, gc.textWidgets.CustomHtmlWidgetControl>}
	 */
	 ...
	 *
	 * @alias gc.customHtmlWidgets.handleWidgetAdded
	 *
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		...
		renderWhenAnimationDone = function() {
			if ( ! ( gc.customize ? widgetContainer.parent().hasClass( 'expanded' ) : widgetContainer.hasClass( 'open' ) ) ) { // Core merge: The gc.customize condition can be eliminated with this change being in core: https://github.com/xgc/gechiui-develop/pull/247/commits/5322387d
	 ...
	 *
	 * @alias gc.customHtmlWidgets.setupAccessibleMode
	 *
	 * @return {void}
	 */
	 ...
	 *
	 * @alias gc.customHtmlWidgets.handleWidgetUpdated
	 *
	 * @param {jQuery.Event} event - Event.
	 * @param {jQuery}       widgetContainer - Widget container element.
	 * @return {void}
	 ...
	 *
	 * @alias gc.customHtmlWidgets.init
	 *
