...
(function( component ) {
	 ...
	 *
	 * @class    gc.mediaWidgets.controlConstructors~VideoDetailsMediaFrame
	 * @augments gc.media.view.MediaFrame.VideoDetails
	 *
	 * @private
	 */
	VideoDetailsMediaFrame = gc.media.view.MediaFrame.VideoDetails.extend(/** @lends gc.mediaWidgets.controlConstructors~VideoDetailsMediaFrame.prototype */{
		...
		createStates: function createStates() {
			this.states.add([
				...
				new gc.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: gc.media.view.l10n.videoAddSourceTitle,
				...
				new gc.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: gc.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See GC_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    gc.mediaWidgets.modelConstructors.media_video
	 ...
	 * See GC_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    gc.mediaWidgets.controlConstructors.media_video
	 * @augments gc.mediaWidgets.MediaWidgetControl
	 */
	VideoWidgetControl = component.MediaWidgetControl.extend(/** @lends gc.mediaWidgets.controlConstructors.media_video.prototype */{
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = gc.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( gc.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( gc.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = gc.template( 'gc-media-widget-video-preview' );
