...
( function( $ ){
	window.findPosts = {
		...
		send: function() {
			...
			$.ajax( ajaxurl, {
				...
				if ( ! x.success ) {
					$( '#find-posts-response' ).text( gc.i18n.__( '发生了错误，请刷新此页面并重试。' ) );
				}
	...
	$( function() {
		...
		if ( $mediaGridWrap.length && window.gc && window.gc.media ) {
			settings = _gcMediaGridSettings;

			var frame = window.gc.media({
				frame: 'manage',
			...
			$mediaGridWrap.trigger( 'gc-media-grid-ready', frame );
		}
