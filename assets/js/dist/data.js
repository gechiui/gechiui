...
;// CONCATENATED MODULE: external ["gc","reduxRoutine"]
var external_gc_reduxRoutine_namespaceObject = window["gc"]["reduxRoutine"];
var external_gc_reduxRoutine_default = /*#__PURE__*/__webpack_require__.n(external_gc_reduxRoutine_namespaceObject);
 ...
 * @example
 * ```js
 * import { store as coreStore } from '@gechiui/core-data';
 * import { store as editorStore } from '@gechiui/editor';
 *
...
;// CONCATENATED MODULE: ./node_modules/@gechiui/data/build-module/lock-unlock.js
/**
...
const {
  lock,
  unlock
} = (0,external_gc_privateApis_namespaceObject.__dangerousOptInToUnstableAPIsOnlyForCoreModules)('I know using unstable features means my plugin or theme will inevitably break on the next GeChiUI release.', '@gechiui/data');
...
function instantiateReduxStore(key, options, registry, thunkArgs) {
  ...
  const middlewares = [resolvers_cache_middleware(registry, key), promise_middleware, external_gc_reduxRoutine_default()(normalizedControls), createThunkMiddleware(thunkArgs)];
  ...
  return createStore(enhancedReducer, {
    root: initialState
  }, (0,external_gc_compose_namespaceObject.compose)(enhancers));
}
/**
...
function createRegistry(storeConfigs = {}, parent = null) {
  ...
  function registerGenericStore(name, store) {
    external_gc_deprecated_default()('gc.data.registerGenericStore', {
      since: '5.9',
      alternative: 'gc.data.register( storeDescriptor )'
    });
...
const Context = (0,external_gc_element_namespaceObject.createContext)(default_registry);
const {
  Consumer,
  Provider
} = Context;
...
function useRegistry() {
  return (0,external_gc_element_namespaceObject.useContext)(Context);
}
...
const context_Context = (0,external_gc_element_namespaceObject.createContext)(false);
const {
...
function useAsyncMode() {
  return (0,external_gc_element_namespaceObject.useContext)(context_Context);
}
...
const renderQueue = (0,external_gc_priorityQueue_namespaceObject.createQueue)();
/**
 * @typedef {import('../../types').StoreDescriptor<C>} StoreDescriptor
 * @template {import('../../types').AnyConfig} C
 */
...
function Store(registry, suspense) {
  ...
  return (mapSelect, isAsync) => {
    function updateValue() {
      ...
      if (!external_gc_isShallowEqual_default()(lastMapResult, mapResult)) {
...
function useMappingSelect(suspense, mapSelect, deps) {
  ...
  const store = (0,external_gc_element_namespaceObject.useMemo)(() => Store(registry, suspense), [registry]);
  const selector = (0,external_gc_element_namespaceObject.useCallback)(mapSelect, deps);
  const {
    subscribe,
    getValue
  } = store(selector, isAsync);
  const result = (0,external_gc_element_namespaceObject.useSyncExternalStore)(subscribe, getValue, getValue);
  (0,external_gc_element_namespaceObject.useDebugValue)(result);
  return result;
}
/**
...
function useSelect(mapSelect, deps) {
  ...
  const staticSelectModeRef = (0,external_gc_element_namespaceObject.useRef)(staticSelectMode);
...
const withSelect = mapSelectToProps => (0,external_gc_compose_namespaceObject.createHigherOrderComponent)(WrappedComponent => (0,external_gc_compose_namespaceObject.pure)(ownProps => {
  const mapSelect = (select, registry) => mapSelectToProps(select, ownProps, registry);

  const mergeProps = useSelect(mapSelect);
  return (0,external_gc_element_namespaceObject.createElement)(WrappedComponent, { ...ownProps,
    ...mergeProps
  });
}), 'withSelect');
...
const useDispatchWithMap = (dispatchMap, deps) => {
  const registry = useRegistry();
  const currentDispatchMap = (0,external_gc_element_namespaceObject.useRef)(dispatchMap);
  (0,external_gc_compose_namespaceObject.useIsomorphicLayoutEffect)(() => {
...
const withDispatch = mapDispatchToProps => (0,external_gc_compose_namespaceObject.createHigherOrderComponent)(WrappedComponent => ownProps => {
...
const withRegistry = (0,external_gc_compose_namespaceObject.createHigherOrderComponent)(OriginalComponent => props => (0,external_gc_element_namespaceObject.createElement)(RegistryConsumer, null, registry => (0,external_gc_element_namespaceObject.createElement)(OriginalComponent, { ...props,
  registry: registry
})), 'withRegistry');
...
;// CONCATENATED MODULE: ./node_modules/@gechiui/data/build-module/dispatch.js
/**
 * Internal dependencies
 */
...
;// CONCATENATED MODULE: ./node_modules/@gechiui/data/build-module/select.js
/**
 * Internal dependencies
 */
...
/**
 * Registers a standard `@gechiui/data` store descriptor.
 *
 * @example
 * ```js
...
(window.gc = window.gc || {}).data = __webpack_exports__;
/******/ })()
;