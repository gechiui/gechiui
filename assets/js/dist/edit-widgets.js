...
function transformWidgetToBlock(widget) {
  if (widget.id_base === 'block') {
    ...
    if (!parsedBlocks.length) {
      return (0,external_gc_widgets_namespaceObject.addWidgetIdToBlock)((0,external_gc_blocks_namespaceObject.createBlock)('core/paragraph', {}, []), widget.id);
    }

    return (0,external_gc_widgets_namespaceObject.addWidgetIdToBlock)(parsedBlocks[0], widget.id);
  }
  ...
  return (0,external_gc_widgets_namespaceObject.addWidgetIdToBlock)((0,external_gc_blocks_namespaceObject.createBlock)('core/legacy-widget', attributes, []), widget.id);
}
/**
...
function transformBlockToWidget(block, relatedWidget = {}) {
  ...
  if (isValidLegacyWidgetBlock) {
    ...
    widget = { ...relatedWidget,
      ...
      instance: {
        raw: {
          content: (0,external_gc_blocks_namespaceObject.serialize)(block)
        }
      }
    };
...
const persistStubPost = (id, blocks) => ({
  ...
  registry.dispatch(external_gc_coreData_namespaceObject.store).receiveEntityRecords(KIND, POST_TYPE, stubPost, {
    id: stubPost.id
  }, false);
  return stubPost;
};
...
const saveEditedWidgetAreas = () => async ({
  ...
  try {
    ...
    registry.dispatch(external_gc_notices_namespaceObject.store).createSuccessNotice((0,external_gc_i18n_namespaceObject.__)('小工具已保存。'), {
      type: 'snackbar'
    });
  } catch (e) {
    registry.dispatch(external_gc_notices_namespaceObject.store).createErrorNotice(
    /* translators: %s: The error message. */
    (0,external_gc_i18n_namespaceObject.sprintf)((0,external_gc_i18n_namespaceObject.__)('出现错误。 %s'), e.message), {
      type: 'snackbar'
    });
  }
};
...
const saveWidgetAreas = widgetAreas => async ({
  ...
  try {
    ...
    await registry.dispatch(external_gc_coreData_namespaceObject.store).finishResolution('getEntityRecord', KIND, WIDGET_AREA_ENTITY_TYPE, buildWidgetAreasQuery());
  }
};
/**
...
const saveWidgetArea = widgetAreaId => async ({
  ...
  const post = registry.select(external_gc_coreData_namespaceObject.store).getEditedEntityRecord(KIND, POST_TYPE, buildWidgetAreaPostId(widgetAreaId)); // Get all widgets from this area
  ...
  for (let i = 0; i < widgetsBlocks.length; i++) {
    ...
    const widgetId = (0,external_gc_widgets_namespaceObject.getWidgetIdFromBlock)(block);
    ...
    if (oldWidget) {
      ...
      registry.dispatch(external_gc_coreData_namespaceObject.store).editEntityRecord('root', 'widget', widgetId, { ...widget,
        sidebar: widgetAreaId
      }, {
        undoIgnore: true
      });
      const hasEdits = registry.select(external_gc_coreData_namespaceObject.store).hasEditsForEntityRecord('root', 'widget', widgetId);
  ...
  const records = await registry.dispatch(external_gc_coreData_namespaceObject.store).__experimentalBatch(batchTasks);
  ...
  for (let i = 0; i < preservedRecords.length; i++) {
    ...
    const error = registry.select(external_gc_coreData_namespaceObject.store).getLastEntitySaveError('root', 'widget', widget.id);
  ...
  if (failedWidgetNames.length) {
    throw new Error((0,external_gc_i18n_namespaceObject.sprintf)(
    /* translators: %s: List of widget names */
    (0,external_gc_i18n_namespaceObject.__)('无法保存以下小工具：%s。'), failedWidgetNames.join(', ')));
  }

  registry.dispatch(external_gc_coreData_namespaceObject.store).editEntityRecord(KIND, WIDGET_AREA_ENTITY_TYPE, widgetAreaId, {
    widgets: sidebarWidgetsIds
  }, {
    undoIgnore: true
  });
  dispatch(trySaveWidgetArea(widgetAreaId));
  registry.dispatch(external_gc_coreData_namespaceObject.store).receiveEntityRecords(KIND, POST_TYPE, post, undefined);
};

const trySaveWidgetArea = widgetAreaId => ({
  registry
}) => {
  registry.dispatch(external_gc_coreData_namespaceObject.store).saveEditedEntityRecord(KIND, WIDGET_AREA_ENTITY_TYPE, widgetAreaId, {
    throwOnError: true
  });
};
/**
...
const moveBlockToWidgetArea = (clientId, widgetAreaId) => async ({
  ...
  const sourceRootClientId = registry.select(external_gc_blockEditor_namespaceObject.store).getBlockRootClientId(clientId); // Search the top level blocks (widget areas) for the one with the matching
  // id attribute. Makes the assumption that all top-level blocks are widget
  // areas.

  const widgetAreas = registry.select(external_gc_blockEditor_namespaceObject.store).getBlocks();
  ...
  const destinationInnerBlocksClientIds = registry.select(external_gc_blockEditor_namespaceObject.store).getBlockOrder(destinationRootClientId);
  ...
  registry.dispatch(external_gc_blockEditor_namespaceObject.store).moveBlocksToPosition([clientId], sourceRootClientId, destinationRootClientId, destinationIndex);
};
...
const getWidgetAreas = () => async ({
  ...
  const widgetAreas = await registry.resolveSelect(external_gc_coreData_namespaceObject.store).getEntityRecords(KIND, WIDGET_AREA_ENTITY_TYPE, query);
  ...
  for (const widgetArea of sortedWidgetAreas) {
    widgetAreaBlocks.push((0,external_gc_blocks_namespaceObject.createBlock)('core/widget-area', {
      id: widgetArea.id,
...
const getWidgets = () => async ({
  ...
  const widgets = await registry.resolveSelect(external_gc_coreData_namespaceObject.store).getEntityRecords('root', 'widget', query);
...
const selectors_getWidgets = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => () => {
  const widgets = select(external_gc_coreData_namespaceObject.store).getEntityRecords('root', 'widget', buildWidgetsQuery());
...
const getWidget = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, id) => {
...
const selectors_getWidgetAreas = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => () => {
  const query = buildWidgetAreasQuery();
  return select(external_gc_coreData_namespaceObject.store).getEntityRecords(KIND, WIDGET_AREA_ENTITY_TYPE, query);
});
/**
...
const getWidgetAreaForWidgetId = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, widgetId) => {
  const widgetAreas = select(constants_STORE_NAME).getWidgetAreas();
  return widgetAreas.find(widgetArea => {
    const post = select(external_gc_coreData_namespaceObject.store).getEditedEntityRecord(KIND, POST_TYPE, buildWidgetAreaPostId(widgetArea.id));
    const blockWidgetIds = post.blocks.map(block => (0,external_gc_widgets_namespaceObject.getWidgetIdFromBlock)(block));
...
const getParentWidgetAreaBlock = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, clientId) => {
  const {
    getBlock,
    getBlockName,
    getBlockParents
...
const getEditedWidgetAreas = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, ids) => {
  ...
  return widgetAreas.filter(({
    id
  }) => select(external_gc_coreData_namespaceObject.store).hasEditsForEntityRecord(KIND, POST_TYPE, buildWidgetAreaPostId(id))).map(({
    id
  }) => select(external_gc_coreData_namespaceObject.store).getEditedEntityRecord(KIND, WIDGET_AREA_ENTITY_TYPE, id));
});
/**
...
const getReferenceWidgetBlocks = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, referenceWidgetName = null) => {
  const results = [];
  const widgetAreas = select(constants_STORE_NAME).getWidgetAreas();

  for (const _widgetArea of widgetAreas) {
    const post = select(external_gc_coreData_namespaceObject.store).getEditedEntityRecord(KIND, POST_TYPE, buildWidgetAreaPostId(_widgetArea.id));
...
const isSavingWidgetAreas = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => () => {
  ...
  for (const id of widgetAreasIds) {
    const isSaving = select(external_gc_coreData_namespaceObject.store).isSavingEntityRecord(KIND, WIDGET_AREA_ENTITY_TYPE, id);
  ...
  for (const id of widgetIds) {
    const isSaving = select(external_gc_coreData_namespaceObject.store).isSavingEntityRecord('root', 'widget', id);
...
const canInsertBlockInWidgetArea = (0,external_gc_data_namespaceObject.createRegistrySelector)(select => (state, blockName) => {
  // Widget areas are always top-level blocks, which getBlocks will return.
  const widgetAreas = select(external_gc_blockEditor_namespaceObject.store).getBlocks(); // Makes an assumption that a block that can be inserted into one
  // widget area can be inserted into any widget area. Uses the first
  // widget area for testing whether the block can be inserted.

  const [firstWidgetArea] = widgetAreas;
  return select(external_gc_blockEditor_namespaceObject.store).canInsertBlockType(blockName, firstWidgetArea.clientId);
});
/**
 ...
 *
 * @see https://github.com/GeChiUI/gutenberg/blob/HEAD/packages/data/README.md#register
 *
 * @type {Object}
 */
...
(0,external_gc_data_namespaceObject.register)(store_store); // This package uses a few in-memory post types as wrappers for convenience.
// This middleware prevents any network requests related to these types as they are
// bound to fail anyway.

external_gc_apiFetch_default().use(function (options, next) {
  if (options.path?.indexOf('/gc/v2/types/widget-area') === 0) {
...
const withMoveToWidgetAreaToolbarItem = (0,external_gc_compose_namespaceObject.createHigherOrderComponent)(BlockEdit => props => {
  const {
    clientId,
    name: blockName
  } = props;
  ...
  }), isMoveToWidgetAreaVisible && (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockControls, null, (0,external_gc_element_namespaceObject.createElement)(external_gc_widgets_namespaceObject.MoveToWidgetArea, {
...
(0,external_gc_hooks_namespaceObject.addFilter)('editor.BlockEdit', 'core/edit-widgets/block-edit', withMoveToWidgetAreaToolbarItem);
...
(0,external_gc_hooks_namespaceObject.addFilter)('editor.MediaUpload', 'core/edit-widgets/replace-media-upload', replaceMediaUpload);
...
function WidgetAreaInnerBlocks({
  ...
  const [blocks, onInput, onChange] = (0,external_gc_coreData_namespaceObject.useEntityBlockEditor)('root', 'postType');
  const innerBlocksRef = (0,external_gc_element_namespaceObject.useRef)();
...
function WidgetAreaEdit({
  ...
  const isOpen = (0,external_gc_data_namespaceObject.useSelect)(select => select(store_store).getIsWidgetAreaOpen(clientId), [clientId]);
  const {
    setIsWidgetAreaOpen
  } = (0,external_gc_data_namespaceObject.useDispatch)(store_store);
  const wrapper = (0,external_gc_element_namespaceObject.useRef)();
  const setOpen = (0,external_gc_element_namespaceObject.useCallback)(openState => setIsWidgetAreaOpen(clientId, openState), [clientId]);
  const isDragging = useIsDragging(wrapper);
  const isDraggingWithin = use_is_dragging_within(wrapper);
  const [openedWhileDragging, setOpenedWhileDragging] = (0,external_gc_element_namespaceObject.useState)(false);
  (0,external_gc_element_namespaceObject.useEffect)(() => {
    if (!isDragging) {
  ...
  return (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.Panel, {
    className: className,
    ref: wrapper
  ...
  (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.__unstableDisclosureContent, {
    ...
    type: "postType",
    id: `widget-area-${id}`
  }, (0,external_gc_element_namespaceObject.createElement)(WidgetAreaInnerBlocks, {
    id: id
  })))));
}
/**
...
const useIsDragging = elementRef => {
  const [isDragging, setIsDragging] = (0,external_gc_element_namespaceObject.useState)(false);
  (0,external_gc_element_namespaceObject.useEffect)(() => {
    const {
      ownerDocument
    } = elementRef.current;
...
const settings = {
  title: (0,external_gc_i18n_namespaceObject.__)('小工具区域'),
  description: (0,external_gc_i18n_namespaceObject.__)('小工具区域容器。'),
  __experimentalLabel: ({
    name: label
  }) => label,
  edit: WidgetAreaEdit
...
function KeyboardShortcuts() {
  ...
  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/undo', event => {
    undo();
    event.preventDefault();
  });
  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/redo', event => {
    redo();
    event.preventDefault();
  });
  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/save', event => {
    event.preventDefault();
    saveEditedWidgetAreas();
  });
  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/transform-heading-to-paragraph', event => handleTextLevelShortcut(event, 0));
  [1, 2, 3, 4, 5, 6].forEach(level => {
    //the loop is based off on a constant therefore
    //the hook will execute the same way every time
    //eslint-disable-next-line react-hooks/rules-of-hooks
    (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)(`core/edit-widgets/transform-paragraph-to-heading-${level}`, event => handleTextLevelShortcut(event, level));
  });
  return null;
}
...
const useLastSelectedWidgetArea = () => (0,external_gc_data_namespaceObject.useSelect)(select => {
  const {
...
;// CONCATENATED MODULE: ./node_modules/@gechiui/edit-widgets/build-module/lock-unlock.js
/**
 * GeChiUI dependencies
 */
...
const {
  lock,
  unlock
} = (0,external_gc_privateApis_namespaceObject.__dangerousOptInToUnstableAPIsOnlyForCoreModules)('I know using unstable features means my plugin or theme will inevitably break on the next GeChiUI release.', '@gechiui/edit-widgets');
...
function WidgetAreasBlockEditorProvider({
  ...
  const mediaPermissions = (0,external_gc_coreData_namespaceObject.useResourcePermissions)('media');
  const {
    reusableBlocks,
    ...
    reusableBlocks: ALLOW_REUSABLE_BLOCKS ? select(external_gc_coreData_namespaceObject.store).getEntityRecords('postType', 'gc_block') : [],
    isFixedToolbarActive: !!select(external_gc_preferences_namespaceObject.store).get('core/edit-widgets', 'fixedToolbar'),
    keepCaretInsideBlock: !!select(external_gc_preferences_namespaceObject.store).get('core/edit-widgets', 'keepCaretInsideBlock')
  }), []);
  const {
    setIsInserterOpened
  } = (0,external_gc_data_namespaceObject.useDispatch)(store_store);
  ...
  const [blocks, onInput, onChange] = (0,external_gc_coreData_namespaceObject.useEntityBlockEditor)(KIND, POST_TYPE, {
    id: buildWidgetAreasPostId()
  });
  return (0,external_gc_element_namespaceObject.createElement)(external_gc_keyboardShortcuts_namespaceObject.ShortcutProvider, null, (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockEditorKeyboardShortcuts.Register, null), (0,external_gc_element_namespaceObject.createElement)(keyboard_shortcuts.Register, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.SlotFillProvider, null, (0,external_gc_element_namespaceObject.createElement)(ExperimentalBlockEditorProvider, {
    value: blocks,
    onInput: onInput,
    onChange: onChange,
    settings: settings,
    ...
    ...props
  }, (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.CopyHandler, null, children), (0,external_gc_element_namespaceObject.createElement)(external_gc_reusableBlocks_namespaceObject.ReusableBlocksMenuItems, {
...
function WidgetAreas({
  ...
  const widgetAreas = (0,external_gc_data_namespaceObject.useSelect)(select => select(store_store).getWidgetAreas(), []);
  const selectedWidgetArea = (0,external_gc_element_namespaceObject.useMemo)(() => selectedWidgetAreaId && widgetAreas?.find(widgetArea => widgetArea.id === selectedWidgetAreaId), [selectedWidgetAreaId, widgetAreas]);
  let description;

  if (!selectedWidgetArea) {
    description = (0,external_gc_i18n_namespaceObject.__)('小工具区域是系统布局中可以支持区块的全局部分。这些区域因主题而异，但通常都是您的边栏或页脚部分。');
  } else if (selectedWidgetAreaId === 'gc_inactive_widgets') {
    description = (0,external_gc_i18n_namespaceObject.__)('此小工具区域中的区块将不会显示在您的系统中。');
  } else {
  ...
  return (0,external_gc_element_namespaceObject.createElement)("div", {
    ...
    icon: block_default
  }), (0,external_gc_element_namespaceObject.createElement)("div", null, (0,external_gc_element_namespaceObject.createElement)("p", {
    // Use `dangerouslySetInnerHTML` to keep backwards
    // compatibility. Basic markup in the description is an
    // established feature of GeChiUI.
    // @see https://github.com/GeChiUI/gutenberg/issues/33106
    dangerouslySetInnerHTML: {
      __html: (0,external_gc_dom_namespaceObject.safeHTML)(description)
    }
  }), widgetAreas?.length === 0 && (0,external_gc_element_namespaceObject.createElement)("p", null, (0,external_gc_i18n_namespaceObject.__)('您的主题不包含任何小工具区域。')), !selectedWidgetArea && (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.Button, {
    href: (0,external_gc_url_namespaceObject.addQueryArgs)('customize.php', {
      'autofocus[panel]': 'widgets',
      return: window.location.pathname
    }),
    variant: "tertiary"
  }, (0,external_gc_i18n_namespaceObject.__)('使用实时预览进行管理')))));
}
...
function ComplementaryAreaTab({
  ...
  return (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.Button, {
    ...
    "aria-label": isActive ? // translators: %s: sidebar label e.g: "小工具区域".
    (0,external_gc_i18n_namespaceObject.sprintf)((0,external_gc_i18n_namespaceObject.__)('%s（已选中）'), label) : label,
    "data-label": label
  }, label);
}
...
function Sidebar() {
  ...
  return (0,external_gc_element_namespaceObject.createElement)(complementary_area, {
    ...
    header: (0,external_gc_element_namespaceObject.createElement)("ul", null, (0,external_gc_element_namespaceObject.createElement)("li", null, (0,external_gc_element_namespaceObject.createElement)(ComplementaryAreaTab, {
      identifier: WIDGET_AREAS_IDENTIFIER,
      label: selectedWidgetAreaBlock ? selectedWidgetAreaBlock.attributes.name : (0,external_gc_i18n_namespaceObject.__)('小工具区域'),
      isActive: currentArea === WIDGET_AREAS_IDENTIFIER
    })), (0,external_gc_element_namespaceObject.createElement)("li", null, (0,external_gc_element_namespaceObject.createElement)(ComplementaryAreaTab, {
      identifier: BLOCK_INSPECTOR_IDENTIFIER,
      label: (0,external_gc_i18n_namespaceObject.__)('区块'),
  ...
  }, currentArea === WIDGET_AREAS_IDENTIFIER && (0,external_gc_element_namespaceObject.createElement)(WidgetAreas, {
    selectedWidgetAreaId: selectedWidgetAreaBlock?.attributes.id
  }), currentArea === BLOCK_INSPECTOR_IDENTIFIER && (hasSelectedNonAreaBlock ? (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockInspector, null) : // Pretend that Widget Areas are part of the UI by not
  // showing the Block Inspector when one is selected.
  (0,external_gc_element_namespaceObject.createElement)("span", {
    className: "block-editor-block-inspector__no-blocks"
  }, (0,external_gc_i18n_namespaceObject.__)('未选择区块。'))));
}
...
function SaveButton() {
  ...
  }, isSaving ? (0,external_gc_i18n_namespaceObject.__)('正在保存...' ) : (0,external_gc_i18n_namespaceObject.__)('更新'));
}
...
function KeyboardShortcutHelpModal({
  ...
  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/keyboard-shortcuts', toggleModal, {
    bindGlobal: true
  });
...
;// CONCATENATED MODULE: ./node_modules/@gechiui/edit-widgets/build-module/components/more-menu/tools-more-menu-group.js
...
const {
  ...
  Slot
} = (0,external_gc_components_namespaceObject.createSlotFill)('EditWidgetsToolsMoreMenuGroup');
...
function MoreMenu() {
  const [isKeyboardShortcutsModalActive, setIsKeyboardShortcutsModalVisible] = (0,external_gc_element_namespaceObject.useState)(false);

  const toggleKeyboardShortcutsModal = () => setIsKeyboardShortcutsModalVisible(!isKeyboardShortcutsModalActive);

  (0,external_gc_keyboardShortcuts_namespaceObject.useShortcut)('core/edit-widgets/keyboard-shortcuts', toggleKeyboardShortcutsModal);
  const isLargeViewport = (0,external_gc_compose_namespaceObject.useViewportMatch)('medium');
  return (0,external_gc_element_namespaceObject.createElement)(external_gc_element_namespaceObject.Fragment, null, (0,external_gc_element_namespaceObject.createElement)(MoreMenuDropdown, null, onClose => (0,external_gc_element_namespaceObject.createElement)(external_gc_element_namespaceObject.Fragment, null, isLargeViewport && (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.MenuGroup, {
  ...
  }, (0,external_gc_i18n_namespaceObject.__)('键盘快捷键')), (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.MenuItem, {
    role: "menuitem",
    icon: library_external,
    href: (0,external_gc_i18n_namespaceObject.__)('https://www.gechiui.com/support/block-based-widgets-editor/'),
    target: "_blank",
  ...
  (0,external_gc_i18n_namespaceObject.__)('（在新窗口中打开）'))), (0,external_gc_element_namespaceObject.createElement)(tools_more_menu_group.Slot, {
    ...
    label: (0,external_gc_i18n_namespaceObject.__)('偏好设置')
  }, (0,external_gc_element_namespaceObject.createElement)(external_gc_preferences_namespaceObject.PreferenceToggleMenuItem, {
    scope: "core/edit-widgets",
    name: "keepCaretInsideBlock",
    label: (0,external_gc_i18n_namespaceObject.__)('在区块内包含文本光标'),
    info: (0,external_gc_i18n_namespaceObject.__)('阻止文字光标离开区块以辅助屏幕阅读器。'),
    messageActivated: (0,external_gc_i18n_namespaceObject.__)('启用区块内的文本光标'),
    messageDeactivated: (0,external_gc_i18n_namespaceObject.__)('禁用区块内的文本光标')
  }), (0,external_gc_element_namespaceObject.createElement)(external_gc_preferences_namespaceObject.PreferenceToggleMenuItem, {
    scope: "core/edit-widgets",
    name: "themeStyles",
    info: (0,external_gc_i18n_namespaceObject.__)('使编辑器看起来像您的主题。'),
    label: (0,external_gc_i18n_namespaceObject.__)('使用主题样式')
  }), isLargeViewport && (0,external_gc_element_namespaceObject.createElement)(external_gc_preferences_namespaceObject.PreferenceToggleMenuItem, {
    scope: "core/edit-widgets",
    name: "showBlockBreadcrumbs",
    label: (0,external_gc_i18n_namespaceObject.__)('显示区块面包屑'),
    info: (0,external_gc_i18n_namespaceObject.__)('在编辑器的底部显示区块的面包屑。'),
    messageActivated: (0,external_gc_i18n_namespaceObject.__)('显示区块面包屑已启用'),
    messageDeactivated: (0,external_gc_i18n_namespaceObject.__)('显示区块面包屑已禁用')
  })))), (0,external_gc_element_namespaceObject.createElement)(KeyboardShortcutHelpModal, {
...
function Header() {
  const isMediumViewport = (0,external_gc_compose_namespaceObject.useViewportMatch)('medium');
  const inserterButton = (0,external_gc_element_namespaceObject.useRef)();
  const widgetAreaClientId = use_last_selected_widget_area();
  const isLastSelectedWidgetAreaOpen = (0,external_gc_data_namespaceObject.useSelect)(select => select(store_store).getIsWidgetAreaOpen(widgetAreaClientId), [widgetAreaClientId]);
  const {
    isInserterOpen,
    isListViewOpen
  } = (0,external_gc_data_namespaceObject.useSelect)(select => {
  ...
  return (0,external_gc_element_namespaceObject.createElement)(external_gc_element_namespaceObject.Fragment, null, (0,external_gc_element_namespaceObject.createElement)("div", {
  ...
  }, isMediumViewport && (0,external_gc_element_namespaceObject.createElement)("h1", {
    className: "edit-widgets-header__title"
  }, (0,external_gc_i18n_namespaceObject.__)('小工具')), !isMediumViewport && (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.VisuallyHidden, {
    as: "h1",
    className: "edit-widgets-header__title"
  }, (0,external_gc_i18n_namespaceObject.__)('小工具')), (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.NavigableToolbar, {
    ...
    characters. */
    ,
    label: (0,external_gc_i18n_namespaceObject._x)('Toggle block inserter', 'Generic label for block inserter button')
  }), isMediumViewport && (0,external_gc_element_namespaceObject.createElement)(external_gc_element_namespaceObject.Fragment, null, (0,external_gc_element_namespaceObject.createElement)(UndoButton, null), (0,external_gc_element_namespaceObject.createElement)(RedoButton, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.ToolbarItem, {
    as: external_gc_components_namespaceObject.Button,
  ...
  }, (0,external_gc_element_namespaceObject.createElement)(save_button, null), (0,external_gc_element_namespaceObject.createElement)(pinned_items.Slot, {
    scope: "core/edit-widgets"
  }), (0,external_gc_element_namespaceObject.createElement)(MoreMenu, null))));
}
...
function Notices() {
  ...
  }), (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.SnackbarList, {
...
function WidgetAreasBlockEditorContent({
  ...
  const hasThemeStyles = (0,external_gc_data_namespaceObject.useSelect)(select => !!select(external_gc_preferences_namespaceObject.store).get('core/edit-widgets', 'themeStyles'), []);
  const styles = (0,external_gc_element_namespaceObject.useMemo)(() => {
    return hasThemeStyles ? blockEditorSettings.styles : [];
  }, [blockEditorSettings, hasThemeStyles]);
  return (0,external_gc_element_namespaceObject.createElement)("div", {
    className: "edit-widgets-block-editor"
  }, (0,external_gc_element_namespaceObject.createElement)(notices, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockTools, null, (0,external_gc_element_namespaceObject.createElement)(keyboard_shortcuts, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.__unstableEditorStyles, {
    styles: styles
  }), (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockSelectionClearer, null, (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.WritingFlow, null, (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.ObserveTyping, null, (0,external_gc_element_namespaceObject.createElement)(external_gc_blockEditor_namespaceObject.BlockList, {
...
const useWidgetLibraryInsertionPoint = () => {
  const firstRootId = (0,external_gc_data_namespaceObject.useSelect)(select => {
  ...
  return (0,external_gc_data_namespaceObject.useSelect)(select => {
    ...
    const insertionPoint = select(store_store).__experimentalGetInsertionPoint(); // "浏览全部" in the quick inserter will set the rootClientId to the current block.
...
function InserterSidebar() {
  ...
  const closeInserter = (0,external_gc_element_namespaceObject.useCallback)(() => {
...
function ListViewSidebar() {
  const {
    setIsListViewOpened
  } = (0,external_gc_data_namespaceObject.useDispatch)(store_store); // Use internal state instead of a ref to make sure that the component
  // re-renders when the dropZoneElement updates.

  const [dropZoneElement, setDropZoneElement] = (0,external_gc_element_namespaceObject.useState)(null);
  ...
  return (// eslint-disable-next-line jsx-a11y/no-static-element-interactions
    (0,external_gc_element_namespaceObject.createElement)("div", {
      ...
      ref: (0,external_gc_compose_namespaceObject.useMergeRefs)([contentFocusReturnRef, focusOnMountRef, setDropZoneElement])
...
const interfaceLabels = {
  ...
  header: (0,external_gc_i18n_namespaceObject.__)('小工具顶部栏'),

  /* translators: accessibility text for the widgets screen content landmark region. */
  body: (0,external_gc_i18n_namespaceObject.__)('小工具与区块'),

  /* translators: accessibility text for the widgets screen settings landmark region. */
  sidebar: (0,external_gc_i18n_namespaceObject.__)('小工具设置'),

  /* translators: accessibility text for the widgets screen footer landmark region. */
  footer: (0,external_gc_i18n_namespaceObject.__)('小工具页脚')
};
...
function Interface({
  ...
  const {
    ...
    hasBlockBreadCrumbsEnabled: !!select(external_gc_preferences_namespaceObject.store).get('core/edit-widgets', 'showBlockBreadcrumbs'),
    previousShortcut: select(external_gc_keyboardShortcuts_namespaceObject.store).getAllShortcutKeyCombinations('core/edit-widgets/previous-region'),
    nextShortcut: select(external_gc_keyboardShortcuts_namespaceObject.store).getAllShortcutKeyCombinations('core/edit-widgets/next-region')
  ...
  const secondarySidebarLabel = isListViewOpened ? (0,external_gc_i18n_namespaceObject.__)('列表视图') : (0,external_gc_i18n_namespaceObject.__)('区块库');
  const hasSecondarySidebar = isListViewOpened || isInserterOpened;
  return (0,external_gc_element_namespaceObject.createElement)(interface_skeleton, {
    labels: { ...interfaceLabels,
      secondarySidebar: secondarySidebarLabel
    },
    header: (0,external_gc_element_namespaceObject.createElement)(header, null),
    secondarySidebar: hasSecondarySidebar && (0,external_gc_element_namespaceObject.createElement)(SecondarySidebar, null),
    sidebar: hasSidebarEnabled && (0,external_gc_element_namespaceObject.createElement)(complementary_area.Slot, {
      scope: "core/edit-widgets"
    }),
    content: (0,external_gc_element_namespaceObject.createElement)(external_gc_element_namespaceObject.Fragment, null, (0,external_gc_element_namespaceObject.createElement)(WidgetAreasBlockEditorContent, {
      blockEditorSettings: blockEditorSettings
    })),
    footer: hasBlockBreadCrumbsEnabled && !isMobileViewport && (0,external_gc_element_namespaceObject.createElement)("div", {
      ...
      rootLabelText: (0,external_gc_i18n_namespaceObject.__)('小工具')
    })),
    shortcuts: {
...
function Layout({
  ...
  return (0,external_gc_element_namespaceObject.createElement)(ErrorBoundary, null, (0,external_gc_element_namespaceObject.createElement)(WidgetAreasBlockEditorProvider, {
    blockEditorSettings: blockEditorSettings
  }, (0,external_gc_element_namespaceObject.createElement)(layout_interface, {
    blockEditorSettings: blockEditorSettings
  }), (0,external_gc_element_namespaceObject.createElement)(Sidebar, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_components_namespaceObject.Popover.Slot, null), (0,external_gc_element_namespaceObject.createElement)(external_gc_plugins_namespaceObject.PluginArea, {
    onError: onPluginAreaError
  }), (0,external_gc_element_namespaceObject.createElement)(UnsavedChangesWarning, null)));
}
...
function initializeEditor(id, settings) {
  ...
  const coreBlocks = (0,external_gc_blockLibrary_namespaceObject.__experimentalGetCoreBlocks)().filter(block => {
  ...
  (0,external_gc_data_namespaceObject.dispatch)(external_gc_preferences_namespaceObject.store).setDefaults('core/edit-widgets', {
    fixedToolbar: false,
  ...
  (0,external_gc_widgets_namespaceObject.registerLegacyWidgetVariations)(settings);
  registerBlock(widget_area_namespaceObject);
  (0,external_gc_widgets_namespaceObject.registerWidgetGroupBlock)();

  settings.__experimentalFetchLinkSuggestions = (search, searchOptions) => (0,external_gc_coreData_namespaceObject.__experimentalFetchLinkSuggestions)(search, searchOptions, settings); // As we are unregistering `core/freeform` to avoid the Classic block, we must
  // replace it with something as the default freeform content handler. Failure to
  // do this will result in errors in the default block parser.
  // see: https://github.com/GeChiUI/gutenberg/issues/33097


  (0,external_gc_blocks_namespaceObject.setFreeformContentHandlerName)('core/html');
  root.render((0,external_gc_element_namespaceObject.createElement)(layout, {
...
function reinitializeEditor() {
  external_gc_deprecated_default()('gc.editWidgets.reinitializeEditor', {
    since: '6.2',
    version: '6.3'
  });
}
...
const registerBlock = block => {
  ...
  if (metadata) {
    (0,external_gc_blocks_namespaceObject.unstable__bootstrapServerSideBlockDefinitions)({
      [name]: metadata
    });
  }

  (0,external_gc_blocks_namespaceObject.registerBlockType)(name, settings);
};

}();
(window.gc = window.gc || {}).editWidgets = __webpack_exports__;
/******/ })()
;