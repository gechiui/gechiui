this["gc"] = this["gc"] || {}; this["gc"]["editSite"] =
...
/******/ 	return __webpack_require__(__webpack_require__.s = "3MWg");
/******/ })
/************************************************************************/
/******/ ({
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/store/defaults.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/navigation-sidebar/navigation-panel/constants.js
/**
 * GeChiUI dependencies
 */
...
const TEMPLATE_PARTS_SUB_MENUS = [{
  ...
  title: Object(external_gc_i18n_["__"])('边栏')
}, {
  area: TEMPLATE_PART_AREA_GENERAL,
  menu: MENU_TEMPLATE_PARTS_GENERAL,
  title: Object(external_gc_i18n_["__"])('常规')
}];
...
function* actions_setTemplate(templateId, templateSlug) {
  ...
  if (!templateSlug) {
    const template = yield external_gc_data_["controls"].resolveSelect(external_gc_coreData_["store"], 'getEntityRecord', 'postType', 'gc_template', templateId);
...
function* addTemplate(template) {
  const newTemplate = yield external_gc_data_["controls"].dispatch(external_gc_coreData_["store"], 'saveEntityRecord', 'postType', 'gc_template', template);

  if (template.content) {
    yield external_gc_data_["controls"].dispatch(external_gc_coreData_["store"], 'editEntityRecord', 'postType', 'gc_template', newTemplate.id, {
      blocks: Object(external_gc_blocks_["parse"])(template.content)
    }, {
      undoIgnore: true
    });
  }
...
function* actions_removeTemplate(template) {
  try {
    yield external_gc_data_["controls"].dispatch(external_gc_coreData_["store"], 'deleteEntityRecord', 'postType', template.type, template.id, {
      force: true
    });
    const lastError = yield external_gc_data_["controls"].select(external_gc_coreData_["store"], 'getLastEntityDeleteError', 'postType', template.type, template.id);

    if (lastError) {
      throw lastError;
    }

    yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createSuccessNotice', Object(external_gc_i18n_["sprintf"])(
    /* translators: The template/part's name. */
    Object(external_gc_i18n_["__"])('“%s”已移除。'), template.title.rendered), {
      type: 'snackbar'
    });
  } catch (error) {
    const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('删除模板时出错。');
    yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createErrorNotice', errorMessage, {
      type: 'snackbar'
    });
  }
}
...
function* actions_setPage(page) {
  ...
  if (!page.path && (_page$context = page.context) !== null && _page$context !== void 0 && _page$context.postId) {
    const entity = yield external_gc_data_["controls"].resolveSelect(external_gc_coreData_["store"], 'getEntityRecord', 'postType', page.context.postType || 'post', page.context.postId);
...
function* actions_revertTemplate(template) {
  ...
  if (!isTemplateRevertable(template)) {
    yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createErrorNotice', Object(external_gc_i18n_["__"])('此模板无法恢复。'), {
      type: 'snackbar'
    });
    return;
  }
  ...
  try {
    ...
    const templateEntity = yield external_gc_data_["controls"].select(external_gc_coreData_["store"], 'getEntity', 'postType', template.type);

    if (!templateEntity) {
      yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createErrorNotice', Object(external_gc_i18n_["__"])('编辑器意外错误。请重新加载。'), {
        type: 'snackbar'
      });
      return;
    }

    const fileTemplatePath = Object(external_gc_url_["addQueryArgs"])(`${templateEntity.baseURL}/${template.id}`, {
      context: 'edit',
      source: 'theme'
    });
    const fileTemplate = yield Object(external_gc_dataControls_["apiFetch"])({
    ...
    const edited = yield external_gc_data_["controls"].select(external_gc_coreData_["store"], 'getEditedEntityRecord', 'postType', template.type, template.id); // We are fixing up the undo level here to make sure we can undo
    // the revert in the header toolbar correctly.

    yield external_gc_data_["controls"].dispatch(external_gc_coreData_["store"], 'editEntityRecord', 'postType', template.type, template.id, {
    ...
    const blocks = Object(external_gc_blocks_["parse"])(fileTemplate === null || fileTemplate === void 0 ? void 0 : (_fileTemplate$content = fileTemplate.content) === null || _fileTemplate$content === void 0 ? void 0 : _fileTemplate$content.raw);
    yield external_gc_data_["controls"].dispatch(external_gc_coreData_["store"], 'editEntityRecord', 'postType', template.type, fileTemplate.id, {
      content: serializeBlocks,
      blocks,
      source: 'theme'
    });
    ...
    if (allowUndo) {
      const undoRevert = async () => {
        await Object(external_gc_data_["dispatch"])(external_gc_coreData_["store"]).editEntityRecord('postType', template.type, edited.id, {
          content: serializeBlocks,
          blocks: edited.blocks,
          source: 'custom'
        });
      ...
      yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createSuccessNotice', Object(external_gc_i18n_["__"])('模板已还原。'), {
        type: 'snackbar',
        actions: [{
    ...
    const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('模板还原失败。请重新加载。');
    yield external_gc_data_["controls"].dispatch(external_gc_notices_["store"], 'createErrorNotice', errorMessage, {
      type: 'snackbar'
    });
  }
...
function* openGeneralSidebar(name) {
  yield external_gc_data_["controls"].dispatch(build_module["i" /* store */], 'enableComplementaryArea', STORE_NAME, name);
}
/**
...
const getCanUserCreateMedia = Object(external_gc_data_["createRegistrySelector"])(select => () => select(external_gc_coreData_["store"]).canUser('create', 'media'));
/**
...
const getReusableBlocks = Object(external_gc_data_["createRegistrySelector"])(select => () => {
  const isWeb = external_gc_element_["Platform"].OS === 'web';
  return isWeb ? select(external_gc_coreData_["store"]).getEntityRecords('postType', 'gc_block', {
    per_page: -1
  }) : [];
});
/**
...
const getCurrentTemplateNavigationPanelSubMenu = Object(external_gc_data_["createRegistrySelector"])(select => state => {
  const templateType = selectors_getEditedPostType(state);
  const templateId = selectors_getEditedPostId(state);
  const template = templateId ? select(external_gc_coreData_["store"]).getEntityRecord('postType', templateType, templateId) : null;

  if (!template) {
    return MENU_ROOT;
  }

  if ('gc_template_part' === templateType) {
  ...
  const templates = select(external_gc_coreData_["store"]).getEntityRecords('postType', 'gc_template');
  const showOnFront = select(external_gc_coreData_["store"]).getEditedEntityRecord('root', 'site').show_on_front;
...
const getCurrentTemplateTemplateParts = Object(external_gc_data_["createRegistrySelector"])(select => state => {
  var _template$blocks;

  const templateType = selectors_getEditedPostType(state);
  const templateId = selectors_getEditedPostId(state);
  const template = select(external_gc_coreData_["store"]).getEditedEntityRecord('postType', templateType, templateId);
  const templateParts = select(external_gc_coreData_["store"]).getEntityRecords('postType', 'gc_template_part', {
    per_page: -1
  });
  const templatePartsById = Object(external_lodash_["keyBy"])(templateParts, templatePart => templatePart.id);
  return ((_template$blocks = template.blocks) !== null && _template$blocks !== void 0 ? _template$blocks : []).filter(block => Object(external_gc_blocks_["isTemplatePart"])(block)).map(block => {
    const {
      attributes: {
        theme,
        slug
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/utils/history.js
/**
 * External dependencies
 */
...
function history_push(params, state) {
  return originalHistoryPush.call(history_history, Object(external_gc_url_["addQueryArgs"])(window.location.href, params), state);
}

function history_replace(params, state) {
  return originalHistoryReplace.call(history_history, Object(external_gc_url_["addQueryArgs"])(window.location.href, params), state);
}
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/routes/index.js
...
const RoutesContext = Object(external_gc_element_["createContext"])();
const HistoryContext = Object(external_gc_element_["createContext"])();
function useLocation() {
  return Object(external_gc_element_["useContext"])(RoutesContext);
}
function useHistory() {
  return Object(external_gc_element_["useContext"])(HistoryContext);
}
...
function Routes(_ref) {
  ...
  const [location, setLocation] = Object(external_gc_element_["useState"])(() => getLocationWithParams(utils_history.location));
  Object(external_gc_element_["useEffect"])(() => {
  ...
  return Object(external_gc_element_["createElement"])(HistoryContext.Provider, {
    value: utils_history
  }, Object(external_gc_element_["createElement"])(RoutesContext.Provider, {
    value: location
  }, children(location)));
}
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/feature-toggle/index.js
...
function FeatureToggle(_ref) {
  ...
  const isActive = Object(external_gc_data_["useSelect"])(select => {
...
function SiteExport() {
  ...
  }, Object(external_gc_i18n_["_x"])('导出', 'site exporter menu item'));
}
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/undo-redo/redo.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/document-actions/index.js
...
function getBlockDisplayText(block) {
  if (block) {
    const blockType = Object(external_gc_blocks_["getBlockType"])(block.name);
    return blockType ? Object(external_gc_blocks_["__experimentalGetBlockLabel"])(blockType, block.attributes) : null;
  }
...
function useSecondaryText() {
  ...
  const activeEntityBlockId = Object(external_gc_data_["useSelect"])(select => select(external_gc_blockEditor_["store"]).__experimentalGetActiveBlockIdByBlockNames(['core/template-part']), []);
...
function DocumentActions(_ref) {
  ...
  const titleRef = Object(external_gc_element_["useRef"])(); // Return a simple loading indicator until we have information to show.

  if (!isLoaded) {
    return Object(external_gc_element_["createElement"])("div", {
      className: "edit-site-document-actions"
    }, Object(external_gc_i18n_["__"])('正在加载…'));
  ...
  if (!entityTitle) {
    return Object(external_gc_element_["createElement"])("div", {
      className: "edit-site-document-actions"
    }, Object(external_gc_i18n_["__"])('未找到模板'));
  }
  ...
  Object(external_gc_i18n_["__"])('编辑%s：'), entityLabel)), entityTitle), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalText"], {
    size: "body",
    className: "edit-site-document-actions__secondary-item"
  }, label !== null && label !== void 0 ? label : ''), dropdownContent && Object(external_gc_element_["createElement"])(external_gc_components_["Dropdown"], {
    popoverProps: {
    ...
    renderToggle: _ref2 => {
      ...
      return Object(external_gc_element_["createElement"])(external_gc_components_["Button"], {
        ...
        Object(external_gc_i18n_["__"])('显示%s的详细信息'), entityLabel)
      });
    },
...
function useLink() {
  ...
  return {
    href: Object(external_gc_url_["addQueryArgs"])(window.location.href, params),
    onClick
  };
}
function Link(_ref) {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/template-details/template-areas.js
...
function TemplatePartItemMore(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(external_gc_components_["MenuGroup"], null, Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], Object(esm_extends["a" /* default */])({}, editLinkProps, {
    onClick: editTemplatePart
  }), Object(external_gc_i18n_["sprintf"])(
  /* translators: %s: template part title */
  Object(external_gc_i18n_["__"])('编辑 %s'), (_templatePart$title = templatePart.title) === null || _templatePart$title === void 0 ? void 0 : _templatePart$title.rendered))), isTemplateRevertable(templatePart) && Object(external_gc_element_["createElement"])(external_gc_components_["MenuGroup"], null, Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
    info: Object(external_gc_i18n_["__"])('将模板还原至默认状态'),
    onClick: clearCustomizations
  }, Object(external_gc_i18n_["__"])('清除自定义选项'))));
}
...
function TemplatePartItem(_ref2) {
  ...
  const templatePartArea = Object(external_gc_data_["useSelect"])(select => {
    const defaultAreas = select(external_gc_editor_["store"]).__experimentalGetDefaultTemplatePartAreas();
  ...
  }, templatePartArea === null || templatePartArea === void 0 ? void 0 : templatePartArea.label), Object(external_gc_element_["createElement"])(external_gc_components_["DropdownMenu"], {
    icon: more_vertical["a" /* default */],
    label: Object(external_gc_i18n_["__"])('更多选项'),
    className: "edit-site-template-details__template-areas-item-more"
  }, _ref3 => {
    let {
      onClose
    } = _ref3;
    return Object(external_gc_element_["createElement"])(TemplatePartItemMore, {
      onClose: onClose,
...
function TemplateAreas(_ref4) {
  ...
  const templateParts = Object(external_gc_data_["useSelect"])(select => select(store).getCurrentTemplateTemplateParts(), []);
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["MenuGroup"], {
    ...
    return Object(external_gc_element_["createElement"])(TemplatePartItem, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/template-details/edit-template-title.js
...
function EditTemplateTitle(_ref) {
  ...
  const [title, setTitle] = Object(external_gc_coreData_["useEntityProp"])('postType', template.type, 'title', template.id);
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/template-details/index.js
...
function TemplateDetails(_ref) {
  ...
  const {
    title,
    description
  } = Object(external_gc_data_["useSelect"])(select => select(external_gc_editor_["store"]).__experimentalGetTemplateInfo(template), []);
  const {
    revertTemplate
  } = Object(external_gc_data_["useDispatch"])(store);
  const templateSubMenu = Object(external_gc_element_["useMemo"])(() => {
    if ((template === null || template === void 0 ? void 0 : template.type) === 'gc_template') {
      return {
        title: Object(external_gc_i18n_["__"])('模板'),
        menu: MENU_TEMPLATES
      };
    }
  ...
  }, template.is_custom ? Object(external_gc_element_["createElement"])(EditTemplateTitle, {
    template: template
  }) : Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHeading"], {
    level: 4,
    weight: 600,
    className: "edit-site-template-details__title"
  }, title), description && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalText"], {
    size: "body",
    className: "edit-site-template-details__description",
    as: "p"
  }, description)), Object(external_gc_element_["createElement"])(TemplateAreas, {
    closeTemplateDetailsDropdown: onClose
  }), isTemplateRevertable(template) && Object(external_gc_element_["createElement"])(external_gc_components_["MenuGroup"], {
    className: "edit-site-template-details__group edit-site-template-details__revert"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
    className: "edit-site-template-details__revert-button",
    info: Object(external_gc_i18n_["__"])('将模板还原至默认状态'),
    onClick: revert
  }, Object(external_gc_i18n_["__"])('清除自定义选项'))), Object(external_gc_element_["createElement"])(external_gc_components_["Button"], Object(esm_extends["a" /* default */])({
    className: "edit-site-template-details__show-all-button"
  }, browseAllLinkProps), Object(external_gc_i18n_["sprintf"])(
  /* translators: the template part's area name ("Headers", "Sidebars") or "templates". */
  Object(external_gc_i18n_["__"])('浏览所有%s'), templateSubMenu.title)));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/index.js
...
function Header(_ref) {
  ...
  const isFocusMode = templateType === 'gc_template_part';
  return Object(external_gc_element_["createElement"])("div", {
  ...
  }, Object(external_gc_element_["createElement"])(DocumentActions, {
    entityTitle: entityTitle,
    entityLabel: templateType === 'gc_template_part' ? 'template part' : 'template',
    isLoaded: isLoaded
  }, _ref2 => {
    let {
      onClose
    } = _ref2;
    return Object(external_gc_element_["createElement"])(TemplateDetails, {
      template: template,
      onClose: onClose
    });
  })), Object(external_gc_element_["createElement"])("div", {
  ...
  }, !isFocusMode && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalPreviewOptions"], {
    deviceType: deviceType,
    setDeviceType: setPreviewDeviceType
  }), Object(external_gc_element_["createElement"])(SaveButton, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/default-sidebar.js
...
function DefaultSidebar(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(build_module["b" /* ComplementaryArea */], {
    className: className,
    scope: "core/edit-site",
    identifier: identifier,
    title: title,
  ...
  }, children), Object(external_gc_element_["createElement"])(build_module["c" /* ComplementaryAreaMoreMenuItem */], {
    scope: "core/edit-site",
    identifier: identifier,
    icon: icon
  }, title));
...
const styles_styles = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  viewBox: "0 0 24 24",
  xmlns: "http://www.w3.org/2000/svg"
}, Object(external_gc_element_["createElement"])(external_gc_primitives_["Path"], {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/utils.js
/**
 * External dependencies
 */
...
function getValueFromVariable(features, blockName, variable) {
  ...
  const THEME_VALUE_PREFIX = 'var(--gc--';
  ...
  if (variable.startsWith(USER_VALUE_PREFIX)) {
    ...
    // We don't know how to parse the value: either is raw of uses complex CSS such as `calc(1px * var(--gc--variable) )`
    return variable;
  }
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/context.js
/**
 * GeChiUI dependencies
 */
...
const GlobalStylesContext = Object(external_gc_element_["createContext"])(DEFAULT_GLOBAL_STYLES_CONTEXT);
...
const useGlobalStylesReset = () => {
  const {
    user: config,
    setUserConfig
  } = Object(external_gc_element_["useContext"])(GlobalStylesContext);
  const canReset = !!config && !Object(external_lodash_["isEqual"])(config, EMPTY_CONFIG);
  return [canReset, Object(external_gc_element_["useCallback"])(() => setUserConfig(() => EMPTY_CONFIG), [setUserConfig])];
};
function useSetting(path, blockName) {
  ...
  const setSetting = newValue => {
    setUserConfig(currentConfig => {
      ...
      const pathToSet = external_gc_blocks_["__EXPERIMENTAL_PATHS_WITH_MERGE"][path] ? fullPath + '.custom' : fullPath;
  ...
  const getSettingValueForContext = name => {
    ...
    const getSettingValue = configToUse => {
      ...
      if (external_gc_blocks_["__EXPERIMENTAL_PATHS_WITH_MERGE"][path]) {
...
function getSupportedGlobalStylesPanels(name) {
  ...
  const blockType = Object(external_gc_blocks_["getBlockType"])(name);
  ...
  const supportKeys = [];
  Object.keys(external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"]).forEach(styleName => {
    if (!external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"][styleName].support) {
      return;
    ...
    if (external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"][styleName].requiresOptOut) {
      if (Object(external_lodash_["has"])(blockType.supports, external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"][styleName].support[0]) && Object(external_lodash_["get"])(blockType.supports, external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"][styleName].support) !== false) {
        return supportKeys.push(styleName);
      }
    }

    if (Object(external_lodash_["get"])(blockType.supports, external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"][styleName].support, false)) {
      return supportKeys.push(styleName);
    }
  });
  return supportKeys;
...
function useColorsPerOrigin(name) {
  ...
  return Object(external_gc_element_["useMemo"])(() => {
    ...
    if (themeColors && themeColors.length) {
      result.push({
        name: Object(external_gc_i18n_["_x"])('主题', 'Indicates this palette comes from the theme.'),
        colors: themeColors
      });
    }
    ...
    if (shouldDisplayDefaultColors && defaultColors && defaultColors.length) {
      result.push({
        name: Object(external_gc_i18n_["_x"])('默认', 'Indicates this palette comes from GeChiUI.'),
    ...
    if (customColors && customColors.length) {
      result.push({
        name: Object(external_gc_i18n_["_x"])('自定义', 'Indicates this palette is created by the user.'),
        colors: customColors
      });
    }
...
const StylesPreview = () => {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["Card"], {
  ...
  }, "Aa"), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    spacing: 2
  }, Object(external_gc_element_["createElement"])(external_gc_components_["ColorIndicator"], {
...
function NavigationButton(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItem"], Object(esm_extends["a" /* default */])({
    onClick: () => navigator.push(path, {
      isBack
    })
  }, props), icon && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHStack"], {
    justify: "flex-start"
    ...
    icon: icon,
    size: 24
  })), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, children)), !icon && children);
}
...
const typography = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Object(external_gc_element_["createElement"])(external_gc_primitives_["Path"], {
...
const color_color = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  viewBox: "0 0 24 24",
  xmlns: "http://www.w3.org/2000/svg"
}, Object(external_gc_element_["createElement"])(external_gc_primitives_["Path"], {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/border-panel.js
...
function BorderPanel(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["PanelBody"], {
    title: Object(external_gc_i18n_["__"])('边框'),
    initialOpen: true
  }, (hasBorderWidth || hasBorderStyle) && Object(external_gc_element_["createElement"])("div", {
    className: "edit-site-global-styles-sidebar__border-controls-row"
  }, hasBorderWidth && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalUnitControl"], {
    ...
    units: units
  }), hasBorderStyle && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalBorderStyleControl"], {
    value: borderStyle,
    onChange: setBorderStyle
  })), hasBorderColor && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalColorGradientControl"], {
  ...
  }), hasBorderRadius && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalBorderRadiusControl"], {
    values: borderRadiusValues,
    onChange: setBorderRadius
  }));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/color-utils.js
/**
 * Internal dependencies
 */
...
function useHasGap(name) {
  ...
  return settings && supports.includes('--gc--style--block-gap');
}
...
function DimensionsPanel(_ref) {
  ...
  const paddingSides = Object(external_gc_blockEditor_["__experimentalUseCustomSides"])(name, 'padding');
  ...
  const marginSides = Object(external_gc_blockEditor_["__experimentalUseCustomSides"])(name, 'margin');
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToolsPanel"], {
    label: Object(external_gc_i18n_["__"])('尺寸'),
    resetAll: resetAll
  }, showPaddingControl && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToolsPanelItem"], {
    hasValue: hasPaddingValue,
    label: Object(external_gc_i18n_["__"])('内边距'),
    onDeselect: resetPaddingValue,
    isShownByDefault: true
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalBoxControl"], {
  ...
  })), showMarginControl && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToolsPanelItem"], {
    hasValue: hasMarginValue,
    label: Object(external_gc_i18n_["__"])('边距'),
  ...
  })), showGapControl && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToolsPanelItem"], {
    hasValue: hasGapValue,
    label: Object(external_gc_i18n_["__"])('块间​​距'),
...
function TypographyPanel(_ref) {
  ...
  }, "Aa"), supports.includes('fontFamily') && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalFontFamilyControl"], {
    fontFamilies: fontFamilies,
    value: fontFamily,
    onChange: setFontFamily
  }), supports.includes('fontSize') && Object(external_gc_element_["createElement"])(external_gc_components_["FontSizePicker"], {
    value: fontSize,
    onChange: setFontSize,
    fontSizes: fontSizes,
    disableCustomFontSizes: disableCustomFontSizes
  }), hasLineHeightEnabled && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["LineHeightControl"], {
    value: lineHeight,
    onChange: setLineHeight
  }), hasAppearanceControl && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalFontAppearanceControl"], {
    value: {
      fontStyle,
      fontWeight
    },
  ...
  }), hasLetterSpacingControl && Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalLetterSpacingControl"], {
    value: letterSpacing,
    onChange: setLetterSpacing
  }));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/context-menu.js
...
function ContextMenu(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItemGroup"], null, hasTypographyPanel && Object(external_gc_element_["createElement"])(navigation_button, {
    icon: library_typography,
    path: parentMenu + '/typography'
  }, Object(external_gc_i18n_["__"])('排版')), hasColorPanel && Object(external_gc_element_["createElement"])(navigation_button, {
    icon: library_color,
    path: parentMenu + '/colors'
  }, Object(external_gc_i18n_["__"])('颜色')), hasLayoutPanel && Object(external_gc_element_["createElement"])(navigation_button, {
    icon: layout["a" /* default */],
    path: parentMenu + '/layout'
  }, Object(external_gc_i18n_["__"])('布局')));
}
...
function ScreenRoot() {
  return Object(external_gc_element_["createElement"])(external_gc_components_["Card"], {
    size: "small"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["CardBody"], null, Object(external_gc_element_["createElement"])(preview, null)), Object(external_gc_element_["createElement"])(external_gc_components_["CardBody"], null, Object(external_gc_element_["createElement"])(context_menu, null)), Object(external_gc_element_["createElement"])(external_gc_components_["CardDivider"], null), Object(external_gc_element_["createElement"])(external_gc_components_["CardBody"], null, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItemGroup"], null, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItem"], null, Object(external_gc_i18n_["__"])('为全站定制特定区块的外观。')), Object(external_gc_element_["createElement"])(navigation_button, {
    path: "/blocks"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHStack"], {
...
function ScreenHeader(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    ...
    spacing: 2
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalView"], null, Object(external_gc_element_["createElement"])(navigation_button, {
    path: back,
...
function BlockMenuItem(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(navigation_button, {
    path: '/blocks/' + block.name
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHStack"], {
    justify: "flex-start"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockIcon"], {
    icon: block.icon
  })), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, block.title)));
}
...
function ScreenBlock(_ref) {
  ...
  }), Object(external_gc_element_["createElement"])(context_menu, {
...
function Subtitle(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHeading"], {
...
function Item(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(navigation_button, {
    ...
    justify: "flex-start"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], {
  ...
  }, Object(external_gc_i18n_["__"])('Aa')), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, label)));
}
...
function ScreenTypography(_ref2) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(global_styles_header, {
    ...
    title: Object(external_gc_i18n_["__"])('排版'),
    description: Object(external_gc_i18n_["__"])('管理不同元素的排版设置。')
  }), !name && Object(external_gc_element_["createElement"])("div", {
    className: "edit-site-global-styles-screen-typography"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    spacing: 3
  }, Object(external_gc_element_["createElement"])(subtitle, null, Object(external_gc_i18n_["__"])('元素')), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItemGroup"], {
    isBordered: true,
    isSeparated: true
  ...
  })))), !!name && Object(external_gc_element_["createElement"])(TypographyPanel, {
    name: name,
    element: "text"
  }));
}
...
function ScreenTypographyElement(_ref) {
  ...
  }), Object(external_gc_element_["createElement"])(TypographyPanel, {
    name: name,
    element: element
  }));
}
...
function Palette(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    ...
    isBordered: true,
    isSeparated: true
  }, Object(external_gc_element_["createElement"])(navigation_button, {
    path: screenPath
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHStack"], {
    isReversed: colors.length === 0
  }, Object(external_gc_element_["createElement"])(external_gc_components_["FlexBlock"], null, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalZStack"], {
    isLayered: false,
    offset: -8
    ...
    return Object(external_gc_element_["createElement"])(external_gc_components_["ColorIndicator"], {
      key: color,
      colorValue: color
    });
  }))), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, paletteButtonText)))));
}
...
function BackgroundColorItem(_ref) {
  ...
  })), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, Object(external_gc_i18n_["__"])('背景'))));
}
...
function LinkColorItem(_ref3) {
  ...
  return Object(external_gc_element_["createElement"])(navigation_button, {
    ...
    colorValue: color
  })), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, Object(external_gc_i18n_["__"])('链接'))));
}
...
function ScreenColors(_ref4) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(global_styles_header, {
    ...
    description: Object(external_gc_i18n_["__"])('管理网站上不同全局元素的调色盘和默认颜色。')
  }), Object(external_gc_element_["createElement"])("div", {
    className: "edit-site-global-styles-screen-colors"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    spacing: 10
  }, Object(external_gc_element_["createElement"])(palette, {
    name: name
  }), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    spacing: 3
  }, Object(external_gc_element_["createElement"])(subtitle, null, Object(external_gc_i18n_["__"])('元素')), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalItemGroup"], {
    isBordered: true,
    isSeparated: true
  }, Object(external_gc_element_["createElement"])(BackgroundColorItem, {
    name: name,
    parentMenu: parentMenu
  }), Object(external_gc_element_["createElement"])(TextColorItem, {
    name: name,
    parentMenu: parentMenu
  }), Object(external_gc_element_["createElement"])(LinkColorItem, {
    name: name,
    parentMenu: parentMenu
  }))))));
}
...
function ColorPalettePanel(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    ...
    spacing: 10
  }, !!themeColors && !!themeColors.length && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalPaletteEdit"], {
    canReset: themeColors !== baseThemeColors,
    canOnlyChangeValues: true,
    colors: themeColors,
    onChange: setThemeColors,
    paletteLabel: Object(external_gc_i18n_["__"])('主题')
  }), !!defaultColors && !!defaultColors.length && !!defaultPaletteEnabled && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalPaletteEdit"], {
    canReset: defaultColors !== baseDefaultColors,
    canOnlyChangeValues: true,
    colors: defaultColors,
    onChange: setDefaultColors,
    paletteLabel: Object(external_gc_i18n_["__"])('默认')
  }), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalPaletteEdit"], {
    colors: customColors,
    onChange: setCustomColors,
    paletteLabel: Object(external_gc_i18n_["__"])('自定义'),
    emptyMessage: Object(external_gc_i18n_["__"])('自定义颜色为空！请添加一些颜色来创建您自己的调色盘。'),
    slugPrefix: "custom-"
  }));
}
...
function GradientPalettePanel(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalVStack"], {
    ...
    spacing: 10
  }, !!themeGradients && !!themeGradients.length && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalPaletteEdit"], {
    canReset: themeGradients !== baseThemeGradients,
    canOnlyChangeValues: true,
    gradients: themeGradients,
    onChange: setThemeGradients,
    paletteLabel: Object(external_gc_i18n_["__"])('主题')
  }), !!defaultGradients && !!defaultGradients.length && !!defaultPaletteEnabled && Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalPaletteEdit"], {
    ...
    paletteLabel: Object(external_gc_i18n_["__"])('自定义'),
    emptyMessage: Object(external_gc_i18n_["__"])('自定义渐变为空！请添加一些渐变来创建您自己的调色盘。'),
    slugPrefix: "custom-"
  }), Object(external_gc_element_["createElement"])("div", null, Object(external_gc_element_["createElement"])(subtitle, null, Object(external_gc_i18n_["__"])('双色')), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalSpacer"], {
    margin: 3
  }), Object(external_gc_element_["createElement"])(external_gc_components_["DuotonePicker"], {
...
function ScreenColorPalette(_ref) {
  ...
  const [currentTab, setCurrentTab] = Object(external_gc_element_["useState"])('solid');
  const parentMenu = name === undefined ? '' : '/blocks/' + name;
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(global_styles_header, {
    back: parentMenu + '/colors',
    title: Object(external_gc_i18n_["__"])('调色板'),
    description: Object(external_gc_i18n_["__"])('调色盘用于给区块和各种设计工具提供默认颜色选项。您可以在此处编辑带标签的颜色。')
    ...
    label: Object(external_gc_i18n_["__"])('选择调色盘类型'),
    hideLabelFromVision: true,
    isBlock: true
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToggleGroupControlOption"], {
    value: "solid",
    label: Object(external_gc_i18n_["__"])('纯色')
  }), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalToggleGroupControlOption"], {
    value: "gradient",
    label: Object(external_gc_i18n_["__"])('渐变色')
  })), currentTab === 'solid' && Object(external_gc_element_["createElement"])(ColorPalettePanel, {
    name: name
  }), currentTab === 'gradient' && Object(external_gc_element_["createElement"])(GradientPalettePanel, {
    name: name
  }));
}
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/screen-text-color.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/screen-link-color.js
...
function ScreenLayout(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(global_styles_header, {
    back: parentMenu ? parentMenu : '/',
    title: Object(external_gc_i18n_["__"])('布局')
  }), hasDimensionsPanel && Object(external_gc_element_["createElement"])(DimensionsPanel, {
    name: name
  }), hasBorderPanel && Object(external_gc_element_["createElement"])(BorderPanel, {
    name: name
  }));
}
...
function ContextScreens(_ref) {
  ...
  const parentMenu = name === undefined ? '' : '/blocks/' + name;
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/typography'
  }, Object(external_gc_element_["createElement"])(screen_typography, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/typography/text'
  }, Object(external_gc_element_["createElement"])(screen_typography_element, {
    name: name,
    element: "text"
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    ...
    path: parentMenu + '/colors'
  }, Object(external_gc_element_["createElement"])(screen_colors, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/colors/palette'
  }, Object(external_gc_element_["createElement"])(screen_color_palette, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/colors/background'
  }, Object(external_gc_element_["createElement"])(screen_background_color, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/colors/text'
  }, Object(external_gc_element_["createElement"])(screen_text_color, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/colors/link'
  }, Object(external_gc_element_["createElement"])(screen_link_color, {
    name: name
  })), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: parentMenu + '/layout'
  }, Object(external_gc_element_["createElement"])(screen_layout, {
    name: name
  })));
}

function GlobalStylesUI() {
  const blocks = Object(external_gc_blocks_["getBlockTypes"])();
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorProvider"], {
    initialPath: "/"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: "/"
  }, Object(external_gc_element_["createElement"])(screen_root, null)), Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    path: "/blocks"
  }, Object(external_gc_element_["createElement"])(screen_block_list, null)), blocks.map(block => Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigatorScreen"], {
    key: 'menu-block-' + block.name,
    path: '/blocks/' + block.name
  }, Object(external_gc_element_["createElement"])(screen_block, {
    name: block.name
  }))), Object(external_gc_element_["createElement"])(ContextScreens, null), blocks.map(block => Object(external_gc_element_["createElement"])(ContextScreens, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/global-styles/use-global-styles-output.js
/**
 * External dependencies
 */
    ...
    ['default', 'theme', 'custom'].forEach(origin => {
      if (presetByOrigin[origin]) {
        presetByOrigin[origin].forEach(value => {
          declarations.push(`--gc--preset--${cssVarInfix}--${Object(external_lodash_["kebabCase"])(value.slug)}: ${value[valueKey]}`);
        });
      }
    });
    return declarations;
      ...
      if (presetByOrigin[origin]) {
        presetByOrigin[origin].forEach(_ref3 => {
          ...
          classes.forEach(_ref4 => {
            ...
            const value = `var(--gc--preset--${cssVarInfix}--${Object(external_lodash_["kebabCase"])(slug)})`;
...
function getStylesDeclarations() {
  ...
  return Object(external_lodash_["reduce"])(external_gc_blocks_["__EXPERIMENTAL_STYLE_PROPERTY"], (declarations, _ref5, key) => {
    let {
      value,
      properties
    } = _ref5;
...
const getNodesWithStyles = (tree, blockSelectors) => {
  ...
  Object(external_lodash_["forEach"])((_tree$styles = tree.styles) === null || _tree$styles === void 0 ? void 0 : _tree$styles.elements, (value, key) => {
    if (!!value && !!external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"][key]) {
      nodes.push({
        styles: value,
        selector: external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"][key]
      });
    }
  ...
  Object(external_lodash_["forEach"])((_tree$styles2 = tree.styles) === null || _tree$styles2 === void 0 ? void 0 : _tree$styles2.blocks, (node, blockName) => {
    ...
    Object(external_lodash_["forEach"])(node === null || node === void 0 ? void 0 : node.elements, (value, elementName) => {
      if (!!value && !!(blockSelectors !== null && blockSelectors !== void 0 && blockSelectors[blockName]) && !!(external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"] !== null && external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"] !== void 0 && external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"][elementName])) {
        nodes.push({
          styles: value,
          selector: blockSelectors[blockName].selector.split(',').map(sel => sel + ' ' + external_gc_blocks_["__EXPERIMENTAL_ELEMENTS"][elementName]).join(',')
        });
      }
    });
  });
...
const toCustomProperties = (tree, blockSelectors) => {
  ...
  settings.forEach(_ref7 => {
    ...
    const customProps = flattenTree(custom, '--gc--custom--', '--');
...
const toStyles = (tree, blockSelectors) => {
  ...
  let ruleset = '.gc-site-blocks > * { margin-top: 0; margin-bottom: 0; }.gc-site-blocks > * + * { margin-top: var( --gc--style--block-gap ); }';
...
const getBlockSelectors = blockTypes => {
  ...
  blockTypes.forEach(blockType => {
    ...
    const selector = (_blockType$supports$_ = blockType === null || blockType === void 0 ? void 0 : (_blockType$supports = blockType.supports) === null || _blockType$supports === void 0 ? void 0 : _blockType$supports.__experimentalSelector) !== null && _blockType$supports$_ !== void 0 ? _blockType$supports$_ : '.gc-block-' + name.replace('core/', '').replace('/', '-');
    result[name] = {
      name,
      selector
    };
...
function useGlobalStylesOutput() {
  const [stylesheets, setStylesheets] = Object(external_gc_element_["useState"])([]);
  const [settings, setSettings] = Object(external_gc_element_["useState"])({});
  const {
    merged: mergedConfig
  } = Object(external_gc_element_["useContext"])(GlobalStylesContext);
  Object(external_gc_element_["useEffect"])(() => {
    ...
    const blockSelectors = getBlockSelectors(Object(external_gc_blocks_["getBlockTypes"])());
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/global-styles-sidebar.js
...
function GlobalStylesSidebar() {
  ...
  return Object(external_gc_element_["createElement"])(DefaultSidebar, {
    className: "edit-site-global-styles-sidebar",
    identifier: "edit-site/global-styles",
    title: Object(external_gc_i18n_["__"])('样式'),
    icon: library_styles,
    ...
    header: Object(external_gc_element_["createElement"])(external_gc_components_["Flex"], null, Object(external_gc_element_["createElement"])(external_gc_components_["FlexBlock"], null, Object(external_gc_element_["createElement"])("strong", null, Object(external_gc_i18n_["__"])('样式')), Object(external_gc_element_["createElement"])("span", {
      ...
      controls: [{
        title: Object(external_gc_i18n_["__"])('重置为默认'),
        onClick: onReset
      }, {
        title: Object(external_gc_i18n_["__"])('欢迎指南'),
        onClick: () => toggleFeature('welcomeGuideStyles')
      }]
    })))
  }, Object(external_gc_element_["createElement"])(ui, null));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/constants.js
const SIDEBAR_TEMPLATE = 'edit-site/template';
const SIDEBAR_BLOCK = 'edit-site/block-inspector';

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/settings-header/index.js
...
const SettingsHeader = _ref => {
  ...
  [Object(external_gc_i18n_["__"])('模板（选定）'), 'is-active'] : // translators: ARIA label for the Template Settings Sidebar tab, not selected.
  [Object(external_gc_i18n_["__"])('模板'), ''];
  ...
  return Object(external_gc_element_["createElement"])("ul", null, Object(external_gc_element_["createElement"])("li", null, Object(external_gc_element_["createElement"])(external_gc_components_["Button"], {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/template-card/template-areas.js
...
function template_areas_TemplateAreas() {
  ...
  return Object(external_gc_element_["createElement"])("section", {
  ...
  }, Object(external_gc_i18n_["__"])('区域')), Object(external_gc_element_["createElement"])("ul", {
    ...
    return Object(external_gc_element_["createElement"])("li", {
      key: templatePart.slug
    }, Object(external_gc_element_["createElement"])(TemplateAreaItem, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/sidebar/template-card/index.js
...
function TemplateCard() {
  ...
  }, title), Object(external_gc_element_["createElement"])("div", {
    className: "edit-site-template-card__description"
  }, description), Object(external_gc_element_["createElement"])(template_areas_TemplateAreas, null)));
}
...
const {
  Slot: InspectorSlot,
  Fill: InspectorFill
} = Object(external_gc_components_["createSlotFill"])('EditSiteSidebarInspector');
...
function SidebarComplementaryAreaFills() {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(DefaultSidebar, {
    identifier: sidebarName,
    title: Object(external_gc_i18n_["__"])('设置'),
    icon: cog["a" /* default */],
    closeLabel: Object(external_gc_i18n_["__"])('关闭设置边栏'),
    header: Object(external_gc_element_["createElement"])(settings_header, {
      sidebarName: sidebarName
    }),
    headerClassName: "edit-site-sidebar__panel-tabs"
  }, sidebarName === SIDEBAR_TEMPLATE && Object(external_gc_element_["createElement"])(external_gc_components_["PanelBody"], null, Object(external_gc_element_["createElement"])(TemplateCard, null)), sidebarName === SIDEBAR_BLOCK && Object(external_gc_element_["createElement"])(InspectorSlot, {
    bubblesVirtually: true
  })), Object(external_gc_element_["createElement"])(GlobalStylesSidebar, null));
}
...
const home = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Object(external_gc_element_["createElement"])(external_gc_primitives_["Path"], {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/main-dashboard-button/index.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/navigation-sidebar/navigation-panel/index.js
...
function NavLink(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigationItem"], Object(esm_extends["a" /* default */])({}, linkProps, props));
}
...
const NavigationPanel = _ref2 => {
  ...
  return (// eslint-disable-next-line jsx-a11y/no-static-element-interactions
    Object(external_gc_element_["createElement"])("div", {
    ...
    }, Object(external_gc_htmlEntities_["decodeEntities"])(siteTitle))), Object(external_gc_element_["createElement"])("div", {
      className: "edit-site-navigation-panel__scroll-container"
    }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigation"], {
      activeItem: activeItem
    }, Object(external_gc_element_["createElement"])(main_dashboard_button.Slot, null, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalNavigationBackButton"], {
      backButtonLabel: Object(external_gc_i18n_["__"])('仪表盘'),
      ...
      title: Object(external_gc_i18n_["__"])('模版组件'),
      item: "gc_template_part",
      params: {
        postId: undefined,
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/navigation-sidebar/navigation-toggle/index.js
...
function NavigationToggle(_ref) {
  ...
  const navigationToggleRef = Object(external_gc_element_["useRef"])();
  Object(external_gc_element_["useEffect"])(() => {
    // TODO: Remove this effect when alternative solution is merged.
    // See: https://github.com/GeChiUI/gutenberg/pull/37314
    if (!isNavigationOpen) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__unstableMotion"].div, {
    ...
    label: Object(external_gc_i18n_["__"])('切换导航'),
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/navigation-sidebar/index.js
...
} = Object(external_gc_components_["createSlotFill"])('EditSiteNavigationPanelPreview');
const {
  Fill: NavigationSidebarFill,
  Slot: NavigationSidebarSlot
} = Object(external_gc_components_["createSlotFill"])('EditSiteNavigationSidebar');
...
function NavigationSidebar(_ref) {
  ...
  const isDesktopViewport = Object(external_gc_compose_["useViewportMatch"])('medium');
  const {
    setIsNavigationPanelOpened
  } = Object(external_gc_data_["useDispatch"])(store);
  Object(external_gc_element_["useEffect"])(function autoOpenNavigationPanelOnViewportChange() {
    setIsNavigationPanelOpened(isDefaultOpen && isDesktopViewport);
  }, [isDefaultOpen, isDesktopViewport, setIsNavigationPanelOpened]);
  return Object(external_gc_element_["createElement"])(NavigationSidebarFill, null, Object(external_gc_element_["createElement"])(navigation_toggle, null), Object(external_gc_element_["createElement"])(navigation_panel, {
    activeItem: activeTemplateType
  }), Object(external_gc_element_["createElement"])(NavigationPanelPreviewSlot, null));
}
...
function ConvertToRegularBlocks(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockSettingsMenuControls"], null, _ref2 => {
    ...
    }, Object(external_gc_i18n_["__"])('从模板组件分离区块'));
  });
}
...
function CreateTemplatePartModal(_ref) {
  ...
  const [area, setArea] = Object(external_gc_element_["useState"])(TEMPLATE_PART_AREA_GENERAL);
  const [isSubmitting, setIsSubmitting] = Object(external_gc_element_["useState"])(false);
  const instanceId = Object(external_gc_compose_["useInstanceId"])(CreateTemplatePartModal);
  const templatePartAreas = Object(external_gc_data_["useSelect"])(select => select(external_gc_editor_["store"]).__experimentalGetDefaultTemplatePartAreas(), []);
  return Object(external_gc_element_["createElement"])(external_gc_components_["Modal"], {
    title: Object(external_gc_i18n_["__"])('创建模板组件'),
    closeLabel: Object(external_gc_i18n_["__"])('关闭'),
  ...
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalRadioGroup"], {
    label: Object(external_gc_i18n_["__"])('区域'),
    ...
    return Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalRadio"], {
      key: label,
      value: value,
      ...
      align: "start",
      justify: "start"
    }, Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], null, Object(external_gc_element_["createElement"])(external_gc_components_["Icon"], {
      icon: icon
    })), Object(external_gc_element_["createElement"])(external_gc_components_["FlexBlock"], {
      className: "edit-site-create-template-part-modal__option-label"
    }, label, Object(external_gc_element_["createElement"])("div", null, description)), Object(external_gc_element_["createElement"])(external_gc_components_["FlexItem"], {
      className: "edit-site-create-template-part-modal__checkbox"
    }, area === value && Object(external_gc_element_["createElement"])(external_gc_components_["Icon"], {
...
function ConvertToTemplatePart(_ref) {
  ...
  const onConvert = async _ref2 => {
    ...
    replaceBlocks(clientIds, Object(external_gc_blocks_["createBlock"])('core/template-part', {
      slug: templatePart.slug,
      theme: templatePart.theme
    }));
    createSuccessNotice(Object(external_gc_i18n_["__"])('模板组件已创建。'), {
      type: 'snackbar'
    }); // The modal and this component will be unmounted because of `replaceBlocks` above,
    // so no need to call `closeModal` or `onClose`.
  };

  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockSettingsMenuControls"], null, () => Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
    onClick: () => {
      setIsModalOpen(true);
    }
  }, Object(external_gc_i18n_["__"])('制作模版组件'))), isModalOpen && Object(external_gc_element_["createElement"])(CreateTemplatePartModal, {
    closeModal: () => {
...
function TemplatePartConverter() {
  ...
  if (blocks.length === 1 && ((_blocks$ = blocks[0]) === null || _blocks$ === void 0 ? void 0 : _blocks$.name) === 'core/template-part') {
    return Object(external_gc_element_["createElement"])(ConvertToRegularBlocks, {
      clientId: clientIds[0]
    });
  }

  return Object(external_gc_element_["createElement"])(ConvertToTemplatePart, {
    clientIds: clientIds,
    blocks: blocks
  });
}
...
var edit = __webpack_require__("7JoZ");

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/navigate-to-link/index.js
...
function NavigateToLink(_ref) {
  ...
  const post = Object(external_gc_data_["useSelect"])(select => type && id && type !== 'URL' && select(external_gc_coreData_["store"]).getEntityRecord('postType', type, id), [type, id]);
  const onClick = Object(external_gc_element_["useMemo"])(() => {
    if (!(post !== null && post !== void 0 && post.link)) return null;
    const path = Object(external_gc_url_["getPathAndQueryString"])(post.link);
  ...
  return onClick && Object(external_gc_element_["createElement"])(external_gc_components_["Button"], {
    icon: edit["a" /* default */],
    label: Object(external_gc_i18n_["__"])('编辑页面模板'),
    onClick: onClick
  });
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/block-editor/block-inspector-button.js
...
function BlockInspectorButton(_ref) {
  ...
  const {
    ...
    shortcut: select(external_gc_keyboardShortcuts_["store"]).getShortcutRepresentation('core/edit-site/toggle-block-settings-sidebar'),
  ...
  const label = isBlockInspectorOpen ? Object(external_gc_i18n_["__"])('隐藏更多设置') : Object(external_gc_i18n_["__"])('显示更多设置');
  return Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
    onClick: () => {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/edit-template-part-menu-button/index.js
...
function EditTemplatePartMenuButton() {
  return Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockSettingsMenuControls"], null, _ref => {
    let {
      selectedClientIds,
      onClose
    } = _ref;
    return Object(external_gc_element_["createElement"])(EditTemplatePartMenuItem, {
...
function EditTemplatePartMenuItem(_ref2) {
  ...
  const selectedTemplatePart = Object(external_gc_data_["useSelect"])(select => {
    const block = select(external_gc_blockEditor_["store"]).getBlock(selectedClientId);

    if (block && Object(external_gc_blocks_["isTemplatePart"])(block)) {
      const {
        theme,
        slug
      } = block.attributes;
      return select(external_gc_coreData_["store"]).getEntityRecord('postType', 'gc_template_part', // Ideally this should be an official public API.
      `${theme}//${slug}`);
    }
  }, [selectedClientId]);
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], Object(esm_extends["a" /* default */])({}, linkProps, {
    onClick: event => {
  ...
  /* translators: %s: template part title */
  Object(external_gc_i18n_["sprintf"])(Object(external_gc_i18n_["__"])('编辑 %s'), selectedTemplatePart.slug));
}
...
function BackButton() {
  ...
  }, Object(external_gc_i18n_["__"])('返回'));
}
...
function ResizeHandle(_ref) {
  ...
  function handleKeyDown(event) {
    ...
    if (direction === 'left' && keyCode === external_gc_keycodes_["LEFT"] || direction === 'right' && keyCode === external_gc_keycodes_["RIGHT"]) {
      resizeWidthBy(DELTA_DISTANCE);
    } else if (direction === 'left' && keyCode === external_gc_keycodes_["RIGHT"] || direction === 'right' && keyCode === external_gc_keycodes_["LEFT"]) {
      resizeWidthBy(-DELTA_DISTANCE);
    }
  }

  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])("button", {
    className: `resizable-editor__drag-handle is-${direction}`,
    "aria-label": Object(external_gc_i18n_["__"])('拖动调整大小'),
    "aria-describedby": `resizable-editor__resize-help-${direction}`,
    onKeyDown: handleKeyDown
  }), Object(external_gc_element_["createElement"])(external_gc_components_["VisuallyHidden"], {
    id: `resizable-editor__resize-help-${direction}`
  }, Object(external_gc_i18n_["__"])('使用左右方向键调整画布大小。')));
}
...
function ResizableEditor(_ref) {
  ...
  const deviceType = Object(external_gc_data_["useSelect"])(select => select(store).__experimentalGetPreviewDeviceType(), []);
  const deviceStyles = Object(external_gc_blockEditor_["__experimentalUseResizeCanvas"])(deviceType);
  const [width, setWidth] = Object(external_gc_element_["useState"])(DEFAULT_STYLES.width);
  const [height, setHeight] = Object(external_gc_element_["useState"])(DEFAULT_STYLES.height);
  const iframeRef = Object(external_gc_element_["useRef"])();
  const mouseMoveTypingResetRef = Object(external_gc_blockEditor_["__unstableUseMouseMoveTypingReset"])();
  const ref = Object(external_gc_compose_["useMergeRefs"])([iframeRef, mouseMoveTypingResetRef]);
  Object(external_gc_element_["useEffect"])(function autoResizeIframeHeight() {
  ...
  const resizeWidthBy = Object(external_gc_element_["useCallback"])(deltaPixels => {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["ResizableBox"], {
    ...
    resizeRatio: 2,
    handleComponent: {
      left: Object(external_gc_element_["createElement"])(ResizeHandle, {
        direction: "left",
        resizeWidthBy: resizeWidthBy
      }),
      right: Object(external_gc_element_["createElement"])(ResizeHandle, {
        direction: "right",
        resizeWidthBy: resizeWidthBy
      })
    },
  ...
  }, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__unstableIframe"], Object(esm_extends["a" /* default */])({
    style: enableResizing ? undefined : deviceStyles,
    head: Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__unstableEditorStyles"], {
      styles: settings.styles
    }), Object(external_gc_element_["createElement"])("style", null, // Forming a "block formatting context" to prevent margin collapsing.
    // @see https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Block_formatting_context
    `.is-root-container { display: flow-root; }`), enableResizing && Object(external_gc_element_["createElement"])("style", null, // Force the <html> and <body>'s heights to fit the content.
...
function BlockEditor(_ref) {
  ...
  const [blocks, onInput, onChange] = Object(external_gc_coreData_["useEntityBlockEditor"])('postType', templateType);
  const {
    setPage
  } = Object(external_gc_data_["useDispatch"])(store);
  const contentRef = Object(external_gc_element_["useRef"])();
  const mergedRefs = Object(external_gc_compose_["useMergeRefs"])([contentRef, Object(external_gc_blockEditor_["__unstableUseTypingObserver"])()]);
  const isMobileViewport = Object(external_gc_compose_["useViewportMatch"])('small', '<');
  const {
    clearSelectedBlock
  } = Object(external_gc_data_["useDispatch"])(external_gc_blockEditor_["store"]);
  const isTemplatePart = templateType === 'gc_template_part';
  return Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockEditorProvider"], {
    settings: settings,
    value: blocks,
    onInput: onInput,
    onChange: onChange,
    useSubRegistry: false
  }, Object(external_gc_element_["createElement"])(EditTemplatePartMenuButton, null), Object(external_gc_element_["createElement"])(TemplatePartConverter, null), Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__experimentalLinkControl"].ViewerFill, null, Object(external_gc_element_["useCallback"])(fillProps => Object(external_gc_element_["createElement"])(NavigateToLink, Object(esm_extends["a" /* default */])({}, fillProps, {
    activePage: page,
    onActivePageChange: setPage
  })), [page])), Object(external_gc_element_["createElement"])(SidebarInspectorFill, null, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockInspector"], null)), Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockTools"], {
  ...
  }, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockEditorKeyboardShortcuts"].Register, null), Object(external_gc_element_["createElement"])(back_button, null), Object(external_gc_element_["createElement"])(resizable_editor // Reinitialize the editor and reset the states when the template changes.
  , {
    key: templateId,
  ...
  }, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockList"], {
    className: "edit-site-block-editor__block-list gc-site-blocks",
    __experimentalLayout: LAYOUT,
    renderAppender: isTemplatePart ? false : undefined
  })), Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__unstableBlockSettingsMenuFirstItem"], null, _ref2 => {
    let {
      onClose
    } = _ref2;
    return Object(external_gc_element_["createElement"])(BlockInspectorButton, {
      onClick: onClose
    });
  })), Object(external_gc_element_["createElement"])(external_gc_reusableBlocks_["ReusableBlocksMenuItems"], null));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/keyboard-shortcuts/index.js
/**
 * GeChiUI dependencies
 */
...
function KeyboardShortcuts(_ref) {
  ...
  const {
    __experimentalGetDirtyEntityRecords,
    isSavingEntityRecord
  } = Object(external_gc_data_["useSelect"])(external_gc_coreData_["store"]);
  const isListViewOpen = Object(external_gc_data_["useSelect"])(select => select(store).isListViewOpened(), []);
  const isBlockInspectorOpen = Object(external_gc_data_["useSelect"])(select => select(build_module["i" /* store */]).getActiveComplementaryArea(store.name) === SIDEBAR_BLOCK, []);
  const {
    redo,
    undo
  } = Object(external_gc_data_["useDispatch"])(external_gc_coreData_["store"]);
  ...
  Object(external_gc_keyboardShortcuts_["useShortcut"])('core/edit-site/save', event => {
    event.preventDefault();
  ...
  Object(external_gc_keyboardShortcuts_["useShortcut"])('core/edit-site/undo', event => {
    undo();
    event.preventDefault();
  });
  Object(external_gc_keyboardShortcuts_["useShortcut"])('core/edit-site/redo', event => {
    redo();
    event.preventDefault();
  });
  Object(external_gc_keyboardShortcuts_["useShortcut"])('core/edit-site/toggle-list-view', () => {
    setIsListViewOpened(!isListViewOpen);
  });
  Object(external_gc_keyboardShortcuts_["useShortcut"])('core/edit-site/toggle-block-settings-sidebar', event => {
...
function KeyboardShortcutsRegister() {
  ...
  Object(external_gc_element_["useEffect"])(() => {
    ...
    registerShortcut({
      ...
      category: 'global',
      description: Object(external_gc_i18n_["__"])('显示或隐藏区块设置侧边栏。'),
      keyCombination: {
        modifier: 'primaryShift',
        character: ','
      }
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/url-query-controller/index.js
/**
 * GeChiUI dependencies
 */
...
function InserterSidebar() {
  ...
  const insertionPoint = Object(external_gc_data_["useSelect"])(select => select(store).__experimentalGetInsertionPoint(), []);
  const isMobile = Object(external_gc_compose_["useViewportMatch"])('medium', '<');
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/welcome-guide/image.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/welcome-guide/editor.js
...
function WelcomeGuideEditor() {
  ...
  const isActive = Object(external_gc_data_["useSelect"])(select => select(store).isFeatureActive('welcomeGuide'), []);
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["Guide"], {
    ...
    contentLabel: Object(external_gc_i18n_["__"])('欢迎来到站点编辑器'),
    finishButtonText: Object(external_gc_i18n_["__"])('开始'),
    ...
    pages: [{
      ...
      }, Object(external_gc_i18n_["__"])('编辑您的站点')), Object(external_gc_element_["createElement"])("p", {
        className: "edit-site-welcome-guide__text"
      }, Object(external_gc_i18n_["__"])('使用区块设计您站点上从页眉到页脚的所有内容—。')), Object(external_gc_element_["createElement"])("p", {
        className: "edit-site-welcome-guide__text"
      }, Object(external_gc_element_["createInterpolateElement"])(Object(external_gc_i18n_["__"])('单击<StylesIconImage />开始设计您的区块，并选择您的排版、布局和颜色。'), {
        StylesIconImage: Object(external_gc_element_["createElement"])("img", {
          alt: Object(external_gc_i18n_["__"])('样式'),
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/welcome-guide/styles.js
...
function WelcomeGuideStyles() {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["Guide"], {
    className: "edit-site-welcome-guide",
    contentLabel: Object(external_gc_i18n_["__"])('欢迎来到样式'),
    finishButtonText: Object(external_gc_i18n_["__"])('开始'),
    ...
    pages: [{
      ...
      content: Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])("h1", {
        className: "edit-site-welcome-guide__heading"
      }, Object(external_gc_i18n_["__"])('欢迎来到样式')), Object(external_gc_element_["createElement"])("p", {
        className: "edit-site-welcome-guide__text"
      }, Object(external_gc_i18n_["__"])('调整您的站点，或赋予其全新的外观！发挥创意——为您的按钮设计新的调色盘，或选择一种新字体如何？看看您可以在这里做什么。')))
    }, {
      image: Object(external_gc_element_["createElement"])(WelcomeGuideImage, {
      ...
      content: Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])("h1", {
        className: "edit-site-welcome-guide__heading"
      }, Object(external_gc_i18n_["__"])('设定设计')), Object(external_gc_element_["createElement"])("p", {
        className: "edit-site-welcome-guide__text"
      }, Object(external_gc_i18n_["__"])('您可以随心所欲地用不同的颜色、排版和布局来定制您的站点。或者，如果您愿意，就将其留给您的主题来处理吧!')))
    }, {
      image: Object(external_gc_element_["createElement"])(WelcomeGuideImage, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/welcome-guide/index.js
...
function WelcomeGuide() {
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(WelcomeGuideEditor, null), Object(external_gc_element_["createElement"])(WelcomeGuideStyles, null));
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/editor/global-styles-renderer.js
/**
 * GeChiUI dependencies
 */
...
function useGlobalStylesUserConfig() {
  const {
    ...
    const _globalStylesId = select(external_gc_coreData_["store"]).__experimentalGetCurrentGlobalStylesId();

    const record = _globalStylesId ? select(external_gc_coreData_["store"]).getEditedEntityRecord('root', 'globalStyles', _globalStylesId) : undefined;
    return {
  ...
  const config = Object(external_gc_element_["useMemo"])(() => {
    return {
      settings: settings !== null && settings !== void 0 ? settings : {},
      styles: styles !== null && styles !== void 0 ? styles : {}
    };
  }, [settings, styles]);
  const setConfig = Object(external_gc_element_["useCallback"])(callback => {
...
function useGlobalStylesBaseConfig() {
  const baseConfig = Object(external_gc_data_["useSelect"])(select => {
    return select(external_gc_coreData_["store"]).__experimentalGetCurrentThemeBaseGlobalStyles();
  }, []);
...
function useGlobalStylesContext() {
  ...
  const mergedConfig = Object(external_gc_element_["useMemo"])(() => {
  ...
  const context = Object(external_gc_element_["useMemo"])(() => {
    return {
...
function GlobalStylesProvider(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(GlobalStylesContext.Provider, {
    value: context
  }, children);
}
...
function useTitle(title) {
  const location = useLocation();
  const siteTitle = Object(external_gc_data_["useSelect"])(select => {
    var _select$getEntityReco;

    return (_select$getEntityReco = select(external_gc_coreData_["store"]).getEntityRecord('root', 'site')) === null || _select$getEntityReco === void 0 ? void 0 : _select$getEntityReco.title;
  }, []);
  const isInitialLocationRef = Object(external_gc_element_["useRef"])(true);
  Object(external_gc_element_["useEffect"])(() => {
  ...
  Object(external_gc_element_["useEffect"])(() => {
    ...
    if (title && siteTitle) {
      ...
      const formattedTitle = Object(external_gc_i18n_["sprintf"])(
      /* translators: Admin screen title. 1: Admin screen name, 2: Network or site name. */
      Object(external_gc_i18n_["__"])('%1$s ‹ %2$s — GeChiUI'), title, siteTitle);
      ...
      Object(external_gc_i18n_["__"])('当前显示：%s'), document.title), 'assertive');
    }
...
const interfaceLabels = {
  secondarySidebar: Object(external_gc_i18n_["__"])('区块库'),
  drawer: Object(external_gc_i18n_["__"])('导航侧边栏')
};
...
function Editor(_ref) {
  ...
  const {
    ...
    return {
      ...
      previousShortcut: select(external_gc_keyboardShortcuts_["store"]).getAllShortcutKeyCombinations('core/edit-site/previous-region'),
      nextShortcut: select(external_gc_keyboardShortcuts_["store"]).getAllShortcutKeyCombinations('core/edit-site/next-region')
    };
  }, []);
  const {
    setPage,
  ...
  const [isEntitiesSavedStatesOpen, setIsEntitiesSavedStatesOpen] = Object(external_gc_element_["useState"])(false);
  const openEntitiesSavedStates = Object(external_gc_element_["useCallback"])(() => setIsEntitiesSavedStatesOpen(true), []);
  const closeEntitiesSavedStates = Object(external_gc_element_["useCallback"])(() => {
    setIsEntitiesSavedStatesOpen(false);
  }, []);
  const blockContext = Object(external_gc_element_["useMemo"])(() => ({ ...(page === null || page === void 0 ? void 0 : page.context),
  ...
  Object(external_gc_element_["useEffect"])(function openGlobalStylesOnLoad() {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(URLQueryController, null), isReady && Object(external_gc_element_["createElement"])(external_gc_keyboardShortcuts_["ShortcutProvider"], null, Object(external_gc_element_["createElement"])(external_gc_coreData_["EntityProvider"], {
    ...
    type: templateType,
    id: entityId
  }, Object(external_gc_element_["createElement"])(GlobalStylesProvider, null, Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockContextProvider"], {
    value: blockContext
  }, Object(external_gc_element_["createElement"])(GlobalStylesRenderer, null), Object(external_gc_element_["createElement"])(error_boundary_ErrorBoundary, {
    onError: onError
  }, Object(external_gc_element_["createElement"])(keyboard_shortcuts.Register, null), Object(external_gc_element_["createElement"])(SidebarComplementaryAreaFills, null), Object(external_gc_element_["createElement"])(build_module["e" /* InterfaceSkeleton */], {
    labels: interfaceLabels,
    secondarySidebar: secondarySidebar(),
    sidebar: sidebarIsOpened && Object(external_gc_element_["createElement"])(build_module["b" /* ComplementaryArea */].Slot, {
      scope: "core/edit-site"
    }),
    drawer: Object(external_gc_element_["createElement"])(navigation_sidebar.Slot, null),
    ...
    content: Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(external_gc_editor_["EditorNotices"], null), template && Object(external_gc_element_["createElement"])(BlockEditor, {
      setIsInserterOpen: setIsInserterOpened
    }), templateResolved && !template && (settings === null || settings === void 0 ? void 0 : settings.siteUrl) && entityId && Object(external_gc_element_["createElement"])(external_gc_components_["Notice"], {
      status: "warning",
      isDismissible: false
    }, Object(external_gc_i18n_["__"])("您试图编辑一个不存在的项目。也许该项目已被删除？")), Object(external_gc_element_["createElement"])(keyboard_shortcuts, {
      openEntitiesSavedStates: openEntitiesSavedStates
    })),
    actions: Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, isEntitiesSavedStatesOpen ? Object(external_gc_element_["createElement"])(external_gc_editor_["EntitiesSavedStates"], {
    ...
    footer: Object(external_gc_element_["createElement"])(external_gc_blockEditor_["BlockBreadcrumb"], null),
    shortcuts: {
      previous: previousShortcut,
      next: nextShortcut
    }
  }), Object(external_gc_element_["createElement"])(WelcomeGuide, null), Object(external_gc_element_["createElement"])(external_gc_components_["Popover"].Slot, null), Object(external_gc_element_["createElement"])(external_gc_plugins_["PluginArea"], null))))))));
}

/* harmony default export */ var editor = (Editor);

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/use-register-shortcuts.js
/**
 * GeChiUI dependencies
 */
...
function NewTemplate(_ref) {
  ...
  const {
    ...
    templates: select(external_gc_coreData_["store"]).getEntityRecords('postType', 'gc_template', {
      per_page: -1
    }),
    defaultTemplateTypes: select(external_gc_editor_["store"]).__experimentalGetDefaultTemplateTypes()
  }), []);
  const {
    saveEntityRecord
  } = Object(external_gc_data_["useDispatch"])(external_gc_coreData_["store"]);
  ...
  async function createTemplate(_ref2) {
    ...
    try {
      ...
      const template = await saveEntityRecord('postType', 'gc_template', {
        excerpt: description,
      ...
      const lastEntitySaveError = getLastEntitySaveError('postType', 'gc_template', template.id);
      ...
      const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('创建模板出错。');
      createErrorNotice(errorMessage, {
        type: 'snackbar'
      });
    }
  ...
  }, () => Object(external_gc_element_["createElement"])(external_gc_components_["NavigableMenu"], {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/add-new-template/new-template-part.js
...
function NewTemplatePart(_ref) {
  ...
  async function createTemplatePart(_ref2) {
    ...
    if (!title) {
      createErrorNotice(Object(external_gc_i18n_["__"])('标题未定义。'), {
        type: 'snackbar'
      });
      return;
    }
    ...
    try {
      ...
      const lastEntitySaveError = getLastEntitySaveError('postType', 'gc_template_part', templatePart.id);
      ...
      const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('创建模板组件时出错。');
      createErrorNotice(errorMessage, {
        type: 'snackbar'
      });
      setIsModalOpen(false);
  ...
  }, postType.labels.add_new), isModalOpen && Object(external_gc_element_["createElement"])(CreateTemplatePartModal, {
...
function AddNewTemplate(_ref) {
  ...
  const postType = Object(external_gc_data_["useSelect"])(select => select(external_gc_coreData_["store"]).getPostType(templateType), [templateType]);
  ...
  if (templateType === 'gc_template') {
    return Object(external_gc_element_["createElement"])(NewTemplate, {
      postType: postType
    });
  } else if (templateType === 'gc_template_part') {
    return Object(external_gc_element_["createElement"])(NewTemplatePart, {
      postType: postType
    });
  }
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/header.js
...
function header_Header(_ref) {
  ...
  return Object(external_gc_element_["createElement"])("header", {
    className: "edit-site-list-header"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHeading"], {
    level: 1,
    className: "edit-site-list-header__title"
  }, (_postType$labels = postType.labels) === null || _postType$labels === void 0 ? void 0 : _postType$labels.name), Object(external_gc_element_["createElement"])("div", {
    className: "edit-site-list-header__right"
  }, Object(external_gc_element_["createElement"])(AddNewTemplate, {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/actions/rename-menu-item.js
...
function RenameMenuItem(_ref) {
  ...
  const [title, setTitle] = Object(external_gc_element_["useState"])(() => template.title.rendered);
  const [isModalOpen, setIsModalOpen] = Object(external_gc_element_["useState"])(false);
  const {
    getLastEntitySaveError
  } = Object(external_gc_data_["useSelect"])(external_gc_coreData_["store"]);
  ...
  async function onTemplateRename(event) {
    ...
    try {
      ...
      createSuccessNotice(Object(external_gc_i18n_["__"])('模板已重命名。'), {
        type: 'snackbar'
      });
    } catch (error) {
      const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('重命名模板时出错。');
      createErrorNotice(errorMessage, {
        type: 'snackbar'
      });
    }
  ...
  }, Object(external_gc_i18n_["__"])('重命名')), isModalOpen && Object(external_gc_element_["createElement"])(external_gc_components_["Modal"], {
    title: Object(external_gc_i18n_["__"])('重命名模板'),
    closeLabel: Object(external_gc_i18n_["__"])('关闭'),
    onRequestClose: () => {
      setIsModalOpen(false);
    },
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/actions/index.js
...
function Actions(_ref) {
  ...
  async function revertAndSaveTemplate() {
    try {
      ...
      await saveEditedEntityRecord('postType', template.type, template.id);
      createSuccessNotice(Object(external_gc_i18n_["__"])('模板已还原。'), {
        type: 'snackbar'
      });
    } catch (error) {
      const errorMessage = error.message && error.code !== 'unknown_error' ? error.message : Object(external_gc_i18n_["__"])('还原模板时出错。');
      createErrorNotice(errorMessage, {
        type: 'snackbar'
      });
    }
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["DropdownMenu"], {
    icon: more_vertical["a" /* default */],
    label: Object(external_gc_i18n_["__"])('操作'),
    className: "edit-site-list-table__actions"
  }, _ref2 => {
    let {
      onClose
    } = _ref2;
    return Object(external_gc_element_["createElement"])(external_gc_components_["MenuGroup"], null, isRemovable && Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(RenameMenuItem, {
      template: template,
      onClose: onClose
    }), Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
      isDestructive: true,
    ...
    }, Object(external_gc_i18n_["__"])('删除模板 '))), isRevertable && Object(external_gc_element_["createElement"])(external_gc_components_["MenuItem"], {
      info: Object(external_gc_i18n_["__"])('将模板还原至默认状态'),
      onClick: () => {
...
const commentAuthorAvatar = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
}, Object(external_gc_element_["createElement"])(external_gc_primitives_["Path"], {
...
function CustomizedTooltip(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["Tooltip"], {
    text: Object(external_gc_i18n_["__"])('此模板已自定义')
  }, children);
}
...
function AddedBy(_ref6) {
  ...
  if (TEMPLATE_POST_TYPE_NAMES.includes(templateType)) {
    ...
    if (template.has_theme_file && (template.origin === 'theme' || !template.origin && ['theme', 'custom'].includes(template.source))) {
      return Object(external_gc_element_["createElement"])(AddedByTheme, {
        slug: template.theme,
    ...
    if (template.has_theme_file && template.origin === 'plugin') {
      return Object(external_gc_element_["createElement"])(AddedByPlugin, {
        slug: template.theme,
        isCustomized: template.source === 'custom'
      });
    ...
    if (!template.has_theme_file && template.source === 'custom' && !template.author) {
      return Object(external_gc_element_["createElement"])(AddedBySite, null);
    }
  } // Simply show the author for templates created from scratch that have an
  // author or for any other post type.


  return Object(external_gc_element_["createElement"])(AddedByAuthor, {
    id: template.author
  });
}

// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/table.js
...
function Table(_ref) {
  ...
  if (!templates.length) {
    ...
    return Object(external_gc_element_["createElement"])("div", null, Object(external_gc_i18n_["sprintf"])( // translators: The template type name, should be either "templates" or "template parts".
    Object(external_gc_i18n_["__"])('未找到%s。'), postType === null || postType === void 0 ? void 0 : (_postType$labels = postType.labels) === null || _postType$labels === void 0 ? void 0 : (_postType$labels$name = _postType$labels.name) === null || _postType$labels$name === void 0 ? void 0 : _postType$labels$name.toLowerCase()));
  }

  return (// These explicit aria roles are needed for Safari.
    // See https://developer.mozilla.org/en-US/docs/Web/CSS/display#tables
    Object(external_gc_element_["createElement"])("table", {
      className: "edit-site-list-table",
      role: "table"
    }, Object(external_gc_element_["createElement"])("thead", null, Object(external_gc_element_["createElement"])("tr", {
      className: "edit-site-list-table-head",
      role: "row"
    }, Object(external_gc_element_["createElement"])("th", {
      className: "edit-site-list-table-column",
      role: "columnheader"
    }, Object(external_gc_i18n_["__"])('模板')), Object(external_gc_element_["createElement"])("th", {
      className: "edit-site-list-table-column",
      role: "columnheader"
    }, Object(external_gc_i18n_["__"])('添加者')), Object(external_gc_element_["createElement"])("th", {
      className: "edit-site-list-table-column",
      role: "columnheader"
    }, Object(external_gc_element_["createElement"])(external_gc_components_["VisuallyHidden"], null, Object(external_gc_i18n_["__"])('操作'))))), Object(external_gc_element_["createElement"])("tbody", null, templates.map(template => {
      var _template$title;
      ...
      return Object(external_gc_element_["createElement"])("tr", {
        ...
        role: "row"
      }, Object(external_gc_element_["createElement"])("td", {
        className: "edit-site-list-table-column",
        role: "cell"
      }, Object(external_gc_element_["createElement"])(external_gc_components_["__experimentalHeading"], {
        level: 4
      }, Object(external_gc_element_["createElement"])(Link, {
        params: {
          postId: template.id,
          postType: template.type
        }
      }, ((_template$title = template.title) === null || _template$title === void 0 ? void 0 : _template$title.rendered) || template.slug)), template.description), Object(external_gc_element_["createElement"])("td", {
        className: "edit-site-list-table-column",
        role: "cell"
      }, Object(external_gc_element_["createElement"])(AddedBy, {
        templateType: templateType,
        template: template
      })), Object(external_gc_element_["createElement"])("td", {
        className: "edit-site-list-table-column",
        role: "cell"
      }, Object(external_gc_element_["createElement"])(Actions, {
        template: template
      })));
    })))
  );
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/list/index.js
...
function List() {
  ...
  const detailedRegionLabels = postType ? {
    header: Object(external_gc_i18n_["sprintf"])( // translators: %s - the name of the page, 'Header' as in the header area of that page.
    Object(external_gc_i18n_["__"])('%s - 标题'), itemsListLabel),
    body: Object(external_gc_i18n_["sprintf"])( // translators: %s - the name of the page, 'Content' as in the content area of that page.
    Object(external_gc_i18n_["__"])('%s - 内容'), itemsListLabel)
  } : undefined;
  return Object(external_gc_element_["createElement"])(build_module["e" /* InterfaceSkeleton */], {
    ...
    header: Object(external_gc_element_["createElement"])(header_Header, {
      templateType: templateType
    }),
    drawer: Object(external_gc_element_["createElement"])(navigation_sidebar.Slot, null),
    notices: Object(external_gc_element_["createElement"])(external_gc_editor_["EditorSnackbars"], null),
    content: Object(external_gc_element_["createElement"])(Table, {
      templateType: templateType
    }),
    shortcuts: {
      previous: previousShortcut,
...
function EditSiteApp(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["SlotFillProvider"], null, Object(external_gc_element_["createElement"])(external_gc_editor_["UnsavedChangesWarning"], null), Object(external_gc_element_["createElement"])(Routes, null, _ref2 => {
    let {
      params
    } = _ref2;
    const isListPage = getIsListPage(params);
    return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, isListPage ? Object(external_gc_element_["createElement"])(List, null) : Object(external_gc_element_["createElement"])(editor, {
      onError: reboot
    }), Object(external_gc_element_["createElement"])(navigation_sidebar // Open the navigation sidebar by default when in the list page.
    , {
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/plugin-sidebar-more-menu-item/index.js
...
// CONCATENATED MODULE: ./node_modules/@gechiui/edit-site/build-module/components/header/plugin-more-menu-item/index.js
/**
 * GeChiUI dependencies
 */
...
async function reinitializeEditor(target, settings) {
  ...
  {
    Object(external_gc_data_["dispatch"])(store).updateSettings(settings); // Keep the defaultTemplateTypes in the core/editor settings too,
    // so that they can be selected with core/editor selectors in any editor.
    // This is needed because edit-site doesn't initialize with EditorProvider,
    // which internally uses updateEditorSettings as well.

    Object(external_gc_data_["dispatch"])(external_gc_editor_["store"]).updateEditorSettings({
      defaultTemplateTypes: settings.defaultTemplateTypes,
      defaultTemplatePartAreas: settings.defaultTemplatePartAreas
    });
    const isLandingOnListPage = getIsListPage(Object(external_gc_url_["getQueryArgs"])(window.location.href));

    if (isLandingOnListPage) {
      // Default the navigation panel to be opened when we're in a bigger
      // screen and land in the list screen.
      Object(external_gc_data_["dispatch"])(store).setIsNavigationPanelOpened(Object(external_gc_data_["select"])(external_gc_viewport_["store"]).isViewportMatch('medium'));
    }
  }
  Object(external_gc_element_["render"])(Object(external_gc_element_["createElement"])(EditSiteApp, {
    reboot: reboot
  }), target);
}
/**
...
function initializeEditor(id, settings) {
  settings.__experimentalFetchLinkSuggestions = (search, searchOptions) => Object(external_gc_coreData_["__experimentalFetchLinkSuggestions"])(search, searchOptions, settings);

  settings.__experimentalFetchRichUrlData = external_gc_coreData_["__experimentalFetchUrlData"];
