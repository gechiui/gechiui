...
/******/ 	return __webpack_require__(__webpack_require__.s = "UyNY");
/******/ })
/************************************************************************/
/******/ ({
...
function createPreloadingMiddleware(preloadedData) {
  const cache = Object.keys(preloadedData).reduce((result, path) => {
    result[Object(external_gc_url_["normalizePath"])(path)] = preloadedData[path];
    return result;
  },
  ...
  return (options, next) => {
    ...
    if (!rawPath && options.url) {
      const pathFromQuery = Object(external_gc_url_["getQueryArg"])(options.url, 'rest_route');
    ...
    if (typeof rawPath === 'string') {
      ...
      const path = Object(external_gc_url_["normalizePath"])(rawPath);
...
const modifyQuery = (_ref, queryArgs) => {
  ...
  return { ...options,
    url: url && Object(external_gc_url_["addQueryArgs"])(url, queryArgs),
    path: path && Object(external_gc_url_["addQueryArgs"])(path, queryArgs)
  };
};
/**
...
const userLocaleMiddleware = (options, next) => {
  if (typeof options.url === 'string' && !Object(external_gc_url_["hasQueryArg"])(options.url, '_locale')) {
    options.url = Object(external_gc_url_["addQueryArgs"])(options.url, {
      _locale: 'user'
    });
  }

  if (typeof options.path === 'string' && !Object(external_gc_url_["hasQueryArg"])(options.path, '_locale')) {
    options.path = Object(external_gc_url_["addQueryArgs"])(options.path, {
      _locale: 'user'
    });
  }
...
const parseJsonAndNormalizeError = response => {
  const invalidJsonError = {
    code: 'invalid_json',
    message: Object(external_gc_i18n_["__"])('此响应不是合法的JSON响应。')
  };
...
function parseAndThrowError(response) {
  ...
  return parseJsonAndNormalizeError(response).then(error => {
    const unknownError = {
      code: 'unknown_error',
      message: Object(external_gc_i18n_["__"])('发生了未知错误。')
    };
    ...
    if (response.status >= 500 && response.status < 600 && attachmentId) {
      return postProcess(attachmentId).catch(() => {
        if (options.parse !== false) {
          return Promise.reject({
            code: 'post_process',
            message: Object(external_gc_i18n_["__"])('媒体上传失败。如果这是一张较大的照片或图片，请将其缩小并重新上传。')
          });
        }
...
const defaultFetchHandler = nextOptions => {
  ...
  return responsePromise.then(value => Promise.resolve(value).then(checkStatus).catch(response => parseAndThrowError(response, parse)).then(response => parseResponseAndNormalizeError(response, parse)), err => {
    ...
    throw {
      code: 'fetch_error',
      message: Object(external_gc_i18n_["__"])('您可能已经掉线。')
    };
  });
};
/** @type {FetchHandler} */
