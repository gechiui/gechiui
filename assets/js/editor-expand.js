...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#gc-content-wrap' ),
			$tools = $( '#gc-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.gc.editor && window.gc.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.gc.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.gc && editor.gc.getView && ( view = editor.gc.getView( node ) ) ) {
			 ...
			 * the window when the cursor moves out of the viewport to a gcview.
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'gcAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'gc-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent gc-autoresize gc-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'gc-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent gc-autoresize gc-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'gc-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .gc-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'gc-editor-expand' );
			 ...
			 */
			$document.on( 'gc-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'gc-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'gcAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.gc_autoresize_on = true;
				mceEditor.execCommand( 'gcAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'gc-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.gc_autoresize_on = false;
				mceEditor.execCommand( 'gcAutoResizeOff' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'gcToggleDFW', toggle );
			editor.addShortcut( 'access+w', '', 'gcToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'gc-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'gc-autoresize', recalcEditorRect );
				};
