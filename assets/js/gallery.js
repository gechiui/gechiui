...
jQuery(window).on( 'unload', function () { window.tinymce = window.tinyMCE = window.gcgallery = null; } ); // Cleanup.
...
window.gcgallery = {
	...
	setup : function() {
		...
		if ( t.el.nodeName !== 'IMG' || ! ed.dom.hasClass(t.el, 'gcGallery') ) {
			if ( ( g = ed.dom.select('img.gcGallery') ) && g[0] ) {
				t.el = g[0];
			} else {
