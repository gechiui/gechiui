...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo gc_customize_url(); ?>">Open Customizer</a>
	 *
	 * @memberOf gc.customize
	 *
	 * @class
	 * @augments gc.customize.Events
	 */
	Loader = $.extend( {}, api.Events,/** @lends gc.customize.Loader.prototype */{
		/**
		...
		initialize: function() {
			...
			// the Customizer.
			$('#gcbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'gc_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'gc_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _gcCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.gc.customize.Loader.
	api.Loader = Loader;
})( gc, jQuery );
