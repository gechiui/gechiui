...
// Setup app structure.
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: gc.template });
...
themes.view.Appearance = gc.Backbone.View.extend({

	el: '#gcbody-content .wrap .theme-browser',
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="gc-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el )
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.gechiui.com/themes.
	apiCall: function( request, paginated ) {
		return gc.ajax.send( 'query-themes', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 100
...
themes.view.Theme = gc.Backbone.View.extend({
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'gc-theme-update-success', function( event, response ) {
...
themes.view.Details = gc.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.gc-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'gc-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'gc-auto-update-setting-changed', callback );
	},
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( gc.themes.data.settings.confirmDelete ) ) {
			return;
		}

		gc.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'gc-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'gc-full-overlay expanded',
	...
	tooglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.gc-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'gc-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = gc.Backbone.View.extend({

	className: 'themes gc-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( gc.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			gc.a11y.speak( l10n.noThemesFound );
		} else {
			gc.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = gc.Backbone.View.extend({

	tagName: 'input',
	className: 'gc-filter-search',
	id: 'gc-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.gc-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.gc-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	events: {
		...
		'keyup #gcorg-username-input': 'saveUsername'
	},
	...
	applyFilters: function( event ) {
		...
		if ( ! tags ) {
			gc.a11y.speak( l10n.selectFeatureFilter );
			return;
		}
	...
	// Save the user's www.GeChiUI.com username and get his favorite themes.
	saveUsername: function ( event ) {
		var username = $( '#gcorg-username-input' ).val(),
			nonce = $( '#gcorg-username-nonce' ).val(),
		...
		return gc.ajax.send( 'save-gcorg-username', {
			data: {
				_gcnonce: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `gc.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.gc-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
...
$( function() {
	...
	$( document.body ).on( 'click', '.load-customize', function() {
		...
		urlParser.search = $.param( _.extend(
			gc.customize.utils.parseQueryString( urlParser.search.substr( 1 ) ),
			{
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _gcThemeSettings.settings.confirmDelete );
	});
});
...
jQuery( function($) {
	window.tb_position = function() {
		...
		if ( $('#gcadminbar').length ) {
			adminbar_height = parseInt( $('#gcadminbar').css('height'), 10 );
		}
