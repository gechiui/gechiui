...
( function( window, gc, shortcode, $ ) {
	...
	gc.mce.views = {
		 ...
		 * @param {Object} extend An object to extend gc.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = gc.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content, editor ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-gcview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-gcview-marker=/g, '<p data-gcview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {gc.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {gc.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-gcview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-gcview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	gc.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	gc.mce.View.extend = Backbone.View.extend;

	_.extend( gc.mce.View.prototype, /** @lends gc.mce.View.prototype */{
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-gcview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-gcview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== tinymce.DOM.decode( this.text ) ) {
					editor.dom.setAttrib( node, 'data-gcview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="gcview gcview-wrap" data-gcview-text="' + this.encodedText + '" data-gcview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			if ( _.isObject( content ) && ( content.sandbox || content.head || content.body.indexOf( '<script' ) !== -1 ) ) {
				...
				this.getNodes( function( editor, node ) {
					...
					editor.undoManager.transact( function() {
						...
						editor.dom.add( node, 'span', { 'class': 'gcview-end' } );
					} );
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/gechiui/gc-content.css' ) === -1 ) {
				...
				editor.undoManager.transact( function() {
					...
					dom.add( node, 'span', { 'class': 'gcview-end' } );
				} );
						...
						'<head>' +
							...
							'<style>' +
								'html {' +
									...
									'margin: 0;' +
								'}' +
								'body#gcview-iframe-sandbox {' +
									'background: transparent;' +
									'padding: 1px 0 !important;' +
									'margin: -1px 0 0 !important;' +
								'}' +
								'body#gcview-iframe-sandbox:before,' +
								'body#gcview-iframe-sandbox:after {' +
									'display: none;' +
									'content: "";' +
								'}' +
								'iframe {' +
							...
							'</style>' +
						'</head>' +
						'<body id="gcview-iframe-sandbox" class="' + bodyClasses + '">' +
							body +
						'</body>' +
					'</html>'
				);
		...
		setLoader: function( dashicon ) {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="gcview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {string} dashicon A dashicon ID. Optional. {@link https://developer.gechiui.com/resource/dashicons/}
		 */
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-gcview-text', encodeURIComponent( text ) );
					gc.mce.views.createInstance( type, text, match.options, force ).render();
...
} )( window, window.gc, window.gc.shortcode, window.jQuery );

/*
 * The GeChiUI core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'gcview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.gcview-sandbox', content ).get( 0 );
...
} )( window, window.gc.mce.views, window.gc.media, window.jQuery );
