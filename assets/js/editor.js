...
( function( $, gc ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'gc-switch-editor' ) ) {
						id = target.attr( 'data-gc-editor-id' );
		...
		function switchEditor( id, mode ) {
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				if ( editor ) {
					keepSelection = editor.getParam( 'gc_keep_scroll_position' );
				} else {
					keepSelection = window.tinyMCEPreInit.mceInit[ id ] &&
									window.tinyMCEPreInit.mceInit[ id ].gc_keep_scroll_position;
				}
				...
				if ( editor ) {
					...
					if ( editor.getParam( 'gc_keep_scroll_position' ) ) {
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = gc.shortcode.regexp( allShortcodes.join( '|' ) ),
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#gc-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#gc-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viewport (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.gc_autoresize_on ) {
		...
		/**
		 * Replaces <p> tags with two line breaks. "Opposite" of gcautop().
		 *
		...
		function removep( html ) {
			...
			if ( preserve.length ) {
				html = html.replace( /<gc-preserve>/g, function() {
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<gc-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<gc-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		gc.editor.autop = gcautop;
		gc.editor.removep = pre_gcautop;

		exports = {
			go: switchEditor,
			gcautop: gcautop,
			pre_gcautop: pre_gcautop,
			_gc_Autop: autop,
			_gc_Nop: removep
		};
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with gc_enqueue_editor() (PHP).
	 *
	 ...
	 * }
	 */
	gc.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! gc.editor.getDefaultSettings ) {
			return;
		}

		defaults = gc.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'gc-core-ui gc-editor-wrap tmce-active',
					id: 'gc-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="gc-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-gc-editor-id': id
				} );
			...
			if ( settings.mediaButtons ) {
				...
				$addMediaButton.append( '<span class="gc-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="gc-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="gc-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'gc-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'gc-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'gc-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'gc-before-quicktags-init', init );
			window.quicktags( init );

			if ( ! window.gcActiveEditor ) {
				window.gcActiveEditor = init.id;
			}
		}
	};
	 ...
	 */
	gc.editor.getContent = function( id ) {
		var editor;
