...
( function( document, window, navigator ) {
	document.addEventListener( 'DOMContentLoaded', function() {
		var adminBar = document.getElementById( 'gcadminbar' ),
			topMenuItems,
			allMenuItems,
			adminBarLogout,
			adminBarSearchForm,
		...
		adminBarLogout = document.getElementById( 'gc-admin-bar-logout' );
		adminBarSearchForm = document.getElementById( 'adminbarsearch' );
		shortlink = document.getElementById( 'gc-admin-bar-get-shortlink' );
	...
	function scrollToTop( event ) {
		// Only scroll when clicking on the gcadminbar, not on menus or submenus.
		if (
			event.target &&
			event.target.id !== 'gcadminbar' &&
			event.target.id !== 'gc-admin-bar-top-secondary'
		) {
			return;
		}
