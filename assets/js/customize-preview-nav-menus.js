...
/** @namespace gc.customize.navMenusPreview */
gc.customize.navMenusPreview = gc.customize.MenusCustomizerPreview = ( function( $, _, gc, api ) {
	'use strict';
	...
	if ( 'undefined' !== typeof _gcCustomizePreviewNavMenusExports ) {
		_.extend( self.data, _gcCustomizePreviewNavMenusExports );
	}
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				 ...
				 * being exported to the frontend by the _is_valid_nav_menu_item filter in gc_get_nav_menu_items(),
			...
			/*
			 * Ensure that gc_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of gc_nav_menu().
		 *
		 * @memberOf gc.customize.navMenusPreview
		 * @alias gc.customize.navMenusPreview.NavMenuInstancePartial
		 *
		 * @class
		...
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(/** @lends gc.customize.navMenusPreview.NavMenuInstancePartial.prototype */{
			 ...
			 *
			 * @inheritdoc
			 * @param {gc.customize.selectiveRefresh.Placement} placement
			 */
			renderContent: function( placement ) {
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-menu-refreshed', [ {
						...
						gcNavArgs: placement.context, // @deprecated
						gcNavMenuArgs: placement.context,
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no gc_nav_menu() instances that refer to the theme location, do full refresh.
