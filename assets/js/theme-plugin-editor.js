...
gc.themePluginEditor = (function( $ ) {
	'use strict';
	var component, TreeLinks,
		__ = gc.i18n.__, _n = gc.i18n._n, sprintf = gc.i18n.sprintf;
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = gc.template( 'gc-file-editor-notice' );
		...
		if ( false !== component.codeEditor ) {
			/*
			 * Defer adding notices until after DOM ready as workaround for GC Admin injecting
	...
	component.showWarning = function() {
		...
		// Hide all the #gcwrap content from assistive technologies.
		$( '#gcwrap' ).attr( 'aria-hidden', 'true' );
		...
		setTimeout( function() {
			gc.a11y.speak( gc.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.submit = function( event ) {
		...
		request = gc.ajax.post( 'edit-theme-plugin-file', data );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = gc.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
 ...
 * @type {object}
 */
gc.themePluginEditor.l10n = gc.themePluginEditor.l10n || {
	saveAlert: '',
	saveError: '',
	lintError: {
		alternative: 'gc.i18n',
...
gc.themePluginEditor.l10n = window.gc.deprecateL10nObject( 'gc.themePluginEditor.l10n', gc.themePluginEditor.l10n, '5.5.0' );
