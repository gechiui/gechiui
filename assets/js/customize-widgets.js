...
(function( gc, $ ){
	...
	/**
	 * @namespace gc.customize.Widgets
	 */
	api.Widgets = api.Widgets || {};
	api.Widgets.savedWidgetIds = {};

	// Link settings.
	api.Widgets.data = _gcCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;

	/**
	 * gc.customize.Widgets.WidgetModel
	 *
	 * A single widget model.
	 *
	 * @class    gc.customize.Widgets.WidgetModel
	 * @augments Backbone.Model
	 */
	api.Widgets.WidgetModel = Backbone.Model.extend(/** @lends gc.customize.Widgets.WidgetModel.prototype */{
		id: null,
		temp_id: null,
		classname: null,
		control_tpl: null,
	...
	/**
	 * gc.customize.Widgets.WidgetCollection
	 *
	 * Collection for widget models.
	 *
	 * @class    gc.customize.Widgets.WidgetCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.WidgetCollection = Backbone.Collection.extend(/** @lends gc.customize.Widgets.WidgetCollection.prototype */{
	...
	/**
	 * gc.customize.Widgets.SidebarModel
	 *
	 * A single sidebar model.
	 *
	 * @class    gc.customize.Widgets.SidebarModel
	 * @augments Backbone.Model
	 */
	api.Widgets.SidebarModel = Backbone.Model.extend(/** @lends gc.customize.Widgets.SidebarModel.prototype */{
		after_title: null,
		after_widget: null,
		before_title: null,
		before_widget: null,
	...
	/**
	 * gc.customize.Widgets.SidebarCollection
	 *
	 * Collection for sidebar models.
	 *
	 * @class    gc.customize.Widgets.SidebarCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.SidebarCollection = Backbone.Collection.extend(/** @lends gc.customize.Widgets.SidebarCollection.prototype */{
	...
	api.Widgets.AvailableWidgetsPanelView = gc.Backbone.View.extend(/** @lends gc.customize.Widgets.AvailableWidgetsPanelView.prototype */{
		 ...
		 *
		 * @constructs gc.customize.Widgets.AvailableWidgetsPanelView
	 ...
	 *
	 * @alias    gc.customize.Widgets.formSyncHandlers
	 */
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends gc.customize.Widgets.WidgetControl.prototype */{
		...
		/**
		 * gc.customize.Widgets.WidgetControl
		 *
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.gc-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						gc.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						gc.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // gc_widget_control() outputs the button as "Delete".
		...
		/**
		 * @return {gc.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( gc.ajax.settings.url, data );
	...
	/**
	 * gc.customize.Widgets.WidgetsPanel
	 *
	 ...
	 */
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends gc.customize.Widgets.WigetsPanel.prototype */{
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.gechiui.com/ticket/38794>.
	...
	/**
	 * gc.customize.Widgets.SidebarSection
	 *
	 ...
	 */
	api.Widgets.SidebarSection = api.Section.extend(/** @lends gc.customize.Widgets.SidebarSection.prototype */{
	...
	/**
	 * gc.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 ...
	 * @augments gc.customize.Control
	 */
	api.Widgets.SidebarControl = api.Control.extend(/** @lends gc.customize.Widgets.SidebarControl.prototype */{
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[gc_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[gc_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[gc_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				gc.a11y.speak( l10n.reorderModeOn );
				...
				gc.a11y.speak( l10n.reorderModeOff );
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo This can use widgetIdToSettingId(), then pass into gc.customize.control( x ).getSidebarWidgetsControl().
	 ...
	 *
	 * @param {gc.customize.Section|gc.customize.Panel|gc.customize.Control} focusConstruct - The object to initially focus.
	 * @param {gc.customize.Section|gc.customize.Panel|gc.customize.Control} returnConstruct - The object to return focus.
	 */
