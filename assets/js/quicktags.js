 ...
 * Quicktags
 *
 * This is the HTML editor in GeChiUI. It can be attached to any textarea and will
...
/* global adminpage, gcActiveEditor, quicktagsL10n, gcLink, prompt, edButtons */
...
(function(){
	...
	var qt = window.QTags = function(settings) {
		...
		wrap = document.getElementById( 'gc-' + id + '-wrap' );
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'gc_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(gcActiveEditor), event;
	...
	qt.Button.prototype.html = function(idPrefix) {
		var active, on, gc,
			...
			dfw = ( gc = window.gc ) && gc.editor && gc.editor.dfw;
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( gc = window.gc ) || ! gc.editor || ! gc.editor.dfw ) {
			return;
		}

		window.gc.editor.dfw.toggle();
	};
