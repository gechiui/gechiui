 ...
 *
 * @since 2.9.0
 * @output assets/js/image-edit.js
 */
	...
	refreshEditor : function(postid, nonce, callback) {
		...
		img = $( '<img id="image-preview-' + postid + '" alt="" />' )
			.on( 'load', { history: data.history }, function( event ) {
				...
				var successMessage = __( '图片已更新。' );

				t.toggleEditor(postid, 0);
				gc.a11y.speak( successMessage, 'assertive' );
			})
			.on( 'error', function() {
				var errorMessage = __( '无法加载预览图像。请重新加载页面，然后重试。' );
	...
	action : function(postid, nonce, action) {
		...
		$.post( ajaxurl, data, function( response ) {
			...
			if ( response && response.data.message.msg ) {
				gc.a11y.speak( response.data.message.msg );
				return;
			}

			if ( response && response.data.message.error ) {
				gc.a11y.speak( response.data.message.error );
			}
		} );
	},
	...
	save : function(postid, nonce) {
		...
		$.post( ajaxurl, data, function( response ) {
			...
			if ( response.data.error ) {
				...
				gc.a11y.speak( response.data.error );
				return;
			}
			...
			if ( response.data.msg ) {
				...
				gc.a11y.speak( response.data.msg );
			}
	...
	setRatioSelection : function(postid, n, el) {
		...
		if ( x && y ) {
			...
			if ( sel = this.iasapi.getSelection(true) ) {
				...
				if ( r > h ) {
					r = h;
					var errorMessage = __( '选定的裁剪比例超出了图像的边界。试试不同的比例。' );
