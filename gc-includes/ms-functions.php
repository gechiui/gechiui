...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new GC_Error( 'user_does_not_exist', __( '该用户不存在。' ) );
	}
	...
	gc_cache_delete( $blog_id . '_user_count', 'blog-details' );
 ...
 * at gc-admin/network/settings.php. The check is only run on
 ...
 * administrators.
 *
 * The {@see 'gcmu_validate_user_signup'} hook provides an easy way to modify the sign-up
 * process. The value $result, which is passed to the hook, contains both the user-provided
 * info and the error messages created by the function. {@see 'gcmu_validate_user_signup'}
 ...
 *     @type GC_Error $errors        GC_Error object containing any errors found.
 * }
 */
function gcmu_validate_user_signup( $user_name, $user_email ) {
	global $gcdb;
	...
	if ( $user_name != $orig_username || preg_match( '/[^a-z0-9]/', $user_name ) ) {
		$errors->add( 'user_name', __( '用户名只能包含小写字母（a-z）和数字。' ) );
	...
	if ( empty( $user_name ) ) {
		$errors->add( 'user_name', __( '请输入用户名。' ) );
	}
	...
	if ( in_array( $user_name, $illegal_names, true ) ) {
		$errors->add( 'user_name', __( '抱歉，该用户名不可用。' ) );
	}
	...
	if ( strlen( $user_name ) < 4 ) {
		$errors->add( 'user_name', __( '用户名至少4个字符长。' ) );
	}

	if ( strlen( $user_name ) > 60 ) {
		$errors->add( 'user_name', __( '用户名不能超过60个字符。' ) );
	}

	// All numeric?
	if ( preg_match( '/^[0-9]*$/', $user_name ) ) {
		$errors->add( 'user_name', __( '抱歉，用户名必须要有字母！' ) );
	}
	...
	if ( is_array( $limited_email_domains ) && ! empty( $limited_email_domains ) ) {
		...
		if ( ! in_array( $emaildomain, $limited_email_domains, true ) ) {
			$errors->add( 'user_email', __( '抱歉，不允许使用此电子邮箱！' ) );
		}
	}

	// Check if the username has been used already.
	if ( username_exists( $user_name ) ) {
		$errors->add( 'user_name', __( '抱歉，用户名已存在！' ) );
	}
	...
	$signup = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->signups WHERE user_login = %s", $user_name ) );
	if ( $signup instanceof stdClass ) {
		...
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$gcdb->delete( $gcdb->signups, array( 'user_login' => $user_name ) );
		} else {
			$errors->add( 'user_name', __( '该用户名目前已被保留，几天后可能会被注册使用。' ) );
		}
	}

	$signup = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->signups WHERE user_email = %s", $user_email ) );
	if ( $signup instanceof stdClass ) {
		$diff = time() - mysql2date( 'U', $signup->registered );
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$gcdb->delete( $gcdb->signups, array( 'user_email' => $user_email ) );
		} else {
			$errors->add( 'user_email', __( '此电子邮箱已被使用。请检查您的收件箱是否收到激活邮件。如果您不进行任何操作，该电子邮箱将再几天后重新开放注册。' ) );
		}
	}
	 ...
	 * }
	 */
	return apply_filters( 'gcmu_validate_user_signup', $result );
}
 ...
 * }
 */
function gcmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	global $gcdb, $domain;
	...
	if ( in_array( $blogname, $illegal_names, true ) ) {
		$errors->add( 'blogname', __( '不可使用该名称。' ) );
	}
	 ...
	 * }
	 */
	return apply_filters( 'gcmu_validate_blog_signup', $result );
}
 ...
 */
function gcmu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	global $gcdb;

	$key = substr( md5( time() . gc_rand() . $domain ), 0, 16 );
 ...
 */
function gcmu_signup_user( $user, $user_email, $meta = array() ) {
	global $gcdb;

	// Format data.
	$user       = preg_replace( '/\s+/', '', sanitize_user( $user, true ) );
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . gc_rand() . $user_email ), 0, 16 );
 ...
 * is enabled.
 *
 * Filter {@see 'gcmu_signup_blog_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'gcmu_signup_blog_notification_email'} and
 ...
 * @return bool
 */
function gcmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string       $user_email User email address.
	 * @param string       $key        Activation key created in gcmu_signup_blog().
	 * @param array        $meta       Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'gcmu_signup_blog_notification', $domain, $path, $title, $user_login, $user_email, $key, $meta ) ) {
		return false;
	}

	// Send email with activation link.
	if ( ! is_subdomain_install() || get_current_network_id() != 1 ) {
		$activate_url = network_site_url( "gc-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}gc-activate.php?key=$key"; // @todo Use *_url() API.
	}
	...
	if ( '' === $admin_email ) {
		$admin_email = 'support@' . gc_parse_url( network_home_url(), PHP_URL_HOST );
	}

	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'GeChiUI';
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'gcmu_signup_blog_notification_subject',
			/* translators: New site notification email subject. 1: Network title, 2: New site URL. */
			_x( '[%1$s] 激活账户%2$s', 'New site notification email subject' ),
			$domain,
			$path,
			$title,
			$user_login,
	...
	gc_mail( $user_email, gc_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * been requested.
 *
 * Filter {@see 'gcmu_signup_user_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'gcmu_signup_user_notification_email'} and
 ...
 * @param string $user_login The user's login name.
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in gcmu_signup_user()
 * @param array  $meta       Optional. Signup meta data. Default empty array.
 * @return bool
 */
function gcmu_signup_user_notification( $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string $user_login User login name.
	 * @param string $user_email User email address.
	 * @param string $key        Activation key created in gcmu_signup_user().
	 * @param array  $meta       Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'gcmu_signup_user_notification', $user_login, $user_email, $key, $meta ) ) {
		return false;
	}
	...
	$message         = sprintf(
		...
		site_url( "gc-activate.php?key=$key" )
	);
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'gcmu_signup_user_notification_subject',
			/* translators: New user notification email subject. 1: Network title, 2: New user login. */
			_x( '[%1$s] 激活账户%2$s', 'New user notification email subject' ),
			$user_login,
			$user_email,
			$key,
			$meta
...
function gcmu_activate_signup( $key ) {
	...
	$signup = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->signups WHERE activation_key = %s", $key ) );

	if ( empty( $signup ) ) {
		return new GC_Error( 'invalid_key', __( '启用密钥有误。' ) );
	}
	...
	if ( ! $user_id ) {
		$user_id = gcmu_create_user( $signup->user_login, $password, $signup->user_email );
	} else {
		$user_already_exists = true;
	}

	if ( ! $user_id ) {
		return new GC_Error( 'create_user', __( '无法创建用户' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new GC_Error( 'user_already_exists', __( '该用户名已激活。' ), $signup );
		}
		 ...
		 */
		do_action( 'gcmu_activate_user', $user_id, $password, $meta );
	...
	$blog_id = gcmu_create_blog( $signup->domain, $signup->path, $signup->title, $user_id, $meta, get_current_network_id() );
	 ...
	 * @param array  $meta          Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	do_action( 'gcmu_activate_blog', $blog_id, $user_id, $password, $signup->title, $meta );
 ...
 */
function gc_delete_signup_on_user_delete( $id, $reassign, $user ) {
	global $gcdb;

	$gcdb->delete( $gcdb->signups, array( 'user_login' => $user->user_login ) );
}
...
function gcmu_create_user( $user_name, $password, $email ) {
	...
	$user_id = gc_create_user( $user_name, $password, $email );
	 ...
	 * @param int $user_id User ID.
	 */
	do_action( 'gcmu_new_user', $user_id );
...
function gcmu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	...
	// Data to pass to gc_initialize_site().
	...
	$blog_id = gc_insert_site( array_merge( $site_data, $site_initialization_data ) );
...
禁用这些通知：%3$s'
	...
	gc_mail( $email, sprintf( __( '新注册用户：%s' ), $user->user_login ), $msg );
 ...
 *
 * Filter {@see 'gcmu_welcome_notification'} to disable or bypass.
 *
 ...
 */
function gcmu_welcome_notification( $blog_id, $user_id, $password, $title, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array     $meta     Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'gcmu_welcome_notification', $blog_id, $user_id, $password, $title, $meta ) ) {
		return false;
	}
...
——SITE_NAME团队'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'GeChiUI';
	}
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, gc_unslash( $title ) ) );

	gc_mail( $user->user_email, gc_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * @return bool Whether the email notification was sent.
 */
function gcmu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param GC_User $user User object of the administrator of the new site.
	 */
...
名称：%3$s'
	...
	gc_mail(
		$new_site_email['to'],
		gc_specialchars_decode( $new_site_email['subject'] ),
 ...
 * @return bool
 */
function gcmu_welcome_user_notification( $user_id, $password, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array  $meta     Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'gcmu_welcome_user_notification', $user_id, $password, $meta ) ) {
		return false;
	}
	...
	$welcome_email = str_replace( 'USERNAME', $user->user_login, $welcome_email );
	$welcome_email = str_replace( 'PASSWORD', $password, $welcome_email );
	$welcome_email = str_replace( 'LOGINLINK', gc_login_url(), $welcome_email );
...
function update_posts_count( $deprecated = '' ) {
	global $gcdb;
	update_option( 'post_count', (int) $gcdb->get_var( "SELECT COUNT(ID) FROM {$gcdb->posts} WHERE post_status = 'publish' and post_type = 'post'" ) );
}
 ...
 */
function gcmu_log_new_registrations( $blog_id, $user_id ) {
	global $gcdb;
	...
	if ( $user ) {
		$gcdb->insert(
			$gcdb->registration_log,
			array(
				'email'           => $user->user_email,
				'IP'              => preg_replace( '/[^0-9., ]/', '', gc_unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'GC_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}

	if ( strlen( $upload['bits'] ) > ( KB_IN_BYTES * get_site_option( 'fileupload_maxk', 1500 ) ) ) {
		/* translators: %s: Maximum allowed file size in kilobytes. */
		return sprintf( __( '此文件太大，文件必须小于%s KB。' ) . '<br />', get_site_option( 'fileupload_maxk', 1500 ) );
	}
...
function signup_nonce_fields() {
	...
	gc_nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'gc-signup.php' ) ) {
		return $result;
	}

	if ( ! gc_verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
		$result['errors']->add( 'invalid_nonce', __( '无法提交此表单，请重试。' ) );
	}
 ...
 * }
 * @return true|GC_Error|void True on success or a GC_Error object if the user doesn't exist
...
function gc_schedule_update_network_counts() {
	...
	if ( ! gc_next_scheduled( 'update_network_counts' ) && ! gc_installing() ) {
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function gc_update_network_counts( $network_id = null ) {
	gc_update_network_user_counts( $network_id );
	gc_update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function gc_maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! gc_is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function gc_maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! gc_is_large_network( 'users', $network_id );

	/** This filter is documented in gc-includes/ms-functions.php */
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function gc_update_network_site_counts( $network_id = null ) {
 ...
 */
function gc_is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	/** This filter is documented in gc-includes/ms-functions.php */
	return apply_filters( 'gc_is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', gc_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
 ...
 */
function gc_network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// Get network name.
	$network_name = gc_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );
