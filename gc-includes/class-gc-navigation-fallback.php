...
class GC_Navigation_Fallback {
	 ...
	 * @since 6.3.0
	 *
	 * @return GC_Post|null the fallback Navigation Post or null.
	 */
	public static function get_fallback() {
		 ...
		 */
		$should_create_fallback = apply_filters( 'gc_navigation_should_create_fallback', true );
		...
		if ( $fallback && ! is_gc_error( $fallback ) ) {
			// Return the newly created fallback post object which will now be the most recently created navigation menu.
			return $fallback instanceof GC_Post ? $fallback : static::get_most_recently_published_navigation();
		}
	...
	/**
	 * Finds the most recently published `gc_navigation` post type.
	 *
	 * @since 6.3.0
	 *
	 ...
	 * @since 6.3.0
	 *
	 * @return int|GC_Error The post ID of the default fallback menu or a GC_Error object.
	 */
	private static function create_classic_menu_fallback() {
		...
		if ( ! $classic_nav_menu ) {
			return new GC_Error( 'no_classic_menus', __( '找不到经典菜单。' ) );
		}

		// If there is a classic menu then convert it to blocks.
		$classic_nav_menu_blocks = GC_Classic_To_Block_Menu_Converter::convert( $classic_nav_menu );

		if ( is_gc_error( $classic_nav_menu_blocks ) ) {
			return $classic_nav_menu_blocks;
		}

		if ( empty( $classic_nav_menu_blocks ) ) {
			return new GC_Error( 'cannot_convert_classic_menu', __( '无法将经典菜单转换为块。' ) );
		}

		// Create a new navigation menu from the classic menu.
		$classic_menu_fallback = gc_insert_post(
			array(
	 ...
	 * @since 6.3.0
	 *
	 * @return GC_Term|null The most appropriate classic navigation menu to use as a fallback.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @param GC_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return GC_Term The most recently created classic nav menu.
	 */
	 ...
	 *
	 * @param GC_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return GC_Term|null The classic nav menu with the slug `primary` or null.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @return GC_Term|null The classic nav menu assigned to the `primary` location or null.
	 */
	...
	private static function create_default_fallback() {
		...
		$default_fallback = gc_insert_post(
			array(
				'post_content' => $default_blocks,
				'post_title'   => _x( '导航', 'Title of a Navigation menu' ),
	...
	private static function get_default_fallback_blocks() {
		...
		return $registry->is_registered( 'core/page-list' ) ? '<!-- gc:page-list /-->' : '';
	}
}
