...
/**
 * Checks whether GeChiUI should replace old HTTP URLs to the site with their HTTPS counterpart.
 *
 * If a GeChiUI site had its URL changed from HTTP to HTTPS, by default this will return `true`, causing GeChiUI to
 ...
 */
function gc_should_replace_insecure_home_url() {
	$should_replace_insecure_home_url = gc_is_using_https()
		&& get_option( 'https_migration_required' )
		// For automatic replacement, both 'home' and 'siteurl' need to not only use HTTPS, they also need to be using
		// the same domain.
		&& gc_parse_url( home_url(), PHP_URL_HOST ) === gc_parse_url( site_url(), PHP_URL_HOST );

	/**
	 * Filters whether GeChiUI should replace old HTTP URLs to the site with their HTTPS counterpart.
	 *
	 ...
	 */
	return apply_filters( 'gc_should_replace_insecure_home_url', $should_replace_insecure_home_url );
}
 ...
 */
function gc_replace_insecure_home_url( $content ) {
	if ( ! gc_should_replace_insecure_home_url() ) {
		return $content;
	}
 ...
 */
function gc_update_urls_to_https() {
 ...
 */
function gc_update_https_migration_required( $old_url, $new_url ) {
	// Do nothing if GeChiUI is being installed.
	if ( gc_installing() ) {
		return;
	}
