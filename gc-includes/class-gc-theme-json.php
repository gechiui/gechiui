...
class GC_Theme_JSON {
	 ...
	 *                       Example output: "--gc--preset--duotone--blue: <value>" will generate as many CSS Custom Properties as presets defined
	 ...
	 * @since 6.3.0 Replaced value_func for duotone with `null`. Custom properties are handled by class-gc-duotone.php.
	 * @var array
	 */
	const PRESETS_METADATA = array(
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'gradient',
			'css_vars'          => '--gc--preset--gradient--$slug',
		...
		array(
			...
			'use_default_names' => false,
			'value_func'        => null, // CSS Custom Properties for duotone are handled by block supports in class-gc-duotone.php.
			'css_vars'          => null,
			'classes'           => array(),
		...
		array(
			...
			'value_func'        => 'gc_get_typography_font_size_value',
			'css_vars'          => '--gc--preset--font-size--$slug',
		...
		array(
			...
			'css_vars'          => '--gc--preset--font-family--$slug',
		...
		array(
			...
			'use_default_names' => true,
			'value_key'         => 'size',
			'css_vars'          => '--gc--preset--spacing--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'shadow',
			'css_vars'          => '--gc--preset--shadow--$slug',
			'classes'           => array(),
	 ...
	 *              `letter-spacing`, `margin-*`, `padding-*`, `--gc--style--block-gap`,
	 ...
	 *              `--gc--style--root--padding-*`, and `box-shadow` properties,
	 *              removed the `--gc--style--block-gap` property.
	...
	const PROPERTIES_METADATA = array(
		...
		'--gc--style--root--padding'        => array( 'spacing', 'padding' ),
		'--gc--style--root--padding-top'    => array( 'spacing', 'padding', 'top' ),
		'--gc--style--root--padding-right'  => array( 'spacing', 'padding', 'right' ),
		'--gc--style--root--padding-bottom' => array( 'spacing', 'padding', 'bottom' ),
		'--gc--style--root--padding-left'   => array( 'spacing', 'padding', 'left' ),
	 ...
	 *
	 * See https://core.trac.gechiui.com/ticket/56928.
	...
	const ELEMENTS = array(
		'link'    => 'a:where(:not(.gc-element-button))', // The `where` is needed to lower the specificity.
		...
		'h5'      => 'h5',
		'h6'      => 'h6',
		// We have the .gc-block-button__link class so that this will target older buttons that have been serialized.
	...
	const __EXPERIMENTAL_ELEMENT_CLASS_NAMES = array(
		'button'  => 'gc-element-button',
		'caption' => 'gc-element-caption',
	);
	...
	public static function get_element_class_name( $element ) {
		...
		/*
		 * TODO: Replace array_key_exists() with isset() check once GeChiUI drops
		 * support for PHP 5.6. See https://core.trac.gechiui.com/ticket/57067.
		 */
	...
	public function __construct( $theme_json = array(), $origin = 'theme' ) {
		...
		$this->theme_json    = GC_Theme_JSON_Schema::migrate( $theme_json );
	...
	protected static function do_opt_in_into_settings( &$context ) {
		foreach ( static::APPEARANCE_TOOLS_OPT_INS as $path ) {
			 ...
			 */
			if ( 'unset prop' === _gc_array_get( $context, $path, 'unset prop' ) ) {
				_gc_array_set( $context, $path, true );
			}
		}
	...
	protected static function get_blocks_metadata() {
		...
		foreach ( $blocks as $block_name => $block_type ) {
			$root_selector = gc_get_block_css_selector( $block_type );
			...
			$duotone_selector = gc_get_block_css_selector( $block_type, 'filter.duotone' );

			// Keep backwards compatibility for support.color.__experimentalDuotone.
			if ( null === $duotone_selector ) {
				$duotone_support = _gc_array_get( $block_type->supports, array( 'color', '__experimentalDuotone' ), null );

				if ( $duotone_support ) {
					$root_selector    = gc_get_block_css_selector( $block_type );
					$duotone_selector = GC_Theme_JSON::scope_selector( $root_selector, $duotone_support );
				}
			}
	...
	public function get_stylesheet( $types = array( 'variables', 'styles', 'presets' ), $origins = null, $options = array() ) {
		...
		if ( in_array( 'styles', $types, true ) ) {
			...
			$columns_selector       = '.gc-block-columns';
			$post_template_selector = '.gc-block-post-template';
			if ( ! empty( $options['scope'] ) ) {
	...
	public function get_custom_css() {
		...
		$stylesheet = _gc_array_get( $this->theme_json, array( 'styles', 'css' ), '' );

		// Add the global styles block CSS.
		if ( isset( $this->theme_json['styles']['blocks'] ) ) {
			foreach ( $this->theme_json['styles']['blocks'] as $name => $node ) {
				$custom_block_css = _gc_array_get( $this->theme_json, array( 'styles', 'blocks', $name, 'css' ) );
				if ( $custom_block_css ) {
	...
	protected function get_layout_styles( $block_metadata ) {
		...
		if ( isset( $block_metadata['name'] ) ) {
			$block_type = GC_Block_Type_Registry::get_instance()->get_registered( $block_metadata['name'] );
		...
		$has_block_gap_support    = _gc_array_get( $this->theme_json, array( 'settings', 'spacing', 'blockGap' ) ) !== null;
		$has_fallback_gap_support = ! $has_block_gap_support; // This setting isn't useful yet: it exists as a placeholder for a future explicit fallback gap styles support.
		$node                     = _gc_array_get( $this->theme_json, $block_metadata['path'], array() );
		$layout_definitions       = gc_get_layout_definitions();
		...
		if ( $has_block_gap_support || $has_fallback_gap_support ) {
			...
			if ( ! $has_block_gap_support ) {
				...
				if ( ! empty( $block_type ) ) {
					$block_gap_value = _gc_array_get( $block_type->supports, array( 'spacing', 'blockGap', '__experimentalDefault' ), null );
				}
			} else {
			...
			if ( null !== $block_gap_value && false !== $block_gap_value && '' !== $block_gap_value ) {
				foreach ( $layout_definitions as $layout_definition_key => $layout_definition ) {
					...
					$class_name    = sanitize_title( _gc_array_get( $layout_definition, array( 'className' ), false ) );
					$spacing_rules = _gc_array_get( $layout_definition, array( 'spacingStyles' ), array() );
		...
		if (
			...
			foreach ( $layout_definitions as $layout_definition ) {
				$class_name       = sanitize_title( _gc_array_get( $layout_definition, array( 'className' ), false ) );
				$base_style_rules = _gc_array_get( $layout_definition, array( 'baseStyles' ), array() );
	...
	protected static function get_settings_values_by_slug( $settings, $preset_metadata, $origins ) {
		$preset_per_origin = _gc_array_get( $settings, $preset_metadata['path'], array() );
		...
		foreach ( $origins as $origin ) {
			...
			foreach ( $preset_per_origin[ $origin ] as $preset ) {
				$slug = _gc_to_kebab_case( $preset['slug'] );
	 ...
	 * @return string The CSS Custom Property. Something along the lines of `--gc--preset--color--black`.
	 */
	...
	protected static function compute_theme_vars( $settings ) {
		$declarations  = array();
		$custom_values = _gc_array_get( $settings, array( 'custom' ), array() );
	...
	protected static function flatten_tree( $tree, $prefix = '', $token = '--' ) {
		...
		foreach ( $tree as $property => $value ) {
			$new_key = $prefix . str_replace(
				'/',
				'-',
				strtolower( _gc_to_kebab_case( $property ) )
			);
	...
	protected static function compute_style_properties( $styles, $settings = array(), $properties = null, $theme_json = null, $selector = null, $use_root_padding = null ) {
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			if ( str_starts_with( $css_property, '--gc--style--root--' ) && ( static::ROOT_BLOCK_SELECTOR !== $selector || ! $use_root_padding ) ) {
				continue;
			}
			/*
			 * Root-level padding styles don't currently support strings with CSS shorthand values.
			 * This may change: https://github.com/GeChiUI/gutenberg/issues/40132.
			 */
			if ( '--gc--style--root--padding' === $css_property && is_string( $value ) ) {
				continue;
			}

			if ( str_starts_with( $css_property, '--gc--style--root--' ) && $use_root_padding ) {
				$root_variable_duplicates[] = substr( $css_property, strlen( '--gc--style--root--' ) );
			}
			...
			if ( is_array( $value_path ) ) {
				...
				if (
					...
					_gc_array_get( $settings, static::PROTECTED_PROPERTIES[ $path_string ], null ) === null
				) {
					continue;
				}
			}
			...
			if ( 'font-size' === $css_property ) {
				/*
				 * gc_get_typography_font_size_value() will check
				 * if fluid typography has been activated and also
				 * whether the incoming value can be converted to a fluid value.
				 * Values that already have a clamp() function will not pass the test,
				 * and therefore the original $value will be returned.
				 */
				$value = gc_get_typography_font_size_value( array( 'size' => $value ) );
			}
	 ...
	 *              to the standard form "--gc--preset--color--secondary".
	...
	protected static function get_property_value( $styles, $path, $theme_json = null ) {
		...
		if ( is_array( $value ) && isset( $value['ref'] ) ) {
			...
			$ref_value  = _gc_array_get( $theme_json, $value_path );
			...
			if ( is_array( $ref_value ) && isset( $ref_value['ref'] ) ) {
				...
				_doing_it_wrong(
					...
					sprintf(
						...
						__( '您的 %1$s 文件为路径 %3$s 使用了一个动态值（%2$s）。但是 %3$s 处的值也是一个动态值（指向 %4$s），并且系统不支持指向另一个动态值。请更新 %3$s，使其直接指向 %4$s。' ),
						'theme.json',
						$ref_value_string,
						$path_string,
						$ref_value['ref']
	...
	protected static function get_style_nodes( $theme_json, $selectors = array() ) {
		 ...
		 */
		return apply_filters( 'gc_theme_json_get_style_nodes', $nodes );
	}
	...
	public function get_styles_for_block( $block_metadata ) {
		...
		$settings             = _gc_array_get( $this->theme_json, array( 'settings' ) );
		...
		if ( ! empty( $block_metadata['variations'] ) ) {
			foreach ( $block_metadata['variations'] as $style_variation ) {
				$style_variation_node           = _gc_array_get( $this->theme_json, $style_variation['path'], array() );
		...
		if ( '.gc-block-separator' === $selector ) {
	...
	public function get_root_layout_rules( $selector, $block_metadata ) {
		...
		* @link https://github.com/GeChiUI/gutenberg/issues/36147.
		*/
		$css .= 'body { margin: 0;';
		...
		*/
		if ( isset( $settings['layout']['contentSize'] ) || isset( $settings['layout']['wideSize'] ) ) {
			...
			$css         .= '--gc--style--global--content-size: ' . $content_size . ';';
			$css         .= '--gc--style--global--wide-size: ' . $wide_size . ';';
		}
		...
		if ( $use_root_padding ) {
			...
			$css .= '.gc-site-blocks { padding-top: var(--gc--style--root--padding-top); padding-bottom: var(--gc--style--root--padding-bottom); }';
			// Right and left padding are applied to the first container with `.has-global-padding` class.
			$css .= '.has-global-padding { padding-right: var(--gc--style--root--padding-right); padding-left: var(--gc--style--root--padding-left); }';
			// Nested containers with `.has-global-padding` class do not get padding.
			$css .= '.has-global-padding :where(.has-global-padding) { padding-right: 0; padding-left: 0; }';
			// Alignfull children of the container with left and right padding have negative margins so they can still be full width.
			$css .= '.has-global-padding > .alignfull { margin-right: calc(var(--gc--style--root--padding-right) * -1); margin-left: calc(var(--gc--style--root--padding-left) * -1); }';
			...
			$css .= '.has-global-padding > .alignfull:where(:not(.has-global-padding)) > :where([class*="gc-block-"]:not(.alignfull):not([class*="__"]),p,h1,h2,h3,h4,h5,h6,ul,ol) { padding-right: var(--gc--style--root--padding-right); padding-left: var(--gc--style--root--padding-left); }';
			// The above rule also has to be negated for blocks inside nested `.has-global-padding` blocks.
			$css .= '.has-global-padding :where(.has-global-padding) > .alignfull:where(:not(.has-global-padding)) > :where([class*="gc-block-"]:not(.alignfull):not([class*="__"]),p,h1,h2,h3,h4,h5,h6,ul,ol) { padding-right: 0; padding-left: 0; }';
		}

		$css .= '.gc-site-blocks > .alignleft { float: left; margin-right: 2em; }';
		$css .= '.gc-site-blocks > .alignright { float: right; margin-left: 2em; }';
		$css .= '.gc-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';

		$block_gap_value       = _gc_array_get( $this->theme_json, array( 'styles', 'spacing', 'blockGap' ), '0.5em' );
		$has_block_gap_support = _gc_array_get( $this->theme_json, array( 'settings', 'spacing', 'blockGap' ) ) !== null;
		if ( $has_block_gap_support ) {
			$block_gap_value = static::get_property_value( $this->theme_json, array( 'styles', 'spacing', 'blockGap' ) );
			$css            .= ":where(.gc-site-blocks) > * { margin-block-start: $block_gap_value; margin-block-end: 0; }";
			$css            .= ':where(.gc-site-blocks) > :first-child:first-child { margin-block-start: 0; }';
			$css            .= ':where(.gc-site-blocks) > :last-child:last-child { margin-block-end: 0; }';

			// For backwards compatibility, ensure the legacy block gap CSS variable is still available.
			$css .= "$selector { --gc--style--block-gap: $block_gap_value; }";
		}
	 ...
	 *
	 * @param GC_Theme_JSON $incoming Data to merge.
	 */
	public function merge( $incoming ) {
		...
		foreach ( $nodes as $node ) {
			...
			$content = _gc_array_get( $incoming_data, $path, null );
			if ( isset( $content ) ) {
				_gc_array_set( $this->theme_json, $path, $content );
			}
			...
			foreach ( static::PRESETS_METADATA as $preset ) {
				...
				foreach ( static::VALID_ORIGINS as $origin ) {
					...
					if (
						...
						$slugs_for_preset = _gc_array_get( $slugs, $preset['path'], array() );
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			foreach ( $origins as $origin ) {
				...
				foreach ( $duotone_presets[ $origin ] as $duotone_preset ) {
					$filters .= gc_get_duotone_filter_svg( $duotone_preset );
				}
			}
		}
	...
	protected static function should_override_preset( $theme_json, $path, $override ) {
		...
		if ( is_array( $override ) ) {
			$value = _gc_array_get( $theme_json, array_merge( $path, $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}

			// Search the top-level key if none was found for this node.
			$value = _gc_array_get( $theme_json, array_merge( array( 'settings' ), $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}
	...
	protected static function get_default_slugs( $data, $node_path ) {
		...
		foreach ( static::PRESETS_METADATA as $metadata ) {
			...
			$preset = _gc_array_get( $data, $path, null );
			if ( ! isset( $preset ) ) {
				continue;
			}
			...
			_gc_array_set( $slugs, $metadata['path'], $slugs_for_preset );
		}
	...
	protected function get_name_from_defaults( $slug, $base_path ) {
		$path            = $base_path;
		$path[]          = 'default';
		$default_content = _gc_array_get( $this->theme_json, $path, null );
		if ( ! $default_content ) {
			return null;
		}
	...
	public static function remove_insecure_properties( $theme_json ) {
		...
		$theme_json = GC_Theme_JSON_Schema::migrate( $theme_json );
		...
		foreach ( $style_nodes as $metadata ) {
			$input = _gc_array_get( $theme_json, $metadata['path'], array() );
			if ( empty( $input ) ) {
				continue;
			}
			...
			if ( ! empty( $output ) ) {
				_gc_array_set( $sanitized, $metadata['path'], $output );
			}
		}
	...
	protected static function remove_insecure_settings( $input ) {
		...
		foreach ( static::PRESETS_METADATA as $preset_metadata ) {
			foreach ( static::VALID_ORIGINS as $origin ) {
				...
				$presets            = _gc_array_get( $input, $path_with_origin, null );
				if ( null === $presets ) {
					continue;
				}
				...
				if ( ! empty( $escaped_preset ) ) {
					_gc_array_set( $output, $path_with_origin, $escaped_preset );
				}
			}
		}
	...
	protected static function remove_insecure_styles( $input ) {
		...
		foreach ( $declarations as $declaration ) {
			if ( static::is_safe_css_declaration( $declaration['name'], $declaration['value'] ) ) {
				 ...
				 */
				$value = _gc_array_get( $input, $path, array() );
				if ( ! is_array( $value ) ) {
					_gc_array_set( $output, $path, $value );
				}
			}
		}
	...
	private static function remove_indirect_properties( $input, &$output ) {
		foreach ( static::INDIRECT_PROPERTIES_METADATA as $property => $paths ) {
			foreach ( $paths as $path ) {
				$value = _gc_array_get( $input, $path );
				if (
					is_string( $value ) &&
	...
	public function get_data() {
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _gc_array_get( $output, $path, null );
				if ( null === $preset ) {
					continue;
				}
				...
				_gc_array_set( $output, $path, $flattened_preset );
			}
		}
		...
		foreach ( $nodes as $node ) {
			...
			foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
				 ...
				 * "null" can be a valid value for some props (e.g. blockGap).
				 */
				$opt_in_value = _gc_array_get( $output, $full_path, 'unset prop' );
			...
			if ( $all_opt_ins_are_set ) {
				...
				_gc_array_set( $output, $node_path_with_appearance_tools, true );
				foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
					$full_path = $node['path'];
					foreach ( $opt_in_path as $opt_in_path_item ) {
						$full_path[] = $opt_in_path_item;
	...
	public function set_spacing_sizes() {
		$spacing_scale = _gc_array_get( $this->theme_json, array( 'settings', 'spacing', 'spacingScale' ), array() );
		...
		if ( ! isset( $spacing_scale['steps'] )
			...
			if ( ! empty( $spacing_scale ) ) {
				trigger_error( __( 'theme.json 中 settings.spacing.spacingScale 的某些值无效' ), E_USER_NOTICE );
			}
			return null;
		}
		...
		for ( $below_midpoint_count = $steps_mid_point - 1; $spacing_scale['steps'] > 1 && $slug > 0 && $below_midpoint_count > 0; $below_midpoint_count-- ) {
			...
			$below_sizes[] = array(
				...
				'name' => $below_midpoint_count === $steps_mid_point - 1 ? __( '小' ) : sprintf( __( '%s 倍小' ), (string) $x_small_count ),
				'slug' => (string) $slug,
		...
		for ( $above_midpoint_count = 0; $above_midpoint_count < $steps_above; $above_midpoint_count++ ) {
			...
			$above_sizes[] = array(
				...
				'name' => 0 === $above_midpoint_count ? __( '大' ) : sprintf( __( '%s 倍大' ), (string) $x_large_count ),
				'slug' => (string) $slug,
				'size' => round( $current_step, 2 ) . $unit,
			);
		...
		_gc_array_set( $this->theme_json, array( 'settings', 'spacing', 'spacingSizes', 'default' ), $spacing_sizes );
	}

	/**
	 * This is used to convert the internal representation of variables to the CSS representation.
	 * For example, `var:preset|color|vivid-green-cyan` becomes `var(--gc--preset--color--vivid-green-cyan)`.
	 *
	 * @since 6.3.0
	...
	protected static function get_block_selectors( $block_type, $root_selector ) {
		...
		foreach ( static::BLOCK_SUPPORT_FEATURE_LEVEL_SELECTORS as $key => $feature ) {
			$feature_selector = gc_get_block_css_selector( $block_type, $key );
			...
			if ( 0 <= strpos( $style, 'var(' ) ) {
				...
				if ( $has_matches ) {
					...
					foreach ( $var_parts[1] as $index => $var_part ) {
						...
						$rule_to_replace = $var_parts[0][ $index ]; // the css rule to replace e.g. var(--gc--preset--color--vivid-green-cyan).
	 ...
	 *
	 * @since 6.3.0
	 * @param GC_Theme_JSON $theme_json The theme json resolver.
	 *
	 * @return GC_Theme_JSON The $theme_json with resolved variables.
	 */
