...
class GC_Theme_JSON {
	...
	const PRESETS_METADATA = array(
		...
		array(
			...
			'override'          => true,
			'use_default_names' => false,
			'value_func'        => 'gc_render_duotone_filter_preset',
	...
	private static function get_blocks_metadata() {
		...
		foreach ( $blocks as $block_name => $block_type ) {
			if (
				...
				self::$blocks_metadata[ $block_name ]['selector'] = '.gc-block-' . str_replace( '/', '-', str_replace( 'core/', '', $block_name ) );
			}
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			if ( is_array( $value_path ) ) {
				...
				if (
					...
					_gc_array_get( $settings, self::PROTECTED_PROPERTIES[ $path_string ], null ) === null
				) {
					continue;
				}
			}
