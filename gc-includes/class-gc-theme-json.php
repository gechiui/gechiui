...
class GC_Theme_JSON {
	 ...
	 *                        Example output: "--gc--preset--duotone--blue: <value>" will generate
	...
	const PRESETS_METADATA = array(
		...
		array(
			...
			'override'          => true,
			'use_default_names' => false,
			'value_func'        => 'gc_get_duotone_filter_property',
			'css_vars'          => '--gc--preset--duotone--$slug',
			'classes'           => array(),
	...
	const PROPERTIES_METADATA = array(
		...
		'--gc--style--block-gap'     => array( 'spacing', 'blockGap' ),
	...
	protected static function get_blocks_metadata() {
		...
		foreach ( $blocks as $block_name => $block_type ) {
			if (
				...
				static::$blocks_metadata[ $block_name ]['selector'] = '.gc-block-' . str_replace( '/', '-', str_replace( 'core/', '', $block_name ) );
			}
	...
	protected function get_block_classes( $style_nodes ) {
		...
		foreach ( $style_nodes as $metadata ) {
			...
			if ( static::ROOT_BLOCK_SELECTOR === $selector ) {
				$block_rules .= '.gc-site-blocks > .alignleft { float: left; margin-right: 2em; }';
				$block_rules .= '.gc-site-blocks > .alignright { float: right; margin-left: 2em; }';
				$block_rules .= '.gc-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';

				$has_block_gap_support = _gc_array_get( $this->theme_json, array( 'settings', 'spacing', 'blockGap' ) ) !== null;
				if ( $has_block_gap_support ) {
					$block_rules .= '.gc-site-blocks > * { margin-top: 0; margin-bottom: 0; }';
					$block_rules .= '.gc-site-blocks > * + * { margin-top: var( --gc--style--block-gap ); }';
				}
			}
		}
