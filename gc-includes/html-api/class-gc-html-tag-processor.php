<?php
/**
 * HTML API: GC_HTML_Tag_Processor class
 *
 ...
 * Example:
 *
 *     $tags = new GC_HTML_Tag_Processor( $html );
 ...
 * Example:
 *
 *     if ( $tags->next_tag( array( 'class' => 'gc-group-block' ) ) ) {
...
class GC_HTML_Tag_Processor {
	 ...
	 * @var int
	 *
	 * @see GC_HTML_Tag_Processor::set_bookmark()
	 */
	 ...
	 * @var int
	 *
	 * @see GC_HTML_Tag_Processor::seek()
	 */
	 ...
	 *         'id' => new GC_HTML_Attribute_Match( 'id', null, 6, 17 )
	 *     );
	 *
	 ...
	 *     $this->attributes = array(
	 *         'id'    => new GC_HTML_Attribute_Match( 'id', null, 6, 17 ),
	 *         'class' => new GC_HTML_Attribute_Match( 'class', 'outline', 18, 32 )
	 *     );
	 *
	 ...
	 * Example:
	 *
	 *     // Add the `gc-block-group` class, remove the `gc-group` class.
	 *     $classname_updates = array(
	 *         // Indexed by a comparable class name.
	 *         'gc-block-group' => GC_HTML_Tag_Processor::ADD_CLASS,
	 *         'gc-group'       => GC_HTML_Tag_Processor::REMOVE_CLASS
	 *     );
	 *
	 * @since 6.2.0
	 * @var bool[]
	 ...
	 *     $modifications[] = new GC_HTML_Text_Replacement( $start, $end, $new_value );
	 *
	 *     // Correspondingly, something like this will appear in this array.
	 *     $lexical_updates = array(
	 *         GC_HTML_Text_Replacement( 14, 28, 'https://my-site.my-domain/gc-content/uploads/2014/08/kittens.jpg' )
	 *     );
	 *
	 * @since 6.2.0
	 * @var GC_HTML_Text_Replacement[]
	 */
	 ...
	 *
	 *     $p = new GC_HTML_Tag_Processor( $html );
	 *     $in_list = false;
	...
	public function set_bookmark( $name ) {
		...
		$this->bookmarks[ $name ] = new GC_HTML_Span(
	...
	private function parse_next_attribute() {
		...
		if ( ! array_key_exists( $comparable_name, $this->attributes ) ) {
			$this->attributes[ $comparable_name ] = new GC_HTML_Attribute_Token(
				$attribute_name,
				$value_start,
				$value_length,
				$attribute_start,
	 ...
	 * @since 6.2.0
	 *
	 * @see GC_HTML_Tag_Processor::$lexical_updates
	 * @see GC_HTML_Tag_Processor::$classname_updates
	 */
	...
	/**
	 * Compare two GC_HTML_Text_Replacement objects.
	 *
	 * @since 6.2.0
	 *
	 * @param GC_HTML_Text_Replacement $a First attribute update.
	 * @param GC_HTML_Text_Replacement $b Second attribute update.
	 ...
	 * Example:
	 *
	 *     $p = new GC_HTML_Tag_Processor( '<div enabled class="test" data-test-id="14">Test</div>' );
	 ...
	 * Example:
	 *
	 *     $p = new GC_HTML_Tag_Processor( '<div data-ENABLED class="test" DATA-test-id="14">Test</div>' );
	 *     $p->next_tag( array( 'class_name' => 'test' ) ) === true;
	 ...
	 * Example:
	 *
	 *     $p = new GC_HTML_Tag_Processor( '<div class="test">Test</div>' );
	 *     $p->next_tag() === true;
	 *     $p->get_tag() === 'DIV';
	 *
	 *     $p->next_tag() === false;
	 ...
	 * Example:
	 *
	 *     $p = new GC_HTML_Tag_Processor( '<div></div>' );
	...
	public function set_attribute( $name, $value ) {
		...
		/*
		 * GeChiUI rejects more characters than are strictly forbidden
		 * in HTML5. This is to prevent additional security risks deeper
		 * in the GeChiUI and plugin stack. Specifically the
		...
		if ( isset( $this->attributes[ $comparable_name ] ) ) {
			...
			$this->lexical_updates[ $comparable_name ] = new GC_HTML_Text_Replacement(
	...
	public function remove_attribute( $name ) {
		 ...
		 *    Result: <div />
		 */
		$this->lexical_updates[ $name ] = new GC_HTML_Text_Replacement(
	 ...
	 * @since 6.2.0
	 *
	 * @see GC_HTML_Tag_Processor::get_updated_html()
	 *
	...
	private function matches() {
		...
		if ( $needs_class_name ) {
			 ...
			 */
			while (
				// phpcs:ignore GeChiUI.CodeAnalysis.AssignmentInCondition.FoundInWhileCondition
