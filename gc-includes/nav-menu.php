 ...
 */
function gc_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_gc_error( $menu_obj ) ) {
		$menu_obj = false;
	}
	 ...
	 */
	return apply_filters( 'gc_get_nav_menu_object', $menu_obj, $menu );
}
...
function register_nav_menus( $locations = array() ) {
	...
	foreach ( $locations as $key => $value ) {
		if ( is_int( $key ) ) {
			_doing_it_wrong( __FUNCTION__, __( '导航菜单位置必须为字符串。' ), '5.3.0' );
			break;
		}
	}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_gc_registered_nav_menus ) && isset( $_gc_registered_nav_menus[ $location ] ) ) {
		unset( $_gc_registered_nav_menus[ $location ] );
		if ( empty( $_gc_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_gc_registered_nav_menus;
	if ( isset( $_gc_registered_nav_menus ) ) {
		return $_gc_registered_nav_menus;
	}
	return array();
}
 ...
 */
function gc_get_nav_menu_name( $location ) {
	$menu_name = '';
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( 'gc_get_nav_menu_name', $menu_name, $location );
}
 ...
 */
function gc_create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return gc_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 */
function gc_delete_nav_menu( $menu ) {
	$menu = gc_get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = gc_delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is_gc_error( $result ) ) {
		 ...
		 */
		do_action( 'gc_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|GC_Error Menu ID on success, GC_Error object on failure.
 */
function gc_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = gc_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_gc_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_gc_error( $_menu ) ) {
		...
		$_menu = gc_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_gc_error( $_menu ) ) {
			return $_menu;
		}
		 ...
		 */
		do_action( 'gc_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = gc_update_term( $menu_id, 'nav_menu', $args );

	if ( is_gc_error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'gc_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * @return int|GC_Error The menu item's database ID or GC_Error object on failure.
 */
function gc_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// Make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects.
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new GC_Error( 'update_nav_menu_item_failed', __( '给出的对象ID不是菜单项的ID。' ) );
	}

	$menu = gc_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new GC_Error( 'invalid_menu_id', __( '菜单ID无效。' ) );
	}
	...
	$args = gc_parse_args( $menu_item_data, $defaults );

	if ( 0 == $menu_id ) {
		$args['menu-item-position'] = 1;
	} elseif ( 0 == (int) $args['menu-item-position'] ) {
		$menu_items                 = 0 == $menu_id ? array() : (array) gc_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( gc_unslash( $args['menu-item-title'] ) === gc_specialchars_decode( $original_title ) ) {
			$args['menu-item-title'] = '';
		}

		// Hack to get gc to create a post object when too many properties are empty.
	...
	$post_date = gc_resolve_post_date( $args['menu-item-post-date'], $args['menu-item-post-date-gmt'] );
	if ( $post_date ) {
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = gc_insert_post( $post, true, $fire_after_hooks );
		if ( ! $menu_item_db_id || is_gc_error( $menu_item_db_id ) ) {
	...
	if ( $update ) {
		...
		$update_post = gc_update_post( $post, true );
		if ( is_gc_error( $update_post ) ) {
			return $update_post;
		}
	}
	 ...
	 */
	do_action( 'gc_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 *                    Default empty array.
 * @return GC_Term[] An array of menu objects.
 */
function gc_get_nav_menus( $args = array() ) {
	$defaults = array(
		'taxonomy'   => 'nav_menu',
		'hide_empty' => false,
		'orderby'    => 'name',
	 ...
	 * @see get_terms()
	 *
	 * @param GC_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 */
	return apply_filters( 'gc_get_nav_menus', get_terms( $args ), $args );
}
 ...
 */
function gc_get_nav_menu_items( $menu, $args = array() ) {
	$menu = gc_get_nav_menu_object( $menu );
	 ...
	 */
	return apply_filters( 'gc_get_nav_menu_items', $items, $menu, $args );
}
...
function gc_setup_nav_menu_item( $menu_item ) {
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $object ) {
					...
					if ( function_exists( 'get_post_states' ) ) {
						...
						if ( $post_states ) {
							$menu_item->type_label = gc_strip_all_tags( implode( ', ', $post_states ) );
						}
					}
				} else {
				...
				if ( '' === $original_title ) {
					...
					$original_title = sprintf( __( '#%d（无标题）' ), $menu_item->object_id );
				}
				...
				$menu_item->type_label = __( '文章类型归档' );
				$post_content          = gc_trim_words( $menu_item->post_content, 200 );
				...
				if ( $original_object && ! is_gc_error( $original_object ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', gc_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			if ( '' === $menu_item->post_title ) {
				/* translators: %d: ID of a post. */
				$menu_item->post_title = sprintf( __( '#%d（无标题）' ), $menu_item->ID );
			}
	 ...
	 */
	return apply_filters( 'gc_setup_nav_menu_item', $menu_item );
}
 ...
 */
function gc_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function _gc_delete_post_menu_item( $object_id ) {
	$object_id = (int) $object_id;

	$menu_item_ids = gc_get_associated_nav_menu_items( $object_id, 'post_type' );
 ...
 */
function _gc_delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = gc_get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 * @param GC_Post $post       The post object being transitioned from one status to another.
 */
function _gc_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = gc_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		foreach ( $items as $item ) {
		...
		gc_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function _gc_delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '_gc_delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			gc_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			gc_trash_post( $stub_post_id );
...
function _gc_menus_changed() {
	...
	$mapped_nav_menu_locations = gc_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function gc_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
