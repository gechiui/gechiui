...
<channel>
	...
	if ( is_singular() ) {
		...
		printf( ent2ncr( __( '《%s》的评论' ) ), get_the_title_rss() );
	} elseif ( is_search() ) {
		/* translators: Comments feed title. 1: Site title, 2: Search query. */
		printf( ent2ncr( __( '%1$s的评论搜索“%2$s”' ) ), get_bloginfo_rss( 'name' ), get_search_query() );
	} else {
		/* translators: Comments feed title. %s: Site title. */
		printf( ent2ncr( __( '“%s”的评论' ) ), get_gc_title_rss() );
	}
	?>
	</title>
	...
	<sy:updatePeriod>
	<?php
		/** This filter is documented in gc-includes/feed-rss2.php */
	...
	<item>
		...
		if ( ! is_singular() ) {
			...
			/** This filter is documented in gc-includes/feed.php */
			$title = apply_filters( 'the_title_rss', $title );
			/* translators: Individual comment title. 1: Post title, 2: Comment author name. */
			printf( ent2ncr( __( '%2$s对《%1$s》的评论' ) ), $title, get_comment_author_rss() );
		} else {
			/* translators: Comment author title. %s: Comment author name. */
			printf( ent2ncr( __( '评论者：%s' ) ), get_comment_author_rss() );
		}
		?>
		</title>
		...
		<?php if ( post_password_required( $comment_post ) ) : ?>
			<description><?php echo ent2ncr( __( '评论受保护：要查看请输入密码。' ) ); ?></description>
