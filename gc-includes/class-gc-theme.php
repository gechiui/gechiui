 ...
 *
 */
final class GC_Theme implements ArrayAccess {
	...
	/**
	 * Absolute path to the theme root, usually gc-content/themes
	 *
	 * @var string
	 */
	private $theme_root;
	...
	/**
	 * URL to the theme root, usually an absolute URL to gc-content/themes
	 *
	 * @var string
	 */
	 ...
	 *
	 * Default is false. Can be set with the {@see 'gc_cache_themes_persistently'} filter.
	 *
	 * @var bool
	 */
	 ...
	 * @param GC_Theme|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'gc_cache_themes_persistently', false, 'GC_Theme' );
			if ( self::$persistently_cache ) {
				gc_cache_add_global_groups( 'themes' );
				if ( is_int( self::$persistently_cache ) ) {
					self::$cache_expiration = self::$persistently_cache;
				}
			} else {
				gc_cache_add_non_persistent_groups( 'themes' );
			}
		}
		...
		if ( ! in_array( $theme_root, (array) $gc_theme_directories, true )
			&& in_array( dirname( $theme_root ), (array) $gc_theme_directories, true )
		) {
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) ) {
				...
				$this->errors = new GC_Error( 'theme_no_stylesheet', __( '缺少样式表。' ) );
			}
			...
			if ( ! file_exists( $this->theme_root ) ) { // Don't cache this one.
				$this->errors->add( 'theme_root_missing', __( '错误：主题目录是个空目录或主题目录不存在。请检查主题是否安装妥当。' ) );
			}
			return;
		} elseif ( ! is_readable( $this->theme_root . '/' . $theme_file ) ) {
			$this->headers['Name'] = $this->stylesheet;
			$this->errors          = new GC_Error( 'theme_stylesheet_not_readable', __( '样式表文件不可读。' ) );
			$this->template        = $this->stylesheet;
			$this->cache_add(
				'theme',
				array(
			...
			// Properly identify default themes that are inside a directory within gc-content/themes.
		...
		if ( ! $this->template ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet . '/index.php' ) ) {
				$error_message = sprintf(
					...
					__( '模板丢失。独立主题需要有%1$s模板文件，<a href="%2$s">子主题</a>必须在%3$s样式表中包含模板标头。' ),
					'<code>index.php</code>',
					__( 'https://developer.gechiui.com/themes/advanced-topics/child-themes/' ),
					'<code>style.css</code>'
				);
				$this->errors = new GC_Error( 'theme_no_index', $error_message );
				$this->cache_add(
					'theme',
					array(
						'headers'    => $this->headers,
		...
		if ( ! is_array( $cache ) && $this->template != $this->stylesheet && ! file_exists( $this->theme_root . '/' . $this->template . '/index.php' ) ) {
			...
			// gc-content/themes/directory-of-themes/*
			...
			if ( '.' !== $parent_dir && file_exists( $this->theme_root . '/' . $parent_dir . '/' . $this->template . '/index.php' ) ) {
				...
				$this->parent = new GC_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template != $this->stylesheet ) {
			...
			if ( $_child instanceof GC_Theme && $_child->template == $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new GC_Error(
			...
			$this->parent = new GC_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}

		if ( gc_paused_themes()->get( $this->stylesheet ) && ( ! is_gc_error( $this->errors ) || ! isset( $this->errors->errors['theme_paused'] ) ) ) {
			$this->errors = new GC_Error( 'theme_paused', __( '此主题未能正确加载且已于管理后台中禁用。' ) );
		}
	 ...
	 *
	 *
	 * @return GC_Error|false GC_Error if there are errors, or false.
	 */
	public function errors() {
		return is_gc_error( $this->errors ) ? $this->errors : false;
	}
	 ...
	 *
	 *
	 * @return GC_Theme|false Parent theme, or false if the active theme is not a child theme.
	 */
	 ...
	 * @return bool Return value from gc_cache_add()
	 */
	private function cache_add( $key, $data ) {
		return gc_cache_add( $key . '-' . $this->cache_hash, $data, 'themes', self::$cache_expiration );
	}
	...
	private function cache_get( $key ) {
		return gc_cache_get( $key . '-' . $this->cache_hash, 'themes' );
	}
	...
	public function cache_delete() {
		foreach ( array( 'theme', 'screenshot', 'headers', 'post_templates' ) as $key ) {
			gc_cache_delete( $key . '-' . $this->cache_hash, 'themes' );
		}
		$this->template          = null;
	 ...
	 *                       'ThemeURI', 'AuthorURI', 'Status', 'Tags', 'RequiresGC', 'RequiresPHP'.
	...
	private function sanitize_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Name':
				...
				$value = gc_kses( $value, $header_tags );
				break;
			case 'Author':
			...
			case 'description':
				...
				$value = gc_kses( $value, $header_tags_with_a );
				break;
			case 'ThemeURI':
			case 'AuthorURI':
			...
			case 'Tags':
				...
				if ( ! isset( $tags_list ) ) {
					$tags_list = array(
						...
						'white'             => __( '白色' ),
						'yellow'            => __( '黄色' ),
						'dark'              => _x( '深色', 'color scheme' ),
						'light'             => _x( '浅色', 'color scheme' ),
						'fixed-layout'      => __( '固定布局' ),
						'fluid-layout'      => __( '流动布局' ),
						'responsive-layout' => __( '响应式布局' ),
						'blavatar'          => __( 'Blavatar' ),
						'photoblogging'     => __( '摄影博客' ),
						'seasonal'          => __( '四季' ),
					);
	 ...
	 *
	 * This is typically the absolute path to gc-content/themes.
	 *
	 *
	 ...
	 *
	 * This is typically the absolute URL to gc-content/themes. This forms the basis
	 * for all other URLs returned by GC_Theme, so we pass it to the public function
	...
	public function get_post_templates() {
		...
		if ( ! is_array( $post_templates ) ) {
			...
			if ( current_theme_supports( 'block-templates' ) ) {
				$block_templates = get_block_templates( array(), 'gc_template' );
	...
	public function get_page_templates( $post = null, $post_type = 'page' ) {
		 ...
		 * @param GC_Theme     $theme          The theme object.
		 * @param GC_Post|null $post           The post being edited, provided for context, or null.
		 * @param string       $post_type      要获取模板的文章类型。
		 */
	...
	/**
	 * Determines the latest GeChiUI default theme that is installed.
	 *
	 * This hits the filesystem.
	 *
	 *
	 * @return GC_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	...
	public static function get_allowed_on_site( $blog_id = null ) {
		...
		/** This filter is documented in gc-includes/class-gc-theme.php */
		return (array) apply_filters( 'site_allowed_themes', $allowed_themes[ $blog_id ], $blog_id );
	}
	 ...
	 *
	 *
	 * @param GC_Theme[] $themes Array of theme objects to sort (passed by reference).
	 */
	public static function sort_by_name( &$themes ) {
		if ( 0 === strpos( get_user_locale(), 'en_' ) ) {
			uasort( $themes, array( 'GC_Theme', '_name_sort' ) );
		} else {
			foreach ( $themes as $key => $theme ) {
				$theme->translate_header( 'Name', $theme->headers['Name'] );
			}
			uasort( $themes, array( 'GC_Theme', '_name_sort_i18n' ) );
		}
	}
	 ...
	 *
	 *
	 * @param GC_Theme $a First theme.
	 * @param GC_Theme $b Second theme.
