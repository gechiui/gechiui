 ...
 * @return bool|GC_Error True if event successfully scheduled. False or GC_Error on failure.
 */
function gc_schedule_single_event( $timestamp, $hook, $args = array(), $gc_error = false ) {
	// Make sure timestamp is a positive integer.
	if ( ! is_numeric( $timestamp ) || $timestamp <= 0 ) {
		if ( $gc_error ) {
			return new GC_Error(
				'invalid_timestamp',
				__( '事件时间戳必须是一个有效的UNIX时间戳。' )
			);
		}
	 ...
	 * have this set to a recurrence from gc_get_schedules(). Recurring
	 * events also have the integer recurrence interval set as `$event->interval`.
	 *
	 * For plugins replacing gc-cron, it is recommended you check for an
	 ...
	 */
	$pre = apply_filters( 'pre_schedule_event', null, $event, $gc_error );

	if ( null !== $pre ) {
		if ( $gc_error && false === $pre ) {
			return new GC_Error(
	...
	return _set_cron_array( $crons, $gc_error );
}

/**
 * Schedules a recurring event.
 *
 * Schedules a hook which will be triggered by GeChiUI at the specified interval.
 * The action will trigger when someone visits your GeChiUI site if the scheduled
 * time has passed.
 *
 ...
 *                           See gc_get_schedules() for accepted values.
 * @param string $hook       Action hook to execute when the event is run.
 * @param array  $args       Optional. Array containing arguments to pass to the
 *                           hook's callback function. Each value in the array
 *                           is passed to the callback as an individual parameter.
 ...
 * @return bool|GC_Error True if event successfully scheduled. False or GC_Error on failure.
 */
function gc_schedule_event( $timestamp, $recurrence, $hook, $args = array(), $gc_error = false ) {
	// Make sure timestamp is a positive integer.
	if ( ! is_numeric( $timestamp ) || $timestamp <= 0 ) {
		if ( $gc_error ) {
			return new GC_Error(
 ...
 *                           The array keys are ignored. Default empty array.
 * @param bool   $gc_error   Optional. Whether to return a GC_Error on failure. Default false.
 * @return bool|GC_Error True if event successfully rescheduled. False or GC_Error on failure.
 */
function gc_reschedule_event( $timestamp, $recurrence, $hook, $args = array(), $gc_error = false ) {
	// Make sure timestamp is a positive integer.
	if ( ! is_numeric( $timestamp ) || $timestamp <= 0 ) {
		if ( $gc_error ) {
			return new GC_Error(
	...
	if ( 0 === $interval ) {
		$scheduled_event = gc_get_scheduled_event( $hook, $args, $timestamp );
	 ...
	 * @param bool               $gc_error Whether to return a GC_Error on failure.
	 */
	$pre = apply_filters( 'pre_reschedule_event', null, $event, $gc_error );
	...
	return gc_schedule_event( $timestamp, $recurrence, $hook, $args, $gc_error );
}
 ...
 *                          Default empty array.
 * @param bool   $gc_error  Optional. Whether to return a GC_Error on failure. Default false.
 * @return bool|GC_Error True if event successfully unscheduled. False or GC_Error on failure.
 */
function gc_unschedule_event( $timestamp, $hook, $args = array(), $gc_error = false ) {
	// Make sure timestamp is a positive integer.
	if ( ! is_numeric( $timestamp ) || $timestamp <= 0 ) {
		if ( $gc_error ) {
			return new GC_Error(
	 ...
	 * @param bool               $gc_error  Whether to return a GC_Error on failure.
	 */
	$pre = apply_filters( 'pre_unschedule_event', null, $timestamp, $hook, $args, $gc_error );
 ...
 *                         Default empty array.
 * @param bool   $gc_error Optional. Whether to return a GC_Error on failure. Default false.
 * @return int|false|GC_Error On success an integer indicating number of events unscheduled (0 indicates no
 *                            events were registered with the hook and arguments combination), false or GC_Error
...
function gc_clear_scheduled_hook( $hook, $args = array(), $gc_error = false ) {
	 ...
	 * @param bool                    $gc_error Whether to return a GC_Error on failure.
	 */
	$pre = apply_filters( 'pre_clear_scheduled_hook', null, $hook, $args, $gc_error );
	...
	/*
	 * This logic duplicates gc_next_scheduled().
	 * It's required due to a scenario where gc_unschedule_event() fails due to update_option() failing,
	 * and, gc_next_scheduled() returns the same schedule in an infinite loop.
	 */
	$crons = _get_cron_array();
	if ( empty( $crons ) ) {
		return 0;
	...
	foreach ( $crons as $timestamp => $cron ) {
		if ( isset( $cron[ $hook ][ $key ] ) ) {
			$results[] = gc_unschedule_event( $timestamp, $hook, $args, true );
		}
	}

	$errors = array_filter( $results, 'is_gc_error' );
	$error  = new GC_Error();
 ...
 * @param bool   $gc_error Optional. Whether to return a GC_Error on failure. Default false.
 * @return int|false|GC_Error On success an integer indicating number of events unscheduled (0 indicates no
 *                            events were registered on the hook), false or GC_Error if unscheduling fails.
 */
function gc_unschedule_hook( $hook, $gc_error = false ) {
	/**
	 ...
	 * @param bool                    $gc_error Whether to return a GC_Error on failure.
	 */
	$pre = apply_filters( 'pre_unschedule_hook', null, $hook, $gc_error );
	...
	$set = _set_cron_array( $crons, $gc_error );
 ...
 */
function gc_next_scheduled( $hook, $args = array() ) {
	$next_event = gc_get_scheduled_event( $hook, $args );
	if ( ! $next_event ) {
		return false;
	}
...
function spawn_cron( $gmt_time = 0 ) {
	...
	if ( defined( 'DOING_CRON' ) || isset( $_GET['doing_gc_cron'] ) ) {
		return false;
	}
	...
	if ( $lock + GC_CRON_LOCK_TIMEOUT > $gmt_time ) {
		return false;
	}
	...
	if ( defined( 'ALTERNATE_GC_CRON' ) && ALTERNATE_GC_CRON ) {
		if ( 'GET' !== $_SERVER['REQUEST_METHOD'] || defined( 'DOING_AJAX' ) || defined( 'XMLRPC_REQUEST' ) ) {
			return false;
		}

		$doing_gc_cron = sprintf( '%.22F', $gmt_time );
		set_transient( 'doing_cron', $doing_gc_cron );

		ob_start();
		gc_redirect( add_query_arg( 'doing_gc_cron', $doing_gc_cron, gc_unslash( $_SERVER['REQUEST_URI'] ) ) );
		echo ' ';
	 ...
	 *     }
	 * }
	 * @param string $doing_gc_cron The unix timestamp of the cron lock.
	 */
	$cron_request = apply_filters(
		'cron_request',
		array(
			'url'  => add_query_arg( 'doing_gc_cron', $doing_gc_cron, site_url( 'gc-cron.php' ) ),
			'key'  => $doing_gc_cron,
			'args' => array(
				'timeout'   => 0.01,
				'blocking'  => false,
	...
	$result = gc_remote_post( $cron_request['url'], $cron_request['args'] );
	return ! is_gc_error( $result );
}
 ...
 *                       void if the function registered _gc_cron() to run on the action.
 */
function gc_cron() {
	...
	add_action( 'gc_loaded', '_gc_cron', 20 );
}
 ...
 */
function gc_get_schedule( $hook, $args = array() ) {
	$schedule = false;
	$event    = gc_get_scheduled_event( $hook, $args );
 ...
 */
function gc_get_ready_cron_jobs() {
	/**
 ...
 * @param bool    $gc_error Optional. Whether to return a GC_Error on failure. Default false.
 * @return bool|GC_Error True if cron array updated. False or GC_Error on failure.
 */
function _set_cron_array( $cron, $gc_error = false ) {
	if ( ! is_array( $cron ) ) {
		$cron = array();
	}
