...
function gc_authenticate_username_password( $user, $username, $password ) {
	...
	if ( ! $user ) {
		return new GC_Error(
			...
			sprintf(
				...
				__( '<strong>错误：</strong>用户名<strong>%s</strong>未在本系统注册。如果您不确定您的用户名，请改用电子邮箱进行尝试。' ),
				$username
			)
		);
	}
	...
	if ( ! gc_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new GC_Error(
			...
			sprintf(
				/* translators: %s: User name. */
				__( '<strong>错误：</strong>为用户名 %s 指定的密码不正确。' ),
	...
	if ( ! gc_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new GC_Error(
			...
			sprintf(
				...
				__( '<strong>错误：</strong>您为电子邮箱 %s 输入的密码不正确。' ),
				'<strong>' . $email . '</strong>'
			) .
			' <a href="' . gc_lostpassword_url() . '">' .
			__( '忘记密码？' ) .
...
/**
 * Authenticates the user using the GeChiUI auth cookie.
 *
 * @global string $auth_secure_cookie
 *
 * @param GC_User|GC_Error|null $user     GC_User or GC_Error object from a previous callback. Default null.
...
function gc_authenticate_spam_check( $user ) {
	if ( $user instanceof GC_User && is_multisite() ) {
		...
		if ( $spammed ) {
			return new GC_Error( 'spammer_account', __( '<strong>错误：</strong>您的账户已被标记为垃圾评论账户。' ) );
		}
	}
	return $user;
}
...
function gc_list_users( $args = array() ) {
	 ...
	 * @param array $parsed_args The arguments passed to gc_list_users() combined with the defaults.
	 */
	$query_args = apply_filters( 'gc_list_users_args', $query_args, $parsed_args );
...
function get_blogs_of_user( $user_id, $all = false ) {
	...
	foreach ( $keys as $key ) {
		...
		if ( $gcdb->base_prefix && ! str_starts_with( $key, $gcdb->base_prefix ) ) {
			continue;
		}
 ...
 * The number of users with no role is now included in the `none` element. The `$site_id` parameter was added to support multisite.
 *
 * @global gcdb $gcdb GeChiUI database abstraction object.
 *
 ...
 *
 * @since MU (3.0.0) The `$network_id` parameter has been added.
 * @since 6.0.0 Moved to gc-includes/user.php.
 *
 ...
 */
function gc_maybe_update_user_counts( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$is_small_network = ! gc_is_large_user_count( $network_id );
	/** This filter is documented in gc-includes/ms-functions.php */
	if ( ! apply_filters( 'enable_live_network_counts', $is_small_network, 'users' ) ) {
		return false;
	}

	return gc_update_user_counts( $network_id );
}
 ...
 * @return bool Whether the update was successful.
 */
function gc_update_user_counts( $network_id = null ) {
	global $gcdb;
	...
	$query = "SELECT COUNT(ID) as c FROM $gcdb->users";
	if ( is_multisite() ) {
		$query .= " WHERE spam = '0' AND deleted = '0'";
	}

	$count = $gcdb->get_var( $query );
 ...
 * @since 6.0.0
 */
function gc_schedule_update_user_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! gc_next_scheduled( 'gc_update_user_counts' ) && ! gc_installing() ) {
		gc_schedule_event( time(), 'twicedaily', 'gc_update_user_counts' );
	}
}
 ...
 */
function gc_is_large_user_count( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	 ...
	 */
	return apply_filters( 'gc_is_large_user_count', $count > 10000, $count, $network_id );
}
 ...
 * Added the 'display_name_with_login' value for 'show'. Added the `$role`, `$role__in`, and `$role__not_in` parameters.
 *
 ...
 * 'clean_user_cache' action was added.
...
function clean_user_cache( $user ) {
	...
	gc_cache_delete( $user->ID, 'user_meta' );
	gc_cache_set_users_last_changed();
 ...
 *
 * @since 2.0.1 Empty sanitized usernames are now considered invalid.
 *
 ...
 * The `aim`, `jabber`, and `yim` fields were removed as default user contact
 *              methods for new installations. See gc_get_user_contact_methods(). The `locale` field can be passed to `$userdata`.
...
function gc_insert_user( $userdata ) {
	...
	if ( ! empty( $userdata['spam'] ) && ! is_multisite() ) {
		return new GC_Error( 'no_spam', __( '抱歉，仅能在多系统中标记用户为垃圾。' ) );
	}
...
###SITEURL###'
	 ...
	 */
	do_action( 'gc_update_user', $user_id, $userdata, $userdata_raw );
 ...
 *                      be created.
 */
function gc_create_user( $username, $password, $email = '' ) {
...
function get_password_reset_key( $user ) {
	...
	$password_reset_allowed = gc_is_password_reset_allowed_for_user( $user );
	if ( ! $password_reset_allowed ) {
		return new GC_Error( 'no_password_reset', __( '不能重设该用户的密码' ) );
	} elseif ( is_gc_error( $password_reset_allowed ) ) {
...
function retrieve_password( $user_login = null ) {
	...
	$user_login = trim( gc_unslash( $user_login ) );
	...
	$message .= sprintf( __( '系统名称：%s' ), $site_name ) . "\r\n\r\n";
	...
	if ( ! gc_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			...
			sprintf(
				...
				__( '<strong>错误：</strong>邮件未能发送。您的系统可能没有正确配置邮件发送。<a href="%s">获取重设您密码的帮助</a>。' ),
	...
	if ( ! $user_id || is_gc_error( $user_id ) ) {
		$errors->add(
			...
			sprintf(
				...
				__( '<strong>错误：</strong>无法完成您的注册请求&hellip;请联系<a href="mailto:%s">系统管理员</a>！' ),
 ...
 * Converted the `$notify` parameter to accept 'user' for sending
...
function new_user_email_admin_notice() {
	...
	if ( 'profile.php' === $pagenow && isset( $_GET['updated'] ) ) {
		...
		if ( $email ) {
			...
			echo setting_error( sprintf( __( '您的电子邮箱尚未更新，请在%s的收件箱中检查确认邮件。' ), '<code>' . esc_html( $email['newemail'] ) . '</code>' ), 'primary' );
		}
	}
}
 ...
 *
 * @since 5.4.0 Added '社区活动位置' group to the export data.
 * @since 5.4.0 Added '会话令牌' group to the export data.
 *
 ...
 *
 * By default all users can use Application Passwords. Use {@see 'gc_is_appkeys_available_for_user'}
 ...
 * @global gcdb $gcdb GeChiUI database abstraction object.
 */
function gc_register_persisted_preferences_meta() {
	/*
 ...
 * @since 6.3.0
 */
function gc_cache_set_users_last_changed() {
	gc_cache_set_last_changed( 'users' );
}
 ...
 *
 * @param int|GC_User $user The user to check.
 * @return bool|GC_Error True if allowed, false or GC_Error otherwise.
 */
function gc_is_password_reset_allowed_for_user( $user ) {
	if ( ! is_object( $user ) ) {
		$user = get_userdata( $user );
	}
