<?php
/**
 * GeChiUI Customize Manager classes
 *
 * @package GeChiUI
...
final class GC_Customize_Manager {
	 ...
	 */
	public function gc_die_handler() {
		_deprecated_function( __METHOD__, '4.7.0' );

		if ( $this->doing_ajax() || isset( $_POST['customized'] ) ) {
			return '_ajax_gc_die_handler';
		}

		return '_default_gc_die_handler';
	}
	...
	public function setup_theme() {
		...
		if ( isset( $this->_changeset_uuid ) && false !== $this->_changeset_uuid && ! gc_is_uuid( $this->_changeset_uuid ) ) {
			$this->gc_die( -1, __( '无效的变更集UUID' ) );
		}

		/*
		 * Clear incoming post data if the user lacks a CSRF token (nonce). Note that the customizer
		 * application will inject the customize_preview_nonce query parameter into all Ajax requests.
		 * For similar behavior elsewhere in GeChiUI, see rest_cookie_check_errors() which logs out
		...
		if ( ! current_user_can( 'customize' ) && ! $this->changeset_post_id() ) {
			$this->gc_die( $this->messenger_channel ? 0 : -1, __( '不存在的变更集UUID。' ) );
		}
		...
		if ( $this->is_theme_active() ) {
			...
			if ( $this->theme()->errors() ) {
				$this->gc_die( -1, $this->theme()->errors()->get_error_message() );
			}

			// If the theme isn't allowed per multisite settings, bail.
			if ( ! $this->theme()->is_allowed() ) {
				$this->gc_die( -1, __( '请求的主题不存在。' ) );
			}
		}
	...
	public function after_setup_theme() {
		...
		if ( ! $doing_ajax_or_is_customized && ! validate_current_theme() ) {
			gc_redirect( 'themes.php?broken=true' );
			exit;
		}
	}
	...
	public function start_previewing_theme() {
		...
		if ( ! $this->is_theme_active() ) {
			...
			// @link: https://core.trac.gechiui.com/ticket/20027
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of GeChiUI users,
		 ...
		 * @param GC_Customize_Manager $gc_customize    Manager instance.
		 */
	 ...
	 * @return int
	 */
	public function gc_redirect_status( $status ) {
	...
	public function find_changeset_post_id( $uuid ) {
		...
		if ( ! empty( $changeset_post_query->posts ) ) {
			...
			gc_cache_set( $uuid, $changeset_post_id, $cache_group );
			return $changeset_post_id;
		}
	 ...
	 * }
	 * @return GC_Post[] Auto-draft changesets.
	 */
	...
	public function changeset_data() {
		...
		if ( ! $changeset_post_id ) {
			...
			if ( $this->autosaved() && is_user_logged_in() ) {
				$autosave_post = gc_get_post_autosave( $changeset_post_id, get_current_user_id() );
				if ( $autosave_post ) {
	 ...
	 * 4. Any programmatically-set post values via `GC_Customize_Manager::set_post_value()`.
	 *
	...
	public function unsanitized_post_values( $args = array() ) {
		...
		if ( ! $this->is_theme_active() ) {
			...
			if ( isset( $stashed_theme_mods[ $stylesheet ] ) ) {
				$values = array_merge( $values, gc_list_pluck( $stashed_theme_mods[ $stylesheet ], 'value' ) );
			}
		}
		...
		if ( ! $args['exclude_post_data'] ) {
			if ( ! isset( $this->_post_values ) ) {
				if ( isset( $_POST['customized'] ) ) {
					$post_values = json_decode( gc_unslash( $_POST['customized'] ), true );
				} else {
					$post_values = array();
				}
	...
	public function add_state_query_params( $url ) {
		$parsed_original_url = gc_parse_url( $url );
		$is_allowed          = false;
		foreach ( $this->get_allowed_urls() as $allowed_url ) {
			$parsed_allowed_url = gc_parse_url( $allowed_url );
			$is_allowed         = (
	...
	public function customize_preview_settings() {
		...
		foreach ( $allowed_urls as $allowed_url ) {
			$parsed = gc_parse_url( $allowed_url );
		...
		$l10n            = array(
			...
			'formUnpreviewable' => __( '此表单不能实时预览。' ),
		);
		if ( $switched_locale ) {
		...
		<script type="text/javascript">
			...
			_gcCustomizeSettings.values = {};
			(function( v ) {
				<?php
				/*
				 * Serialize settings separately from the initial _gcCustomizeSettings
				...
				foreach ( $this->settings as $id => $setting ) {
					if ( $setting->check_capabilities() ) {
						printf(
							"v[%s] = %s;\n",
							gc_json_encode( $id ),
							gc_json_encode( $setting->js_value() )
						);
					}
				}
				?>
	 ...
	 * }
	 * @return array Mapping of setting IDs to return value of validate method calls, either `true` or `GC_Error`.
	 */
		...
		foreach ( $setting_values as $setting_id => $unsanitized_value ) {
			...
			if ( ! $setting ) {
				if ( $options['validate_existence'] ) {
					$validities[ $setting_id ] = new GC_Error( 'unrecognized', __( '设置不存在或无法识别。' ) );
				}
				continue;
			}
			if ( $options['validate_capability'] && ! current_user_can( $setting->capability ) ) {
				$validity = new GC_Error( 'unauthorized', __( '因为权限不足，未授权修改设置。' ) );
			} else {
			...
			if ( false === $validity ) {
				$validity = new GC_Error( 'invalid_value', __( '无效值。' ) );
			}
	 ...
	 * }
	 *
	 * @return array|GC_Error Returns array on success and GC_Error with array data on error.
	 */
	public function save_changeset_post( $args = array() ) {
		...
		if ( $changeset_post_id ) {
			...
			if ( 'publish' === $existing_status || 'trash' === $existing_status ) {
				return new GC_Error(
					...
					__( '前一个变更集已被发布。请尝试重新保存您当前的变更。' ),
					array(
						'next_changeset_uuid' => gc_generate_uuid4(),
					)
				);
			}

			$existing_changeset_data = $this->get_changeset_post_data( $changeset_post_id );
			if ( is_gc_error( $existing_changeset_data ) ) {
				return $existing_changeset_data;
			}
		}

		// Fail if attempting to publish but publish hook is missing.
		if ( 'publish' === $args['status'] && false === has_action( 'transition_post_status', '_gc_customize_publish_changeset' ) ) {
			return new GC_Error( 'missing_publish_callback' );
		}
		...
		if ( $args['date_gmt'] ) {
			...
			if ( ! $is_future_dated ) {
				return new GC_Error( 'not_future_date', __( '您必须提供一个将来的日期来计划发布。' ) ); // Only future dates are allowed.
			}

			if ( ! $this->is_theme_active() && ( 'future' === $args['status'] || $is_future_dated ) ) {
				return new GC_Error( 'cannot_schedule_theme_switches' ); // This should be allowed in the future, when theme is a regular setting.
			}
			...
			if ( $will_remain_auto_draft ) {
				return new GC_Error( 'cannot_supply_date_for_auto_draft_changeset' );
			}
			...
			if ( mysql2date( 'U', $changeset_post->post_date_gmt, false ) <= mysql2date( 'U', $now, false ) ) {
				return new GC_Error( 'not_future_date', __( '您必须提供一个将来的日期来计划发布。' ) );
			}
		}
		...
		// Validate autosave param. See _gc_post_revision_fields() for why these fields are disallowed.
		if ( $args['autosave'] ) {
			if ( $args['date_gmt'] ) {
				return new GC_Error( 'illegal_autosave_with_date_gmt' );
			} elseif ( $args['status'] ) {
				return new GC_Error( 'illegal_autosave_with_status' );
			} elseif ( $args['user_id'] && get_current_user_id() !== $args['user_id'] ) {
				return new GC_Error( 'illegal_autosave_with_non_current_user' );
			}
		}

		// The request was made via gc.customize.previewer.save().
		...
		$invalid_setting_count = count( array_filter( $setting_validities, 'is_gc_error' ) );
		...
		if ( $update_transactionally && $invalid_setting_count > 0 ) {
			$response = array(
				...
				'message'            => sprintf( _n( '由于 %s 个设置无效，无法保存。', '由于 %s 个设置无效，无法保存。', $invalid_setting_count ), number_format_i18n( $invalid_setting_count ) ),
			);
			return new GC_Error( 'transaction_fail', '', $response );
		}
		...
		foreach ( $args['data'] as $setting_id => $setting_params ) {
			...
			if ( isset( $setting_validities[ $setting_id ] ) && is_gc_error( $setting_validities[ $setting_id ] ) ) {
				continue;
			}
		...
		$filter_context = array(
			...
			'previous_data' => is_gc_error( $original_changeset_data ) ? array() : $original_changeset_data,
			'manager'       => $this,
		);
		 ...
		 *     @type GC_Customize_Manager $manager       Manager instance.
		 * }
		 */
		...
		// Gather the data for gc_insert_post()/gc_update_post().
		$post_array = array(
			// JSON_UNESCAPED_SLASHES is only to improve readability as slashes needn't be escaped in storage.
			'post_content' => gc_json_encode( $data, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ),
		);
		if ( $args['title'] ) {
		...
		add_filter( 'gc_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ), 5, 3 );

		/*
		 * Update the changeset post. The publish_customize_changeset action will cause the settings in the
		 * changeset to be saved via GC_Customize_Setting::save(). Updating a post with publish status will
		 * trigger GC_Customize_Manager::publish_changeset_values().
		 */
		add_filter( 'gc_insert_post_data', array( $this, 'preserve_insert_changeset_post_content' ), 5, 3 );
		if ( $changeset_post_id ) {
			if ( $args['autosave'] && 'auto-draft' !== get_post_status( $changeset_post_id ) ) {
				// See _gc_translate_postdata() for why this is required as it will use the edit_post meta capability.
				add_filter( 'map_meta_cap', array( $this, 'grant_edit_post_capability_for_changeset' ), 10, 4 );

				$post_array['post_ID']   = $post_array['ID'];
				$post_array['post_type'] = 'customize_changeset';

				$r = gc_create_post_autosave( gc_slash( $post_array ) );
				...
				$r = gc_update_post( gc_slash( $post_array ), true );

				// Delete autosave revision for user when the changeset is updated.
				if ( ! empty( $args['user_id'] ) ) {
					$autosave_draft = gc_get_post_autosave( $changeset_post_id, $args['user_id'] );
					if ( $autosave_draft ) {
						gc_delete_post( $autosave_draft->ID, true );
					}
				}
			}
		} else {
			$r = gc_insert_post( gc_slash( $post_array ), true );
			if ( ! is_gc_error( $r ) ) {
				$this->_changeset_post_id = $r; // Update cached post ID for the loaded changeset.
			}
		}
		remove_filter( 'gc_insert_post_data', array( $this, 'preserve_insert_changeset_post_content' ), 5 );

		$this->_changeset_data = null; // Reset so GC_Customize_Manager::changeset_data() will re-populate with updated contents.

		remove_filter( 'gc_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ) );
		...
		if ( is_gc_error( $r ) ) {
			...
			return new GC_Error( 'changeset_post_save_failure', '', $response );
		}
	 ...
	 *
	 * Note that GC_Customize_Manager::validate_setting_values() have already
	 ...
	 * GC_Customize_Setting::sanitize() method when being read out of the
	 * changeset, via GC_Customize_Manager::post_value(), and this sanitized
	 * value will also be sent into GC_Customize_Setting::update() for
	 ...
	 * @param array $unsanitized_postarr An array of slashed yet *unsanitized* and unprocessed post data as originally passed to gc_insert_post().
	 ...
	 *
	 * @param int|GC_Post $post The changeset post.
	 * @return mixed A GC_Post object for the trashed post or an empty value on failure.
	 */
			...
			if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->delete_post, $changeset_post_id ) ) {
				gc_send_json_error(
					array(
						...
						'message' => __( '无法将更改移至回收站。' ),
					)
				);
			}

			$lock_user = (int) gc_check_post_lock( $changeset_post_id );

			if ( $lock_user && get_current_user_id() !== $lock_user ) {
				gc_send_json_error(
					array(
						'code'     => 'changeset_locked',
						'message'  => __( '变更集正在被其他用户编辑。' ),
	 ...
	 * required in core for `gc_create_post_autosave()` because it will call
	 * `_gc_translate_postdata()` which in turn will check if a user can 'edit_post', but the
	...
	public function check_changeset_lock_with_heartbeat( $response, $data, $screen_id ) {
		...
		if (
			...
			$lock_user_id = gc_check_post_lock( $changeset_post_id );
	...
	public function handle_override_changeset_lock_request() {
		if ( ! $this->is_preview() ) {
			gc_send_json_error( 'not_preview', 400 );
		}
		...
		gc_send_json_success( 'changeset_taken_over' );
	}
	 ...
	 * called directly and instead `gc_publish_post()` should be used.
	 *
	 ...
	 * @return true|GC_Error True or error info.
	 */
	public function _publish_changeset_values( $changeset_post_id ) {
		...
		if ( is_gc_error( $publishing_changeset_data ) ) {
		 ...
		 * on the $gc_customize object passed to hooks during the save logic.
		 */
		...
		foreach ( $changeset_setting_ids as $setting_id ) {
			...
			if ( $setting ) {
				 ...
				 * when a scheduled changeset publishes via GC Cron.
				 */
				if ( isset( $setting_user_ids[ $setting_id ] ) ) {
					gc_set_current_user( $setting_user_ids[ $setting_id ] );
				} else {
					gc_set_current_user( $original_user_id );
				}
	...
	public function handle_dismiss_autosave_or_lock_request() {
		...
		if ( ! is_user_logged_in() ) {
			gc_send_json_error( 'unauthenticated', 401 );
		}
		...
		if ( ! check_ajax_referer( 'customize_dismiss_autosave_or_lock', 'nonce', false ) ) {
			gc_send_json_error( 'invalid_nonce', 403 );
		}
		...
		if ( $dismiss_autosave ) {
			if ( empty( $changeset_post_id ) || 'auto-draft' === get_post_status( $changeset_post_id ) ) {
				...
				if ( $dismissed > 0 ) {
					gc_send_json_success( 'auto_draft_dismissed' );
				} else {
					gc_send_json_error( 'no_auto_draft_to_delete', 404 );
				}
			} else {
				$revision = gc_get_post_autosave( $changeset_post_id, get_current_user_id() );

				if ( $revision ) {
					if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->delete_post, $changeset_post_id ) ) {
						gc_send_json_error( 'cannot_delete_autosave_revision', 403 );
					}

					if ( ! gc_delete_post( $revision->ID, true ) ) {
						gc_send_json_error( 'autosave_revision_deletion_failure', 500 );
					} else {
						gc_send_json_success( 'autosave_revision_deleted' );
					}
				} else {
					gc_send_json_error( 'no_autosave_revision_to_delete', 404 );
				}
			}
		}

		gc_send_json_error( 'unknown_error', 500 );
	}
	 ...
	 *
	 * @param GC_Customize_Setting|string $id   Customize Setting object, or ID.
	 * @param array                       $args Optional. Array of properties for the new Setting object.
	 *                                          See GC_Customize_Setting::__construct() for information
	 *                                          on accepted arguments. Default empty array.
	 * @return GC_Customize_Setting The instance of the setting that was added.
	 */
	public function add_setting( $id, $args = array() ) {
		if ( $id instanceof GC_Customize_Setting ) {
			$setting = $id;
		} else {
	 ...
	 * on the front end and have been sent to GeChiUI in `$_POST['customized']`. When GC
	 ...
	 *
	 * @param GC_Customize_Panel|string $id   Customize Panel object, or ID.
	 * @param array                     $args Optional. Array of properties for the new Panel object.
	 *                                        See GC_Customize_Panel::__construct() for information
	 *                                        on accepted arguments. Default empty array.
	 * @return GC_Customize_Panel The instance of the panel that was added.
	 */
		...
		if ( in_array( $id, $this->components, true ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					__( '手工移除%1$s会导致PHP警告。请改用%2$s过滤器。' ),
					$id,
					sprintf(
						'<a href="%1$s">%2$s</a>',
						esc_url( 'https://developer.gechiui.com/reference/hooks/customize_loaded_components/' ),
	 ...
	 *
	 * @param string $panel Name of a custom panel which is a subclass of GC_Customize_Panel.
	 */
	 ...
	 *
	 * @param GC_Customize_Section|string $id   Customize Section object, or ID.
	 * @param array                       $args Optional. Array of properties for the new Section object.
	 *                                          See GC_Customize_Section::__construct() for information
	 *                                          on accepted arguments. Default empty array.
	 * @return GC_Customize_Section The instance of the section that was added.
	 */
	 ...
	 *
	 * @param string $section Name of a custom section which is a subclass of GC_Customize_Section.
	 */
	 ...
	 *
	 * @param GC_Customize_Control|string $id   Customize Control object, or ID.
	 * @param array                       $args Optional. Array of properties for the new Control object.
	 *                                          See GC_Customize_Control::__construct() for information
	 *                                          on accepted arguments. Default empty array.
	 * @return GC_Customize_Control The instance of the control that was added.
	 */
	public function add_control( $id, $args = array() ) {
		if ( $id instanceof GC_Customize_Control ) {
			$control = $id;
		} else {
			$control = new GC_Customize_Control( $this, $id, $args );
		}
	...
	public function render_control_templates() {
		if ( $this->branching() ) {
			$l10n = array(
				...
				'locked'                => __( '%s已经在自定义此变更集，请等待该用户完成自定义。您最后的修改已被自动保存。' ),
				/* translators: %s: User who is customizing the changeset in customizer. */
				'locked_allow_override' => __( '%s已经在自定义此变更集，您想要接管吗？' ),
			);
		} else {
			$l10n = array(
				/* translators: %s: User who is customizing the changeset in customizer. */
	 ...
	 * @param GC_Customize_Panel|GC_Customize_Section|GC_Customize_Control $b Object B.
	 * @return int
	 */
	...
	public function get_document_title_template() {
		if ( $this->is_theme_active() ) {
			...
			$document_title_tmpl = __( '自定义：%s' );
		} else {
			/* translators: %s: Document title from the preview. */
			$document_title_tmpl = __( '实时预览：%s' );
		}
	...
	public function is_cross_domain() {
		$admin_origin = gc_parse_url( admin_url() );
		$home_origin  = gc_parse_url( home_url() );
	...
	public function get_return_url() {
		...
		$excluded_referer_basenames = array( 'customize.php', 'gc-login.php' );
	...
	public function set_autofocus( $autofocus ) {
		$this->autofocus = array_filter( gc_array_slice_assoc( $autofocus, array( 'panel', 'section', 'control' ) ), 'is_string' );
	}
	...
	public function get_nonces() {
		$nonces = array(
			'save'                     => gc_create_nonce( 'save-customize_' . $this->get_stylesheet() ),
			'preview'                  => gc_create_nonce( 'preview-customize_' . $this->get_stylesheet() ),
			'switch_themes'            => gc_create_nonce( 'switch_themes' ),
			'dismiss_autosave_or_lock' => gc_create_nonce( 'customize_dismiss_autosave_or_lock' ),
			'override_lock'            => gc_create_nonce( 'customize_override_changeset_lock' ),
			'trash'                    => gc_create_nonce( 'trash_customize_changeset' ),
		);
	...
	public function customize_pane_settings() {
		...
		if ( $current_user_can_publish ) {
			$status_choices[] = array(
				'status' => 'future',
				'label'  => _x( '计划', 'customizer changeset action/button label' ),
			);
		}
		...
		<script type="text/javascript">
			...
			echo "})( _gcCustomizeSettings.settings );\n";
			...
			echo "})( _gcCustomizeSettings.controls );\n";
			?>
		</script>
		<?php
	}
	...
	public function register_controls() {
		...
		$this->add_panel(
			new GC_Customize_Themes_Panel(
				$this,
				'themes',
				array(
					'title'       => $this->theme()->display( 'Name' ),
					'description' => (
					'<p>' . __( '要查找某个主题？您可以搜索或浏览www.GeChiUI.com主题目录，安装及预览主题，然后在这里启用它们。' ) . '</p>' .
					'<p>' . __( '在您预览新主题时，您可以继续调整小工具及菜单，也可以探索主题特定的选项。' ) . '</p>'
					),
		...
		$this->add_section(
			new GC_Customize_Themes_Section(
				$this,
				'installed_themes',
				array(
					'title'      => __( '已安装的主题' ),
		...
		$this->add_control(
			new GC_Customize_Site_Icon_Control(
				$this,
				'site_icon',
				array(
					'label'       => __( '系统图标' ),
		...
		$this->add_control(
			new GC_Customize_Cropped_Image_Control(
				$this,
				'custom_logo',
				array(
					'label'         => __( 'Logo' ),
					...
					'button_labels' => array(
						...
						'remove'       => __( '移除' ),
						'default'      => __( '默认' ),
						'placeholder'  => __( '未选择 logo' ),
						'frame_title'  => __( '选择 logo' ),
						'frame_button' => __( '选择 logo' ),
					),
				)
			)
		);
		...
		$this->add_control(
			new GC_Customize_Color_Control(
				$this,
				'header_textcolor',
				array(
					'label'   => __( '页眉文字颜色' ),
		...
		if ( current_theme_supports( 'custom-header', 'video' ) ) {
			$title       = __( '页眉媒体' );
			$description = '<p>' . __( '如果您加入视频，此图片将在视频载入时候补显示。' ) . '</p>';
			...
			if ( $width && $height ) {
				$control_description = sprintf(
					...
					__( '以%1$s上传您的视频来优化文件大小并获得最佳结果。您的主题推荐使用%2$s的视频尺寸。' ),
					'<code>.mp4</code>',
				...
				$control_description = sprintf(
					...
					__( '以%1$s上传您的视频来优化文件大小并获得最佳结果。您的主题推荐使用%2$s的视频宽度。' ),
					'<code>.mp4</code>',
				...
				$control_description = sprintf(
					...
					__( '以%1$s上传您的视频来优化文件大小并获得最佳结果。您的主题推荐使用%2$s的视频高度。' ),
					'<code>.mp4</code>',
		...
		$this->add_setting(
			new GC_Customize_Header_Image_Setting(
				$this,
				'header_image_data',
				array(
					'theme_supports' => 'custom-header',
		...
		$this->add_control(
			new GC_Customize_Media_Control(
				$this,
				'header_video',
				array(
					'theme_supports'  => array( 'custom-header', 'video' ),
		...
		$this->add_control( new GC_Customize_Header_Image_Control( $this ) );

		$this->selective_refresh->add_partial(
			'custom_header',
			array(
				'selector'            => '#gc-custom-header',
		...
		$this->add_setting(
			new GC_Customize_Background_Image_Setting(
				$this,
		...
		$this->add_control( new GC_Customize_Background_Image_Control( $this ) );
		...
		$this->add_control(
			'background_preset',
			array(
				'label'   => _x( '预设', 'Background Preset' ),
				'section' => 'background_image',
				'type'    => 'select',
				'choices' => array(
					'default' => _x( '默认', 'Default Preset' ),
					'fill'    => __( '填满屏幕' ),
					'fit'     => __( '适合屏幕' ),
					'repeat'  => _x( '重复', 'Repeat Image' ),
					'custom'  => _x( '自定义', 'Custom Preset' ),
				),
			)
		);
		...
		$this->add_control(
			new GC_Customize_Background_Position_Control(
				$this,
				'background_position',
				array(
					'label'    => __( '图片位置' ),
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.gechiui.com/ticket/19627 which introduces the static-front-page theme_support.
		...
		$section_description .= '<p id="editor-keyboard-trap-help-1">' . __( '用键盘导航时：' ) . '</p>';
		$section_description .= '<ul>';
		$section_description .= '<li id="editor-keyboard-trap-help-2">' . __( '在编辑区域中，Tab键将输入一个制表符。' ) . '</li>';
		$section_description .= '<li id="editor-keyboard-trap-help-3">' . __( '要移开此区域，请先按Esc键再按Tab键。' ) . '</li>';
		$section_description .= '<li id="editor-keyboard-trap-help-4">' . __( '致屏幕阅读器用户：在表单模式中，您可能需要按Esc键两次。' ) . '</li>';
		$section_description .= '</ul>';
		...
		$section_description .= '<button type="button" class="button-link section-description-close">' . __( '关闭' ) . '</button>';
		$section_description .= '</p>';
		...
		$custom_css_setting = new GC_Customize_Custom_CSS_Setting(
			$this,
		...
		$this->add_control(
			new GC_Customize_Code_Editor_Control(
				$this,
				'custom_css',
				array(
					'label'       => __( 'CSS代码' ),
	...
	public function handle_load_themes_request() {
		...
		if ( empty( $_POST['theme_action'] ) ) {
			gc_send_json_error( 'missing_theme_action' );
		}
		...
		if ( ! array_key_exists( 'search', $_POST ) ) {
			...
			$args['search'] = sanitize_text_field( gc_unslash( $_POST['search'] ) );
		}

		if ( ! array_key_exists( 'tags', $_POST ) ) {
			$args['tag'] = '';
		} else {
			$args['tag'] = array_map( 'sanitize_text_field', gc_unslash( (array) $_POST['tags'] ) );
		}
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load all installed themes from gc_prepare_themes_for_js().
			$themes = array( 'themes' => array() );
			foreach ( gc_prepare_themes_for_js() as $theme ) {
				$theme['type']      = 'installed';
				$theme['active']    = ( isset( $_POST['customized_theme'] ) && $_POST['customized_theme'] === $theme['id'] );
				$themes['themes'][] = $theme;
			}
		} elseif ( 'gcorg' === $theme_action ) {

			// Load www.GeChiUI.com themes from the .org API and normalize data to match installed theme objects.
			...
			$args = array_merge( $gcorg_args, $args );
			...
			if ( is_gc_error( $themes ) ) {
				gc_send_json_error();
			}

			// This list matches the allowed tags in gc-admin/includes/theme-install.php.
			...
			$gc_themes        = gc_get_themes();
			foreach ( $gc_themes as $theme ) {
				$installed_themes[] = $theme->get_stylesheet();
			}
			$update_php = network_admin_url( 'update.php?action=install-theme' );

			// Set up properties for themes available on www.GeChiUI.com.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->authorAndUri  = gc_kses( $theme->author['display_name'], $themes_allowedtags );
				$theme->compatibleGC  = is_gc_version_compatible( $theme->requires ); // phpcs:ignore GeChiUI.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore GeChiUI.NamingConventions.ValidVariableName
		 ...
		 * @param GC_Customize_Manager $manager Instance of Customize manager.
		 */
		$themes = apply_filters( 'customize_load_themes', $themes, $args, $this );

		gc_send_json_success( $themes );
	}
	 ...
	 * @return string|GC_Error Background value or validation error.
	 */
	 ...
	 * argument. When GC no longer supports PHP 5.3, this method can be removed
