<?php
/**
 * Blocks API: GC_Block_Pattern_Categories_Registry class
 *
 * @package GeChiUI
 * @subpackage Blocks
 *
	...
	public function unregister( $category_name ) {
		if ( ! $this->is_registered( $category_name ) ) {
			_doing_it_wrong(
				...
				sprintf( __( '未找到区块样板分类“%s”。' ), $category_name ),
				'5.5.0'
			);
			return false;
		}
 ...
 *                                    See GC_Block_Pattern_Categories_Registry::register() for
 *                                    accepted arguments.
...
function unregister_block_pattern_category( $category_name ) {
	return GC_Block_Pattern_Categories_Registry::get_instance()->unregister( $category_name );
}
