<?php
/**
 * Network API: GC_Network_Query class
 *
 * @package GeChiUI
...
class GC_Network_Query {
	...
	public function get_networks() {
		 ...
		 *
		 * Return a non-null value to bypass GeChiUI' default network queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of GC_Network objects.
		 *
		 * Note that if the filter returns an array of network data, it will be assigned
		 * to the `networks` property of the current GC_Network_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_networks` and `max_num_pages` properties of the GC_Network_Query object,
		...
		// Convert to GC_Network instances.
	...
	protected function get_network_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$gcdb->site.id IN ( " . implode( ',', gc_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$gcdb->site.id NOT IN ( " . implode( ',', gc_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $gcdb->prepare( "$gcdb->site.domain = %s", $this->query_vars['domain'] );
		}

		// Parse network domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "$gcdb->site.domain IN ( '" . implode( "', '", $gcdb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse network domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "$gcdb->site.domain NOT IN ( '" . implode( "', '", $gcdb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $gcdb->prepare( "$gcdb->site.path = %s", $this->query_vars['path'] );
		}

		// Parse network path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "$gcdb->site.path IN ( '" . implode( "', '", $gcdb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse network path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "$gcdb->site.path NOT IN ( '" . implode( "', '", $gcdb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			$this->sql_clauses['where']['search'] = $this->get_search_sql(
				...
				array( "$gcdb->site.domain", "$gcdb->site.path" )
			);
		}
		...
		$this->sql_clauses['from']    = "FROM $gcdb->site $join";
		...
		$network_ids = $gcdb->get_col( $this->request );
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			...
			$this->found_networks = (int) $gcdb->get_var( $found_networks_query );
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( 'network__in' === $orderby ) {
			...
			$parsed      = "FIELD( {$gcdb->site}.id, $network__in )";
		} elseif ( 'domain_length' === $orderby || 'path_length' === $orderby ) {
			$field  = substr( $orderby, 0, -7 );
			$parsed = "CHAR_LENGTH($gcdb->site.$field)";
		} elseif ( in_array( $orderby, $allowed_keys, true ) ) {
			$parsed = "$gcdb->site.$orderby";
		}
