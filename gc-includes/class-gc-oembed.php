 ...
 *
 * Used internally by the GC_Embed class, but is designed to be generic.
 *
 * @link https://www.gechiui.com/support/embeds/
...
class GC_oEmbed {
	...
	public function __construct() {
		 ...
		 *
		 * Since GeChiUI 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		...
		// Fix any embeds that contain new lines in the middle of the HTML which breaks gcautop().
	 ...
	 *
	 * The provider is added just-in-time when gc_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when gc_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	 ...
	 *                           See gc_oembed_get() for accepted arguments. Default empty.
	...
	public function discover( $url ) {
		...
		// Fetch URL content.
		$request = gc_safe_remote_get( $url, $args );
		$html    = gc_remote_retrieve_body( $request );
		if ( $html ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = gc_parse_args( $args, gc_embed_defaults( $url ) );
	 ...
	 * @return object|false|GC_Error The result in the form of an object on success, false on failure.
	 */
