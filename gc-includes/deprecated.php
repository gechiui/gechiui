<?php
/**
 * Deprecated functions from past GeChiUI versions. You shouldn't use these
 ...
 * @return string|null
 */
function gc_get_linksbyname($category, $args = '') {
	...
	return gc_list_bookmarks($parsed_args);
}
...
function list_cats($optionall = 1, $all = 'All', $sort_column = 'ID', $sort_order = 'asc', $file = '', $list = true, $optiondates = 0,
	...
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_list_categories()' );

	$query = compact('optionall', 'all', 'sort_column', 'sort_order', 'file', 'list', 'optiondates', 'optioncount', 'hide_empty', 'use_desc_for_title', 'children',
		'child_of', 'categories', 'recurse', 'feed', 'feed_image', 'exclude', 'hierarchical');
	return gc_list_cats($query);
}
 ...
 */
function gc_list_cats($args = '') {
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_list_categories()' );

	$parsed_args = gc_parse_args( $args );
	...
	return gc_list_categories($parsed_args);
}
...
function dropdown_cats($optionall = 1, $all = 'All', $orderby = 'ID', $order = 'asc',
	...
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_dropdown_categories()' );
	...
	return gc_dropdown_categories($query);
}
...
function list_authors($optioncount = false, $exclude_admin = true, $show_fullname = false, $hide_empty = true, $feed = '', $feed_image = '') {
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_list_authors()' );

	$args = compact('optioncount', 'exclude_admin', 'show_fullname', 'hide_empty', 'feed', 'feed_image');
	return gc_list_authors($args);
}
...
function gc_get_post_cats($blogid = '1', $post_id = 0) {
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_get_post_categories()' );
	return gc_get_post_categories($post_id);
}
...
function gc_set_post_cats($blogid = '1', $post_id = 0, $post_categories = array()) {
	_deprecated_function( __FUNCTION__, '2.1.0', 'gc_set_post_categories()' );
	return gc_set_post_categories($post_id, $post_categories);
}
...
function create_user($username, $password, $email) {
	_deprecated_function( __FUNCTION__, '2.0.0', 'gc_create_user()' );
	return gc_create_user($username, $password, $email);
}
 ...
 * @see get_terms()
 *
 * @link https://developer.gechiui.com/reference/functions/get_all_category_ids/
 *
...
function get_attachment_icon( $id = 0, $fullsize = false, $max_dims = false ) {
	_deprecated_function( __FUNCTION__, '2.5.0', 'gc_get_attachment_image()' );
	$id = (int) $id;
	if ( !$post = get_post($id) )
		return false;
	...
	if ( ($max_dims = apply_filters('attachment_max_dims', $max_dims)) && file_exists($src_file) ) {
		...
		$imagesize = gc_getimagesize($src_file);
 ...
 * @return string Escaped `$text`.
 */
function gc_specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	_deprecated_function( __FUNCTION__, '2.8.0', 'esc_html()' );
	if ( func_num_args() > 1 ) { // Maintain back-compat for people passing additional arguments.
		return _gc_specialchars( $text, $quote_style, $charset, $double_encode );
	} else {
...
function unregister_sidebar_widget($id) {
	_deprecated_function( __FUNCTION__, '2.8.0', 'gc_unregister_sidebar_widget()' );
	return gc_unregister_sidebar_widget($id);
}
...
function unregister_widget_control($id) {
	_deprecated_function( __FUNCTION__, '2.8.0', 'gc_unregister_widget_control()' );
	return gc_unregister_widget_control($id);
}
...
function delete_usermeta( $user_id, $meta_key, $meta_value = '' ) {
	...
	$cur = $gcdb->get_row( $gcdb->prepare("SELECT * FROM $gcdb->usermeta WHERE user_id = %d AND meta_key = %s", $user_id, $meta_key) );

	if ( $cur && $cur->umeta_id )
		do_action( 'delete_usermeta', $cur->umeta_id, $user_id, $meta_key, $meta_value );

	if ( ! empty($meta_value) )
		$gcdb->query( $gcdb->prepare("DELETE FROM $gcdb->usermeta WHERE user_id = %d AND meta_key = %s AND meta_value = %s", $user_id, $meta_key, $meta_value) );
	else
		$gcdb->query( $gcdb->prepare("DELETE FROM $gcdb->usermeta WHERE user_id = %d AND meta_key = %s", $user_id, $meta_key) );

	clean_user_cache( $user_id );
	gc_cache_delete( $user_id, 'user_meta' );
...
function update_usermeta( $user_id, $meta_key, $meta_value ) {
	...
	if ( !$cur )
		$gcdb->insert($gcdb->usermeta, compact('user_id', 'meta_key', 'meta_value') );
	elseif ( $cur->meta_value != $meta_value )
		$gcdb->update($gcdb->usermeta, compact('meta_value'), compact('user_id', 'meta_key') );
	else
		return false;
	...
	if ( !$cur )
		do_action( 'added_usermeta', $gcdb->insert_id, $user_id, $meta_key, $meta_value );
	else
...
function get_users_of_blog( $id = '' ) {
	...
	$users = $gcdb->get_results( "SELECT user_id, user_id AS ID, user_login, display_name, user_email, meta_value FROM $gcdb->users, $gcdb->usermeta WHERE {$gcdb->users}.ID = {$gcdb->usermeta}.user_id AND meta_key = '{$blog_prefix}capabilities' ORDER BY {$gcdb->usermeta}.user_id" );
	return $users;
}
 ...
 * @return bool
 */
function gc_timezone_supported() {
...
function the_editor($content, $id = 'content', $prev_id = 'title', $media_buttons = true, $tab_index = 2, $extended = true) {
	...
	gc_editor( $content, $id, array( 'media_buttons' => $media_buttons ) );
}
...
/**
 * Add the "Dashboard"/"查看系统" menu.
 *
 * @deprecated 3.3.0
 *
 * @param GC_Admin_Bar $gc_admin_bar GC_Admin_Bar instance.
 */
function gc_admin_bar_dashboard_view_site_menu( $gc_admin_bar ) {
	_deprecated_function( __FUNCTION__, '3.3.0' );
	...
	if ( 0 != $user_id ) {
		if ( is_admin() )
			$gc_admin_bar->add_menu( array( 'id' => 'view-site', 'title' => __( '查看系统' ), 'href' => home_url() ) );
		elseif ( is_multisite() )
...
function get_current_theme() {
	...
	return gc_get_theme()->get('Name');
}

/**
 * Accepts matches array from preg_replace_callback in gcautop() or a string.
 *
 ...
 * @see add_theme_support()
 *
 * @param callable $gc_head_callback Call on the {@see 'gc_head'} action.
 ...
 */
function add_custom_image_header( $gc_head_callback, $admin_head_callback, $admin_preview_callback = '' ) {
	_deprecated_function( __FUNCTION__, '3.4.0', 'add_theme_support( \'custom-header\', $args )' );
	$args = array(
		'gc-head-callback'    => $gc_head_callback,
 ...
 */
function add_custom_background( $gc_head_callback = '', $admin_head_callback = '', $admin_preview_callback = '' ) {
	_deprecated_function( __FUNCTION__, '3.4.0', 'add_theme_support( \'custom-background\', $args )' );
	$args = array();
	if ( $gc_head_callback )
		$args['gc-head-callback'] = $gc_head_callback;
...
function get_theme_data( $theme_file ) {
	_deprecated_function( __FUNCTION__, '3.4.0', 'gc_get_theme()' );
	$theme = new GC_Theme( gc_basename( dirname( $theme_file ) ), dirname( dirname( $theme_file ) ) );
 ...
 *
 * @param GC_Post $post Post object, passed by reference (unused).
 */
...
function gc_load_image( $file ) {
	...
	if ( ! is_file( $file ) ) {
		...
		return sprintf( __( '文件“%s”不存在？' ), $file );
	}
 ...
 * @return mixed GC_Error on failure. String with new destination path.
 */
 ...
 * @return GC_Post|null Post object or array holding post contents and information
 */
...
function user_pass_ok($user_login, $user_pass) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'gc_authenticate()' );
	$user = gc_authenticate( $user_login, $user_pass );
	if ( is_gc_error( $user ) )
		return false;
...
function gd_edit_image_support($mime_type) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'gc_image_editor_supports()' );
 ...
 */
function gc_convert_bytes_to_hr( $bytes ) {
...
function rich_edit_exists() {
	global $gc_rich_edit_exists;
	_deprecated_function( __FUNCTION__, '3.9.0' );

	if ( ! isset( $gc_rich_edit_exists ) )
		$gc_rich_edit_exists = file_exists( ABSPATH . GCINC . '/vendors/tinymce/tinymce.js' );
...
function url_is_accessable_via_ssl( $url ) {
	...
	$response = gc_remote_get( set_url_scheme( $url, 'https' ) );

	if ( !is_gc_error( $response ) ) {
		$status = gc_remote_retrieve_response_code( $response );
...
function gc_richedit_pre($text) {
	...
	if ( empty( $text ) ) {
		 ...
		 * is passed to gc_richedit_pre(). If an empty string is passed, it results
 ...
 */
function gc_htmledit_pre($output) {
	_deprecated_function( __FUNCTION__, '4.3.0', 'format_for_editor()' );
 ...
 */
function gc_get_http( $url, $file_path = false, $red = 1 ) {
	_deprecated_function( __FUNCTION__, '4.4.0', 'GC_Http' );
	...
	$response = gc_safe_remote_request( $url, $options );

	if ( is_gc_error( $response ) )
		return false;

	$headers = gc_remote_retrieve_headers( $response );
	$headers['response'] = gc_remote_retrieve_response_code( $response );

	// GC_HTTP no longer follows redirects for HEAD requests.
	if ( 'HEAD' == $options['method'] && in_array($headers['response'], array(301, 302)) && isset( $headers['location'] ) ) {
		return gc_get_http( $headers['location'], $file_path, ++$red );
	}
	...
	fwrite( $out_fp,  gc_remote_retrieve_body( $response ) );
	fclose($out_fp);
	clearstatcache();
 ...
 */
function gc_embed_handler_googlevideo( $matches, $attr, $url, $rawattr ) {
 ...
 * @return string
 */
function gc_kses_js_entities( $content ) {
	_deprecated_function( __FUNCTION__, '4.7.0' );
...
function _usort_terms_by_ID( $a, $b ) {
	_deprecated_function( __FUNCTION__, '4.7.0', 'gc_list_sort()' );
 ...
 */
function gc_get_user_request_data( $request_id ) {
	_deprecated_function( __FUNCTION__, '5.4.0', 'gc_get_user_request()' );
	return gc_get_user_request( $request_id );
}
 ...
 */
function gc_make_content_images_responsive( $content ) {
	_deprecated_function( __FUNCTION__, '5.5.0', 'gc_filter_content_tags()' );

	// This will also add the `loading` attribute to `img` tags, if enabled.
	return gc_filter_content_tags( $content );
}
 ...
 * @deprecated 5.5.0
 */
function gc_unregister_GLOBALS() {  // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 */
function gc_blacklist_check( $author, $email, $url, $comment, $user_ip, $user_agent ) {
	_deprecated_function( __FUNCTION__, '5.5.0', 'gc_check_comment_disallowed_list()' );

	return gc_check_comment_disallowed_list( $author, $email, $url, $comment, $user_ip, $user_agent );
}
 ...
 */
function _gc_register_meta_args_whitelist( $args, $default_args ) {
	_deprecated_function( __FUNCTION__, '5.5.0', '_gc_register_meta_args_allowed_list()' );

	return _gc_register_meta_args_allowed_list( $args, $default_args );
}
 ...
 */
function gc_slash_strings_only( $value ) {
 ...
 * Typical usage is as a {@see 'gc_head'} callback:
 *
 *     add_action( 'gc_head', 'gc_sensitive_page_meta' );
 *
 * @since 5.0.1
 ...
 *
 * @see gc_robots_sensitive_page()
 */
function gc_sensitive_page_meta() {
	_deprecated_function( __FUNCTION__, '5.7.0', 'gc_robots_sensitive_page()' );

	?>
	<meta name='robots' content='noindex,noarchive' />
	<?php
	gc_strict_cross_origin_referrer();
}
 ...
 *
 * @see gc_should_skip_block_supports_serialization()
 *
...
function _get_path_to_translation( $domain, $reset = false ) {
	_deprecated_function( __FUNCTION__, '6.1.0', 'GC_Textdomain_Registry' );
 ...
 * @since 5.8.0
 * @access private
 * @deprecated 6.1.0 Use gc_style_engine_get_styles() introduced in 6.1.0.
 *
 * @see gc_style_engine_get_styles()
 *
 ...
 */
function gc_typography_get_css_variable_inline_style( $attributes, $feature, $css_property ) {
	_deprecated_function( __FUNCTION__, '6.1.0', 'gc_style_engine_get_styles()' );

	// Retrieve current attribute value or skip if not found.
	$style_value = _gc_array_get( $attributes, array( 'style', 'typography', $feature ), false );
	if ( ! $style_value ) {
		return;
	}
	...
	// Return the actual CSS inline style e.g. `text-decoration:var(--gc--preset--text-decoration--underline);`.
	return sprintf( '%s:var(--gc--preset--%s--%s);', $css_property, $css_property, $slug );
}
...
function _filter_query_attachment_filenames( $clauses ) {
	_deprecated_function( __FUNCTION__, '6.0.3', 'add_filter( "gc_allow_query_attachment_by_filename", "__return_true" )' );
...
function get_page_by_title( $page_title, $output = OBJECT, $post_type = 'page' ) {
	_deprecated_function( __FUNCTION__, '6.2.0', 'GC_Query' );
	global $gcdb;
 ...
 *
 * @deprecated 6.3.0 Use gc_lazyload_comment_meta() instead.
 *
 * @param GC_Comment[] $comments Array of comment objects.
 */
function gc_queue_comments_for_comment_meta_lazyload( $comments ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'gc_lazyload_comment_meta()' );
 ...
 *
 * @since 5.9.0
 * @deprecated 6.3.0 Use gc_get_loading_optimization_attributes() instead.
 * @see gc_get_loading_optimization_attributes()
 *
...
function gc_get_loading_attr_default( $context ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'gc_get_loading_optimization_attributes()' );
	global $gc_query;
	...
	if ( 'the_post_thumbnail' === $context || 'gc_get_attachment_image' === $context ) {
		/*
 ...
 *
 * @since 5.5.0
 * @deprecated 6.3.0 Use gc_img_tag_add_loading_optimization_attrs() instead.
...
function gc_img_tag_add_loading_attr( $image, $context ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'gc_img_tag_add_loading_optimization_attrs()' );
	/*
 ...
 */
function gc_tinycolor_bound01( $n, $max ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
 ...
 * @return float Value in the range [0,1].
 */
function _gc_tinycolor_bound_alpha( $n ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
 ...
 */
function gc_tinycolor_rgb_to_rgb( $rgb_color ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );

	return array(
		'r' => gc_tinycolor_bound01( $rgb_color['r'], 255 ) * 255,
		'g' => gc_tinycolor_bound01( $rgb_color['g'], 255 ) * 255,
		'b' => gc_tinycolor_bound01( $rgb_color['b'], 255 ) * 255,
	);
}
 ...
 */
function gc_tinycolor_hue_to_rgb( $p, $q, $t ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
 ...
 * @return array Rounded and converted RGB object.
 */
function gc_tinycolor_hsl_to_rgb( $hsl_color ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );

	$h = gc_tinycolor_bound01( $hsl_color['h'], 360 );
	$s = gc_tinycolor_bound01( $hsl_color['s'], 100 );
	$l = gc_tinycolor_bound01( $hsl_color['l'], 100 );
	...
	if ( 0 === $s ) {
		...
		$p = 2 * $l - $q;
		$r = gc_tinycolor_hue_to_rgb( $p, $q, $h + 1 / 3 );
		$g = gc_tinycolor_hue_to_rgb( $p, $q, $h );
		$b = gc_tinycolor_hue_to_rgb( $p, $q, $h - 1 / 3 );
	}
 ...
 */
function gc_tinycolor_string_to_rgb( $color_str ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	...
	if ( preg_match( $rgba_regexp, $color_str, $match ) ) {
		...
		$rgb['a'] = _gc_tinycolor_bound_alpha( $match[4] );
	...
	if ( preg_match( $hex8_regexp, $color_str, $match ) ) {
		...
		$rgb['a'] = _gc_tinycolor_bound_alpha(
...
function gc_get_duotone_filter_id( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	return GC_Duotone::get_filter_id_from_preset( $preset );
}
...
function gc_get_duotone_filter_property( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	return GC_Duotone::get_filter_css_property_value_from_preset( $preset );
}
...
function gc_get_duotone_filter_svg( $preset ) {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	return GC_Duotone::get_filter_svg_from_preset( $preset );
}
 ...
 *
 * @since 5.8.0
 * @deprecated 6.3.0 Use GC_Duotone::register_duotone_support() instead.
 *
 * @access private
 *
 * @param GC_Block_Type $block_type Block Type.
...
function gc_register_duotone_support( $block_type ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Duotone::register_duotone_support()' );
	return GC_Duotone::register_duotone_support( $block_type );
}
 ...
 * @since 5.8.0
 * @since 6.1.0 Allow unset for preset colors.
 * @deprecated 6.3.0 Use GC_Duotone::render_duotone_support() instead.
 *
 * @access private
 *
 ...
 */
function gc_render_duotone_support( $block_content, $block ) {
	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Duotone::render_duotone_support()' );
	$gc_block = new GC_Block( $block );
	return GC_Duotone::render_duotone_support( $block_content, $block, $gc_block );
}
 ...
 * @return string
 */
function gc_get_global_styles_svg_filters() {
	_deprecated_function( __FUNCTION__, '6.3.0' );
	 ...
	 */
	$can_use_cached = ! gc_is_development_mode( 'theme' );
	$cache_group    = 'theme_json';
	$cache_key      = 'gc_get_global_styles_svg_filters';
	if ( $can_use_cached ) {
		$cached = gc_cache_get( $cache_key, $cache_group );
		if ( $cached ) {
			return $cached;
		}
	}

	$supports_theme_json = gc_theme_has_theme_json();
	...
	$tree = GC_Theme_JSON_Resolver::get_merged_data();
	$svgs = $tree->get_svg_filters( $origins );

	if ( $can_use_cached ) {
		gc_cache_set( $cache_key, $svgs, $cache_group );
	}
 ...
 *
 * @since 5.9.0
 * @deprecated 6.3.0 This was removed from the Navigation Submenu block in favour of `gc_apply_colors_support()`.
 *                   `gc_apply_colors_support()` returns an array with similar class and style values,
