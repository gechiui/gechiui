	...
	switch ( $cap ) {
		...
		case 'delete_page':
			...
			if ( $post->post_author && $user_id == $post->post_author ) {
				...
				if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
					...
					$status = get_post_meta( $post->ID, '_gc_trash_meta_status', true );
		...
		case 'edit_themes':
			...
			if ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) {
				$caps[] = 'do_not_allow';
			} elseif ( ! gc_is_file_mod_allowed( 'capability_edit_themes' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
			} else {
 ...
 *
 * @return GC_Roles GC_Roles global instance if not already instantiated.
 */
function gc_roles() {
	global $gc_roles;
 ...
 */
function gc_maybe_grant_install_languages_cap( $allcaps ) {
 ...
 * @return bool[] Filtered array of the user's capabilities.
 */
function gc_maybe_grant_resume_extensions_caps( $allcaps ) {
 ...
 * @return bool[] Filtered array of the user's capabilities.
 */
function gc_maybe_grant_site_health_caps( $allcaps, $caps, $args, $user ) {
