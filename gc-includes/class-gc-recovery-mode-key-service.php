...
final class GC_Recovery_Mode_Key_Service {
	...
	public function generate_recovery_mode_token() {
		return gc_generate_password( 22, false );
	}
	...
	public function generate_and_store_recovery_mode_key( $token ) {
		...
		$key = gc_generate_password( 22, false );
		...
		$hashed = $gc_hasher->HashPassword( $key );
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new GC_Error( 'token_not_found', __( '恢复模式未初始化。' ) );
		}
		...
		if ( time() > $record['created_at'] + $ttl ) {
			return new GC_Error( 'key_expired', __( '恢复密钥已过期。' ) );
		}
