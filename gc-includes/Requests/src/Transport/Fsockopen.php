...
namespace GcOrg\Requests\Transport;

use GcOrg\Requests\Capability;
use GcOrg\Requests\Exception;
use GcOrg\Requests\Exception\InvalidArgument;
use GcOrg\Requests\Port;
use GcOrg\Requests\Requests;
use GcOrg\Requests\Ssl;
use GcOrg\Requests\Transport;
use GcOrg\Requests\Utility\CaseInsensitiveDictionary;
...
final class Fsockopen implements Transport {
	 ...
	 * @param array $options Request options, see {@see \GcOrg\Requests\Requests::response()} for documentation
	 * @return string Raw HTTP result
	 *
	 * @throws \GcOrg\Requests\Exception\InvalidArgument When the passed $url argument is not a string or Stringable.
	 * @throws \GcOrg\Requests\Exception\InvalidArgument When the passed $headers argument is not an array.
	 * @throws \GcOrg\Requests\Exception\InvalidArgument When the passed $data parameter is not an array or string.
	 * @throws \GcOrg\Requests\Exception\InvalidArgument When the passed $options argument is not an array.
	 * @throws \GcOrg\Requests\Exception       On failure to connect to socket (`fsockopenerror`)
	 * @throws \GcOrg\Requests\Exception       On socket timeout (`timeout`)
	 */
	public function request($url, $headers = [], $data = [], $options = []) {
		...
		if (empty($url_parts)) {
			throw new Exception('无效URL。', 'invalidurl', $url);
		}
		...
		$download   = false;
		if ($options['filename']) {
			// phpcs:ignore GeChiUI.PHP.NoSilencedErrors -- Silenced the PHP native warning in favour of throwing an exception.
	 ...
	 *
	 * @param array $requests Request data (array of 'url', 'headers', 'data', 'options') as per {@see \GcOrg\Requests\Transport::request()}
	 * @param array $options Global options, see {@see \GcOrg\Requests\Requests::response()} for documentation
	 * @return array Array of \GcOrg\Requests\Response objects (may contain \GcOrg\Requests\Exception or string responses as well)
	 *
	 ...
	 * @return bool
	 *
	 * @throws \GcOrg\Requests\Exception On failure to connect via TLS (`fsockopen.ssl.connect_error`)
	 * @throws \GcOrg\Requests\Exception On not obtaining a match for the host (`fsockopen.ssl.no_match`)
	 */
	 ...
	 *
	 * The available capabilities to test for can be found in {@see \GcOrg\Requests\Capability}.
	 *
	 * @codeCoverageIgnore
