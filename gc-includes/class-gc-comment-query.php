<?php
/**
 * Comment API: GC_Comment_Query class
 *
 * @package GeChiUI
 * @subpackage Comments
 *
...
class GC_Comment_Query {
	 ...
	 *                                                        stored in a `children` property on the `GC_Comment` object.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See GC_Date_Query.
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	...
	public function get_comments() {
		 ...
		 *
		 * Return a non-null value to bypass GeChiUI' default comment queries.
		 *
	...
	protected function get_comment_ids() {
		...
		$statuses       = gc_parse_list( $this->query_vars['status'] );
		...
		if ( ! in_array( 'any', $statuses, true ) ) {
			foreach ( $statuses as $status ) {
				switch ( $status ) {
					...
					default:
						$status_clauses[] = $gcdb->prepare( 'comment_approved = %s', $status );
						break;
				}
			}
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			...
			foreach ( $include_unapproved as $unapproved_identifier ) {
				...
				if ( is_numeric( $unapproved_identifier ) ) {
					$approved_clauses[] = $gcdb->prepare( "( user_id = %d AND comment_approved = '0' )", $unapproved_identifier );
				} else {
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			if ( empty( $orderby_array ) ) {
				$orderby_array[] = "$gcdb->comments.comment_date_gmt $order";
			}
			...
			if ( ! $found_orderby_comment_id ) {
				...
				$orderby_array[] = "$gcdb->comments.comment_ID $comment_id_order";
			}

			$orderby = implode( ', ', $orderby_array );
		} else {
			$orderby = "$gcdb->comments.comment_date_gmt $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$gcdb->comments.comment_ID";
		}

		$post_id = absint( $this->query_vars['post_id'] );
		if ( ! empty( $post_id ) ) {
			$this->sql_clauses['where']['post_id'] = $gcdb->prepare( 'comment_post_ID = %d', $post_id );
		}

		// Parse comment IDs for an IN clause.
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$gcdb->comments.comment_ID IN ( " . implode( ',', gc_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$gcdb->comments.comment_ID NOT IN ( " . implode( ',', gc_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}

		if ( '' !== $this->query_vars['author_email'] ) {
			$this->sql_clauses['where']['author_email'] = $gcdb->prepare( 'comment_author_email = %s', $this->query_vars['author_email'] );
		}

		if ( '' !== $this->query_vars['author_url'] ) {
			$this->sql_clauses['where']['author_url'] = $gcdb->prepare( 'comment_author_url = %s', $this->query_vars['author_url'] );
		}

		if ( '' !== $this->query_vars['karma'] ) {
			$this->sql_clauses['where']['karma'] = $gcdb->prepare( 'comment_karma = %d', $this->query_vars['karma'] );
		}
		...
		foreach ( $raw_types as $operator => $_raw_types ) {
			...
			foreach ( $_raw_types as $type ) {
				switch ( $type ) {
					...
					default:
						$comment_types[ $operator ][] = $gcdb->prepare( '%s', $type );
						break;
				}
			}
		...
		if ( '' !== $parent ) {
			$this->sql_clauses['where']['parent'] = $gcdb->prepare( 'comment_parent = %d', $parent );
		}

		if ( is_array( $this->query_vars['user_id'] ) ) {
			$this->sql_clauses['where']['user_id'] = 'user_id IN (' . implode( ',', array_map( 'absint', $this->query_vars['user_id'] ) ) . ')';
		} elseif ( '' !== $this->query_vars['user_id'] ) {
			$this->sql_clauses['where']['user_id'] = $gcdb->prepare( 'user_id = %d', $this->query_vars['user_id'] );
		}
		...
		$plucked          = gc_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $post_fields ) ) {
			...
			foreach ( $post_fields as $field_name => $field_value ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $gcdb->prepare( " {$gcdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $field_value );
			}
		}
		...
		foreach ( array( 'post_status', 'post_type' ) as $field_name ) {
			...
			if ( ! empty( $this->query_vars[ $field_name ] ) ) {
				...
				// phpcs:ignore GeChiUI.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$this->sql_clauses['where'][ $field_name ] = $gcdb->prepare( " {$gcdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $q_values );
			}
		}

		// Comment author IDs for an IN clause.
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table                              = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table                                  = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', gc_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}
		...
		if ( $join_posts_table ) {
			$join .= "JOIN $gcdb->posts ON $gcdb->posts.ID = $gcdb->comments.comment_post_ID";
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			if ( ! $this->query_vars['count'] ) {
				$groupby = "{$gcdb->comments}.comment_ID";
			}
		}
		...
		$this->sql_clauses['from']    = "FROM $gcdb->comments $join";
		...
		if ( $this->query_vars['count'] ) {
			...
			$comment_ids = $gcdb->get_col( $this->request );
	...
	protected function fill_descendants( $comments ) {
		...
		$key          = md5( serialize( gc_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = gc_cache_get_last_changed( 'comment' );
		...
		do {
			...
			if ( $uncached_parent_ids ) {
				...
				foreach ( $parent_map as $parent_id => $children ) {
					$cache_key = "get_comment_child_ids:$parent_id:$key:$last_changed";
					gc_cache_set( $cache_key, $children, 'comment' );
				}
			}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $this->query_vars['meta_key'] === $orderby || 'meta_value' === $orderby ) {
			$parsed = "$gcdb->commentmeta.meta_value";
		} elseif ( 'meta_value_num' === $orderby ) {
			$parsed = "$gcdb->commentmeta.meta_value+0";
		} elseif ( 'comment__in' === $orderby ) {
			$comment__in = implode( ',', array_map( 'absint', $this->query_vars['comment__in'] ) );
			$parsed      = "FIELD( {$gcdb->comments}.comment_ID, $comment__in )";
