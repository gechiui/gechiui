...
function register_rest_route( $namespace, $route, $args = array(), $override = false ) {
	if ( empty( $namespace ) ) {
		 ...
		 * non-namespaced route, call `GC_REST_Server::register_route` directly.
		 */
		_doing_it_wrong( 'register_rest_route', __( '路由必须被插件或主题的名字及版本进行命名空间限定。' ), '4.4.0' );
		return false;
	} elseif ( empty( $route ) ) {
		_doing_it_wrong( 'register_rest_route', __( '路由必须被指明。' ), '4.4.0' );
		return false;
	}
		...
		if ( ! isset( $arg_group['permission_callback'] ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					__( '%1$s REST API的路由定义缺少必要的%2$s参数。对于打算公开的REST API路由，请使用%3$s作为权限回调。' ),
...
function rest_api_register_rewrites() {
	...
	add_rewrite_rule( '^' . $gc_rewrite->index . '/' . rest_get_url_prefix() . '/?$', 'index.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . $gc_rewrite->index . '/' . rest_get_url_prefix() . '/(.*)?', 'index.php?rest_route=/$matches[1]', 'top' );
}
...
function rest_api_default_filters() {
	...
	add_filter( 'rest_index', 'rest_add_appkeys_to_index' );
}
...
function create_initial_rest_routes() {
	...
	// Menu Locations.
	$controller = new GC_REST_Menu_Locations_Controller();
	$controller->register_routes();
...
function rest_api_loaded() {
	if ( empty( $GLOBALS['gc']->query_vars['rest_route'] ) ) {
		return;
	}
	...
	$route = untrailingslashit( $GLOBALS['gc']->query_vars['rest_route'] );
	if ( empty( $route ) ) {
		$route = '/';
	}
...
function rest_get_url_prefix() {
	 ...
	 */
	return apply_filters( 'rest_url_prefix', 'gc-json' );
}
...
function get_rest_url( $blog_id = null, $path = '/', $scheme = 'rest' ) {
	...
	if ( is_multisite() && get_blog_option( $blog_id, 'permalink_structure' ) || get_option( 'permalink_structure' ) ) {
		...
		if ( $gc_rewrite->using_index_permalinks() ) {
			$url = get_home_url( $blog_id, $gc_rewrite->index . '/' . rest_get_url_prefix(), $scheme );
		} else {
 ...
 *
 * @return GC_REST_Server REST server instance.
 */
function rest_get_server() {
	/* @var GC_REST_Server $gc_rest_server */
	global $gc_rest_server;

	if ( empty( $gc_rest_server ) ) {
		/**
...
function rest_ensure_request( $request ) {
	if ( $request instanceof GC_REST_Request ) {
		return $request;
	}

	if ( is_string( $request ) ) {
		return new GC_REST_Request( 'GET', $request );
	}
 ...
 *
 * This implements GC_REST_Response, allowing usage of `set_status`/`header`/etc
 ...
 *                                   returns a new GC_REST_Response instance.
 */
 ...
 * @return GC_REST_Response Modified response, either response or `null` to indicate pass-through.
 */
 ...
 * @param GC_REST_Request  $request  The request that was used to make current response.
 * @return GC_REST_Response Response to be served, with "Allow" header if route has allowed methods.
 */
 ...
 * @param GC_REST_Server   $server   ResponseHandler instance (usually GC_REST_Server).
 * @param GC_REST_Request  $request  The request that was used to make current response.
 * @return GC_REST_Response Response to be served, trimmed down to contain a subset of fields.
 */
...
function rest_output_rsd() {
	...
	?>
	<api name="GC-API" blogID="1" preferred="false" apiLink="<?php echo esc_url( $api_root ); ?>" />
	<?php
}
 ...
 * @see get_rest_url()
 */
function rest_output_link_gc_head() {
	$api_root = get_rest_url();
 ...
 *
 * @param GC_Error|mixed $result Error from another authentication handler,
 *                               null if we should handle it, or another value if not.
 * @return GC_Error|mixed|bool GC_Error if the cookie is invalid, the $result, otherwise true.
 */
function rest_cookie_check_errors( $result ) {
	...
	if ( ! $result ) {
		return new GC_Error( 'rest_cookie_invalid_nonce', __( 'Cookie 检查失败' ), array( 'status' => 403 ) );
	}
...
function rest_appkey_collect_status( $user_or_error, $app_password = array() ) {
	...
	$gc_rest_appkey_status = $user_or_error;

	if ( empty( $app_password['uuid'] ) ) {
		$gc_rest_appkey_uuid = null;
	} else {
		$gc_rest_appkey_uuid = $app_password['uuid'];
	}
}
...
function rest_appkey_check_errors( $result ) {
	...
	if ( is_gc_error( $gc_rest_appkey_status ) ) {
		$data = $gc_rest_appkey_status->get_error_data();

		if ( ! isset( $data['status'] ) ) {
			$data['status'] = 401;
		}

		$gc_rest_appkey_status->add_data( $data );

		return $gc_rest_appkey_status;
	}

	if ( $gc_rest_appkey_status instanceof GC_User ) {
		return true;
	}
 ...
 */
function rest_add_appkeys_to_index( $response ) {
	if ( ! gc_is_appkeys_available() ) {
		return $response;
	}

	$response->data['authentication']['appkeys'] = array(
		'endpoints' => array(
...
function rest_is_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		$maybe_array = gc_parse_list( $maybe_array );
	}

	return gc_is_numeric_array( $maybe_array );
}
...
function rest_sanitize_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		return gc_parse_list( $maybe_array );
	}
...
function rest_handle_multi_type_schema( $value, $args, $param = '' ) {
	...
	if ( $invalid_types ) {
		_doing_it_wrong(
			...
			gc_sprintf( __( '%1$s“type”模式描述关键字仅可以是下列内置类型：%2$l。' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
...
function rest_stabilize_value( $value ) {
	...
	if ( is_object( $value ) ) {
		_doing_it_wrong( __FUNCTION__, __( '无法使对象稳定化。请先将对象转换为数组。' ), '5.5.0' );
...
function rest_format_combining_operation_error( $param, $error ) {
	...
	if ( isset( $error['schema']['title'] ) ) {
		...
		return new GC_Error(
			...
			sprintf( __( '%1$s不是有效的%2$s。原因：%3$s' ), $param, $title, $reason ),
	...
	return new GC_Error(
		...
		sprintf( __( '%1$s与预期格式不匹配。原因：%2$s' ), $param, $reason ),
		array( 'position' => $position )
	);
}
 ...
 * @return GC_Error      The combining operation error.
 */
function rest_get_combining_operation_error( $value, $param, $errors ) {
	...
	if ( count( $schema_titles ) === count( $errors ) ) {
		...
		return new GC_Error( 'rest_no_matching_schema', gc_sprintf( __( '%1$s不是有效的%2$l。' ), $param, $schema_titles ) );
	}

	/* translators: %s: Parameter. */
	return new GC_Error( 'rest_no_matching_schema', sprintf( __( '%s与任何预期格式不匹配。' ), $param ) );
}
 ...
 * @param array  $args    The schema array to use.
 * @param string $param   The parameter name, used in error messages.
 * @return array|GC_Error The matching schema or GC_Error instance if all schemas do not match.
 */
 ...
 * @return array|GC_Error                The matching schema or GC_Error instance if the number of matching schemas is not equal to one.
 */
	...
	if ( count( $matching_schemas ) > 1 ) {
		...
		if ( count( $schema_titles ) === count( $matching_schemas ) ) {
			return new GC_Error(
				...
				/* translators: 1: Parameter, 2: Schema titles. */
				gc_sprintf( __( '%1$s与%2$l相匹配，但仅应匹配其中一个。' ), $param, $schema_titles ),
 ...
 * @param array  $args   The schema array to use.
 * @param string $param  The parameter name, used in error messages.
 * @return true|GC_Error True if the "enum" contains the value or a GC_Error instance otherwise.
 */
function rest_validate_enum( $value, $args, $param ) {
	$sanitized_value = rest_sanitize_value_from_schema( $value, $args, $param );
	if ( is_gc_error( $sanitized_value ) ) {
	...
	foreach ( $args['enum'] as $enum_value ) {
		$encoded_enum_values[] = is_scalar( $enum_value ) ? $enum_value : gc_json_encode( $enum_value );
	}

	if ( count( $encoded_enum_values ) === 1 ) {
		/* translators: 1: Parameter, 2: Valid values. */
		return new GC_Error( 'rest_not_in_enum', gc_sprintf( __( '%1$s不是%2$s。' ), $param, $encoded_enum_values[0] ) );
	}

	/* translators: 1: Parameter, 2: List of valid values. */
	return new GC_Error( 'rest_not_in_enum', gc_sprintf( __( '%1$s不是%2$l之一。' ), $param, $encoded_enum_values ) );
}
...
function rest_validate_value_from_schema( $value, $args, $param = '' ) {
	...
	if ( ! isset( $args['type'] ) ) {
		/* translators: %s: Parameter. */
		_doing_it_wrong( __FUNCTION__, sprintf( __( '%s必须要有“type”模式描述关键字。' ), $param ), '5.5.0' );
	}
	...
	if ( is_array( $args['type'] ) ) {
		...
		if ( ! $best_type ) {
			return new GC_Error(
				...
				sprintf( __( '%1$s的类型不是%2$s。' ), $param, implode( ',', $args['type'] ) ),
				array( 'param' => $param )
			);
		}
	...
	if ( ! in_array( $args['type'], $allowed_types, true ) ) {
		_doing_it_wrong(
			...
			gc_sprintf( __( '%1$s“type”模式描述关键字仅可以是下列内置类型之一：%2$l。' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
	...
	if ( ! empty( $args['enum'] ) ) {
		...
		if ( is_gc_error( $enum_contains_value ) ) {
		...
		switch ( $args['format'] ) {
			case 'hex-color':
				if ( ! rest_parse_hex_color( $value ) ) {
					return new GC_Error( 'rest_invalid_hex_color', __( '无效的颜色十六进制值。' ) );
				}
				break;

			case 'date-time':
				if ( ! rest_parse_date( $value ) ) {
					return new GC_Error( 'rest_invalid_date', __( '无效日期。' ) );
				}
				break;

			case 'email':
				if ( ! is_email( $value ) ) {
					return new GC_Error( 'rest_invalid_email', __( '电子邮箱无效。' ) );
				}
				break;
			case 'ip':
				if ( ! rest_is_ip_address( $value ) ) {
					/* translators: %s: IP address. */
					return new GC_Error( 'rest_invalid_ip', sprintf( __( '%s不是有效的IP地址。' ), $param ) );
				}
				break;
			case 'uuid':
				if ( ! gc_is_uuid( $value ) ) {
					/* translators: %s: The name of a JSON field expecting a valid UUID. */
					return new GC_Error( 'rest_invalid_uuid', sprintf( __( '%s不是有效的UUID。' ), $param ) );
				}
				break;
		}
	}
...
function rest_validate_null_value_from_schema( $value, $param ) {
	if ( null !== $value ) {
		return new GC_Error(
			...
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'null' ),
			array( 'param' => $param )
		);
	}
...
function rest_validate_boolean_value_from_schema( $value, $param ) {
	if ( ! rest_is_boolean( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'boolean' ),
			array( 'param' => $param )
		);
	}
...
function rest_validate_object_value_from_schema( $value, $args, $param ) {
	if ( ! rest_is_object( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'object' ),
			array( 'param' => $param )
		);
	}
		...
		foreach ( $args['required'] as $name ) {
			if ( ! array_key_exists( $name, $value ) ) {
				return new GC_Error(
					...
					sprintf( __( '%1$s是%2$s必须的属性。' ), $name, $param )
				);
			}
		}
		...
		if ( isset( $args['additionalProperties'] ) ) {
			if ( false === $args['additionalProperties'] ) {
				return new GC_Error(
					...
					sprintf( __( '%1$s不是Object的合法属性。' ), $property )
				);
			}
...
function rest_validate_array_value_from_schema( $value, $args, $param ) {
	if ( ! rest_is_array( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'array' ),
			array( 'param' => $param )
		);
	}
	...
	if ( ! empty( $args['uniqueItems'] ) && ! rest_validate_array_contains_unique_items( $value ) ) {
		/* translators: %s: Parameter. */
		return new GC_Error( 'rest_duplicate_items', sprintf( __( '%s有重复项。' ), $param ) );
	}
...
function rest_validate_number_value_from_schema( $value, $args, $param ) {
	if ( ! is_numeric( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, $args['type'] ),
			array( 'param' => $param )
		);
	}
	...
	if ( isset( $args['multipleOf'] ) && fmod( $value, $args['multipleOf'] ) !== 0.0 ) {
		return new GC_Error(
			...
			sprintf( __( '%1$s必须是%2$s的倍数。' ), $param, $args['multipleOf'] )
		);
	}
	...
	if ( isset( $args['minimum'] ) && ! isset( $args['maximum'] ) ) {
		if ( ! empty( $args['exclusiveMinimum'] ) && $value <= $args['minimum'] ) {
			return new GC_Error(
				...
				sprintf( __( '%1$s必须大于%2$d' ), $param, $args['minimum'] )
			);
		}
		...
		if ( empty( $args['exclusiveMinimum'] ) && $value < $args['minimum'] ) {
			return new GC_Error(
				'rest_out_of_bounds',
				/* translators: 1: Parameter, 2: Minimum number. */
				sprintf( __( '%1$s必须大于或等于%2$d' ), $param, $args['minimum'] )
			);
		}
	}
	...
	if ( isset( $args['maximum'] ) && ! isset( $args['minimum'] ) ) {
		if ( ! empty( $args['exclusiveMaximum'] ) && $value >= $args['maximum'] ) {
			return new GC_Error(
				...
				sprintf( __( '%1$s必须小于%2$d' ), $param, $args['maximum'] )
			);
		}
		...
		if ( empty( $args['exclusiveMaximum'] ) && $value > $args['maximum'] ) {
			return new GC_Error(
				'rest_out_of_bounds',
				/* translators: 1: Parameter, 2: Maximum number. */
				sprintf( __( '%1$s必须小于或等于%2$d' ), $param, $args['maximum'] )
			);
		}
	}
...
function rest_validate_string_value_from_schema( $value, $args, $param ) {
	if ( ! is_string( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'string' ),
			array( 'param' => $param )
		);
	}
	...
	if ( isset( $args['pattern'] ) && ! rest_validate_json_schema_pattern( $args['pattern'], $value ) ) {
		return new GC_Error(
			...
			sprintf( __( '%1$s与样板%2$s不相匹配。' ), $param, $args['pattern'] )
		);
	}
...
function rest_validate_integer_value_from_schema( $value, $args, $param ) {
	...
	if ( is_gc_error( $is_valid_number ) ) {
	...
	if ( ! rest_is_integer( $value ) ) {
		return new GC_Error(
			'rest_invalid_type',
			/* translators: 1: Parameter, 2: Type name. */
			sprintf( __( '%1$s的类型不是%2$s。' ), $param, 'integer' ),
			array( 'param' => $param )
		);
	}
 ...
 * @param string $param The parameter name, used in error messages.
 * @return mixed|GC_Error The sanitized value or a GC_Error instance if the value cannot be safely sanitized.
 */
...
function rest_preload_api_request( $memo, $path ) {
	...
	$request = new GC_REST_Request( $method, $path_parts['path'] );
...
function rest_get_route_for_taxonomy_items( $taxonomy ) {
	...
	$namespace = ! empty( $taxonomy->rest_namespace ) ? $taxonomy->rest_namespace : 'gc/v2';
...
function rest_get_queried_resource_route() {
	if ( is_singular() ) {
		...
		$route = '/gc/v2/users/' . get_queried_object_id();
	} else {
		$route = '';
	}
 ...
 *                       on `EDITABLE` endpoints. Default GC_REST_Server::CREATABLE.
 * @return array The endpoint arguments.
 */
function rest_get_endpoint_args_for_schema( $schema, $method = GC_REST_Server::CREATABLE ) {
	...
	foreach ( $schema_properties as $field_id => $params ) {
		...
		if ( GC_REST_Server::CREATABLE === $method && isset( $params['default'] ) ) {
			$endpoint_args[ $field_id ]['default'] = $params['default'];
		}

		if ( GC_REST_Server::CREATABLE === $method && ! empty( $params['required'] ) ) {
		...
		if ( isset( $params['arg_options'] ) ) {
			...
			if ( GC_REST_Server::CREATABLE !== $method ) {
...
function rest_convert_error_to_response( $error ) {
	...
	return new GC_REST_Response( $data, $status );
}
