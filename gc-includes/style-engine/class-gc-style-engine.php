<?php
/**
 * Style Engine: GC_Style_Engine class
 *
 * @package GeChiUI
...
/**
 * The main class integrating all other GC_Style_Engine_* classes.
 *
 ...
 * Please, use gc_style_engine_get_styles() instead.
 *
 * @access private
 * @since 6.1.0
...
final class GC_Style_Engine {
	...
	const BLOCK_STYLE_DEFINITIONS_METADATA = array(
		...
		'dimensions' => array(
			'minHeight' => array(
				...
				'css_vars'      => array(
					'spacing' => '--gc--preset--spacing--$slug',
				),
			),
		),
		'spacing'    => array(
	...
	protected static function get_slug_from_preset_value( $style_value, $property_key ) {
		if ( is_string( $style_value ) && is_string( $property_key )
			...
			return _gc_to_kebab_case( substr( $style_value, $index_to_splice ) );
		}
		return '';
	}

	/**
	 * Util: Generates a CSS var string, e.g. `var(--gc--preset--color--background)`
	 ...
	 * @return GC_Style_Engine_CSS_Rules_Store|null
	 */
	public static function get_store( $store_name ) {
		return GC_Style_Engine_CSS_Rules_Store::get_store( $store_name );
	}
	...
	public static function parse_block_styles( $block_styles, $options ) {
		...
		foreach ( static::BLOCK_STYLE_DEFINITIONS_METADATA as $definition_group_key => $definition_group_style ) {
			...
			foreach ( $definition_group_style as $style_definition ) {
				$style_value = _gc_array_get( $block_styles, $style_definition['path'], null );
	...
	protected static function get_css_declarations( $style_value, $style_definition, $options = array() ) {
		...
		/*
		 * Build CSS var values from `var:preset|<PRESET_TYPE>|<PRESET_SLUG>` values, e.g, `var(--gc--css--rule-slug )`.
		...
		if ( is_array( $style_value ) ) {
			...
			foreach ( $style_value as $key => $value ) {
				...
				$individual_property = sprintf( $style_property_keys['individual'], _gc_to_kebab_case( $key ) );
	...
	protected static function get_individual_property_css_declarations( $style_value, $individual_property_definition, $options = array() ) {
		...
		foreach ( $style_value as $css_property => $value ) {
			...
			$style_definition      = _gc_array_get( static::BLOCK_STYLE_DEFINITIONS_METADATA, $style_definition_path, null );
	...
	public static function compile_css( $css_declarations, $css_selector ) {
		...
		if ( $css_selector ) {
			$css_rule = new GC_Style_Engine_CSS_Rule( $css_selector, $css_declarations );
			return $css_rule->get_css();
		}

		$css_declarations = new GC_Style_Engine_CSS_Declarations( $css_declarations );
	 ...
	 * @since 6.1.0
	 *
	 * @param GC_Style_Engine_CSS_Rule[] $css_rules An array of GC_Style_Engine_CSS_Rule objects
	...
	public static function compile_stylesheet_from_css_rules( $css_rules, $options = array() ) {
		$processor = new GC_Style_Engine_Processor();
