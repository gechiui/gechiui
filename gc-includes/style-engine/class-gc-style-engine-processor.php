<?php
/**
 * Style Engine: GC_Style_Engine_Processor class
 *
 * @package GeChiUI
...
class GC_Style_Engine_Processor {

	/**
	 * A collection of Style Engine Store objects.
	 *
	 * @since 6.1.0
	 * @var GC_Style_Engine_CSS_Rules_Store[]
	 */
	 ...
	 * @since 6.1.0
	 *
	 * @param GC_Style_Engine_CSS_Rules_Store $store The store to add.
	 * @return GC_Style_Engine_Processor Returns the object to allow chaining methods.
	 */
	public function add_store( $store ) {
		if ( ! $store instanceof GC_Style_Engine_CSS_Rules_Store ) {
			_doing_it_wrong(
				__METHOD__,
				__( '$store 必须是 GC_Style_Engine_CSS_Rules_Store 的一个实例' ),
				'6.1.0'
			);
			return $this;
		}
	 ...
	 * @since 6.1.0
	 *
	 * @param GC_Style_Engine_CSS_Rule|GC_Style_Engine_CSS_Rule[] $css_rules A single, or an array of,
	 *                                                                       GC_Style_Engine_CSS_Rule objects
	...
	private function combine_rules_selectors() {
		...
		foreach ( $this->css_rules as $rule ) {
			...
			ksort( $declarations );
			$selectors_json[ $rule->get_selector() ] = gc_json_encode( $declarations );
		}
		...
		foreach ( $selectors_json as $selector => $json ) {
			...
			$this->css_rules[ $duplicate_selectors ] = new GC_Style_Engine_CSS_Rule( $duplicate_selectors, $declarations );
		}
	}
}
