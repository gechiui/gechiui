 ...
 *
 * GC_Meta_Query is a helper that allows primary query classes, such as GC_Query and GC_User_Query,
 *
...
class GC_Meta_Query {
	...
	public function parse_query_vars( $qv ) {
		...
		// GC_Query sets 'meta_value' = '' by default.
	 ...
	 * @param string $primary_table     Database table where the object being filtered is stored (eg gc_users).
		...
		if ( false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( 'LIKE' === $meta_compare_key ) {
					$join .= $gcdb->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key LIKE %s )", '%' . $gcdb->esc_like( $clause['key'] ) . '%' );
				} else {
					$join .= $gcdb->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key = %s )", $clause['key'] );
				}
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( in_array( $meta_compare_key, array( '!=', 'NOT IN', 'NOT LIKE', 'NOT EXISTS', 'NOT REGEXP' ), true ) ) {
					...
					$meta_compare_string_start .= "SELECT 1 FROM $gcdb->postmeta $subquery_alias ";
				...
				switch ( $meta_compare_key ) {
					case '=':
					case 'EXISTS':
						$where = $gcdb->prepare( "$alias.meta_key = %s", trim( $clause['key'] ) ); // phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
						break;
					case 'LIKE':
						$meta_compare_value = '%' . $gcdb->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $gcdb->prepare( "$alias.meta_key LIKE %s", $meta_compare_value ); // phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
						break;
					case 'IN':
					...
					case 'NOT LIKE':
						...
						$meta_compare_value = '%' . $gcdb->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $gcdb->prepare( $meta_compare_string, $meta_compare_value ); // phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
						break;
					case 'NOT IN':
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $meta_compare ) {
				...
				case 'NOT IN':
					...
					$where               = $gcdb->prepare( $meta_compare_string, $meta_value );
					break;

				case 'BETWEEN':
				case 'NOT BETWEEN':
					$where = $gcdb->prepare( '%s AND %s', $meta_value[0], $meta_value[1] );
					break;

				case 'LIKE':
				case 'NOT LIKE':
					$meta_value = '%' . $gcdb->esc_like( $meta_value ) . '%';
					$where      = $gcdb->prepare( '%s', $meta_value );
					break;
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param GC_Meta_Query $query        GC_Meta_Query object.
		 */
