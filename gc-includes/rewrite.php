 ...
 *                       see GC_Rewrite::add_permastruct() for full details. Default empty array.
 */
 ...
 * @return string
 */
function _gc_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, gc_unique_post_slug() prevents the creation of post slugs
 ...
 */
function gc_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof GC_Post ) {
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $gc->public_query_vars, true ) ) {
					$query[ $key ] = $value;
			...
			$query = gc_resolve_numeric_slug_conflicts( $query );

			// Do the query.
			$query = new GC_Query( $query );
