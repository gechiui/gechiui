...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'gc-custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'gc-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'gc-custom-header-video-button' );
			button.setAttribute( 'class', 'gc-custom-header-video-button gc-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'gc-custom-header-video-button gc-custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window.gc ) {
					window.gc.a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'gc-custom-header-video-button gc-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.gc ) {
					window.gc.a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	 ...
	 * @class
	 */
	NativeHandler = BaseHandler.extend(/** @lends gc.NativeHandler.prototype */{
		/**
		...
		ready: function() {
			...
			video.id = 'gc-custom-header-video';
			video.autoplay = true;
			video.loop = true;
			video.muted = true;
	...
	window.gc.customHeader = new CustomHeader();
	document.addEventListener( 'DOMContentLoaded', window.gc.customHeader.initialize.bind( window.gc.customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window.gc ) {
		window.gc.customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window.gc.customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window.gc.customHeader.initialize();
			}
		});
	}

})( window, window._gcCustomHeaderSettings || {} );
