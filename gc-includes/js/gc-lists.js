...
gcList = {
	 ...
	 * 4. `_gcnonce` value in element's href attribute.
	 * 5. `_gcnonce` input field that is a descendant of element.
	...
	nonce: function( element, settings ) {
		var url      = gcAjax.unserialize( element.attr( 'href' ) ),
			$element = $( '#' + settings.element );

		return settings.nonce || url._ajax_nonce || $element.find( 'input[name="_ajax_nonce"]' ).val() || url._gcnonce || $element.find( 'input[name="_gcnonce"]' ).val() || 0;
	},

	/**
	 * Extract list item data from a DOM element.
	 *
	 * Example 1: data-gc-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-gc-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		var data = [], gcListsData;

		try {
			gcListsData = $( element ).data( 'gc-lists' ) || '';
			gcListsData = gcListsData.match( new RegExp( type + ':[\\S]+' ) );

			if ( gcListsData ) {
				data = gcListsData[0].split( ':' );
			}
		} catch ( error ) {}
	...
	pre: function( list, settings, action ) {
		...
		settings = $.extend( {}, this.gcList.settings, {
			element: null,
			nonce:   0,
			target:  list.get( 0 )
		}, settings || {} );
	...
	ajaxAdd: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = gcList.parseData( $element, 'add' ),
			formValues, formData, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = gcList.pre.call( list, $element, settings, 'add' );
		...
		if ( ! $element.is( '[id="' + settings.element + '-submit"]' ) ) {
			return ! gcList.add.call( list, $element, settings );
		}
		...
		settings.nonce  = gcList.nonce( $element, settings );

		if ( ! gcAjax.validateForm( '#' + settings.element ) ) {
			return false;
		}
		...
		}, gcAjax.unserialize( data[4] || '' ) ) );

		formValues = $( '#' + settings.element + ' :input' ).not( '[name="_ajax_nonce"], [name="_gcnonce"], [name="action"]' );
		...
		settings.success = function( response ) {
			parsedResponse   = gcAjax.parseAjaxResponse( response, settings.response, settings.element );
			...
			$.each( parsedResponse.responses, function() {
				gcList.add.call( list, this.data, $.extend( {}, settings, { // this.firstChild.nodevalue
			...
			list.gcList.recolor();
			$( list ).trigger( 'gcListAddEnd', [ settings, list.gcList ] );
			gcList.clear.call( list, '#' + settings.element );
		};
	...
	ajaxDel: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = gcList.parseData( $element, 'delete' ),
			$eventTarget, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = gcList.pre.call( list, $element, settings, 'delete' );
		...
		if ( 'none' !== settings.delColor ) {
			$eventTarget.css( 'backgroundColor', settings.delColor ).fadeOut( 350, function() {
				list.gcList.recolor();
				$( list ).trigger( 'gcListDelEnd', [ settings, list.gcList ] );
			} );
		} else {
			list.gcList.recolor();
			$( list ).trigger( 'gcListDelEnd', [ settings, list.gcList ] );
	...
	ajaxDim: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = gcList.parseData( $element, 'dim' ),
		...
		settings = settings || {};
		settings = gcList.pre.call( list, $element, settings, 'dim' );
		...
		color        = gcList.getColor( $eventTarget );
			...
			$eventTarget
				...
				.animate( { backgroundColor: color }, {
					complete: function() {
						...
						$( list ).trigger( 'gcListDimEnd', [ settings, list.gcList ] );
					}
				} );
		} else {
			$( list ).trigger( 'gcListDimEnd', [ settings, list.gcList ] );
	...
	add: function( element, settings ) {
		...
		settings = $.extend( { position: 0, id: 0, oldId: null }, this.gcList.settings, settings );
		...
		if ( 'none' !== settings.addColor ) {
			$element.css( 'backgroundColor', settings.addColor ).animate( { backgroundColor: gcList.getColor( $element ) }, {
				complete: function() {
					$( this ).css( 'backgroundColor', '' );
				}
			} );
	...
	clear: function( elementId ) {
		...
		if ( list.gcList && $element.parents( '#' + list.id ).length ) {
			return;
		}
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-gc-lists^="add:' + list.id + ':"]', function() {
		...
		$element.on( 'click', '[data-gc-lists^="delete:' + list.id + ':"]', function() {
			return list.gcList.del( this );
		} );

		$element.on( 'click', '[data-gc-lists^="dim:' + list.id + ':"]', function() {
			return list.gcList.dim( this );
		} );
	},
	...
	recolor: function() {
		...
		if ( list.gcList.settings.altOffset % 2 ) {
			evenOdd.reverse();
		}

		items.filter( evenOdd[0] ).addClass( list.gcList.settings.alt ).end();
		items.filter( evenOdd[1] ).removeClass( list.gcList.settings.alt );
	},
	...
	init: function() {
		...
		$list.gcList.process = function( element ) {
			$list.each( function() {
				this.gcList.process( element );
			} );
		};

		$list.gcList.recolor = function() {
			$list.each( function() {
				this.gcList.recolor();
			} );
		};
 ...
 *
 * @param {gcList~confirm}   settings.confirm     Callback that's run before a request is made. Default: null.
 * @param {gcList~addBefore} settings.addBefore   Callback that's run before an item gets added to the list.
 *                                                Default: null.
 * @param {gcList~addAfter}  settings.addAfter    Callback that's run after an item got added to the list.
 *                                                Default: null.
 * @param {gcList~delBefore} settings.delBefore   Callback that's run before an item gets deleted from the list.
 *                                                Default: null.
 * @param {gcList~delAfter}  settings.delAfter    Callback that's run after an item got deleted from the list.
 *                                                Default: null.
 * @param {gcList~dimBefore} settings.dimBefore   Callback that's run before an item gets dim'd. Default: null.
 * @param {gcList~dimAfter}  settings.dimAfter    Callback that's run after an item got dim'd. Default: null.
 * @return {$.fn} gcList API function.
 */
$.fn.gcList = function( settings ) {
	this.each( function( index, list ) {
		list.gcList = {
			settings: $.extend( {}, gcList.settings, { what: gcList.parseData( list, 'list' )[1] || '' }, settings )
		};

		$.each( functions, function( func, callback ) {
			list.gcList[func] = function( element, setting ) {
				return gcList[callback].call( list, element, setting );
			};
		} );
	} );

	gcList.init.call( this );
	this.gcList.process();
