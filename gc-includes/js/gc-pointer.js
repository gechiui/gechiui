/**
 * @output gc-includes/js/gc-pointer.js
 */

/**
 * Initializes the gc-pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;

	$.widget('gc.pointer',/** @lends $.widget.gc.pointer.prototype */{
		options: {
			pointerClass: 'gc-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var button = $('<a class="close" href="#"></a>').text( gc.i18n.__( '不再显示' ) );
		...
		/**
		 * A class that represents a GeChiUI pointer.
		 *
		 * @private
		 *
		 * @constructs $.widget.gc.pointer
		 */
		_create: function() {
			var positioning,
				family;

			this.content = $('<div class="gc-pointer-content"></div>');
			this.arrow   = $('<div class="gc-pointer-arrow"><div class="gc-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="gc-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /gc-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'gc-pointer-' + edge );
		},
