...
/******/ 	return __webpack_require__(__webpack_require__.s = "tpFy");
/******/ })
/************************************************************************/
/******/ ({
...
const getNewBlockTypes = Object(external_gc_data_["createRegistrySelector"])(select => state => {
  const usedBlockTree = select(external_gc_blockEditor_["store"]).getBlocks();
...
const getUnusedBlockTypes = Object(external_gc_data_["createRegistrySelector"])(select => state => {
  const usedBlockTree = select(external_gc_blockEditor_["store"]).getBlocks();
  const installedBlockTypes = getInstalledBlockTypes(state);
  return installedBlockTypes.filter(blockType => !hasBlockType(blockType, usedBlockTree));
});
...
const actions_installBlockType = block => async _ref => {
  ...
  try {
    ...
    await loadAssets();
    const registeredBlocks = registry.select(external_gc_blocks_["store"]).getBlockTypes();

    if (!registeredBlocks.some(i => i.name === block.name)) {
      throw new Error(Object(external_gc_i18n_["__"])('注册区块出错，尝试重新加载页面。'));
    }

    registry.dispatch(external_gc_notices_["store"]).createInfoNotice(Object(external_gc_i18n_["sprintf"])( // translators: %s is the block title.
    Object(external_gc_i18n_["__"])('区块%s已安装并添加。'), block.title), {
      speak: true,
      type: 'snackbar'
    });
    success = true;
  } catch (error) {
    let message = error.message || Object(external_gc_i18n_["__"])('发生了一个错误。'); // Errors we throw are fatal


    let isFatal = error instanceof Error; // Specific API errors that are fatal

    const fatalAPIErrors = {
      folder_exists: Object(external_gc_i18n_["__"])('该区块已安装。尝试重新加载页面。'),
      unable_to_connect_to_filesystem: Object(external_gc_i18n_["__"])('安装区块时出错，您可以重新加载页面并重试。')
    };
    ...
    registry.dispatch(external_gc_notices_["store"]).createErrorNotice(message, {
      speak: true,
      isDismissible: true
    });
  }
...
const actions_uninstallBlockType = block => async _ref2 => {
  ...
  try {
    ...
    registry.dispatch(external_gc_notices_["store"]).createErrorNotice(error.message || Object(external_gc_i18n_["__"])('发生了一个错误。'));
  }
};
/**
...
function AutoBlockUninstaller() {
  ...
  const shouldRemoveBlockTypes = Object(external_gc_data_["useSelect"])(select => {
    const {
      isAutosavingPost,
      isSavingPost
  ...
  const unusedBlockTypes = Object(external_gc_data_["useSelect"])(select => select(store).getUnusedBlockTypes(), []);
  Object(external_gc_element_["useEffect"])(() => {
    if (shouldRemoveBlockTypes && unusedBlockTypes.length) {
      unusedBlockTypes.forEach(blockType => {
        uninstallBlockType(blockType);
        Object(external_gc_blocks_["unregisterBlockType"])(blockType.name);
      });
    }
...
const starHalf = Object(external_gc_element_["createElement"])(external_gc_primitives_["SVG"], {
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24"
...
function Stars(_ref) {
  ...
  return Object(external_gc_element_["createElement"])("span", {
    ...
    Object(external_gc_i18n_["__"])('五星中有%s星'), stars)
  }, Object(external_lodash_["times"])(fullStarCount, i => Object(external_gc_element_["createElement"])(build_module_icon["a" /* default */], {
    key: `full_stars_${i}`,
    className: "block-directory-block-ratings__star-full",
    icon: star_filled["a" /* default */],
    size: 16
  })), Object(external_lodash_["times"])(halfStarCount, i => Object(external_gc_element_["createElement"])(build_module_icon["a" /* default */], {
    key: `half_stars_${i}`,
    className: "block-directory-block-ratings__star-half-full",
    icon: star_half,
    size: 16
  })), Object(external_lodash_["times"])(emptyStarCount, i => Object(external_gc_element_["createElement"])(build_module_icon["a" /* default */], {
    key: `empty_stars_${i}`,
...
const BlockRatings = _ref => {
  ...
  }, Object(external_gc_element_["createElement"])(block_ratings_stars, {
    rating: rating
  }));
};
...
function DownloadableBlockIcon(_ref) {
  ...
  return icon.match(/\.(jpeg|jpg|gif|png|svg)(?:\?.*)?$/) !== null ? Object(external_gc_element_["createElement"])("img", {
    className: className,
    src: icon,
    alt: ""
...
const DownloadableBlockNotice = _ref => {
  ...
  const errorNotice = Object(external_gc_data_["useSelect"])(select => select(store).getErrorNoticeForBlock(block.id), [block]);
  ...
  }, errorNotice.message, errorNotice.isFatal ? ' ' + Object(external_gc_i18n_["__"])('尝试重新加载页面。') : null));
};
...
function getDownloadableBlockLabel(_ref, _ref2) {
  ...
  if (!isInstalled && hasNotice) {
    ...
    return Object(external_gc_i18n_["sprintf"])('Retry installing %s.', Object(external_gc_htmlEntities_["decodeEntities"])(title));
  }

  if (isInstalled) {
    /* translators: %1$s: block title */
    return Object(external_gc_i18n_["sprintf"])('Add %s.', Object(external_gc_htmlEntities_["decodeEntities"])(title));
  }

  if (isInstalling) {
    /* translators: %1$s: block title */
    return Object(external_gc_i18n_["sprintf"])('Installing %s.', Object(external_gc_htmlEntities_["decodeEntities"])(title));
  } // No ratings yet, just use the title.


  if (ratingCount < 1) {
    /* translators: %1$s: block title */
    return Object(external_gc_i18n_["sprintf"])('Install %s.', Object(external_gc_htmlEntities_["decodeEntities"])(title));
  }
...
function DownloadableBlockListItem(_ref3) {
  ...
  const isInstalled = !!Object(external_gc_blocks_["getBlockType"])(item.name);
  const {
    hasNotice,
    isInstalling,
    isInstallable
  ...
  if (isInstalled) {
    statusText = Object(external_gc_i18n_["__"])('已安装！');
  } else if (isInstalling) {
    statusText = Object(external_gc_i18n_["__"])('正在安装…');
  }
  ...
  }, Object(external_gc_element_["createElement"])(downloadable_block_icon, {
    icon: icon,
    title: title
  }), isInstalling ? Object(external_gc_element_["createElement"])("span", {
    className: "block-directory-downloadable-block-list-item__spinner"
  }, Object(external_gc_element_["createElement"])(external_gc_components_["Spinner"], null)) : Object(external_gc_element_["createElement"])(block_ratings, {
    rating: rating
  ...
  }, Object(external_gc_element_["createInterpolateElement"])(Object(external_gc_i18n_["sprintf"])(
  /* translators: %1$s: block title, %2$s: author name. */
  Object(external_gc_i18n_["__"])('%1$s <span>作者%2$s</span>'), Object(external_gc_htmlEntities_["decodeEntities"])(title), author), {
    span: Object(external_gc_element_["createElement"])("span", {
      className: "block-directory-downloadable-block-list-item__author"
    })
  })), hasNotice ? Object(external_gc_element_["createElement"])(downloadable_block_notice, {
    block: item
  }) : Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])("span", {
    className: "block-directory-downloadable-block-list-item__desc"
  }, !!statusText ? statusText : Object(external_gc_htmlEntities_["decodeEntities"])(description)), isInstallable && !(isInstalled || isInstalling) && Object(external_gc_element_["createElement"])(external_gc_components_["VisuallyHidden"], null, Object(external_gc_i18n_["__"])('安装区块')))));
}
...
function DownloadableBlocksList(_ref) {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["__unstableComposite"], Object(esm_extends["a" /* default */])({}, composite, {
    ...
    "aria-label": Object(external_gc_i18n_["__"])('可供安装的区块')
  }), items.map(item => {
    return Object(external_gc_element_["createElement"])(downloadable_block_list_item, {
      key: item.id,
      composite: composite,
      onClick: () => {
        ...
        if (Object(external_gc_blocks_["getBlockType"])(item.name)) {
          onSelect(item);
        } else {
...
function DownloadableBlocksInserterPanel(_ref) {
  ...
  Object(external_gc_element_["useEffect"])(() => {
    ...
    Object(external_gc_i18n_["_n"])('%d个区块可供安装。', '%d additional blocks are available to install.', count), count));
  }, [count]);
  return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, !hasLocalBlocks && Object(external_gc_element_["createElement"])("p", {
    className: "block-directory-downloadable-blocks-panel__no-local"
  }, Object(external_gc_i18n_["__"])('未在您安装的区块中找到结果。')), Object(external_gc_element_["createElement"])("div", {
  ...
  }, Object(external_gc_i18n_["__"])('可供安装')), Object(external_gc_element_["createElement"])("p", {
    className: "block-directory-downloadable-blocks-panel__description"
  }, Object(external_gc_i18n_["__"])('选择一个区块进行安装，并将其添加到您的文章中。'))), children));
}
...
function DownloadableBlocksNoResults() {
  ...
  }), Object(external_gc_element_["createElement"])("p", null, Object(external_gc_i18n_["__"])('未找到结果。')));
}
...
function DownloadableBlocksPanel(_ref) {
  ...
  if (typeof hasPermission === 'undefined' || isLoading || isTyping) {
    return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, hasPermission && !hasLocalBlocks && Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])("p", {
      className: "block-directory-downloadable-blocks-panel__no-local"
    }, Object(external_gc_i18n_["__"])('未在您安装的区块中找到结果。')), Object(external_gc_element_["createElement"])("div", {
      className: "block-editor-inserter__quick-inserter-separator"
    })), Object(external_gc_element_["createElement"])("div", {
      className: "block-directory-downloadable-blocks-panel has-blocks-loading"
    }, Object(external_gc_element_["createElement"])(external_gc_components_["Spinner"], null)));
  }

  if (false === hasPermission) {
    if (!hasLocalBlocks) {
      return Object(external_gc_element_["createElement"])(no_results, null);
    }

    return null;
  }

  return !!downloadableItems.length ? Object(external_gc_element_["createElement"])(inserter_panel, {
    downloadableItems: downloadableItems,
    hasLocalBlocks: hasLocalBlocks
  }, Object(external_gc_element_["createElement"])(downloadable_blocks_list, {
    items: downloadableItems,
    onSelect: onSelect,
    onHover: onHover
  })) : !hasLocalBlocks && Object(external_gc_element_["createElement"])(no_results, null);
}

/* harmony default export */ var downloadable_blocks_panel = (Object(external_gc_compose_["compose"])([Object(external_gc_data_["withSelect"])((select, _ref2) => {
  let {
    filterValue,
    rootClientId = null
  } = _ref2;
  ...
  const hasPermission = select(external_gc_coreData_["store"]).canUser('read', 'block-directory/search');
...
function InserterMenuDownloadableBlocksPanel() {
  const [debouncedFilterValue, setFilterValue] = Object(external_gc_element_["useState"])('');
  const debouncedSetFilterValue = Object(external_lodash_["debounce"])(setFilterValue, 400);
  return Object(external_gc_element_["createElement"])(external_gc_blockEditor_["__unstableInserterMenuExtension"], null, _ref => {
    let {
      onSelect,
      onHover,
      filterValue,
    ...
    return Object(external_gc_element_["createElement"])(downloadable_blocks_panel, {
      onSelect: onSelect,
      onHover: onHover,
...
// EXTERNAL MODULE: external ["gc","editPost"]
var external_gc_editPost_ = __webpack_require__("Qylf");
...
function CompactList(_ref) {
  ...
  return Object(external_gc_element_["createElement"])("ul", {
    ...
    Object(external_gc_i18n_["__"])('作者：%s'), author))));
  }));
}
...
function InstalledBlocksPrePublishPanel() {
  const newBlockTypes = Object(external_gc_data_["useSelect"])(select => select(store).getNewBlockTypes(), []);
...
function InstallButton(_ref) {
  ...
  const isInstallingBlock = Object(external_gc_data_["useSelect"])(select => select(store).isInstalling(block.id), [block.id]);
  const {
    installBlockType
  } = Object(external_gc_data_["useDispatch"])(store);
  const {
  ...
  return Object(external_gc_element_["createElement"])(external_gc_components_["Button"], {
    onClick: () => installBlockType(block).then(success => {
      if (success) {
        ...
        const [originalBlock] = Object(external_gc_blocks_["parse"])(attributes.originalContent);

        if (originalBlock && blockType) {
          replaceBlock(clientId, Object(external_gc_blocks_["createBlock"])(blockType.name, originalBlock.attributes, originalBlock.innerBlocks));
        }
      }
    }),
  ...
  Object(external_gc_i18n_["__"])('安装 %s'), block.title));
}
...
const getInstallMissing = OriginalComponent => props => {
  ...
  const {
    ...
    return {
      hasPermission: select(external_gc_coreData_["store"]).canUser('read', 'block-directory/search'),
      block: blocks.length && blocks[0]
    };
  }, [originalName]); // The user can't install blocks, or the block isn't available for download.

  if (!hasPermission || !block) {
    return Object(external_gc_element_["createElement"])(OriginalComponent, props);
  }

  return Object(external_gc_element_["createElement"])(ModifiedWarning, Object(esm_extends["a" /* default */])({}, props, {
    originalBlock: block
  }));
};
...
const ModifiedWarning = _ref2 => {
  ...
  const convertToHTML = () => {
    replaceBlock(props.clientId, Object(external_gc_blocks_["createBlock"])('core/html', {
  ...
  let messageHTML = Object(external_gc_i18n_["sprintf"])(
  /* translators: %s: block name */
  Object(external_gc_i18n_["__"])('您的站点不支持 %s区块。您可以尝试安装该区块或完全将其移除!'), originalBlock.title || originalName);
  const actions = [Object(external_gc_element_["createElement"])(InstallButton, {
    key: "install",
    block: originalBlock,
  ...
  if (hasContent && hasHTMLBlock) {
    ...
    /* translators: %s: block name */
    Object(external_gc_i18n_["__"])('您的站点不支持%s区块。您可以尝试安装该区块并将其转换为自定义的HTML区块，或者将其彻底移除。'), originalBlock.title || originalName);
  ...
  return Object(external_gc_element_["createElement"])("div", Object(external_gc_blockEditor_["useBlockProps"])(), Object(external_gc_element_["createElement"])(external_gc_blockEditor_["Warning"], {
    actions: actions
  }, messageHTML), Object(external_gc_element_["createElement"])(external_gc_element_["RawHTML"], null, originalUndelimitedContent));
};
...
Object(external_gc_plugins_["registerPlugin"])('block-directory', {
  render() {
    return Object(external_gc_element_["createElement"])(external_gc_element_["Fragment"], null, Object(external_gc_element_["createElement"])(AutoBlockUninstaller, null), Object(external_gc_element_["createElement"])(inserter_menu_downloadable_blocks_panel, null), Object(external_gc_element_["createElement"])(InstalledBlocksPrePublishPanel, null));
  }

});
Object(external_gc_hooks_["addFilter"])('blocks.registerBlockType', 'block-directory/fallback', (settings, name) => {
