...
/******/ 	return __webpack_require__(__webpack_require__.s = "Fja4");
/******/ })
/************************************************************************/
/******/ ({
...
const deleteEntityRecord = function (kind, name, recordId, query) {
  let {
    __unstableFetch = external_gc_apiFetch_default.a
  ...
  return async _ref => {
    ...
    try {
      ...
      try {
        ...
        if (query) {
          path = Object(external_gc_url_["addQueryArgs"])(path, query);
        }
...
const defaultEntities = [{
  label: Object(external_gc_i18n_["__"])('根目录（Base）'),
  name: '__unstableBase',
  kind: 'root',
  baseURL: '/'
}, {
  label: Object(external_gc_i18n_["__"])('站点'),
  name: 'site',
  kind: 'root',
  baseURL: '/gc/v2/settings',
  getTitle: record => {
    return Object(external_lodash_["get"])(record, ['title'], Object(external_gc_i18n_["__"])('站点标题'));
  }
}, {
  ...
  plural: 'mediaItems',
  label: Object(external_gc_i18n_["__"])('媒体')
}, {
  name: 'taxonomy',
  kind: 'root',
  key: 'slug',
  ...
  label: Object(external_gc_i18n_["__"])('小工具区域')
}, {
  name: 'widget',
  kind: 'root',
  ...
  label: Object(external_gc_i18n_["__"])('小工具')
}, {
  name: 'widgetType',
  kind: 'root',
  ...
  plural: 'widgetTypes',
  label: Object(external_gc_i18n_["__"])('小工具类型')
}, {
  label: Object(external_gc_i18n_["__"])('用户'),
  name: 'user',
  kind: 'root',
  baseURL: '/gc/v2/users',
  baseURLParams: {
  ...
  plural: 'comments',
  label: Object(external_gc_i18n_["__"])('评论')
}, {
  name: 'menu',
  kind: 'root',
  baseURL: '/gc/v2/menus',
  ...
  plural: 'menuItems',
  label: Object(external_gc_i18n_["__"])('菜单项'),
  ...
  label: Object(external_gc_i18n_["__"])('菜单位置'),
  key: 'name'
}, {
  name: 'navigationArea',
  kind: 'root',
  ...
  label: Object(external_gc_i18n_["__"])('导航区'),
  key: 'name',
  getTitle: record => record === null || record === void 0 ? void 0 : record.description
}, {
  label: Object(external_gc_i18n_["__"])('全局样式'),
  name: 'globalStyles',
  kind: 'root',
  ...
  label: Object(external_gc_i18n_["__"])('主题'),
  name: 'theme',
  kind: 'root',
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    switch (key) {
      ...
      default:
        ...
        parts.stableKey += (parts.stableKey ? '&' : '') + Object(external_gc_url_["addQueryArgs"])('', {
          [key]: value
        }).slice(1);
    }
  }
...
function reducer_entity(entityConfig) {
  ...
  })])(Object(external_gc_data_["combineReducers"])({
    queriedData: reducer,
    edits: function () {
...
const reducer_entities = function () {
  ...
  if (!entitiesDataReducer || newConfig !== state.config) {
    ...
    entitiesDataReducer = Object(external_gc_data_["combineReducers"])(Object.entries(entitiesByKind).reduce((memo, _ref) => {
      let [kind, subEntities] = _ref;
      const kindReducer = Object(external_gc_data_["combineReducers"])(subEntities.reduce((kindMemo, entityConfig) => ({ ...kindMemo,
...
/* harmony default export */ var build_module_reducer = (Object(external_gc_data_["combineReducers"])({
  terms,
  users: reducer_users,
...
const isRequestingEmbedPreview = Object(external_gc_data_["createRegistrySelector"])(select => (state, url) => {
...
function getAuthors(state, query) {
  ...
  const path = Object(external_gc_url_["addQueryArgs"])('/gc/v2/users/?who=authors&per_page=100', query);
...
const hasFetchedAutosaves = Object(external_gc_data_["createRegistrySelector"])(select => (state, postType, postId) => {
...
function __experimentalGetTemplateForLink(state, link) {
  ...
  if (template) {
    return selectors_getEditedEntityRecord(state, 'postType', 'gc_template', template.id);
  }
...
const resolvers_getEntityRecord = function (kind, name) {
  ...
  return async _ref3 => {
    ...
    try {
      ...
      const path = Object(external_gc_url_["addQueryArgs"])(entity.baseURL + (key ? '/' + key : ''), { ...entity.baseURLParams,
        ...query
      });
...
const resolvers_getEntityRecords = function (kind, name) {
  ...
  return async _ref4 => {
    ...
    try {
      ...
      const path = Object(external_gc_url_["addQueryArgs"])(entity.baseURL, { ...entity.baseURLParams,
        ...query
      });
...
const resolvers_getEmbedPreview = url => async _ref6 => {
  ...
  try {
    const embedProxyResponse = await external_gc_apiFetch_default()({
      path: Object(external_gc_url_["addQueryArgs"])('/oembed/1.0/proxy', {
        url
      })
    });
...
const resolvers_experimentalGetTemplateForLink = link => async _ref11 => {
  ...
  try {
    template = await window.fetch(Object(external_gc_url_["addQueryArgs"])(link, {
...
const resolvers_experimentalGetCurrentGlobalStylesId = () => async _ref13 => {
  ...
  const globalStylesURL = Object(external_lodash_["get"])(activeThemes, [0, '_links', 'gc:user-global-styles', 0, 'href']);
...
const entity_provider_entities = { ...defaultEntities.reduce((acc, entity) => {
    ...
    acc[entity.kind][entity.name] = {
      context: Object(external_gc_element_["createContext"])()
    };
    return acc;
  }, {}),
...
function useEntityId(kind, type) {
  return Object(external_gc_element_["useContext"])(entity_provider_getEntity(kind, type).context);
}
/**
...
function useEntityProp(kind, type, prop, _id) {
  ...
  const {
    editEntityRecord
  } = Object(external_gc_data_["useDispatch"])(STORE_NAME);
  const setValue = Object(external_gc_element_["useCallback"])(newValue => {
...
function useEntityBlockEditor(kind, type) {
  ...
  Object(external_gc_element_["useEffect"])(() => {
    ...
    if (content && typeof content !== 'function' && !blocks) {
      const parsedContent = Object(external_gc_blocks_["parse"])(content);
      editEntityRecord(kind, type, id, {
  ...
  const onChange = Object(external_gc_element_["useCallback"])((newBlocks, options) => {
    const {
      selection
    } = options;
    const edits = {
  ...
  const onInput = Object(external_gc_element_["useCallback"])((newBlocks, options) => {
    const {
      selection
    } = options;
    const edits = {
...
const fetchLinkSuggestions = async function (search) {
  ...
  if (!type || type === 'post') {
    queries.push(external_gc_apiFetch_default()({
      path: Object(external_gc_url_["addQueryArgs"])('/gc/v2/search', {
        search,
        page,
        per_page: perPage,
        type: 'post',
  ...
  return Promise.all(queries).then(results => {
    ...
    result => {
      ...
      return {
        id: result.id,
        url: result.url,
        title: Object(external_gc_htmlEntities_["decodeEntities"])(result.title || '') || Object(external_gc_i18n_["__"])('（无标题）'),
...
const fetchUrlData = async function (url) {
  ...
  const args = {
    url: Object(external_gc_url_["prependHTTP"])(url)
  };

  if (!Object(external_gc_url_["isURL"])(url)) {
    return Promise.reject(`${url} is not a valid URL.`);
  } // Test for "http" based URL as it is possible for valid
  // yet unusable URLs such as `tel:123456` to be passed.


  const protocol = Object(external_gc_url_["getProtocol"])(url);

  if (!Object(external_gc_url_["isValidProtocol"])(protocol) || !protocol.startsWith('http') || !/^https?:\/\/[^\/\s]/i.test(url)) {
  ...
  return external_gc_apiFetch_default()({
    path: Object(external_gc_url_["addQueryArgs"])(endpoint, args),
    ...options
  }).then(res => {
    CACHE.set(url, res);
    return res;
...
const build_module_store = Object(external_gc_data_["createReduxStore"])(STORE_NAME, storeConfig());
Object(external_gc_data_["register"])(build_module_store);
