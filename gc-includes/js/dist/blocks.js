...
function switchToBlockType(blocks, name) {
  ...
  const ret = transformationResults.map(result => {
     ...
     */
    return Object(external_gc_hooks_["applyFilters"])('blocks.switchToBlockType.transformedBlock', result, blocks);
  });
  return ret;
}
/**
...
function processBlockType(blockType) {
  ...
  if ('category' in settings && !Object(external_lodash_["some"])(Object(external_gc_data_["select"])(STORE_NAME).getCategories(), {
...
function* __experimentalReapplyBlockTypeFilters() {
  const unprocessedBlockTypes = Object(external_gc_data_["select"])(STORE_NAME).__experimentalGetUnprocessedBlockTypes();
