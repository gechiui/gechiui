...
final class GC_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.gc-menu-image empty
	...
	public function add_rewrite_rules() {
		...
		if ( false !== $this->query_var && $gc && is_post_type_viewable( $this ) ) {
		...
		if ( false !== $this->rewrite && ( is_admin() || get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $gc_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $gc_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $gc_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $gc_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$gc_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_gc_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $gc, $gc_rewrite, $post_type_meta_caps;
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : GC_REST_Posts_Controller::class;
