...
function gcmu_update_blogs_date() {
	 ...
	 */
	do_action( 'gcmu_blog_updated', $site_id );
}
...
function get_blog_details( $fields = null, $get_all = true ) {
	...
	if ( is_array( $fields ) ) {
		if ( isset( $fields['blog_id'] ) ) {
			...
			$blog = gc_cache_get( $key, 'blog-lookup' );
			if ( false !== $blog ) {
				return $blog;
			}
			if ( 'www.' === substr( $fields['domain'], 0, 4 ) ) {
				...
				$blog = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->blogs WHERE domain = %s AND path = %s", $fields['domain'], $fields['path'] ) );
			}
			if ( $blog ) {
				gc_cache_set( $blog->blog_id . 'short', $blog, 'blog-details' );
			...
			if ( 'www.' === substr( $fields['domain'], 0, 4 ) ) {
				...
				$blog = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->blogs WHERE domain = %s", $fields['domain'] ) );
			}
			if ( $blog ) {
				gc_cache_set( $blog->blog_id . 'short', $blog, 'blog-details' );
				$blog_id = $blog->blog_id;
	...
	$all     = $get_all ? '' : 'short';
	$details = gc_cache_get( $blog_id . $all, 'blog-details' );
	...
	if ( $details ) {
		if ( ! is_object( $details ) ) {
			if ( -1 == $details ) {
				...
				gc_cache_delete( $blog_id . $all, 'blog-details' );
				unset( $details );
			}
		} else {
			return $details;
	...
	// Try the other cache.
	if ( $get_all ) {
		$details = gc_cache_get( $blog_id . 'short', 'blog-details' );
	} else {
		$details = gc_cache_get( $blog_id, 'blog-details' );
	...
	if ( empty( $details ) ) {
		$details = GC_Site::get_instance( $blog_id );
		if ( ! $details ) {
			// Set the full cache.
			gc_cache_set( $blog_id, -1, 'blog-details' );
			return false;
		}
	}

	if ( ! $details instanceof GC_Site ) {
		$details = new GC_Site( $details );
	}

	if ( ! $get_all ) {
		gc_cache_set( $blog_id . $all, $details, 'blog-details' );
		return $details;
	}
	...
	gc_cache_set( $key, $details, 'blog-lookup' );
...
function update_blog_details( $blog_id, $details = array() ) {
	...
	$site = gc_update_site( $blog_id, $details );
...
function clean_site_details_cache( $site_id = 0 ) {
	...
	gc_cache_delete( $site_id, 'site-details' );
	gc_cache_delete( $site_id, 'blog-details' );
}
...
function switch_to_blog( $new_blog_id, $deprecated = null ) {
	...
	$GLOBALS['_gc_switched_stack'][] = $prev_blog_id;
 ...
 */
function gc_switch_roles_and_user( $new_site_id, $old_site_id ) {
	...
	gc_roles()->for_site( $new_site_id );
	gc_get_current_user()->for_site( $new_site_id );
}
...
function ms_is_switched() {
	return ! empty( $GLOBALS['_gc_switched_stack'] );
}
...
function get_blog_status( $id, $pref ) {
	...
	return $gcdb->get_var( $gcdb->prepare( "SELECT %s FROM {$gcdb->blogs} WHERE blog_id = %d", $pref, $id ) );
}
...
function get_last_updated( $deprecated = '', $start = 0, $quantity = 40 ) {
	...
	return $gcdb->get_results( $gcdb->prepare( "SELECT blog_id, domain, path FROM $gcdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' AND last_updated != '0000-00-00 00:00:00' ORDER BY last_updated DESC limit %d, %d", get_current_network_id(), $start, $quantity ), ARRAY_A );
}
 ...
 * }
 */
function gc_count_sites( $network_id = null ) {
	if ( empty( $network_id ) ) {
