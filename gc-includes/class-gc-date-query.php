 ...
 *
 * GC_Date_Query is a helper that allows primary query classes, such as GC_Query, to filter
 * their results by date columns, by generating `WHERE` subclauses to be attached to the
 * primary SQL query string.
 *
 * Attempting to filter by an invalid date value (eg month=13) will generate SQL that will
 * return no results. In these cases, a _doing_it_wrong() error notice is also thrown.
 * See GC_Date_Query::validate_date_values().
 *
...
class GC_Date_Query {
	 ...
	 *                                the `$default_column` parameter. See GC_Date_Query::validate_column() and
	 ...
	 *                                               See GC_Date_Query::validate_column() and
	 ...
	 *     }
	 * }
	 * @param string $default_column Optional. Default column to query against. See GC_Date_Query::validate_column()
	...
	public function validate_date_values( $date_query = array() ) {
			...
			foreach ( (array) $date_query[ $key ] as $_value ) {
				...
				if ( ! is_numeric( $_value ) || ! $is_between ) {
					$error = sprintf(
						...
						__( '为%2$s指定的值%1$s无效，有效范围为%3$s至%4$s。' ),
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! gc_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
				$day_month_year_error_msg = sprintf(
					/* translators: 1: Year, 2: Month, 3: Day of month. */
					__( '以下值描述了一个无效的日期：%1$s年，%2$s月，%3$s日。' ),
			 ...
			 */
			if ( ! gc_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
				$day_month_year_error_msg = sprintf(
					/* translators: 1: Month, 2: Day of month. */
					__( '以下值描述了一个无效的日期：%1$s月，%2$s日。' ),
					'<code>' . esc_html( $date_query['month'] ) . '</code>',
					'<code>' . esc_html( $date_query['day'] ) . '</code>'
				);
	 ...
	 * allowed and known tables, and then, if found, have a table prefix (such as 'gc_posts.')
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) ) {
			$where_parts[] = $gcdb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );
		}
		if ( ! empty( $query['before'] ) ) {
			$where_parts[] = $gcdb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		}
			...
			foreach ( $query_parts as $query_part ) {
				if ( isset( $query[ $query_part ] ) ) {
					...
					if ( $value ) {
						switch ( $sql_part ) {
							case '_gc_mysql_week':
								$where_parts[] = _gc_mysql_week( $column ) . " $compare $value";
								break;
							case 'WEEKDAY':
	...
	public function build_mysql_datetime( $datetime, $default_to_max = false ) {
		if ( ! is_array( $datetime ) ) {
			...
			if ( ! is_array( $datetime ) ) {
				...
				$dt = date_create( $datetime, $gc_timezone );

				if ( false === $dt ) {
					return gmdate( 'Y-m-d H:i:s', false );
				}

				return $dt->setTimezone( $gc_timezone )->format( 'Y-m-d H:i:s' );
			}
		}
