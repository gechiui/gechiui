...
/**
 * GeChiUI XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional GeChiUI API for managing comments, pages, posts,
 * options, etc.
 *
...
class gc_xmlrpc_server extends IXR_Server {
	...
	/**
	 * Flags that the user authentication has failed in this instance of gc_xmlrpc_server.
	 *
	 * @var bool
	 */
	...
	public function __construct() {
		$this->methods = array(
			// GeChiUI API.
			'gc.getUsersBlogs'                 => 'this:gc_getUsersBlogs',
			'gc.newPost'                       => 'this:gc_newPost',
			'gc.editPost'                      => 'this:gc_editPost',
			'gc.deletePost'                    => 'this:gc_deletePost',
			'gc.getPost'                       => 'this:gc_getPost',
			'gc.getPosts'                      => 'this:gc_getPosts',
			'gc.newTerm'                       => 'this:gc_newTerm',
			'gc.editTerm'                      => 'this:gc_editTerm',
			'gc.deleteTerm'                    => 'this:gc_deleteTerm',
			'gc.getTerm'                       => 'this:gc_getTerm',
			'gc.getTerms'                      => 'this:gc_getTerms',
			'gc.getTaxonomy'                   => 'this:gc_getTaxonomy',
			'gc.getTaxonomies'                 => 'this:gc_getTaxonomies',
			'gc.getUser'                       => 'this:gc_getUser',
			'gc.getUsers'                      => 'this:gc_getUsers',
			'gc.getProfile'                    => 'this:gc_getProfile',
			'gc.editProfile'                   => 'this:gc_editProfile',
			'gc.getPage'                       => 'this:gc_getPage',
			'gc.getPages'                      => 'this:gc_getPages',
			'gc.newPage'                       => 'this:gc_newPage',
			'gc.deletePage'                    => 'this:gc_deletePage',
			'gc.editPage'                      => 'this:gc_editPage',
			'gc.getPageList'                   => 'this:gc_getPageList',
			'gc.getAuthors'                    => 'this:gc_getAuthors',
			'gc.getCategories'                 => 'this:mw_getCategories',     // Alias.
			'gc.getTags'                       => 'this:gc_getTags',
			'gc.newCategory'                   => 'this:gc_newCategory',
			'gc.deleteCategory'                => 'this:gc_deleteCategory',
			'gc.suggestCategories'             => 'this:gc_suggestCategories',
			'gc.uploadFile'                    => 'this:mw_newMediaObject',    // Alias.
			'gc.deleteFile'                    => 'this:gc_deletePost',        // Alias.
			'gc.getCommentCount'               => 'this:gc_getCommentCount',
			'gc.getPostStatusList'             => 'this:gc_getPostStatusList',
			'gc.getPageStatusList'             => 'this:gc_getPageStatusList',
			'gc.getPageTemplates'              => 'this:gc_getPageTemplates',
			'gc.getOptions'                    => 'this:gc_getOptions',
			'gc.setOptions'                    => 'this:gc_setOptions',
			'gc.getComment'                    => 'this:gc_getComment',
			'gc.getComments'                   => 'this:gc_getComments',
			'gc.deleteComment'                 => 'this:gc_deleteComment',
			'gc.editComment'                   => 'this:gc_editComment',
			'gc.newComment'                    => 'this:gc_newComment',
			'gc.getCommentStatusList'          => 'this:gc_getCommentStatusList',
			'gc.getMediaItem'                  => 'this:gc_getMediaItem',
			'gc.getMediaLibrary'               => 'this:gc_getMediaLibrary',
			'gc.getPostFormats'                => 'this:gc_getPostFormats',
			'gc.getPostType'                   => 'this:gc_getPostType',
			'gc.getPostTypes'                  => 'this:gc_getPostTypes',
			'gc.getRevisions'                  => 'this:gc_getRevisions',
			'gc.restoreRevision'               => 'this:gc_restoreRevision',
	...
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new GC_Error( 'login_prevented' );
		} else {
			$user = gc_authenticate( $username, $password );
		}

		if ( is_gc_error( $user ) ) {
			$this->error = new IXR_Error( 403, __( '用户名或密码不正确。' ) );

			// Flag that authentication has failed once on this gc_xmlrpc_server instance.
	...
	public function set_custom_fields( $post_id, $fields ) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset( $meta['id'] ) ) {
				...
				if ( isset( $meta['key'] ) ) {
					$meta['key'] = gc_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key ) {
						continue;
					}
					$meta['value'] = gc_unslash( $meta['value'] );
			...
			} elseif ( current_user_can( 'add_post_meta', $post_id, gc_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function gc_getUsersBlogs( $args ) {
		if ( ! $this->minimum_args( $args, 2 ) ) {
			return $this->error;
		}

		// If this isn't on GCMU then just use blogger_getUsersBlogs().
		if ( ! is_multisite() ) {
		 ...
		 * @param gc_xmlrpc_server $server The XML-RPC server instance.
		 */
		do_action( 'xmlrpc_call', 'gc.getUsersBlogs', $args, $this );
	...
	protected function minimum_args( $args, $count ) {
		if ( ! is_array( $args ) || count( $args ) < $count ) {
			$this->error = new IXR_Error( 400, __( '该XML-RPC方法需要更多参数。' ) );
			return false;
		}
	 ...
	 *
	 * @param GC_Taxonomy $taxonomy The unprepared taxonomy data.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields, true ) ) {
			...
			$terms                = gc_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms']       = array();
	 ...
	 *
	 * @param GC_Post $media_item     The unprepared media item data.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => gc_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => gc_get_attachment_metadata( $media_item->ID ),
	 ...
	 *
	 * @param GC_Post $page The unprepared page data.
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( gc_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'gc_slug'                => $page->post_name,
			'gc_password'            => $page->post_password,
			'gc_author'              => $author->display_name,
			'gc_page_parent_id'      => $page->post_parent,
			'gc_page_parent_title'   => $parent_title,
			'gc_page_order'          => $page->menu_order,
			'gc_author_id'           => (string) $author->ID,
			'gc_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'gc_page_template'       => $page_template,
		);
	 ...
	 *
	 * @param GC_Comment $comment The unprepared comment data.
	 ...
	 *
	 * @param GC_User $user   The unprepared user object.
	 ...
	 */
	public function gc_newPost( $args ) {
		...
		do_action( 'xmlrpc_call', 'gc.newPost', $args, $this );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			if ( ! empty( $post_data['sticky'] ) ) {
				return new IXR_Error( 401, __( '抱歉，您不能置顶私密的文章。' ) );
			}
			...
			if ( ! current_user_can( $post_type->cap->edit_others_posts ) ) {
				return new IXR_Error( 401, __( '抱歉，您不能将文章置顶。' ) );
			}

			$sticky = gc_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	 ...
	 * @see gc_insert_post()
	 *
	 * @param GC_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = gc_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );

		$post_type = get_post_type_object( $post_data['post_type'] );
		if ( ! $post_type ) {
			return new IXR_Error( 403, __( '无效的文章类型。' ) );
		}
		...
		switch ( $post_data['post_status'] ) {
			...
			case 'private':
				if ( ! current_user_can( $post_type->cap->publish_posts ) ) {
					return new IXR_Error( 401, __( '抱歉，您不能在此文章类型中创建私人文章。' ) );
				}
				break;
			case 'publish':
			case 'future':
				if ( ! current_user_can( $post_type->cap->publish_posts ) ) {
					return new IXR_Error( 401, __( '抱歉，您不能在此文章类型中发布文章。' ) );
				}
				break;
			default:
		...
		if ( ! empty( $post_data['post_password'] ) && ! current_user_can( $post_type->cap->publish_posts ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能在此文章类型中创建密码保护的文章。' ) );
		}

		$post_data['post_author'] = absint( $post_data['post_author'] );
		if ( ! empty( $post_data['post_author'] ) && $post_data['post_author'] != $user->ID ) {
			if ( ! current_user_can( $post_type->cap->edit_others_posts ) ) {
				return new IXR_Error( 401, __( '抱歉，您不能为此用户创建文章。' ) );
			}

			$author = get_userdata( $post_data['post_author'] );

			if ( ! $author ) {
				return new IXR_Error( 404, __( '作者ID无效。' ) );
			}
		} else {
			...
			if ( isset( $post_data['terms'] ) && is_array( $post_data['terms'] ) ) {
				...
				foreach ( $taxonomies as $taxonomy ) {
					if ( ! array_key_exists( $taxonomy, $post_type_taxonomies ) ) {
						return new IXR_Error( 401, __( '抱歉，文章类型不支持您的分类法。' ) );
					}

					if ( ! current_user_can( $post_type_taxonomies[ $taxonomy ]->cap->assign_terms ) ) {
						return new IXR_Error( 401, __( '抱歉，您不能向某些分类法指定项目。' ) );
					}
					...
					foreach ( $term_ids as $term_id ) {
						...
						if ( ! $term ) {
							return new IXR_Error( 403, __( '无效的项目ID。' ) );
						}
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						if ( in_array( $term_name, $ambiguous_terms, true ) ) {
							return new IXR_Error( 401, __( '层级分类法项名不明确，请指定项目的ID。' ) );
						}
						...
						if ( ! $term ) {
							...
							if ( ! current_user_can( $post_type_taxonomies[ $taxonomy ]->cap->edit_terms ) ) {
								return new IXR_Error( 401, __( '抱歉，您不能向某些分类法添加项目。' ) );
							}

							// Create the new term.
							$term_info = gc_insert_term( $term_name, $taxonomy );
		...
		if ( ! $post_id ) {
			if ( $update ) {
				...
				return new IXR_Error( 401, __( '抱歉，此文章不能被创建。' ) );
			}
		}
	 ...
	 */
	public function gc_editPost( $args ) {
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.editPost', $args, $this );
		...
		if ( isset( $content_struct['if_not_modified_since'] ) ) {
			...
			if ( mysql2date( 'U', $post['post_modified_gmt'] ) > $content_struct['if_not_modified_since']->getTimestamp() ) {
				return new IXR_Error( 409, __( '此文章有更新的修订版本。' ) );
			}
		}
	 ...
	 */
	public function gc_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.deletePost', $args, $this );
	 ...
	 *  - 'enclosure'
	 */
	public function gc_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPost', $args, $this );
	...
	public function gc_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'gc.getPosts' );
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPosts', $args, $this );
		...
		if ( ! current_user_can( $post_type->cap->edit_posts ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能在此文章类型中编辑文章。' ) );
		}
		...
		$posts_list = gc_get_recent_posts( $query );
	 ...
	 */
	public function gc_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.newTerm', $args, $this );

		if ( ! taxonomy_exists( $content_struct['taxonomy'] ) ) {
			return new IXR_Error( 403, __( '无效分类法。' ) );
		}

		$taxonomy = get_taxonomy( $content_struct['taxonomy'] );

		if ( ! current_user_can( $taxonomy->cap->edit_terms ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能在此分类法中创建项目。' ) );
		}
		...
		if ( empty( $term_data['name'] ) ) {
			return new IXR_Error( 403, __( '项目名称不能留空。' ) );
		}

		if ( isset( $content_struct['parent'] ) ) {
			if ( ! $taxonomy['hierarchical'] ) {
				return new IXR_Error( 403, __( '该分类法不具层级关系。' ) );
			}

			$parent_term_id = (int) $content_struct['parent'];
			$parent_term    = get_term( $parent_term_id, $taxonomy['name'] );

			if ( is_gc_error( $parent_term ) ) {
				return new IXR_Error( 500, $parent_term->get_error_message() );
			}

			if ( ! $parent_term ) {
				return new IXR_Error( 403, __( '父项目不存在。' ) );
			}
		...
		$term = gc_insert_term( $term_data['name'], $taxonomy['name'], $term_data );
		...
		if ( ! $term ) {
			return new IXR_Error( 500, __( '抱歉，此项目不能被创建。' ) );
		}
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function gc_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.editTerm', $args, $this );
		...
		if ( ! $term ) {
			return new IXR_Error( 404, __( '无效的项目ID。' ) );
		}

		if ( ! current_user_can( 'edit_term', $term_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑此项目。' ) );
		}
		...
		if ( ! empty( $content_struct['parent'] ) ) {
			if ( ! $taxonomy['hierarchical'] ) {
				return new IXR_Error( 403, __( '不能设置父项目。分类法不分层级。' ) );
			}
		...
		$term = gc_update_term( $term_id, $taxonomy['name'], $term_data );
		...
		if ( ! $term ) {
			return new IXR_Error( 500, __( '抱歉，项目编辑失败。' ) );
		}
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function gc_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.deleteTerm', $args, $this );
		...
		if ( ! current_user_can( 'delete_term', $term_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能删除此项目。' ) );
		}

		$result = gc_delete_term( $term_id, $taxonomy->name );
		...
		if ( ! $result ) {
			return new IXR_Error( 500, __( '抱歉，项目删除失败。' ) );
		}
	 ...
	 *  - 'count'
	 */
	public function gc_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getTerm', $args, $this );
		...
		if ( ! current_user_can( 'assign_term', $term_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能指派此项目。' ) );
		}
	 ...
	 */
	public function gc_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getTerms', $args, $this );
		...
		if ( ! current_user_can( $taxonomy->cap->assign_terms ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能在此分类法中指派项目。' ) );
		}
	 ...
	 */
	public function gc_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getTaxonomy', $args, $this );
	 ...
	 */
	public function gc_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'gc.getTaxonomies' );
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getTaxonomies', $args, $this );
	 ...
	 *  - 'roles'
	 */
	public function gc_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getUser', $args, $this );

		if ( ! current_user_can( 'edit_user', $user_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑此用户。' ) );
		}

		$user_data = get_userdata( $user_id );

		if ( ! $user_data ) {
			return new IXR_Error( 404, __( '用户ID无效。' ) );
		}
	 ...
	 *
	 * @uses get_users()
	 * @see gc_getUser() for more on $fields and return values
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function gc_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'gc.getUsers' );
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getUsers', $args, $this );

		if ( ! current_user_can( 'list_users' ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能列出用户。' ) );
		}
		...
		if ( isset( $filter['role'] ) ) {
			if ( get_role( $filter['role'] ) === null ) {
				return new IXR_Error( 403, __( '无效的角色。' ) );
			}
	 ...
	 */
	public function gc_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'gc.getProfile' );
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getProfile', $args, $this );

		if ( ! current_user_can( 'edit_user', $user->ID ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑您的个人资料。' ) );
		}
	 ...
	 */
	public function gc_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.editProfile', $args, $this );
		...
		$result = gc_update_user( $user_data );
		...
		if ( ! $result ) {
			return new IXR_Error( 500, __( '抱歉，此用户不能被更新。' ) );
		}
	 ...
	 */
	public function gc_getPage( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'edit_page', $page_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑此页面。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPages( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'edit_pages' ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑页面。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPages', $args, $this );
	 ...
	 */
	public function gc_newPage( $args ) {
		// Items not escaped here will be escaped in gc_newPost().
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.newPage', $args, $this );
	 ...
	 */
	public function gc_deletePage( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'delete_page', $page_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能删除此页面。' ) );
		}

		// Attempt to delete the page.
		$result = gc_delete_post( $page_id );
		if ( ! $result ) {
			return new IXR_Error( 500, __( '页面删除失败。' ) );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_gc_deletePage', $page_id, $args ); // phpcs:ignore GeChiUI.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_editPage( $args ) {
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.editPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPageList( $args ) {
		global $gcdb;
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPageList', $args, $this );

		// Get list of page IDs and titles.
		$page_list = $gcdb->get_results(
			"
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getAuthors( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能编辑这篇文章。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getAuthors', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getKeywords', $args, $this );
	 ...
	 */
	public function gc_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.newCategory', $args, $this );
		...
		$cat_id = gc_insert_category( $new_category, true );
		if ( is_gc_error( $cat_id ) ) {
			if ( 'term_exists' === $cat_id->get_error_code() ) {
				return (int) $cat_id->get_error_data();
			} else {
				return new IXR_Error( 500, __( '抱歉，此分类不能被创建。' ) );
			}
		} elseif ( ! $cat_id ) {
			return new IXR_Error( 500, __( '抱歉，此分类不能被创建。' ) );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_gc_newCategory', $cat_id, $args ); // phpcs:ignore GeChiUI.NamingConventions.ValidHookName.NotLowercase
	 ...
	 */
	public function gc_deleteCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.deleteCategory', $args, $this );

		if ( ! current_user_can( 'delete_term', $category_id ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能删除此分类。' ) );
		}

		$status = gc_delete_term( $category_id, 'category' );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_gc_deleteCategory', $category_id, $args ); // phpcs:ignore GeChiUI.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.suggestCategories', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getComment', $args, $this );
	...
	public function gc_getComments( $args ) {
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getComments', $args, $this );
		...
		if ( ! current_user_can( 'moderate_comments' ) && 'approve' !== $status ) {
			return new IXR_Error( 401, __( '评论状态无效。' ) );
		}
		...
		if ( isset( $struct['post_type'] ) ) {
			...
			if ( ! $post_type_object || ! post_type_supports( $post_type_object->name, 'comments' ) ) {
				return new IXR_Error( 404, __( '无效的文章类型。' ) );
			}
	 ...
	 */
	public function gc_deleteComment( $args ) {
		$this->escape( $args );
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function gc_editComment( $args ) {
		$this->escape( $args );
		...
		$result = gc_update_comment( $comment, true );
		if ( is_gc_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}

		if ( ! $result ) {
			return new IXR_Error( 500, __( '抱歉，此评论不能被更新。' ) );
		}
	 ...
	 */
	public function gc_newComment( $args ) {
		$this->escape( $args );
		...
		if ( ! $user ) {
			...
			if ( $allow_anon && get_option( 'comment_registration' ) ) {
				return new IXR_Error( 403, __( '抱歉，您必须登录后再评论。' ) );
			} elseif ( ! $allow_anon ) {
				return $this->error;
			}
		} else {
		...
		if ( ! comments_open( $post_id ) ) {
			return new IXR_Error( 403, __( '抱歉，该项目的评论已关闭。' ) );
		}
		...
		if (
			...
			return new IXR_Error( 403, __( '抱歉，您不能对此文章发表评论。' ) );
		}
		...
		if ( ! $allow_empty && '' === $comment['comment_content'] ) {
			return new IXR_Error( 403, __( '评论是必填的。' ) );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getCommentStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getCommentStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getCommentCount( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new IXR_Error( 403, __( '抱歉，您不能访问此文章的详情。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getCommentCount', $args, $this );

		$count = gc_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPostStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPageStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_setOptions( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			return new IXR_Error( 403, __( '抱歉，您不能更新选项。' ) );
		}
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[ $o_name ]['option'], gc_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function gc_getMediaItem( $args ) {
		$this->escape( $args );
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			return new IXR_Error( 403, __( '抱歉，您不能上传文件。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getMediaItem', $args, $this );
	...
	public function gc_getMediaLibrary( $args ) {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			return new IXR_Error( 401, __( '抱歉，您不能上传文件。' ) );
		}

		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getMediaLibrary', $args, $this );
	 ...
	 */
	public function gc_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPostFormats', $args, $this );
	 ...
	 *  - 'supports'
	 */
	public function gc_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPostType', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function gc_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'gc.getPostTypes' );
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getPostTypes', $args, $this );
	...
	public function gc_getRevisions( $args ) {
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.getRevisions', $args, $this );
		...
		if ( ! gc_revisions_enabled( $post ) ) {
			return new IXR_Error( 401, __( '抱歉，修订版本记录功能已关闭。' ) );
		}
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves.
			if ( gc_is_post_autosave( $revision ) ) {
				continue;
			}
	 ...
	 */
	public function gc_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in gc-includes/class-gc-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'gc.restoreRevision', $args, $this );
		...
		$post = gc_restore_post_revision( $revision_id );
	...
	public function blogger_getPost( $args ) {
		...
		$content .= gc_unslash( $post_data['post_content'] );
	...
	public function blogger_getRecentPosts( $args ) {
		...
		if ( ! $posts_list ) {
			$this->error = new IXR_Error( 500, __( '没有文章，或发生了其他异常。' ) );
			return $this->error;
		}
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = implode( ',', gc_get_post_categories( $entry['ID'] ) );

			$content  = '<title>' . gc_unslash( $entry['post_title'] ) . '</title>';
			$content .= '<category>' . $categories . '</category>';
			$content .= gc_unslash( $entry['post_content'] );
	...
	public function blogger_getTemplate( $args ) {
		return new IXR_Error( 403, __( '抱歉，不支持此方法。' ) );
	}
	...
	public function blogger_editPost( $args ) {
		...
		if ( ! $result ) {
			return new IXR_Error( 500, __( '抱歉，此文章不能被更新。' ) );
		}
	...
	public function mw_newPost( $args ) {
		...
		if ( ! empty( $content_struct['post_type'] ) ) {
			if ( 'page' === $content_struct['post_type'] ) {
				...
				return new IXR_Error( 401, __( '无效的文章类型。' ) );
			}
		} else {
			if ( $publish ) {
				$cap = 'publish_posts';
		...
		if ( isset( $content_struct['gc_post_format'] ) ) {
			$content_struct['gc_post_format'] = sanitize_key( $content_struct['gc_post_format'] );
			if ( ! array_key_exists( $content_struct['gc_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( '无效的文章形式。' ) );
			}
		}
		...
		if ( isset( $content_struct['gc_author_id'] ) && ( $user->ID != $content_struct['gc_author_id'] ) ) {
			switch ( $post_type ) {
				case 'post':
				...
				case 'page':
					if ( ! current_user_can( 'edit_others_pages' ) ) {
						return new IXR_Error( 401, __( '抱歉，您不能作为此用户创建文章。' ) );
					}
					break;
				default:
					return new IXR_Error( 401, __( '无效的文章类型。' ) );
			}
			$author = get_userdata( $content_struct['gc_author_id'] );
			if ( ! $author ) {
				return new IXR_Error( 404, __( '作者ID无效。' ) );
			}
		...
		if ( isset( $content_struct['gc_post_thumbnail'] ) ) {
			...
			unset( $content_struct['gc_post_thumbnail'] );
		}
	...
	public function mw_editPost( $args ) {
		...
		// Use gc.editPost to edit post types other than post and page.
		...
		// Let GeChiUI manage slug if none was provided.
		...
		// If an author id was provided then use it instead.
		if ( isset( $content_struct['gc_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['gc_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
							return new IXR_Error( 401, __( '抱歉，您不能作为此用户修改文章作者。' ) );
						}
						break;
					case 'page':
						if ( ! current_user_can( 'edit_others_pages' ) ) {
							return new IXR_Error( 401, __( '抱歉，您不能作为此用户修改页面作者。' ) );
						}
						break;
					default:
						return new IXR_Error( 401, __( '无效的文章类型。' ) );
		...
		if ( 'publish' === $post_status || 'private' === $post_status ) {
			if ( 'page' === $post_type && ! current_user_can( 'publish_pages' ) ) {
				return new IXR_Error( 401, __( '抱歉，您不能发布此页面。' ) );
		...
		$result = gc_update_post( $newpost, true );
		if ( is_gc_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
	...
	public function mw_getPost( $args ) {
		...
		if ( '' !== $postdata['post_date'] ) {
			...
			$resp = array(
				...
				'gc_more_text'           => $post['more_text'],
				'mt_allow_comments'      => $allow_comments,
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'gc_slug'                => $postdata['post_name'],
				'gc_password'            => $postdata['post_password'],
				'gc_author_id'           => (string) $author->ID,
				'gc_author_display_name' => $author->display_name,
			...
			$resp['gc_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = array();
			$catids     = gc_get_post_categories( $entry['ID'] );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = gc_get_post_tags( $entry['ID'] );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'gc_slug'                => $entry['post_name'],
				'gc_password'            => $entry['post_password'],
				'gc_author_id'           => (string) $author->ID,
				'gc_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				...
				'gc_post_thumbnail'      => get_post_thumbnail_id( $entry['ID'] ),
			);
		}
	...
	public function mw_newMediaObject( $args ) {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			$this->error = new IXR_Error( 401, __( '抱歉，您不能上传文件。' ) );
			return $this->error;
		}
		...
		$upload = gc_upload_bits( $name, null, $bits );
		if ( ! empty( $upload['error'] ) ) {
			/* translators: 1: File name, 2: Error message. */
			$errorString = sprintf( __( '无法写入文件%1$s（%2$s）。' ), $name, $upload['error'] );
		...
		// Save the data.
		$id = gc_insert_attachment( $attachment, $upload['file'], $post_id );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newMediaObject', $id, $args ); // phpcs:ignore GeChiUI.NamingConventions.ValidHookName.NotLowercase
	...
	public function pingback_ping( $args ) {
		...
		if ( ! $pagelinkedfrom ) {
			return $this->pingback_error( 0, __( 'URL无效。' ) );
		}

		// Check if the page linked to is on our site.
		$pos1 = strpos( $pagelinkedto, str_replace( array( 'http://www.', 'http://', 'https://www.', 'https://' ), '', get_option( 'home' ) ) );
		if ( ! $pos1 ) {
			return $this->pingback_error( 0, __( '是否没有我们的链接？' ) );
		}
		...
		$user_agent = apply_filters( 'http_headers_useragent', 'GeChiUI/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $pagelinkedfrom );
		...
		$request                = gc_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source          = gc_remote_retrieve_body( $request );
		$remote_source_original = $remote_source;

		if ( ! $remote_source ) {
			return $this->pingback_error( 16, __( '源URL不存在。' ) );
		}
		...
		foreach ( $p as $para ) {
			if ( str_contains( $para, $pagelinkedto ) ) { // It exists, but is it a link?
				...
				$marker      = '<gccontext>' . $context[1] . '</gccontext>';  // Set up our marker.
				$excerpt     = str_replace( $context[0], $marker, $excerpt ); // Swap out the link for our marker.
				$excerpt     = strip_tags( $excerpt, '<gccontext>' );         // Strip all tags but our context marker.
		...
		if ( empty( $context ) ) { // Link to target not found.
			return $this->pingback_error( 17, __( '源URL不包含目标URL的链接，因此无法作为源。' ) );
		}
