 ...
 * type are valid.
 *
 * Also see the {@link https://developer.gechiui.com/plugins/ Plugin API} for
...
require __DIR__ . '/class-gc-hook.php';

/** @var GC_Hook[] $gc_filter */
global $gc_filter;
 ...
 *
 * GeChiUI offers filter hooks to allow plugins to modify
...
function remove_all_filters( $hook_name, $priority = false ) {
	...
	if ( isset( $gc_filter[ $hook_name ] ) ) {
		$gc_filter[ $hook_name ]->remove_all_filters( $priority );

		if ( ! $gc_filter[ $hook_name ]->has_filters() ) {
			unset( $gc_filter[ $hook_name ] );
		}
	}
 ...
 *
 *     // Old filter.
 *     return apply_filters( 'gcdocs_filter', $value, $extra_arg );
 *
 *     // Deprecated.
...
function plugin_basename( $file ) {
	...
	// $gc_plugin_paths contains normalized paths.
	$file = gc_normalize_path( $file );
 ...
 */
function gc_register_plugin_realpath( $file ) {
	global $gc_plugin_paths;

	// Normalize, but store as static to avoid recalculation of a constant value.
	static $gc_plugin_path = null, $gcmu_plugin_path = null;

	if ( ! isset( $gc_plugin_path ) ) {
		$gc_plugin_path   = gc_normalize_path( GC_PLUGIN_DIR );
		$gcmu_plugin_path = gc_normalize_path( GCMU_PLUGIN_DIR );
	}

	$plugin_path     = gc_normalize_path( dirname( $file ) );
	$plugin_realpath = gc_normalize_path( dirname( realpath( $file ) ) );

	if ( $plugin_path === $gc_plugin_path || $plugin_path === $gcmu_plugin_path ) {
		return false;
	}

	if ( $plugin_path !== $plugin_realpath ) {
		$gc_plugin_paths[ $plugin_path ] = $plugin_realpath;
	}
 ...
 * plugin is located in gc-content/plugins/sampleplugin/sample.php, then
...
function register_uninstall_hook( $file, $callback ) {
	if ( is_array( $callback ) && is_object( $callback[0] ) ) {
		_doing_it_wrong( __FUNCTION__, __( '卸载钩子只能注册静态类或静态函数。' ), '3.1.0' );
		return;
	}
 ...
 */
function _gc_call_all_hook( $args ) {
	global $gc_filter;

	$gc_filter['all']->do_all_hook( $args );
}
 ...
 */
function _gc_filter_build_unique_id( $hook_name, $callback, $priority ) {
