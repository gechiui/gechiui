 ...
 * @return string|GC_Error A list of category parents on success, GC_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	...
	$rel = ( is_object( $gc_rewrite ) && $gc_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of GeChiUI, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the GeChiUI Loop.
 ...
 */
function gc_dropdown_categories( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'orderby'           => 'id',
	...
	if ( empty( $categories ) && ! $parsed_args['hide_if_empty'] && ! empty( $parsed_args['show_option_none'] ) ) {
		 ...
		 * @param GC_Term|null $category The category object, or null if there's no corresponding category.
		 */
	 ...
	 */
	$output = apply_filters( 'gc_dropdown_cats', $output, $parsed_args );
 ...
 */
function gc_list_categories( $args = '' ) {
	$defaults = array(
		'child_of'            => 0,
		'current_category'    => 0,
		'depth'               => 0,
	...
	if ( true == $parsed_args['hierarchical'] ) {
		...
		if ( $parsed_args['exclude_tree'] ) {
			$exclude_tree = array_merge( $exclude_tree, gc_parse_id_list( $parsed_args['exclude_tree'] ) );
		}

		if ( $parsed_args['exclude'] ) {
			$exclude_tree = array_merge( $exclude_tree, gc_parse_id_list( $parsed_args['exclude'] ) );
		}
	 ...
	 *                             gc_list_categories() for information on accepted arguments.
	 */
	$html = apply_filters( 'gc_list_categories', $output, $args );
...
function gc_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_gc_error( $tags ) ) {
		return;
	}
	...
	$return = gc_generate_tag_cloud( $tags, $args );
	 ...
	 */
	$return = apply_filters( 'gc_tag_cloud', $return, $args );
 ...
 */
function gc_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
		'smallest'                   => 8,
		'largest'                    => 22,
		'unit'                       => 'pt',
	 ...
	 *
	 *
	 * @param GC_Term[] $tags Ordered array of terms.
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_gc_object_name_sort_cb' );
			} else {
				uasort( $tags, '_gc_object_count_sort_cb' );
			}
	 ...
	 */
	$tags_data = apply_filters( 'gc_generate_tag_cloud_data', $tags_data );
	...
	switch ( $args['format'] ) {
		...
		case 'list':
			 ...
			 */
			$return  = "<ul class='gc-tag-cloud' role='list'>\n\t<li>";
			$return .= implode( "</li>\n\t<li>", $a );
			$return .= "</li>\n</ul>\n";
			break;
		default:
	...
	if ( $args['filter'] ) {
		 ...
		 *                                equals 'array'.
		 * @param GC_Term[]       $tags   An array of terms used in the tag cloud.
		 * @param array           $args   An array of gc_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'gc_generate_tag_cloud', $return, $tags, $args );
	} else {
		return $return;
	}
}
 ...
 */
function _gc_object_name_sort_cb( $a, $b ) {
 ...
 * @return string|false|GC_Error A list of tags on success, false if there are no terms,
 *                               GC_Error on failure.
 */
...
function term_description( $term = 0, $deprecated = null ) {
	...
	return is_gc_error( $description ) ? '' : $description;
}
 ...
 * @return string|false|GC_Error A list of terms on success, false if there are no terms,
 *                               GC_Error on failure.
 */
 ...
 * }
 * @return string|GC_Error A list of term parents on success, GC_Error or empty string on failure.
 */
