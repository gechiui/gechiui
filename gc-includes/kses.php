 ...
 *
 * Added gc_ prefix to avoid conflicts with existing kses users
 *
 * @version 0.2.2
...
if ( ! CUSTOM_TAGS ) {
	...
	$allowedposttags = array(
		...
		'object'     => array(
			'data' => array(
				'required'       => true,
				'value_callback' => '_gc_kses_allow_pdf_objects',
			),
			'type' => array(
				'required' => true,
 ...
 */
function gc_kses_allowed_html( $context = '' ) {
	...
	if ( is_array( $context ) ) {
		 ...
		 */
		return apply_filters( 'gc_kses_allowed_html', $html, $context );
	}

	switch ( $context ) {
		case 'post':
			/** This filter is documented in gc-includes/kses.php */
			$tags = apply_filters( 'gc_kses_allowed_html', $allowedposttags, $context );
			...
			if ( ! CUSTOM_TAGS && ! isset( $tags['form'] ) && ( isset( $tags['input'] ) || isset( $tags['select'] ) ) ) {
				...
				/** This filter is documented in gc-includes/kses.php */
				$tags = apply_filters( 'gc_kses_allowed_html', $tags, $context );
			}
		...
		case 'pre_user_description':
			...
			$tags['a']['rel'] = true;
			/** This filter is documented in gc-includes/kses.php */
			return apply_filters( 'gc_kses_allowed_html', $tags, $context );

		case 'strip':
			/** This filter is documented in gc-includes/kses.php */
			return apply_filters( 'gc_kses_allowed_html', array(), $context );

		case 'entities':
			/** This filter is documented in gc-includes/kses.php */
			return apply_filters( 'gc_kses_allowed_html', $allowedentitynames, $context );

		case 'data':
		default:
			/** This filter is documented in gc-includes/kses.php */
			return apply_filters( 'gc_kses_allowed_html', $allowedtags, $context );
	}
}
 ...
 */
function gc_kses_uri_attributes() {
	 ...
	 */
	$uri_attributes = apply_filters( 'gc_kses_uri_attributes', $uri_attributes );
...
/**
 * Callback for `gc_kses_split()` for fixing malformed HTML tags.
 *
...
function gc_kses_split2( $string, $allowed_html, $allowed_protocols ) {
	...
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = gc_kses_allowed_html( $allowed_html );
	}
	...
	return gc_kses_attr( $elem, $attrlist, $allowed_html, $allowed_protocols );
}

/**
 * Removes all attributes, if none are allowed for this element.
 *
 * If some are allowed it calls `gc_kses_hair()` to split them further, and then
 * it builds up new HTML code from the data that `gc_kses_hair()` returns. It also
 ...
 */
function gc_kses_attr( $element, $attr, $allowed_html, $allowed_protocols ) {
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = gc_kses_allowed_html( $allowed_html );
	}
	...
	// Split it.
	$attrarr = gc_kses_hair( $attr, $allowed_protocols );
	...
	foreach ( $attrarr as $arreach ) {
		...
		if ( gc_kses_attr_check( $arreach['name'], $arreach['value'], $arreach['whole'], $arreach['vless'], $element, $allowed_html ) ) {
			$attr2 .= ' ' . $arreach['whole'];
 ...
 */
function gc_kses_attr_check( &$name, &$value, &$whole, $vless, $element, $allowed_html ) {
	...
	if ( is_array( $allowed_attr[ $name_low ] ) ) {
		...
		foreach ( $allowed_attr[ $name_low ] as $currkey => $currval ) {
			if ( ! gc_kses_check_attr_val( $value, $vless, $currkey, $currval ) ) {
				$name  = '';
				$value = '';
				$whole = '';
				return false;
 ...
 */
function gc_kses_hair( $attr, $allowed_protocols ) {
	$attrarr  = array();
	$mode     = 0;
	$attrname = '';
	$uris     = gc_kses_uri_attributes();
			...
			case 2:
				if ( preg_match( '%^"([^"]*)"(\s+|/?$)%', $attr, $match ) ) {
					...
					if ( in_array( strtolower( $attrname ), $uris, true ) ) {
						$thisval = gc_kses_bad_protocol( $thisval, $allowed_protocols );
					}
 ...
 */
function gc_kses_attr_parse( $element ) {
	...
	// Split it.
	$attrarr = gc_kses_hair_parse( $attr );
	if ( false === $attrarr ) {
		return false;
	}
 ...
 */
function gc_kses_hair_parse( $attr ) {
	if ( '' === $attr ) {
		return array();
	}
 ...
 */
function gc_kses_check_attr_val( $value, $vless, $checkname, $checkvalue ) {
	$ok = true;
 ...
 */
function gc_kses_array_lc( $inarray ) {
	$outarray = array();
...
/**
 * Handles parsing errors in `gc_kses_hair()`.
 *
...
/**
 * Callback for `gc_kses_bad_protocol_once()` regular expression.
 *
...
/**
 * Callback for `gc_kses_normalize_entities()` regular expression.
 *
 ...
 */
function gc_kses_named_entities( $matches ) {
 ...
 * @return string Correctly encoded entity.
 */
function gc_kses_xml_named_entities( $matches ) {
 ...
 * Callback for `gc_kses_normalize_entities()` regular expression.
 *
 * This function helps `gc_kses_normalize_entities()` to only accept 16-bit
 ...
 * @return string Correctly encoded entity.
 */
function gc_kses_normalize_entities2( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
...
/**
 * Callback for `gc_kses_normalize_entities()` for regular expression.
 *
 ...
 * @return string Correctly encoded entity.
 */
function gc_kses_normalize_entities3( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
 ...
 * @return string Filtered content.
 */
function gc_filter_kses( $data ) {
	return addslashes( gc_kses( stripslashes( $data ), current_filter() ) );
}
 ...
 * @return string Filtered content.
 */
function gc_kses_data( $data ) {
	return gc_kses( $data, current_filter() );
}
 ...
 */
function gc_filter_post_kses( $data ) {
	return addslashes( gc_kses( stripslashes( $data ), 'post' ) );
}
 ...
 */
function gc_filter_global_styles_post( $data ) {
	$decoded_data        = json_decode( gc_unslash( $data ), true );
	...
	if (
		...
		$data_to_encode = GC_Theme_JSON::remove_insecure_properties( $decoded_data );

		$data_to_encode['isGlobalStylesUserThemeJSON'] = true;
		return gc_slash( gc_json_encode( $data_to_encode ) );
	}
	return $data;
}
 ...
 * @return string Filtered post content with allowed HTML tags and attributes intact.
 */
function gc_kses_post( $data ) {
	return gc_kses( $data, 'post' );
}
 ...
 */
function gc_kses_post_deep( $data ) {
	return map_deep( $data, 'gc_kses_post' );
}
 ...
 */
function gc_filter_nohtml_kses( $data ) {
	return addslashes( gc_kses( stripslashes( $data ), 'strip' ) );
}

/**
 * Adds all KSES input form content filters.
 *
 * All hooks have default priority. The `gc_filter_kses()` function is added to
...
function kses_init_filters() {
	// Normal filtering.
	add_filter( 'title_save_pre', 'gc_filter_kses' );

	// Comment filtering.
	if ( current_user_can( 'unfiltered_html' ) ) {
		add_filter( 'pre_comment_content', 'gc_filter_post_kses' );
	} else {
		add_filter( 'pre_comment_content', 'gc_filter_kses' );
	}

	// Global Styles filtering: Global Styles filters should be executed before normal post_kses HTML filters.
	add_filter( 'content_save_pre', 'gc_filter_global_styles_post', 9 );
	add_filter( 'content_filtered_save_pre', 'gc_filter_global_styles_post', 9 );

	// Post filtering.
	add_filter( 'content_save_pre', 'gc_filter_post_kses' );
	add_filter( 'excerpt_save_pre', 'gc_filter_post_kses' );
	add_filter( 'content_filtered_save_pre', 'gc_filter_post_kses' );
}
...
function kses_remove_filters() {
	// Normal filtering.
	remove_filter( 'title_save_pre', 'gc_filter_kses' );
	...
	remove_filter( 'content_save_pre', 'gc_filter_global_styles_post', 9 );
	remove_filter( 'content_filtered_save_pre', 'gc_filter_global_styles_post', 9 );

	// Post filtering.
	remove_filter( 'content_save_pre', 'gc_filter_post_kses' );
	remove_filter( 'excerpt_save_pre', 'gc_filter_post_kses' );
	remove_filter( 'content_filtered_save_pre', 'gc_filter_post_kses' );
}
	...
	foreach ( $css_array as $css_item ) {
		...
		if ( $found && $url_attr ) {
			...
			foreach ( $url_matches[0] as $url_match ) {
				...
				if ( empty( $url ) || gc_kses_bad_protocol( $url, $allowed_protocols ) !== $url ) {
					$found = false;
					break;
				} else {
 ...
 */
function _gc_kses_allow_pdf_objects( $url ) {
	...
	$upload_info = gc_upload_dir( null, false );
	$parsed_url  = gc_parse_url( $upload_info['url'] );
