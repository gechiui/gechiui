<?php
/**
 * Customize API: GC_Customize_Date_Time_Control class
 *
 * @package GeChiUI
 ...
 */
class GC_Customize_Date_Time_Control extends GC_Customize_Control {
	...
	public function content_template() {
		...
		<# _.defaults( data, <?php echo gc_json_encode( $data ); ?> ); #>
	...
	public function get_month_choices() {
		...
		for ( $i = 1; $i < 13; $i++ ) {
			...
			$months[ $i ]['text']  = sprintf( __( '%2$s' ), $i, $month_text );
			$months[ $i ]['value'] = $i;
		}
		return array(
		...
		if ( $tz_string ) {
			...
			if ( $tz ) {
				...
				$timezone_info['description'] = sprintf(
					...
					__( '您的时区已设置为%1$s（%2$s），现在是%3$s（协调世界时%4$s）。' ),
					$tz_name,
