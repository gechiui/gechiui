<?php
/**
 * Customize API: GC_Customize_Nav_Menu_Item_Setting class
 *
 * @package GeChiUI
 ...
 */
class GC_Customize_Nav_Menu_Item_Setting extends GC_Customize_Setting {
	...
	public $default = array(
		// The $menu_item_data for gc_update_nav_menu_item().
		'object_id'        => 0,
	 ...
	 * @see GC_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	...
	public function __construct( GC_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected GC_Customize_Manager::$nav_menus to be set.' );
		}
		...
		add_action( 'gc_update_nav_menu_item', array( $this, 'flush_cached_value' ), 10, 2 );
	...
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = gc_get_post_terms(
				$this->post_id,
				GC_Customize_Nav_Menu_Setting::TAXONOMY,
				array(
					'fields' => 'ids',
				)
			);
	...
	public function preview() {
		...
		add_filter( 'gc_get_nav_menu_items', array( $this, 'filter_gc_get_nav_menu_items' ), 10, 3 );

		$sort_callback = array( __CLASS__, 'sort_gc_get_nav_menu_items' );
		if ( ! has_filter( 'gc_get_nav_menu_items', $sort_callback ) ) {
			add_filter( 'gc_get_nav_menu_items', array( __CLASS__, 'sort_gc_get_nav_menu_items' ), 1000, 3 );
		}
	 ...
	 *
	 * @param GC_Post[] $items An array of menu item post objects.
	 * @param GC_Term   $menu  The menu object.
	 * @param array     $args  An array of arguments used to retrieve menu item objects.
	 * @return GC_Post[] Array of menu item objects.
	 */
	public function filter_gc_get_nav_menu_items( $items, $menu, $args ) {
		...
		if ( $should_update ) {
			foreach ( $items as $item ) {
				if ( $item->db_id === $this->post_id ) {
					foreach ( get_object_vars( $this->value_as_gc_post_nav_menu_item() ) as $key => $value ) {
						$item->$key = $value;
					}
					$mutated = true;
				}
			...
			if ( ! $mutated ) {
				$items[] = $this->value_as_gc_post_nav_menu_item();
			}
		}
	 ...
	 * @return GC_Post[] Array of menu item objects.
	 */
	public static function sort_gc_get_nav_menu_items( $items, $menu, $args ) {
	 ...
	 */
	public function value_as_gc_post_nav_menu_item() {
		...
		// 'classes' should be an array, as in gc_setup_nav_menu_item().
		...
		/** This filter is documented in gc-includes/nav-menu.php */
		$post->description = apply_filters( 'nav_menu_description', gc_trim_words( $post->description, 200 ) );

		/** This filter is documented in gc-includes/nav-menu.php */
		$post = apply_filters( 'gc_setup_nav_menu_item', $post );
	 ...
	 *                           entirely. See GC_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = gc_delete_post( $this->post_id, true );

				if ( false === $r ) {
					$this->update_error  = new GC_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof GC_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( (int) $value['nav_menu_term_id'] !== $nav_menu_setting->previous_term_id ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof GC_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( (int) $value['menu_item_parent'] !== $parent_nav_menu_item_setting->previous_post_id ) {
					$this->update_status = 'error';
					$this->update_error  = new GC_Error( 'unexpected_previous_post_id' );
					return;
				}
	 ...
	 * @see GC_Customize_Nav_Menu_Item_Setting::update()
	 *
	 * @param array $data Additional information passed back to the 'saved' event on `gc.customize`.
