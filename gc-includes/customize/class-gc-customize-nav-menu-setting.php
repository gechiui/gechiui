<?php
/**
 * Customize API: GC_Customize_Nav_Menu_Setting class
 *
 * @package GeChiUI
 ...
 *
 *
 * @see gc_get_nav_menu_object()
 * @see GC_Customize_Setting
 */
class GC_Customize_Nav_Menu_Setting extends GC_Customize_Setting {
	 ...
	 * @see GC_Customize_Nav_Menu_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by gc_update_nav_menu_object() when setting is updated.
	 *
	 * @var GC_Error
	 *
	 * @see GC_Customize_Nav_Menu_Setting::update()
	...
	public function value() {
		if ( $this->is_previewed && get_current_blog_id() === $this->_previewed_blog_id ) {
			...
			if ( $this->term_id > 0 ) {
				$term = gc_get_nav_menu_object( $this->term_id );

				if ( $term ) {
					$value = gc_array_slice_assoc( (array) $term, array_keys( $this->default ) );
	...
	public function preview() {
		...
		add_filter( 'gc_get_nav_menus', array( $this, 'filter_gc_get_nav_menus' ), 10, 2 );
		add_filter( 'gc_get_nav_menu_object', array( $this, 'filter_gc_get_nav_menu_object' ), 10, 2 );
	...
	/**
	 * Filters the gc_get_nav_menus() result to ensure the inserted menu object is included, and the deleted one is removed.
	 *
	 *
	 * @see gc_get_nav_menus()
	 *
	 * @param GC_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 * @return GC_Term[] Array of menu objects.
	 */
	public function filter_gc_get_nav_menus( $menus, $args ) {
	 ...
	 * @var string
	 *
	 * @see GC_Customize_Nav_Menu_Setting::filter_gc_get_nav_menus()
	 * @see GC_Customize_Nav_Menu_Setting::_sort_menus_by_orderby()
	 */
	...
	/**
	 * Filters the gc_get_nav_menu_object() result to supply the previewed menu object.
	 *
	 ...
	 * @see gc_get_nav_menu_object()
	 *
	 * @param object|null $menu_obj Object returned by gc_get_nav_menu_object().
	 * @param string      $menu_id  ID of the nav_menu term. Requests by slug or name will be ignored.
	 * @return object|null
	 */
	public function filter_gc_get_nav_menu_object( $menu_obj, $menu_id ) {
		$ok = (
	...
	public function sanitize( $value ) {
		...
		if ( '' === $value['name'] ) {
			$value['name'] = _x( '（未命名）', 'Missing menu name.' );
		}
	 ...
	 *
	 *
	 * @see gc_update_nav_menu_object()
	 *
	 * @param array|false $value {
	 *     The value to update. Note that slug cannot be updated via gc_update_nav_menu_object().
	...
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = gc_delete_nav_menu( $this->term_id );
			...
			$menu_data              = gc_array_slice_assoc( $value, array( 'description', 'parent' ) );
			$menu_data['menu-name'] = $value['name'];

			$menu_id              = $is_placeholder ? 0 : $this->term_id;
			$r                    = gc_update_nav_menu_object( $menu_id, gc_slash( $menu_data ) );
			$original_name        = $menu_data['menu-name'];
			$name_conflict_suffix = 1;
			while ( is_gc_error( $r ) && 'menu_exists' === $r->get_error_code() ) {
				$name_conflict_suffix += 1;
		...
		if ( 'inserted' === $this->update_status ) {
			...
			foreach ( array_keys( $this->manager->unsanitized_post_values() ) as $setting_id ) {
				...
				$widget_instance = $nav_menu_widget_setting->post_value(); // Note that this calls GC_Customize_Widgets::sanitize_widget_instance().
	 ...
	 *
	 *
	 * @see GC_Customize_Nav_Menu_Setting::filter_nav_menu_options()
	 * @see GC_Customize_Nav_Menu_Setting::update()
	 *
