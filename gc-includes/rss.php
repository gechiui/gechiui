 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', GCINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'GeChiUI/' . $GLOBALS['gc_version']);
...
class MagpieRSS {
	...
	function __construct( $source ) {
		...
		if ( ! function_exists('xml_parser_create') ) {
			return trigger_error( "PHP的XML扩展不可用。请联系您的主机提供商来启用PHP的XML扩展。" );
		}
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = gc_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_gc_error($resp) ) {
	...
	// Also note, GC_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( gc_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = gc_remote_retrieve_response_code( $resp );
	$response->response_code = gc_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = gc_remote_retrieve_body( $resp );
 ...
 */
function gc_rss( $url, $num_items = -1 ) {
