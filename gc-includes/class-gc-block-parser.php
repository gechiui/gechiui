...
class GC_Block_Parser {
		...
		switch ( $token_type ) {
			...
			case 'void-block':
				...
				if ( 0 === $stack_depth ) {
					...
					$this->output[] = (array) new GC_Block_Parser_Block( $block_name, $attrs, array(), '', array() );
				...
				$this->add_inner_block(
					new GC_Block_Parser_Block( $block_name, $attrs, array(), '', array() ),
					$start_offset,
					$token_length
				);
				$this->offset = $start_offset + $token_length;
	...
	public function next_token() {
		...
		$has_match = preg_match(
			'/<!--\s+(?P<closer>\/)?gc:(?P<namespace>[a-z][a-z0-9_-]*\/)?(?P<name>[a-z][a-z0-9_-]*)\s+(?P<attrs>{(?:(?:[^}]+|}+(?=})|(?!}\s+\/?-->).)*+)?}\s+)?(?P<void>\/)?-->/s',
			$this->document,
			$matches,
			PREG_OFFSET_CAPTURE,
			$this->offset
