 ...
 */
function gc_embed_register_handler( $id, $regex, $callback, $priority = 10 ) {
	global $gc_embed;
	$gc_embed->register_handler( $id, $regex, $callback, $priority );
}
 ...
 */
function gc_embed_unregister_handler( $id, $priority = 10 ) {
	global $gc_embed;
	$gc_embed->unregister_handler( $id, $priority );
}
 ...
 * }
 */
function gc_embed_defaults( $url = '' ) {
 ...
 */
function gc_oembed_get( $url, $args = '' ) {
	$oembed = _gc_oembed_get_object();
 ...
 */
function _gc_oembed_get_object() {
	static $gc_oembed = null;

	if ( is_null( $gc_oembed ) ) {
		$gc_oembed = new GC_oEmbed();
	}
	return $gc_oembed;
}
 ...
 */
function gc_oembed_add_provider( $format, $provider, $regex = false ) {
	if ( did_action( 'plugins_loaded' ) ) {
		$oembed                       = _gc_oembed_get_object();
		$oembed->providers[ $format ] = array( $provider, $regex );
	} else {
		GC_oEmbed::_add_provider_early( $format, $provider, $regex );
	}
}
 ...
 */
function gc_oembed_remove_provider( $format ) {
	if ( did_action( 'plugins_loaded' ) ) {
		$oembed = _gc_oembed_get_object();
		...
		GC_oEmbed::_remove_provider_early( $format );
	}
...
function gc_maybe_load_embeds() {
	 ...
	 */
	gc_embed_register_handler( 'audio', '#^https?://.+?\.(' . implode( '|', gc_get_audio_extensions() ) . ')$#i', apply_filters( 'gc_audio_embed_handler', 'gc_embed_handler_audio' ), 9999 );
	 ...
	 */
	gc_embed_register_handler( 'video', '#^https?://.+?\.(' . implode( '|', gc_get_video_extensions() ) . ')$#i', apply_filters( 'gc_video_embed_handler', 'gc_embed_handler_video' ), 9999 );
}
 ...
 * @return string The embed HTML.
 */
function gc_embed_handler_audio( $matches, $attr, $url, $rawattr ) {
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'gc_embed_handler_audio', $audio, $attr, $url, $rawattr );
}
 ...
 * @return string The embed HTML.
 */
function gc_embed_handler_video( $matches, $attr, $url, $rawattr ) {
	$dimensions = '';
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'gc_embed_handler_video', $video, $attr, $url, $rawattr );
}
 ...
 *
 *     add_action( 'gc_head', 'gc_oembed_add_host_js' )
 *
...
/**
 * Enqueue the gc-embed script if the provided oEmbed HTML contains a post embed.
 *
 ...
 */
function gc_maybe_enqueue_oembed_host_js( $html ) {
	if (
		has_action( 'gc_head', 'gc_oembed_add_host_js' )
		&&
		preg_match( '/<blockquote\s[^>]*?gc-embedded-content/', $html )
	) {
...
function get_post_embed_html( $width, $height, $post = null ) {
	...
	$secret     = gc_generate_password( 10, false );
	$embed_url .= "#?secret={$secret}";

	$output = sprintf(
		'<blockquote class="gc-embedded-content" data-secret="%1$s"><a href="%2$s">%3$s</a></blockquote>',
		esc_attr( $secret ),
	...
	$output .= sprintf(
		'<iframe sandbox="allow-scripts" security="restricted" src="%1$s" width="%2$d" height="%3$d" title="%4$s" data-secret="%5$s" frameborder="0" marginwidth="0" marginheight="0" scrolling="no" class="gc-embedded-content"></iframe>',
		esc_url( $embed_url ),
		absint( $width ),
		absint( $height ),
		esc_attr(
...
function get_oembed_response_data_for_url( $url, $args ) {
	...
	/** This filter is documented in gc-includes/class-gc-oembed-controller.php */
...
function get_oembed_response_data_rich( $data, $post, $width, $height ) {
	...
	if ( 'attachment' === get_post_type( $post ) ) {
		if ( gc_attachment_is_image( $post ) ) {
	...
	if ( $thumbnail_id ) {
		list( $thumbnail_url, $thumbnail_width, $thumbnail_height ) = gc_get_attachment_image_src( $thumbnail_id, array( $width, 99999 ) );
 ...
 */
function gc_oembed_ensure_format( $format ) {
 ...
 */
function gc_filter_oembed_iframe_title_attribute( $result, $data, $url ) {
	...
	if ( preg_match( $pattern, $result, $matches ) ) {
		$attrs = gc_kses_hair( $matches[1], gc_allowed_protocols() );
	...
	if ( isset( $attrs['title'] ) ) {
		unset( $attrs['title'] );
		$attr_string = implode( ' ', gc_list_pluck( $attrs, 'whole' ) );
 ...
 */
function gc_filter_oembed_result( $result, $data, $url ) {
	if ( false === $result || ! in_array( $data->type, array( 'rich', 'video' ), true ) ) {
		return $result;
	}

	$gc_oembed = _gc_oembed_get_object();

	// Don't modify the HTML for trusted providers.
	if ( false !== $gc_oembed->get_provider( $url, array( 'discover' => false ) ) ) {
		return $result;
	}
	...
	$html = gc_kses( $result, $allowed_html );
	...
	if ( ! empty( $content[1] ) ) {
		...
		$html = str_replace( '<blockquote', '<blockquote class="gc-embedded-content"', $html );
	}

	$html = str_ireplace( '<iframe', '<iframe class="gc-embedded-content" sandbox="allow-scripts" security="restricted"', $html );
 ...
 */
function gc_embed_excerpt_more( $more_string ) {
	if ( ! is_embed() ) {
		return $more_string;
	}

	$link = sprintf(
		'<a href="%1$s" class="gc-embed-more" target="_top">%2$s</a>',
		esc_url( get_permalink() ),
		/* translators: %s: Post title. */
		sprintf( __( '继续阅读%s' ), '<span class="screen-reader-text">' . get_the_title() . '</span>' )
	);
	return ' &hellip; ' . $link;
}
 ...
 */
function gc_embed_excerpt_attachment( $content ) {
	if ( is_attachment() ) {
...
function _oembed_filter_feed_content( $content ) {
	return str_replace( '<iframe class="gc-embedded-content" sandbox="allow-scripts" security="restricted" style="position: absolute; clip: rect(1px, 1px, 1px, 1px);"', '<iframe class="gc-embedded-content" sandbox="allow-scripts" security="restricted"', $content );
}
...
function print_embed_sharing_button() {
	...
	<div class="gc-embed-share">
		<button type="button" class="gc-embed-share-dialog-open" aria-label="<?php esc_attr_e( '打开分享对话框' ); ?>">
...
function print_embed_sharing_dialog() {
	...
	<div class="gc-embed-share-dialog hidden" role="dialog" aria-label="<?php esc_attr_e( '分享选项' ); ?>">
		<div class="gc-embed-share-dialog-content">
			...
			<button type="button" class="gc-embed-share-dialog-close" aria-label="<?php esc_attr_e( '关闭分享对话框' ); ?>">
...
function the_embed_site_title() {
	...
	$site_title = '<div class="gc-embed-site-title">' . $site_title . '</div>';
 ...
 */
function gc_filter_pre_oembed_result( $result, $url, $args ) {
	$data = get_oembed_response_data_for_url( $url, $args );

	if ( $data ) {
		return _gc_oembed_get_object()->data2html( $data, $url );
	}
