<?php
/**
 * Widget API: GC_Widget base class
 *
 * @package GeChiUI
 * @subpackage Widgets
 */
...
class GC_Widget {
	...
	/**
	 * Option array passed to gc_register_sidebar_widget().
	 *
	 * @var array
	 */
	...
	/**
	 * Option array passed to gc_register_widget_control().
	 *
	 * @var array
	 */
	...
	public function widget( $args, $instance ) {
		die( 'function GC_Widget::widget() must be overridden in a subclass.' );
	}
	...
	public function form( $instance ) {
		echo '<p class="no-options-widget">' . __( '这个小工具没有提供选项。' ) . '</p>';
		return 'noform';
	}
	 ...
	 * @param array  $control_options Optional. Widget control options. See gc_register_widget_control() for
	 *                                information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		if ( ! empty( $id_base ) ) {
			...
			$id_base = preg_replace( '/(gc_)?widget_/', '', strtolower( get_class( $this ) ) );
		}

		$this->id_base         = $id_base;
		$this->name            = $name;
		$this->option_name     = 'widget_' . $this->id_base;
		$this->widget_options  = gc_parse_args(
			$widget_options,
			array(
		...
		$this->control_options = gc_parse_args( $control_options, array( 'id_base' => $this->id_base ) );
	}
	 ...
	 *                                information on accepted arguments. Default empty array.
	 */
	public function GC_Widget( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		_deprecated_constructor( 'GC_Widget', '4.3.0', get_class( $this ) );
		GC_Widget::__construct( $id_base, $name, $widget_options, $control_options );
	}
	...
	/**
	 * Constructs id attributes for use in GC_Widget::form() fields.
	 *
	...
	public function is_preview() {
		global $gc_customize;
		return ( isset( $gc_customize ) && $gc_customize->is_preview() );
	}

	/**
	 * Generates the actual widget content (Do NOT override).
	 *
	 * Finds the instance and calls GC_Widget::widget().
	 *
	 *
	 * @param array     $args        Display arguments. See GC_Widget::widget() for information
	...
	public function display_callback( $args, $widget_args = 1 ) {
		...
		$widget_args = gc_parse_args( $widget_args, array( 'number' => -1 ) );
		...
		if ( isset( $instances[ $this->number ] ) ) {
			...
			$was_cache_addition_suspended = gc_suspend_cache_addition();
			...
			if ( $this->is_preview() ) {
				gc_suspend_cache_addition( $was_cache_addition_suspended );
			}
		}
	}
	...
	public function update_callback( $deprecated = 1 ) {
		...
		if ( isset( $_POST['delete_widget'] ) && $_POST['delete_widget'] ) {
			...
			if ( isset( $gc_registered_widgets[ $del_id ]['params'][0]['number'] ) ) {
				$number = $gc_registered_widgets[ $del_id ]['params'][0]['number'];
	...
	public function get_settings() {
		...
		if ( ! empty( $settings ) && ! isset( $settings['_multiwidget'] ) ) {
			...
			$settings = gc_convert_widget_settings( $this->id_base, $this->option_name, $settings );
		}
