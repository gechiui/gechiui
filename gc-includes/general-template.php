 ...
 * form into the sidebar and also by the search widget in GeChiUI.
 *
 ...
 */
function gc_logout_url( $redirect = '' ) {
	$args = array();
	if ( ! empty( $redirect ) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg( $args, site_url( 'gc-login.php?action=logout', 'login' ) );
	$logout_url = gc_nonce_url( $logout_url, 'log-out' );
 ...
 */
function gc_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'gc-login.php', 'login' );
...
function gc_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'gc-login.php?action=register', 'login' ) );
}
 ...
 *     @type string $id_submit      ID attribute value for the submit button. Default 'gc-submit'.
 *     @type bool   $remember       Whether to display the "rememberme" checkbox in the form.
 *     @type string $value_username Default value for the username field. Default empty.
 *     @type bool   $value_remember Whether the "记住我" checkbox should be checked by default.
 *                                  Default false (unchecked).
 *
 * }
 * @return void|string Void if 'echo' argument is true, login form HTML if 'echo' is false.
 */
function gc_login_form( $args = array() ) {
	$defaults = array(
		'echo'           => true,
		// Default 'redirect' value takes the user back to the request URI.
		'redirect'       => ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],
		'form_id'        => 'loginform',
		'label_username' => __( '用户名或电子邮箱' ),
	 ...
	 */
	$args = gc_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form =
		sprintf(
			...
			esc_url( site_url( 'gc-login.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
 ...
 */
function gc_lostpassword_url( $redirect = '' ) {
	$args = array(
		'action' => 'lostpassword',
	);
	...
	$lostpassword_url = add_query_arg( $args, network_site_url( $gc_login_path, 'login' ) );
...
/**
 * Theme container function for the 'gc_meta' action.
 *
 ...
 * - 'gcurl' - The GeChiUI address (URL) (set in Settings > General)
...
function get_site_icon_url( $size = 512, $url = '', $blog_id = 0 ) {
	...
	if ( $site_icon_id ) {
		...
		$url = gc_get_attachment_image_url( $site_icon_id, $size_data );
	}
...
function get_custom_logo( $blog_id = 0 ) {
	...
	if ( $custom_logo_id ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			 ...
			 * to the attributes used by gc_get_attachment_image().
			 */
			$image_alt = get_post_meta( $custom_logo_id, '_gc_attachment_image_alt', true );
			if ( empty( $image_alt ) ) {
		 ...
		 * because gc_get_attachment_image() already adds the alt attribute.
		 */
		$image = gc_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr );
 ...
 */
function gc_get_document_title() {
	...
	// If it's a 404 page, use a "未找到页面" title.
	if ( is_404() ) {
		...
		$title['title'] = sprintf( __( '“%s”的搜索结果' ), get_search_query() );
		...
		*/
		...
		$title['title'] = get_the_date( _x( 'Y年', 'yearly archives date format' ) );

	} elseif ( is_month() ) {
		$title['title'] = get_the_date( _x( 'Y年n月', 'monthly archives date format' ) );
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		...
		$title['page'] = sprintf( __( '第 %s 页' ), max( $paged, $page ) );
	}
...
function gc_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	...
	$t_sep = '%GC_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
	...
	if ( is_archive() && ! empty( $m ) ) {
		...
		$title    = $my_year .
			( $my_month ? $t_sep . $gc_locale->get_month( $my_month ) : '' ) .
			( $my_day ? $t_sep . $my_day : '' );
	}
	...
	if ( is_archive() && ! empty( $year ) ) {
		$title = $year;
		if ( ! empty( $monthnum ) ) {
			$title .= $t_sep . $gc_locale->get_month( $monthnum );
		}
		if ( ! empty( $day ) ) {
	...
	if ( is_search() ) {
		...
		$title = sprintf( __( '搜索结果 %1$s %2$s' ), $t_sep, strip_tags( $search ) );
	}
	 ...
	 */
	$title_array = apply_filters( 'gc_title_parts', explode( $t_sep, $title ) );
	 ...
	 */
	$title = apply_filters( 'gc_title', $title, $sep, $seplocation );
...
function single_month_title( $prefix = '', $display = true ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		$my_year  = $year;
		$my_month = $gc_locale->get_month( $monthnum );
	} elseif ( ! empty( $m ) ) {
		$my_year  = substr( $m, 0, 4 );
		$my_month = $gc_locale->get_month( substr( $m, 4, 2 ) );
	}
...
function get_the_archive_title() {
	...
	if ( is_category() ) {
		...
		$prefix = _x( '分类：', 'category archive title prefix' );
	} elseif ( is_tag() ) {
		$title  = single_tag_title( '', false );
		$prefix = _x( '标签：', 'tag archive title prefix' );
	} elseif ( is_author() ) {
		$title  = get_the_author();
		$prefix = _x( '作者：', 'author archive title prefix' );
	} elseif ( is_year() ) {
		$title  = get_the_date( _x( 'Y年', 'yearly archives date format' ) );
		$prefix = _x( '年度归档：', 'date archive title prefix' );
	} elseif ( is_month() ) {
		$title  = get_the_date( _x( 'Y年n月', 'monthly archives date format' ) );
		$prefix = _x( '月度归档：', 'date archive title prefix' );
	} elseif ( is_day() ) {
		$title  = get_the_date( _x( 'Y年n月j日', 'daily archives date format' ) );
		$prefix = _x( '每日归档：', 'date archive title prefix' );
	} elseif ( is_tax( 'post_format' ) ) {
		if ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = _x( '日志', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = _x( '相册', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = _x( '图片', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = _x( '视频', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = _x( '引语', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = _x( '链接', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = _x( '状态', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = _x( '音频', 'post format archive title' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = _x( '聊天', 'post format archive title' );
		}
	} elseif ( is_post_type_archive() ) {
		$title  = post_type_archive_title( '', false );
		$prefix = _x( '归档：', 'post type archive title prefix' );
	} elseif ( is_tax() ) {
...
function gc_get_archives( $args = '' ) {
	...
	$sql_where = $gcdb->prepare( "WHERE post_type = %s AND post_status = 'publish'", $parsed_args['post_type'] );
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		...
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%2$d年%1$s' ), $gc_locale->get_month( $result->month ), $result->year );
...
function get_calendar( $initial = true, $echo = true ) {
	...
	$cache = gc_cache_get( 'get_calendar', 'calendar' );
	...
	if ( ! $posts ) {
		$gotsome = $gcdb->get_var( "SELECT 1 as test FROM $gcdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1" );
		if ( ! $gotsome ) {
			$cache[ $key ] = '';
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$d         = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = $gcdb->get_var( "SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')" );
	} elseif ( ! empty( $m ) ) {
		$thisyear = (int) substr( $m, 0, 4 );
		if ( strlen( $m ) < 6 ) {
			$thismonth = '01';
	...
	for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
		$myweek[] = $gc_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
	}

	foreach ( $myweek as $wd ) {
		$day_name         = $initial ? $gc_locale->get_weekday_initial( $wd ) : $gc_locale->get_weekday_abbrev( $wd );
		$wd               = esc_attr( $wd );
	...
	// Get days with posts.
	$dayswithposts = $gcdb->get_results(
		"SELECT DISTINCT DAYOFMONTH(post_date)
		FROM $gcdb->posts WHERE post_date >= '{$thisyear}-{$thismonth}-01 00:00:00'
		AND post_type = 'post' AND post_status = 'publish'
	...
	for ( $day = 1; $day <= $daysinmonth; ++$day ) {
		...
		if ( in_array( $day, $daywithpost, true ) ) {
			// Any posts today?
			$date_format = gmdate( _x( 'Y年n月j日', 'daily archives date format' ), strtotime( "{$thisyear}-{$thismonth}-{$day}" ) );
			/* translators: Post calendar label. %s: Date. */
			$label            = sprintf( __( '%s 发布的文章' ), $date_format );
	...
	$calendar_output .= '<nav aria-label="' . __( '上个月及下个月' ) . '" class="gc-calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . '<span class="gc-calendar-nav-prev"><a href="' . get_month_link( $previous->year, $previous->month ) . '">&laquo; ' .
			$gc_locale->get_month_abbrev( $gc_locale->get_month( $previous->month ) ) .
		'</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="gc-calendar-nav-prev">&nbsp;</span>';
	}

	$calendar_output .= "\n\t\t" . '<span class="pad">&nbsp;</span>';

	if ( $next ) {
		$calendar_output .= "\n\t\t" . '<span class="gc-calendar-nav-next"><a href="' . get_month_link( $next->year, $next->month ) . '">' .
			$gc_locale->get_month_abbrev( $gc_locale->get_month( $next->month ) ) .
		' &raquo;</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="gc-calendar-nav-next">&nbsp;</span>';
	}
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param GC_Post|null     $post     GC_Post object or null if no post is found.
	 */
...
function get_post_time( $format = 'U', $gmt = false, $post = null, $translate = false ) {
	...
	if ( 'U' === $format || 'G' === $format ) {
		...
		$time = gc_date( $format, $datetime->getTimestamp(), $gmt ? new DateTimeZone( 'UTC' ) : null );
	} else {
		if ( $gmt ) {
...
function the_weekday() {
	...
	$the_weekday = $gc_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function the_weekday_date( $before = '', $after = '' ) {
	global $gc_locale, $currentday, $previousweekday;
	...
	if ( $currentday !== $previousweekday ) {
		...
		$the_weekday_date .= $gc_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function gc_resource_hints() {
	$hints = array(
		'dns-prefetch' => gc_dependencies_unique_hosts(),
		'preconnect'   => array(),
		'prefetch'     => array(),
		'prerender'    => array(),
	);
	...
	foreach ( $hints as $relation_type => $urls ) {
		 ...
		 */
		$urls = apply_filters( 'gc_resource_hints', $urls, $relation_type );
 ...
 */
function gc_dependencies_unique_hosts() {
	global $gc_scripts, $gc_styles;

	$unique_hosts = array();

	foreach ( array( $gc_scripts, $gc_styles ) as $dependencies ) {
		if ( $dependencies instanceof GC_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				if ( ! isset( $dependencies->registered[ $handle ] ) ) {
					continue;
				}

				/* @var _GC_Dependency $dependency */
				$dependency = $dependencies->registered[ $handle ];
				$parsed     = gc_parse_url( $dependency->src );
...
function user_can_richedit() {
	global $gc_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_IE, $is_edge;

	if ( ! isset( $gc_rich_edit ) ) {
		$gc_rich_edit = false;
	 ...
	 *
	 *
	 * @param bool $gc_rich_edit Whether the user can access the visual editor.
	 */
	return apply_filters( 'user_can_richedit', $gc_rich_edit );
}
 ...
 */
function gc_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // Defaults.
	if ( gc_get_current_user() ) { // Look for cookie.
	 ...
	 */
	return apply_filters( 'gc_default_editor', $r );
}
 ...
 * _GC_Editors should not be used directly. See https://core.trac.gechiui.com/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running gc_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac.gechiui.com/ticket/19173 for more information.
 *
 ...
 */
function gc_editor( $content, $editor_id, $settings = array() ) {
	if ( ! class_exists( '_GC_Editors', false ) ) {
		require ABSPATH . GCINC . '/class-gc-editor.php';
	}
	_GC_Editors::editor( $content, $editor_id, $settings );
}
...
function gc_enqueue_editor() {
	...
	_GC_Editors::enqueue_default_editor();
}
 ...
 *     @type GC_Theme $theme      Theme being edited when on the theme file editor.
 ...
 */
function gc_enqueue_code_editor( $args ) {
	if ( is_user_logged_in() && 'false' === gc_get_current_user()->syntax_highlighting ) {
		return false;
	}

	$settings = gc_get_code_editor_settings( $args );

	if ( empty( $settings ) || empty( $settings['codemirror'] ) ) {
		return false;
	}

	gc_enqueue_script( 'code-editor' );
	gc_enqueue_style( 'code-editor' );
	...
	if ( isset( $settings['codemirror']['mode'] ) ) {
		...
		if ( ! empty( $settings['codemirror']['lint'] ) ) {
			switch ( $mode['name'] ) {
				...
				case 'text/x-php':
					gc_enqueue_script( 'htmlhint' );
					gc_enqueue_script( 'csslint' );
					gc_enqueue_script( 'jshint' );
					if ( ! current_user_can( 'unfiltered_html' ) ) {
						gc_enqueue_script( 'htmlhint-kses' );
					}
					break;
				case 'javascript':
				...
				case 'application/typescript':
					gc_enqueue_script( 'jshint' );
					gc_enqueue_script( 'jsonlint' );
					break;
			}
		}
	}

	gc_add_inline_script( 'code-editor', sprintf( 'jQuery.extend( gc.codeEditor.defaultSettings, %s );', gc_json_encode( $settings ) ) );
	 ...
	 */
	do_action( 'gc_enqueue_code_editor', $settings );
 ...
 */
function gc_get_code_editor_settings( $args ) {
	$settings = array(
		'codemirror' => array(
			'indentUnit'       => 4,
			'indentWithTabs'   => true,
		...
		'jshint'     => array(
			// The following are copied from <https://github.com/GeChiUI/gechiui-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
	...
	if ( isset( $args['type'] ) ) {
		...
		// Supply any types that are not matched by gc_get_mime_types().
		if ( empty( $type ) ) {
			switch ( $extension ) {
				case 'conf':
					$type = 'text/nginx';
	...
	if ( in_array( $type, array( 'text/css', 'text/x-scss', 'text/x-less', 'text/x-sass' ), true ) ) {
		...
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			$settings['htmlhint']['kses'] = gc_kses_allowed_html( 'post' );
		}
	...
	foreach ( gc_array_slice_assoc( $args, array( 'codemirror', 'csslint', 'jshint', 'htmlhint' ) ) as $key => $value ) {
	 ...
	 * }
	 */
	return apply_filters( 'gc_code_editor_settings', $settings, $args );
}
 ...
 *     @type int    $total              The total amount of pages. Default is the value GC_Query's
 *                                      `max_num_pages` or 1.
 ...
 * }
 */
function gc_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_gc_admin_css_colors;

	if ( ! isset( $_gc_admin_css_colors ) ) {
		$_gc_admin_css_colors = array();
	}

	$_gc_admin_css_colors[ $key ] = (object) array(
		'name'        => $name,
		'url'         => $url,
		'colors'      => $colors,
		'icon_colors' => $icons,
 ...
 * @return string
 */
function gc_admin_css_uri( $file = 'gc-admin' ) {
	 ...
	 */
	return apply_filters( 'gc_admin_css_uri', $_file, $file );
}
 ...
 * {@see 'gc_print_styles'} action has *not* yet been called, the CSS file will be
 * enqueued. If the {@see 'gc_print_styles'} action has been called, the CSS link will
 * be printed. Printing may be forced by passing true as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with GeChiUI 2.3 calling method: If the $file
...
function gc_admin_css( $file = 'gc-admin', $force_echo = false ) {
	 ...
	 */
	echo apply_filters( 'gc_admin_css', $stylesheet_link, $file );

	if ( function_exists( 'is_rtl' ) && is_rtl() ) {
		$rtl_stylesheet_link = sprintf(
			"<link rel='stylesheet' href='%s' type='text/css' />\n",
			esc_url( gc_admin_css_uri( "$file-rtl" ) )
		);

		/** This filter is documented in gc-includes/general-template.php */
		echo apply_filters( 'gc_admin_css', $rtl_stylesheet_link, "$file-rtl" );
	}
}
...
/**
 * Displays the XHTML generator that is generated on the gc_head hook.
 *
 * See {@see 'gc_head'}.
 *
 *
...
function gc_generator() {
	 ...
	 */
	the_generator( apply_filters( 'gc_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="GeChiUI ' . esc_attr( get_bloginfo( 'version' ) ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="GeChiUI ' . esc_attr( get_bloginfo( 'version' ) ) . '" />';
			break;
		...
		case 'comment':
			...
			break;
		case 'export':
			$gen = '<!-- generator="GeChiUI/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
	 ...
	 *
	 *
	 * @param string $gen  The HTML markup output to gc_head().
 ...
 */
function gc_heartbeat_settings( $settings ) {
	if ( ! is_admin() ) {
		$settings['ajaxurl'] = admin_url( 'admin-ajax.php', 'relative' );
	}

	if ( is_user_logged_in() ) {
		$settings['nonce'] = gc_create_nonce( 'heartbeat-nonce' );
	}
