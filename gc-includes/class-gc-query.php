...
class GC_Query {
	 ...
	 *
	 * @var GC_Tax_Query A taxonomy query instance.
	 */
	public $tax_query;
	 ...
	 *
	 * @since 1.5.0
	 * @var GC_Term|GC_Post_Type|GC_Post|GC_User|null
	 */
	 ...
	 *
	 * The query flags are what page info GeChiUI was able to figure out.
	 *
	 * @since 2.0.0
	 */
	 ...
	 *     @type array           $date_query             An associative array of GC_Date_Query arguments.
	 *                                                   See GC_Date_Query::__construct().
	 *     @type int             $day                    Day of the month. Default empty. Accepts numbers 1-31.
	 *     @type bool            $exact                  Whether to search by exact keyword. Default false.
	 *     @type string          $fields                 Post fields to query for. Accepts:
	 *                                                   - '' Returns an array of complete post objects (`GC_Post[]`).
	 ...
	 *                                                   the 'gc_query_search_exclusion_prefix' filter.
	 ...
	 *     @type array           $tax_query              An associative array of GC_Tax_Query arguments.
	 *                                                   See GC_Tax_Query::__construct().
	...
	public function parse_query( $query = '' ) {
		if ( ! empty( $query ) ) {
			$this->init();
			$this->query      = gc_parse_args( $query );
		...
		if ( ( '' !== $qv['attachment'] ) || ! empty( $qv['attachment_id'] ) ) {
			...
			if ( $qv['day'] ) {
				if ( ! $this->is_date ) {
					...
					if ( $qv['monthnum'] && $qv['year'] && ! gc_checkdate( $qv['monthnum'], $qv['day'], $qv['year'], $date ) ) {
						$qv['error'] = '404';
					} else {
						$this->is_day  = true;
						$this->is_date = true;
		...
		if ( $this->is_home && 'page' === get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
			$_query = gc_parse_args( $this->query );
		...
		if ( '' !== $qv['pagename'] ) {
			...
			if ( isset( $this->queried_object_id ) && get_option( 'gc_page_for_privacy_policy' ) == $this->queried_object_id ) {
		...
		if ( $qv['page_id'] ) {
			...
			if ( get_option( 'gc_page_for_privacy_policy' ) == $qv['page_id'] ) {
				$this->is_privacy_policy = true;
			}
		}
		 ...
		 * @since 1.5.0
		 *
		 * @param GC_Query $query The GC_Query instance (passed by reference).
		 */
	...
	public function parse_tax_query( &$q ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $q[ $t->query_var ] ) ) {
				...
				if ( ! empty( $t->rewrite['hierarchical'] ) ) {
					$q[ $t->query_var ] = gc_basename( $q[ $t->query_var ] );
				}
		...
		$this->tax_query = new GC_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param GC_Query $query The GC_Query instance.
		 */
	...
	protected function parse_search( &$q ) {
		...
		/**
		 * Filters the columns to search in a GC_Query search.
		 *
		 ...
		 * @param GC_Query $query          The current GC_Query instance.
		 */
		 ...
		 */
		$exclusion_prefix = apply_filters( 'gc_query_search_exclusion_prefix', '-' );
		...
		foreach ( $q['search_terms'] as $term ) {
			...
			if ( $n && ! $exclude ) {
				$like                        = '%' . $gcdb->esc_like( $term ) . '%';
				$q['search_orderby_title'][] = $gcdb->prepare( "{$gcdb->posts}.post_title LIKE %s", $like );
			}

			$like = $n . $gcdb->esc_like( $term ) . $n;

			$search_columns_parts = array();
			foreach ( $search_columns as $search_column ) {
				$search_columns_parts[ $search_column ] = $gcdb->prepare( "({$gcdb->posts}.$search_column $like_op %s)", $like );
			}

			if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
				$search_columns_parts['attachment'] = $gcdb->prepare( "(sq1.meta_value $like_op %s)", $like );
			}
		...
		if ( ! empty( $search ) ) {
			...
			if ( ! is_user_logged_in() ) {
				$search .= " AND ({$gcdb->posts}.post_password = '') ";
			}
		}
	...
	protected function get_search_stopwords() {
		 ...
		 */
		$this->stopwords = apply_filters( 'gc_search_stopwords', $stopwords );
		return $this->stopwords;
	}
	...
	protected function parse_search_order( &$q ) {
		...
		if ( $q['search_terms_count'] > 1 ) {
			...
			if ( $like ) {
				$search_orderby .= $gcdb->prepare( "WHEN {$gcdb->posts}.post_title LIKE %s THEN 1 ", $like );
			}
			...
			if ( $like ) {
				$search_orderby .= $gcdb->prepare( "WHEN {$gcdb->posts}.post_excerpt LIKE %s THEN 4 ", $like );
				$search_orderby .= $gcdb->prepare( "WHEN {$gcdb->posts}.post_content LIKE %s THEN 5 ", $like );
			}
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			...
			case 'menu_order':
			case 'comment_count':
				$orderby_clause = "{$gcdb->posts}.{$orderby}";
				break;
			case 'rand':
				$orderby_clause = 'RAND()';
				break;
			...
			case 'post__in':
				if ( ! empty( $this->query_vars['post__in'] ) ) {
					$orderby_clause = "FIELD({$gcdb->posts}.ID," . implode( ',', array_map( 'absint', $this->query_vars['post__in'] ) ) . ')';
				}
				break;
			case 'post_parent__in':
				if ( ! empty( $this->query_vars['post_parent__in'] ) ) {
					$orderby_clause = "FIELD( {$gcdb->posts}.post_parent," . implode( ', ', array_map( 'absint', $this->query_vars['post_parent__in'] ) ) . ' )';
				}
				break;
			case 'post_name__in':
				if ( ! empty( $this->query_vars['post_name__in'] ) ) {
					...
					$orderby_clause       = "FIELD( {$gcdb->posts}.post_name," . $post_name__in_string . ' )';
				}
				break;
			default:
				if ( array_key_exists( $orderby, $meta_clauses ) ) {
					...
					$orderby_clause = "{$gcdb->posts}.post_" . sanitize_key( $orderby );
				}
	 ...
	 *
	 * @since 1.5.0 The `$default_value` argument was introduced.
	 *
	...
	public function get_posts() {
		 ...
		 * like is_main_query() test against the global $gc_query instance, not the passed one.
		 *
		 * @since 2.0.0
		 *
		 * @param GC_Query $query The GC_Query instance (passed by reference).
		 ...
		 */
		$this->allow_query_attachment_by_filename = apply_filters( 'gc_allow_query_attachment_by_filename', false );
		remove_all_filters( 'gc_allow_query_attachment_by_filename' );
		...
		switch ( $q['fields'] ) {
			case 'ids':
				$fields = "{$gcdb->posts}.ID";
				break;
			case 'id=>parent':
				$fields = "{$gcdb->posts}.ID, {$gcdb->posts}.post_parent";
				break;
			default:
				$fields = "{$gcdb->posts}.*";
		}
		...
		if ( '' !== $q['menu_order'] ) {
			$where .= " AND {$gcdb->posts}.menu_order = " . $q['menu_order'];
		}
		// The "m" parameter is meant for months but accepts datetimes of varying specificity.
		if ( $q['m'] ) {
			$where .= " AND YEAR({$gcdb->posts}.post_date)=" . substr( $q['m'], 0, 4 );
			if ( strlen( $q['m'] ) > 5 ) {
				$where .= " AND MONTH({$gcdb->posts}.post_date)=" . substr( $q['m'], 4, 2 );
			}
			if ( strlen( $q['m'] ) > 7 ) {
				$where .= " AND DAYOFMONTH({$gcdb->posts}.post_date)=" . substr( $q['m'], 6, 2 );
			}
			if ( strlen( $q['m'] ) > 9 ) {
				$where .= " AND HOUR({$gcdb->posts}.post_date)=" . substr( $q['m'], 8, 2 );
			}
			if ( strlen( $q['m'] ) > 11 ) {
				$where .= " AND MINUTE({$gcdb->posts}.post_date)=" . substr( $q['m'], 10, 2 );
			}
			if ( strlen( $q['m'] ) > 13 ) {
				$where .= " AND SECOND({$gcdb->posts}.post_date)=" . substr( $q['m'], 12, 2 );
			}
		}
		...
		if ( $date_parameters ) {
			$date_query = new GC_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $q['date_query'] ) ) {
			$this->date_query = new GC_Date_Query( $q['date_query'] );
		...
		if ( '' !== $q['title'] ) {
			$where .= $gcdb->prepare( " AND {$gcdb->posts}.post_title = %s", stripslashes( $q['title'] ) );
		}

		// Parameters related to 'post_name'.
		if ( '' !== $q['name'] ) {
			$q['name'] = sanitize_title_for_query( $q['name'] );
			$where    .= " AND {$gcdb->posts}.post_name = '" . $q['name'] . "'";
		} elseif ( '' !== $q['pagename'] ) {
			...
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || empty( $page_for_posts ) || ( $reqpage != $page_for_posts ) ) {
				$q['pagename'] = sanitize_title_for_query( gc_basename( $q['pagename'] ) );
				$q['name']     = $q['pagename'];
				$where        .= " AND ({$gcdb->posts}.ID = '$reqpage')";
			...
			$q['attachment'] = sanitize_title_for_query( gc_basename( $q['attachment'] ) );
			$q['name']       = $q['attachment'];
			$where          .= " AND {$gcdb->posts}.post_name = '" . $q['attachment'] . "'";
		} elseif ( is_array( $q['post_name__in'] ) && ! empty( $q['post_name__in'] ) ) {
			$q['post_name__in'] = array_map( 'sanitize_title_for_query', $q['post_name__in'] );
			$post_name__in      = "'" . implode( "','", $q['post_name__in'] ) . "'";
			$where             .= " AND {$gcdb->posts}.post_name IN ($post_name__in)";
		}
		...
		if ( $q['p'] ) {
			$where .= " AND {$gcdb->posts}.ID = " . $q['p'];
		} elseif ( $q['post__in'] ) {
			$post__in = implode( ',', array_map( 'absint', $q['post__in'] ) );
			$where   .= " AND {$gcdb->posts}.ID IN ($post__in)";
		} elseif ( $q['post__not_in'] ) {
			$post__not_in = implode( ',', array_map( 'absint', $q['post__not_in'] ) );
			$where       .= " AND {$gcdb->posts}.ID NOT IN ($post__not_in)";
		}

		if ( is_numeric( $q['post_parent'] ) ) {
			$where .= $gcdb->prepare( " AND {$gcdb->posts}.post_parent = %d ", $q['post_parent'] );
		} elseif ( $q['post_parent__in'] ) {
			$post_parent__in = implode( ',', array_map( 'absint', $q['post_parent__in'] ) );
			$where          .= " AND {$gcdb->posts}.post_parent IN ($post_parent__in)";
		} elseif ( $q['post_parent__not_in'] ) {
			$post_parent__not_in = implode( ',', array_map( 'absint', $q['post_parent__not_in'] ) );
			$where              .= " AND {$gcdb->posts}.post_parent NOT IN ($post_parent__not_in)";
		}

		if ( $q['page_id'] ) {
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || ( get_option( 'page_for_posts' ) != $q['page_id'] ) ) {
				$q['p'] = $q['page_id'];
				$where  = " AND {$gcdb->posts}.ID = " . $q['page_id'];
			}
		}
		...
		if ( ! $q['suppress_filters'] ) {
			 ...
			 * @param GC_Query $query  The current GC_Query object.
			 */
		...
		if ( ! $this->is_singular ) {
			...
			$clauses = $this->tax_query->get_sql( $gcdb->posts, 'ID' );
		...
		if ( ! empty( $q['author__not_in'] ) ) {
			...
			$where         .= " AND {$gcdb->posts}.post_author NOT IN ($author__not_in) ";
		} elseif ( ! empty( $q['author__in'] ) ) {
			$author__in = implode( ',', array_map( 'absint', array_unique( (array) $q['author__in'] ) ) );
			$where     .= " AND {$gcdb->posts}.post_author IN ($author__in) ";
		}
		...
		if ( '' !== $q['author_name'] ) {
			...
			$whichauthor .= " AND ({$gcdb->posts}.post_author = " . absint( $q['author'] ) . ')';
		}
		...
		if ( isset( $q['comment_count'] ) ) {
			...
			if ( isset( $q['comment_count']['value'] ) ) {
				...
				$where .= $gcdb->prepare( " AND {$gcdb->posts}.comment_count {$q['comment_count']['compare']} %d", $q['comment_count']['value'] );
			}
		}
		...
		if ( isset( $q['post_mime_type'] ) && '' !== $q['post_mime_type'] ) {
			$whichmimetype = gc_post_mime_type_where( $q['post_mime_type'], $gcdb->posts );
		}
		$where .= $search . $whichauthor . $whichmimetype;

		if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
			$join .= " LEFT JOIN {$gcdb->postmeta} AS sq1 ON ( {$gcdb->posts}.ID = sq1.post_id AND sq1.meta_key = '_gc_attached_file' )";
		}

		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'post', $gcdb->posts, 'ID', $this );
			$join   .= $clauses['join'];
			$where  .= $clauses['where'];
		}
		...
		if ( empty( $q['orderby'] ) ) {
			...
			if ( isset( $q['orderby'] ) && ( is_array( $q['orderby'] ) || false === $q['orderby'] ) ) {
				...
				$orderby = "{$gcdb->posts}.post_date " . $q['order'];
			}
		...
		if ( isset( $q['post_password'] ) ) {
			$where .= $gcdb->prepare( " AND {$gcdb->posts}.post_password = %s", $q['post_password'] );
			if ( empty( $q['perm'] ) ) {
				$q['perm'] = 'readable';
			}
		} elseif ( isset( $q['has_password'] ) ) {
			$where .= sprintf( " AND {$gcdb->posts}.post_password %s ''", $q['has_password'] ? '!=' : '=' );
		}

		if ( ! empty( $q['comment_status'] ) ) {
			$where .= $gcdb->prepare( " AND {$gcdb->posts}.comment_status = %s ", $q['comment_status'] );
		}

		if ( ! empty( $q['ping_status'] ) ) {
			$where .= $gcdb->prepare( " AND {$gcdb->posts}.ping_status = %s ", $q['ping_status'] );
		}
		...
		if ( 'any' === $post_type ) {
			...
			if ( empty( $in_search_post_types ) ) {
				...
				$post_type_where = " AND {$gcdb->posts}.post_type IN ('" . implode( "', '", array_map( 'esc_sql', $in_search_post_types ) ) . "')";
			}
		} elseif ( ! empty( $post_type ) && is_array( $post_type ) ) {
			$post_type_where = " AND {$gcdb->posts}.post_type IN ('" . implode( "', '", esc_sql( $post_type ) ) . "')";
		} elseif ( ! empty( $post_type ) ) {
			$post_type_where  = $gcdb->prepare( " AND {$gcdb->posts}.post_type = %s", $post_type );
			$post_type_object = get_post_type_object( $post_type );
		} elseif ( $this->is_attachment ) {
			$post_type_where  = " AND {$gcdb->posts}.post_type = 'attachment'";
			$post_type_object = get_post_type_object( 'attachment' );
		} elseif ( $this->is_page ) {
			$post_type_where  = " AND {$gcdb->posts}.post_type = 'page'";
			$post_type_object = get_post_type_object( 'page' );
		} else {
			$post_type_where  = " AND {$gcdb->posts}.post_type = 'post'";
		...
		if ( $skip_post_status ) {
			...
			if ( in_array( 'any', $q_status, true ) ) {
				foreach ( get_post_stati( array( 'exclude_from_search' => true ) ) as $status ) {
					if ( ! in_array( $status, $q_status, true ) ) {
						$e_status[] = "{$gcdb->posts}.post_status <> '$status'";
					}
				}
			} else {
				foreach ( get_post_stati() as $status ) {
					if ( in_array( $status, $q_status, true ) ) {
						if ( 'private' === $status ) {
							$p_status[] = "{$gcdb->posts}.post_status = '$status'";
						} else {
							$r_status[] = "{$gcdb->posts}.post_status = '$status'";
						}
					}
				}
			}
			...
			if ( ! empty( $r_status ) ) {
				if ( ! empty( $q['perm'] ) && 'editable' === $q['perm'] && ! current_user_can( $edit_others_cap ) ) {
					$statuswheres[] = "({$gcdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $r_status ) . '))';
				} else {
			...
			if ( ! empty( $p_status ) ) {
				if ( ! empty( $q['perm'] ) && 'readable' === $q['perm'] && ! current_user_can( $read_private_cap ) ) {
					$statuswheres[] = "({$gcdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $p_status ) . '))';
				} else {
					$statuswheres[] = '(' . implode( ' OR ', $p_status ) . ')';
				}
			}
			if ( $post_status_join ) {
				$join .= " LEFT JOIN {$gcdb->posts} AS p2 ON ({$gcdb->posts}.post_parent = p2.ID) ";
				foreach ( $statuswheres as $index => $statuswhere ) {
					$statuswheres[ $index ] = "($statuswhere OR ({$gcdb->posts}.post_status = 'inherit' AND " . str_replace( $gcdb->posts, 'p2', $statuswhere ) . '))';
				}
			}
			...
			if ( ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					$type_where = '(' . $gcdb->prepare( "{$gcdb->posts}.post_type = %s AND (", $queried_post_type );

					// Public statuses.
					$public_statuses = get_post_stati( array( 'public' => true ) );
					$status_clauses  = array();
					foreach ( $public_statuses as $public_status ) {
						$status_clauses[] = "{$gcdb->posts}.post_status = '$public_status'";
					}
					...
					if ( $this->is_admin ) {
						...
						foreach ( $admin_all_statuses as $admin_all_status ) {
							$type_where .= " OR {$gcdb->posts}.post_status = '$admin_all_status'";
						}
					}

					// Add private states that are visible to current user.
					if ( is_user_logged_in() && $queried_post_type_object instanceof GC_Post_Type ) {
						...
						foreach ( $private_statuses as $private_status ) {
							$type_where .= current_user_can( $read_private_cap ) ? " \nOR {$gcdb->posts}.post_status = '$private_status'" : " \nOR ({$gcdb->posts}.post_author = $user_id AND {$gcdb->posts}.post_status = '$private_status')";
						}
					}
		...
		if ( $this->is_comment_feed && ! $this->is_singular ) {
			if ( $this->is_archive || $this->is_search ) {
				$cjoin    = "JOIN {$gcdb->posts} ON ( {$gcdb->comments}.comment_post_ID = {$gcdb->posts}.ID ) $join ";
				$cwhere   = "WHERE comment_approved = '1' $where";
				$cgroupby = "{$gcdb->comments}.comment_id";
			} else { // Other non-singular, e.g. front.
				$cjoin    = "JOIN {$gcdb->posts} ON ( {$gcdb->comments}.comment_post_ID = {$gcdb->posts}.ID )";
			...
			$comments_request = "SELECT $distinct {$gcdb->comments}.comment_ID FROM {$gcdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$key          = md5( $comments_request );
			$last_changed = gc_cache_get_last_changed( 'comment' ) . ':' . gc_cache_get_last_changed( 'posts' );

			$cache_key   = "comment_feed:$key:$last_changed";
			$comment_ids = gc_cache_get( $cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $gcdb->get_col( $comments_request );
				gc_cache_add( $cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );

			// Convert to GC_Comment.
			/** @var GC_Comment[] */
			...
			$join     = '';
			if ( $post_ids ) {
				$where = "AND {$gcdb->posts}.ID IN ($post_ids) ";
			} else {
				$where = 'AND 0';
			}
		}
		 ...
		 *
		 * Return a non-null value to bypass GeChiUI' default post queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_posts` and `max_num_pages` properties of the GC_Query object,
		 * passed to the filter by reference. If GC_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 4.6.0
		 *
		 * @param GC_Post[]|int[]|null $posts Return an array of post data to short-circuit GC's query,
		...
		$cacheable_field_values = array(
			"{$gcdb->posts}.*",
			"{$gcdb->posts}.ID, {$gcdb->posts}.post_parent",
			"{$gcdb->posts}.ID",
		);
		...
		if ( $q['cache_results'] && $id_query_is_cacheable ) {
			$new_request = str_replace( $fields, "{$gcdb->posts}.*", $this->request );
			$cache_key   = $this->generate_cache_key( $q, $new_request );

			$cache_found = false;
			if ( null === $this->posts ) {
				$cached_results = gc_cache_get( $cache_key, 'post-queries', false, $cache_found );
		...
		if ( 'ids' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $gcdb->get_col( $this->request );
			}
			...
			if ( $q['cache_results'] && $id_query_is_cacheable ) {
				...
				gc_cache_set( $cache_key, $cache_value, 'post-queries' );
			}
		...
		if ( 'id=>parent' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $gcdb->get_results( $this->request );
			}
		...
		if ( null === $this->posts ) {
			$split_the_query = ( $old_request == $this->request && "{$gcdb->posts}.*" === $fields && ! empty( $limits ) && $q['posts_per_page'] < 500 );
			...
			if ( $split_the_query ) {
				...
				$this->request = "
					SELECT $found_rows $distinct {$gcdb->posts}.ID
					FROM {$gcdb->posts} $join
					WHERE 1=1 $where
					$groupby
					$orderby
				...
				$post_ids = $gcdb->get_col( $this->request );
		...
		if ( $q['cache_results'] && $id_query_is_cacheable && ! $cache_found ) {
			$post_ids = gc_list_pluck( $this->posts, 'ID' );
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			/** This filter is documented in gc-includes/query.php */
			...
			$comments_request = "SELECT {$gcdb->comments}.comment_ID FROM {$gcdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$comment_key          = md5( $comments_request );
			$comment_last_changed = gc_cache_get_last_changed( 'comment' );

			$comment_cache_key = "comment_feed:$comment_key:$comment_last_changed";
			$comment_ids       = gc_cache_get( $comment_cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $gcdb->get_col( $comments_request );
				gc_cache_add( $comment_cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param GC_Post  $post_preview  The Post object.
				 * @param GC_Query $query         The GC_Query instance (passed by reference).
				 */
		 ...
		 * of the type GC_Post and are filtered.
		 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
		...
		if ( $q['lazy_load_term_meta'] ) {
			gc_queue_posts_for_term_meta_lazyload( $this->posts );
		}
	...
	private function set_found_posts( $q, $limits ) {
		...
		if ( ! empty( $limits ) ) {
			...
			$this->found_posts = (int) $gcdb->get_var( $found_posts_query );
		} else {
	...
	/**
	 * Iterates current comment index and returns GC_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return GC_Comment Comment object.
	 */
	...
	/**
	 * Sets up the GeChiUI query by parsing query string.
	 *
	 * @since 1.5.0
	 *
	 * @see GC_Query::parse_query() for all available arguments.
	 *
	 ...
	 * Constructor.
	 *
	 * Sets up the GeChiUI query, if parameter is not empty.
	 *
	 * @since 1.5.0
	 *
	 * @see GC_Query::parse_query() for all available arguments.
	 ...
	 *
	 *
	 * @see GC_Query::is_category()
	 * @see GC_Query::is_tag()
	 * @see GC_Query::is_author()
	 * @see GC_Query::is_date()
	 * @see GC_Query::is_post_type_archive()
	 * @see GC_Query::is_tax()
	 *
	...
	public function is_tax( $taxonomy = '', $term = '' ) {
		...
		$tax_array      = array_intersect( array_keys( $gc_taxonomies ), (array) $taxonomy );
	 ...
	 *
	 * Otherwise the same as @see GC_Query::is_home()
	 *
	 *
	 ...
	 *
	 * If you set a static page for the front page of your site, this function will return
	 * true only on the page you set as the "文章页".
	 *
	 *
	 * @see GC_Query::is_front_page()
	 *
	 ...
	 *
	 * Depends on the site's "更改您的隐私政策页面" Privacy Settings 'gc_page_for_privacy_policy'.
	 *
	...
	public function is_privacy_policy() {
		if ( get_option( 'gc_page_for_privacy_policy' )
			&& $this->is_page( get_option( 'gc_page_for_privacy_policy' ) )
		) {
			return true;
		} else {
			return false;
	 ...
	 *
	 *
	 * @see GC_Query::is_single()
	 * @see GC_Query::is_singular()
	 *
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param GC_Post  $post  The Post object (passed by reference).
		 * @param GC_Query $query The current Query object (passed by reference).
		 */
	...
	public function generate_postdata( $post ) {
		...
		if ( str_contains( $content, '<!--nextpage-->' ) ) {
			...
			$content = str_replace( '<!-- gc:nextpage -->', '', $content );
			$content = str_replace( '<!-- /gc:nextpage -->', '', $content );
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$placeholder = $gcdb->placeholder_escape();
		array_walk_recursive(
			$args,
			/*
			 * Replace gcdb placeholders with the string used in the database
			 ...
			 */
			static function ( &$value ) use ( $gcdb, $placeholder ) {
				if ( is_string( $value ) && str_contains( $value, $placeholder ) ) {
					$value = $gcdb->remove_placeholder_escape( $value );
				}
			}
		);
		...
		return "gc_query:$key:$last_changed";
	}
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See gc_queue_posts_for_term_meta_lazyload().
	 *
	 * @param mixed $check
	 * @param int   $term_id
	 * @return mixed
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See gc_lazyload_comment_meta().
	 *
	 * @param mixed $check
