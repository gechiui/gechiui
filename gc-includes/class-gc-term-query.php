...
/**
 * Taxonomy API: GC_Term_Query class.
 *
 * @package GeChiUI
 * @subpackage Taxonomy
 *
...
class GC_Term_Query {
	 ...
	 *                                                   - 'all' Returns an array of complete term objects (`GC_Term[]`).
	 *                                                   - 'all_with_object_id' Returns an array of term objects
	 *                                                     with the 'object_id' param (`GC_Term[]`). Works only
	...
	public function parse_query( $query = '' ) {
		...
		$query = gc_parse_args( $query, $this->query_var_defaults );
	 ...
	 *
	 * The following will result in an array of `GC_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	...
	public function get_terms() {
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', gc_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( gc_parse_id_list( $exclude ), $exclusions );
		}
		...
		$this->sql_clauses['from']    = "FROM $gcdb->terms AS t $join";
		 ...
		 * @param GC_Term_Query $query The GC_Term_Query instance, passed by reference.
		 */
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', gc_parse_id_list( $this->query_vars['include'] ) );
