...
class GC_Theme_JSON_Resolver {
	...
	protected static function read_json_file( $file_path ) {
		if ( $file_path ) {
			...
			$decoded_file = gc_json_file_decode( $file_path, array( 'associative' => true ) );
	...
	protected static function translate( $theme_json, $domain = 'default' ) {
		if ( null === static::$i18n_schema ) {
			$i18n_schema         = gc_json_file_decode( __DIR__ . '/theme-i18n.json' );
	 ...
	 * @since 5.8.0
	 *
	 * @return GC_Theme_JSON Entity that holds core data.
	 */
	public static function get_core_data() {
		...
		/**
		 * Filters the default data provided by GeChiUI for global styles & settings.
		 *
		 * @since 6.1.0
		 *
		 * @param GC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json   = apply_filters( 'gc_theme_json_data_default', new GC_Theme_JSON_Data( $config, 'default' ) );
		$config       = $theme_json->get_data();
		static::$core = new GC_Theme_JSON( $config, 'default' );
	 ...
	 * }
	 * @return GC_Theme_JSON Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		$options = gc_parse_args( $options, array( 'with_supports' => true ) );

		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			$gc_theme        = gc_get_theme();
			$theme_json_file = $gc_theme->get_file_path( 'theme.json' );
			if ( is_readable( $theme_json_file ) ) {
				$theme_json_data = static::read_json_file( $theme_json_file );
				$theme_json_data = static::translate( $theme_json_data, $gc_theme->get( 'TextDomain' ) );
			} else {
			 ...
			 * @param GC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
			 */
			$theme_json      = apply_filters( 'gc_theme_json_data_theme', new GC_Theme_JSON_Data( $theme_json_data, 'theme' ) );
			$theme_json_data = $theme_json->get_data();
			static::$theme   = new GC_Theme_JSON( $theme_json_data );

			if ( $gc_theme->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_file = $gc_theme->parent()->get_file_path( 'theme.json' );
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					$parent_theme_json_data = static::read_json_file( $parent_theme_json_file );
					$parent_theme_json_data = static::translate( $parent_theme_json_data, $gc_theme->parent()->get( 'TextDomain' ) );
					$parent_theme           = new GC_Theme_JSON( $parent_theme_json_data );
		 ...
		 */
		$theme_support_data = GC_Theme_JSON::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		...
		$with_theme_supports = new GC_Theme_JSON( $theme_support_data );
	...
	public static function get_block_data() {
		...
		foreach ( $blocks as $block_name => $block_type ) {
			...
			if (
				...
				null === _gc_array_get( $config, array( 'styles', 'blocks', $block_name, 'spacing', 'blockGap' ), null )
			) {
				/*
		 ...
		 * @param GC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'gc_theme_json_data_blocks', new GC_Theme_JSON_Data( $config, 'blocks' ) );
		$config     = $theme_json->get_data();

		static::$blocks = new GC_Theme_JSON( $config, 'blocks' );
		return static::$blocks;
	}
	 ...
	 * @since 5.9.0
	 *
	 * @param GC_Theme $theme              The theme object. If empty, it
	 ...
	 */
	public static function get_user_data_from_gc_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		 ...
		 *
		 * Since gc_theme_has_theme_json() only supports the active
		 * theme, the extra condition for whether $theme is the active theme is
		 * present here.
		 */
		if ( $theme->get_stylesheet() === get_stylesheet() && ! gc_theme_has_theme_json() ) {
			return array();
		}

		$user_cpt         = array();
		$post_type_filter = 'gc_global_styles';
		...
		$global_style_query = new GC_Query();
		$recent_posts       = $global_style_query->query( $args );
		if ( count( $recent_posts ) === 1 ) {
			$user_cpt = get_object_vars( $recent_posts[0] );
		} elseif ( $create_post ) {
			$cpt_post_id = gc_insert_post(
				array(
					'post_content' => '{"version": ' . GC_Theme_JSON::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					'post_title'   => 'Custom Styles', // Do not make string translatable, see https://core.trac.gechiui.com/ticket/54518.
					'post_type'    => $post_type_filter,
					'post_name'    => sprintf( 'gc-global-styles-%s', urlencode( $stylesheet ) ),
					'tax_input'    => array(
						'gc_theme' => array( $stylesheet ),
					),
				),
				true
			);
			if ( ! is_gc_error( $cpt_post_id ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return GC_Theme_JSON Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		$config   = array();
		$user_cpt = static::get_user_data_from_gc_global_styles( gc_get_theme() );
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				 ...
				 * @param GC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'gc_theme_json_data_user', new GC_Theme_JSON_Data( $config, 'custom' ) );
				$config     = $theme_json->get_data();
				return new GC_Theme_JSON( $config, 'custom' );
			}
		...
		/** This filter is documented in gc-includes/class-gc-theme-json-resolver.php */
		$theme_json   = apply_filters( 'gc_theme_json_data_user', new GC_Theme_JSON_Data( $config, 'custom' ) );
		$config       = $theme_json->get_data();
		static::$user = new GC_Theme_JSON( $config, 'custom' );
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.gechiui.com/reference/classes/gc_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.gechiui.com/reference/classes/gc_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.gechiui.com/reference/classes/gc_theme_json_resolver/get_user_data/ get_user_data},
	...
	public static function get_user_global_styles_post_id() {
		...
		$user_cpt = static::get_user_data_from_gc_global_styles( gc_get_theme(), true );
	 ...
	 * @deprecated 6.2.0 Use gc_theme_has_theme_json() instead.
	 *
	 * @return bool
	 */
	public static function theme_has_support() {
		_deprecated_function( __METHOD__, '6.2.0', 'gc_theme_has_theme_json()' );

		return gc_theme_has_theme_json();
	}
	...
	public static function get_style_variations() {
		...
		foreach ( $variation_files as $path => $file ) {
			$decoded_file = gc_json_file_decode( $path, array( 'associative' => true ) );
			if ( is_array( $decoded_file ) ) {
				$translated = static::translate( $decoded_file, gc_get_theme()->get( 'TextDomain' ) );
				$variation  = ( new GC_Theme_JSON( $translated ) )->get_raw_data();
