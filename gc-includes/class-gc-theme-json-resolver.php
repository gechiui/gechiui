...
class GC_Theme_JSON_Resolver {
	...
	public static function get_theme_data( $deprecated = array() ) {
		...
		if ( null === static::$theme ) {
			...
			$theme_json_data = static::translate( $theme_json_data, gc_get_theme()->get( 'TextDomain' ) );
			static::$theme     = new GC_Theme_JSON( $theme_json_data );

			if ( gc_get_theme()->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_data = static::read_json_file( static::get_file_path_from_theme( 'theme.json', true ) );
				$parent_theme_json_data = static::translate( $parent_theme_json_data, gc_get_theme()->parent()->get( 'TextDomain' ) );
		 ...
		 */
		$theme_support_data = GC_Theme_JSON::get_from_editor_settings( get_default_block_editor_settings() );
	...
	public static function get_user_data_from_gc_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		...
		$cache_key = sprintf( 'gc_global_styles_%s', md5( serialize( $args ) ) );
		$post_id   = gc_cache_get( $cache_key );
		...
		$recent_posts = gc_get_recent_posts( $args );
		if ( is_array( $recent_posts ) && ( count( $recent_posts ) === 1 ) ) {
			...
			$cpt_post_id = gc_insert_post(
				array(
					...
					'post_type'    => $post_type_filter,
					'post_name'    => 'gc-global-styles-' . urlencode( gc_get_theme()->get_stylesheet() ),
					'tax_input'    => array(
						'gc_theme' => array( gc_get_theme()->get_stylesheet() ),
					),
				),
				true
			);
		...
		gc_cache_set( $cache_key, $user_cpt ? $user_cpt['ID'] : -1, '', $cache_expiration );
