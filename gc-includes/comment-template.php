 ...
 *
 * These functions are meant to live inside of the GeChiUI loop.
 *
 * @package GeChiUI
 * @subpackage Template
 */
...
function get_comment_link( $comment = null, $args = array() ) {
	global $gc_rewrite, $in_comment_loop;
...
function trackback_rdf( $deprecated = '' ) {
	...
	the_permalink();
	echo '"' . "\n";
	echo '    dc:title="' . str_replace( '--', '&#x2d;&#x2d;', gctexturize( strip_tags( get_the_title() ) ) ) . '"' . "\n";
...
function gc_comment_form_unfiltered_html_nonce() {
	...
	if ( current_user_can( 'unfiltered_html' ) ) {
		...
		echo "<script>(function(){if(window===window.parent){document.getElementById('_gc_unfiltered_html_comment_disabled').name='_gc_unfiltered_html_comment';}})();</script>\n";
	}
}
 ...
 *
 * Uses the GeChiUI database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the GeChiUI process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $gc_query, $withcomments, $post, $gcdb, $id, $comment, $user_login, $user_identity, $overridden_cpage;
	...
	if ( get_option( 'page_comments' ) ) {
		...
		if ( $page ) {
			...
			$top_level_query = new GC_Comment_Query();
	...
	$comment_query = new GC_Comment_Query( $comment_args );
	...
	$gc_query->comment_count         = count( $gc_query->comments );
	$gc_query->max_num_comment_pages = $comment_query->max_num_pages;

	if ( $separate_comments ) {
		$gc_query->comments_by_type = separate_comments( $comments );
		$comments_by_type           = &$gc_query->comments_by_type;
	} else {
		$gc_query->comments_by_type = array();
	}

	$overridden_cpage = false;

	if ( '' == get_query_var( 'cpage' ) && $gc_query->max_num_comment_pages > 1 ) {
	...
	if ( file_exists( $include ) ) {
		...
		require ABSPATH . GCINC . '/theme-compat/comments.php';
	}
}
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	if ( false === $zero ) {
		...
		$zero = sprintf( __( '<span class="screen-reader-text">%s</span>无评论' ), $post_title );
	}

	if ( false === $one ) {
		/* translators: %s: Post title. */
		$one = sprintf( __( '<span class="screen-reader-text">%s</span>有1条评论' ), $post_title );
	}
 ...
 *     @type string $login_text The text of the link to reply if logged out. Default '登录以回复'.
 ...
 *                              to addComment.moveForm(), and appended to the link URL as a hash value.
 *                              Default 'respond'.
 *     @type string $reply_text Text of the Reply link. Default is '发表评论'.
 *     @type string $login_text Text of the link to reply if logged out. Default is '登录以发表评论'.
 ...
 */
function gc_list_comments( $args = array(), $comments = null ) {
	global $gc_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 */
	$parsed_args = apply_filters( 'gc_list_comments_args', $parsed_args );
	...
	if ( null !== $comments ) {
		...
		/*
		 * If 'page' or 'per_page' has been passed, and does not match what's in $gc_query,
		 * perform a separate comment query and allow Walker_Comment to paginate.
		 */
		if ( $parsed_args['page'] || $parsed_args['per_page'] ) {
			$current_cpage = get_query_var( 'cpage' );
			if ( ! $current_cpage ) {
				$current_cpage = 'newest' === get_option( 'default_comments_page' ) ? 1 : $gc_query->max_num_comment_pages;
			}
			...
			// Otherwise, fall back on the comments from `$gc_query->comments`.
		} else {
			if ( empty( $gc_query->comments ) ) {
				return;
			}
			if ( 'all' !== $parsed_args['type'] ) {
				if ( empty( $gc_query->comments_by_type ) ) {
					$gc_query->comments_by_type = separate_comments( $gc_query->comments );
				}
				if ( empty( $gc_query->comments_by_type[ $parsed_args['type'] ] ) ) {
					return;
				}
				$_comments = $gc_query->comments_by_type[ $parsed_args['type'] ];
			} else {
				$_comments = $gc_query->comments;
			}

			if ( $gc_query->max_num_comment_pages ) {
 ...
 *     @type string $action               The comment form element action attribute. Default '/gc-comments-post.php'.
 ...
 *     @type string $cancel_reply_link    The translatable 'cancel reply' button label. Default '取消回复'.
...
function comment_form( $args = array(), $post_id = null ) {
	...
	if ( has_action( 'set_comment_cookies', 'gc_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		...
		$fields['cookies'] = sprintf(
			...
			sprintf(
				'<input id="gc-comment-cookies-consent" name="gc-comment-cookies-consent" type="checkbox" value="yes"%s />',
				$consent
			),
			sprintf(
				'<label for="gc-comment-cookies-consent">%s</label>',
				__( '在此浏览器中保存我的显示名称、邮箱地址和网站地址，以便下次评论时使用。' )
			)
		);
