 ...
 */
function gc_register_typography_support( $block_type ) {
	if ( ! property_exists( $block_type, 'supports' ) ) {
		return;
	}

	$typography_supports = _gc_array_get( $block_type->supports, array( 'typography' ), false );
	if ( ! $typography_supports ) {
		return;
	}

	$has_font_family_support     = _gc_array_get( $typography_supports, array( '__experimentalFontFamily' ), false );
	$has_font_size_support       = _gc_array_get( $typography_supports, array( 'fontSize' ), false );
	$has_font_style_support      = _gc_array_get( $typography_supports, array( '__experimentalFontStyle' ), false );
	$has_font_weight_support     = _gc_array_get( $typography_supports, array( '__experimentalFontWeight' ), false );
	$has_letter_spacing_support  = _gc_array_get( $typography_supports, array( '__experimentalLetterSpacing' ), false );
	$has_line_height_support     = _gc_array_get( $typography_supports, array( 'lineHeight' ), false );
	$has_text_columns_support    = _gc_array_get( $typography_supports, array( 'textColumns' ), false );
	$has_text_decoration_support = _gc_array_get( $typography_supports, array( '__experimentalTextDecoration' ), false );
	$has_text_transform_support  = _gc_array_get( $typography_supports, array( '__experimentalTextTransform' ), false );
 ...
 */
function gc_apply_typography_support( $block_type, $block_attributes ) {
	if ( ! property_exists( $block_type, 'supports' ) ) {
		return array();
	}
	...
	if ( gc_should_skip_block_supports_serialization( $block_type, 'typography' ) ) {
		return array();
	}
	...
	$should_skip_font_size       = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontSize' );
	$should_skip_font_family     = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontFamily' );
	$should_skip_font_style      = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontStyle' );
	$should_skip_font_weight     = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontWeight' );
	$should_skip_line_height     = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'lineHeight' );
	$should_skip_text_columns    = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'textColumns' );
	$should_skip_text_decoration = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'textDecoration' );
	$should_skip_text_transform  = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'textTransform' );
	$should_skip_letter_spacing  = gc_should_skip_block_supports_serialization( $block_type, 'typography', 'letterSpacing' );
	...
	if ( $has_font_size_support && ! $should_skip_font_size ) {
		...
		$custom_font_size                    = isset( $block_attributes['style']['typography']['fontSize'] )
			...
			: null;
		$typography_block_styles['fontSize'] = $preset_font_size ? $preset_font_size : gc_get_typography_font_size_value(
			array(
	...
	if ( $has_font_family_support && ! $should_skip_font_family ) {
		...
		$custom_font_family                    = isset( $block_attributes['style']['typography']['fontFamily'] )
			? gc_typography_get_preset_inline_style_value( $block_attributes['style']['typography']['fontFamily'], 'font-family' )
			: null;
	...
	if (
		...
		$typography_block_styles['fontStyle'] = gc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['fontWeight'] = gc_typography_get_preset_inline_style_value(
	...
	if ( $has_line_height_support && ! $should_skip_line_height ) {
		$typography_block_styles['lineHeight'] = _gc_array_get( $block_attributes, array( 'style', 'typography', 'lineHeight' ) );
	}

	if ( $has_text_columns_support && ! $should_skip_text_columns && isset( $block_attributes['style']['typography']['textColumns'] ) ) {
		$typography_block_styles['textColumns'] = _gc_array_get( $block_attributes, array( 'style', 'typography', 'textColumns' ), null );
	}
	...
	if (
		...
		$typography_block_styles['textDecoration'] = gc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['textTransform'] = gc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['letterSpacing'] = gc_typography_get_preset_inline_style_value(
	...
	$attributes = array();
	$styles     = gc_style_engine_get_styles(
 ...
 * * It mostly replaces the deprecated `gc_typography_get_css_variable_inline_style()`,
 *   but skips compiling a CSS declaration as the style engine takes over this role.
 * @link https://github.com/gechiui/gutenberg/pull/27555
 *
 * @since 6.1.0
 *
 ...
 */
function gc_typography_get_preset_inline_style_value( $style_value, $css_property ) {
	 ...
	 * Presets were removed in GeChiUI/gutenberg#27555.
	 * A preset CSS variable is the style.
	 * Gets the style value from the string and return CSS style.
	 */
	$index_to_splice = strrpos( $style_value, '|' ) + 1;
	$slug            = _gc_to_kebab_case( substr( $style_value, $index_to_splice ) );

	// Return the actual CSS inline style value,
	// e.g. `var(--gc--preset--text-decoration--underline);`.
	return sprintf( 'var(--gc--preset--%s--%s);', $css_property, $slug );
}
 ...
 */
function gc_render_typography_support( $block_content, $block ) {
	if ( ! isset( $block['attrs']['style']['typography']['fontSize'] ) ) {
		return $block_content;
	}

	$custom_font_size = $block['attrs']['style']['typography']['fontSize'];
	$fluid_font_size  = gc_get_typography_font_size_value( array( 'size' => $custom_font_size ) );
 ...
 *                    `null` on failure.
 */
function gc_get_typography_value_and_unit( $raw_value, $options = array() ) {
	if ( ! is_string( $raw_value ) && ! is_int( $raw_value ) && ! is_float( $raw_value ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			__( '原始大小值必须是字符串、整数或浮点数。' ),
			'6.1.0'
		);
		return null;
	}
 ...
 */
function gc_get_computed_fluid_typography_value( $args = array() ) {
	...
	$minimum_font_size = gc_get_typography_value_and_unit( $minimum_font_size_raw );
	...
	$maximum_font_size = gc_get_typography_value_and_unit(
	...
	$minimum_font_size_rem = gc_get_typography_value_and_unit(
	...
	$maximum_viewport_width = gc_get_typography_value_and_unit(
		$maximum_viewport_width_raw,
		array(
			'coerce_to' => $font_size_unit,
		)
	);
	$minimum_viewport_width = gc_get_typography_value_and_unit(
 ...
 */
function gc_get_typography_font_size_value( $preset, $should_use_fluid_typography = false ) {
	...
	$global_settings     = gc_get_global_settings();
	...
	$has_min_font_size                    = isset( $fluid_settings['minFontSize'] ) &&
		! empty( gc_get_typography_value_and_unit( $fluid_settings['minFontSize'] ) );
	...
	// Font sizes.
	$preferred_size = gc_get_typography_value_and_unit( $preset['size'] );
	 ...
	 */
	$minimum_font_size_limit = gc_get_typography_value_and_unit(
	...
	$fluid_font_size_value = gc_get_computed_fluid_typography_value(
		array(
...
GC_Block_Supports::get_instance()->register(
	'typography',
	array(
		'register_attribute' => 'gc_register_typography_support',
		'apply'              => 'gc_apply_typography_support',
	)
);
