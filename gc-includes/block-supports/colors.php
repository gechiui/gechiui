 ...
 */
function gc_register_colors_support( $block_type ) {
	$color_support = false;
	if ( property_exists( $block_type, 'supports' ) ) {
		$color_support = _gc_array_get( $block_type->supports, array( 'color' ), false );
	}
	$has_text_colors_support       = true === $color_support || ( is_array( $color_support ) && _gc_array_get( $color_support, array( 'text' ), true ) );
	$has_background_colors_support = true === $color_support || ( is_array( $color_support ) && _gc_array_get( $color_support, array( 'background' ), true ) );
	$has_gradients_support         = _gc_array_get( $color_support, array( 'gradients' ), false );
	$has_link_colors_support       = _gc_array_get( $color_support, array( 'link' ), false );
 ...
 */
function gc_apply_colors_support( $block_type, $block_attributes ) {
	$color_support = _gc_array_get( $block_type->supports, array( 'color' ), false );
	...
	if ( $has_text_colors_support ) {
		...
		if ( $has_named_text_color ) {
			$classes[] = sprintf( 'has-%s-color', _gc_to_kebab_case( $block_attributes['textColor'] ) );
	...
	if ( $has_background_colors_support ) {
		...
		if ( $has_named_background_color ) {
			$classes[] = sprintf( 'has-%s-background-color', _gc_to_kebab_case( $block_attributes['backgroundColor'] ) );
	...
	if ( $has_gradients_support ) {
		...
		if ( $has_named_gradient ) {
			$classes[] = sprintf( 'has-%s-gradient-background', _gc_to_kebab_case( $block_attributes['gradient'] ) );
...
GC_Block_Supports::get_instance()->register(
	'colors',
	array(
		'register_attribute' => 'gc_register_colors_support',
		'apply'              => 'gc_apply_colors_support',
	)
);
