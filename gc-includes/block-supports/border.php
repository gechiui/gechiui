 ...
 */
function gc_register_border_support( $block_type ) {
	...
	if ( gc_has_border_feature_support( $block_type, 'color' ) && ! array_key_exists( 'borderColor', $block_type->attributes ) ) {
...
function gc_apply_border_support( $block_type, $block_attributes ) {
	...
	$has_border_color_support = gc_has_border_feature_support( $block_type, 'color' );
	$has_border_width_support = gc_has_border_feature_support( $block_type, 'width' );
	...
	if (
		...
		$custom_border_color          = _gc_array_get( $block_attributes, array( 'style', 'border', 'color' ), null );
	...
	if ( $has_border_color_support || $has_border_width_support ) {
		foreach ( array( 'top', 'right', 'bottom', 'left' ) as $side ) {
			$border                       = _gc_array_get( $block_attributes, array( 'style', 'border', $side ), null );
			$border_side_values           = array(
				'width' => isset( $border['width'] ) && ! gc_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'width' ) ? $border['width'] : null,
				'color' => isset( $border['color'] ) && ! gc_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'color' ) ? $border['color'] : null,
				'style' => isset( $border['style'] ) && ! gc_should_skip_block_supports_serialization( $block_type, '__experimentalBorder', 'style' ) ? $border['style'] : null,
			);
	...
	$attributes = array();
	$styles     = gc_style_engine_get_styles( array( 'border' => $border_block_styles ) );
 ...
 */
function gc_has_border_feature_support( $block_type, $feature, $default_value = false ) {
	// Check if all border support features have been opted into via `"__experimentalBorder": true`.
	if (
		property_exists( $block_type, 'supports' ) &&
		( true === _gc_array_get( $block_type->supports, array( '__experimentalBorder' ), $default_value ) )
	) {
		return true;
	}
...
GC_Block_Supports::get_instance()->register(
	'border',
	array(
		'register_attribute' => 'gc_register_border_support',
		'apply'              => 'gc_apply_border_support',
	)
);
