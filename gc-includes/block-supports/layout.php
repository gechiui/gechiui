 ...
 */
function gc_get_layout_definitions() {
		...
		'constrained' => array(
			...
			'baseStyles'    => array(
				...
				array(
					...
					'rules'    => array(
						'max-width'    => 'var(--gc--style--global--content-size)',
						'margin-left'  => 'auto !important',
						'margin-right' => 'auto !important',
					),
				),
				array(
					'selector' => ' > .alignwide',
					'rules'    => array(
						'max-width' => 'var(--gc--style--global--wide-size)',
					),
				),
			),
			'spacingStyles' => array(
 ...
 */
function gc_get_layout_style( $selector, $layout, $has_block_gap_support = false, $gap_value = null, $should_skip_gap_serialization = false, $fallback_gap_value = '0.5em', $block_spacing = null ) {
	...
	if ( 'default' === $layout_type ) {
		if ( $has_block_gap_support ) {
			...
			if ( null !== $gap_value && ! $should_skip_gap_serialization ) {
				...
				if ( is_string( $gap_value ) && str_contains( $gap_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _gc_to_kebab_case( substr( $gap_value, $index_to_splice ) );
					$gap_value       = "var(--gc--preset--spacing--$slug)";
				}
		...
		if ( $content_size || $wide_size ) {
			...
			if ( isset( $block_spacing ) ) {
				$block_spacing_values = gc_style_engine_get_styles(
					array(
		...
		if ( $has_block_gap_support && isset( $gap_value ) ) {
			...
			foreach ( $gap_sides as $gap_side ) {
				$process_value = is_string( $gap_value ) ? $gap_value : _gc_array_get( $gap_value, array( $gap_side ), $fallback_gap_value );
				// Get spacing CSS variable from preset value if provided.
				if ( is_string( $process_value ) && str_contains( $process_value, 'var:preset|spacing|' ) ) {
					$index_to_splice = strrpos( $process_value, '|' ) + 1;
					$slug            = _gc_to_kebab_case( substr( $process_value, $index_to_splice ) );
					$process_value   = "var(--gc--preset--spacing--$slug)";
				}
	...
	if ( ! empty( $layout_styles ) ) {
		 ...
		 * Since https://github.com/GeChiUI/gutenberg/pull/42452,
		 * gc_enqueue_block_support_styles is no longer called in this block supports file.
		 */
		return gc_style_engine_get_stylesheet_from_css_rules(
			$layout_styles,
			array(
...
function gc_render_layout_support_flag( $block_content, $block ) {
	...
	if ( $has_child_layout && ( 'fixed' === $block['attrs']['style']['layout']['selfStretch'] || 'fill' === $block['attrs']['style']['layout']['selfStretch'] ) ) {
		$container_content_class = gc_unique_id( 'gc-container-content-' );
	...
	if ( ! $support_layout && ! empty( $outer_class_names ) ) {
		$content = new GC_HTML_Tag_Processor( $block_content );
		$content->next_tag();
	...
	$fallback_layout = ! empty( _gc_array_get( $block_type->supports, array( 'layout', 'default' ), array() ) ) ? _gc_array_get( $block_type->supports, array( 'layout', 'default' ), array() ) : _gc_array_get( $block_type->supports, array( '__experimentalLayout', 'default' ), array() );
	$used_layout     = isset( $block['attrs']['layout'] ) ? $block['attrs']['layout'] : $fallback_layout;

	$class_names        = array();
	$layout_definitions = gc_get_layout_definitions();
	$container_class    = gc_unique_id( 'gc-container-' );
	$layout_classname   = '';
	...
	$root_padding_aware_alignments = _gc_array_get( $global_settings, array( 'useRootPaddingAwareAlignments' ), false );
	 ...
	 * removed in the 5.9 release (https://github.com/GeChiUI/gutenberg/issues/38719). It is
	...
	if ( isset( $used_layout['type'] ) ) {
		$layout_classname = _gc_array_get( $layout_definitions, array( $used_layout['type'], 'className' ), '' );
	} else {
		$layout_classname = _gc_array_get( $layout_definitions, array( 'default', 'className' ), '' );
	}
	...
	if ( ! current_theme_supports( 'disable-layout-styles' ) ) {
		...
		$fallback_gap_value = _gc_array_get( $block_type->supports, array( 'spacing', 'blockGap', '__experimentalDefault' ), '0.5em' );
		$block_spacing      = _gc_array_get( $block, array( 'attrs', 'style', 'spacing' ), null );

		/*
		 * If a block's block.json skips serialization for spacing or spacing.blockGap,
		 * don't apply the user-defined value to the styles.
		 */
		$should_skip_gap_serialization = gc_should_skip_block_supports_serialization( $block_type, 'spacing', 'blockGap' );

		$block_gap             = _gc_array_get( $global_settings, array( 'spacing', 'blockGap' ), null );
		$has_block_gap_support = isset( $block_gap );

		$style = gc_get_layout_style(
	...
	$class_names[] = 'gc-block-' . end( $block_name ) . '-' . $layout_classname;

	$content_with_outer_classnames = '';

	if ( ! empty( $outer_class_names ) ) {
		$content_with_outer_classnames = new GC_HTML_Tag_Processor( $block_content );
	...
	*/
	...
	if ( isset( $block['innerContent'][0] ) && 'string' === gettype( $block['innerContent'][0] ) && count( $block['innerContent'] ) > 1 ) {
		$tags            = new GC_HTML_Tag_Processor( $block['innerContent'][0] );
		$last_classnames = '';
		while ( $tags->next_tag() ) {
	...
	$content = $content_with_outer_classnames ? new GC_HTML_Tag_Processor( $content_with_outer_classnames ) : new GC_HTML_Tag_Processor( $block_content );
...
add_filter( 'render_block_core/group', 'gc_restore_group_inner_container', 10, 2 );
 ...
 * @return string Filtered block content.
 */
function gc_restore_image_outer_container( $block_content, $block ) {
	$image_with_align = "
...
(
	...
	$wrapper_classnames = array( 'gc-block-image' );
...
add_filter( 'render_block_core/image', 'gc_restore_image_outer_container', 10, 2 );
