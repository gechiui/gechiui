<?php
/**
 * Taxonomy API: GC_Tax_Query class
 *
 * @package GeChiUI
 * @subpackage Taxonomy
 *
 ...
 * taxonomy terms.
 *
 * GC_Tax_Query is a helper that allows primary query classes, such as GC_Query, to filter
...
class GC_Tax_Query {
	 ...
	 *
	 * See GC_Tax_Query::__construct() for information on tax query arguments.
	 *
	 * @var array
	 */
	...
	public function sanitize_query( $queries ) {
		...
		foreach ( $queries as $key => $query ) {
			if ( 'relation' === $key ) {
				 ...
				 * $queried_terms array, for use in GC_Query.
				 */
	...
	public function get_sql_for_clause( &$clause, $parent_query ) {
		...
		if ( 'IN' === $operator ) {
			...
			if ( false === $alias ) {
				...
				$alias = $i ? 'tt' . $i : $gcdb->term_relationships;
				...
				$join .= " LEFT JOIN $gcdb->term_relationships";
				$join .= $i ? " AS $alias" : '';
	...
	private function clean_query( &$query ) {
		if ( empty( $query['taxonomy'] ) ) {
			if ( 'term_taxonomy_id' !== $query['field'] ) {
				$query = new GC_Error( 'invalid_taxonomy', __( '无效分类法。' ) );
				return;
			}
		...
		if ( 'slug' === $query['field'] || 'name' === $query['field'] ) {
			...
			$query['terms'] = gc_parse_id_list( $query['terms'] );
		}

		if ( is_taxonomy_hierarchical( $query['taxonomy'] ) && $query['include_children'] ) {
			$this->transform_query( $query, 'term_id' );

			if ( is_gc_error( $query ) ) {
				return;
			}
	...
	public function transform_query( &$query, $resulting_field ) {
		...
		if ( 'AND' === $query['operator'] && count( $term_list ) < count( $query['terms'] ) ) {
			$query = new GC_Error( 'inexistent_terms', __( '不存在的项目。' ) );
			return;
		}

		$query['terms'] = gc_list_pluck( $term_list, $resulting_field );
		$query['field'] = $resulting_field;
	}
}
