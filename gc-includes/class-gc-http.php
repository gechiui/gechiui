...
class GC_Http {
	 ...
	 *                                             Default 'GeChiUI/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through gc_http_validate_url().
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . GCINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be dropped it in the GC temp dir and its name will
	 ...
	 *
	 * }
	 * @return array|GC_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A GC_Error instance upon error.
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			 ...
			 * @param string $url        The request URL.
			 */
			'user-agent'          => apply_filters( 'http_headers_useragent', 'GeChiUI/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url ),
			/**
			...
			'decompress'          => true,
			'sslverify'           => true,
			'sslcertificates'     => ABSPATH . GCINC . '/certificates/ca-bundle.crt',
			'stream'              => false,
			'filename'            => null,
		...
		if ( function_exists( 'gc_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
				$url = gc_http_validate_url( $url );
			}
			if ( $url ) {
				$url = gc_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		// If we are streaming to a file but no filename was given drop it in the GC temp dir
		...
		// GC allows passing in headers as a string, weirdly.
		if ( ! is_array( $parsed_args['headers'] ) ) {
			$processed_headers      = GC_Http::processHeaders( $parsed_args['headers'] );
		...
		if ( function_exists( 'gc_kses_bad_protocol' ) && $parsed_args['reject_unsafe_urls'] ) {
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new GC_Error( 'http_failure', __( '没有可以完成请求的HTTP传输器。' ) );
		}
	 ...
	 *                                        an array containing `GC_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new GC_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to GC_Http_Cookie instances,
	 ...
	 * prevent plugins from working and core functionality, if you don't include `api.gechiui.com`.
	 *
	 * You block external URL requests by defining `GC_HTTP_BLOCK_EXTERNAL` as true in your `gc-config.php`
	 * file and this will only allow localhost and your site to make requests. The constant
	 * `GC_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `GC_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.gechiui.com` will allow for all subdomains of `www.gechiui.com` to be contacted.
	 *
	 *
	 * @link https://core.trac.gechiui.com/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.gechiui.com/ticket/14636 Allow wildcard domains in GC_ACCESSIBLE_HOSTS
	 *
	 ...
	 *
	 * @deprecated 4.4.0 Use gc_parse_url()
	 * @see gc_parse_url()
	 *
	...
	protected static function parse_url( $url ) {
		_deprecated_function( __METHOD__, '4.4.0', 'gc_parse_url()' );
		return gc_parse_url( $url );
	}
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = gc_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = gc_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|GC_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a GC_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new GC_Error( 'http_request_failed', __( '重定向过多。' ) );
		}
