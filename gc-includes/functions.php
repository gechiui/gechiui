 ...
 */
function gc_timezone() {
	return new DateTimeZone( gc_timezone_string() );
}
 ...
 *
 * Note that due to the way GC typically generates a sum of timestamp and offset
 ...
 */
function gc_date( $format, $timestamp = null, $timezone = null ) {
	global $gc_locale;
	...
	if ( empty( $gc_locale->month ) || empty( $gc_locale->weekday ) ) {
		...
		for ( $i = 0; $i < $format_length; $i ++ ) {
			switch ( $format[ $i ] ) {
				...
				case 'l':
					...
					break;
				case 'M':
					$new_format .= addcslashes( $gc_locale->get_month_abbrev( $month ), '\\A..Za..z' );
					break;
				case 'a':
					$new_format .= addcslashes( $gc_locale->get_meridiem( $datetime->format( 'a' ) ), '\\A..Za..z' );
					break;
				case 'A':
					$new_format .= addcslashes( $gc_locale->get_meridiem( $datetime->format( 'A' ) ), '\\A..Za..z' );
					break;
		...
		$date = gc_maybe_decline_date( $date, $format );
	}
	 ...
	 */
	$date = apply_filters( 'gc_date', $date, $format, $timestamp, $timezone );
 ...
 */
function gc_maybe_decline_date( $date, $format = '' ) {
	global $gc_locale;
	...
	if ( 'on' === _x( 'off', 'decline months names: on or off' ) ) {
		...
		$months          = $gc_locale->month;
		$months_genitive = $gc_locale->month_genitive;
...
function human_readable_duration( $duration = '' ) {
	...
	if ( is_numeric( $hour ) ) {
		...
		$human_readable_duration[] = sprintf( _n( '%s小时', '%s小时', $hour ), (int) $hour );
	}

	// Add the minute part to the string.
	if ( is_numeric( $minute ) ) {
		/* translators: %s: Time duration in minute or minutes. */
		$human_readable_duration[] = sprintf( _n( '%s分', '%s分', $minute ), (int) $minute );
	}

	// Add the second part to the string.
	if ( is_numeric( $second ) ) {
		/* translators: %s: Time duration in second or seconds. */
		$human_readable_duration[] = sprintf( _n( '%s秒', '%s秒', $second ), (int) $second );
	}
...
function maybe_serialize( $data ) {
	 ...
	 * See https://core.trac.gechiui.com/ticket/12930
 ...
 */
function gc_extract_urls( $content ) {
	preg_match_all(
		"#([\"']?)("
			. '(?:([\w-]+:)?//?)'
			. '[^\s()<>]+'
...
function do_enclose( $content, $post ) {
	...
	$post_links_temp = gc_extract_urls( $content );

	foreach ( $pung as $link_test ) {
		// Link is no longer in post.
		if ( ! in_array( $link_test, $post_links_temp, true ) ) {
			$mids = $gcdb->get_col( $gcdb->prepare( "SELECT meta_id FROM $gcdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE %s", $post->ID, $gcdb->esc_like( $link_test ) . '%' ) );
			foreach ( $mids as $mid ) {
...
function gc_get_http_headers( $url, $deprecated = false ) {
	...
	$response = gc_safe_remote_head( $url );

	if ( is_gc_error( $response ) ) {
		return false;
	}

	return gc_remote_retrieve_headers( $response );
}
 ...
 */
function gc_removable_query_args() {
...
function gc_remote_fopen( $uri ) {
	...
	$response = gc_safe_remote_get( $uri, $options );

	if ( is_gc_error( $response ) ) {
		return false;
	}

	return gc_remote_retrieve_body( $response );
}
 ...
 * @global GC_Query $gc_the_query Copy of the GeChiUI Query object.
 *
 * @param string|array $query_vars Default GC_Query arguments.
 */
function gc( $query_vars = '' ) {
	global $gc, $gc_query, $gc_the_query;

	$gc->main( $query_vars );

	if ( ! isset( $gc_the_query ) ) {
...
function get_status_header_desc( $code ) {
	...
	if ( ! isset( $gc_header_to_desc ) ) {
		$gc_header_to_desc = array(
			100 => 'Continue',
	...
	if ( isset( $gc_header_to_desc[ $code ] ) ) {
		return $gc_header_to_desc[ $code ];
	} else {
		return '';
	}
}
...
function do_feed() {
	 ...
	 */
	do_action( "do_feed_{$feed}", $gc_query->is_comment_feed, $feed );
}
...
function do_feed_rss2( $for_comments ) {
	if ( $for_comments ) {
		load_template( ABSPATH . GCINC . '/feed-rss2-comments.php' );
	} else {
		load_template( ABSPATH . GCINC . '/feed-rss2.php' );
	}
}
...
function do_robots() {
	...
	$output  .= "Disallow: $path/gc-admin/\n";
	$output  .= "Allow: $path/gc-admin/admin-ajax.php\n";
 ...
 * the cache values, then this will work. If you use the default GeChiUI
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether GeChiUI is installed.
 *
 * For more information on this and similar theme functions, check out
 * the {@link https://developer.gechiui.com/themes/basics/conditional-tags/
 * Conditional Tags} article in the Theme Developer Handbook.
...
function is_blog_installed() {
	...
	foreach ( $gc_tables as $table ) {
		...
		$gcdb->error = sprintf(
			...
			__( '一些数据表不可用。也许需要<a href="%s">修复数据库</a>。' ),
 ...
 */
function gc_nonce_url( $actionurl, $action = -1, $name = '_gcnonce' ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return esc_html( add_query_arg( $name, gc_create_nonce( $action ), $actionurl ) );
}
...
function gc_original_referer_field( $echo = true, $jump_back_to = 'current' ) {
	...
	if ( ! $ref ) {
		$ref = ( 'previous' === $jump_back_to ) ? gc_get_referer() : gc_unslash( $_SERVER['REQUEST_URI'] );
	}
 ...
 */
function gc_mkdir_p( $target ) {
	$wrapper = null;

	// Strip the protocol.
	if ( gc_is_stream( $target ) ) {
 ...
 */
function gc_normalize_path( $path ) {
	$wrapper = '';

	if ( gc_is_stream( $path ) ) {
 ...
 *
 * Same as gc_upload_dir() but "light weight" as it doesn't attempt to create the uploads directory.
 ...
 */
function gc_get_upload_dir() {
	return gc_upload_dir( null, false );
}
 ...
 * 'GC_CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'GC_CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'GC_CONTENT_URL' constant and appending '/uploads' to the path.
 *
 ...
 * }
 */
function gc_upload_dir( $time = null, $create_dir = true, $refresh_cache = false ) {
	static $cache = array(), $tested_paths = array();

	$key = sprintf( '%d-%s', get_current_blog_id(), (string) $time );

	if ( $refresh_cache || empty( $cache[ $key ] ) ) {
		$cache[ $key ] = _gc_upload_dir( $time );
	}
 ...
 *
 * @param string $time Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See gc_upload_dir()
 */
function _gc_upload_dir( $time = null ) {
	...
	if ( ! $url ) {
		if ( empty( $upload_path ) || ( 'gc-content/uploads' === $upload_path ) || ( $upload_path == $dir ) ) {
			$url = GC_CONTENT_URL . '/uploads';
		} else {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had gc-content/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as gc-content/uploads is used
			 *    there, and
			 ...
			 * rewriting in multisite, the resulting URL is /files. (#GC22702 for background.)
			 */
 ...
 */
function gc_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	// Sanitize the file name before we begin processing.
	$filename = sanitize_file_name( $filename );
	$ext2     = null;

	// Initialize vars used in the gc_unique_filename filter.
	$number        = '';
	$alt_filenames = array();
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		...
		/*
		 * Get the mime type. Uploaded files were already checked with gc_check_filetype_and_ext()
		 * in _gc_handle_upload(). Using gc_check_filetype() would be sufficient here.
		 */
		 ...
		 * for better compatibility with different filesystems. Fixes an inconsistency in GC < 2.9
		...
		if ( $name && $ext && @is_dir( $dir ) && false !== strpos( $dir, $upload_dir['basedir'] ) ) {
			 ...
			 */
			$files = apply_filters( 'pre_gc_unique_filename_file_list', null, $dir, $filename );
			...
			if ( ! empty( $files ) ) {
				...
				while ( $i <= $count && _gc_check_existing_file_names( $filename, $files ) ) {
					$new_number = (int) $number + 1;
		...
		if ( $is_image ) {
			...
			foreach ( $alt_types as $alt_type ) {
				$alt_ext = gc_get_default_extension_for_mime_type( $alt_type );
			...
			if ( ! empty( $alt_filenames ) ) {
				...
				while ( $i <= $count && _gc_check_alternate_file_names( $alt_filenames, $_dir, $files ) ) {
					$new_number = (int) $number + 1;
	 ...
	 */
	return apply_filters( 'gc_unique_filename', $filename, $ext, $dir, $unique_filename_callback, $alt_filenames, $number );
}
 ...
 */
function _gc_check_alternate_file_names( $filenames, $dir, $files ) {
	foreach ( $filenames as $filename ) {
		if ( file_exists( $dir . $filename ) ) {
			return true;
		}

		if ( ! empty( $files ) && _gc_check_existing_file_names( $filename, $files ) ) {
			return true;
		}
	}
 ...
 * @return bool True if the tested file name could match an existing file, false otherwise.
 */
function _gc_check_existing_file_names( $filename, $files ) {
	$fname = pathinfo( $filename, PATHINFO_FILENAME );
	$ext   = pathinfo( $filename, PATHINFO_EXTENSION );
 ...
 * }
 */
function gc_upload_bits( $name, $deprecated, $bits, $time = null ) {
	if ( ! empty( $deprecated ) ) {
		_deprecated_argument( __FUNCTION__, '2.0.0' );
	}

	if ( empty( $name ) ) {
		return array( 'error' => __( '文件名为空' ) );
	}

	$gc_filetype = gc_check_filetype( $name );
	if ( ! $gc_filetype['ext'] && ! current_user_can( 'unfiltered_upload' ) ) {
		return array( 'error' => __( '抱歉，您无权上传此文件类型。' ) );
	}

	$upload = gc_upload_dir( $time );
	...
	$filename = gc_unique_filename( $upload['path'], $name );
	...
	if ( ! $ifp ) {
		return array(
			...
			'error' => sprintf( __( '无法写文件%s' ), $new_file ),
		);
	}
	...
	/** This filter is documented in gc-admin/includes/file.php */
	return apply_filters(
		'gc_handle_upload',
		array(
			'file'  => $new_file,
			'url'   => $url,
			'type'  => $gc_filetype['type'],
			'error' => false,
		),
		'sideload'
	);
 ...
 */
function gc_ext2type( $ext ) {
	$ext = strtolower( $ext );

	$ext2type = gc_get_ext_types();
 ...
 * @return string|false
 */
function gc_get_default_extension_for_mime_type( $mime_type ) {
	$extensions = explode( '|', array_search( $mime_type, gc_get_mime_types(), true ) );
 ...
 * }
 */
function gc_check_filetype( $filename, $mimes = null ) {
	if ( empty( $mimes ) ) {
 ...
 * }
 */
function gc_check_filetype_and_ext( $file, $filename, $mimes = null ) {
	$proper_filename = false;

	// Do basic extension validation and MIME mapping.
	$gc_filetype = gc_check_filetype( $filename, $mimes );
	$ext         = $gc_filetype['ext'];
	$type        = $gc_filetype['type'];
	...
	if ( $type && 0 === strpos( $type, 'image/' ) ) {
		...
		if ( $real_mime && $real_mime != $type ) {
			...
			if ( ! empty( $mime_to_ext[ $real_mime ] ) ) {
				...
				$gc_filetype = gc_check_filetype( $new_filename, $mimes );
				$ext         = $gc_filetype['ext'];
				$type        = $gc_filetype['type'];
			} else {
	...
	try {
		if ( is_callable( 'exif_imagetype' ) ) {
			...
			if ( defined( 'GC_DEBUG' ) && GC_DEBUG
				...
				// Not using gc_getimagesize() here to avoid an infinite loop.
				$imagesize = getimagesize( $file );
			} else {
				// phpcs:ignore GeChiUI.PHP.NoSilencedErrors
				$imagesize = @getimagesize( $file );
			}
...
function gc_get_ext_types() {
	...
	return apply_filters(
		...
		array(
			...
			'document'    => array( 'doc', 'docx', 'docm', 'dotm', 'odt', 'pages', 'pdf', 'xps', 'oxps', 'rtf', 'gc', 'gcd', 'psd', 'xcf' ),
...
function gc_nonce_ays( $action ) {
	...
	gc_die( $html, $title, $response_code );
}

/**
 * Kills GeChiUI execution and displays HTML page with an error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when GeChiUI is still
 *                                  loading and the site's locale is not set up yet. Accepts 'rtl' and 'ltr'.
 *                                  Default is the value of is_rtl().
 *     @type string $charset        Character set of the HTML output. Default 'utf-8'.
 *     @type string $code           Error code to use. Default is 'gc_die', or the main error code if $message
 *                                  is a GC_Error.
 *     @type bool   $exit           Whether to exit the process after completion. Default true.
 * }
 */
function gc_die( $message = '', $title = '', $args = array() ) {
	global $gc_query;
...
function _default_gc_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _gc_die_process_input( $message, $title, $args );

	if ( is_string( $message ) ) {
		if ( ! empty( $parsed_args['additional_errors'] ) ) {
			$message = array_merge(
				array( $message ),
				gc_list_pluck( $parsed_args['additional_errors'], 'message' )
			);
			$message = "<ul>\n\t\t<li>" . implode( "</li>\n\t\t<li>", $message ) . "</li>\n\t</ul>";
		}

		$message = sprintf(
			'<div class="gc-die-message">%s</div>',
			$message
		);
	}
...
<head>
	...
	<meta name="viewport" content="width=device-width">
		<?php
		if ( function_exists( 'gc_robots' ) && function_exists( 'gc_robots_no_robots' ) && function_exists( 'add_filter' ) ) {
...
/**
 * Kills GeChiUI execution and displays Ajax response with an error message.
 *
...
/**
 * Kills GeChiUI execution and displays JSON response with an error message.
 *
...
/**
 * Kills GeChiUI execution and displays JSONP response with an error message.
 *
...
/**
 * Kills GeChiUI execution and displays XML response with an error message.
 *
...
function _xmlrpc_gc_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( $gc_xmlrpc_server ) {
		...
		$gc_xmlrpc_server->output( $error->getXml() );
	}
	if ( $parsed_args['exit'] ) {
		die();
	}
...
/**
 * Kills GeChiUI execution and displays an error message.
 *
 ...
 * }
 */
function _gc_die_process_input( $message, $title = '', $args = array() ) {
	$defaults = array(
		'response'          => 0,
		'code'              => '',
		'exit'              => true,
	...
	if ( function_exists( 'is_gc_error' ) && is_gc_error( $message ) ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'GeChiUI &rsaquo; 错误' ) : 'GeChiUI &rsaquo; 错误';
	}
 ...
 */
function gc_json_encode( $data, $options = 0, $depth = 512 ) {
	...
	try {
		$data = _gc_json_sanity_check( $data, $depth );
	} catch ( Exception $e ) {
		return false;
	}
 ...
 */
function _gc_json_sanity_check( $data, $depth ) {
	if ( $depth < 0 ) {
	...
	if ( is_array( $data ) ) {
		...
		foreach ( $data as $id => $el ) {
			...
			if ( is_array( $el ) || is_object( $el ) ) {
				$output[ $clean_id ] = _gc_json_sanity_check( $el, $depth - 1 );
			} elseif ( is_string( $el ) ) {
				$output[ $clean_id ] = _gc_json_convert_string( $el );
			} else {
				$output[ $clean_id ] = $el;
			}
		}
		...
		return _gc_json_convert_string( $data );
	} else {
		return $data;
	}
 ...
 */
function _gc_json_prepare_data( $data ) {
 ...
 * @param int   $options     Optional. Options to be passed to json_encode(). Default 0.
 */
function gc_send_json( $response, $status_code = null, $options = 0 ) {
	if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: GC_REST_Response, 2: GC_Error */
				__( '使用REST API时，从回调结果返回%1$s或%2$s对象。' ),
				'GC_REST_Response',
				'GC_Error'
			),
			'5.5.0'
	...
	echo gc_json_encode( $response, $options );
 ...
 * @param int   $options     Optional. Options to be passed to json_encode(). Default 0.
 */
function gc_send_json_success( $data = null, $status_code = null, $options = 0 ) {
	...
	gc_send_json( $response, $status_code, $options );
}
 ...
 * @param int   $options     Optional. Options to be passed to json_encode(). Default 0.
 */
function gc_send_json_error( $data = null, $status_code = null, $options = 0 ) {
 ...
 */
function gc_check_jsonp_callback( $callback ) {
...
function gc_json_file_decode( $filename, $options = array() ) {
	...
	$options      = gc_parse_args( $options, array( 'associative' => false ) );
 ...
 *
 * Plugins may override the default smiley list by setting the $gcsmiliestrans
 * to an array, with the key the code the blogger types in and the value the
 * image file.
 *
 * The $gc_smiliessearch global is for the regular expression and is set each
...
function smilies_init() {
	global $gcsmiliestrans, $gc_smiliessearch;
	 ...
	 */
	krsort( $gcsmiliestrans );

	$spaces = gc_spaces_regexp();

	// Begin first "subpattern".
	$gc_smiliessearch = '/(?<=' . $spaces . '|^)';

	$subchar = '';
	foreach ( (array) $gcsmiliestrans as $smiley => $img ) {
		...
		if ( $firstchar != $subchar ) {
			if ( '' !== $subchar ) {
				$gc_smiliessearch .= ')(?=' . $spaces . '|$)';  // End previous "subpattern".
				$gc_smiliessearch .= '|(?<=' . $spaces . '|^)'; // Begin another "subpattern".
			}
			$subchar           = $firstchar;
			$gc_smiliessearch .= preg_quote( $firstchar, '/' ) . '(?:';
		} else {
			$gc_smiliessearch .= '|';
		}
		$gc_smiliessearch .= preg_quote( $rest, '/' );
	}

	$gc_smiliessearch .= ')(?=' . $spaces . '|$)/m';
 ...
 */
function gc_parse_args( $args, $defaults = array() ) {
	if ( is_object( $args ) ) {
		$parsed_args = get_object_vars( $args );
	} elseif ( is_array( $args ) ) {
		$parsed_args =& $args;
	} else {
		gc_parse_str( $args, $parsed_args );
	}
 ...
 */
function gc_is_numeric_array( $data ) {
	if ( ! is_array( $data ) ) {
		return false;
	}
...
function gc_maybe_load_widgets() {
	 ...
	 *
	 *
	 * @param bool $gc_maybe_load_widgets Whether to load the Widgets library.
	 *                                    Default true.
	 */
	...
	require_once ABSPATH . GCINC . '/default-widgets.php';

	add_action( '_admin_menu', 'gc_widgets_add_menu' );
}
 ...
 * then the GeChiUI DB error will be displayed instead.
 *
 * The GeChiUI DB error sets the HTTP status header to 500 to try to prevent
...
function dead_db() {
	...
	// Otherwise, be terse.
	gc_die( '<h1>' . __( '建立数据库连接时出错' ) . '</h1>', __( '数据库错误' ) );
}
...
function _deprecated_function( $function, $version, $replacement = '' ) {
	 ...
	 */
	if ( GC_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
...
function _deprecated_constructor( $class, $version, $parent_class = '' ) {
	 ...
	 * @param bool $trigger Whether to trigger the error for deprecated functions. Default true.
	 */
	if ( GC_DEBUG && apply_filters( 'deprecated_constructor_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $parent_class ) {
				trigger_error(
					sprintf(
...
function _deprecated_file( $file, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( GC_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
...
function _deprecated_argument( $function, $version, $message = '' ) {
	 ...
	 */
	if ( GC_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $message ) {
				trigger_error(
					sprintf(
 ...
 *
 * Default behavior is to trigger a user error if `GC_DEBUG` is true.
 *
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( GC_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;
	...
	if ( GC_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
				...
				$version = sprintf( __( '（这个消息是在%s版本添加的。）' ), $version );
			}

			$message .= ' ' . sprintf(
				/* translators: %s: Documentation URL. */
				__( '请查阅<a href="%s">调试GeChiUI</a>来获取更多信息。' ),
				__( 'https://www.gechiui.com/support/debugging-in-gechiui/' )
			);
			...
			if ( $version ) {
				$version = sprintf( '（这个消息是在%s版本添加的。）', $version );
			}
...
function gc_guess_url() {
	if ( defined( 'GC_SITEURL' ) && '' !== GC_SITEURL ) {
		$url = GC_SITEURL;
	} else {
 ...
 * @return bool The current suspend setting.
 */
function gc_suspend_cache_invalidation( $suspend = true ) {
	global $_gc_suspend_cache_invalidation;

	$current_suspend                = $_gc_suspend_cache_invalidation;
	$_gc_suspend_cache_invalidation = $suspend;
...
function is_site_meta_supported() {
	...
	if ( false === $supported ) {
		$supported = $gcdb->get_var( "SHOW TABLES LIKE '{$gcdb->blogmeta}'" ) ? 1 : 0;
 ...
 */
function gc_timezone_override_offset() {
	$timezone_string = get_option( 'timezone_string' );
	if ( ! $timezone_string ) {
		return false;
	}
 ...
 * @return int
 */
function _gc_timezone_choice_usort_callback( $a, $b ) {
...
function gc_timezone_choice( $selected_zone, $locale = null ) {
	...
	foreach ( timezone_identifiers_list() as $zone ) {
		...
		// phpcs:disable GeChiUI.GC.I18n.LowLevelTranslationFunction,GeChiUI.GC.I18n.NonSingularStringLiteralText
		$zonen[] = array(
	...
	usort( $zonen, '_gc_timezone_choice_usort_callback' );
	...
	// Do UTC.
	$structure[] = '<optgroup label="' . esc_attr__( '协调世界时（UTC）' ) . '">';
	$selected    = '';
	if ( 'UTC' === $selected_zone ) {
		$selected = 'selected="selected" ';
	}
	$structure[] = '<option ' . $selected . 'value="' . esc_attr( 'UTC' ) . '">' . __( '协调世界时（UTC）' ) . '</option>';
	$structure[] = '</optgroup>';

	// Do manual UTC offsets.
	$structure[]  = '<optgroup label="' . esc_attr__( '手动指定偏差' ) . '">';
	$offset_range = array(
		-12,
		-11.5,
		-11,
 ...
 * @access private
 *
 * @see https://core.trac.gechiui.com/ticket/8497
 *
 ...
 * @global gcdb $gcdb GeChiUI database abstraction object.
 */
function gc_scheduled_delete() {
	global $gcdb;

	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	$posts_to_delete = $gcdb->get_results( $gcdb->prepare( "SELECT post_id FROM $gcdb->postmeta WHERE meta_key = '_gc_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	$comments_to_delete = $gcdb->get_results( $gcdb->prepare( "SELECT comment_id FROM $gcdb->commentmeta WHERE meta_key = '_gc_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	foreach ( (array) $comments_to_delete as $comment ) {
		...
		if ( ! $del_comment || 'trash' !== $del_comment->comment_approved ) {
			delete_comment_meta( $comment_id, '_gc_trash_meta_time' );
			delete_comment_meta( $comment_id, '_gc_trash_meta_status' );
		} else {
			gc_delete_comment( $del_comment );
		}
	}
}
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_empty_string() { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
 ...
 */
function _gc_mysql_week( $column ) {
...
function gc_find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	...
	$arbitrary_loop_member = gc_find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args );
	if ( ! $arbitrary_loop_member ) {
		return array();
	}

	return gc_find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 *               $_return_loop
 */
function gc_find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
	$tortoise        = $start;
	$hare            = $start;
 ...
 *                      of individual calls.
 */
function gc_debug_backtrace_summary( $ignore_class = null, $skip_frames = 0, $pretty = true ) {
	...
	if ( ! isset( $truncate_paths ) ) {
		$truncate_paths = array(
			gc_normalize_path( GC_CONTENT_DIR ),
			gc_normalize_path( ABSPATH ),
		);
	}
	...
	foreach ( $trace as $call ) {
		if ( $skip_frames > 0 ) {
			...
			if ( in_array( $call['function'], array( 'do_action', 'apply_filters', 'do_action_ref_array', 'apply_filters_ref_array' ), true ) ) {
				...
				$caller[] = $call['function'] . "('" . str_replace( $truncate_paths, '', gc_normalize_path( $filename ) ) . "')";
			} else {
 ...
 */
function gc_checkdate( $month, $day, $year, $source_date ) {
	/**
	 ...
	 */
	return apply_filters( 'gc_checkdate', checkdate( $month, $day, $year ), $source_date );
}
...
function gc_auth_check_load() {
	 ...
	 */
	if ( apply_filters( 'gc_auth_check_load', $show, $screen ) ) {
		gc_enqueue_style( 'gc-auth-check' );
		gc_enqueue_script( 'gc-auth-check' );

		add_action( 'admin_print_footer_scripts', 'gc_auth_check_html', 5 );
		add_action( 'gc_print_footer_scripts', 'gc_auth_check_html', 5 );
	}
}
...
function gc_auth_check_html() {
	 ...
	 */
	$same_domain = apply_filters( 'gc_auth_check_same_domain', $same_domain );
	...
	if ( $same_domain ) {
		$login_src = add_query_arg(
			array(
				'interim-login' => '1',
				'gc_lang'       => get_user_locale(),
			),
			$login_url
		);
		?>
		<div id="gc-auth-check-form" class="loading" data-src="<?php echo esc_url( $login_src ); ?>"></div>
		<?php
	}

	?>
	<div class="gc-auth-fallback">
		<p><b class="gc-auth-fallback-expired" tabindex="0"><?php _e( '会话已过期' ); ?></b></p>
		<p><a href="<?php echo esc_url( $login_url ); ?>" target="_blank"><?php _e( '请重新登录。' ); ?></a>
		<?php _e( '登录页将在新标签页中打开。登录后您可关闭该页并返回本页。' ); ?></p>
	</div>
	</div>
	</div>
	<?php
 ...
 * @return array The Heartbeat response with 'gc-auth-check' value set.
 */
function gc_auth_check( $response ) {
	$response['gc-auth-check'] = is_user_logged_in() && empty( $GLOBALS['login_grace_period'] );
	return $response;
}
 ...
 * @access private
 *
 * @see https://core.trac.gechiui.com/ticket/23688
 *
...
function gc_delete_file( $file ) {
	 ...
	 */
	$delete = apply_filters( 'gc_delete_file', $file );
	if ( ! empty( $delete ) ) {
		@unlink( $delete );
	}
}
 ...
 */
function gc_delete_file_from_directory( $file, $directory ) {
	if ( gc_is_stream( $file ) ) {
		$real_file      = $file;
		$real_directory = $directory;
	} else {
		$real_file      = realpath( gc_normalize_path( $file ) );
		$real_directory = realpath( gc_normalize_path( $directory ) );
	}

	if ( false !== $real_file ) {
		$real_file = gc_normalize_path( $real_file );
	}
 ...
 */
function gc_raise_memory_limit( $context = 'admin' ) {
	...
	$gc_max_limit     = GC_MAX_MEMORY_LIMIT;
	$gc_max_limit_int = gc_convert_hr_to_bytes( $gc_max_limit );
	$filtered_limit   = $gc_max_limit;
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * The `GC_MAX_MEMORY_LIMIT` constant specifically defines the maximum memory
	...
	$filtered_limit_int = gc_convert_hr_to_bytes( $filtered_limit );

	if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $gc_max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
		if ( false !== ini_set( 'memory_limit', $filtered_limit ) ) {
			return $filtered_limit;
		} else {
			return false;
		}
	} elseif ( -1 === $gc_max_limit_int || $gc_max_limit_int > $current_limit_int ) {
		if ( false !== ini_set( 'memory_limit', $gc_max_limit ) ) {
			return $gc_max_limit;
		} else {
			return false;
		}
 ...
 */
function gc_is_uuid( $uuid, $version = null ) {
	...
	if ( is_numeric( $version ) ) {
		if ( 4 !== (int) $version ) {
			_doing_it_wrong( __FUNCTION__, __( '现时仅支持UUID V4。' ), '4.9.0' );
			return false;
		}
 ...
 */
function gc_site_admin_email_change_notification( $old_email, $new_email, $option_name ) {
	$send = true;
 ...
 */
function gc_privacy_anonymize_ip( $ip_addr, $ipv6_fallback = false ) {
	if ( empty( $ip_addr ) ) {
		return '0.0.0.0';
	}
...
function gc_privacy_anonymize_data( $type, $data = '' ) {
	...
	switch ( $type ) {
		...
		case 'url':
			...
			break;
		case 'ip':
			$anonymous = gc_privacy_anonymize_ip( $data );
			break;
		case 'date':
	 ...
	 */
	return apply_filters( 'gc_privacy_anonymize_data', $anonymous, $type, $data );
}
 ...
 */
function gc_privacy_exports_dir() {
	$upload_dir  = gc_upload_dir();
	$exports_dir = trailingslashit( $upload_dir['basedir'] ) . 'gc-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'gc_privacy_exports_dir', $exports_dir );
}
 ...
 */
function gc_privacy_exports_url() {
	$upload_dir  = gc_upload_dir();
	$exports_url = trailingslashit( $upload_dir['baseurl'] ) . 'gc-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'gc_privacy_exports_url', $exports_url );
}
...
function gc_schedule_delete_old_privacy_export_files() {
	...
	if ( ! gc_next_scheduled( 'gc_privacy_delete_old_export_files' ) ) {
		gc_schedule_event( time(), 'hourly', 'gc_privacy_delete_old_export_files' );
	}
}
 ...
 *
 * The export files are stored in `gc-content/uploads`, and are therefore publicly
 ...
 *
 * This URL can be overridden by specifying an environment variable `GC_UPDATE_PHP_URL` or by using the
 * {@see 'gc_update_php_url'} filter. Providing an empty string is not allowed and will result in the
 ...
 */
function gc_get_update_php_url() {
	$default_url = gc_get_default_update_php_url();

	$update_url = $default_url;
	if ( false !== getenv( 'GC_UPDATE_PHP_URL' ) ) {
		$update_url = getenv( 'GC_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'gc_update_php_url', $update_url );
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see gc_get_update_php_url()} when relying on the URL.
 ...
 */
function gc_get_default_update_php_url() {
	return _x( 'https://www.gechiui.com/support/update-php/', 'localized PHP upgrade information page' );
}

/**
 * Prints the default annotation for the web host altering the "更新PHP" page URL.
 *
...
/**
 * Returns the default annotation for the web hosting altering the "更新PHP" page URL.
 *
 ...
 */
function gc_get_update_php_annotation() {
	$update_url  = gc_get_update_php_url();
	$default_url = gc_get_default_update_php_url();
 ...
 *
 * A URL will only be returned if the `GC_DIRECT_UPDATE_PHP_URL` environment variable is specified or
 ...
 */
function gc_get_direct_php_update_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'GC_DIRECT_UPDATE_PHP_URL' ) ) {
		$direct_update_url = getenv( 'GC_DIRECT_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$direct_update_url = apply_filters( 'gc_direct_php_update_url', $direct_update_url );
 ...
 *
 * This URL can be overridden by specifying an environment variable `GC_UPDATE_HTTPS_URL` or by using the
 * {@see 'gc_update_https_url'} filter. Providing an empty string is not allowed and will result in the
 * default URL being used. Furthermore the page the URL links to should preferably be localized in the
 * site language.
 *
 *
 ...
 */
function gc_get_update_https_url() {
	$default_url = gc_get_default_update_https_url();

	$update_url = $default_url;
	if ( false !== getenv( 'GC_UPDATE_HTTPS_URL' ) ) {
		$update_url = getenv( 'GC_UPDATE_HTTPS_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'gc_update_https_url', $update_url );
	if ( empty( $update_url ) ) {
		$update_url = $default_url;
	}
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see gc_get_update_https_url()} when relying on the URL.
 * This function does not allow modifying the returned URL, and is only used to compare the actually used URL with the
 * default one.
 *
 *
 ...
 *
 * A URL will only be returned if the `GC_DIRECT_UPDATE_HTTPS_URL` environment variable is specified or
 ...
 */
function gc_get_direct_update_https_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'GC_DIRECT_UPDATE_HTTPS_URL' ) ) {
		$direct_update_url = getenv( 'GC_DIRECT_UPDATE_HTTPS_URL' );
	}
	 ...
	 * @param string $direct_update_url URL for directly updating PHP.
	 */
	$direct_update_url = apply_filters( 'gc_direct_update_https_url', $direct_update_url );
 ...
 *                                   The timeout is global and is measured from the moment GeChiUI started to load.
			...
			while ( ( $file = readdir( $handle ) ) !== false ) {
				...
				if ( '.' !== $file && '..' !== $file ) {
					...
					if ( $max_execution_time > 0 &&
						( microtime( true ) - GC_START_TIMESTAMP ) > $max_execution_time
					) {
 ...
 */
function is_gc_version_compatible( $required ) {
	global $gc_version;

	// Strip off any -alpha, -RC, -beta, -src suffixes.
	list( $version ) = explode( '-', $gc_version );
