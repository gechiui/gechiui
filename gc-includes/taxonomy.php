 ...
 *
 * This function fires twice: in gc-settings.php before plugins are loaded (for
...
function create_initial_taxonomies() {
	...
	if ( ! did_action( 'init' ) ) {
		...
		$rewrite          = array(
			'category'    => array(
				...
				'with_front'   => ! get_option( 'category_base' ) || $gc_rewrite->using_index_permalinks(),
				'ep_mask'      => EP_CATEGORIES,
			),
			'post_tag'    => array(
				'hierarchical' => false,
				'slug'         => get_option( 'tag_base' ) ? get_option( 'tag_base' ) : 'tag',
				'with_front'   => ! get_option( 'tag_base' ) || $gc_rewrite->using_index_permalinks(),
				'ep_mask'      => EP_TAGS,
			),
	...
	register_taxonomy(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'menus',
			'rest_controller_class' => 'GC_REST_Menus_Controller',
		)
	);
	...
	register_taxonomy(
		...
		array(
			...
			'labels'       => array(
				'name'                       => __( '链接分类' ),
				'singular_name'              => __( '链接分类' ),
				'search_items'               => __( '搜索链接分类' ),
				'popular_items'              => null,
				'all_items'                  => __( '所有链接分类' ),
				'edit_item'                  => __( '编辑链接分类' ),
				'update_item'                => __( '更新链接分类' ),
				'add_new_item'               => __( '添加链接分类' ),
				'new_item_name'              => __( '新的链接分类名' ),
				'separate_items_with_commas' => null,
				'add_or_remove_items'        => null,
				'choose_from_most_used'      => null,
				'back_to_items'              => __( '&larr; 转到“链接分类”页面' ),
			),
			'capabilities' => array(
	...
	register_taxonomy(
		'gc_theme',
		array( 'gc_template', 'gc_template_part', 'gc_global_styles' ),
		array(
			'public'            => false,
			'hierarchical'      => false,
			'labels'            => array(
	...
	register_taxonomy(
		'gc_template_part_area',
		array( 'gc_template_part' ),
		array(
			'public'            => false,
			'hierarchical'      => false,
			'labels'            => array(
				'name'          => __( '模板组件区' ),
				'singular_name' => __( '模板组件区' ),
			),
			'query_var'         => false,
			'rewrite'           => false,
			'show_ui'           => false,
 ...
 *                         Default 'and'.
 * @return string[]|GC_Taxonomy[] An array of taxonomy names or objects.
 */
 ...
 *         @type bool   $with_front   Should the permastruct be prepended with GC_Rewrite::$front. Default true.
 ...
 *                                                provided to `gc_set_object_terms()`. Default null which equates to false.
 *     @type array         $args                  Array of arguments to automatically use inside `gc_get_object_terms()`
 *                                                for this taxonomy.
 *     @type bool          $_builtin              This taxonomy is a "built-in" taxonomy. INTERNAL USE ONLY!
 *                                                Default false.
 * }
 * @return GC_Taxonomy|GC_Error The registered taxonomy object on success, GC_Error object on failure.
 */
function register_taxonomy( $taxonomy, $object_type, $args = array() ) {
	...
	if ( empty( $taxonomy ) || strlen( $taxonomy ) > 32 ) {
		_doing_it_wrong( __FUNCTION__, __( '分类法名的长度必须在1至32个字符间。' ), '4.2.0' );
		return new GC_Error( 'taxonomy_length_invalid', __( '分类法名的长度必须在1至32个字符间。' ) );
	}
 ...
 *                                              '多个标签请用英文逗号（,）分开', used in the meta box.
 *     @type string $add_or_remove_items        This label is only used for non-hierarchical taxonomies. Default
 *                                              '添加或移除标签', used in the meta box when JavaScript
 *                                              is disabled.
...
function unregister_taxonomy_for_object_type( $taxonomy, $object_type ) {
	...
	$key = array_search( $object_type, $gc_taxonomies[ $taxonomy ]->object_type, true );
	if ( false === $key ) {
		return false;
	}

	unset( $gc_taxonomies[ $taxonomy ]->object_type[ $key ] );
...
function get_tax_sql( $tax_query, $primary_table, $primary_id_column ) {
	$tax_query_obj = new GC_Tax_Query( $tax_query );
 ...
 * be able to be captured by the hooks. Failure would be the same value as $gcdb
 ...
 *
 * @param int|GC_Term|object $term     If integer, term data will be fetched from the database,
 *                                     or from the cache if available.
 *                                     If stdClass object (as in the results of a database query),
 *                                     will apply filters and return a `GC_Term` object with the `$term` data.
 *                                     If `GC_Term`, will return `$term`.
 * @param string             $taxonomy Optional. Taxonomy name that `$term` is part of.
 * @param string             $output   Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                     correspond to a GC_Term object, an associative array, or a numeric array,
 *                                     respectively. Default OBJECT.
 * @param string             $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return GC_Term|array|GC_Error|null GC_Term instance (or array) on success, depending on the `$output` value.
 *                                     GC_Error if `$taxonomy` does not exist. Null for miscellaneous failure.
 */
function get_term( $term, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	if ( empty( $term ) ) {
		return new GC_Error( 'invalid_term', __( '空项目。' ) );
	}
 ...
 *                             respectively. Default OBJECT.
 * @param string     $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return GC_Term|array|false GC_Term instance (or array) on success, depending on the `$output` value.
...
function get_term_by( $field, $value, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	...
	if ( is_gc_error( $terms ) || empty( $terms ) ) {
		return false;
	}
 ...
 * @param int|GC_Term $term     Term ID or object.
 * @param string      $taxonomy Optional. Taxonomy name. Default empty.
 * @param string      $context  Optional. How to sanitize term fields. Look at sanitize_term_field() for available options.
 *                              Default 'display'.
 * @return string|int|null|GC_Error Will return an empty string if $term is not an object or if $field is not set in $term.
 */
 ...
 * @param string     $taxonomy Taxonomy name.
 * @return string|int|null|GC_Error Will return empty string if $term is not an object.
 */
 ...
 * GC_Term_Query::get_terms() for details. In all cases, a `GC_Error` object will
 ...
 *                           Default false.
 * @return int|false|GC_Error Meta ID on success, false on failure.
 *                            GC_Error when term_id is ambiguous between taxonomies.
 */
function add_term_meta( $term_id, $meta_key, $meta_value, $unique = false ) {
	if ( gc_term_is_shared( $term_id ) ) {
		return new GC_Error( 'ambiguous_term_id', __( '无法加入项目元数据，因其在多个分类法间共享。' ), $term_id );
	}
 ...
 * @return int|bool|GC_Error Meta ID if the key didn't exist. true on successful update,
 ...
 *                                 function parameter will be parsed as a taxonomy or array of taxonomies.
 *                                 Default empty.
 * @return string|GC_Error Numeric string containing the number of terms in that
 *                         taxonomy or GC_Error if the taxonomy does not exist.
 */
function gc_count_terms( $args = array(), $deprecated = '' ) {
	...
	if ( $args
		&& ( is_string( $args ) && taxonomy_exists( $args )
			|| is_array( $args ) && gc_is_numeric_array( $args ) )
	) {
 ...
 */
function gc_delete_object_term_relationships( $object_id, $taxonomies ) {
	...
	foreach ( (array) $taxonomies as $taxonomy ) {
		$term_ids = gc_get_object_terms( $object_id, $taxonomy, array( 'fields' => 'ids' ) );
		$term_ids = array_map( 'intval', $term_ids );
		gc_remove_object_terms( $object_id, $term_ids, $taxonomy );
	}
}
 ...
 *                               Default false.
 * }
 * @return bool|int|GC_Error True on success, false if term does not exist. Zero on attempted
 *                           deletion of default Category. GC_Error if the taxonomy does not exist.
 */
function gc_delete_term( $term, $taxonomy, $args = array() ) {
	global $gcdb;
	...
	if ( is_taxonomy_hierarchical( $taxonomy ) ) {
		...
		if ( is_gc_error( $term_obj ) ) {
			return $term_obj;
		}
		$parent = $term_obj->parent;

		$edit_ids    = $gcdb->get_results( "SELECT term_id, term_taxonomy_id FROM $gcdb->term_taxonomy WHERE `parent` = " . (int) $term_obj->term_id );
		$edit_tt_ids = gc_list_pluck( $edit_ids, 'term_taxonomy_id' );
		...
		$gcdb->update( $gcdb->term_taxonomy, compact( 'parent' ), array( 'parent' => $term_obj->term_id ) + compact( 'taxonomy' ) );

		// Clean the cache for all child terms.
		$edit_term_ids = gc_list_pluck( $edit_ids, 'term_id' );
	...
	$object_ids = (array) $gcdb->get_col( $gcdb->prepare( "SELECT object_id FROM $gcdb->term_relationships WHERE term_taxonomy_id = %d", $tt_id ) );

	foreach ( $object_ids as $object_id ) {
		if ( ! isset( $default ) ) {
			gc_remove_object_terms( $object_id, $term, $taxonomy );
			continue;
		}
		...
		gc_set_object_terms( $object_id, $terms, $taxonomy );
	}
	...
	$term_meta_ids = $gcdb->get_col( $gcdb->prepare( "SELECT meta_id FROM $gcdb->termmeta WHERE term_id = %d ", $term ) );
	...
	$gcdb->delete( $gcdb->term_taxonomy, array( 'term_taxonomy_id' => $tt_id ) );
	...
	if ( ! $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_taxonomy WHERE term_id = %d", $term ) ) ) {
		$gcdb->delete( $gcdb->terms, array( 'term_id' => $term ) );
	}
	 ...
	 * @param int     $tt_id        Term taxonomy ID.
	 * @param string  $taxonomy     Taxonomy slug.
	 * @param GC_Term $deleted_term Copy of the already-deleted term.
...
function gc_get_object_terms( $object_ids, $taxonomies, $args = array() ) {
	 ...
	 */
	$args = apply_filters( 'gc_get_object_terms_args', $args, $object_ids, $taxonomies );
	...
	if ( count( $taxonomies ) > 1 ) {
		foreach ( $taxonomies as $index => $taxonomy ) {
			...
			if ( isset( $t->args ) && is_array( $t->args ) && array_merge( $args, $t->args ) != $args ) {
				...
				$terms = array_merge( $terms, gc_get_object_terms( $object_ids, $taxonomy, array_merge( $args, $t->args ) ) );
			}
		}
	} else {
 ...
 * a GC_Error object will be returned.
 *
 ...
 *     An array of the new term data, GC_Error otherwise.
 *
 *     @type int        $term_id          The new term ID.
 *     @type int|string $term_taxonomy_id The new term taxonomy ID. Can be a numeric string.
 * }
 */
function gc_insert_term( $term, $taxonomy, $args = array() ) {
	global $gcdb;
	...
	if ( is_int( $term ) && 0 === $term ) {
		return new GC_Error( 'invalid_term_id', __( '无效的项目ID。' ) );
	}

	if ( '' === trim( $term ) ) {
		return new GC_Error( 'empty_term_name', __( '项目需要一个名字。' ) );
	}
	...
	if ( (int) $args['parent'] > 0 && ! term_exists( (int) $args['parent'] ) ) {
		return new GC_Error( 'missing_parent', __( '父项目不存在。' ) );
	}
	...
	$name        = gc_unslash( $args['name'] );
	$description = gc_unslash( $args['description'] );
	...
	if ( $args['alias_of'] ) {
		...
		if ( ! empty( $alias->term_group ) ) {
			 ...
			 */
			$term_group = $gcdb->get_var( "SELECT MAX(term_group) FROM $gcdb->terms" ) + 1;
	...
	if ( $name_match ) {
		...
		if ( ! $slug_provided || $name_match->slug === $slug || $slug_match ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				...
				$existing_term = null;
				$sibling_names = gc_list_pluck( $siblings, 'name' );
				$sibling_slugs = gc_list_pluck( $siblings, 'slug' );
				...
				if ( $existing_term ) {
					return new GC_Error( 'term_exists', __( '父项目中已存在同名项目。' ), $existing_term->term_id );
				}
			} else {
				return new GC_Error( 'term_exists', __( '分类法中已存在同名项目。' ), $name_match->term_id );
			}
		}
	}
	 ...
	 */
	$data = apply_filters( 'gc_insert_term_data', $data, $taxonomy, $args );

	if ( false === $gcdb->insert( $gcdb->terms, $data ) ) {
		return new GC_Error( 'db_insert_error', __( '未能将项目加入数据库。' ), $gcdb->last_error );
	}
	...
	if ( empty( $slug ) ) {
		...
		$gcdb->update( $gcdb->terms, compact( 'slug' ), compact( 'term_id' ) );

		/** This action is documented in gc-includes/taxonomy.php */
		do_action( 'edited_terms', $term_id, $taxonomy );
	}

	$tt_id = $gcdb->get_var( $gcdb->prepare( "SELECT tt.term_taxonomy_id FROM $gcdb->term_taxonomy AS tt INNER JOIN $gcdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = %s AND t.term_id = %d", $taxonomy, $term_id ) );
	...
	if ( false === $gcdb->insert( $gcdb->term_taxonomy, compact( 'term_id', 'taxonomy', 'description', 'parent' ) + array( 'count' => 0 ) ) ) {
		return new GC_Error( 'db_insert_error', __( '未能将项目分类法加入数据库。' ), $gcdb->last_error );
	}

	$tt_id = (int) $gcdb->insert_id;
	 ...
	 * are not fired.
	 */
	$duplicate_term = $gcdb->get_row( $gcdb->prepare( "SELECT t.term_id, t.slug, tt.term_taxonomy_id, tt.taxonomy FROM $gcdb->terms AS t INNER JOIN $gcdb->term_taxonomy AS tt ON ( tt.term_id = t.term_id ) WHERE t.slug = %s AND tt.parent = %d AND tt.taxonomy = %s AND t.term_id < %d AND tt.term_taxonomy_id != %d", $slug, $parent, $taxonomy, $term_id, $tt_id ) );
	...
	if ( $duplicate_term ) {
		$gcdb->delete( $gcdb->terms, array( 'term_id' => $term_id ) );
		$gcdb->delete( $gcdb->term_taxonomy, array( 'term_taxonomy_id' => $tt_id ) );
 ...
 */
function gc_set_object_terms( $object_id, $terms, $taxonomy, $append = false ) {
	global $gcdb;
	...
	if ( ! $append ) {
		$old_tt_ids = gc_get_object_terms(
			$object_id,
			$taxonomy,
			array(
				'fields'                 => 'tt_ids',
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( ! $term_info ) {
			...
			$term_info = gc_insert_term( $term, $taxonomy );
		}

		if ( is_gc_error( $term_info ) ) {
			return $term_info;
		}
		...
		if ( $gcdb->get_var( $gcdb->prepare( "SELECT term_taxonomy_id FROM $gcdb->term_relationships WHERE object_id = %d AND term_taxonomy_id = %d", $object_id, $tt_id ) ) ) {
			continue;
		}
	...
	if ( $new_tt_ids ) {
		gc_update_term_count( $new_tt_ids, $taxonomy );
	}
	...
	if ( ! $append ) {
		...
		if ( $delete_tt_ids ) {
			...
			$delete_term_ids  = $gcdb->get_col( $gcdb->prepare( "SELECT tt.term_id FROM $gcdb->term_taxonomy AS tt WHERE tt.taxonomy = %s AND tt.term_taxonomy_id IN ($in_delete_tt_ids)", $taxonomy ) );
			$delete_term_ids  = array_map( 'intval', $delete_term_ids );

			$remove = gc_remove_object_terms( $object_id, $delete_term_ids, $taxonomy );
			if ( is_gc_error( $remove ) ) {
				return $remove;
			}
		}
	}
	...
	if ( ! $append && isset( $t->sort ) && $t->sort ) {
		...
		$final_tt_ids = gc_get_object_terms(
			$object_id,
			$taxonomy,
			array(
				'fields'                 => 'tt_ids',
		...
		foreach ( $tt_ids as $tt_id ) {
			if ( in_array( (int) $tt_id, $final_tt_ids, true ) ) {
				$values[] = $gcdb->prepare( '(%d, %d, %d)', $object_id, $tt_id, ++$term_order );
			}
		}

		if ( $values ) {
			if ( false === $gcdb->query( "INSERT INTO $gcdb->term_relationships (object_id, term_taxonomy_id, term_order) VALUES " . implode( ',', $values ) . ' ON DUPLICATE KEY UPDATE term_order = VALUES(term_order)' ) ) {
				return new GC_Error( 'db_insert_error', __( '未能将项目关系加入数据库。' ), $gcdb->last_error );
			}
		}
	}
 ...
 * @return array|GC_Error Term taxonomy IDs of the affected terms.
 */
function gc_add_object_terms( $object_id, $terms, $taxonomy ) {
	return gc_set_object_terms( $object_id, $terms, $taxonomy, true );
}
 ...
 * @param string           $taxonomy  Taxonomy name.
 * @return bool|GC_Error True on success, false or GC_Error on failure.
 */
function gc_remove_object_terms( $object_id, $terms, $taxonomy ) {
	global $gcdb;
 ...
 */
function gc_unique_term_slug( $slug, $term ) {
	global $gcdb;
	...
	if ( $needs_suffix && is_taxonomy_hierarchical( $term->taxonomy ) && ! empty( $term->parent ) ) {
		...
		while ( ! empty( $the_parent ) ) {
			...
			if ( is_gc_error( $parent_term ) || empty( $parent_term ) ) {
				break;
			}
	 ...
	 */
	if ( apply_filters( 'gc_unique_term_slug_is_bad_slug', $needs_suffix, $slug, $term ) ) {
		if ( $parent_suffix ) {
			$slug .= $parent_suffix;
		}

		if ( ! empty( $term->term_id ) ) {
			$query = $gcdb->prepare( "SELECT slug FROM $gcdb->terms WHERE slug = %s AND term_id != %d", $slug, $term->term_id );
		} else {
			$query = $gcdb->prepare( "SELECT slug FROM $gcdb->terms WHERE slug = %s", $slug );
		}

		if ( $gcdb->get_var( $query ) ) { // phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
			$num = 2;
			do {
				$alt_slug = $slug . "-$num";
				$num++;
	 ...
	 */
	return apply_filters( 'gc_unique_term_slug', $slug, $term, $original_slug );
}
 ...
 *     @type string $slug        The term slug to use. Default empty string.
 * }
 * @return array|GC_Error An array containing the `term_id` and `term_taxonomy_id`,
 *                        GC_Error otherwise.
 */
function gc_update_term( $term_id, $taxonomy, $args = array() ) {
	global $gcdb;
	...
	if ( $duplicate && $duplicate->term_id !== $term_id ) {
		...
		if ( $empty_slug || ( $parent !== (int) $term['parent'] ) ) {
			...
			return new GC_Error( 'duplicate_term_slug', sprintf( __( '此别名“%s”已被其他项目使用。' ), $slug ) );
		}
	}

	$tt_id = (int) $gcdb->get_var( $gcdb->prepare( "SELECT tt.term_taxonomy_id FROM $gcdb->term_taxonomy AS tt INNER JOIN $gcdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = %s AND t.term_id = %d", $taxonomy, $term_id ) );

	// Check whether this is a shared term that needs splitting.
	$_term_id = _split_shared_term( $term_id, $tt_id );
	if ( ! is_gc_error( $_term_id ) ) {
		$term_id = $_term_id;
	}
	 ...
	 */
	$data = apply_filters( 'gc_update_term_data', $data, $term_id, $taxonomy, $args );

	$gcdb->update( $gcdb->terms, $data, compact( 'term_id' ) );
 ...
 */
function gc_defer_term_counting( $defer = null ) {
	static $_defer = false;
	...
	if ( is_bool( $defer ) ) {
		...
		if ( ! $defer ) {
			gc_update_term_count( null, null, true );
		}
	}
 ...
 */
function gc_update_term_count( $terms, $taxonomy, $do_deferred = false ) {
	static $_deferred = array();

	if ( $do_deferred ) {
		foreach ( (array) array_keys( $_deferred ) as $tax ) {
			gc_update_term_count_now( $_deferred[ $tax ], $tax );
			unset( $_deferred[ $tax ] );
		}
	}
	...
	return gc_update_term_count_now( $terms, $taxonomy );
}
 ...
 */
function gc_update_term_count_now( $terms, $taxonomy ) {
	$terms = array_map( 'intval', $terms );
 ...
 * @param string $taxonomy Taxonomy name.
 * @return bool|GC_Term[]|GC_Error Array of `GC_Term` objects, if cached.
 *                                 False if cache is empty for `$taxonomy` and `$id`.
 *                                 GC_Error if get_term() returns an error object for any term.
 */
...
function update_object_term_cache( $object_ids, $object_type ) {
	...
	foreach ( $taxonomies as $taxonomy ) {
		$cache_values = gc_cache_get_multiple( (array) $object_ids, "{$taxonomy}_relationships" );
 ...
 * @return array|GC_Error The subset of $terms that are descendants of $term_id.
 */
 ...
 * @global gcdb $gcdb GeChiUI database abstraction object.
 *
 * @param object[]|GC_Term[] $terms    List of term objects (passed by reference).
...
function _pad_term_counts( &$terms, $taxonomy ) {
	...
	$results      = $gcdb->get_results( "SELECT object_id, term_taxonomy_id FROM $gcdb->term_relationships INNER JOIN $gcdb->posts ON object_id = ID WHERE term_taxonomy_id IN (" . implode( ',', array_keys( $term_ids ) ) . ") AND post_type IN ('" . implode( "', '", $object_types ) . "') AND post_status = 'publish'" );
...
function _update_post_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( $check_attachments ) {
			// phpcs:ignore GeChiUI.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholderGeneration
			$count += (int) $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_relationships, $gcdb->posts p1 WHERE p1.ID = $gcdb->term_relationships.object_id AND ( post_status IN ('" . implode( "', '", $post_statuses ) . "') OR ( post_status = 'inherit' AND post_parent > 0 AND ( SELECT post_status FROM $gcdb->posts WHERE ID = p1.post_parent ) IN ('" . implode( "', '", $post_statuses ) . "') ) ) AND post_type = 'attachment' AND term_taxonomy_id = %d", $term ) );
		}

		if ( $object_types ) {
			// phpcs:ignore GeChiUI.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholderGeneration
			$count += (int) $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_relationships, $gcdb->posts WHERE $gcdb->posts.ID = $gcdb->term_relationships.object_id AND post_status IN ('" . implode( "', '", $post_statuses ) . "') AND post_type IN ('" . implode( "', '", $object_types ) . "') AND term_taxonomy_id = %d", $term ) );
		}

		/** This action is documented in gc-includes/taxonomy.php */
		do_action( 'edit_term_taxonomy', $term, $taxonomy->name );
		$gcdb->update( $gcdb->term_taxonomy, compact( 'count' ), array( 'term_taxonomy_id' => $term ) );
...
function _update_generic_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		$count = $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_relationships WHERE term_taxonomy_id = %d", $term ) );
 ...
 *                                     Default: true.
 * @return int|GC_Error When the current term does not need to be split (or cannot be split on the current
 *                      database schema), `$term_id` is returned. When the term is successfully split, the
 *                      new term_id is returned. A GC_Error is returned for miscellaneous errors.
 */
function _split_shared_term( $term_id, $term_taxonomy_id, $record = true ) {
	...
	$shared_tt_count = (int) $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_taxonomy tt WHERE tt.term_id = %d AND tt.term_taxonomy_id != %d", $term_id, $term_taxonomy_id ) );
	 ...
	 */
	$check_term_id = (int) $gcdb->get_var( $gcdb->prepare( "SELECT term_id FROM $gcdb->term_taxonomy WHERE term_taxonomy_id = %d", $term_taxonomy_id ) );
	...
	if ( empty( $shared_term ) ) {
		$shared_term = $gcdb->get_row( $gcdb->prepare( "SELECT t.* FROM $gcdb->terms t WHERE t.term_id = %d", $term_id ) );
	}
	...
	if ( false === $gcdb->insert( $gcdb->terms, $new_term_data ) ) {
		return new GC_Error( 'db_insert_error', __( '未能分离共享项目。' ), $gcdb->last_error );
	}

	$new_term_id = (int) $gcdb->insert_id;
	...
	if ( empty( $term_taxonomy ) ) {
		$term_taxonomy = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->term_taxonomy WHERE term_taxonomy_id = %d", $term_taxonomy_id ) );
	}

	$children_tt_ids = $gcdb->get_col( $gcdb->prepare( "SELECT term_taxonomy_id FROM $gcdb->term_taxonomy WHERE parent = %d AND taxonomy = %s", $term_id, $term_taxonomy->taxonomy ) );
	...
	$shared_term_taxonomies = $gcdb->get_col( $gcdb->prepare( "SELECT taxonomy FROM $gcdb->term_taxonomy WHERE term_id = %d", $term_id ) );
	...
	// Keep a record of term_ids that have been split, keyed by old term_id. See gc_get_split_term().
	if ( $record ) {
	...
	$shared_terms_exist = $gcdb->get_results(
		"SELECT tt.term_id, t.*, count(*) as term_tt_count FROM {$gcdb->term_taxonomy} tt
		 LEFT JOIN {$gcdb->terms} t ON t.term_id = tt.term_id
		 GROUP BY t.term_id
 ...
 * @global gcdb $gcdb GeChiUI database abstraction object.
 */
function _gc_batch_split_terms() {
	global $gcdb;

	$lock_name = 'term_split.lock';

	// Try to lock.
	$lock_result = $gcdb->query( $gcdb->prepare( "INSERT IGNORE INTO `$gcdb->options` ( `option_name`, `option_value`, `autoload` ) VALUES (%s, %s, 'no') /* LOCK */", $lock_name, time() ) );
	...
	if ( ! $lock_result ) {
		...
		if ( ! $lock_result || ( $lock_result > ( time() - HOUR_IN_SECONDS ) ) ) {
			gc_schedule_single_event( time() + ( 5 * MINUTE_IN_SECONDS ), 'gc_split_shared_term_batch' );
			return;
		}
	}
	...
	gc_schedule_single_event( time() + ( 2 * MINUTE_IN_SECONDS ), 'gc_split_shared_term_batch' );
	...
	$shared_tts      = $gcdb->get_results( "SELECT * FROM {$gcdb->term_taxonomy} WHERE `term_id` IN ({$shared_term_ids})" );
...
function _gc_check_for_scheduled_split_terms() {
	if ( ! get_option( 'finished_splitting_shared_terms' ) && ! gc_next_scheduled( 'gc_split_shared_term_batch' ) ) {
		gc_schedule_single_event( time() + MINUTE_IN_SECONDS, 'gc_split_shared_term_batch' );
	}
}
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _gc_check_split_default_terms( $term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
	if ( 'category' !== $taxonomy ) {
		return;
	}
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _gc_check_split_terms_in_menus( $term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
	global $gcdb;
	$post_ids = $gcdb->get_col(
		$gcdb->prepare(
			"SELECT m1.post_id
		FROM {$gcdb->postmeta} AS m1
			INNER JOIN {$gcdb->postmeta} AS m2 ON ( m2.post_id = m1.post_id )
			INNER JOIN {$gcdb->postmeta} AS m3 ON ( m3.post_id = m1.post_id )
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _gc_check_split_nav_menu_terms( $term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
 ...
 */
function gc_get_split_terms( $old_term_id ) {
 ...
 */
function gc_get_split_term( $old_term_id, $taxonomy ) {
	$split_terms = gc_get_split_terms( $old_term_id );
 ...
 */
function gc_term_is_shared( $term_id ) {
	global $gcdb;
	...
	$tt_count = $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->term_taxonomy WHERE term_id = %d", $term_id ) );
 ...
 * @global GC_Rewrite $gc_rewrite GeChiUI rewrite component.
 *
 * @param GC_Term|int|string $term     The term object, ID, or slug whose link will be retrieved.
 * @param string             $taxonomy Optional. Taxonomy. Default empty.
 * @return string|GC_Error URL of the taxonomy term archive on success, GC_Error if term does not exist.
 */
function get_term_link( $term, $taxonomy = '' ) {
	...
	if ( ! is_object( $term ) ) {
		$term = new GC_Error( 'invalid_term', __( '空项目。' ) );
	}
	...
	$termlink = $gc_rewrite->get_extra_permastruct( $taxonomy );
 ...
 *
 *     @type int|GC_Post $post   Post ID or object to get taxonomies of. Default current post.
 ...
 * @return bool|GC_Error GC_Error on input error.
 */
function is_object_in_term( $object_id, $taxonomy, $terms = null ) {
	$object_id = (int) $object_id;
	if ( ! $object_id ) {
		return new GC_Error( 'invalid_object', __( '无效的对象ID。' ) );
	}

	$object_terms = get_object_term_cache( $object_id, $taxonomy );
	if ( false === $object_terms ) {
		$object_terms = gc_get_object_terms( $object_id, $taxonomy, array( 'update_term_meta_cache' => false ) );
		if ( is_gc_error( $object_terms ) ) {
			return $object_terms;
		}

		gc_cache_set( $object_id, gc_list_pluck( $object_terms, 'term_id' ), "{$taxonomy}_relationships" );
	}
...
function get_ancestors( $object_id = 0, $object_type = '', $resource_type = '' ) {
	...
	if ( 'taxonomy' === $resource_type ) {
		...
		while ( ! is_gc_error( $term ) && ! empty( $term->parent ) && ! in_array( $term->parent, $ancestors, true ) ) {
 ...
 */
function gc_get_term_taxonomy_parent_id( $term_id, $taxonomy ) {
	$term = get_term( $term_id, $taxonomy );
 ...
 */
function gc_check_term_meta_support_prefilter( $check ) {
