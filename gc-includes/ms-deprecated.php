<?php
/**
 * Deprecated functions from GeChiUI MU and the multisite feature. You shouldn't
 ...
 * @see get_site()
 *
 * @return GC_Site Current site object.
 */
...
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'gc_generate_password()' );
	return gc_generate_password( $len );
}
...
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'gc_die()' );
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'gc_get_sites()' );

	global $gcdb;
	$blogs = $gcdb->get_results( $gcdb->prepare( "SELECT blog_id, domain, path FROM $gcdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", get_current_network_id() ), ARRAY_A );

	$blog_list = array();
	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $gcdb->get_var( "SELECT COUNT(ID) FROM " . $gcdb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
 ...
 */
function gcmu_admin_do_redirect( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0', 'gc_redirect()' );
	...
	if ( $ref ) {
		$ref = gcmu_admin_redirect_add_updated_param( $ref );
		gc_redirect( $ref );
		exit;
	}
	if ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		gc_redirect( $_SERVER['HTTP_REFERER'] );
		exit;
	}

	$url = gcmu_admin_redirect_add_updated_param( $url );
...
function get_admin_users_for_domain( $domain = '', $path = '' ) {
	...
	if ( $network_id )
		return $gcdb->get_results( $gcdb->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $gcdb->users AS u, $gcdb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $network_id ), ARRAY_A );
 ...
 * }
 * @return array[] An empty array if the installation is considered "large" via gc_is_large_network(). Otherwise,
 *                 an associative array of GC_Site data as arrays.
 */
function gc_get_sites( $args = array() ) {
 ...
 *
 * @param int    $blog_id    The value returned by gc_insert_site().
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $gcdb->get_results( "DESCRIBE {$gcdb->posts}" ) ) {
		die( '<h1>' . __( '已安装过' ) . '</h1><p>' . __( '您的GeChiUI看起来已经安装妥当。如果想重新安装，请删除数据库中的旧数据表。' ) . '</p></body></html>' );
	}
	...
	update_option( 'blogname', gc_unslash( $blog_title ) );
 ...
 * @param string $pref       The column in the gc_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	_deprecated_function( __FUNCTION__, '5.3.0', 'gc_update_user()' );

	if ( null !== $deprecated ) {
		_deprecated_argument( __FUNCTION__, '3.0.2' );
	}

	$gcdb->update( $gcdb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
