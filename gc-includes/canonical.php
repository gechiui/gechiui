<?php
/**
 * Canonical API to handle GeChiUI Redirecting
 *
 ...
 * page/post previews, GC admin, Trackbacks, robots.txt, favicon.ico, searches,
 * or on POST requests.
 *
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $gc_rewrite, $is_IIS, $gc_query, $gcdb, $gc;
	...
	if ( is_preview() && get_query_var( 'p' ) && 'publish' === get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			...
			|| ! gc_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] )
		) {
			$gc_query->is_preview = false;
		}
	}
	...
	if ( is_singular() && $gc_query->post_count < 1 && $post_id ) {

		$vars = $gcdb->get_results( $gcdb->prepare( "SELECT post_type, post_parent FROM $gcdb->posts WHERE ID = %d", $post_id ) );
	...
	// These tests give us a GC-generated permalink.
	if ( is_404() ) {
		...
		if ( $year && $month && $day ) {
			...
			if ( ! gc_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) ) {
			...
			if ( $gc_query->queried_object instanceof GC_Post ) {
				$post_id = $gc_query->queried_object->ID;
			} elseif ( $gc_query->post ) {
				$post_id = $gc_query->post->ID;
			}
	...
	} elseif ( is_object( $gc_rewrite ) && $gc_rewrite->using_permalinks() ) {

		// Rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101.
		if ( is_attachment()
			&& ! array_diff( array_keys( $gc->query_vars ), array( 'attachment', 'attachment_id' ) )
			&& ! $redirect_url
		) {
			...
			$redirect_url = get_permalink( $gc_query->get_queried_object_id() );
			$redirect_obj = get_post( $gc_query->get_queried_object_id() );
			...
			if ( false !== $author
				&& $gcdb->get_var( $gcdb->prepare( "SELECT ID FROM $gcdb->posts WHERE $gcdb->posts.post_author = %d AND $gcdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) )
			) {
			...
			if ( $term_count <= 1 && ! empty( $obj->term_id ) ) {
				...
				if ( $tax_url && ! is_gc_error( $tax_url ) ) {
					if ( ! empty( $redirect['query'] ) ) {
						...
						$rewrite_vars = array_diff( array_keys( $gc_query->query ), array_keys( $_GET ) );
			...
			if ( $category_name ) {
				...
				if ( ! $category || is_gc_error( $category )
					|| ! has_term( $category->term_id, 'category', $gc_query->get_queried_object_id() )
				) {
					$redirect_url = get_permalink( $gc_query->get_queried_object_id() );
					$redirect_obj = get_post( $gc_query->get_queried_object_id() );
				}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				...
				if ( is_front_page() ) {
					$redirect_url .= user_trailingslashit( "$gc_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'sitemap' ) ) {
			...
			while ( preg_match( "#/$gc_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] )
				|| preg_match( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+)?$#', $redirect['path'] )
				|| preg_match( "#/{$gc_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] )
			) {
				// Strip off any existing paging.
				$redirect['path'] = preg_replace( "#/$gc_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] );
				// Strip off feed endings.
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] );
				// Strip off any existing comment paging.
				$redirect['path'] = preg_replace( "#/{$gc_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] );
			}

			$addl_path    = '';
			$default_feed = get_default_feed();

			if ( is_feed() && in_array( $feed, $gc_rewrite->feeds, true ) ) {
				...
				$old_feed_files = array(
					'gc-atom.php'         => 'atom',
					'gc-commentsrss2.php' => 'comments_rss2',
					'gc-feed.php'         => $default_feed,
					'gc-rdf.php'          => 'rdf',
					'gc-rss.php'          => 'rss2',
					'gc-rss2.php'         => 'rss2',
				);
			...
			if ( $paged > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( ! is_single() ) {
						...
						if ( $paged > 1 ) {
							$addl_path .= user_trailingslashit( "$gc_rewrite->pagination_base/$paged", 'paged' );
						}
					}
				} elseif ( $paged > 1 ) {
			...
			if ( get_option( 'page_comments' )
				...
				$addl_path  = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' );
				$addl_path .= user_trailingslashit( $gc_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' );

				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			// Strip off trailing /index.php/.
			$redirect['path'] = preg_replace( '|/' . preg_quote( $gc_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] );
		...
		if ( 'gc-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in gc-login.php */
				$redirect_url = apply_filters( 'gc_signup_location', network_site_url( 'gc-signup.php' ) );
			} else {
				$redirect_url = gc_registration_url();
			}
	...
	if ( ! $gc_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $gc_rewrite->index . '/', '/', $redirect['path'] );
	}

	// Trailing slashes.
	if ( is_object( $gc_rewrite ) && $gc_rewrite->using_permalinks()
	...
	if ( $redirect_obj instanceof GC_Post ) {
...
function redirect_guess_404_permalink() {
	...
	if ( get_query_var( 'name' ) ) {
		...
		if ( $strict_guess ) {
			$where = $gcdb->prepare( 'post_name = %s', get_query_var( 'name' ) );
		} else {
			$where = $gcdb->prepare( 'post_name LIKE %s', $gcdb->esc_like( get_query_var( 'name' ) ) . '%' );
		}
		...
		if ( get_query_var( 'post_type' ) ) {
			if ( is_array( get_query_var( 'post_type' ) ) ) {
				...
				$where .= $gcdb->prepare( ' AND post_type = %s', get_query_var( 'post_type' ) );
			}
		} else {
		...
		if ( get_query_var( 'year' ) ) {
			$where .= $gcdb->prepare( ' AND YEAR(post_date) = %d', get_query_var( 'year' ) );
		}
		if ( get_query_var( 'monthnum' ) ) {
			$where .= $gcdb->prepare( ' AND MONTH(post_date) = %d', get_query_var( 'monthnum' ) );
		}
 ...
 * @global GC_Rewrite $gc_rewrite GeChiUI rewrite component.
 */
function gc_redirect_admin_locations() {
	global $gc_rewrite;

	if ( ! ( is_404() && $gc_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'gc-admin', 'relative' ),
