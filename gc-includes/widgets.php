 ...
 * themes
 *
 * Includes both internal GeChiUI routines and theme-use routines.
 *
...
/** @ignore */
global $gc_registered_sidebars, $gc_registered_widgets, $gc_registered_widget_controls, $gc_registered_widget_updates;
 ...
 * Private
 *
 * @global array $_gc_deprecated_widgets_callbacks
 */
$GLOBALS['_gc_deprecated_widgets_callbacks'] = array(
	'gc_widget_pages',
	'gc_widget_pages_control',
	'gc_widget_calendar',
	'gc_widget_calendar_control',
	'gc_widget_archives',
	'gc_widget_archives_control',
	'gc_widget_links',
	'gc_widget_meta',
	'gc_widget_meta_control',
	'gc_widget_search',
	'gc_widget_recent_entries',
	'gc_widget_recent_entries_control',
	'gc_widget_tag_cloud',
	'gc_widget_tag_cloud_control',
	'gc_widget_categories',
	'gc_widget_categories_control',
	'gc_widget_text',
	'gc_widget_text_control',
	'gc_widget_rss',
	'gc_widget_rss_control',
	'gc_widget_recent_comments',
	'gc_widget_recent_comments_control',
);
...
function unregister_widget( $widget ) {
	...
	$gc_widget_factory->unregister( $widget );
}
 ...
 *
 * @global array $gc_registered_sidebars The new sidebars are stored in this array by sidebar ID.
 *
 ...
 *                        Default 'Sidebar' for the first sidebar, otherwise '边栏%d'.
 * }
 */
function register_sidebars( $number = 1, $args = array() ) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( isset( $args['id'] ) ) {
			...
			$n = count( $gc_registered_sidebars );
			do {
				$_args['id'] = 'sidebar-' . ++$n;
			} while ( is_registered_sidebar( $_args['id'] ) );
		}
 ...
 * of default arguments for the new sidebar. GeChiUI will automatically
 ...
 * }
 * @return string Sidebar ID added to $gc_registered_sidebars global.
 */
	...
	if ( $id_is_empty ) {
		_doing_it_wrong(
			...
			sprintf(
				...
				__( '“%2$s”侧边栏的参数数组中未设置%1$s，缺省为“%3$s”。要消除此通知并保持现有的侧边栏内容，请手动将%1$s设置为“%3$s”。' ),
				'<code>id</code>',
				$sidebar['name'],
				$sidebar['id']
			),
...
function is_registered_sidebar( $sidebar_id ) {
	...
	return isset( $gc_registered_sidebars[ $sidebar_id ] );
}
 ...
 *                                         Only available for GC_Widget based widgets.
 * }
 * @param mixed      ...$params       Optional additional parameters to pass to the callback function when it's called.
 */
function gc_register_sidebar_widget( $id, $name, $output_callback, $options = array(), ...$params ) {
	global $gc_registered_widgets, $gc_registered_widget_controls, $gc_registered_widget_updates, $_gc_deprecated_widgets_callbacks;

	$id = strtolower( $id );

	if ( empty( $output_callback ) ) {
		unset( $gc_registered_widgets[ $id ] );
		return;
	}

	$id_base = _get_widget_id_base( $id );
	if ( in_array( $output_callback, $_gc_deprecated_widgets_callbacks, true ) && ! is_callable( $output_callback ) ) {
		unset( $gc_registered_widget_controls[ $id ] );
		unset( $gc_registered_widget_updates[ $id_base ] );
		return;
	}
	...
	if ( is_callable( $output_callback ) && ( ! isset( $gc_registered_widgets[ $id ] ) || did_action( 'widgets_init' ) ) ) {
		 ...
		 */
		do_action( 'gc_register_sidebar_widget', $widget );
		$gc_registered_widgets[ $id ] = $widget;
	}
}
 ...
 */
function gc_widget_description( $id ) {
	if ( ! is_scalar( $id ) ) {
		return;
	}
	...
	if ( isset( $gc_registered_widgets[ $id ]['description'] ) ) {
		return esc_html( $gc_registered_widgets[ $id ]['description'] );
	}
}
 ...
 */
function gc_sidebar_description( $id ) {
	if ( ! is_scalar( $id ) ) {
		return;
	}
	...
	if ( isset( $gc_registered_sidebars[ $id ]['description'] ) ) {
		return gc_kses( $gc_registered_sidebars[ $id ]['description'], 'sidebar_description' );
	}
}
...
function gc_unregister_sidebar_widget( $id ) {
	 ...
	 */
	do_action( 'gc_unregister_sidebar_widget', $id );

	gc_register_sidebar_widget( $id, '', '' );
	gc_unregister_widget_control( $id );
}
 ...
 * @param mixed      ...$params        Optional additional parameters to pass to the callback function when it's called.
 */
function gc_register_widget_control( $id, $name, $control_callback, $options = array(), ...$params ) {
	global $gc_registered_widget_controls, $gc_registered_widget_updates, $gc_registered_widgets, $_gc_deprecated_widgets_callbacks;
	...
	if ( in_array( $control_callback, $_gc_deprecated_widgets_callbacks, true ) && ! is_callable( $control_callback ) ) {
		unset( $gc_registered_widgets[ $id ] );
		return;
	}

	if ( isset( $gc_registered_widget_controls[ $id ] ) && ! did_action( 'widgets_init' ) ) {
		return;
	}
	...
	$gc_registered_widget_controls[ $id ] = $widget;
	...
	$gc_registered_widget_updates[ $id_base ] = $widget;
}
 ...
 * @global array $gc_registered_widget_updates
 *
 * @param string   $id_base         The base ID of a widget created by extending GC_Widget.
 * @param callable $update_callback Update callback method for the widget.
 * @param array    $options         Optional. Widget control options. See gc_register_widget_control().
 *                                  Default empty array.
 * @param mixed    ...$params       Optional additional parameters to pass to the callback function when it's called.
 */
function _register_widget_update_callback( $id_base, $update_callback, $options = array(), ...$params ) {
	global $gc_registered_widget_updates;
...
function dynamic_sidebar( $index = 1 ) {
	...
	if ( is_int( $index ) ) {
		...
		foreach ( (array) $gc_registered_sidebars as $key => $value ) {
	...
	if ( empty( $gc_registered_sidebars[ $index ] ) || empty( $sidebars_widgets[ $index ] ) || ! is_array( $sidebars_widgets[ $index ] ) ) {
		/** This action is documented in gc-includes/widget.php */
		do_action( 'dynamic_sidebar_before', $index, false );
		/** This action is documented in gc-includes/widget.php */
		do_action( 'dynamic_sidebar_after', $index, false );
		/** This filter is documented in gc-includes/widget.php */
		return apply_filters( 'dynamic_sidebar_has_widgets', false, $index );
	}

	$sidebar = $gc_registered_sidebars[ $index ];
	...
	foreach ( (array) $sidebars_widgets[ $index ] as $id ) {
		...
		$params = array_merge(
			array(
				array_merge(
					...
					array(
						'widget_id'   => $id,
						'widget_name' => $gc_registered_widgets[ $id ]['name'],
					)
				),
			),
			(array) $gc_registered_widgets[ $id ]['params']
		);

		// Substitute HTML `id` and `class` attributes into `before_widget`.
		$classname_ = '';
		foreach ( (array) $gc_registered_widgets[ $id ]['classname'] as $cn ) {
			if ( is_string( $cn ) ) {
				$classname_ .= '_' . $cn;
			} elseif ( is_object( $cn ) ) {
		...
		$callback = $gc_registered_widgets[ $id ]['callback'];
		 ...
		 * }
		 */
		do_action( 'dynamic_sidebar', $gc_registered_widgets[ $id ] );
 ...
 * $id_base is the first argument when extending GC_Widget class
 ...
 *                                      Default false.
 * @param string|false   $id_base       Optional. The base ID of a widget created by extending GC_Widget.
 *                                      Default false.
 * @param bool           $skip_inactive Optional. Whether to check in 'gc_inactive_widgets'.
 *                                      Default true.
...
function is_dynamic_sidebar() {
	global $gc_registered_widgets, $gc_registered_sidebars;

	$sidebars_widgets = get_option( 'sidebars_widgets' );

	foreach ( (array) $gc_registered_sidebars as $index => $sidebar ) {
		if ( ! empty( $sidebars_widgets[ $index ] ) ) {
			foreach ( (array) $sidebars_widgets[ $index ] as $widget ) {
				if ( array_key_exists( $widget, $gc_registered_widgets ) ) {
					return true;
				}
			}
		}
 ...
 */
function gc_get_sidebars_widgets( $deprecated = true ) {
	if ( true !== $deprecated ) {
	...
	if ( ! is_admin() ) {
		...
		$sidebars_widgets = $_gc_sidebars_widgets;
	} else {
 ...
 */
function gc_get_sidebar( $id ) {
	global $gc_registered_sidebars;

	foreach ( (array) $gc_registered_sidebars as $sidebar ) {
		if ( $sidebar['id'] === $id ) {
			return $sidebar;
		}
	}

	if ( 'gc_inactive_widgets' === $id ) {
		return array(
 ...
 */
function gc_set_sidebars_widgets( $sidebars_widgets ) {
	global $_gc_sidebars_widgets;

	// Clear cached value used in gc_get_sidebars_widgets().
	$_gc_sidebars_widgets = null;
 ...
 * @return array
 */
function gc_get_widget_defaults() {
	global $gc_registered_sidebars;
 ...
 */
function gc_convert_widget_settings( $base_name, $option_name, $settings ) {
	...
	if ( $single ) {
		...
		if ( is_admin() ) {
			...
			if ( empty( $GLOBALS['_gc_sidebars_widgets'] ) ) {
				$GLOBALS['_gc_sidebars_widgets'] = get_option( 'sidebars_widgets', array() );
			}
			$sidebars_widgets = &$GLOBALS['_gc_sidebars_widgets'];
		}
...
function the_widget( $widget, $instance = array(), $args = array() ) {
	...
	if ( ! isset( $gc_widget_factory->widgets[ $widget ] ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: register_widget() */
				__( '小工具需要使用%s注册后才能被显示。' ),
	...
	$widget_obj = $gc_widget_factory->widgets[ $widget ];
	if ( ! ( $widget_obj instanceof GC_Widget ) ) {
		return;
	}
	...
	$instance = gc_parse_args( $instance );
...
/**
 * Validates and remaps any "orphaned" widgets to gc_inactive_widgets sidebar,
 ...
 * moves all the widgets previously assigned to the footer under gc_inactive_widgets.
 *
...
function retrieve_widgets( $theme_changed = false ) {
	global $gc_registered_sidebars, $sidebars_widgets, $gc_registered_widgets;

	$registered_sidebars_keys = array_keys( $gc_registered_sidebars );
	$registered_widgets_ids   = array_keys( $gc_registered_widgets );
	...
	if ( ! is_array( get_theme_mod( 'sidebars_widgets' ) ) ) {
		...
		if ( $sidebars_widgets_keys === $registered_sidebars_keys ) {
			$sidebars_widgets = _gc_remove_unregistered_widgets( $sidebars_widgets, $registered_widgets_ids );
	...
	$sidebars_widgets = _gc_remove_unregistered_widgets( $sidebars_widgets, $registered_widgets_ids );
	$sidebars_widgets = gc_map_sidebars_widgets( $sidebars_widgets );
	...
	$sidebars_widgets['gc_inactive_widgets'] = array_merge( $lost_widgets, (array) $sidebars_widgets['gc_inactive_widgets'] );
 ...
 */
function gc_map_sidebars_widgets( $existing_sidebars_widgets ) {
	global $gc_registered_sidebars;
	...
	if ( 1 === count( $existing_sidebars_widgets ) && 1 === count( $gc_registered_sidebars ) ) {
		$new_sidebars_widgets[ key( $gc_registered_sidebars ) ] = array_pop( $existing_sidebars_widgets );
	...
	foreach ( $gc_registered_sidebars as $sidebar => $name ) {
	...
	if ( ! empty( $existing_sidebars_widgets ) ) {
		...
		foreach ( $common_slug_groups as $slug_group ) {
			...
			foreach ( $slug_group as $slug ) {
				...
				foreach ( $gc_registered_sidebars as $new_sidebar => $args ) {
				...
				} // End foreach ( $gc_registered_sidebars as $new_sidebar => $args ).
			} // End foreach ( $slug_group as $slug ).
	...
	foreach ( $existing_sidebars_widgets as $widgets ) {
		if ( is_array( $widgets ) && ! empty( $widgets ) ) {
			$new_sidebars_widgets['gc_inactive_widgets'] = array_merge( $new_sidebars_widgets['gc_inactive_widgets'], $widgets );
		}
	}
	...
	if ( is_array( $old_sidebars_widgets ) ) {
		...
		$old_sidebars_widgets = _gc_remove_unregistered_widgets( $old_sidebars_widgets );
				...
				foreach ( $new_sidebars_widgets as $new_sidebar => $new_widgets ) {
					...
					foreach ( $old_widgets as $key => $widget_id ) {
						...
						if ( false !== $active_key ) {
							...
							if ( 'gc_inactive_widgets' === $new_sidebar ) {

								// ...remove it from there and keep the active version...
								unset( $new_sidebars_widgets['gc_inactive_widgets'][ $active_key ] );
							} else {
 ...
 */
function _gc_remove_unregistered_widgets( $sidebars_widgets, $allowed_widget_ids = array() ) {
	if ( empty( $allowed_widget_ids ) ) {
		$allowed_widget_ids = array_keys( $GLOBALS['gc_registered_widgets'] );
	}
 ...
 */
function gc_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed( $rss );
	...
	if ( ! $rss->get_item_quantity() ) {
		echo '<ul><li>' . __( '发生错误，Feed 可能已经宕机，请稍后再试。' ) . '</li></ul>';
		$rss->__destruct();
		unset( $rss );
		return;
	}
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		$desc = esc_attr( gc_trim_words( $desc, 55, ' [&hellip;]' ) );
 ...
 */
function gc_widget_rss_form( $args, $inputs = null ) {
	...
	$inputs         = gc_parse_args( $inputs, $default_inputs );
	...
	if ( $inputs['url'] ) :
		?>
	<p><label for="rss-url-<?php echo $esc_number; ?>"><?php _e( '在此输入RSS feed URL：' ); ?></label>
...
<?php endif; if ( $inputs['title'] ) : ?>
	<p><label for="rss-title-<?php echo $esc_number; ?>"><?php _e( '为feed拟个标题（可选）：' ); ?></label>
...
<?php endif; if ( $inputs['items'] ) : ?>
	<p><label for="rss-items-<?php echo $esc_number; ?>"><?php _e( '您希望显示多少个项目？' ); ?></label>
...
<?php endif; if ( $inputs['show_summary'] || $inputs['show_author'] || $inputs['show_date'] ) : ?>
	...
	<?php if ( $inputs['show_summary'] ) : ?>
		...
		<label for="rss-show-summary-<?php echo $esc_number; ?>"><?php _e( '显示条目内容？' ); ?></label><br />
	<?php endif; if ( $inputs['show_author'] ) : ?>
		...
		<label for="rss-show-author-<?php echo $esc_number; ?>"><?php _e( '如果可用，显示条目作者？' ); ?></label><br />
	<?php endif; if ( $inputs['show_date'] ) : ?>
		...
		<label for="rss-show-date-<?php echo $esc_number; ?>"><?php _e( '显示条目日期？' ); ?></label><br />
	<?php endif; ?>
	</p>
	<?php
 ...
 * @return array
 */
function gc_widget_rss_process( $widget_rss, $check_feed = true ) {
...
function gc_widgets_init() {
	...
	register_widget( 'GC_Widget_Pages' );

	register_widget( 'GC_Widget_Calendar' );

	register_widget( 'GC_Widget_Archives' );

	if ( get_option( 'link_manager_enabled' ) ) {
		register_widget( 'GC_Widget_Links' );
	}

	register_widget( 'GC_Widget_Media_Audio' );

	register_widget( 'GC_Widget_Media_Image' );

	register_widget( 'GC_Widget_Media_Gallery' );

	register_widget( 'GC_Widget_Media_Video' );

	register_widget( 'GC_Widget_Meta' );
	...
	register_widget( 'GC_Widget_Recent_Posts' );

	register_widget( 'GC_Widget_Recent_Comments' );

	register_widget( 'GC_Widget_RSS' );

	register_widget( 'GC_Widget_Tag_Cloud' );

	register_widget( 'GC_Nav_Menu_Widget' );

	register_widget( 'GC_Widget_Custom_HTML' );

	register_widget( 'GC_Widget_Block' );
 ...
 * @access private
 */
function gc_setup_widgets_block_editor() {
 ...
 */
function gc_use_widgets_block_editor() {
	/**
 ...
 */
function gc_parse_widget_id( $id ) {
	$parsed = array();
 ...
 */
function gc_find_widgets_sidebar( $widget_id ) {
 ...
 */
function gc_assign_widget_to_sidebar( $widget_id, $sidebar_id ) {
 ...
 * @return string
 */
function gc_render_widget( $widget_id, $sidebar_id ) {
	global $gc_registered_widgets, $gc_registered_sidebars;
	...
	if ( isset( $gc_registered_sidebars[ $sidebar_id ] ) ) {
		$sidebar = $gc_registered_sidebars[ $sidebar_id ];
	} elseif ( 'gc_inactive_widgets' === $sidebar_id ) {
		$sidebar = array();
	} else {
		return '';
	}
	...
	$params = array_merge(
		array(
			array_merge(
				...
				array(
					'widget_id'   => $widget_id,
					'widget_name' => $gc_registered_widgets[ $widget_id ]['name'],
				)
			),
		),
		(array) $gc_registered_widgets[ $widget_id ]['params']
	);

	// Substitute HTML `id` and `class` attributes into `before_widget`.
	$classname_ = '';
	foreach ( (array) $gc_registered_widgets[ $widget_id ]['classname'] as $cn ) {
		if ( is_string( $cn ) ) {
			$classname_ .= '_' . $cn;
		} elseif ( is_object( $cn ) ) {
			$classname_ .= '_' . get_class( $cn );
	...
	/** This filter is documented in gc-includes/widgets.php */
	$params = apply_filters( 'dynamic_sidebar_params', $params );

	$callback = $gc_registered_widgets[ $widget_id ]['callback'];

	ob_start();

	/** This filter is documented in gc-includes/widgets.php */
	do_action( 'dynamic_sidebar', $gc_registered_widgets[ $widget_id ] );
 ...
 * @return string|null
 */
function gc_render_widget_control( $id ) {
	global $gc_registered_widget_controls;

	if ( ! isset( $gc_registered_widget_controls[ $id ]['callback'] ) ) {
		return null;
	}

	$callback = $gc_registered_widget_controls[ $id ]['callback'];
	$params   = $gc_registered_widget_controls[ $id ]['params'];
 ...
 *
 * The 'gc-editor' script module is exposed as window.gc.editor. This overrides
 ...
 */
function gc_check_widget_editor_deps() {
	global $gc_scripts, $gc_styles;

	if (
		$gc_scripts->query( 'gc-edit-widgets', 'enqueued' ) ||
		$gc_scripts->query( 'gc-customize-widgets', 'enqueued' )
	) {
		if ( $gc_scripts->query( 'gc-editor', 'enqueued' ) ) {
			_doing_it_wrong(
				'gc_enqueue_script()',
				sprintf(
					/* translators: 1: 'gc-editor', 2: 'gc-edit-widgets', 3: 'gc-customize-widgets'. */
					__( '“%1$s”脚本不应与新的小部件编辑器（%2$s 或 %3$s）一起入队。' ),
					'gc-editor',
					'gc-edit-widgets',
		...
		if ( $gc_styles->query( 'gc-edit-post', 'enqueued' ) ) {
			_doing_it_wrong(
				'gc_enqueue_style()',
				sprintf(
					/* translators: 1: 'gc-edit-post', 2: 'gc-edit-widgets', 3: 'gc-customize-widgets'. */
					__( '“%1$s”样式不应与新的小部件编辑器（%2$s 或 %3$s）一起入队。' ),
					'gc-edit-post',
					'gc-edit-widgets',
					'gc-customize-widgets'
				),
