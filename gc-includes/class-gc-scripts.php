<?php
/**
 * Dependencies API: GC_Scripts class
 *
 * @package GeChiUI
 ...
 * @see GC_Dependencies
 */
class GC_Scripts extends GC_Dependencies {
	/**
	...
	public function init() {
		...
		/**
		 * Fires when the GC_Scripts instance is initialized.
		 *
		 * @since 2.6.0
		 *
		 * @param GC_Scripts $gc_scripts GC_Scripts instance (passed by reference).
		 */
		do_action_ref_array( 'gc_default_scripts', array( &$this ) );
	}
	...
	public function print_scripts_l10n( $handle, $display = true ) {
		_deprecated_function( __FUNCTION__, '3.3.0', 'GC_Scripts::print_extra_script()' );
	 ...
	 *
	 * @since 2.6.0 Added the `$group` parameter.
	 *
	 * @see GC_Dependencies::do_item()
	 *
	 * @param string    $handle The script's registered handle.
	 * @param int|false $group  Optional. Group level: level (int), no groups (false).
	 *                          Default false.
	...
	public function do_item( $handle, $group = false ) {
		...
		$tag .= sprintf(
			...
			esc_attr( $handle ),
			$strategy ? " {$strategy}" : '',
			$intended_strategy ? " data-gc-strategy='{$intended_strategy}'" : ''
		);
	...
	public function print_inline_script( $handle, $position = 'after', $display = true ) {
		_deprecated_function( __METHOD__, '6.3.0', 'GC_Scripts::get_inline_script_data() or GC_Scripts::get_inline_script_tag()' );
	...
	public function get_inline_script_tag( $handle, $position = 'after' ) {
		...
		return gc_get_inline_script_tag( $js, compact( 'id' ) );
	}
	...
	public function localize( $handle, $object_name, $l10n ) {
		...
		if ( ! is_array( $l10n ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: 1: $l10n, 2: gc_add_inline_script() */
					__( '%1$s参数必须是一个数组。若要将任意数据传递给脚本，请改用%2$s函数。' ),
					'<code>$l10n</code>',
		...
		$script = "var $object_name = " . gc_json_encode( $l10n ) . ';';
	...
	public function set_translations( $handle, $domain = 'default', $path = '' ) {
		...
		/** @var \_GC_Dependency $obj */
		$obj = $this->registered[ $handle ];

		if ( ! in_array( 'gc-i18n', $obj->deps, true ) ) {
			$obj->deps[] = 'gc-i18n';
		}
...
JS;
	...
	public function in_default_dir( $src ) {
		...
		if ( str_starts_with( $src, '/' . GCINC . '/js/l10n' ) ) {
			return false;
		}
	...
	/**
	 * This overrides the add_data method from GC_Dependencies, to support normalizing of $args.
	 *
	 * @since 6.3.0
	 *
	 * @param string $handle Name of the item. Should be unique.
			...
			if ( ! empty( $value ) && ! $this->is_delayed_strategy( $value ) ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						__( '在脚本注册过程中，为“%2$s”定义的策略“%1$s”无效。' ),
						$value,
						$handle
					),
					'6.3.0'
				...
				_doing_it_wrong(
					...
					sprintf(
						/* translators: 1: $strategy, 2: $handle */
						__( '无法为脚本“%2$s”提供策略“%1$s”，因为它是别名（缺少“src”值）。' ),
						$value,
						$handle
					),
					'6.3.0'
