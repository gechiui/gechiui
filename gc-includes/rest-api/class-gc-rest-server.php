...
class GC_REST_Server {
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a GC_Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A GC_Error instance can be returned if an error occurs, and this should
	 ...
	 *
	 * Note: This should only be used in GC_REST_Server::serve_request(), as it
	 * cannot handle GC_Error internally. All callbacks and other internal methods
	 ...
	 *
	 * @global GC_User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var GC_User|null $current_user */
		global $current_user;

		if ( $current_user instanceof GC_User && ! $current_user->exists() ) {
			/*
		...
		if ( $send_no_cache_headers ) {
			foreach ( gc_get_nocache_headers() as $header => $header_value ) {
		...
		apply_filters_deprecated(
			...
			sprintf(
				...
				__( 'REST API不再能被完全禁用，不过您可以用%s过滤器来限制对该API的访问。' ),
				'rest_authentication_errors'
			)
		);
		...
		if ( $jsonp_callback ) {
			...
			if ( ! gc_check_jsonp_callback( $jsonp_callback ) ) {
				echo $this->json_error( 'rest_callback_invalid', __( '无效的JSONP回调函数。' ), 400 );
				return false;
			}
		}
		...
		$request = new GC_REST_Request( $_SERVER['REQUEST_METHOD'], $path );

		$request->set_query_params( gc_unslash( $_GET ) );
		$request->set_body_params( gc_unslash( $_POST ) );
		$request->set_file_params( $_FILES );
		$request->set_headers( $this->get_headers( gc_unslash( $_SERVER ) ) );
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = GC_REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
	...
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param GC_REST_Response $response Original response data.
		 */
	...
	public function get_routes( $route_namespace = '' ) {
		...
		foreach ( $endpoints as $route => &$handlers ) {
			...
			foreach ( $handlers as $key => &$handler ) {
				...
				$handler = gc_parse_args( $handler, $defaults );
	...
	public function dispatch( $request ) {
		...
		if ( ! is_gc_error( $error ) ) {
			...
			if ( is_gc_error( $check_required ) ) {
				$error = $check_required;
			} else {
				$check_sanitized = $request->sanitize_params();
				if ( is_gc_error( $check_sanitized ) ) {
	 ...
	 * @param GC_Error|null   $response The current error object if any.
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		...
		if ( ! is_gc_error( $response ) && ! empty( $handler['permission_callback'] ) ) {
			$permission = call_user_func( $handler['permission_callback'], $request );

			if ( is_gc_error( $permission ) ) {
				$response = $permission;
			} elseif ( false === $permission || null === $permission ) {
				$response = new GC_Error(
					'rest_forbidden',
					__( '抱歉，您不能这么做。' ),
	 ...
	 * }
	 * @return GC_REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new GC_REST_Response( $available );
		$response->add_link( 'help', 'https://developer.gechiui.com/rest-api/' );
	 ...
	 */
	protected function add_active_theme_link_to_index( GC_REST_Response $response ) {
		...
		if ( $should_add ) {
			$theme = gc_get_theme();
			$response->add_link( 'https://api.w.org/active-theme', rest_url( 'gc/v2/themes/' . $theme->get_stylesheet() ) );
		}
	}

	/**
	 * Exposes the site logo through the GeChiUI REST API.
	 *
	 ...
	 * @param GC_REST_Response $response REST API response.
	 */
	protected function add_site_logo_to_index( GC_REST_Response $response ) {
	...
	/**
	 * Exposes the site icon through the GeChiUI REST API.
	 *
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 ...
	 * @param GC_REST_Response $response REST API response.
	 */
	protected function add_site_icon_to_index( GC_REST_Response $response ) {
	...
	/**
	 * Exposes an image through the GeChiUI REST API.
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 * @since 5.9.0
	 ...
	 */
	protected function add_image_to_index( GC_REST_Response $response, $image_id, $type ) {
	 ...
	 *                                   GC_Error if the namespace isn't set.
	 */
	 ...
	 */
	public function serve_batch_request_v1( GC_REST_Request $batch_request ) {
		$requests = array();

		foreach ( $batch_request['requests'] as $args ) {
			$parsed_url = gc_parse_url( $args['path'] );

			if ( false === $parsed_url ) {
				$requests[] = new GC_Error( 'parse_path_failed', __( '无法解析此路径。' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new GC_REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );

			if ( ! empty( $parsed_url['query'] ) ) {
				$query_args = null; // Satisfy linter.
				gc_parse_str( $parsed_url['query'], $query_args );
		...
		return new GC_REST_Response( array( 'responses' => $responses ), GC_Http::MULTI_STATUS );
	}
