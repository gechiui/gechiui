<?php
/**
 * REST API: GC_REST_URL_Details_Controller class
 *
 * @package GeChiUI
 * @subpackage REST_API
 *
 ...
 */
class GC_REST_URL_Details_Controller extends GC_REST_Controller {
	...
	public function parse_url_details( $request ) {
		...
		if ( empty( $url ) ) {
			return new GC_Error( 'rest_invalid_url', __( '无效的URL' ), array( 'status' => 404 ) );
		}
		...
		if ( ! empty( $cached_response ) ) {
			...
			if ( is_gc_error( $remote_url_response ) || empty( $remote_url_response ) ) {
	 ...
	 *                         GC_Error if no response or no content.
	 */
	private function get_remote_url( $url ) {

		/*
		 * Provide a modified UA string to workaround web properties which block GeChiUI "Pingbacks".
		 * Why? The UA string used for pingback requests contains `GeChiUI/` which is very similar
		 * to that used as the default UA string by the GC HTTP API. Therefore requests from this
		 * REST endpoint are being unintentionally blocked as they are misidentified as pingback requests.
		 * By slightly modifying the UA string, but still retaining the "GeChiUI" identification (via "GC")
		 * we are able to work around this issue.
		 * Example UA string: `GC-URLDetails/5.9-alpha-51389 (+http://localhost:8888)`.
		*/
		$modified_user_agent = 'GC-URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		...
		$response = gc_safe_remote_get( $url, $args );

		if ( GC_Http::OK !== gc_remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new GC_Error(
				'no_response',
				__( '找不到URL。响应返回了此URL的非200状态代码。' ),
				array( 'status' => GC_Http::NOT_FOUND )
			);
		}

		$remote_body = gc_remote_retrieve_body( $response );

		if ( empty( $remote_body ) ) {
			return new GC_Error(
				'no_content',
				__( '无法从此URL的响应中获取body。' ),
				array( 'status' => GC_Http::NOT_FOUND )
			);
		}
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = GC_Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = GC_Http::make_absolute_url( $image, $root_url );
		}
	...
	private function prepare_metadata_for_output( $metadata ) {
		...
		$metadata = gc_strip_all_tags( $metadata );
		return $metadata;
	}
