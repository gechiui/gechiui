<?php
/**
 * REST API: GC_REST_Sidebars_Controller class
 *
 * Original code from {@link https://github.com/martin-pettersson/gc-rest-api-sidebars Martin Pettersson (martin_pettersson@outlook.com)}.
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class GC_REST_Sidebars_Controller extends GC_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		foreach ( gc_get_sidebars_widgets() as $id => $widgets ) {
	...
	public function get_items( $request ) {
		...
		foreach ( gc_get_sidebars_widgets() as $id => $widgets ) {
			...
			if ( is_gc_error( $permissions_check ) && ! $this->check_read_permission( $sidebar ) ) {
				continue;
			}
	...
	public function get_item( $request ) {
		...
		$sidebar = $this->get_sidebar( $request['id'] );
		if ( ! $sidebar ) {
			return new GC_Error( 'rest_sidebar_not_found', __( '不存在具有该id的侧边栏。' ), array( 'status' => 404 ) );
		}
		...
		if ( isset( $request['widgets'] ) ) {
			...
			foreach ( $sidebars as $sidebar_id => $widgets ) {
				foreach ( $widgets as $i => $widget_id ) {
					...
					if ( $sidebar_id === $request['id'] && ! in_array( $widget_id, $request['widgets'], true ) ) {
						$sidebars['gc_inactive_widgets'][] = $widget_id;
					}
				}
			}
	 ...
	 * @param GC_REST_Request $request Full details about the request.
	 * @return GC_REST_Response Prepared response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( isset( $gc_registered_sidebars[ $id ] ) ) {
			$registered_sidebar = $gc_registered_sidebars[ $id ];

			$sidebar['status']        = 'active';
			$sidebar['name']          = isset( $registered_sidebar['name'] ) ? $registered_sidebar['name'] : '';
			$sidebar['description']   = isset( $registered_sidebar['description'] ) ? gc_sidebar_description( $id ) : '';
		...
		if ( rest_is_field_included( 'widgets', $fields ) ) {
			...
			$widgets  = array_filter(
				...
				static function ( $widget_id ) use ( $gc_registered_widgets ) {
					return isset( $gc_registered_widgets[ $widget_id ] );
				}
			);
	...
	protected function prepare_links( $sidebar ) {
		return array(
			...
			'https://api.w.org/widget' => array(
				'href'       => add_query_arg( 'sidebar', $sidebar['id'], rest_url( '/gc/v2/widgets' ) ),
				'embeddable' => true,
			),
		);
	}
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				'id'            => array(
					'description' => __( '边栏的ID。' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'name'          => array(
					'description' => __( '标识侧栏的唯一名称。' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'description'   => array(
					'description' => __( '侧边栏的描述。' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'class'         => array(
					'description' => __( '额外的CSS类，可分配给小工具界面的侧栏。' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'before_widget' => array(
					'description' => __( '分配给此侧栏时附加到每个小工具HTML输出的HTML内容。默认值是打开列表项元素。' ),
					'type'        => 'string',
					'default'     => '',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				...
				'after_widget'  => array(
					'description' => __( '分配给此侧栏时附加到每个小工具HTML输出的HTML内容。默认值是关闭列表项元素。' ),
					'type'        => 'string',
					'default'     => '',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				...
				'before_title'  => array(
					'description' => __( '显示时附加到边栏标题前的HTML内容。默认是打开h2元素。' ),
					'type'        => 'string',
					'default'     => '',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				...
				'after_title'   => array(
					'description' => __( '显示时附加到边栏标题后的HTML内容。默认是关闭h2元素。' ),
					'type'        => 'string',
					'default'     => '',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
