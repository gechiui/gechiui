<?php
/**
 * REST API: GC_REST_Site_Health_Controller class
 *
 * @package GeChiUI
 * @subpackage REST_API
 */
 ...
 */
class GC_REST_Site_Health_Controller extends GC_REST_Controller {
	...
	public function __construct( $site_health ) {
		$this->namespace = 'gc-site-health/v1';
	...
	public function get_directory_sizes() {
		...
		if ( isset( $all_sizes['total_size']['debug'] ) && 'not available' === $all_sizes['total_size']['debug'] ) {
			return new GC_Error( 'not_available', __( '无法返回目录大小。' ), array( 'status' => 500 ) );
		}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'title'      => 'gc-site-health-test',
			'type'       => 'object',
			'properties' => array(
				'test'        => array(
					'type'        => 'string',
					'description' => __( '正在运行的测试的名称。' ),
					'readonly'    => true,
				),
				'label'       => array(
					'type'        => 'string',
					'description' => __( '描述测试的标签。' ),
					'readonly'    => true,
				),
				'status'      => array(
					'type'        => 'string',
					'description' => __( '测试的状态。' ),
					'enum'        => array( 'good', 'recommended', 'critical' ),
					'readonly'    => true,
				),
				'badge'       => array(
					'type'        => 'object',
					'description' => __( '此测试所在的分类。' ),
					'properties'  => array(
						'label' => array(
							'type'     => 'string',
							'readonly' => true,
				...
				'description' => array(
					'type'        => 'string',
					'description' => __( '更具描述性的测试目的说明，及测试对用户很重要的原因。' ),
					'readonly'    => true,
				),
				'actions'     => array(
					'type'        => 'string',
					'description' => __( 'HTML包含了将用户引导至可以解决问题地方的操作。' ),
					'readonly'    => true,
				),
			),
		);
