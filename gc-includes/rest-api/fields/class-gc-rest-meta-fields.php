<?php
/**
 * REST API: GC_REST_Meta_Fields class
 *
 * @package GeChiUI
 * @subpackage REST_API
 *
...
abstract class GC_REST_Meta_Fields {
	 ...
	 * @param int   $object_id Object ID to fetch meta for.
	 * @return null|GC_Error Null on success, GC_Error object on failure.
	 */
		...
		foreach ( $fields as $meta_key => $args ) {
			...
			if ( is_null( $value ) || ( array() === $value && ! $args['single'] ) ) {
				...
				if ( $args['single'] ) {
					...
					if ( is_gc_error( rest_validate_value_from_schema( $current, $args['schema'] ) ) ) {
						return new GC_Error(
	 ...
	 * @return true|GC_Error True if meta field is deleted, GC_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! current_user_can( "delete_{$meta_type}_meta", $object_id, $meta_key ) ) {
			return new GC_Error(
				'rest_cannot_delete',
				/* translators: %s: Custom field key. */
				sprintf( __( '抱歉，您不能编辑自定义字段%s。' ), $name ),
				array(
					'key'    => $name,
		...
		if ( null === get_metadata_raw( $meta_type, $object_id, gc_slash( $meta_key ) ) ) {
			return true;
		}

		if ( ! delete_metadata( $meta_type, $object_id, gc_slash( $meta_key ) ) ) {
			return new GC_Error(
				'rest_meta_database_error',
				__( '未能从数据库删除元数据。' ),
				array(
					'key'    => $name,
					'status' => GC_Http::INTERNAL_SERVER_ERROR,
				)
			);
		}
	 ...
	 * @return true|GC_Error True if meta fields are updated, GC_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, gc_slash( $meta_key ), gc_slash( $value ) ) ) {
				return new GC_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
					sprintf( __( '未能在数据库更新元数据%s。' ), $meta_key ),
					array(
						'key'    => $name,
						'status' => GC_Http::INTERNAL_SERVER_ERROR,
					)
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, gc_slash( $meta_key ), gc_slash( $value ) ) ) {
				return new GC_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
					sprintf( __( '未能在数据库更新元数据%s。' ), $meta_key ),
	 ...
	 * @return true|GC_Error True if the meta field was updated, GC_Error otherwise.
	 */
