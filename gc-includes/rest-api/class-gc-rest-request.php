<?php
/**
 * REST API: GC_REST_Request class
 *
 * @package GeChiUI
 * @subpackage REST_API
 */
...
class GC_REST_Request implements ArrayAccess {
	...
	public function is_json_content_type() {
		...
		return isset( $content_type['value'] ) && gc_is_json_media_type( $content_type['value'] );
	}
	...
	protected function parse_json_params() {
		...
		if ( null === $params && JSON_ERROR_NONE !== json_last_error() ) {
			...
			$error_data = array(
				'status'             => GC_Http::BAD_REQUEST,
				'json_error_code'    => json_last_error(),
				'json_error_message' => json_last_error_msg(),
			);

			return new GC_Error( 'rest_invalid_json', __( '传入的JSON体无效。' ), $error_data );
		}
	...
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				/** @var mixed|GC_Error $sanitized_value */
		...
		if ( $invalid_params ) {
			return new GC_Error(
				...
				sprintf( __( '无效参数：%s' ), implode( ', ', array_keys( $invalid_params ) ) ),
				array(
					'status'  => 400,
	...
	public function has_valid_params() {
		...
		if ( ! empty( $required ) ) {
			return new GC_Error(
				...
				sprintf( __( '缺少参数：%s' ), implode( ', ', $required ) ),
				array(
					'status' => 400,
					'params' => $required,
				)
		...
		foreach ( $args as $key => $arg ) {
			...
			if ( null !== $param && ! empty( $arg['validate_callback'] ) ) {
				/** @var bool|\GC_Error $valid_check */
				$valid_check = call_user_func( $arg['validate_callback'], $param, $this, $key );

				if ( false === $valid_check ) {
					$invalid_params[ $key ] = __( '无效参数。' );
				}
		...
		if ( isset( $attributes['validate_callback'] ) ) {
			...
			if ( false === $valid_check ) {
				// A GC_Error instance is preferred, but false is supported for parity with the per-arg validate_callback.
				return new GC_Error( 'rest_invalid_params', __( '无效的参数。' ), array( 'status' => 400 ) );
			}
		}
	...
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			gc_parse_str( $bits['query'], $query_params );
		}
