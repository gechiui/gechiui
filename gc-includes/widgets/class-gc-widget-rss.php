<?php
/**
 * Widget API: GC_Widget_RSS class
 *
 * @package GeChiUI
 * @subpackage Widgets
 *
 ...
 * @see GC_Widget
 */
class GC_Widget_RSS extends GC_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'description'                 => __( '任意RSS或Atom feed中的条目。' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $title ) ) {
			$title = ! empty( $desc ) ? $desc : __( '未知Feed' );
		}
		...
		if ( $title ) {
			...
			$feed_link = sprintf(
				...
				esc_url( $feed_icon ),
				esc_attr__( 'RSS' ),
				( gc_lazy_loading_enabled( 'img', 'rss_widget_feed_icon' ) ? ' loading="lazy"' : '' )
			);
		...
		gc_widget_rss_output( $rss, $instance );
	...
	public function update( $new_instance, $old_instance ) {
		...
		return gc_widget_rss_process( $new_instance, $testurl );
	}
