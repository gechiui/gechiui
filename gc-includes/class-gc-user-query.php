...
class GC_User_Query {
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the GC_User_Query has been parsed.
		 *
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = gc_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $qv['has_published_posts'] && $blog_id ) {
			...
			foreach ( $post_types as &$post_type ) {
				$post_type = $gcdb->prepare( '%s', $post_type );
			}

			$posts_table        = $gcdb->get_blog_prefix( $blog_id ) . 'posts';
			$this->query_where .= " AND $gcdb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . implode( ', ', $post_types ) . ' ) )';
		}

		// nicename
		if ( '' !== $qv['nicename'] ) {
			$this->query_where .= $gcdb->prepare( ' AND user_nicename = %s', $qv['nicename'] );
		}
		...
		// login
		if ( '' !== $qv['login'] ) {
			$this->query_where .= $gcdb->prepare( ' AND user_login = %s', $qv['login'] );
		}
		...
		if ( isset( $qv['who'] ) && 'authors' === $qv['who'] && $blog_id ) {
			...
			$who_query = array(
				'key'     => $gcdb->get_blog_prefix( $blog_id ) . 'user_level',
				'value'   => 0,
				'compare' => '!=',
			);
		...
		if ( $blog_id && ! empty( $capabilities ) ) {
			...
			foreach ( $capabilities as $cap ) {
				...
				$clause[] = array(
					'key'     => $gcdb->get_blog_prefix( $blog_id ) . 'capabilities',
					'value'   => '"' . $cap . '"',
					'compare' => 'LIKE',
				);
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses            = $this->meta_query->get_sql( 'user', $gcdb->users, 'ID', $this );
		...
		if ( isset( $qv['number'] ) && $qv['number'] > 0 ) {
			if ( $qv['offset'] ) {
				$this->query_limit = $gcdb->prepare( 'LIMIT %d, %d', $qv['offset'], $qv['number'] );
			} else {
				$this->query_limit = $gcdb->prepare( 'LIMIT %d, %d', $qv['number'] * ( $qv['paged'] - 1 ), $qv['number'] );
			}
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				} elseif ( preg_match( '|^https?://|', $search ) && ! ( is_multisite() && gc_is_large_network( 'users' ) ) ) {
			...
			/**
			 * Filters the columns to search in a GC_User_Query search.
			 *
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids                = implode( ',', $include );
			$this->query_where .= " AND $gcdb->users.ID IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids                = implode( ',', gc_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $gcdb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new GC_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the GC_User_Query has been parsed, and before
	...
	public function query() {
		 ...
		 *
		 * Return a non-null value to bypass GeChiUI' default user queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `total_users` property of the GC_User_Query object, passed to the filter
		 ...
		 * @param GC_User_Query $query   The GC_User_Query instance (passed by reference).
		 */
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ), true ) ) {
			...
			$include     = gc_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby    = "FIELD( $gcdb->users.ID, $include_sql )";
