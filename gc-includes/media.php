 ...
 */
function gc_get_additional_image_sizes() {
	global $_gc_additional_image_sizes;

	if ( ! $_gc_additional_image_sizes ) {
		$_gc_additional_image_sizes = array();
	}

	return $_gc_additional_image_sizes;
}
...
function image_downsize( $id, $size = 'medium' ) {
	$is_image = gc_attachment_is_image( $id );
	...
	$img_url          = gc_get_attachment_url( $id );
...
function remove_image_size( $name ) {
	...
	if ( isset( $_gc_additional_image_sizes[ $name ] ) ) {
		unset( $_gc_additional_image_sizes[ $name ] );
		return true;
	}
 ...
 * parameters are: what GeChiUI will use for the class, the Attachment ID,
...
function get_image_tag( $id, $alt, $title, $align, $size = 'medium' ) {
	...
	$class      = 'align' . esc_attr( $align ) . ' size-' . esc_attr( $size_class ) . ' gc-image-' . $id;
 ...
 * }
 */
function gc_constrain_dimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
	if ( ! $max_width && ! $max_height ) {
	 ...
	 */
	return apply_filters( 'gc_constrain_dimensions', array( $w, $h ), $current_width, $current_height, $max_width, $max_height );
}

/**
 * Retrieves calculated resize dimensions for use in GC_Image_Editor.
 *
...
function image_resize_dimensions( $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = gc_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}

	if ( gc_fuzzy_number_match( $new_w, $orig_w ) && gc_fuzzy_number_match( $new_h, $orig_h ) ) {
		 ...
		 */
		$proceed = (bool) apply_filters( 'gc_image_resize_identical_dimensions', false, $orig_w, $orig_h );
 ...
 */
function gc_image_matches_ratio( $source_width, $source_height, $target_width, $target_height ) {
	/*
	 * To test for varying crops, we constrain the dimensions of the larger image
	 * to the dimensions of the smaller image and see if they match.
	 */
	if ( $source_width > $target_width ) {
		$constrained_size = gc_constrain_dimensions( $source_width, $source_height, $target_width );
		$expected_size    = array( $target_width, $target_height );
	} else {
		$constrained_size = gc_constrain_dimensions( $target_width, $target_height, $source_width );
		$expected_size    = array( $source_width, $source_height );
	}

	// If the image dimensions are within 1px of the expected size, we consider it a match.
	$matched = ( gc_fuzzy_number_match( $constrained_size[0], $expected_size[0] ) && gc_fuzzy_number_match( $constrained_size[1], $expected_size[1] ) );
...
function image_get_intermediate_size( $post_id, $size = 'thumbnail' ) {
	$imagedata = gc_get_attachment_metadata( $post_id );
		...
		foreach ( $imagedata['sizes'] as $_size => $data ) {
			...
			if ( $data['width'] >= $size[0] && $data['height'] >= $size[1] ) {
				...
				if ( 0 === $size[0] || 0 === $size[1] ) {
					$same_ratio = gc_image_matches_ratio( $data['width'], $data['height'], $imagedata['width'], $imagedata['height'] );
				} else {
					$same_ratio = gc_image_matches_ratio( $data['width'], $data['height'], $size[0], $size[1] );
				}
...
function get_intermediate_image_sizes() {
	...
	$additional_sizes = gc_get_additional_image_sizes();
 ...
 */
function gc_get_registered_image_subsizes() {
	$additional_sizes = gc_get_additional_image_sizes();
	$all_sizes        = array();
 ...
 * }
 */
function gc_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	...
	if ( ! $image ) {
		...
		if ( $icon ) {
			$src = gc_mime_type_icon( $attachment_id );
	 ...
	 */
	return apply_filters( 'gc_get_attachment_image_src', $image, $attachment_id, $size, $icon );
}
 ...
 */
function gc_get_attachment_image( $attachment_id, $size = 'thumbnail', $icon = false, $attr = '' ) {
	$html  = '';
	$image = gc_get_attachment_image_src( $attachment_id, $size, $icon );
	...
	if ( $image ) {
		...
		if ( empty( $attr['srcset'] ) ) {
			...
			if ( is_array( $image_meta ) ) {
				...
				$srcset     = gc_calculate_image_srcset( $size_array, $src, $image_meta, $attachment_id );
				$sizes      = gc_calculate_image_sizes( $size_array, $src, $image_meta, $attachment_id );
		 ...
		 * @param GC_Post      $attachment Image attachment post.
		 * @param string|int[] $size       Requested image size. Can be any registered image size name, or
		 *                                 an array of width and height values in pixels (in that order).
		 */
		$attr = apply_filters( 'gc_get_attachment_image_attributes', $attr, $attachment, $size );
	 ...
	 *                                    See gc_get_attachment_image().
	 */
	return apply_filters( 'gc_get_attachment_image', $html, $attachment_id, $size, $icon, $attr );
}
 ...
 */
function gc_get_attachment_image_url( $attachment_id, $size = 'thumbnail', $icon = false ) {
	$image = gc_get_attachment_image_src( $attachment_id, $size, $icon );
 ...
 */
function _gc_get_attachment_relative_path( $file ) {
 ...
 * }
 */
function _gc_get_image_size_from_meta( $size_name, $image_meta ) {
	if ( 'full' === $size_name ) {
		return array(
 ...
 * @param string|int[] $size          Optional. Image size. Accepts any registered image size name, or an array of
 *                                    width and height values in pixels (in that order). Default 'medium'.
 * @param array        $image_meta    Optional. The image meta data as returned by 'gc_get_attachment_metadata()'.
 *                                    Default null.
 * @return string|false A 'srcset' value string or false.
 */
function gc_get_attachment_image_srcset( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = gc_get_attachment_image_src( $attachment_id, $size );
	...
	return gc_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
}
...
function gc_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	 ...
	 */
	$image_meta = apply_filters( 'gc_calculate_image_srcset_meta', $image_meta, $size_array, $image_src, $attachment_id );
	...
	$image_basename = gc_basename( $image_meta['file'] );

	/*
	 * GeChiUI flattens animated GIFs into one frame when generating intermediate sizes.
	...
	$dirname = _gc_get_attachment_relative_path( $image_meta['file'] );
	...
	/*
	 * Images that have been edited in GeChiUI after being uploaded will
	 * contain a unique hash. Look for that hash and use it later to filter
	 * out images that are leftovers from previous versions.
	 */
	$image_edited = preg_match( '/-e[0-9]{13}/', gc_basename( $image_src ), $image_edit_hash );
	...
	foreach ( $image_sizes as $image ) {
		...
		if ( gc_image_matches_ratio( $image_width, $image_height, $image['width'], $image['height'] ) ) {
	 ...
	 */
	$sources = apply_filters( 'gc_calculate_image_srcset', $sources, $size_array, $image_src, $image_meta, $attachment_id );
 ...
 */
function gc_get_attachment_image_sizes( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = gc_get_attachment_image_src( $attachment_id, $size );
	...
	return gc_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
}
 ...
 * @return string|false A valid source size value for use in a 'sizes' attribute or false.
 */
function gc_calculate_image_sizes( $size, $image_src = null, $image_meta = null, $attachment_id = 0 ) {
	$width = 0;
	...
	if ( is_array( $size ) ) {
		...
		if ( is_array( $image_meta ) ) {
			$size_array = _gc_get_image_size_from_meta( $size, $image_meta );
			if ( $size_array ) {
	 ...
	 * @param array|null   $image_meta    The image meta data as returned by gc_get_attachment_metadata() or null.
	 * @param int          $attachment_id Image attachment ID of the original image or 0.
	 */
	return apply_filters( 'gc_calculate_image_sizes', $sizes, $size, $image_src, $image_meta, $attachment_id );
}
 ...
 */
function gc_image_file_matches_image_meta( $image_location, $image_meta, $attachment_id = 0 ) {
	$match = false;
	 ...
	 * @param int    $attachment_id  The image attachment ID or 0 if not supplied.
	 */
	return apply_filters( 'gc_image_file_matches_image_meta', $match, $image_location, $image_meta, $attachment_id );
}
 ...
 */
function gc_image_src_get_dimensions( $image_src, $image_meta, $attachment_id = 0 ) {
	$dimensions = false;
	...
	if ( ! $dimensions && ! empty( $image_meta['sizes'] ) ) {
		$src_filename = gc_basename( $image_src );
	 ...
	 */
	return apply_filters( 'gc_image_src_get_dimensions', $dimensions, $image_src, $image_meta, $attachment_id );
}
 ...
 */
function gc_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id ) {
	...
	if ( $width && $height ) {
		...
		$size_array = gc_image_src_get_dimensions( $image_src, $image_meta, $attachment_id );
		if ( ! $size_array ) {
			return $image;
		}
	}

	$srcset = gc_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
	...
	if ( $srcset ) {
		...
		if ( ! $sizes ) {
			$sizes = gc_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
		}
	}
...
function gc_lazy_loading_enabled( $tag_name, $context ) {
	 ...
	 *                         or the function name from where this was called.
	 */
	return (bool) apply_filters( 'gc_lazy_loading_enabled', $default, $tag_name, $context );
}
 ...
 */
function gc_filter_content_tags( $content, $context = null ) {
	if ( null === $context ) {
	...
	foreach ( $matches as $match ) {
		...
		switch ( $tag_name ) {
			case 'img':
				if ( preg_match( '/gc-image-([0-9]+)/i', $tag, $class_id ) ) {
	...
	foreach ( $matches as $match ) {
		...
		if ( isset( $images[ $match[0] ] ) ) {
			 ...
			 */
			$filtered_image = apply_filters( 'gc_content_img_tag', $filtered_image, $context, $attachment_id );
 ...
 */
function gc_img_tag_add_width_and_height_attr( $image, $context, $attachment_id ) {
	$image_src         = preg_match( '/src="([^"]+)"/', $image, $match_src ) ? $match_src[1] : '';
	list( $image_src ) = explode( '?', $image_src );
	 ...
	 */
	$add = apply_filters( 'gc_img_tag_add_width_and_height_attr', true, $image, $context, $attachment_id );
 ...
 */
function gc_img_tag_add_srcset_and_sizes_attr( $image, $context, $attachment_id ) {
	/**
	 ...
	 * @param int    $attachment_id The image attachment ID.
	 */
	$add = apply_filters( 'gc_img_tag_add_srcset_and_sizes_attr', true, $image, $context, $attachment_id );

	if ( true === $add ) {
		$image_meta = gc_get_attachment_metadata( $attachment_id );
		return gc_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id );
	}
...
function gc_iframe_tag_add_loading_attr( $iframe, $context ) {
	 ...
	 */
	$value = apply_filters( 'gc_iframe_tag_add_loading_attr', $value, $iframe, $context );
...
/**
 * Adds a 'gc-post-image' class to post thumbnails. Internal use only.
 *
 ...
 */
function _gc_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' gc-post-image';
	return $attr;
}

/**
 * Adds '_gc_post_thumbnail_class_filter' callback to the 'gc_get_attachment_image_attributes'
...
function _gc_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'gc_get_attachment_image_attributes', '_gc_post_thumbnail_class_filter' );
}

/**
 * Removes the '_gc_post_thumbnail_class_filter' callback from the 'gc_get_attachment_image_attributes'
 * filter hook. Internal use only.
 *
 * @ignore
 *
...
function img_caption_shortcode( $attr, $content = '' ) {
	...
	$class = trim( 'gc-caption ' . $atts['align'] . ' ' . $atts['class'] );
	...
	if ( $html5 ) {
		$html = sprintf(
			...
			sprintf(
				'<figcaption %sclass="gc-caption-text">%s</figcaption>',
				$caption_id,
				$atts['caption']
			)
		);
...
function gallery_shortcode( $attr ) {
	...
	if ( is_feed() ) {
		...
		foreach ( $attachments as $att_id => $attachment ) {
			if ( ! empty( $atts['link'] ) ) {
				if ( 'none' === $atts['link'] ) {
					$output .= gc_get_attachment_image( $att_id, $atts['size'], false, $attr );
				} else {
					$output .= gc_get_attachment_link( $att_id, $atts['size'], false );
				}
			} else {
				$output .= gc_get_attachment_link( $att_id, $atts['size'], true );
			}
			$output .= "\n";
		}
		return $output;
	...
	$valid_tags = gc_kses_allowed_html( 'post' );
	...
	if ( apply_filters( 'use_default_gallery_style', ! $html5 ) ) {
		...
		<style{$type_attr}>
			...
			/* see gallery_shortcode() in gc-includes/media.php */
		</style>\n\t\t";
	}
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( ! empty( $atts['link'] ) && 'file' === $atts['link'] ) {
			$image_output = gc_get_attachment_link( $id, $atts['size'], false, false, false, $attr );
		} elseif ( ! empty( $atts['link'] ) && 'none' === $atts['link'] ) {
			$image_output = gc_get_attachment_image( $id, $atts['size'], false, $attr );
		} else {
			$image_output = gc_get_attachment_link( $id, $atts['size'], true, false, false, $attr );
		}
		...
		if ( $captiontag && trim( $attachment->post_excerpt ) ) {
			$output .= "
				<{$captiontag} class='gc-caption-text gallery-caption' id='$selector-$id'>
				" . gctexturize( $attachment->post_excerpt ) . "
				</{$captiontag}>";
		}
...
function gc_underscore_playlist_templates() {
	?>
<script type="text/html" id="tmpl-gc-playlist-current-item">
...
<script type="text/html" id="tmpl-gc-playlist-item">
	<div class="gc-playlist-item">
		<a class="gc-playlist-caption" href="{{ data.src }}">
		...
		<div class="gc-playlist-item-length">{{ data.meta.length_formatted }}</div>
		<# } #>
	</div>
</script>
	<?php
 ...
 */
function gc_playlist_scripts( $type ) {
	gc_enqueue_style( 'gc-mediaelement' );
	gc_enqueue_script( 'gc-playlist' );
	?>
<!--[if lt IE 9]><script>document.createElement('<?php echo esc_js( $type ); ?>');</script><![endif]-->
	<?php
	add_action( 'gc_footer', 'gc_underscore_playlist_templates', 0 );
	add_action( 'admin_footer', 'gc_underscore_playlist_templates', 0 );
}
 ...
 */
function gc_playlist_shortcode( $attr ) {
	global $content_width;
	$post = get_post();
	...
	if ( is_feed() ) {
		$output = "\n";
		foreach ( $attachments as $att_id => $attachment ) {
			$output .= gc_get_attachment_link( $att_id ) . "\n";
		}
		return $output;
	}
	...
	foreach ( $attachments as $attachment ) {
		...
		if ( $atts['images'] ) {
			...
			if ( ! empty( $thumb_id ) ) {
				...
				list( $src, $width, $height ) = gc_get_attachment_image_src( $thumb_id, 'thumbnail' );
...
add_shortcode( 'playlist', 'gc_playlist_shortcode' );
 ...
 */
function gc_mediaelement_fallback( $url ) {
	/**
	 ...
	 */
	return apply_filters( 'gc_mediaelement_fallback', sprintf( '<a href="%1$s">%1$s</a>', esc_url( $url ) ), $url );
}
...
function gc_get_audio_extensions() {
	 ...
	 */
	return apply_filters( 'gc_audio_extensions', array( 'mp3', 'ogg', 'flac', 'm4a', 'wav' ) );
}
 ...
 */
function gc_get_attachment_id3_keys( $attachment, $context = 'display' ) {
	$fields = array(
		'artist' => __( '艺术家' ),
		'album'  => __( '相册' ),
	);
	...
	if ( 'display' === $context ) {
		$fields['genre']            = __( '流派' );
		$fields['year']             = __( '年' );
		$fields['length_formatted'] = _x( '长度', 'video or audio' );
	} elseif ( 'js' === $context ) {
		$fields['bitrate']      = __( '比特率' );
		$fields['bitrate_mode'] = __( '比特率模式' );
	}
	 ...
	 */
	return apply_filters( 'gc_get_attachment_id3_keys', $fields, $attachment, $context );
}
/**
 ...
 *     @type string $class    The 'class' attribute for the `<audio>` element. Default 'gc-audio-shortcode'.
 *     @type string $style    The 'style' attribute for the `<audio>` element. Default 'width: 100%;'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display audio.
 */
function gc_audio_shortcode( $attr, $content = '' ) {
	 ...
	 */
	$override = apply_filters( 'gc_audio_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = gc_get_audio_extensions();
	$defaults_atts = array(
		'src'      => '',
		'loop'     => '',
		'autoplay' => '',
	...
	if ( ! empty( $atts['src'] ) ) {
		...
		if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
			return sprintf( '<a class="gc-embedded-audio" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
		}
		...
		foreach ( $default_types as $ext ) {
			if ( ! empty( $atts[ $ext ] ) ) {
				$type = gc_check_filetype( $atts[ $ext ], gc_get_mime_types() );
	...
	if ( ! $primary ) {
		...
		$audio       = reset( $audios );
		$atts['src'] = gc_get_attachment_url( $audio->ID );
	 ...
	 */
	$library = apply_filters( 'gc_audio_shortcode_library', 'mediaelement' );
	 ...
	 */
	$atts['class'] = apply_filters( 'gc_audio_shortcode_class', $atts['class'], $atts );
	...
	foreach ( $default_types as $fallback ) {
		if ( ! empty( $atts[ $fallback ] ) ) {
			...
			$type  = gc_check_filetype( $atts[ $fallback ], gc_get_mime_types() );
	...
	if ( 'mediaelement' === $library ) {
		$html .= gc_mediaelement_fallback( $fileurl );
	}
	 ...
	 * @param string $library Media library used for the audio shortcode.
	 */
	return apply_filters( 'gc_audio_shortcode', $html, $atts, $audio, $post_id, $library );
}
add_shortcode( 'audio', 'gc_audio_shortcode' );
...
function gc_get_video_extensions() {
	 ...
	 */
	return apply_filters( 'gc_video_extensions', array( 'mp4', 'm4v', 'webm', 'ogv', 'flv' ) );
}
 ...
 *                            Default 'gc-video-shortcode'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display video.
 */
function gc_video_shortcode( $attr, $content = '' ) {
	global $content_width;
	$post_id = get_post() ? get_the_ID() : 0;
	 ...
	 *
	 * @param string $html     Empty variable to be replaced with shortcode markup.
	 * @param array  $attr     Attributes of the shortcode. @see gc_video_shortcode()
	 * @param string $content  Video shortcode content.
	 * @param int    $instance Unique numeric ID of this video shortcode instance.
	 */
	$override = apply_filters( 'gc_video_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = gc_get_video_extensions();
	$defaults_atts = array(
		'src'      => '',
		'poster'   => '',
		'loop'     => '',
	...
	if ( ! empty( $atts['src'] ) ) {
		...
		if ( ! $is_youtube && ! $is_vimeo ) {
			...
			if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
				return sprintf( '<a class="gc-embedded-video" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
			}
		}
	...
	if ( ! $primary ) {
		...
		$video       = reset( $videos );
		$atts['src'] = gc_get_attachment_url( $video->ID );
		if ( empty( $atts['src'] ) ) {
			return;
		}
	 ...
	 */
	$library = apply_filters( 'gc_video_shortcode_library', 'mediaelement' );
	if ( 'mediaelement' === $library && did_action( 'init' ) ) {
		gc_enqueue_style( 'gc-mediaelement' );
		gc_enqueue_script( 'gc-mediaelement' );
		gc_enqueue_script( 'mediaelement-vimeo' );
	...
	if ( 'mediaelement' === $library ) {
		if ( $is_youtube ) {
			...
			$parsed_vimeo_url = gc_parse_url( $atts['src'] );
	 ...
	 */
	$atts['class'] = apply_filters( 'gc_video_shortcode_class', $atts['class'], $atts );
	...
	$output = sprintf( '<div style="%s" class="gc-video">%s</div>', $width_rule, $html );
	 ...
	 * @param string $library Media library used for the video shortcode.
	 */
	return apply_filters( 'gc_video_shortcode', $output, $atts, $video, $post_id, $library );
}
add_shortcode( 'video', 'gc_video_shortcode' );
...
function get_adjacent_image_link( $prev = true, $size = 'thumbnail', $text = false ) {
	...
	if ( $attachments ) {
		...
		if ( isset( $attachments[ $k ] ) ) {
			...
			$output        = gc_get_attachment_link( $attachment_id, $size, true, false, $text, $attr );
		}
	}
 ...
 *                                     Default is 'names'.
 * @return string[]|GC_Taxonomy[] List of taxonomies or taxonomy names. Empty array on failure.
 */
 ...
 *                       Default 'names'.
 * @return string[]|GC_Taxonomy[] Array of names or objects of registered taxonomies for attachments.
 */
 ...
 *                                False on failure.
 */
function gc_imagecreatetruecolor( $width, $height ) {
 ...
 * }
 */
function gc_expand_dimensions( $example_width, $example_height, $max_width, $max_height ) {
	$example_width  = (int) $example_width;
	$example_height = (int) $example_height;
	$max_width      = (int) $max_width;
	$max_height     = (int) $max_height;

	return gc_constrain_dimensions( $example_width * 1000000, $example_height * 1000000, $max_width, $max_height );
}
...
function gc_max_upload_size() {
	$u_bytes = gc_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = gc_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
 ...
 *                     Default empty array.
 * @return GC_Image_Editor|GC_Error The GC_Image_Editor object on success,
...
function gc_get_image_editor( $path, $args = array() ) {
	...
	$implementation = _gc_image_editor_choose( $args );

	if ( $implementation ) {
		$editor = new $implementation( $path );
		$loaded = $editor->load();

		if ( is_gc_error( $loaded ) ) {
			return $loaded;
		}
	...
	return new GC_Error( 'image_no_editor', __( '无可选编辑器。' ) );
}
 ...
 */
function gc_image_editor_supports( $args = array() ) {
	return (bool) _gc_image_editor_choose( $args );
}
 ...
 */
function _gc_image_editor_choose( $args = array() ) {
...
function gc_plupload_default_settings() {
	...
	$params['_gcnonce'] = gc_create_nonce( 'media-form' );
	...
	$script = 'var _gcPluploadSettings = ' . gc_json_encode( $settings ) . ';';

	if ( $data ) {
		$script = "$data\n$script";
	}

	$gc_scripts->add_data( 'gc-plupload', 'data', $script );
}
 ...
 *     @type string $url                   Direct URL to the attachment file (from gc-content).
 *     @type int    $width                 If the attachment is an image, represents the width of the image in pixels.
 * }
 *
 */
function gc_prepare_attachment_for_js( $attachment ) {
	$attachment = get_post( $attachment );
	...
	$attachment_url = gc_get_attachment_url( $attachment->ID );
	$base_url       = str_replace( gc_basename( $attachment_url ), '', $attachment_url );

	$response = array(
		'id'            => $attachment->ID,
		'title'         => $attachment->post_title,
		'filename'      => gc_basename( get_attached_file( $attachment->ID ) ),
		'url'           => $attachment_url,
		'link'          => get_attachment_link( $attachment->ID ),
		'alt'           => get_post_meta( $attachment->ID, '_gc_attachment_image_alt', true ),
		...
		'type'          => $type,
		'subtype'       => $subtype,
		'icon'          => gc_mime_type_icon( $attachment->ID ),
		'dateFormatted' => mysql2date( __( 'Y年n月j日' ), $attachment->post_date ),
		'nonces'        => array(
			'update' => false,
			'delete' => false,
			'edit'   => false,
	...
	$author = new GC_User( $attachment->post_author );
	...
	if ( $author->exists() ) {
		...
		$response['authorName'] = __( '（无作者）' );
	}

	if ( $attachment->post_parent ) {
		$post_parent = get_post( $attachment->post_parent );
		if ( $post_parent ) {
			$response['uploadedToTitle'] = $post_parent->post_title ? $post_parent->post_title : __( '（无标题）' );
	...
	if ( isset( $meta['filesize'] ) ) {
		...
		$bytes = gc_filesize( $attached_file );
	} else {
		$bytes = '';
	}
	...
	$context             = get_post_meta( $attachment->ID, '_gc_attachment_context', true );
	$response['context'] = ( $context ) ? $context : '';

	if ( current_user_can( 'edit_post', $attachment->ID ) ) {
		$response['nonces']['update'] = gc_create_nonce( 'update-post_' . $attachment->ID );
		$response['nonces']['edit']   = gc_create_nonce( 'image_editor-' . $attachment->ID );
		$response['editLink']         = get_edit_post_link( $attachment->ID, 'raw' );
	}

	if ( current_user_can( 'delete_post', $attachment->ID ) ) {
		$response['nonces']['delete'] = gc_create_nonce( 'delete-post_' . $attachment->ID );
	}
	...
	if ( $meta && ( 'image' === $type || ! empty( $meta['sizes'] ) ) ) {
		...
		if ( 'image' === $type ) {
			if ( ! empty( $meta['original_image'] ) ) {
				$response['originalImageURL']  = gc_get_original_image_url( $attachment->ID );
				$response['originalImageName'] = gc_basename( gc_get_original_image_path( $attachment->ID ) );
			}
	...
	if ( $meta && ( 'audio' === $type || 'video' === $type ) ) {
		...
		$response['meta'] = array();
		foreach ( gc_get_attachment_id3_keys( $attachment, 'js' ) as $key => $label ) {
		...
		if ( ! empty( $id ) ) {
			list( $src, $width, $height ) = gc_get_attachment_image_src( $id, 'full' );
			$response['image']            = compact( 'src', 'width', 'height' );
			list( $src, $width, $height ) = gc_get_attachment_image_src( $id, 'thumbnail' );
	 ...
	 */
	return apply_filters( 'gc_prepare_attachment_for_js', $response, $attachment, $meta );
}
...
function gc_enqueue_media( $args = array() ) {
	...
	if ( did_action( 'gc_enqueue_media' ) ) {
		return;
	}

	global $content_width, $gcdb, $gc_locale;
	...
	$exts      = array_merge( gc_get_audio_extensions(), gc_get_video_extensions() );
	...
	foreach ( $months as $month_year ) {
		$month_year->text = sprintf(
			...
			__( '%2$d年%1$s' ),
			$gc_locale->get_month( $month_year->month ),
			$month_year->year
		);
	}
	...
	$settings = array(
		...
		'mimeTypes'         => gc_list_pluck( get_post_mime_types(), 0 ),
		/** This filter is documented in gc-admin/includes/media.php */
	...
	if ( isset( $args['post'] ) ) {
		...
		$settings['post'] = array(
			'id'    => $post->ID,
			'nonce' => gc_create_nonce( 'update-post_' . $post->ID ),
		);
	...
	$strings = array(
		 ...
		 */
		'selected'                    => __( '已选%d个' ),
		'dragInfo'                    => __( '拖放来重新排序媒体文件。' ),

		// Upload.
		'uploadFilesTitle'            => __( '上传文件' ),
		'uploadImagesTitle'           => __( '上传图片' ),

		// Library.
		'mediaLibraryTitle'           => __( '媒体库' ),
		'insertMediaTitle'            => __( '添加媒体' ),
		'createNewGallery'            => __( '创建新相册' ),
		'createNewPlaylist'           => __( '创建新的播放列表' ),
		'createNewVideoPlaylist'      => __( '创建新的视频播放列表' ),
		'returnToLibrary'             => __( '&#8592; 转到“媒体库”页面' ),
		'allMediaItems'               => __( '所有多媒体项目' ),
		'allDates'                    => __( '全部日期' ),
		'noItemsFound'                => __( '找不到条目。' ),
		'insertIntoPost'              => $post_type_object->labels->insert_into_item,
		'unattached'                  => _x( '独立', 'media items' ),
		...
		'noMedia'                     => __( '未找到媒体项目。' ),
		'noMediaTryNewSearch'         => __( '未找到媒体项目，请重新搜索。' ),

		// Library Details.
		'attachmentDetails'           => __( '附件详情' ),

		// From URL.
		'insertFromUrlTitle'          => __( '从URL插入' ),
		...
		// Gallery.
		'createGalleryTitle'          => __( '创建相册' ),
		'editGalleryTitle'            => __( '编辑相册' ),
		'cancelGalleryTitle'          => __( '&#8592; 取消相册' ),
		'insertGallery'               => __( '插入相册' ),
		'updateGallery'               => __( '更新相册' ),
		'addToGallery'                => __( '添加至相册' ),
		'addToGalleryTitle'           => __( '添加至相册' ),
		'reverseOrder'                => __( '反转顺序' ),

		// Edit Image.
		'imageDetailsTitle'           => __( '图片详情' ),
		'imageReplaceTitle'           => __( '更换图片' ),
		'imageDetailsCancel'          => __( '取消编辑' ),
		'editImage'                   => __( '编辑图片' ),

		// Crop Image.
		'chooseImage'                 => __( '选择图片' ),
		'selectAndCrop'               => __( '选择并裁剪' ),
		'skipCropping'                => __( '跳过裁剪' ),
		'cropImage'                   => __( '裁剪图片' ),
		'cropYourImage'               => __( '裁剪您的图片' ),
		'cropping'                    => __( '裁剪中&hellip;' ),
		/* translators: 1: Suggested width number, 2: Suggested height number. */
		'suggestedDimensions'         => __( '建议的图片尺寸：%1$s x %2$s像素。' ),
		'cropError'                   => __( '裁剪您的图片时发生错误。' ),

		// Edit Audio.
		'audioDetailsTitle'           => __( '音频详情' ),
		'audioReplaceTitle'           => __( '替换音频' ),
		'audioAddSourceTitle'         => __( '添加音频源' ),
		'audioDetailsCancel'          => __( '取消编辑' ),

		// Edit Video.
		'videoDetailsTitle'           => __( '视频详情' ),
		'videoReplaceTitle'           => __( '替换视频' ),
		'videoAddSourceTitle'         => __( '添加视频源' ),
		'videoDetailsCancel'          => __( '取消编辑' ),
		'videoSelectPosterImageTitle' => __( '选择海报图片' ),
		'videoAddTrackTitle'          => __( '添加字幕' ),

		// Playlist.
		'playlistDragInfo'            => __( '拖放来排序曲目。' ),
		'createPlaylistTitle'         => __( '创建音频播放列表' ),
		'editPlaylistTitle'           => __( '编辑音频播放列表' ),
		'cancelPlaylistTitle'         => __( '&#8592; 取消音频播放列表' ),
		'insertPlaylist'              => __( '插入音频播放列表' ),
		'updatePlaylist'              => __( '更新音频播放列表' ),
		'addToPlaylist'               => __( '添加至音频播放列表' ),
		'addToPlaylistTitle'          => __( '添加至音频播放列表' ),

		// Video Playlist.
		'videoPlaylistDragInfo'       => __( '拖放来排序视频。' ),
		'createVideoPlaylistTitle'    => __( '创建视频播放列表' ),
		'editVideoPlaylistTitle'      => __( '编辑视频播放列表' ),
		'cancelVideoPlaylistTitle'    => __( '&#8592; 取消视频播放列表' ),
		'insertVideoPlaylist'         => __( '插入视频播放列表' ),
		'updateVideoPlaylist'         => __( '更新视频播放列表' ),
		'addToVideoPlaylist'          => __( '添加至视频播放列表' ),
		'addToVideoPlaylistTitle'     => __( '添加至视频播放列表' ),

		// Headings.
		'filterAttachments'           => __( '筛选媒体' ),
		'attachmentsList'             => __( '媒体列表' ),
	);
	...
	gc_enqueue_script( 'media-audiovideo' );
	gc_enqueue_style( 'media-views' );
	if ( is_admin() ) {
		gc_enqueue_script( 'mce-view' );
		gc_enqueue_script( 'image-edit' );
	}
	gc_enqueue_style( 'imgareaselect' );
	gc_plupload_default_settings();

	require_once ABSPATH . GCINC . '/media-template.php';
	add_action( 'admin_footer', 'gc_print_media_templates' );
	add_action( 'gc_footer', 'gc_print_media_templates' );
	add_action( 'customize_controls_print_footer_scripts', 'gc_print_media_templates' );
 ...
 * @return GC_Post[] Array of media attached to the given post.
 */
	...
	if ( has_block( 'gallery', $post->post_content ) ) {
		...
		while ( $block = array_shift( $post_blocks ) ) {
			...
			if ( $has_inner_blocks && $html ) {
				$block_html  = gc_list_pluck( $block['innerBlocks'], 'innerHTML' );
			...
			if ( $has_inner_blocks ) {
				$attrs = gc_list_pluck( $block['innerBlocks'], 'attrs' );
				$ids   = gc_list_pluck( $attrs, 'id' );
 ...
 * @param GC_Post $attachment Attachment object.
 */
function gc_maybe_generate_attachment_metadata( $attachment ) {
	...
	if ( empty( $meta ) && file_exists( $file ) ) {
		...
		$_lock = 'gc_generating_att_' . $attachment_id;

		if ( ! array_key_exists( '_gc_attachment_metadata', $_meta ) && ! get_transient( $_lock ) ) {
...
function attachment_url_to_postid( $url ) {
	...
	$sql = $gcdb->prepare(
		"SELECT post_id, meta_value FROM $gcdb->postmeta WHERE meta_key = '_gc_attached_file' AND meta_value = %s",
		$path
	);

	$results = $gcdb->get_results( $sql );
	$post_id = null;
 ...
 */
function gc_register_media_personal_data_exporter( $exporters ) {
	$exporters['gechiui-media'] = array(
		'exporter_friendly_name' => __( 'GeChiUI媒体' ),
		'callback'               => 'gc_media_personal_data_exporter',
	);
 ...
 */
function gc_media_personal_data_exporter( $email_address, $page = 1 ) {
 ...
 */
function gc_show_heic_upload_error( $plupload_settings ) {
 ...
 */
function gc_getimagesize( $filename, array &$image_info = null ) {
	...
	if ( defined( 'GC_DEBUG' ) && GC_DEBUG
		 ...
		 *
		 * See https://core.trac.gechiui.com/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
 ...
 * }
 */
function gc_get_webp_info( $filename ) {
	$width  = false;
	$height = false;
	$type   = false;
 ...
 */
function gc_omit_loading_attr_threshold( $force = false ) {
 ...
 */
function gc_increase_content_media_count( $amount = 1 ) {
