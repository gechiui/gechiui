<?php
/**
 * Sitemaps: GC_Sitemaps_Posts class
 *
...
class GC_Sitemaps_Posts extends GC_Sitemaps_Provider {
	...
	public function get_object_subtypes() {
		 ...
		 */
		return apply_filters( 'gc_sitemaps_post_types', $post_types );
	}
	...
	public function get_url_list( $page_num, $object_subtype = '' ) {
		...
		$url_list = apply_filters(
			'gc_sitemaps_posts_pre_url_list',
			null,
			$post_type,
			$page_num
		);
		...
		if ( 'page' === $post_type && 1 === $page_num && 'posts' === get_option( 'show_on_front' ) ) {
			 ...
			 */
			$sitemap_entry = apply_filters( 'gc_sitemaps_posts_show_on_front_entry', $sitemap_entry );
		...
		foreach ( $query->posts as $post ) {
			 ...
			 */
			$sitemap_entry = apply_filters( 'gc_sitemaps_posts_entry', $sitemap_entry, $post, $post_type );
			$url_list[]    = $sitemap_entry;
		}
	...
	public function get_max_num_pages( $object_subtype = '' ) {
		 ...
		 * @param string   $post_type     Post type name.
		 */
		$max_num_pages = apply_filters( 'gc_sitemaps_posts_pre_max_num_pages', null, $post_type );
	...
	protected function get_posts_query_args( $post_type ) {
		 ...
		 */
		$args = apply_filters(
			'gc_sitemaps_posts_query_args',
			array(
				'orderby'                => 'ID',
				'order'                  => 'ASC',
				'post_type'              => $post_type,
				'posts_per_page'         => gc_sitemaps_get_max_urls( $this->object_type ),
