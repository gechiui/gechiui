...
function register_block_script_handle( $metadata, $field_name ) {
	...
	if ( ! empty( $metadata['textdomain'] ) && in_array( 'gc-i18n', $script_dependencies, true ) ) {
		gc_set_script_translations( $script_handle, $metadata['textdomain'] );
	}
...
function get_block_metadata_i18n_schema() {
	...
	if ( ! isset( $i18n_block_schema ) ) {
		$i18n_block_schema = gc_json_file_decode( __DIR__ . '/block-i18n.json' );
	}
 ...
 *                               of `GC_Block_Type`. See GC_Block_Type::__construct() for information
...
function register_block_type_from_metadata( $file_or_folder, $args = array() ) {
	...
	return GC_Block_Type_Registry::get_instance()->register(
		$metadata['name'],
		$settings
	);
}
 ...
 *                                         or a complete GC_Block_Type instance.
 *                                         In case a GC_Block_Type is provided, the $args parameter will be ignored.
 * @param array                $args       Optional. Array of block type arguments. Accepts any public property
 *                                         of `GC_Block_Type`. See GC_Block_Type::__construct() for information
 *                                         on accepted arguments. Default empty array.
 *
 ...
 * @return GC_Block_Type|false The unregistered block type on success, or false on failure.
 */
function unregister_block_type( $name ) {
	return GC_Block_Type_Registry::get_instance()->unregister( $name );
}
 ...
 * @see parse_blocks()
 *
 * @param int|string|GC_Post|null $post Optional. Post content, post ID, or post object.
...
function serialize_block_attributes( $block_attributes ) {
	$encoded_attributes = gc_json_encode( $block_attributes, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function filter_block_kses_value( $value, $allowed_html, $allowed_protocols = array() ) {
	if ( is_array( $value ) ) {
		...
		return gc_kses( $value, $allowed_html, $allowed_protocols );
	}
...
function render_block( $parsed_block ) {
	 ...
	 * @param GC_Block|null $parent_block If this is a nested block, a reference to the parent block.
	 */
	...
	$block = new GC_Block( $parsed_block, $context );
 ...
 */
function _restore_gcautop_hook( $content ) {
	$current_priority = has_filter( 'the_content', '_restore_gcautop_hook' );

	add_filter( 'the_content', 'gcautop', $current_priority - 1 );
	remove_filter( 'the_content', '_restore_gcautop_hook', $current_priority );
...
function unregister_block_style( $block_name, $block_style_name ) {
	return GC_Block_Styles_Registry::get_instance()->unregister( $block_name, $block_style_name );
}
 ...
 */
function gc_migrate_old_typography_shape( $metadata ) {
	...
	foreach ( $typography_keys as $typography_key ) {
		$support_for_key = _gc_array_get( $metadata['supports'], array( $typography_key ), null );
		...
		if ( null !== $support_for_key ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					__( '区块“%1$s”正在 %4$s 下的 %3$s 文件中声明 %2$s 支持。现在，%2$s 支持被声明在 %5$s 下。' ),
					$metadata['name'],
			...
			_gc_array_set( $metadata['supports'], array( 'typography', $typography_key ), $support_for_key );
...
/**
 * Helper function that constructs a GC_Query args array from
 ...
 *
 * @return array Returns the constructed GC_Query arguments.
 */
