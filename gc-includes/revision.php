...
function _gc_post_revision_fields( $post = array(), $deprecated = false ) {
	...
	// GC uses these internally either in versioning or elsewhere - they cannot be versioned.
 ...
 * @access private
 *
 * @param array|GC_Post $post     Optional. A post array or a GC_Post object to be processed
 *                                for insertion as a post revision. Default empty array.
 * @param bool          $autosave Optional. Is the revision an autosave? Default false.
 * @return array Post array ready to be inserted as a post revision.
 */
function _gc_post_revision_data( $post = array(), $autosave = false ) {
	if ( ! is_array( $post ) ) {
		$post = get_post( $post, ARRAY_A );
	}

	$fields = _gc_post_revision_fields( $post );
 ...
 */
function gc_save_post_revision( $post_id ) {
	...
	for ( $i = 0; isset( $revisions[ $i ] ); $i++ ) {
		...
		gc_delete_post_revision( $revisions[ $i ]->ID );
	}
...
function _gc_put_post_revision( $post = null, $autosave = false ) {
	...
	if ( isset( $post['post_type'] ) && 'revision' === $post['post_type'] ) {
		return new GC_Error( 'post_type', __( '无法从一个修订版本上创建新的修订' ) );
	}

	$post = _gc_post_revision_data( $post, $autosave );
	$post = gc_slash( $post ); // Since data is from DB.

	$revision_id = gc_insert_post( $post, true );
	if ( is_gc_error( $revision_id ) ) {
		return $revision_id;
	}
	...
	if ( $revision_id ) {
		 ...
		 */
		do_action( '_gc_put_post_revision', $revision_id );
	}
...
function gc_restore_post_revision( $revision, $fields = null ) {
	...
	if ( ! is_array( $fields ) ) {
		$fields = array_keys( _gc_post_revision_fields( $revision ) );
	}
	 ...
	 * @param int $revision_id Post revision ID.
	 */
	do_action( 'gc_restore_post_revision', $post_id, $revision['ID'] );
...
function gc_delete_post_revision( $revision ) {
	...
	if ( $delete ) {
		 ...
		 */
		do_action( 'gc_delete_post_revision', $revision->ID, $revision );
	}
...
function gc_get_post_revisions( $post = 0, $args = null ) {
	...
	if ( $args['check_enabled'] && ! gc_revisions_enabled( $post ) ) {
		return array();
	}
 ...
 */
function gc_revisions_enabled( $post ) {
	return gc_revisions_to_keep( $post ) !== 0;
}
 ...
 */
function gc_revisions_to_keep( $post ) {
	$num = GC_POST_REVISIONS;
	 ...
	 */
	$num = apply_filters( 'gc_revisions_to_keep', $num, $post );

	/**
	 * Filters the number of revisions to save for the given post by its post type.
	 *
	 * Overrides both the value of GC_POST_REVISIONS and the {@see 'gc_revisions_to_keep'} filter.
	 *
	 ...
	 * @param GC_Post $post Post object.
	 */
	$num = apply_filters( "gc_{$post->post_type}_revisions_to_keep", $num, $post );
...
function _set_preview( $post ) {
	...
	$preview = gc_get_post_autosave( $post->ID );
	...
	add_filter( 'get_the_terms', '_gc_preview_terms_filter', 10, 3 );
	add_filter( 'get_post_metadata', '_gc_preview_post_thumbnail_filter', 10, 3 );
...
function _show_post_preview() {
	if ( isset( $_GET['preview_id'] ) && isset( $_GET['preview_nonce'] ) ) {
		...
		if ( false === gc_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . $id ) ) {
			gc_die( __( '抱歉，您不能预览草稿。' ), 403 );
		}
 ...
 * @access private
 *
 * @param GC_Post $revision
 * @return int|false
 */
function _gc_get_post_revision_version( $revision ) {
...
function _gc_upgrade_revisions_of_post( $post, $revisions ) {
	...
	do {
		...
		$this_revision_version = _gc_get_post_revision_version( $this_revision );
		 ...
		 */
		if ( $prev_revision ) {
			$prev_revision_version = _gc_get_post_revision_version( $prev_revision );
		...
		$result = $gcdb->update( $gcdb->posts, $update, array( 'ID' => $this_revision->ID ) );

		if ( $result ) {
			gc_cache_delete( $this_revision->ID, 'posts' );
		}
	} while ( $prev_revision );
