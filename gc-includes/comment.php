 ...
 *
 * @param GC_Comment|string|int $comment Comment to retrieve.
 * @param string                $output  Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                       correspond to a GC_Comment object, an associative array, or a numeric array,
 *                                       respectively. Default OBJECT.
 * @return GC_Comment|array|null Depends on $output value.
 */
...
function get_comment_statuses() {
	$status = array(
		'hold'    => __( '驳回' ),
		'approve' => _x( '已批准', 'comment status' ),
		'spam'    => _x( '垃圾', 'comment status' ),
		'trash'   => _x( '回收站', 'comment status' ),
	);
 ...
 */
function gc_set_comment_cookies( $comment, $user, $cookies_consent = true ) {
...
function sanitize_comment_cookies() {
	if ( isset( $_COOKIE[ 'comment_author_' . COOKIEHASH ] ) ) {
		...
		$comment_author = gc_unslash( $comment_author );
	...
	if ( isset( $_COOKIE[ 'comment_author_email_' . COOKIEHASH ] ) ) {
		...
		$comment_author_email = gc_unslash( $comment_author_email );
	...
	if ( isset( $_COOKIE[ 'comment_author_url_' . COOKIEHASH ] ) ) {
		...
		$comment_author_url = gc_unslash( $comment_author_url );
 ...
 * @param bool  $gc_error    When true, a disallowed comment will result in the function
 *                           returning a GC_Error object, rather than executing gc_die().
 *                           Default false.
 * @return int|string|GC_Error Allowed comments return the approval status (0|1|'spam'|'trash').
 *                             If `$gc_error` is true, disallowed comments return a GC_Error.
 */
function gc_allow_comment( $commentdata, $gc_error = false ) {
	...
	$dupe = $gcdb->prepare(
		...
		gc_unslash( $commentdata['comment_parent'] ),
		gc_unslash( $commentdata['comment_author'] )
	);
	if ( $commentdata['comment_author_email'] ) {
		$dupe .= $gcdb->prepare(
			'AND comment_author_email = %s ',
			gc_unslash( $commentdata['comment_author_email'] )
		);
	}
	$dupe .= $gcdb->prepare(
		') AND comment_content = %s LIMIT 1',
		gc_unslash( $commentdata['comment_content'] )
	);

	$dupe_id = $gcdb->get_var( $dupe );
	...
	if ( $dupe_id ) {
		 ...
		 */
		$comment_duplicate_message = apply_filters( 'comment_duplicate_message', __( '检测到重复评论，您似乎已经提交过这条评论了！' ) );

		if ( $gc_error ) {
			return new GC_Error( 'comment_duplicate', $comment_duplicate_message, 409 );
		} else {
			if ( gc_doing_ajax() ) {
				die( $comment_duplicate_message );
			}

			gc_die( $comment_duplicate_message, 409 );
		}
	}
	 ...
	 *                                     gc_die() or die() if a comment flood is occurring.
	 */
	do_action(
		'check_comment_flood',
	...
	if ( $is_flood ) {
		/** This filter is documented in gc-includes/comment-template.php */
		$comment_flood_message = apply_filters( 'comment_flood_message', __( '您提交评论的速度太快了，请稍后再发表评论。' ) );

		return new GC_Error( 'comment_flood', $comment_flood_message, 429 );
	}

	if ( ! empty( $commentdata['user_id'] ) ) {
		$user        = get_userdata( $commentdata['user_id'] );
		$post_author = $gcdb->get_var(
			$gcdb->prepare(
				"SELECT post_author FROM $gcdb->posts WHERE ID = %d LIMIT 1",
	...
	if ( isset( $user ) && ( $commentdata['user_id'] == $post_author || $user->has_cap( 'moderate_comments' ) ) ) {
		...
		if ( gc_check_comment_disallowed_list(
			$commentdata['comment_author'],
			$commentdata['comment_author_email'],
			$commentdata['comment_author_url'],
			$commentdata['comment_content'],
 ...
 *                          returning without executing gc_die() or die(). Default false.
...
function gc_check_comment_flood( $is_flood, $ip, $email, $date, $avoid_die = false ) {
	...
	$sql = $gcdb->prepare(
		"SELECT `comment_date_gmt` FROM `$gcdb->comments` WHERE `comment_date_gmt` >= %s AND ( $check_column = %s OR `comment_author_email` = %s ) ORDER BY `comment_date_gmt` DESC LIMIT 1",
		$hour_ago,
		$user,
		$email
	);

	$lasttime = $gcdb->get_var( $sql );
	...
	if ( $lasttime ) {
		...
		if ( $flood_die ) {
			...
			if ( $avoid_die ) {
				...
				gc_die( $comment_flood_message, 429 );
			}
		}
	}
...
function get_page_of_comment( $comment_id, $args = array() ) {
	...
	if ( null === $page ) {
		...
		$comment_args = array(
			...
			'date_query' => array(
				array(
					'column' => "$gcdb->comments.comment_date_gmt",
		 ...
		 *     @type array  $date_query         Date query clauses to limit comments by. See GC_Date_Query.
		...
		$comment_query       = new GC_Comment_Query();
 ...
 */
function gc_get_comment_fields_max_lengths() {
	global $gcdb;
	...
	if ( $gcdb->is_mysql ) {
		foreach ( $lengths as $column => $length ) {
			$col_length = $gcdb->get_col_length( $gcdb->comments, $column );
			$max_length = 0;

			// No point if we can't get the DB column lengths.
			if ( is_gc_error( $col_length ) ) {
				break;
			}
	 ...
	 */
	return apply_filters( 'gc_get_comment_fields_max_lengths', $lengths );
}
 ...
 *                       otherwise true.
 */
function gc_check_comment_data_max_lengths( $comment_data ) {
 ...
 */
function gc_check_comment_disallowed_list( $author, $email, $url, $comment, $user_ip, $user_agent ) {
	/**
	...
	do_action_deprecated(
		...
		'5.5.0',
		'gc_check_comment_disallowed_list',
		__( '请考虑编写更具包容性的代码。' )
	);
	 ...
	 * @param string $user_agent Comment author's browser user agent.
	 */
	do_action( 'gc_check_comment_disallowed_list', $author, $email, $url, $comment, $user_ip, $user_agent );
	...
	$comment_without_html = gc_strip_all_tags( $comment );
 ...
 * }
 */
function gc_count_comments( $post_id = 0 ) {
	$post_id = (int) $post_id;
	 ...
	 */
	$filtered = apply_filters( 'gc_count_comments', array(), $post_id );
	if ( ! empty( $filtered ) ) {
		return $filtered;
	}

	$count = gc_cache_get( "comments-{$post_id}", 'counts' );
	if ( false !== $count ) {
		return $count;
	}
	...
	gc_cache_set( "comments-{$post_id}", $stats_object, 'counts' );
...
function gc_delete_comment( $comment_id, $force_delete = false ) {
	...
	if ( ! $force_delete && EMPTY_TRASH_DAYS && ! in_array( gc_get_comment_status( $comment ), array( 'trash', 'spam' ), true ) ) {
		return gc_trash_comment( $comment_id );
	}
	...
	$children = $gcdb->get_col( $gcdb->prepare( "SELECT comment_ID FROM $gcdb->comments WHERE comment_parent = %d", $comment->comment_ID ) );
	if ( ! empty( $children ) ) {
		$gcdb->update( $gcdb->comments, array( 'comment_parent' => $comment->comment_parent ), array( 'comment_parent' => $comment->comment_ID ) );
		clean_comment_cache( $children );
	}

	// Delete metadata.
	$meta_ids = $gcdb->get_col( $gcdb->prepare( "SELECT meta_id FROM $gcdb->commentmeta WHERE comment_id = %d", $comment->comment_ID ) );
	foreach ( $meta_ids as $mid ) {
		delete_metadata_by_mid( 'comment', $mid );
	}

	if ( ! $gcdb->delete( $gcdb->comments, array( 'comment_ID' => $comment->comment_ID ) ) ) {
		return false;
	}
	...
	if ( $post_id && 1 == $comment->comment_approved ) {
		gc_update_comment_count( $post_id );
	}

	clean_comment_cache( $comment->comment_ID );

	/** This action is documented in gc-includes/comment.php */
	do_action( 'gc_set_comment_status', $comment->comment_ID, 'delete' );

	gc_transition_comment_status( 'delete', $comment->comment_approved, $comment );
 ...
 * @return bool True on success, false on failure.
 */
function gc_trash_comment( $comment_id ) {
	if ( ! EMPTY_TRASH_DAYS ) {
		return gc_delete_comment( $comment_id, true );
	}
	...
	if ( gc_set_comment_status( $comment, 'trash' ) ) {
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_status' );
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_time' );
		add_comment_meta( $comment->comment_ID, '_gc_trash_meta_status', $comment->comment_approved );
		add_comment_meta( $comment->comment_ID, '_gc_trash_meta_time', time() );
 ...
 * @return bool True on success, false on failure.
 */
function gc_untrash_comment( $comment_id ) {
	$comment = get_comment( $comment_id );
	if ( ! $comment ) {
		return false;
	}
	...
	$status = (string) get_comment_meta( $comment->comment_ID, '_gc_trash_meta_status', true );
	if ( empty( $status ) ) {
		$status = '0';
	}

	if ( gc_set_comment_status( $comment, $status ) ) {
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_time' );
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_status' );
 ...
 * @return bool True on success, false on failure.
 */
function gc_spam_comment( $comment_id ) {
	$comment = get_comment( $comment_id );
	if ( ! $comment ) {
		return false;
	}
	...
	if ( gc_set_comment_status( $comment, 'spam' ) ) {
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_status' );
		delete_comment_meta( $comment->comment_ID, '_gc_trash_meta_time' );
		add_comment_meta( $comment->comment_ID, '_gc_trash_meta_status', $comment->comment_approved );
		add_comment_meta( $comment->comment_ID, '_gc_trash_meta_time', time() );
 ...
 * @return bool True on success, false on failure.
 */
function gc_unspam_comment( $comment_id ) {
	$comment = get_comment( $comment_id );
	if ( ! $comment ) {
		return false;
	}
 ...
 */
function gc_get_comment_status( $comment_id ) {
	$comment = get_comment( $comment_id );
	if ( ! $comment ) {
		return false;
	}
 ...
 * @param GC_Comment $comment    Comment object.
 */
function gc_transition_comment_status( $new_status, $old_status, $comment ) {
	/*
	...
	$comment_statuses = array(
		0         => 'unapproved',
		'hold'    => 'unapproved', // gc_set_comment_status() uses "hold".
		1         => 'approved',
		'approve' => 'approved',   // gc_set_comment_status() uses "approve".
	);
 ...
 * }
 */
function gc_get_current_commenter() {
	 ...
	 * }
	 */
	return apply_filters( 'gc_get_current_commenter', compact( 'comment_author', 'comment_author_email', 'comment_author_url' ) );
}
 ...
 */
function gc_get_unapproved_comment_author_email() {
	$commenter_email = '';
	...
	if ( ! empty( $_GET['unapproved'] ) && ! empty( $_GET['moderation-hash'] ) ) {
		...
		if ( $comment && hash_equals( $_GET['moderation-hash'], gc_hash( $comment->comment_date_gmt ) ) ) {
...
function gc_insert_comment( $commentdata ) {
	 ...
	 * @param GC_Comment $comment Comment object.
	 */
	do_action( 'gc_insert_comment', $id, $comment );
 ...
 */
function gc_filter_comment( $commentdata ) {
 ...
 */
function gc_throttle_comment_flood( $block, $time_lastcomment, $time_newcomment ) {
 ...
 * and whether comment is approved by GeChiUI. Also has {@see 'preprocess_comment'}
 * filter for processing the comment data before the function handles it.
 *
 * We use `REMOTE_ADDR` here directly. If you are behind a proxy, you should ensure
 * that it is properly set, such as in gc-config.php, for your environment.
 *
 ...
 * }
 * @param bool  $gc_error Should errors be returned as GC_Error objects instead of
 *                        executing gc_die()? Default false.
 * @return int|false|GC_Error The ID of the comment on success, false or GC_Error on failure.
 */
function gc_new_comment( $commentdata, $gc_error = false ) {
	...
	$parent_status = ( $commentdata['comment_parent'] > 0 ) ? gc_get_comment_status( $commentdata['comment_parent'] ) : '';
	...
	if ( ! $comment_id ) {
		...
		foreach ( $fields as $field ) {
			if ( isset( $commentdata[ $field ] ) ) {
				$commentdata[ $field ] = $gcdb->strip_invalid_text_for_column( $gcdb->comments, $field, $commentdata[ $field ] );
			}
		}
 ...
 * @param bool           $gc_error       Whether to return a GC_Error object if there is a failure. Default false.
 * @return bool|GC_Error True on success, false or GC_Error on failure.
 */
function gc_set_comment_status( $comment_id, $comment_status, $gc_error = false ) {
	global $gcdb;
	...
	switch ( $comment_status ) {
		...
		case '1':
			$status = '1';
			add_action( 'gc_set_comment_status', 'gc_new_comment_notify_postauthor' );
			break;
		case 'spam':
			$status = 'spam';
			break;
	...
	if ( ! $gcdb->update( $gcdb->comments, array( 'comment_approved' => $status ), array( 'comment_ID' => $comment_old->comment_ID ) ) ) {
		if ( $gc_error ) {
			return new GC_Error( 'db_update_error', __( '无法更新评论状态' ), $gcdb->last_error );
		} else {
			return false;
		}
	}
	 ...
	 */
	do_action( 'gc_set_comment_status', $comment->comment_ID, $comment_status );

	gc_transition_comment_status( $comment_status, $comment_old->comment_approved, $comment );

	gc_update_comment_count( $comment->comment_post_ID );
 ...
 * @return int|false|GC_Error The value 1 if the comment was updated, 0 if not updated.
 *                            False or a GC_Error object on failure.
 */
function gc_update_comment( $commentarr, $gc_error = false ) {
	global $gcdb;
	...
	if ( ! empty( $commentarr['comment_post_ID'] ) && ! get_post( $commentarr['comment_post_ID'] ) ) {
		if ( $gc_error ) {
			return new GC_Error( 'invalid_post_id', __( '文章ID无效。' ) );
		} else {
			return false;
		}
	}
	 ...
	 */
	$data = apply_filters( 'gc_update_comment_data', $data, $comment, $commentarr );
 ...
 * @return bool
 */
function gc_defer_comment_counting( $defer = null ) {
	static $_defer = false;
	...
	if ( is_bool( $defer ) ) {
		...
		if ( ! $defer ) {
			gc_update_comment_count( null, true );
		}
	}
 ...
 *                   not exist.
 */
function gc_update_comment_count( $post_id, $do_deferred = false ) {
	...
	if ( $do_deferred ) {
		...
		foreach ( $_deferred as $i => $_post_id ) {
			gc_update_comment_count_now( $_post_id );
			unset( $_deferred[ $i ] );
			/** @todo Move this outside of the foreach and reset $_deferred to an array instead */
		}
	}

	if ( gc_defer_comment_counting() ) {
		$_deferred[] = $post_id;
		return true;
	} elseif ( $post_id ) {
		return gc_update_comment_count_now( $post_id );
	}
...
function gc_update_comment_count_now( $post_id ) {
	...
	gc_cache_delete( 'comments-0', 'counts' );
	 ...
	 * @param int      $post_id Post ID.
	 */
	$new = apply_filters( 'pre_gc_update_comment_count_now', null, $old, $post_id );

	if ( is_null( $new ) ) {
		$new = (int) $gcdb->get_var( $gcdb->prepare( "SELECT COUNT(*) FROM $gcdb->comments WHERE comment_post_ID = %d AND comment_approved = '1'", $post_id ) );
	} else {
		$new = (int) $new;
	}

	$gcdb->update( $gcdb->posts, array( 'comment_count' => $new ), array( 'ID' => $post_id ) );
	 ...
	 * @param int $old     The old comment count.
	 */
	do_action( 'gc_update_comment_count', $post_id, $new, $old );
	...
	if ( $to_ping ) {
		foreach ( (array) $to_ping as $tb_ping ) {
			...
			if ( ! in_array( $tb_ping, $pinged, true ) ) {
				...
				$gcdb->query(
					$gcdb->prepare(
						"UPDATE $gcdb->posts SET to_ping = TRIM(REPLACE(to_ping, %s,
					'')) WHERE ID = %d",
						$tb_ping,
						$post->ID
					)
...
function pingback( $content, $post ) {
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		...
		if ( $pingback_server_url ) {
			...
			$client          = new GC_HTTP_IXR_Client( $pingback_server_url );
			$client->timeout = 3;
			/**
			 ...
			 */
			$client->useragent = apply_filters( 'pingback_useragent', $client->useragent . ' -- GeChiUI/' . get_bloginfo( 'version' ), $client->useragent, $pingback_server_url, $pagelinkedto, $pagelinkedfrom );
...
function trackback( $trackback_url, $title, $excerpt, $ID ) {
	...
	$response = gc_safe_remote_post( $trackback_url, $options );

	if ( is_gc_error( $response ) ) {
		return;
	}

	$gcdb->query( $gcdb->prepare( "UPDATE $gcdb->posts SET pinged = CONCAT(pinged, '\n', %s) WHERE ID = %d", $trackback_url, $ID ) );
	return $gcdb->query( $gcdb->prepare( "UPDATE $gcdb->posts SET to_ping = TRIM(REPLACE(to_ping, %s, '')) WHERE ID = %d", $trackback_url, $ID ) );
}
...
function weblog_ping( $server = '', $path = '' ) {
	...
	$client->timeout    = 3;
	$client->useragent .= ' -- GeChiUI/' . get_bloginfo( 'version' );
...
function pingback_ping_source_uri( $source_uri ) {
	return (string) gc_http_validate_url( $source_uri );
}
 ...
 * @param GC_Query $query Query object.
 * @return array
 */
...
/**
 * Handles the submission of a comment, usually posted to gc-comments-post.php via a comment form.
 *
 ...
 *     @type string     $_gc_unfiltered_html_comment The nonce value for allowing unfiltered HTML.
 * }
...
function gc_handle_comment_submission( $comment_data ) {
	...
	if ( $user->exists() ) {
		...
		if ( get_option( 'comment_registration' ) ) {
			return new GC_Error( 'not_logged_in', __( '抱歉，您必须登录后再评论。' ), 403 );
		}
	}
	...
	if ( is_gc_error( $check_max_lengths ) ) {
		return $check_max_lengths;
	}

	$comment_id = gc_new_comment( gc_slash( $commentdata ), true );
 ...
 */
function gc_register_comment_personal_data_exporter( $exporters ) {
	$exporters['gechiui-comments'] = array(
		'exporter_friendly_name' => __( 'GeChiUI评论' ),
		'callback'               => 'gc_comments_personal_data_exporter',
	);
 ...
 */
function gc_comments_personal_data_exporter( $email_address, $page = 1 ) {
	...
	$comment_prop_to_export = array(
		'comment_author'       => __( '评论者' ),
		'comment_author_email' => __( '评论者电子邮箱' ),
		'comment_author_url'   => __( '评论者网址' ),
		'comment_author_IP'    => __( '评论者IP地址' ),
		'comment_agent'        => __( '评论者User Agent' ),
		'comment_date'         => __( '评论日期' ),
		'comment_content'      => __( '评论内容' ),
		'comment_link'         => __( '评论URL' ),
	);
 ...
 */
function gc_register_comment_personal_data_eraser( $erasers ) {
	$erasers['gechiui-comments'] = array(
		'eraser_friendly_name' => __( 'GeChiUI评论' ),
		'callback'             => 'gc_comments_personal_data_eraser',
	);
 ...
 * @return array
 */
function gc_comments_personal_data_eraser( $email_address, $page = 1 ) {
	global $gcdb;
	...
	foreach ( (array) $comments as $comment ) {
		...
		$anonymized_comment['comment_author_IP']    = gc_privacy_anonymize_data( 'ip', $comment->comment_author_IP );
		 ...
		 */
		$anon_message = apply_filters( 'gc_anonymize_comment', true, $comment, $anonymized_comment );
		...
		if ( true !== $anon_message ) {
			if ( $anon_message && is_string( $anon_message ) ) {
				...
				$messages[] = sprintf( __( '评论%d包含个人数据，但不能被匿名化。' ), $comment_id );
			}
		...
		$updated = $gcdb->update( $gcdb->comments, $anonymized_comment, $args );
 ...
 * @global gcdb $gcdb GeChiUI database abstraction object.
 */
function _gc_batch_update_comment_type() {
	global $gcdb;
	...
	if ( ! $lock_result ) {
		...
		if ( ! $lock_result || ( $lock_result > ( time() - HOUR_IN_SECONDS ) ) ) {
			gc_schedule_single_event( time() + ( 5 * MINUTE_IN_SECONDS ), 'gc_update_comment_type_batch' );
			return;
		}
	}
	...
	$empty_comment_type = $gcdb->get_var(
	...
	gc_schedule_single_event( time() + ( 2 * MINUTE_IN_SECONDS ), 'gc_update_comment_type_batch' );
	 ...
	 */
	$comment_batch_size = (int) apply_filters( 'gc_update_comment_type_batch_size', 100 );

	// Get the IDs of the comments to update.
	$comment_ids = $gcdb->get_col(
		$gcdb->prepare(
			"SELECT comment_ID
			FROM {$gcdb->comments}
			WHERE comment_type = ''
...
/**
 * In order to avoid the _gc_batch_update_comment_type() job being accidentally removed,
...
function _gc_check_for_scheduled_update_comment_type() {
	if ( ! get_option( 'finished_updating_comment_type' ) && ! gc_next_scheduled( 'gc_update_comment_type_batch' ) ) {
		gc_schedule_single_event( time() + MINUTE_IN_SECONDS, 'gc_update_comment_type_batch' );
	}
}
