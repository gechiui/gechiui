 ...
 *
 * The GeChiUI Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * GeChiUI Query class.
 *
 * The Rewrite along with GC class function as a front controller for GeChiUI.
...
class GC_Rewrite {
	 ...
	 * `GC_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see GC_Rewrite::init()
	 * @see GC_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to GeChiUI' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for GeChiUI into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see GC_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to GeChiUI 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells GC::parse_request() to check if a URL matching the page
	 ...
	 * `GC_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in GC_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see GC_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see GC_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	public function page_uri_index() {
		...
		$pages = $gcdb->get_results( "SELECT ID, post_name, post_parent FROM $gcdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'" );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name.
			$uri         = get_page_uri( $id );
			$attachments = $gcdb->get_results( $gcdb->prepare( "SELECT ID, post_name, post_parent FROM $gcdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ) );
	 ...
	 * @since 1.5.0
	 *
	 * @see GC_Rewrite::$rewritecode
	 * @see GC_Rewrite::$rewritereplace
	 * @see GC_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main GC_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of GC_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see GC_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current GC_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*gc-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*gc-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*gc-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*gc-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*gc-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param GC_Rewrite $gc_rewrite Current GC_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and GC_Rewrite::rewrite_rules() is that
	 ...
	 * @return string[] Array of rewrite rules keyed by their regex pattern.
	 */
	public function gc_rewrite_rules() {
		$this->rules = get_option( 'rewrite_rules' );
		if ( empty( $this->rules ) ) {
			$this->matches = 'matches';
			$this->rewrite_rules();
			if ( ! did_action( 'gc_loaded' ) ) {
				add_action( 'gc_loaded', array( $this, 'flush_rules' ) );
				return $this->rules;
			}
	 ...
	 *
	 * Will add the non_gc_rules property rules to the .htaccess file before
	 * the GeChiUI rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to GC's index.php (and thus shouldn't be handled by GC at all).
		foreach ( (array) $this->non_gc_rules as $match => $query ) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="GeChiUI: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	...
	public function add_external_rule( $regex, $query ) {
		$this->non_gc_rules[ $regex ] = $query;
	}

	/**
	 * Adds an endpoint, like /trackback/.
	 * $query_var parameter added.
	 ...
	 * query strings. The new permastruct is added to the GC_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by GC_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to GC_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how GC_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `GC_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls GC_Rewrite::gc_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * GC_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls GC_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls GC_Rewrite::init() after
