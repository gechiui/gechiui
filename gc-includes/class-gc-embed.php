...
class GC_Embed {
	...
	public function __construct() {
		// Hack to get the [embed] shortcode to run before gcautop().
	 ...
	 * an Ajax request that will call GC_Embed::cache_oembed().
	 */
...
<script type="text/javascript">
	 ...
	 *
	 * Do not use this function directly, use gc_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use gc_embed_unregister_handler() instead.
	 *
	 ...
	 * will be given to the GC_oEmbed class.
	 *
	 * @param array  $attr {
	 *     Shortcode attributes. Optional.
	 *
	...
	/**
	 * Passes any unlinked URLs that are on their own line to GC_Embed::shortcode() for potential embedding.
	 *
	 * @see GC_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = gc_replace_in_html_tags( $content, array( "\n" => '<!-- gc-line-break -->' ) );
		...
		return str_replace( '<!-- gc-line-break -->', "\n", $content );
	}
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = gc_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new GC_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			gc_cache_set( $cache_key, $oembed_post_id, $cache_group );
