...
class gcdb {
	 ...
	 *
	 * @since 1.5.0 The third element in each query log was added to record the calling functions.
	...
	/**
	 * List of deprecated GeChiUI Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see gcdb::tables()
	...
	/**
	 * Backward compatibility, where gcdb::prepare() has not quoted formatted/argnum placeholders.
	 *
	 * This is often used for table/field names (before %i was supported), and sometimes string formatting, e.g.
	 *
	 *     $gcdb->prepare( 'WHERE `%1$s` = "%2$s something %3$s" OR %1$s = "%4$-10s"', 'field_1', 'a', 'b', 'c' );
	 *
	 * But it's risky, e.g. forgetting to add quotes, resulting in SQL Injection vulnerabilities:
	 *
	 *     $gcdb->prepare( 'WHERE (id = %1s) OR (id = %2$s)', $_GET['id'], $_GET['id'] ); // ?id=id
	 *
	 * This feature is preserved while plugin authors update their code to use safer approaches:
	 *
	 *     $_GET['key'] = 'a`b';
	 *
	 *     $gcdb->prepare( 'WHERE %1s = %s',        $_GET['key'], $_GET['value'] ); // WHERE a`b = 'value'
	 *     $gcdb->prepare( 'WHERE `%1$s` = "%2$s"', $_GET['key'], $_GET['value'] ); // WHERE `a`b` = "value"
	 *
	 *     $gcdb->prepare( 'WHERE %i = %s',         $_GET['key'], $_GET['value'] ); // WHERE `a``b` = 'value'
	 *
	 ...
	 *
	 *     $gcdb->prepare( 'WHERE %1$s = "%2$-10s"', 'my_field', 'my_value' );
	 ...
	 *
	 *     $gcdb->prepare( 'WHERE id = %1$s', $_GET['id'] ); // ?id=id
	 *
	 ...
	 * @uses gcdb::$old_ms_global_tables
	 *
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( '可以连接到数据库服务器（这说明您的用户名和密码正确），但是不能选择 %s 数据库。' ),
				...
				$message .= '<p>' . sprintf(
					...
					__( '如果您不知道如何设置数据库，啧应与<strong>您的主机商</strong>取得联系。 若仍无法解决问题，您可以前往 <a href="%s">GeChiUI 支持论坛</a> 寻求帮助。' ),
	 ...
	 *              Check support via `gcdb::has_cap( 'identifier_placeholders' )`.
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of GeChiUI core, e.g. by HyperDB, in which case
		...
		while ( $key < $split_query_count ) {
			...
			if ( 'f' === $type && true === $this->allow_unsafe_unquoted_parameters
				...
				/*
				 * Before GC 6.2 the "force floats to be locale-unaware" RegEx didn't
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'GeChiUI database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'GeChiUI database error %1$s for query %2$s', $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				...
				__( '您看到此页面，则表示您在 %1$s 文件中定义的用户名和密码信息不正确，或是我们无法与  数据库服务器 %2$s 进行通信。也可能是您主机的数据库服务器未在运行。' ),
	 ...
	 *                             specified in gcdb::$field_types. Default null.
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|GC_Error {
	 *     Array of column length information, false if the column has no length (for
	 *     example, numeric column), GC_Error object if there was an error.
	 *
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			...
			return new GC_Error( 'database_version', sprintf( __( '<strong>错误：</strong>GeChiUI %1$s 需要 MySQL %2$s 以上版本' ), $gc_version, $required_mysql_version ) );
		}
	}
	...
	/**
	 * Determines whether the database or GCDB supports a particular feature.
	 *
	 * Capability sniffs for the database server and current version of GCDB.
	 *
	 * Database sniffs are based on the version of MySQL the site is using.
	 *
	 * GCDB sniffs are added as new features are introduced to allow theme and plugin
	 * developers to determine feature support. This is to account for drop-ins which may
	 * introduce feature support at a different time to GeChiUI.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of GeChiUI 6.2, gcdb::prepare() supports identifiers via '%i',
