 ...
 *
 * @global array $gc_cockneyreplace Array of formatted entities for certain common phrases.
 ...
 */
function gctexturize( $text, $reset = false ) {
	global $gc_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		/**
		 * Filters whether to skip running gctexturize().
		 *
		 * Returning false from the filter will effectively short-circuit gctexturize()
		 * and return the original text passed to the function instead.
		 *
		 * The filter runs only once, the first time gctexturize() is called.
		 *
		 * @since 4.0.0
		 *
		 * @see gctexturize()
		 *
		 * @param bool $run_texturize Whether to short-circuit gctexturize().
		 */
		$run_texturize = apply_filters( 'run_gctexturize', $run_texturize );
		...
		if ( isset( $gc_cockneyreplace ) ) {
			$cockney        = array_keys( $gc_cockneyreplace );
			$cockneyreplace = array_values( $gc_cockneyreplace );
		} else {
			/*
	...
	$shortcode_regex  = $found_shortcodes ? _get_gctexturize_shortcode_regex( $tagnames ) : '';
	$regex            = _get_gctexturize_split_regex( $shortcode_regex );

	$textarr = preg_split( $regex, $text, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY );

	foreach ( $textarr as &$curl ) {
		// Only call _gctexturize_pushpop_element if $curl is a delimiter.
		$first = $curl[0];
		if ( '<' === $first ) {
			if ( str_starts_with( $curl, '<!--' ) ) {
				...
				_gctexturize_pushpop_element( $curl, $no_texturize_tags_stack, $no_texturize_tags );
			}
		} elseif ( '' === trim( $curl ) ) {
			...
			if ( ! str_starts_with( $curl, '[[' ) && ! str_ends_with( $curl, ']]' ) ) {
				...
				_gctexturize_pushpop_element( $curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes );
			} else {
			...
			if ( str_contains( $curl, "'" ) ) {
				...
				$curl = gctexturize_primes( $curl, "'", $prime, $open_sq_flag, $closing_single_quote );
				$curl = str_replace( $apos_flag, $apos, $curl );
				$curl = str_replace( $open_sq_flag, $opening_single_quote, $curl );
			}
			if ( str_contains( $curl, '"' ) ) {
				$curl = preg_replace( $dynamic_characters['quote'], $dynamic_replacements['quote'], $curl );
				$curl = gctexturize_primes( $curl, '"', $double_prime, $open_q_flag, $closing_quote );
 ...
 */
function gctexturize_primes( $haystack, $needle, $prime, $open_quote, $close_quote ) {
	$spaces           = gc_spaces_regexp();
	$flag             = '<!--gc-prime-or-quote-->';
 ...
 */
function _gctexturize_pushpop_element( $text, &$stack, $disabled_elements ) {
...
function gcautop( $text, $br = true ) {
	...
	$text = gc_replace_in_html_tags( $text, array( "\n" => ' <!-- gcnl --> ' ) );
	...
	if ( $br ) {
		...
		$text = str_replace( '<GCPreserveNewline />', "\n", $text );
	}
	...
	if ( str_contains( $text, '<!-- gcnl -->' ) ) {
		$text = str_replace( array( ' <!-- gcnl --> ', '<!-- gcnl -->' ), "\n", $text );
	}
 ...
 *
 * @param string $shortcode_regex Optional. The result from _get_gctexturize_shortcode_regex().
 * @return string The regular expression
 */
function _get_gctexturize_split_regex( $shortcode_regex = '' ) {
	static $html_regex;
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with gc_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 * @return string The regular expression
 */
function _get_gctexturize_shortcode_regex( $tagnames ) {
 ...
 */
function gc_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = gc_html_split( $haystack );
	$changed = false;
...
/**
 * Newline preservation help function for gcautop().
 *
 * @access private
 *
...
function shortcode_unautop( $text ) {
	...
	// phpcs:disable Squiz.Strings.ConcatenationSpacing.PaddingFound,GeChiUI.WhiteSpace.PrecisionAlignment.Found -- don't remove regex indentation
	$pattern =
		'/'
		. '<p>'                              // Opening paragraph.
 ...
 */
function _gc_specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	$text = (string) $text;
	...
	// Store the site charset as a static to avoid multiple calls to gc_load_alloptions().
	if ( ! $charset ) {
	...
	if ( ! $double_encode ) {
		 ...
		 */
		$text = gc_kses_normalize_entities( $text, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _gc_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function gc_specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	$text = (string) $text;
	...
	// Match the previous behavior of _gc_specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
 ...
 */
function gc_check_invalid_utf8( $text, $strip = false ) {
	$text = (string) $text;
	...
	// phpcs:ignore GeChiUI.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
 ...
 * Added locale support for `de_CH`, `de_CH_informal`, and `ca`. Added locale support for `sr_RS`. Added locale support for `bs_BA`.
...
function sanitize_file_name( $filename ) {
	...
	if ( ! str_contains( $filename, '.' ) ) {
		...
		$filetype   = gc_check_filetype( 'test.' . $filename, $mime_types );
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = gc_strip_all_tags( $username );
...
function force_balance_tags( $text ) {
	...
	$text = str_replace( '< !--', '<    !--', $text );
	// GC bug fix for LOVE <3 (and other situations with '<' before a number).
	...
	// GC fix for the bug with HTML comments.
 ...
 * The `$richedit` parameter was renamed to `$rich_text` for clarity.
 *
...
function _make_clickable_rel_attr( $url ) {
	$rel_parts        = array();
	$scheme           = strtolower( gc_parse_url( $url, PHP_URL_SCHEME ) );
	$nofollow_schemes = array_intersect( gc_allowed_protocols(), array( 'https', 'http' ) );

	// Apply "nofollow" to external links with qualifying URL schemes (mailto:, tel:, etc... shouldn't be followed).
	if ( ! gc_is_internal_link( $url ) && in_array( $scheme, $nofollow_schemes, true ) ) {
		$rel_parts[] = 'nofollow';
	}
 ...
 */
function gc_rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = gc_kses_hair( $matches[1], gc_allowed_protocols() );

	if ( ! empty( $atts['href'] ) && gc_is_internal_link( $atts['href']['value'] ) ) {
 ...
 */
function gc_rel_nofollow( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function( $matches ) {
			return gc_rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return gc_slash( $text );
}

/**
 * Callback to add `rel="nofollow"` string to HTML A element.
 *
 * @deprecated 5.3.0 Use gc_rel_callback()
 *
 * @param array $matches Single match.
 * @return string HTML A Element with `rel="nofollow"`.
 */
function gc_rel_nofollow_callback( $matches ) {
	return gc_rel_callback( $matches, 'nofollow' );
}
 ...
 * @return string Converted content.
 */
function gc_rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function( $matches ) {
			return gc_rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return gc_slash( $text );
 ...
 *
 * @since 5.1.0 Removed 'noreferrer' relationship.
 *
 * @param string $text Content that may contain HTML A elements.
 * @return string Converted content.
 */
function gc_targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'gc_targeted_link_rel_callback', $part );
	}
 ...
 */
function gc_targeted_link_rel_callback( $matches ) {
	$link_html          = $matches[1];
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by gc_kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = gc_kses_hair( $link_html, gc_allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'gc_targeted_link_rel', 'noopener', $link_html );
 ...
 * @since 5.1.0
 */
function gc_init_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		add_filter( $filter, 'gc_targeted_link_rel' );
	}
}
 ...
 * @since 5.1.0
 */
function gc_remove_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		remove_filter( $filter, 'gc_targeted_link_rel' );
	}
}
 ...
 *
 * Looks up one smiley code in the $gcsmiliestrans global array and returns an
 ...
 *
 * @global string|array $gc_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $gc_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $gc_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] ) {
				$content = preg_replace_callback( $gc_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function gc_iso_descrambler( $subject ) {
	...
	return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_gc_iso_convert', $subject );
}
 ...
 */
function _gc_iso_convert( $matches ) {
...
function get_gmt_from_date( $date_string, $format = 'Y-m-d H:i:s' ) {
	$datetime = date_create( $date_string, gc_timezone() );
...
function human_time_diff( $from, $to = 0 ) {
	...
	if ( $diff < MINUTE_IN_SECONDS ) {
		...
		$since = sprintf( _n( '%s秒', '%s秒', $secs ), $secs );
		...
		$since = sprintf( _n( '%s周', '%s周', $weeks ), $weeks );
		...
		$since = sprintf( _n( '%s月', '%s月', $months ), $months );
		...
		$since = sprintf( _n( '%s年', '%s年', $years ), $years );
	}
 ...
 * @param GC_Post|object|int $post Optional. GC_Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function gc_trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		/*
		 * Temporarily unhook gc_filter_content_tags() since any tags
		 * within the excerpt are stripped out. Modifying the tags here
		 * is wasteful and can lead to bugs in the image counting logic.
		 */
		$filter_removed = remove_filter( 'the_content', 'gc_filter_content_tags' );
		 ...
		 * which is generally used for the filter callback in GeChiUI core.
		 */
		if ( $filter_removed ) {
		...
		$text         = gc_trim_words( $text, $excerpt_length, $excerpt_more );
	 ...
	 */
	return apply_filters( 'gc_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function gc_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = gc_strip_all_tags( $text );
	$num_words     = (int) $num_words;

	if ( str_starts_with( gc_get_word_count_type(), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
	 ...
	 */
	return apply_filters( 'gc_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *
 * Usually you should prepare queries using gcdb::prepare().
 ...
 *                            Defaults to return value of gc_allowed_protocols().
 ...
 * Sanitizes a URL for database or redirect usage.
 *
 * @since 2.3.1 Deprecated in favor of esc_url_raw().
...
function esc_html( $text ) {
	$safe_text = gc_check_invalid_utf8( $text );
	$safe_text = _gc_specialchars( $safe_text, ENT_QUOTES );
	/**
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function( $matches ) {
			...
			if ( isset( $matches['non_cdata'] ) ) {
				...
				return _gc_specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _gc_specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
 ...
 */
function gc_make_link_relative( $link ) {
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email':
			$value = $gcdb->strip_invalid_text_for_column( $gcdb->options, 'option_value', $value );
			if ( is_gc_error( $value ) ) {
				$error = $value->get_error_message();
			} else {
				$value = sanitize_email( $value );
				if ( ! is_email( $value ) ) {
					$error = __( '您输入的电子邮箱无效。请输入有效的电子邮箱。' );
				}
			}
			break;
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $gcdb->strip_invalid_text_for_column( $gcdb->options, 'option_value', gc_encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_gc_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( '您输入的GeChiUI地址可能有误。请输入一个正确的URL。' );
				}
			}
			break;
		...
		case 'home':
			...
			if ( is_gc_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( '您输入的系统地址可能有误。请输入正确的URL。' );
				}
			}
			break;
		...
		case 'GCLANG':
			...
			if ( ! is_multisite() && defined( 'GCLANG' ) && '' !== GCLANG && 'zh_CN' !== GCLANG ) {
				$allowed[] = GCLANG;
			}
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( '自定义固定链接需要一个结构化标签。<a href="%s">了解更多</a>' ),
					__( 'https://www.gechiui.com/support/using-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
	...
	if ( null !== $error ) {
		if ( '' === $error && is_gc_error( $value ) ) {
			/* translators: 1: Option name, 2: Error code. */
			$error = sprintf( __( '无法清理 %1$s 选项。错误代码：%2$s' ), $option, $value->get_error_code() );
		}
 ...
 */
function gc_parse_str( $input_string, &$result ) {
	 ...
	 */
	$result = apply_filters( 'gc_parse_str', $result );
}
 ...
 * @return string Converted text.
 */
function gc_pre_kses_less_than( $content ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'gc_pre_kses_less_than_callback', $content );
}
 ...
 */
function gc_pre_kses_less_than_callback( $matches ) {
 ...
 */
function gc_pre_kses_block_attributes( $content, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `gc_kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'gc_pre_kses_block_attributes', 10 );
...
/**
 * GeChiUI' implementation of PHP sprintf() with filters.
 *
 ...
 */
function gc_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to gc_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'gc_sprintf', $fragment, $arg );
 ...
 */
function gc_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by gc_sprintf_l().
	...
	$l = apply_filters(
		...
		array(
			...
			'between_last_two' => sprintf( __( '%1$s和%2$s' ), '', '' ),
			/* translators: Used to join items in a list with only 2 items. */
			'between_only_two' => sprintf( __( '%1$s和%2$s' ), '', '' ),
		)
	);
 ...
 * @return string The excerpt.
 */
function gc_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}

	$str     = gc_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], gc_allowed_protocols(), true ) ?
			$m[3] :
			GC_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. gc_strip_all_tags will return ''
 *
 * @param string $text          String containing HTML tags
 * @param bool   $remove_breaks Optional. Whether to remove left over line breaks and white space chars
 * @return string The processed string.
 */
function gc_strip_all_tags( $text, $remove_breaks = false ) {
	if ( is_null( $text ) ) {
		return '';
	}
	...
	if ( ! is_scalar( $text ) ) {
		...
		trigger_error(
			sprintf(
				...
				__( '警告：%1$s要求参数%2$s（%3$s）为%4$s，给定%5$s。' ),
				__FUNCTION__,
				'#1',
				'$text',
				'string',
 ...
 * @see gc_check_invalid_utf8()
 * @see gc_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	$filtered = gc_check_invalid_utf8( $str );

	if ( str_contains( $filtered, '<' ) ) {
		$filtered = gc_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = gc_strip_all_tags( $filtered, false );
 ...
 * @return string
 */
function gc_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

// phpcs:disable GeChiUI.GC.CapitalPDangit.Misspelled, GeChiUI.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
 * Forever eliminate "Gechiui" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'gc_title' === $current_filter ) {
		return str_replace( 'Gechiui', 'GeChiUI', $text );
	}
 ...
 */
function gc_slash( $value ) {
	if ( is_array( $value ) ) {
		$value = array_map( 'gc_slash', $value );
	}
...
function gc_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'gc_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
...
function _print_emoji_detection_script() {
	...
	gc_print_inline_script_tag(
		sprintf( 'window._gcemojiSettings = %s;', gc_json_encode( $settings ) ) . "\n" .
			file_get_contents( ABSPATH . 'assets/js/gc-emoji-loader' . gc_scripts_get_suffix() . '.js' )
	);
}
 ...
 */
function gc_encode_emoji( $content ) {
	$emoji = _gc_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function gc_staticize_emoji( $text ) {
	if ( ! str_contains( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = gc_encode_emoji( $text );
 ...
 */
function gc_staticize_emoji_for_email( $mail ) {
	 ...
	 * in gc_mail() - get the `Content-Type` from the headers, if there is one,
	 * then pass it through the {@see 'gc_mail_content_type'} filter, in case
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = gc_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that GeChiUI recognises.
 */
function _gc_emoji_list( $type = 'entities' ) {
 ...
 * Moved to gc-includes/formatting.php from gc-admin/includes/misc.php and added $length param.
 *
