 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various GeChiUI object types. Metadata
...
function update_metadata( $meta_type, $object_id, $meta_key, $meta_value, $prev_value = '' ) {
	...
	$meta_ids = $gcdb->get_col( $gcdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $gcdb->update( $table, $data, $where );
	if ( ! $result ) {
		return false;
	}
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	$query = $gcdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( ! $delete_all ) {
		$query .= $gcdb->prepare( " AND $type_column = %d", $object_id );
	}

	if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
		$query .= $gcdb->prepare( ' AND meta_value = %s', $meta_value );
	}

	$meta_ids = $gcdb->get_col( $query );
	if ( ! count( $meta_ids ) ) {
		return false;
	}
	...
	if ( $delete_all ) {
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$object_ids = $gcdb->get_col( $gcdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s", $meta_key, $meta_value ) );
		} else {
			$object_ids = $gcdb->get_col( $gcdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
		}
	}
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = gc_cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! gc_is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in gc-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in gc-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $gcdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result ) {
			return false;
		}
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		$result = (bool) $gcdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$meta_list = $gcdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
 ...
 */
function gc_metadata_lazyloader() {
	static $gc_metadata_lazyloader;

	if ( null === $gc_metadata_lazyloader ) {
		$gc_metadata_lazyloader = new GC_Metadata_Lazyloader();
	}

	return $gc_metadata_lazyloader;
}
...
function _get_meta_table( $type ) {
	...
	if ( empty( $gcdb->$table_name ) ) {
		return false;
	}

	return $gcdb->$table_name;
}
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( false !== $args['show_in_rest'] && 'array' === $args['type'] ) {
		if ( ! is_array( $args['show_in_rest'] ) || ! isset( $args['show_in_rest']['schema']['items'] ) ) {
			_doing_it_wrong( __FUNCTION__, __( '当在REST API中注册“array”媒体类型来显示时，您必须在“show_in_rest.schema.items”中对每个数组项目指定模式。' ), '5.3.0' );
	...
	if ( array_key_exists( 'default', $args ) ) {
		...
		if ( is_gc_error( $check ) ) {
			_doing_it_wrong( __FUNCTION__, __( '注册默认元值时，数据必须与所提供的类型相匹配。' ), '5.5.0' );
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $gc_meta_keys ) || ! isset( $gc_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $gc_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $gc_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $gc_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $gc_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $gc_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $gc_meta_keys[ $object_type ] ) ) {
		unset( $gc_meta_keys[ $object_type ] );
	}
 ...
 */
function _gc_register_meta_args_allowed_list( $args, $default_args ) {
