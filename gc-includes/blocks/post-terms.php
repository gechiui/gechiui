...
function render_block_core_post_terms( $attributes, $content, $block ) {
	...
	if ( is_gc_error( $post_terms ) || empty( $post_terms ) ) {
		return '';
	}
	...
	if ( isset( $attributes['prefix'] ) && $attributes['prefix'] ) {
		$prefix .= '<span class="gc-block-post-terms__prefix">' . $attributes['prefix'] . '</span>';
	}

	$suffix = '</div>';
	if ( isset( $attributes['suffix'] ) && $attributes['suffix'] ) {
		$suffix = '<span class="gc-block-post-terms__suffix">' . $attributes['suffix'] . '</span>' . $suffix;
	}
	...
	return get_the_term_list(
		...
		$attributes['term'],
		gc_kses_post( $prefix ),
		'<span class="gc-block-post-terms__separator">' . esc_html( $separator ) . '</span>',
		gc_kses_post( $suffix )
	);
}
...
function register_block_core_post_terms() {
	...
	foreach ( $taxonomies as $taxonomy ) {
		$variation = array(
			...
			'description' => sprintf( __( '显示指定的分类法：%s' ), $taxonomy->label ),
			'attributes'  => array(
				'term' => $taxonomy->name,
			),
			'isActive'    => array( 'term' ),
