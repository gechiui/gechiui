...
function render_block_core_post_author( $attributes, $content, $block ) {
	...
	( ! empty( $attributes['showAvatar'] ) ? '<div class="gc-block-post-author__avatar">' . $avatar . '</div>' : '' ) .
	'<div class="gc-block-post-author__content">' .
	( ! empty( $byline ) ? '<p class="gc-block-post-author__byline">' . $byline . '</p>' : '' ) .
	'<p class="gc-block-post-author__name">' . get_the_author_meta( 'display_name', $author_id ) . '</p>' .
	( ! empty( $attributes['showBio'] ) ? '<p class="gc-block-post-author__bio">' . get_the_author_meta( 'user_description', $author_id ) . '</p>' : '' ) .
	'</div>' .
	'</div>';
}
