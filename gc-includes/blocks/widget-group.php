 ...
 * @param GC_Block $block The block.
 *
 * @return string Rendered block.
 */
function render_block_core_widget_group( $attributes, $content, $block ) {
	global $gc_registered_sidebars, $_sidebar_being_rendered;

	if ( isset( $gc_registered_sidebars[ $_sidebar_being_rendered ] ) ) {
		$before_title = $gc_registered_sidebars[ $_sidebar_being_rendered ]['before_title'];
		$after_title  = $gc_registered_sidebars[ $_sidebar_being_rendered ]['after_title'];
	} else {
...
/**
 * Make a note of the sidebar being rendered before GeChiUI starts rendering
...
/**
 * Clear whatever we set in note_sidebar_being_rendered() after GeChiUI
