...
function render_block_core_navigation_submenu( $attributes, $content, $block ) {
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => $css_classes . ' gc-block-navigation-item' . ( $has_submenu ? ' has-child' : '' ) .
	...
	if ( isset( $attributes['label'] ) ) {
		$label .= gc_kses_post( $attributes['label'] );
	}

	$aria_label = sprintf(
		/* translators: Accessibility text. %s: Parent page title. */
		__( '%s 子菜单' ),
		gc_strip_all_tags( $label )
	);
	...
	if ( ! $open_on_click ) {
		...
		$html .= '<a class="gc-block-navigation-item__content"';
		...
		if ( $show_submenu_indicators ) {
			...
			$html .= '<button aria-label="' . esc_attr( $aria_label ) . '" class="gc-block-navigation__submenu-icon gc-block-navigation-submenu__toggle" aria-expanded="false">' . block_core_navigation_submenu_render_submenu_icon() . '</button>';
		}
	} else {
		// If menus open on click, we render the parent as a button.
		$html .= '<button aria-label="' . esc_attr( $aria_label ) . '" class="gc-block-navigation-item__content gc-block-navigation-submenu__toggle" aria-expanded="false">';

		// Wrap title with span to isolate it from submenu icon.
		$html .= '<span class="gc-block-navigation-item__label">';
		...
		$html .= '<span class="gc-block-navigation__submenu-icon">' . block_core_navigation_submenu_render_submenu_icon() . '</span>';
	...
	if ( $has_submenu ) {
		...
		// This allows us to be able to get a response from gc_apply_colors_support.
		$block->block_type->supports['color'] = true;
		$colors_supports                      = gc_apply_colors_support( $block->block_type, $attributes );
		$css_classes                          = 'gc-block-navigation__submenu-container';
		...
		if ( strpos( $inner_blocks_html, 'current-menu-item' ) ) {
			$tag_processor = new GC_HTML_Tag_Processor( $html );
			while ( $tag_processor->next_tag( array( 'class_name' => 'gc-block-navigation-item__content' ) ) ) {
