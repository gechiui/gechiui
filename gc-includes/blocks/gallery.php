...
/**
 * Adds a style tag for the --gc--style--unstable-gallery-gap var.
 *
...
function block_core_gallery_render( $attributes, $content ) {
	$gap = _gc_array_get( $attributes, array( 'style', 'spacing', 'blockGap' ) );
	...
	if ( is_array( $gap ) ) {
		foreach ( $gap as $key => $value ) {
			...
			if ( is_string( $value ) && str_contains( $value, 'var:preset|spacing|' ) ) {
				...
				$slug            = _gc_to_kebab_case( substr( $value, $index_to_splice ) );
				$value           = "var(--gc--preset--spacing--$slug)";
			}
		...
		if ( is_string( $gap ) && str_contains( $gap, 'var:preset|spacing|' ) ) {
			...
			$slug            = _gc_to_kebab_case( substr( $gap, $index_to_splice ) );
			$gap             = "var(--gc--preset--spacing--$slug)";
		}
	}

	$unique_gallery_classname = gc_unique_id( 'gc-block-gallery-' );
	$processed_content        = new GC_HTML_Tag_Processor( $content );
	$processed_content->next_tag();
	$processed_content->add_class( $unique_gallery_classname );

	// --gallery-block--gutter-size is deprecated. --gc--style--gallery-gap-default should be used by themes that want to set a default
	// gap on the gallery.
	$fallback_gap = 'var( --gc--style--gallery-gap-default, var( --gallery-block--gutter-size, var( --gc--style--block-gap, 0.5em ) ) )';
	...
	$gallery_styles = array(
		array(
			'selector'     => ".gc-block-gallery.{$unique_gallery_classname}",
			'declarations' => array(
				'--gc--style--unstable-gallery-gap' => $gap_column,
				'gap'                               => $gap_value,
			),
		),
	);
