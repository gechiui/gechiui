...
function render_block_core_search( $attributes, $content, $block ) {
	...
	// attributes to `__( '搜索' )` meaning that many posts contain `<!--
	// gc:search /-->`. Support these by defaulting an undefined label and
	// buttonText to `__( '搜索' )`.
	$attributes = gc_parse_args(
		$attributes,
		array(
			'label'      => __( '搜索' ),
			'buttonText' => __( '搜索' ),
	...
	$input_id            = gc_unique_id( 'gc-block-search__input-' );
	...
	$label_inner_html = empty( $attributes['label'] ) ? __( '搜索' ) : gc_kses_post( $attributes['label'] );
	$label            = new GC_HTML_Tag_Processor( sprintf( '<label %1$s>%2$s</label>', $inline_styles['label'], $label_inner_html ) );
	if ( $label->next_tag() ) {
		$label->set_attribute( 'for', $input_id );
		$label->add_class( 'gc-block-search__label' );
	...
	$input         = new GC_HTML_Tag_Processor( sprintf( '<input type="search" name="s" required %s/>', $inline_styles['input'] ) );
	$input_classes = array( 'gc-block-search__input' );
	...
	if ( $input->next_tag() ) {
		...
		$view_js_file = 'gc-block-search-view';
	...
	if ( $show_button ) {
		$button_classes         = array( 'gc-block-search__button' );
		...
		if ( ! $use_icon_button ) {
			if ( ! empty( $attributes['buttonText'] ) ) {
				$button_internal_markup = gc_kses_post( $attributes['buttonText'] );
			}
		} else {
		...
		$button_classes[] = gc_theme_get_element_class_name( 'button' );
		$button           = new GC_HTML_Tag_Processor( sprintf( '<button type="submit" %s>%s</button>', $inline_styles['button'], $button_internal_markup ) );

		if ( $button->next_tag() ) {
			$button->add_class( implode( ' ', $button_classes ) );
			if ( 'expand-searchfield' === $attributes['buttonBehavior'] && 'button-only' === $attributes['buttonPosition'] ) {
				$button->set_attribute( 'aria-label', __( '展开搜索字段' ) );
				$button->set_attribute( 'aria-controls', 'gc-block-search__input-' . $input_id );
				$button->set_attribute( 'aria-expanded', 'false' );
			} else {
				$button->set_attribute( 'aria-label', gc_strip_all_tags( $attributes['buttonText'] ) );
			}
		}
	}
	...
	$field_markup         = sprintf(
		'<div class="gc-block-search__inside-wrapper %s" %s>%s</div>',
...
function classnames_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['buttonPosition'] ) ) {
		if ( 'button-inside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'gc-block-search__button-inside';
		}

		if ( 'button-outside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'gc-block-search__button-outside';
		}

		if ( 'no-button' === $attributes['buttonPosition'] ) {
			$classnames[] = 'gc-block-search__no-button';
		}

		if ( 'button-only' === $attributes['buttonPosition'] ) {
			$classnames[] = 'gc-block-search__button-only';
			if ( ! empty( $attributes['buttonBehavior'] ) && 'expand-searchfield' === $attributes['buttonBehavior'] ) {
				$classnames[] = 'gc-block-search__button-behavior-expand gc-block-search__searchfield-hidden';
			}
		}
	}
	...
	if ( isset( $attributes['buttonUseIcon'] ) ) {
		if ( ! empty( $attributes['buttonPosition'] ) && 'no-button' !== $attributes['buttonPosition'] ) {
			if ( $attributes['buttonUseIcon'] ) {
				$classnames[] = 'gc-block-search__icon-button';
			} else {
				$classnames[] = 'gc-block-search__text-button';
			}
		}
	}
...
function apply_block_core_search_border_style( $attributes, $property, $side, &$wrapper_styles, &$button_styles, &$input_styles ) {
	$is_button_inside = 'button-inside' === _gc_array_get( $attributes, array( 'buttonPosition' ), false );
	...
	$value = _gc_array_get( $attributes, $path, false );
	...
	if ( 'color' === $property && $side ) {
		...
		if ( $has_color_preset ) {
			...
			$value             = sprintf( 'var(--gc--preset--color--%s)', $named_color_value );
		}
	}
...
function get_typography_styles_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['style']['typography']['fontSize'] ) ) {
		$typography_styles[] = sprintf(
			'font-size: %s;',
			gc_get_typography_font_size_value(
				array(
