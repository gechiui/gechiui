...
function render_block_core_search( $attributes ) {
	...
	$input_id         = 'gc-block-search__input-' . ++$instance_id;
	...
	$label_markup = sprintf(
		'<label for="%1$s" class="gc-block-search__label screen-reader-text">%2$s</label>',
		esc_attr( $input_id ),
		$label_inner_html
	);
	if ( $show_label && ! empty( $attributes['label'] ) ) {
		$label_markup = sprintf(
			'<label for="%1$s" class="gc-block-search__label">%2$s</label>',
			$input_id,
			$label_inner_html
		);
	}
	...
	if ( $show_input ) {
		...
		$input_markup  = sprintf(
			'<input type="search" id="%s" class="gc-block-search__input %s" name="s" value="%s" placeholder="%s" %s required />',
			$input_id,
	...
	if ( $show_button ) {
		...
		$button_markup = sprintf(
			'<button type="submit" class="gc-block-search__button %s" %s>%s</button>',
