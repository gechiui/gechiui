...
function block_core_home_link_build_li_wrapper_attributes( $context ) {
	...
	$classes[]       = 'gc-block-navigation-item';
...
function render_block_core_home_link( $attributes, $content, $block ) {
	...
	return sprintf(
		'<li %1$s><a class="gc-block-home-link__content gc-block-navigation-item__content" href="%2$s" rel="home"%3$s>%4$s</a></li>',
		block_core_home_link_build_li_wrapper_attributes( $block->context ),
		esc_url( home_url() ),
		$aria_current,
		gc_kses_post( $attributes['label'] )
	);
}
