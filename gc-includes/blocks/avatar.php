...
function render_block_core_avatar( $attributes, $content, $block ) {
	...
	$image_classes = ! empty( $border_attributes['class'] )
		? "gc-block-avatar__image {$border_attributes['class']}"
		: 'gc-block-avatar__image';
	...
	if ( isset( $attributes['isLink'] ) && $attributes['isLink'] && isset( $comment->comment_author_url ) && '' !== $comment->comment_author_url ) {
		...
		if ( '_blank' === $attributes['linkTarget'] ) {
			// translators: %s is the Comment Author name.
			$label = 'aria-label="' . sprintf( esc_attr__( '（%s 网站链接，在新标签页中打开）' ), $comment->comment_author ) . '"';
		}
...
function get_block_core_avatar_border_attributes( $attributes ) {
	...
	$custom_color           = _gc_array_get( $attributes, array( 'style', 'border', 'color' ), null );
	$border_styles['color'] = $preset_color ? $preset_color : $custom_color;

	// Individual border styles e.g. top, left etc.
	foreach ( $sides as $side ) {
		$border                 = _gc_array_get( $attributes, array( 'style', 'border', $side ), null );
	...
	$styles     = gc_style_engine_get_styles( array( 'border' => $border_styles ) );
	$attributes = array();
