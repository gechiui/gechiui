...
/**
 * Finds the first non-empty `gc_navigation` Post.
 *
 * @return GC_Post|null the first non-empty Navigation or null.
 */
function block_core_navigation_get_first_non_empty_navigation() {
	// Order and orderby args set to mirror those in `gc_get_nav_menus`
	// see:
	// - https://github.com/GeChiUI/gechiui-develop/blob/ba943e113d3b31b121f77a2d30aebe14b047c69d/src/gc-includes/nav-menu.php#L613-L619.
	// - https://developer.gechiui.com/reference/classes/gc_query/#order-orderby-parameters.
	$parsed_args = array(
	...
	$navigation_posts = new GC_Query( $parsed_args );
...
function render_block_core_navigation( $attributes, $content, $block ) {
	...
	$should_load_view_script      = ! gc_script_is( 'gc-block-navigation-view' ) && ( $is_responsive_menu || $attributes['openSubmenusOnClick'] || $attributes['showSubmenuIcon'] );
	if ( $should_load_view_script ) {
		gc_enqueue_script( 'gc-block-navigation-view' );
	}
	...
	foreach ( $inner_blocks as $inner_block ) {
		if ( ( 'core/navigation-link' === $inner_block->name || 'core/home-link' === $inner_block->name || 'core/site-title' === $inner_block->name || 'core/site-logo' === $inner_block->name || 'core/navigation-submenu' === $inner_block->name ) && ! $is_list_open ) {
			$is_list_open       = true;
			$inner_blocks_html .= '<ul class="gc-block-navigation__container">';
		}
		if ( 'core/navigation-link' !== $inner_block->name && 'core/home-link' !== $inner_block->name && 'core/site-title' !== $inner_block->name && 'core/site-logo' !== $inner_block->name && 'core/navigation-submenu' !== $inner_block->name && $is_list_open ) {
			$is_list_open       = false;
		...
		if ( 'core/site-title' === $inner_block->name || 'core/site-logo' === $inner_block->name ) {
			$inner_blocks_html .= '<li class="gc-block-navigation-item">' . $inner_block->render() . '</li>';
		} else {
	...
	$responsive_container_markup = sprintf(
		'<button aria-haspopup="true" aria-label="%3$s" class="%6$s" data-micromodal-trigger="modal-%1$s"><svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true" focusable="false"><rect x="4" y="7.5" width="16" height="1.5" /><rect x="4" y="15" width="16" height="1.5" /></svg></button>
			<div class="%5$s" style="%7$s" id="modal-%1$s">
				<div class="gc-block-navigation__responsive-close" tabindex="-1" data-micromodal-close>
					<div class="gc-block-navigation__responsive-dialog" aria-label="%8$s">
							<button aria-label="%4$s" data-micromodal-close class="gc-block-navigation__responsive-container-close"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" aria-hidden="true" focusable="false"><path d="M13 11.8l6.1-6.3-1-1-6.1 6.2-6.1-6.2-1 1 6.1 6.3-6.5 6.7 1 1 6.5-6.6 6.5 6.6 1-1z"></path></svg></button>
						<div class="gc-block-navigation__responsive-container-content" id="modal-%1$s-content">
							%2$s
						</div>
					</div>
				</div>
		...
		$modal_unique_id,
		$inner_blocks_html,
		__( '打开菜单' ), // Open button label.
		__( '关闭菜单' ), // Close button label.
