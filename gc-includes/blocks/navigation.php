...
if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
	/**
	 * Returns the menu items for a GeChiUI menu location.
	 *
	...
	function block_core_navigation_get_menu_items_at_location( $location ) {
		...
		// `gc_nav_menu()` and `gutenberg_output_block_nav_menu`.
		...
		$menu = gc_get_nav_menu_object( $locations[ $location ] );
	 ...
	 * <li
	 *   class="has-child"
	 *   data-gc-context='{ "core": { "navigation": { "isMenuOpen": false, "overlay": false } } }'
	 *   data-gc-effect="effects.core.navigation.initMenu"
	 *   data-gc-on.keydown="actions.core.navigation.handleMenuKeydown"
	 *   data-gc-on.focusout="actions.core.navigation.handleMenuFocusout"
	 * >
	 *   <button
	 *     class="gc-block-navigation-submenu__toggle"
	 *     data-gc-on.click="actions.core.navigation.openMenu"
	 *     data-gc-bind.aria-expanded="context.core.navigation.isMenuOpen"
	 *   >
	 *   </button>
	 *   <span>Title</span>
	 *   <ul class="gc-block-navigation__submenu-container">
	 *     SUBMENU ITEMS
	 *   </ul>
	 * </li>
	 *
	...
	function block_core_navigation_add_directives_to_submenu( $w, $block_attributes ) {
		while ( $w->next_tag(
			...
			$w->set_attribute( 'data-gc-interactive', true );
			$w->set_attribute( 'data-gc-context', '{ "core": { "navigation": { "isMenuOpen": { "click": false, "hover": false }, "overlay": false } } }' );
			$w->set_attribute( 'data-gc-effect', 'effects.core.navigation.initMenu' );
			$w->set_attribute( 'data-gc-on--focusout', 'actions.core.navigation.handleMenuFocusout' );
			$w->set_attribute( 'data-gc-on--keydown', 'actions.core.navigation.handleMenuKeydown' );
			if ( ! isset( $block_attributes['openSubmenusOnClick'] ) || false === $block_attributes['openSubmenusOnClick'] ) {
				$w->set_attribute( 'data-gc-on--mouseenter', 'actions.core.navigation.openMenuOnHover' );
				$w->set_attribute( 'data-gc-on--mouseleave', 'actions.core.navigation.closeMenuOnHover' );
			}
			...
			if ( $w->next_tag(
				array(
					'tag_name'   => 'BUTTON',
					'class_name' => 'gc-block-navigation-submenu__toggle',
				)
			) ) {
				$w->set_attribute( 'data-gc-on--click', 'actions.core.navigation.toggleMenuOnClick' );
				$w->set_attribute( 'data-gc-bind--aria-expanded', 'selectors.core.navigation.isMenuOpen' );
			};
 ...
 *
 * @param GC_Block_List $inner_blocks Inner block instance to be normalized.
 ...
 *
 * This aims to mirror how the fallback mechanic for gc_nav_menu works.
 * See https://developer.gechiui.com/reference/functions/gc_nav_menu/#more-information.
 *
...
function block_core_navigation_get_fallback_blocks() {
	...
	if ( class_exists( 'GC_Navigation_Fallback' ) ) {
		$navigation_post = GC_Navigation_Fallback::get_fallback();
	} else {
 ...
 *
 * @param GC_Block_List $inner_blocks Block list class instance.
 *
 ...
 *
 * @param GC_Block $block Instance of a block.
 *
 * @return array Array of post IDs.
 */
 ...
 * @param GC_Block $block      The parsed block.
 *
...
function render_block_core_navigation( $attributes, $content, $block ) {
	...
	// - we don't have a relationship to a `gc_navigation` Post (via `ref`).
	...
	if (
		...
		$inner_blocks            = new GC_Block_List( $parsed_blocks, $attributes );
	}
	...
	if ( array_key_exists( 'ref', $attributes ) ) {
		...
		if ( 'publish' === $navigation_post->post_status ) {
			...
			$inner_blocks = new GC_Block_List( $compacted_blocks, $attributes );
		}
	}
	...
	if ( empty( $inner_blocks ) ) {
		...
		$inner_blocks = new GC_Block_List( $fallback_blocks, $attributes );
	}
	 ...
	 * @since 6.1.0
	 *
	 * @param \GC_Block_List $inner_blocks
	 */
	...
	$text_decoration       = _gc_array_get( $attributes, array( 'style', 'typography', 'textDecoration' ), null );
	...
	$container_attributes = get_block_wrapper_attributes(
		array(
			'class' => 'gc-block-navigation__container ' . $class,
			'style' => $style,
		)
	);
	...
	foreach ( $inner_blocks as $inner_block ) {
		...
		$p                   = new GC_HTML_Tag_Processor( $inner_block_content );
		if ( $p->next_tag(
			array(
				'name'       => 'LI',
				'class_name' => 'has-child',
		...
		if ( ! empty( $inner_block_content ) ) {
			if ( in_array( $inner_block->name, $needs_list_item_wrapper, true ) ) {
				$inner_blocks_html .= '<li class="gc-block-navigation-item">' . $inner_block_content . '</li>';
			} else {
	...
	$view_js_file            = 'gc-block-navigation-view';
	if ( ! gc_script_is( $view_js_file ) ) {
		$script_handles = $block->block_type->view_script_handles;

		// If the script is not needed, and it is still in the `view_script_handles`, remove it.
		if ( ! $should_load_view_script && in_array( $view_js_file, $script_handles, true ) ) {
			$block->block_type->view_script_handles = array_diff( $script_handles, array( $view_js_file, 'gc-block-navigation-view-2' ) );
		}
		// If the script is needed, but it was previously removed, add it again.
		if ( $should_load_view_script && ! in_array( $view_js_file, $script_handles, true ) ) {
			$block->block_type->view_script_handles = array_merge( $script_handles, array( $view_js_file, 'gc-block-navigation-view-2' ) );
		}
	}

	// Add directives to the submenu if needed.
	if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN && $has_submenus && $should_load_view_script ) {
		$w                 = new GC_HTML_Tag_Processor( $inner_blocks_html );
		$inner_blocks_html = block_core_navigation_add_directives_to_submenu( $w, $attributes );
	}

	$modal_unique_id = gc_unique_id( 'modal-' );
	...
	$responsive_container_classes = array(
		'gc-block-navigation__responsive-container',
		$is_hidden_by_default ? 'hidden-by-default' : '',
		implode( ' ', $colors['overlay_css_classes'] ),
	);
	$open_button_classes          = array(
		'gc-block-navigation__responsive-container-open',
	...
	$toggle_button_content       = $should_display_icon_label ? $toggle_button_icon : __( '菜单' );
	...
	$toggle_close_button_content = $should_display_icon_label ? $toggle_close_button_icon : __( '关闭' );
	$toggle_aria_label_open      = $should_display_icon_label ? 'aria-label="' . __( '打开菜单' ) . '"' : ''; // Open button label.
	$toggle_aria_label_close     = $should_display_icon_label ? 'aria-label="' . __( '关闭菜单' ) . '"' : ''; // Close button label.
	...
	if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN && $should_load_view_script ) {
		$nav_element_directives          = '
			data-gc-interactive
			data-gc-context=\'{ "core": { "navigation": { "isMenuOpen": { "click": false, "hover": false }, "overlay": true, "roleAttribute": "" } } }\'
		';
		$open_button_directives          = '
			data-gc-on--click="actions.core.navigation.openMenuOnClick"
			data-gc-on--keydown="actions.core.navigation.handleMenuKeydown"
		';
		$responsive_container_directives = '
			data-gc-class--has-modal-open="selectors.core.navigation.isMenuOpen"
			data-gc-class--is-menu-open="selectors.core.navigation.isMenuOpen"
			data-gc-effect="effects.core.navigation.initMenu"
			data-gc-on--keydown="actions.core.navigation.handleMenuKeydown"
			data-gc-on--focusout="actions.core.navigation.handleMenuFocusout"
			tabindex="-1"
		';
		$responsive_dialog_directives    = '
			data-gc-bind--aria-modal="selectors.core.navigation.isMenuOpen"
			data-gc-bind--role="selectors.core.navigation.roleAttribute"
			data-gc-effect="effects.core.navigation.focusFirstElement"
		';
		$close_button_directives         = '
			data-gc-on--click="actions.core.navigation.closeMenuOnClick"
		';
	}
	...
	$responsive_container_markup = sprintf(
		'<button aria-haspopup="true" %3$s class="%6$s" data-micromodal-trigger="%1$s" %11$s>%9$s</button>
			<div class="%5$s" style="%7$s" id="%1$s" %12$s>
				<div class="gc-block-navigation__responsive-close" tabindex="-1" data-micromodal-close>
					<div class="gc-block-navigation__responsive-dialog" aria-label="%8$s" %13$s>
							<button %4$s data-micromodal-close class="gc-block-navigation__responsive-container-close" %14$s>%10$s</button>
						<div class="gc-block-navigation__responsive-container-content" id="%1$s-content">
							%2$s
						</div>
					</div>
				</div>
 ...
 * @throws GC_Error An GC_Error exception parsing the block definition.
 */
 ...
 *
 * @deprecated 6.3.0 Use GC_Navigation_Fallback::parse_blocks_from_menu_items() instead.
 *
...
function block_core_navigation_parse_blocks_from_menu_items( $menu_items, $menu_items_by_parent_id ) {
	...
	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Navigation_Fallback::parse_blocks_from_menu_items' );
 ...
 *
 * @deprecated 6.3.0 Use GC_Navigation_Fallback::get_classic_menu_fallback() instead.
 *
 * @return object GC_Term The classic navigation.
 */
function block_core_navigation_get_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Navigation_Fallback::get_classic_menu_fallback' );

	$classic_nav_menus = gc_get_nav_menus();

	// If menus exist.
	if ( $classic_nav_menus && ! is_gc_error( $classic_nav_menus ) ) {
		// Handles simple use case where user has a classic menu and switches to a block theme.

		// Returns the menu assigned to location `primary`.
		$locations = get_nav_menu_locations();
		if ( isset( $locations['primary'] ) ) {
			$primary_menu = gc_get_nav_menu_object( $locations['primary'] );
			if ( $primary_menu ) {
				return $primary_menu;
			}
		}
 ...
 *
 * @deprecated 6.3.0 Use GC_Navigation_Fallback::get_classic_menu_fallback_blocks() instead.
 *
 * @param  object $classic_nav_menu GC_Term The classic navigation object to convert.
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_get_classic_menu_fallback_blocks( $classic_nav_menu ) {

	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Navigation_Fallback::get_classic_menu_fallback_blocks' );

	// BEGIN: Code that already exists in gc_nav_menu().
	$menu_items = gc_get_nav_menu_items( $classic_nav_menu->term_id, array( 'update_post_term_cache' => false ) );
	...
	// END: Code that already exists in gc_nav_menu().
 ...
 *
 * @deprecated 6.3.0 Use GC_Navigation_Fallback::create_classic_menu_fallback() instead.
 *
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_maybe_use_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Navigation_Fallback::create_classic_menu_fallback' );
	...
	$gc_insert_post_result = gc_insert_post(
		array(
	...
	if ( is_gc_error( $gc_insert_post_result ) ) {
		return;
	}
...
/**
 * Finds the most recently published `gc_navigation` Post.
 *
 * @deprecated 6.3.0 Use GC_Navigation_Fallback::get_most_recently_published_navigation() instead.
 *
 * @return GC_Post|null the first non-empty Navigation or null.
 */
function block_core_navigation_get_most_recently_published_navigation() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'GC_Navigation_Fallback::get_most_recently_published_navigation' );
	...
	$navigation_post = new GC_Query( $parsed_args );
