 ...
 *                                Passed to the `gc_initialize_site` hook.
 * }
 * @return int|GC_Error The new site's ID on success, or error object on failure.
 */
function gc_insert_site( array $data ) {
	global $gcdb;
	...
	$prepared_data = gc_prepare_site_data( $data, $defaults );
	if ( is_gc_error( $prepared_data ) ) {
	 ...
	 */
	do_action( 'gc_insert_site', $new_site );
	 ...
	 */
	do_action( 'gc_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'gcmu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// GCLANG was passed with `$meta` to the `gcmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'GCLANG', $meta ) ) {
 ...
 * @param array $data    Site data to update. See {@see gc_insert_site()} for the list of supported keys.
 * @return int|GC_Error The updated site's ID on success, or error object on failure.
 */
function gc_update_site( $site_id, array $data ) {
	global $gcdb;
	...
	$data = gc_prepare_site_data( $data, $defaults, $old_site );
	if ( is_gc_error( $data ) ) {
		return $data;
	}
	 ...
	 */
	do_action( 'gc_update_site', $new_site, $old_site );
 ...
 * @return GC_Site|GC_Error The deleted site object on success, or error object on failure.
 */
function gc_delete_site( $site_id ) {
	global $gcdb;
	 ...
	 */
	do_action( 'gc_validate_site_deletion', $errors, $old_site );
	 ...
	 */
	do_action( 'gc_uninitialize_site', $old_site );

	if ( is_site_meta_supported() ) {
		$blog_meta_ids = $gcdb->get_col( $gcdb->prepare( "SELECT meta_id FROM $gcdb->blogmeta WHERE blog_id = %d ", $old_site->id ) );
	 ...
	 * @param GC_Site $old_site Deleted site object.
	 */
	do_action( 'gc_delete_site', $old_site );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'gc_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'gc_check_site_meta_support_prefilter' );
	}
 ...
 *
 * @param string|array $args Optional. Array or string of arguments. See GC_Site_Query::__construct()
 ...
 * @param GC_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|GC_Error Site data ready for a database transaction, or GC_Error in case a validation
 *                        error occurred.
 */
function gc_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see gc_insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'gc_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( gc_parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 * @param GC_Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'gc_validate_site_data', $errors, $data, $old_site );
 ...
 */
function gc_normalize_site_data( $data ) {
...
function gc_validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		if ( empty( $data[ $date_field ] ) ) {
			$errors->add( 'site_empty_' . $date_field, __( '必须提供注册和最后更新日期。' ) );
			break;
		}
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = gc_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
				$errors->add( 'site_invalid_' . $date_field, __( '注册和最后更新日期必须为合法日期。' ) );
				break;
			}
		}
	}
 ...
 *
 * @param int|GC_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|GC_Error True on success, or error object on failure.
 */
function gc_initialize_site( $site_id, array $args = array() ) {
	 ...
	 * @param GC_Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'gc_initialize_site_args', $args, $site, $network );

	$orig_installing = gc_installing();
	if ( ! $orig_installing ) {
		gc_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'blogname'    => gc_unslash( $args['title'] ),
				'admin_email' => '',
				...
				'GCLANG'      => get_network_option( $network->id, 'GCLANG' ),
			),
			$args['options']
		)
	);
	...
	gc_install_defaults( $args['user_id'] );
 ...
 * @return true|GC_Error True on success, or error object on failure.
 */
function gc_uninitialize_site( $site_id ) {
	global $gcdb;
	 ...
	 */
	$drop_tables = apply_filters( 'gcmu_drop_tables', $tables, $site->id );

	foreach ( (array) $drop_tables as $table ) {
		$gcdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
	}
	 ...
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'gcmu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable GeChiUI.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable GeChiUI.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 */
function gc_is_site_initialized( $site_id ) {
	global $gcdb;
	 ...
	 */
	$pre = apply_filters( 'pre_gc_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	$result   = (bool) $gcdb->get_results( "DESCRIBE {$gcdb->posts}" );
 ...
 *
 * @param GC_Site|int $blog The site object or ID to be cleared from cache.
 */
 ...
 */
function gc_maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		gc_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id != $old_site->network_id ) {
		gc_maybe_update_network_site_counts( $new_site->network_id );
		gc_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 *                               state of that site. Default null.
 */
function gc_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new GC_Site( new stdClass() );
	}
