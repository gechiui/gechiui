...
function get_bookmark( $bookmark, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( empty( $bookmark ) ) {
		...
		gc_cache_add( $bookmark->link_id, $bookmark, 'bookmark' );
		$_bookmark = $bookmark;
	} else {
		if ( isset( $GLOBALS['link'] ) && ( $GLOBALS['link']->link_id == $bookmark ) ) {
			$_bookmark = & $GLOBALS['link'];
		} else {
			$_bookmark = gc_cache_get( $bookmark, 'bookmark' );
			if ( ! $_bookmark ) {
				$_bookmark = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->links WHERE link_id = %d LIMIT 1", $bookmark ) );
				if ( $_bookmark ) {
					$_bookmark->link_category = array_unique( gc_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields' => 'ids' ) ) );
					gc_cache_add( $_bookmark->link_id, $_bookmark, 'bookmark' );
				}
			}
		}
	}
...
function get_bookmarks( $args = '' ) {
	...
	$cache = gc_cache_get( 'get_bookmarks', 'bookmark' );
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$inclinks = gc_parse_id_list( $parsed_args['include'] );
		if ( count( $inclinks ) ) {
	...
	if ( ! empty( $parsed_args['exclude'] ) ) {
		$exlinks = gc_parse_id_list( $parsed_args['exclude'] );
		if ( count( $exlinks ) ) {
	...
	if ( ! empty( $parsed_args['category_name'] ) ) {
		...
		if ( $parsed_args['category'] ) {
			...
			$cache[ $key ] = array();
			gc_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in gc-includes/bookmark.php */
	...
	if ( ! empty( $parsed_args['search'] ) ) {
		$like   = '%' . $gcdb->esc_like( $parsed_args['search'] ) . '%';
		$search = $gcdb->prepare( ' AND ( (link_url LIKE %s) OR (link_name LIKE %s) OR (link_description LIKE %s) ) ', $like, $like, $like );
	}

	$category_query = '';
	$join           = '';
	if ( ! empty( $parsed_args['category'] ) ) {
		$incategories = gc_parse_id_list( $parsed_args['category'] );
	...
	if ( ! empty( $category_query ) ) {
		...
		$join            = " INNER JOIN $gcdb->term_relationships AS tr ON ($gcdb->links.link_id = tr.object_id) INNER JOIN $gcdb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query  = "SELECT * $length $recently_updated_test $get_updated FROM $gcdb->links $join WHERE 1=1 $visible $category_query";
...
function clean_bookmark_cache( $bookmark_id ) {
	gc_cache_delete( $bookmark_id, 'bookmark' );
	gc_cache_delete( 'get_bookmarks', 'bookmark' );
