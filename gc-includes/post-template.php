<?php
/**
 * GeChiUI Post Template Functions.
 *
 ...
 *     @type GC_Post $post   Current post object to retrieve the title for.
 * }
...
function get_the_content( $more_link_text = null, $strip_teaser = false, $post = null ) {
	...
	if ( preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
		if ( has_block( 'more', $content ) ) {
			...
			$content = preg_replace( '/<!-- \/?gc:more(.*?) -->/', '', $content );
		}

		$content = explode( $matches[0], $content, 2 );

		if ( ! empty( $matches[1] ) && ! empty( $more_link_text ) ) {
			$more_link_text = strip_tags( gc_kses_no_null( trim( $matches[1] ) ) );
		}
...
function get_body_class( $class = '' ) {
	...
	if ( is_search() ) {
		$classes[] = 'search';
		$classes[] = $gc_query->posts ? 'search-results' : 'search-no-results';
	}
	if ( is_paged() ) {
		$classes[] = 'paged';
	}
	...
	if ( is_singular() ) {
		$post_id   = $gc_query->get_queried_object_id();
		$post      = $gc_query->get_queried_object();
		...
		if ( is_attachment() ) {
			...
			$page_id = $gc_query->get_queried_object_id();
		...
		if ( is_post_type_archive() ) {
			...
			$author    = $gc_query->get_queried_object();
			$classes[] = 'author';
			...
			$cat       = $gc_query->get_queried_object();
			$classes[] = 'category';
			...
			$tag       = $gc_query->get_queried_object();
			$classes[] = 'tag';
			...
			$term = $gc_query->get_queried_object();
	...
	if ( has_custom_logo() ) {
		$classes[] = 'gc-custom-logo';
	}

	if ( current_theme_supports( 'responsive-embeds' ) ) {
		$classes[] = 'gc-embed-responsive';
	}

	$page = $gc_query->get( 'page' );

	if ( ! $page || $page < 2 ) {
		$page = $gc_query->get( 'paged' );
	}
 ...
 *     @type string       $nextpagelink     Link text for the next page link, if available. Default is '下一页'.
 *     @type string       $previouspagelink Link text for the previous page link, if available. Default is '上一页'.
 ...
 */
function gc_link_pages( $args = '' ) {
	global $page, $numpages, $multipage, $more;

	$defaults = array(
		'before'           => '<p class="post-nav-links">' . __( '页面：' ),
		'after'            => '</p>',
		'link_before'      => '',
		'link_after'       => '',
		'aria_current'     => 'page',
	 ...
	 *
	 *
	 * @param array $parsed_args An array of page link arguments. See gc_link_pages()
	 *                           for information on accepted arguments.
	 */
	$parsed_args = apply_filters( 'gc_link_pages_args', $parsed_args );
	...
	if ( $multipage ) {
		if ( 'number' === $parsed_args['next_or_number'] ) {
			...
			for ( $i = 1; $i <= $numpages; $i++ ) {
				 ...
				 */
				$link = apply_filters( 'gc_link_pages_link', $link, $i );
			...
			$prev    = $page - 1;
			if ( $prev > 0 ) {
				$link = _gc_link_page( $prev ) . $parsed_args['link_before'] . $parsed_args['previouspagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in gc-includes/post-template.php */
				$output .= apply_filters( 'gc_link_pages_link', $link, $prev );
			}
			$next = $page + 1;
			if ( $next <= $numpages ) {
				if ( $prev ) {
				...
				$link = _gc_link_page( $next ) . $parsed_args['link_before'] . $parsed_args['nextpagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in gc-includes/post-template.php */
				$output .= apply_filters( 'gc_link_pages_link', $link, $next );
			}
			$output .= $parsed_args['after'];
		}
	}
	 ...
	 *                             for information on accepted arguments.
	 */
	$html = apply_filters( 'gc_link_pages', $output, $args );
...
function _gc_link_page( $i ) {
	...
	if ( 1 == $i ) {
		...
		if ( ! get_option( 'permalink_structure' ) || in_array( $post->post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$url = trailingslashit( get_permalink() ) . user_trailingslashit( "$gc_rewrite->pagination_base/" . $i, 'single_paged' );
		} else {
	...
	if ( is_preview() ) {
		...
		if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
			$query_args['preview_id']    = gc_unslash( $_GET['preview_id'] );
			$query_args['preview_nonce'] = gc_unslash( $_GET['preview_nonce'] );
		}
 ...
 */
function gc_dropdown_pages( $args = '' ) {
	$defaults = array(
		'depth'                 => 0,
		'child_of'              => 0,
		'selected'              => 0,
	 ...
	 */
	$html = apply_filters( 'gc_dropdown_pages', $output, $parsed_args, $pages );
...
function gc_list_pages( $args = '' ) {
	 ...
	 */
	$parsed_args['exclude'] = implode( ',', apply_filters( 'gc_list_pages_excludes', $exclude_array ) );
	...
	if ( ! empty( $pages ) ) {
		...
		global $gc_query;
		if ( is_page() || is_attachment() || $gc_query->is_posts_page ) {
	 ...
	 * @param array     $parsed_args An array of page-listing arguments. See gc_list_pages()
	 *                               for information on accepted arguments.
	 * @param GC_Post[] $pages       Array of the page objects.
	 */
	$html = apply_filters( 'gc_list_pages', $output, $parsed_args, $pages );
 ...
 */
function gc_page_menu( $args = array() ) {
	$defaults = array(
		'sort_column'  => 'menu_order, post_title',
		'menu_id'      => '',
		'menu_class'   => 'menu',
	 ...
	 * @see gc_page_menu()
	 *
	 * @param array $args An array of page menu arguments. See gc_page_menu()
	 *                    for information on accepted arguments.
	 */
	$args = apply_filters( 'gc_page_menu_args', $args );
	...
	if ( $menu ) {
		...
		// gc_nav_menu() doesn't set before and after.
		if ( isset( $args['fallback_cb'] ) &&
			'gc_page_menu' === $args['fallback_cb'] &&
			'ul' !== $container ) {
			$args['before'] = "<ul>{$n}";
			$args['after']  = '</ul>';
		}
	 ...
	 * @param array  $args An array of arguments. See gc_page_menu()
	 *                     for information on accepted arguments.
	 */
	$menu = apply_filters( 'gc_page_menu', $menu, $args );
...
function gc_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false, $attr = '' ) {
	...
	$url = gc_get_attachment_url( $_post->ID );
	...
	if ( $text ) {
		...
		$link_text = gc_get_attachment_image( $_post->ID, $size, $icon, $attr );
	} else {
		$link_text = '';
	}
...
function gc_post_revision_title( $revision, $link = true ) {
	...
	$datef = _x( 'Y年n月j日 H:i:s', 'revision date format' );
	...
	if ( ! gc_is_post_revision( $revision ) ) {
		$date = sprintf( $currentf, $date );
	} elseif ( gc_is_post_autosave( $revision ) ) {
...
function gc_post_revision_title_expanded( $revision, $link = true ) {
	 ...
	 *                                      gc_post_revision_title_expanded().
	 */
	return apply_filters( 'gc_post_revision_title_expanded', $revision_date_author, $revision, $link );
}
...
function gc_list_post_revisions( $post_id = 0, $type = 'all' ) {
	...
	foreach ( $revisions as $revision ) {
		...
		$is_autosave = gc_is_post_autosave( $revision );
		if ( ( 'revision' === $type && $is_autosave ) || ( 'autosave' === $type && ! $is_autosave ) ) {
			continue;
		}

		$rows .= "\t<li>" . gc_post_revision_title_expanded( $revision ) . "</li>\n";
	}

	echo "<div class='hide-if-js'><p>" . __( '要使用此功能，JavaScript必须被启用。' ) . "</p></div>\n";
